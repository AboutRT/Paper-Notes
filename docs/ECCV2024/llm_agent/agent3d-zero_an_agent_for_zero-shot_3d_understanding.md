---
title: >-
  [论文解读] Agent3D-Zero: An Agent for Zero-shot 3D Understanding
description: >-
  [ECCV 2024][LLM Agent][Zero-shot 3D Understanding] Agent3D-Zero 提出一个基于 VLM 的零样本 3D 场景理解 Agent 框架，通过鸟瞰图上的 Set-of-Line 视觉提示引导 VLM 主动选择观察视角，并综合多视角图像进行 3D 推理，在 ScanQA 等任务上超越了需要微调的 3D-LLM 方法。
tags:
  - ECCV 2024
  - LLM Agent
...
**本文目标**：如何在不需要任何 3D 数据训练的前提下，利用现有 VLM 的 2D 能力完成 3D 场景理解。
...
1. **Set-of-Line Prompting (SoLP)**:
    ...
    - 功能：在鸟瞰图上叠加网格线和坐标刻度，将原始图像 $I_b$ 转换为带有空间参考系的提示图 $I_b^p$
...
2. **主动视角选择策略**:
    ...
    - 功能：让 VLM 基于当前已有的观察自主决定下一步应该从哪些角度观察场景
...
3. **3D 感知能力扩展（Tool-augmented Perception）**:
    ...
    - 功能：将 Agent3D-Zero 从纯推理任务扩展到 3D 语义分割等感知任务
