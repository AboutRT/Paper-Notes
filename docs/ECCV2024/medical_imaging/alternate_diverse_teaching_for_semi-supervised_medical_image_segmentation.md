...
description: >-
  [ECCV 2024][医学图像][半监督学习] 提出 AD-MT（Alternate Diverse Mean Teacher），通过随机周期性交替更新两个教师模型 + 基于熵的冲突调和策略，在半监督医学分割中解决 confirmation bias 问题…
tags:
  - "ECCV 2024"
  - "医学图像"
  - "半监督学习"
...
**代码**: [https://github.com/zhenzhao/AD-MT](https://github.com/zhenzhao/AD-MT)  
**领域**: 医学图像分割 / 半监督学习  
...
**本文目标** (a) 如何让两个教师模型足够不同？(b) 如何利用（而非丢弃）教师间的冲突预测？

**切入角度**：在教师更新维度做文章——互补数据批次 + 不同增强策略 + 随机切换周期三重保障差异化；冲突时比较教师集成和学生熵，选更自信的

**核心 idea**：交替更新两个教师（互补数据+不同增强+随机周期最大化差异），再用基于熵的冲突调和模块从一致/冲突预测中都学习
