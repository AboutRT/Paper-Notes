...
description: >-
  [ECCV 2024][医学图像][图像配准] 提出NePhi——用神经隐式函数表示形变场的医学图像配准框架，通过编码器预测潜码+SIREN解码器生成近似微分同胚形变，在保持配准精度的同时将训练内存降低5倍，在肺部和脑部数据集上达到SOTA配准精度与优异形变规则性。
tags:
  - ECCV 2024
  - 医学图像
  - 图像配准
...
**本文要解决什么？** 设计一种统一框架，同时在四个维度取得优势：低内存、快推理、高精度、好规则性。
...
**核心idea一句话**：用编码器预测潜码 + SIREN 解码器生成连续形变场，统一实现快速泛化推理和精细实例优化。
