...
1. **采集成本高**：视频数据庞大，下载/存储/预处理极其耗费资源
2. **版权风险**：YouTube等平台视频默认禁止下载，Kinetics等数据集存在法律隐患
3. **隐私问题**：视频常包含人脸等个人可识别信息（PII）
4. **偏见与伦理**：大规模数据集可能包含国籍、性别、年龄等偏见
5. **数据可及性**：部分数据集仅对特定研究组开放
