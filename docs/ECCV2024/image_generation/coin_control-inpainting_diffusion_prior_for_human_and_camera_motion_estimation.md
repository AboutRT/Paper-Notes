...
1. **回归方法**（如GLAMR）：忽略相机运动，或假设简单场景，泛化能力差
2. **运动先验+SLAM方法**（如SLAHMR、PACE）：使用VAE运动先验将人体运动约束在低维空间，导致过度平滑；相机优化完全依赖全局人体运动，初始人体运动不准时会灾难性失败
3. **扩散模型作为先验的天然候选**：虽然理论上编码了丰富的运动先验，但直接应用SDS会产生不一致的、过度平滑的运动
