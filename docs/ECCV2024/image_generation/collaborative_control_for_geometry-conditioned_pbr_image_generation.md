---
description: "【论文笔记】Collaborative Control for Geometry-Conditioned PBR Image Generation 论文解读 | ECCV 2024 | arXiv 2402.05919 | PBR材质生成 | 提出 Collaborative Control 范式，通过冻结预训练RGB扩散模型并训练一个并行PBR模型，利用双向跨网络通信层联合建模RGB与PBR图像分布，在有限数据下实现高质量的几何条件PBR材质图像生成。"
tags:
  - ECCV 2024
  - PBR材质生成
  - 多模态扩散
...
**现有痛点**：
1. **逆渲染的固有缺陷**：当前方法先生成RGB图像再通过逆渲染提取PBR属性，但扩散模型生成的RGB图像光照不符合物理规律（模型偏好理想化和艺术化外观），导致逆渲染结果存在严重歧义
2. **数据稀缺**：最大的PBR数据集Objaverse仅有~80万对象，比LAION-5B（50亿）小几个数量级，直接训练生成模型会导致泛化能力不足
3. **高维度困境**：PBR图像包含Albedo（3通道）、Metallic（1通道）、Roughness（1通道）和Bump Map（3通道），共8通道，无法良好压缩进现有RGB VAE的低维潜空间
4. **微调导致灾难性遗忘**：在有限PBR数据上微调预训练RGB模型会丧失泛化能力
...
1. **Collaborative Control 双向通信机制**：
   - **做什么**：在两个模型的每个self-attention模块后插入连接层，实现双向信息流
...
2. **PBR专用VAE**：
   - **做什么**：训练一个专门用于PBR图像压缩的VAE，潜空间维度设为14通道
...
3. **几何切线空间Bump Map表示**：
   - **做什么**：将bump map定义在仅依赖于几何体的切线空间中，而非传统的UV切线空间
...
4. **禁用PBR分支的文本交叉注意力**：
   - **做什么**：在PBR模型中关闭prompt交叉注意力，所有文本引导仅通过冻结的RGB模型流入
