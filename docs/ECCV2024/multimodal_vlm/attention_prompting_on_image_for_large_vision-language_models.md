...
description: >-
  [ECCV 2024][多模态][视觉提示] 本文提出Attention Prompting on Image（API），通过辅助模型（如CLIP或LLaVA）根据文本查询生成注意力热力图，将热力图叠加到原始图像上作为视觉提示输入LVLM，在不修改模型参数的情况下在MM-Vet、LLaVA-Bench等多个VL基准上稳定提升多种LVLM的性能（LLaVA-1.5提升3.8%/2.9%）。
tags:
  - ECCV 2024
  - 多模态
  - 视觉提示
...
1. **基于CLIP的归因图生成**：
    ...
    - 功能：从CLIP的视觉编码器中提取与文本查询相关的patch级别归因图
...
3. **从Token空间到像素空间的转换**：
    ...
    - 功能：将patch级别的归因图转换为像素级别的热力图并叠加到图像上
