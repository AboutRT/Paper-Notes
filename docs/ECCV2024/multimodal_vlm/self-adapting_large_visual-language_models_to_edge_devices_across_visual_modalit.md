...
1. **多模态泛化**：边缘设备通常配备RGB之外的传感器（深度、红外等），但CLIP等VLM的视觉编码器主要针对RGB图像训练，在非RGB图像上的零样本分类性能急剧下降（如ScanNet数据集上深度图像准确率仅为RGB的1/8）
2. **标签稀缺**：边缘设备持续产生大量无标注图像，人工标注成本高昂
3. **资源约束**：大型视觉编码器（如ViT）的计算需求远超边缘设备的内存和算力限制
...
2. **双模态特征蒸馏（Dual-Modality Feature Distillation）**:
   - 做什么：训练一个统一的学生编码器，能同时处理RGB和非RGB图像
