...
1. **跨模型和任务的通用性**：没有一种算法能在不同模型和任务上都有效
2. **大数据集的可扩展性**：许多方法在中等规模数据集（如ImageNet）上就已无法扩展
3. **计算正收益**：当考虑数据选择的开销时，总FLOPs是否真正节省
