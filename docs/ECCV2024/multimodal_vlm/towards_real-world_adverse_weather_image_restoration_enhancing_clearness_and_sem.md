...
**现有痛点**：
1. **域差距问题**：现有方法主要在合成数据集上训练，应用到真实场景时泛化能力严重不足。合成数据与真实天气退化之间存在显著的分布差异。
2. **语义缺失问题**：现有方法主要关注视觉清晰度的恢复，忽略了场景的语义上下文。这导致复原结果对下游高级视觉任务（如检测、分割）帮助有限。
...
1. **VLM图像评估与伪标签生成**:
   - 做什么：利用VLM的零样本能力评估复原图像的天气可见度质量，选择最优伪标签
...
2. **天气提示学习（Weather Prompt Learning）**:
   - 做什么：学习CLIP空间中的天气状态表示，引导复原模型生成更清晰的图像
...
3. **描述辅助的语义增强**:
   - 做什么：利用VLM生成天气图像的场景描述，通过改变天气描述但保持语义内容不变来提供语义正则化
