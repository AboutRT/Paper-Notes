...
1. **规划策略**：LLM应该一次生成完整计划还是逐步生成？现有工作各自使用不同策略但从未系统比较
2. **计划格式**：应该用Python代码还是JSON等结构化格式？不同格式对可执行性有何影响
3. **反馈机制**：解析反馈、规则验证反馈、执行反馈各自如何影响规划质量
...
5. **替代计划生成**:
   - 做什么：为每个查询生成多个合法替代计划，避免仅靠单一ground truth评估
