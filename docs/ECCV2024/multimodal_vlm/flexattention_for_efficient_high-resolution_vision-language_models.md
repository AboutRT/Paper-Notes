...
description: >-
  [ECCV 2024][多模态VLM][高分辨率VLM] 提出 FlexAttention，通过基于注意力图的高分辨率token动态选择和层次化自注意力融合机制，在保持甚至超越现有高分辨率VLM性能的同时，将计算成本降低近40%。
tags:
  - "ECCV 2024"
  - "多模态VLM"
  - "高分辨率VLM"
...
**代码**: [https://vis-www.cs.umass.edu/flexattention](https://vis-www.cs.umass.edu/flexattention)  
**领域**: 多模态VLM  
...
**核心 idea**：利用注意力图动态选择约10%的关键高分辨率token，通过层次化自注意力将其融入低分辨率表征，实现计算量亚线性增长下的高分辨率感知。
...
- **功能**：根据当前层的注意力图，从全部高分辨率token中选择与当前生成最相关的一小部分
- **核心思路**：
...
- **功能**：将选出的高分辨率token信息融合到原始hidden state（低分辨率token + text token）中
- **核心思路**：
...
- **功能**：高分辨率选择和层次化注意力在每个FlexAttention层中交替执行
