---
description: "提出AdaShield自适应防御提示框架，无需微调即可保护多模态大模型免受结构化越狱攻击，通过迭代优化生成场景适配的防御提示池"
tags: [MLLM-safety, jailbreak-defense, prompt-engineering, adaptive-defense, structure-based-attack]
---
...
**本文要解决什么？** 如何自动、自适应地为MLLM生成防御提示，在无需微调的前提下同时实现高防御率和低过度防御。

**切入角度** 利用LLM自身能力作为防御提示生成器，通过与目标MLLM的对话式迭代优化自动生产场景化防御提示。

**核心idea一句话** 让LLM"守护者"通过观察目标模型的越狱失败案例，迭代学习生成针对不同攻击场景的防御提示，形成可检索的防御提示池。
...
2. **自适应防御提示自动优化框架(AdaShield-A)**:
    ...
    - 做什么：自动为不同攻击场景生成定制化防御提示池，推理时自适应检索最佳提示
