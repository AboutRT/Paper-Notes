...
**本文目标**：(a) 实现从脑信号到多模态表示的统一解码；(b) 训练一个跨被试的通用模型。

**切入角度**：将脑信号与预训练图像编码器的中间特征对齐，然后利用MLLM的多任务能力实现不同粒度的解码。

**核心 idea**：脑信号对齐到图像特征空间后，可直接复用MLLM的多模态理解能力。
...
**1. 脑编码器架构**
- 功能：将不同被试的变长fMRI信号编码为统一的固定长度token序列
