...
description: >-
  [ECCV 2024][多模态][高质量caption] ShareGPT4V 构建了一个120万条高质量描述性caption数据集（由GPT4-Vision生成100K种子 + Share-Captioner扩展至1.2M），通过在预训练和SFT两阶段使用该数据集训练LLaVA架构的模型ShareGPT4V-7B，在11个多模态benchmark中9个取得最优，证明了高质量caption是LMM模态对齐的关键瓶颈。
tags:
  - ECCV 2024
  - 多模态
  - 高质量caption
...
1. **数据特定的Prompt工程**:
    ...
    - 功能：为不同数据来源设计专用prompt，引导GPT4-Vision生成高内容关联性描述
...
2. **Share-Captioner（caption模型训练与数据扩展）**:
    ...
    - 功能：用100K GPT4-Vision caption微调一个caption模型，替代昂贵的GPT4-Vision进行大规模caption生成
...
3. **预训练阶段的Vision Encoder解冻策略**:
    ...
    - 功能：在预训练阶段同时微调vision encoder、projector和LLM
...
- [\[ECCV 2024\] CLAP: Isolating Content from Style Through Contrastive Learning with Augmented Prompts](clap_isolating_content_from_style_through_contrastive_learni.md)
