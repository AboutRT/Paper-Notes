...
**现有方法的痛点**：
1. **过度依赖全局特征**：现有方法通过全局特征进行相似度比较，忽略了签名中局部笔画结构和风格细节的差异。全局表征会破坏图像结构，丢失局部判别信息。如Fig.1所示，两个不同人的签名在整体上可能非常相似，但在patch级别存在明显差异。
2. **Transformer天然抑制高频信息**：多头自注意力模块会无差别地抑制高频信号，导致显著的细节信息丢失。实验表明，传统Transformer模型主要学习低频模式，即使输入富含高频细节也无法利用，EER在高频信息增加后几乎不降低。
