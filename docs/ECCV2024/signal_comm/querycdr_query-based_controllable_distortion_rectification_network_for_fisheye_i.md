...
1. **回归式方法**：用网络预测畸变参数后重建图像，需要额外标注且非端到端
2. **生成式方法**：编码器-解码器直接生成矫正图像，端到端但泛化性差
...
1. **Distortion-aware Learnable Query Mechanism (DLQM)**：
   ...
   做什么：维护一组代表不同畸变程度的可学习查询集合，提取位置相关的控制条件。
...
3. **Controllable Attention Modulating Block (CAMB)**：
   ...
   做什么：利用Transformer的全局注意力机制捕获长程畸变映射关系。
