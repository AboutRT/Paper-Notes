...
description: >-
  [ECCV 2024][图像分割][管状结构分割] 利用分形理论将分形维数（FD）从图像级扩展到像素级，生成分形特征图（FFM）作为深度学习模型的额外输入和损失权重，并设计包含边缘解码器和骨架解码器的多解码器网络（MD-Net），在五个管状结构数据集上显著提升分割性能。
tags:
  - ECCV 2024
  - 图像分割
  - 管状结构分割
...
**代码**: 有 (https://github.com/cbmi-group/FFM-Multi-Decoder-Network)  
**领域**: 分割  
