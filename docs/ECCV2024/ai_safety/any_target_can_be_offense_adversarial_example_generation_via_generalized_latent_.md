...
## 研究背景与动机
**领域现状**：定向对抗攻击（targeted adversarial attack）目的是让分类器将输入误分类为指定目标类别。现有生成器方法（MAN、HGN、ESMA）能高效生成对抗样本，但只能攻击训练时见过的类别。
...
**核心矛盾**：想要一个"一次训练、攻击任意类别"的生成器，但现有方法将攻击绑定在固定的类别标签空间上

**本文目标** 如何训练一个类别无关的对抗攻击生成器，使其能泛化到从未见过的目标类别？

**切入角度**：将目标从"类别标签"替换为"视觉特征"——用目标图像的特征而非类别 ID 引导攻击，用余弦距离替代交叉熵损失

**核心 idea**：训练时用余弦距离拉近对抗样本和目标图像的特征（类别无关），在 UNet 生成器中通过 Feature Transform Module 将目标特征注入中间层
