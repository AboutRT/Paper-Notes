...
**本文要解决什么？** 设计一个利用SE(3)等变性的高效点云配准模型，无需显式对应关系监督。

**切入角度**: 用图卷积捕获点云拓扑和几何特征，通过SE(3)消息传播学习等变特征表示，在隐式特征空间做相似度匹配而非显式点搜索。

**核心idea一句话**: 通过SE(3)等变图网络学习旋转等变特征，配合低秩特征变换压缩和隐式相似度匹配，实现无需显式对应监督的鲁棒稀疏点云配准。
...
4. **相似度计算与秩验证 (Similarity + Rank Verification)**:
    ...
    - **做什么**: 计算源-目标帧间特征相似度矩阵，并用秩正则化和子矩阵验证去除离群对应。
