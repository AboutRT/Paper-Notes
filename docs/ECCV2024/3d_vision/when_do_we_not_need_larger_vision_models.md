---
title: >-
  [论文解读] When Do We Not Need Larger Vision Models?
description: >-
  [ECCV2024][3D视觉][多尺度特征] 提出 Scaling on Scales (S2) 策略：冻结小模型（如 ViT-B）在多个图像尺度上运行并拼接特征，无需增加参数即可在分类、分割、深度估计、MLLM 等任务上匹敌甚至超越大模型（ViT-H/G），并从理论和实验上论证了大模型学到的表征大部分可由多尺度小模型线性近似。
tags:
  - ECCV2024
  - 3D视觉
  - 多尺度特征
...
**代码**: [https://github.com/bfshi/scaling_on_scales](https://github.com/bfshi/scaling_on_scales)  
**领域**: 3D视觉  
...
1. **子图切分而非全图处理**:
    ...
    - 功能：把大尺度图像切成与预训练分辨率相同的子图，分别独立推理
...
2. **多尺度特征池化与拼接**:
    ...
    - 功能：各尺度特征图 average-pool 到原始空间大小，再沿通道维度拼接
...
3. **插值而非原始高分辨率输入**:
    ...
    - 功能：从原始 $224^2$ 图像上采样到 $448^2$/$672^2$，而非直接使用高分辨率原图
