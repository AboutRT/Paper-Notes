...
1. **领域现状**：人体姿态估计（HPE）是计算机视觉的核心任务，主流方案依赖RGB(D)相机。毫米波雷达因能穿透障碍物、保护隐私、不受光照影响等优势，正成为HPE的新兴传感器。现有毫米波HPE方法主要通过多帧聚合增强点云密度，并直接借用视觉领域的LSTM或Transformer架构。
2. **现有痛点**：(1) 毫米波雷达空间分辨率有限，点云极度稀疏（通常<100个点），几何信息不足；(2) 多径效应导致幻影点，人体部分会被漏检（miss-detection）；(3) 信号镜面反射和环境干扰导致感知数据时间不一致，引起姿态抖动和尺寸变化。
3. **核心矛盾**：视觉领域的Transformer/LSTM特征编码器是为稠密、一致的视觉数据设计的，无法有效处理毫米波雷达的稀疏性和不一致性。直接使用MLP投影PC特征到关节特征时，漏检关节的噪声会污染整体特征。
4. **本文要解决什么？** 设计专门针对噪声雷达点云的HPE方法，同时解决(1)部分人体漏检导致的特征提取不可靠和(2)信号不一致导致的姿态不稳定两大问题。
5. **切入角度**：扩散模型天然擅长去噪，可从噪声分布中恢复目标分布。漏检的关节可通过已检测关节的信息推断，扭曲的人体结构可被逐步矫正。
6. **核心idea一句话**：用条件扩散模型进行姿态生成，通过四个专用模块从噪声雷达数据中提取干净、一致的条件特征（全局/局部雷达上下文 + 肢体长度/时序运动一致性），引导扩散过程生成准确且稳定的人体姿态。
...
2. **全局雷达上下文 (GRC)**:
   - 做什么：从整体点云特征中提取关节级别的鲁棒特征，应对漏检问题
