...
**本文要解决什么？**: 为自回归场景生成寻找更好的物体排列顺序先验，使模型能按照"先主后次"的直觉空间推理原则生成场景。

**切入角度**: 将场景解析为层次化树结构（功能区→主要家具→附属物品），通过聚类算法自动挖掘隐式层次，用广度优先遍历线性化为有序序列。

**核心idea一句话**: 顺序（order）是自回归室内场景生成中被忽视的重要先验，通过场景森林结构化排列可显著提升生成质量。
...
3. **广度优先遍历（BFS Linearization）**:
    ...
    - **做什么**: 将场景树/森林转换为线性序列供Transformer处理。
