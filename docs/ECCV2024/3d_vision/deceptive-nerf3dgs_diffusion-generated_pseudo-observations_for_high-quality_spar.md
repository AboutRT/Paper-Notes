...
description: >-
  [ECCV2024][3D视觉][稀疏视图重建] 利用微调的 Stable Diffusion + ControlNet 将粗糙 NeRF/3DGS 渲染结果转化为高质量伪观测图像，将稀疏输入视图增密 5-10 倍后重新训练，在 Hypersim/LLFF/ScanNet 等数据集上超越 FreeNeRF 等方法 1-2dB PSNR，训练速度比扩散正则化方法快约 10 倍。
tags:
  - ECCV2024
  - 3D视觉
  - 稀疏视图重建
...
1. **基于对应关系的不确定性图**:
    ...
    - 功能：计算每个新视角像素的不确定性，指导扩散模型哪些区域需要更多生成、哪些区域可信
...
2. **Deceptive 扩散模型**:
    ...
    - 功能：微调 Stable Diffusion + ControlNet，以 5 通道条件（3 RGB + 1 深度 + 1 不确定性）生成逼真伪观测
...
3. **质量筛选与渐进增密**:
    ...
    - 功能：生成伪观测后，用 LPIPS 与粗渲染的相似度筛选，丢弃最差 20%
