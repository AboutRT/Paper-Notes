...
1. **领域现状**：主流人体网格重建方法采用自顶向下设计，依赖SMPL等全身参数化模型，从图像中提取全局特征后回归全身参数。当人体大部分可见时效果良好。
2. **现有痛点**：当输入图像中人体大面积被遮挡或超出视野（如只露出双腿），这类方法性能严重下降。原因有二：(a) 网络难以从少量可见部位识别人体；(b) SMPL各部位参数相互耦合，不可见部位的信息缺失会干扰可见部位的重建。
3. **核心矛盾**：自顶向下模型的参数耦合性使得局部重建必须依赖全局信息，而部分可见场景下全局信息不可靠。
4. **本文要解决什么？** 在只有少数身体部位可见时，仍能准确重建可见部位的3D网格。
5. **切入角度**：如果逐部位独立重建，就能天然避免"信息捕获困难"和"部位间干扰"两个问题。
6. **核心idea一句话**：设计独立的身体部位参数化模型（HPPM），逐部位重建后融合，实现对部分可见人体的鲁棒重建。
...
- **做什么**：为人体15个部位各自设计独立的参数化网格模型，无需部位间依赖即可重建单个部位。
- **核心思路**：
...
- **做什么**：从图像特征回归各部位的HPPM参数，独立重建每个部位网格。
- **核心思路**：
...
- **做什么**：当多个相邻部位同时可见时，将它们无缝连接为一个整体网格。
- **核心思路**：
