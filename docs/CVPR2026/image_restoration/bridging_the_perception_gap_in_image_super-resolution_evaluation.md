...
**代码**: [项目页面](https://color.cvc.uab.cat/rqi/) (有)  
**领域**: 图像超分辨率 / 图像质量评估  
...
### 整体框架
训练: 从 IQA 数据集构建稠密图像对 $\{I_i, I_j\}$ → 计算相对质量标签 $q_i - q_j$（MOS 差值） → 训练 FR-IQA 模型预测该差值。
评估: 给定 SR 输出 $I_{HR}$ 和 GT $I_{GT}$ → 模型输出 $s = f_{RQI}(I_{HR}, I_{GT})$，正值表示 SR 质量优于 GT。
