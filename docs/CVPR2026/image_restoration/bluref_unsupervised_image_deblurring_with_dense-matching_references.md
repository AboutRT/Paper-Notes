...
- **功能**：训练一个稠密匹配模型 $\mathcal{DM}$，输入模糊目标图与清晰参考图，输出变换后的图像 $I_{\text{trans}}$ 和置信度掩码 $M_{\text{conf}}$。
...
- **功能**：给定模糊图像和 $N$ 张参考清晰图，通过稠密匹配聚合多个参考的对应区域，生成一张完整的伪清晰图。
...
- **功能**：去模糊网络和伪清晰图在训练过程中交替优化、互相提升。初始化时直接用模糊图作为第一轮匹配输入。
...
- **功能**：训练完成后，丢弃稠密匹配模块和伪清晰图生成流程，仅保留去模糊网络进行推理。
