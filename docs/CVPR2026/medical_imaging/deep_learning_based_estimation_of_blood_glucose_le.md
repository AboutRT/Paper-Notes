...
description: >-
  [CVPR 2026][医学图像][无创血糖检测] 提出ScleraGluNet多视角深度学习框架，从五方向巩膜血管图像中结合多分支CNN+MRFO特征优化+Transformer跨视角融合，实现93.8%三分类精度和MAE=6.42 mg/dL空腹血糖估计。
tags:
  - CVPR 2026
  - 医学图像
  - scleral imaging
...
1. **多方向标准化采集协议**：
    - 做什么：从正前方(straight)、上方(superior)、下方(inferior)、鼻侧(nasal)、颞侧(temporal)5个注视方向采集巩膜照片
...
2. **MRFO特征精炼 + Transformer跨视角融合**：
    - 做什么：5个CNN分支输出的特征拼接后，先用蝠鲼觅食优化(MRFO)算法选择最优特征子集消除冗余，再用Transformer自注意力建模跨象限远程血管关联
...
3. **多任务学习的双头输出**：
    - 做什么：同时进行三分类（正常/受控糖尿病/高血糖糖尿病）和连续FPG值回归
