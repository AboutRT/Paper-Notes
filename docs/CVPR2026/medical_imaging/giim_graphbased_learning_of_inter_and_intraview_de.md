...
两阶段训练：(1) 对每个视图独立训练 ConvNeXt 特征提取器（利用 7×7 大核卷积和深度可分离卷积捕获形态和强度细节）；(2) 冻结特征提取器，将多病灶多视图特征构建为多异构图（MHG），用异构消息传递 GNN 进行关系推理和分类。

### 关键设计

1. **双类型节点表示**

    - **单视图节点** $N_{single}^v = f_v(l_v)$：某个视图下某病灶的特征
    - **多视图节点** $M_{multi} = \|_{v=1}^V N_{single}^v$：所有视图特征拼接的聚合节点
    - 在乳腺 X 光场景中，因 CC/MLO 视图间病灶对应关系不确定，单视图节点改为该视图所有病灶特征的均值

2. **四类边关系**

    - $E_{intra}$：同一病灶跨不同视图（如动脉期→静脉期→延迟期）→ 捕获时序增强变化
    - $E_{s-m}$：单视图节点到其多视图聚合节点 → 整合各期信息
    - $E_{inter-s}$：同一视图内不同病灶 → 建模空间共现（如 HCC 常与卫星灶共存）
    - $E_{inter-m}$：不同病灶的聚合节点间 → 高层病灶上下文关系，小病灶可借助附近大病灶的上下文

3. **异构消息传递**

    - 对每个节点分别从 single-view 邻居和 multi-view 邻居聚合（各有独立权重矩阵 $\mathbf{W}_{single}^k$、$\mathbf{W}_{multi}^k$），拼接原始状态后经非线性变换更新：$h_n^k = \sigma(\mathbf{W}^k \cdot \text{CONCAT}(h_n^{k-1}, h_{N_{single}(n)}^k, h_{M_{multi}(n)}^k))$
    - 5 层 SAGEConv（512→256→128→64→类数），最后一层直接输出分类概率

4. **缺失视图四策略**

    - **Constant**：零向量填充——简单但让模型学会忽略缺失节点
    - **Learnable**：可学习参数，通过 Frobenius 范数归一化
    - **RAG-based**：检索数据库中可用特征最相似的完整样本，借其缺失特征
    - **Covariance-based**：基于视图间特征差的协方差矩阵计算样本相似度，选最相似样本填充
