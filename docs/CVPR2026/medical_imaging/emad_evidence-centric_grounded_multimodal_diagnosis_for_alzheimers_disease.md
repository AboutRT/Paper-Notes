...
1. **黑箱问题**：多数模型只输出标签或风险评分，无法解释"为何做此判断"及"哪些证据支撑了判断"
2. **多模态整合不足**：许多方法仍在单一模态上操作，忽略跨模态依赖
3. **临床指南脱节**：现有 MLLM 生成的医学报告很少 (i) 将生成句子链接到具体临床条目，(ii) 将声明定位到 3D 脑部解剖结构，(iii) 强制遵循 NIA-AA 等诊断框架
