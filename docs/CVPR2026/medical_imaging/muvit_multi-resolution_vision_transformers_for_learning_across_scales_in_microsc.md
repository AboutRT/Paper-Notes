---
description: "【论文笔记】MuViT: Multi-Resolution Vision Transformers for Learning Across Scales in Microscopy 论文解读 | CVPR 2026 | arXiv 2602.24222 | Transformer 多分辨率、Vision Transformer、RoPE、显微镜图像、语义分割 | 提出 MuViT，一种基于世界坐标 RoPE 位置编码的多分辨率 Vision Transformer，能在单一编码器中联合处理同一场景不同物理分辨率的裁剪图，在显微镜图像分割任务上显著优于单分辨率基线。"
tags:
  - CVPR 2026
  - 多分辨率、Vision Transformer、RoPE、显微镜图像、语义分割
  - Transformer
---
...
MuViT 接收同一图像在 $L$ 个不同物理分辨率下的裁剪 $\mathbf{X} \in \mathbb{R}^{L \times C \times H \times W}$ 及其空间包围盒 $\mathcal{B} \in \mathbb{R}^{L \times 2 \times 2}$。所有分辨率的 patch 被投射到统一的世界坐标系中，通过基于 RoPE 的注意力机制在单一编码器中实现跨分辨率信息融合。记 MuViT$_{[l_1, l_2, \ldots]}$ 表示使用分辨率层级 $l_1, l_2, \ldots$ 的编码器（$l=1$ 为最高分辨率，$l>1$ 表示 $l\times$ 下采样）。

### 关键设计

1. **世界坐标 RoPE 位置编码**：每个 patch 的中心坐标映射到最高分辨率的像素坐标系（称为世界坐标），用 2D 轴向 RoPE 将坐标注入 attention：

$$\theta_k^{(a)} = \mathbf{p}_{l,i,j}^{(a)} / b^{2k/d_a}, \quad k=0,\ldots,d_a/2-1$$

其中 $b$ 为可学习参数（初始化为 10000）。这保证了表示同一空间位置的 patch 无论分辨率如何，都获得相同的位置编码，从而实现有效的跨尺度信息流。实验证明 **准确的世界坐标是不可或缺的**——将其替换为 naive 居中坐标后性能崩溃。

2. **多分辨率输入编码**：每个分辨率层级 $l$ 有独立的线性投影层 $\text{PE}_l$ 和可学习的层级嵌入 $\mathbf{e}_l$：

$$\mathbf{z}_l = \text{PE}_l(\mathbf{X}_l) + \mathbf{e}_l$$

所有层级的 token 拼接后送入 12 层 Transformer 编码器联合处理，模型参数量仅约 25M。

3. **多分辨率 MAE 预训练（MuViT-MAE）**：高掩码率 $\rho=0.75$，各层级的可见 token 比例从 Dirichlet 分布 $\text{Dir}(\alpha=0.5)$ 采样，鼓励跨尺度学习。每个分辨率层级配一个轻量解码器（2层 Transformer），通过交叉注意力访问所有可见编码输出。损失为各层级 masked patch 的 MSE 均值。

### 损失函数 / 训练策略

- **分割损失**：交叉熵 + Dice 损失，权重均为 1.0，仅在最高分辨率层级计算
...
- **训练采样**：随机坐标采样嵌套裁剪，保证粗分辨率裁剪包含细分辨率裁剪；数据存储为 Zarr 金字塔格式，按需加载
