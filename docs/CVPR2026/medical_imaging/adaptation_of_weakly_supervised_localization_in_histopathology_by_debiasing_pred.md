...
description: >-
  [CVPR 2026][医学图像][WSOL] 提出 SFDA-DeP，受机器遗忘启发，将 SFDA 重新定义为"识别并纠正预测偏差"的迭代过程：对 dominant class 中高熵不确定样本执行"遗忘"操作迫使模型放弃偏向性预测，对可靠样本保持自训练，同时用像素级分类器锚定定位能力…
tags:
  - "CVPR 2026"
  - "医学图像"
  - "WSOL"
...
- **功能**：从 dominant class 预测样本中选出最不确定的 top-$\rho$ 子集
...
- **功能**：迫使模型放弃对 forget set 样本的 dominant class 预测
...
- **功能**：训练一个轻量级像素级分类器 $h$，将每个像素分为前景（ROI）/背景
