...
description: >-
  提出即插即用的语义类分布学习框架 SCDL，通过双向分布对齐和语义锚约束解决半监督医学分割中的监督偏差和表示不平衡，在少数类上显著提升。
tags:
  - CVPR 2026
  - 医学图像
  - 半监督分割
...
1. **类分布双向对齐 (CDBA)**
    ...
    - 做什么：在嵌入空间中学习每类的代理分布，并通过双向对齐使嵌入和代理互相靠近
...
2. **语义锚约束 (SAC)**
    ...
    - 做什么：用标注数据为每个代理提供可靠的语义监督，防止代理偏离真实类语义
...
3. **代理采样特征增强**
    ...
    - 做什么：从学到的代理分布中采样，构造三种互补先验注入解码器各阶段
