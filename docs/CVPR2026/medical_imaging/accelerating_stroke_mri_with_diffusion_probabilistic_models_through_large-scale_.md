...
**做什么**：在 fastMRI 脑 MRI 数据集上预训练 DPM，涵盖 T1w（~1332 受试者）、T2w（~1340）、T1-post（~1352）三种对比度，共约 4024 名受试者。
...
**做什么**：让同一 DPM 区分不同 MRI 对比度，输出对比度特异的 score 估计。
...
**做什么**：将预训练 DPM 高效适配到目标域（中风 FLAIR 等）。
...
**做什么**：在推理时利用学到的先验和物理测量模型完成 MRI 重建。
