...
1. **现有 CIR 基准的根本缺陷**：CIRR 和 FashionIQ 等基准仅有单一正确答案、基于 Recall 的评测会忽略假阳性。例如 top-10 中返回 2 个相关+8 个干扰项，与返回 10 个完全相关结果得分相同（Recall@10 = 1.0 但 Precision@10 仅 0.20）。缺少显式负样本标注使得模型无法评估假阳性抑制能力。
2. **真实检索场景的复杂性**：用户可能使用多张参考图组合查询（如"包含[这条裙子]和[这双鞋]的穿搭"），同一语义意图可用不同措辞表达（"改成蓝色" vs "换个颜色为蓝色"），现有基准无法评测这些能力。
3. **多答案的固有性质**：一个组合查询（如"把这件衬衫换成蓝色"）可能有数十个合理匹配，假设唯一正确答案无法衡量真正的排序质量。
4. **CIRCO 的不足**：引入多正样本但缺少显式负样本，规模仅约 800-1000 查询，不够全面。
...
3. **无训练 MLLM 重排方法**
   - **做什么**：使用 Qwen2.5-VL-7B 对一阶段检索结果逐点打分重排
