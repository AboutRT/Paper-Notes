...
- **本文要解决什么**：在极端 non-IID（$\alpha=0.1$）和全局长尾分布（$\rho=20$）的挑战性设置下，设计一个能自适应选择 query 模型、显式促进类公平采样的 FAL 框架。
...
- **核心idea一句话**：类平衡采样能力（尤其是对少数类的获取）是 FAL 性能的最一致预测因子，比不确定性或多样性本身更重要。
