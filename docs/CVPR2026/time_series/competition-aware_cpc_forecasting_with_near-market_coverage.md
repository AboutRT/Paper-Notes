...
1. **领域现状**：搜索广告研究对 GSP 拍卖、排序机制和 CTR 建模已经很多，但对“单个广告主视角下如何预测未来 CPC”研究不足。
2. **现有痛点**：观测到的是结果，不是竞争原因；而 CPC 恰恰对竞争态势极其敏感。
3. **核心矛盾**：真实竞争状态不可直接观测，但它会在可观测变量中留下痕迹。
4. **本文要解决的问题**：能否用一组高质量代理信号，把隐式竞争结构显式化，再交给预测模型吸收。
5. **切入角度**：从三个互补视角近似竞争，分别是关键词语义替代性、历史 CPC 轨迹行为相似性，以及地理意图所对应的局部市场结构。
6. **核心 idea**：与其发明一个全新的预测架构，不如先把“竞争”构造成能被不同模型使用的先验和协变量，让模型在部分可观测环境下获得更稳定的中长期预测能力。
