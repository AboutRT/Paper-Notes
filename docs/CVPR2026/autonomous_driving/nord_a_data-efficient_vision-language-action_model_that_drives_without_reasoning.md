...
description: >-
  [CVPR 2026][自动驾驶][VLA模型] NoRD 证明自动驾驶 VLA 不需要大规模推理标注和海量数据：通过识别 GRPO 在弱 SFT 策略上失败的根因是 **difficulty bias**（高方差 rollout 组的学习信号被压制）…
tags:
  - "CVPR 2026"
  - "自动驾驶"
  - "VLA模型"
...
1. **K-disc 轨迹 token 化**：
    ...
    - 功能：将连续轨迹离散化为 2048 个 codebook token
...
3. **数据高效 SFT**：
    ...
    - 功能：有意用少量数据做 SFT，将主要学习负担转移到 RL 后训练阶段
