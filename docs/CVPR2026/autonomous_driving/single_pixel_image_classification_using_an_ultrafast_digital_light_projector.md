...
1. **领域现状**: 机器视觉是自动驾驶等领域的核心技术，但传统数字相机在高速场景下的带宽成为瓶颈。单像素成像（SPI）通过单点检测器+结构化光照模式可以大幅降低硬件复杂度，但受限于DMD的机械刷新速率（约 10⁴ fps）。
2. **现有痛点**: DMD 刷新速度有限导致 SPI 图像生成率仅 ~10² Hz，与普通 CMOS 相机相当；压缩感知（CS）虽能减少所需模式数但会牺牲图像质量；现有 SPI 分类工作大多停留在仿真阶段。
3. **核心矛盾**: 高速实时图像分类需要快速编码，但传统光调制器（DMD）的切换速度不足；同时需要在极有限的采样下保持足够的分类精度。
4. **本文要解决什么？** 在真实光学实验中实现 kHz 级别的单像素图像分类，跳过图像重建直接从时间序列进行分类。
5. **切入角度**: 采用 microLED-on-CMOS 数字光投影器替代 DMD，将模式切换速度提升约 100 倍，结合极简 ML 模型实现实时高速分类。
6. **核心idea一句话**: 用 microLED 阵列以 330,000 fps 投射 Hadamard 模式，通过单像素检测器采集时间序列，直接在时域进分类而无需重建图像。
