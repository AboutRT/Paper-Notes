---
description: "【论文笔记】IGASA: Integrated Geometry-Aware and Skip-Attention Modules for Enhanced Point Cloud Registration 论文解读 | CVPR2026 | arXiv 2603.12719 | 点云配准 | 提出 IGASA 框架，通过分层金字塔架构 (HPA) + 分层跨层注意力 (HCLA) + 迭代几何感知精修 (IGAR) 三级流水线，弥合多尺度特征的语义鸿沟并动态抑制离群点，在 3D(Lo)Match、KITTI、nuScenes 四大基准上全面超越 SOTA。"
tags:
  - CVPR2026
  - 点云配准
  - 几何感知
...
1. **点云配准 (PCR)** 是 3D 视觉的基础任务，直接服务于自动驾驶、机器人导航、环境建模等下游应用，但在噪声、遮挡、大尺度变换等真实场景下仍面临精度不足和鲁棒性差的问题。
2. **传统 ICP 系列方法**依赖最近邻迭代最小化，对初始化敏感、易陷入局部极小值，在大失配或稀疏数据下表现退化严重。
3. **基于 CNN 的方法**（如 FCGF、D3Feat）受限于固定感受野，难以建模长程依赖；而 **Transformer 方法**（如 GeoTransformer、RoITr）虽能捕获全局上下文，但随着网络加深，精细几何细节因激进下采样而被稀释——即所谓"语义鸿沟"问题。
4. **传统 skip connection** 多使用拼接/求和等朴素融合策略，低层几何线索与高层语义嵌入之间的分辨率不匹配无法被有效校准，关键几何细节在融合过程中被淡化。
5. **粗到精范式**中，精匹配阶段通常依赖 RANSAC 或硬阈值裁剪来剔除离群点，计算昂贵且在低重叠区域容易误杀正确对应。
