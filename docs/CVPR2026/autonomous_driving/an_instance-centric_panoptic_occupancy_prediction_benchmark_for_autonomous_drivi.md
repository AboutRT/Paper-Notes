...
1. **领域现状**：3D占据预测正从纯语义占据向细粒度的全景占据（语义+实例联合预测）发展。SparseOcc、PanoOcc等方法已提出，但受限于数据集质量。
2. **现有痛点**：(1) 现有数据集缺乏实例级标注——SparseOcc/PaSCo通过启发式（3D框分组/聚类）生成伪全景标签，引入边界伪影和实例重叠；(2) 现有地面真值基于LiDAR点云聚合+体素化，分辨率粗糙(0.2-0.5m)、几何不完整（仅传感器可见面）、物理不一致（空洞和断裂）；(3) 缺乏统一的高质量3D模型库——现有资源碎片化、平台绑定。
3. **核心矛盾**：全景占据预测需要精确的实例级几何标注，但现有数据集的生成管线（LiDAR聚合→体素化）从根本上无法提供物理一致且完整的地面真值。
4. **切入角度**：从3D mesh出发而非点云——mesh蕴含完整几何，可在任意分辨率下体素化。
5. **核心idea**：构建统一的3D模型库(ADMesh)→CARLA仿真重建完整场景mesh→拓扑感知体素化生成物理一致的全景占据标签。
...
1. **ADMesh: 统一3D模型库**:
   - 做什么：整合CARLA、BuildingNet、MeshFleet、ShapeNet四个来源的15K+高质量3D模型
...
2. **基于Mesh的场景重建**:
   - 做什么：从3D mesh直接重建每帧的全景场景mesh（非LiDAR点聚合）
   - 核心思路：
