...
description: >-
  [CVPR 2026][自动驾驶][占据预测] GPOcc 提出利用可泛化的视觉几何先验（如 VGGT、DepthAnything）进行单目 3D 占据预测，通过沿相机射线向内延伸表面点生成体积采样，以稀疏高斯基元进行概率占据推断，并设计免训练增量更新策略处理流式输入…
tags:
  - "CVPR 2026"
  - "自动驾驶"
  - "占据预测"
...
### 关键设计
1. **射线体积采样（Ray-based Volumetric Sampling）**:

    - 功能：克服几何先验仅预测可见表面的限制，生成物体内部的体积采样
...
2. **不透明度剪枝 + 稀疏高斯到占据**:
    ...
    - 功能：从稀疏高斯基元推断体素级占据，同时去除无效高斯保持效率
...
3. **免训练增量更新策略**:
    ...
    - 功能：将单帧高斯预测扩展到流式视频输入，逐帧融合为全局一致的场景表示
