...
1. **领域现状**：DiT 架构在图像/视频生成中取得卓越效果，但深层 Transformer + 多步去噪的推理开销巨大。特征缓存（Feature Caching）利用相邻时步间的时序冗余加速推理，是近年热门方向。
2. **现有方案分类**：(a) Cache-Then-Reuse（直接复用上一步特征，如 PAB、DBCache）——忽略时序演化致误差累积；(b) Cache-Then-Forecast（用 Taylor 展开预测，如 TaylorSeer、FORA）——假设特征平滑连续演化，但实际扩散过程是**阶段依赖**的。
3. **关键观察**：通过余弦相似度和 PCA 轨迹分析，扩散过程特征动态呈三阶段：高噪声阶段变化剧烈不稳定，中间阶段稳定连续，低噪声阶段精修细节。单一固定预测策略无法适应。
4. **核心idea**：用**可学习的** KAN 代替训练无关的多项式预测，并为每个噪声阶段分配**专用专家预测器**，显式建模阶段依赖动态。
