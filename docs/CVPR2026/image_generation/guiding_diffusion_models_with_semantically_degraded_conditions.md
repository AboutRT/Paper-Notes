...
2. **引导信号的几何纠缠**：$\boldsymbol{c}$ 与 $\emptyset$ 之间语义差距过大，导致引导信号在去噪主方向上产生干扰分量，混合了内容生成与风格/结构信息。
...
5. **Token 的功能二分性**：Transformer 文本编码器中的 token 自然分为**内容 token**（编码对象语义）和**上下文聚合 token**（padding/特殊 token，通过注意力吸收全局上下文），这一结构可指导退化策略设计。
