...
1. **文档水印的三难困境**：现有文档隐写方法难以同时满足鲁棒性、泛化性和隐蔽性——
...
2. **字符结构的优势**：字符结构（骨架+关键点+连接线）具有三重天然优势：(a) 对噪声干扰稳定，跨介质传输后结构基本保持不变；(b) 提供跨语言/字体的统一表征——无论何种字符都可提取骨架；(c) 修改结构附近少量像素不会改变外观，保证隐蔽性。
3. **扩散模型图像编辑的新能力**：DragDiffusion 等点对点图像编辑方法提供了精确的局部像素操控能力，为基于结构的字符水印提供了技术基础。
