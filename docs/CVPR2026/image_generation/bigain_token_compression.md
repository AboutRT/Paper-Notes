...
description: "提出BiGain框架，通过频率感知的拉普拉斯门控token合并和KV下采样，在扩散模型加速中同时保持生成质量和分类能力"
tags: ["扩散模型", "token压缩", "图像生成", "分类", "推理加速"]
---
...
1. **Laplacian-Gated Token Merging (L-GTM)**:
    ...
    - 做什么：用拉普拉斯频率分数引导 token 合并，保留高频 token 合并低频 token
...
2. **Interpolate-Extrapolate KV-Downsampling (IE-KVD)**:
    ...
    - 做什么：下采样 Key/Value 同时保持 Query 全分辨率，通过可控的插值/外推因子平衡频谱
