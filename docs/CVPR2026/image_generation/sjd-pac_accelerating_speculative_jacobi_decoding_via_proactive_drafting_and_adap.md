...
1. **领域现状**：自回归（AR）文本到图像模型（如Lumina-mGPT、Emu3）生成质量已经可以与扩散模型竞争，但推理延迟严重——需要序列化生成数千个token。Speculative Decoding（SD）是LLM推理加速的主流方法，但在T2I场景下效果不佳。

2. **现有痛点**：标准SD方法（如EAGLE）在T2I模型上几乎无加速效果，因为图像生成的高熵特性导致draft token的接受率极低——即使在标准采样温度下，图像token也有很多候选几乎等概率可行。现有SJD方法虽然是training-free和lossless的，但只能提供约2×的温和加速。

3. **核心矛盾**：作者深入分析发现SJD的接受长度分布高度偏斜——约50%的前向传播只接受1个token（即完全没有加速），平均2×的加速主要由少数成功接受大量token的步骤贡献。这种"长尾分布"是性能瓶颈的根源。

4. **本文要解决什么？**
   - 如何减少单token接受（低效步骤）的频率？
   - 如何增加每步成功验证的token数？

5. **切入角度**：单token接受的根因是"上下文失配的级联效应"——当位置i被拒绝后，后续所有proposal的上下文都无效了。对此需要两个方向的优化：（1）在拒绝点提供多样化候选减少后续级联拒绝（PD），（2）拒绝后不立即终止而是继续验证后续token（AC）。

6. **核心idea一句话**：拒绝后不停下来，而是继续验证+主动多路起草，双管齐下最大化每步接受长度。
...
1. **Adaptive Continuation (AC)**:
   - 做什么：消除标准SJD验证循环中的"first-reject break"机制
...
2. **Proactive Drafting (PD)**:
   - 做什么：在拒绝点构建多路树形proposal，减少后续迭代的级联拒绝
