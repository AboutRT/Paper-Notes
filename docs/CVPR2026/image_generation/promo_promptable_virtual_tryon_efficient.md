...
description: >-
  [CVPR 2026][图像生成][虚拟试穿] PROMO基于FLUX Flow Matching DiT骨干，通过潜空间多模态条件拼接、时序自参考KV缓存、3D-RoPE分组条件、以及fine-tuned VLM风格提示系统，在去除传统参考网络的前提下实现了高保真且高效的多件服装虚拟试穿…
tags:
  - "CVPR 2026"
  - "图像生成"
  - "虚拟试穿"
...
1. **时序自参考与3D-RoPE条件编码**:
    - 功能：实现高效推理加速，并支持单件训练、多件推理的灵活穿搭
    - 核心思路：推理时，条件token（服装图$C_i$）的Key-Value对在第一个时间步计算并缓存，后续步仅计算$z_t$和$T_{style}$的Query复用缓存KV，实现2.4倍加速（22.2s→9.2s）。注意力可见性上，$z_t$和$T_{style}$全局可见，各$C_i$仅自注意。同时利用RoPE时间维度作为条件组标识——$z_t$时间编码为0，空间条件为$i$，服装条件为$(i, x, y+\Delta)$——无参数地让模型区分"哪件衣服穿在哪里"，单次前向完成多件服装试穿，避免迭代累积误差
    - 设计动机：条件token在去噪过程中语义不变，无需每步重复计算；RoPE分组编码利用已有位置编码机制实现零额外参数的多服装泛化

2. **潜空间多模态拼接与空间条件压缩**:
    - 功能：统一编码多模态条件输入，同时大幅降低token数量
    - 核心思路：将mask后的人物图、各件服装图通过统一VAE编码到潜空间后拼接为条件token序列，不同条件按信息密度使用不同分辨率（服装和人物用原始分辨率）。pose条件直接粘贴到agnostic mask图上再整体2倍降采样，将原本2N个token压缩到N/4（87.5% token缩减）。身体解析mask还用于区域感知损失加权，身体区域权重$1+\lambda$、背景权重$1-\lambda$（$\lambda=0.5$），聚焦服装细节
    - 设计动机：避免IC-LoRA等方法要求所有拼接图像统一分辨率的限制，空间条件合并在几乎不损失信息的前提下大幅降低注意力计算开销

3. **风格提示系统与TryOff模块**:
    - 功能：扩展系统的输入灵活性——支持文本风格控制和无独立服装图的场景
    - 核心思路：用Qwen2.5-VL-72B标注少量数据，严格过滤后微调Qwen2.5-VL-7B生成结构化穿搭描述（通过Pydantic OpenAPI JSON schema引导输出），7B模型因训练数据全部合规反而比72B更准确。TryOff模块从模特图中提取服装区域，支持非配对数据训练和推理
    - 设计动机：解决PromptDresser仅支持单件服装且依赖闭源模型的问题，TryOff扩展了实际应用中无独立服装平铺图的使用场景
