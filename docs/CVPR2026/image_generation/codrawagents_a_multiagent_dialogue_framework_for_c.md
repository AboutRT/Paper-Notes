...
description: >-
  [CVPR 2026][图像生成][多智能体组合生成] 提出coDrawAgents交互式多智能体对话框架，Interpreter/Planner/Checker/Painter四智能体闭环协作，以分治策略按语义优先级增量规划布局，基于画布视觉上下文接地并显式纠错，GenEval Overall 0.94 SOTA。
tags:
  - CVPR 2026
  - 图像生成
  - 多智能体
...
**本文要解决什么？** 在复杂多对象场景中实现忠实的组合T2I生成，同时解决布局复杂度、缺乏视觉感知、早期错误无法纠正三大挑战。

**切入角度**：四智能体闭环对话式协作——分治降低复杂度 + 画布视觉接地 + 显式检查纠错。

**核心idea一句话**：让Planner看着正在生成的画面来规划下一步、让Checker回溯所有历史布局来纠错、按语义优先级分组来降低单轮复杂度。
...
1. **Interpreter + 分治策略**
    ...
    - 做什么：决定生成模式，将文本分解为属性丰富的对象描述，按语义显著性排序分组
