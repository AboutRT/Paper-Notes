...
1. **领域现状**：影视音频源分离（CASS）随 DnR 数据集的引入被形式化为语音/音效/音乐三路分离问题。BandIt 等方法推进了音频端性能，但所有现有方法都忽略了电影的多模态本质。
2. **现有痛点**：(a) CASS 方法均为纯音频，忽略了视觉线索（唇动对应语音、场景动作对应音效）；(b) 缺少同时具有源分离音轨和时间对齐视频的数据集；(c) 预测式分离模型容易产生频谱空洞伪影。
3. **核心矛盾**：视觉信息显然有助于音频分离，但获取真实电影的独立音轨几乎不可能。
4. **本文要解决什么**：在无法获取真实隔离音轨的情况下，利用可独立获取的野外音视频数据训练有效的 AV-CASS 模型。
5. **切入角度**：合成训练数据管线（面部视频→语音、场景视频→音效、纯音乐）+ 生成式流匹配分离模型。
6. **核心 idea**：训练用双视频流（面部+场景），推理时从真实电影单视频中提取双流，零样本泛化。
