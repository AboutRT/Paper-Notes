...
description: >-
  [CVPR 2026][AI安全][鲁棒水印/扩散编辑] 从理论和实验两方面系统分析扩散编辑如何非对抗性地破坏鲁棒隐形水印，推导SNR衰减和互信息下界，提出DEW-ST标准化评估协议。
tags:
  - CVPR 2026
  - AI安全
  - 鲁棒水印
...
**本文要解决什么？** 在什么条件下，非对抗性扩散编辑会无意中破坏鲁棒水印恢复？背后的理论机制是什么？

**切入角度**：将扩散编辑建模为Markov核，从SNR衰减→互信息下界→Fano不等式→去噪收缩四个层面建立闭环理论解释，并设计DEW-ST标准化评估协议。

**核心idea一句话**：扩散编辑是一个信息瓶颈，在前向加噪中指数衰减水印SNR，在反向去噪中收缩偏离流形的水印残差，导致恢复在信息论上不可能。
