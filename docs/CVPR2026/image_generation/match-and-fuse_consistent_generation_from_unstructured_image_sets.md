...
1. **领域现状**：日常视觉体验以图像集合（相册、产品目录、房产列表）为单位，但生成 AI 主要关注单图或视频，集合级一致性生成几乎未探索。
2. **核心挑战**：(a) 图像集合不像视频有时序连续性，缺乏运动线索；(b) 共享内容可能有大幅度形变；(c) 需在保持共享元素一致的同时允许非共享区域自由变化。
3. **现有方案不足**：Edicho 仅限成对编辑从单一参考传播；IC-LoRA 需微调 LoRA；FLUX Kontext 缺显式一致性机制；3D/视频编辑假设过强。
4. **关键发现**：T2I 扩散模型有**网格先验**——多图拼在画布上联合生成时自发产生一致性，但不完整且随图像数增加快速退化。
5. **核心idea**：建模图像集合为完全图，利用成对网格先验 + 稠密 2D 对应关系做特征层面的多视角融合和引导。
