...
1. **运动语义歧义**：直接从用户提示生成边界框序列容易产生断裂的运动路径和异常的尺寸变化，因为自然语言本身具有模糊性。
2. **运动引导粗糙**：现有方法对所有实例施加统一的扩散引导，无法区分静止、刚体平移、非刚体形变等不同运动类型，导致生成的运动过于同质化。
3. **LLM 辅助的局限**：LLM 生成的运动规划缺乏结构化建模来处理多实例间复杂交互关系。
