...
1. **类别不平衡（Class Imbalance）**：食物数据天然呈长尾分布，少数常见食物（如米饭、汉堡）样本量大，大量小众菜品样本稀少
2. **步骤不平衡（Step Imbalance）**：不同增量学习步骤引入的类别数量差异显著——现有方法假设每步引入相似数量的类别，但实际中某些阶段可能引入大量新菜品，某些阶段只有少量
...
1. **Balanced Softmax 训练**：
   - 做什么：在 softmax 中加入类别频率先验，平衡类间贡献
