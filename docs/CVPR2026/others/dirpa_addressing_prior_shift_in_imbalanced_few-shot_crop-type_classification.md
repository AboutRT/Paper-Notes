...
1. **训练阶段**：FSL 的 episode 训练通常构建 **人工均衡** 的支持集 (support set)，每个类别包含相同数量的样本（如 N-way K-shot）
2. **部署阶段**：真实场景中查询集 (query set) 的标签分布严重不均，与训练时的均匀先验存在显著差异
3. **后果**：这种先验偏移 (prior shift) 导致模型学到的决策边界偏向均匀分布假设，在长尾分布下泛化能力严重退化
