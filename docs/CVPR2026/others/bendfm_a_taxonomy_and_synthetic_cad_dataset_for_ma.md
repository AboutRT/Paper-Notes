...
description: >-
  [CVPR 2026][CAD/智能制造] 提出可制造性指标二维分类法和首个钣金弯曲合成CAD数据集BenDFM（20000零件），基准测试显示图结构B-rep表示在四类任务上全面优于点云方法
tags:
  - CVPR 2026
  - 可制造性评估
  - 钣金弯曲
...
**本文要解决什么？** 系统化定义可制造性概念空间 + 为钣金弯曲创建首个包含可制造/不可制造设计的合成 CAD 数据集。

**切入角度**：先建立分类法统一语言，再用参数化建模+弯折物理模拟生成标注丰富的数据集。

**核心idea一句话**：二维分类法（配置依赖 x 可行性/复杂度）+ 过程感知合成管线（弯折轨迹+碰撞检测）= 首个可支撑学习型 DFM 的钣金弯曲数据集。
...
2. **BenDFM 数据集生成管线**:
    ...
    - 做什么：生成包含可制造和不可制造零件的大规模合成钣金弯曲数据集
    - 核心思路：
...
3. **双架构基准测试**:
    ...
    - 做什么：评估两种 SOTA 3D 学习架构在四类可制造性指标上的表现
