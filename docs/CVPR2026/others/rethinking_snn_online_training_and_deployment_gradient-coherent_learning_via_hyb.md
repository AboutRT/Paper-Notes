...
1. **梯度不一致**：代理梯度函数依赖于膜电位值（如三角函数 $\frac{\partial s}{\partial m} = \frac{1}{\gamma^2}\max(\gamma - |m - \theta|, 0)$），导致时间梯度贡献权重 $\epsilon^l[i,t]$ 是膜电位的函数，不可分离。在线训练截断时间梯度后，前向和反向传播不一致
2. **推理无优势**：现有在线学习方法仅优化训练内存，得到的 SNN 在推理阶段相比 STBP 训练无额外优势
