...
description: >-
  [CVPR 2026][多模态基准][计数能力评估] 首个统一跨模态（图像/文本/音频）多层级计数基准，含14,301个QA对+三级能力/难度分类，系统评估45个SOTA MLLM揭示推理和困难任务上的显著不足。
tags:
  - CVPR 2026
  - 多模态基准
  - 计数能力
...
**本文要解决什么？** 建立一个覆盖图像/文本/音频三模态、有统一QA格式和评估协议、并能分层诊断能力短板的计数基准。

**切入角度**：设计三级能力分类(Pattern/Semantic/Reasoning)和三级难度分类(Easy/Medium/Hard)的交叉分类体系，配合evidence-first GT和确定性数字解析。

**核心idea一句话**：将计数能力分解为感知计数→语义过滤→规则推理三个层次，跨图像/文本/音频统一评测，用MAE/HitRate等指标分层诊断MLLM的计数瓶颈。
