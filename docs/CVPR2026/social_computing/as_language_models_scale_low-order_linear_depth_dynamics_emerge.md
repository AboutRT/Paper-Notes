...
1. **机制理解**：如果不能描述扰动如何沿深度传播，就只能停留在“某个方向有用”的静态表征层面。
2. **干预设计**：如果能预测每层的增益曲线，就不需要 brute-force layer sweep。
3. **缩放规律**：模型越来越大通常意味着更复杂，但作者怀疑更大模型在局部反而更“规整”，因此可能更适合被低维动力学代理压缩。
...
1. **把深度当作时间，把最后 token 当作系统状态**
   - 做什么：定义 $x_\ell(p)=h_\ell(p)[t(p),:]$，其中 $t(p)$ 是最后一个非 padding token 的位置。
...
2. **冻结上下文，只建模局部 last-token 响应**
   - 做什么：对某个固定 prompt，保持非最后 token 的隐藏状态不变，只扰动最后 token，然后看下一层 block 如何映射它。
...
3. **沿概念方向建模外部驱动项**
   - 做什么：在每一层估计概念方向 $v_\ell$，主实验只考虑沿这个方向的加性注入 $u_\ell v_\ell$。
...
4. **构造 concept-anchored 的低维基底 $P_\ell$**
   - 做什么：每层的降维基底第一列固定为概念方向 $v_\ell$，其余列由 reachability-informed Krylov complement 构造。
...
6. **用 predicted gain 连接分析与控制**
   - 做什么：对单层第 $k$ 层注入，最终概念增益预测为 $g_k^{pred}\approx C\Phi(k+1,L)\bar{B}_k$。
