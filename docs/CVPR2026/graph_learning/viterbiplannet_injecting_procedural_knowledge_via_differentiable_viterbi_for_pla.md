---
description: 提出可微Viterbi层将过程知识图端到端集成到视频过程规划中，模型只需学习发射概率，参数量比扩散/LLM规划器少1-2个数量级且SOTA
tags:
  - CVPR2026
  - 视频理解
  - 过程规划
...
**本文要解决什么？**：如何将过程知识图从推理后处理上升为训练时的引导信号，让结构化先验直接参与端到端优化。
...
**核心idea一句话**：不让网络记忆过程，只让它学习"当前视觉状态与各动作的匹配度"（发射概率），过程的合理顺序由知识图保证——可微Viterbi层使得这种分工可以端到端训练。
...
1. **可微Viterbi层（DVL）**:
    ...
    - 做什么：将标准Viterbi解码算法转化为可微操作，使端到端训练成为可能
