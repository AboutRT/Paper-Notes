...
1. **RGB 类方法**（SiamSNN, Spike-SiamFC++）：虽然使用脉冲神经元，但将脉冲信号解码为连续值进行计算，未实现完全脉冲驱动处理，能效受限
2. **事件相机类方法**：直接模仿 ANN 的 one-stream 架构（如 OSTrack），将模板和搜索区域拼接后送入骨干做双向交互。这种方式存在两个缺陷：
...
1. **非对称 Siamese 骨干**：不对称的时间步输入与单向信息流
   - 做什么：模板分支在多个时间步 $T$ 上扩展（每步分配一个模板），通过神经元时空动态联合建模模板表征；搜索分支仅做单时间步高效推理
...
2. **Memory Retrieval Module（MRM）**：脑启发的记忆检索单向信息传递
   - 做什么：从模板分支缓存的 memory 中检索目标线索，增强搜索分支的目标感知
