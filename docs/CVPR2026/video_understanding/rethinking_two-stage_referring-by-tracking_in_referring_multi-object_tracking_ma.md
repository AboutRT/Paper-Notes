...
description: >-
  [CVPR 2026][视频理解][指代多目标跟踪] 提出 FlexHook，一种新颖的两阶段 Referring-by-Tracking 框架，通过基于采样的 Conditioning Hook（C-Hook）重新定义特征构建…
tags:
  - "CVPR 2026"
  - "视频理解"
  - "指代多目标跟踪"
...
2. **Temporal Integration**：利用坐标网格差分提取显式光流信息
    ...
    - 功能：将帧间网格位移 $\Delta Grid = \text{Cat}(\{P^i_{t+k} - P^i_{t+k-1}\}_{k=1}^p)$ 与多帧特征 $J$ 沿通道维度拼接，经 MLP 压缩得到轨迹特征 $F_J \in \mathbb{R}^{h \times w \times C}$
...
3. **Pairwise Correspondence Decoder（PCD）**：替换 CLIP 余弦相似度的可学习对应解码器
    ...
    - 功能：将 $\hat{N}$ 个采样表达与共享轨迹片段组成 $\hat{N}$ 个样本对，使用可学习查询 $Q \in \mathbb{R}^{\hat{N} \times C}$ 提取每对的匹配分数
