---
description: "【论文笔记】Dual-Agent Reinforcement Learning for Adaptive and Cost-Aware Visual-Inertial Odometry 论文解读 | CVPR2026 | arXiv 2511.21083 | Visual-Inertial Odometry | 提出双智能体强化学习框架，通过 Select Agent（基于IMU信号决定是否启动视觉前端）和 Fusion Agent（自适应融合视觉-惯性状态）两个轻量RL策略，在不完全移除VIBA的前提下大幅降低其调用频率和计算开销，实现精度-效率-显存的更优折中。"
tags:
  - CVPR2026
...
1. **VIO的核心矛盾**：滤波方法（MSCKF、ROVIO）高效但漂移严重；优化方法（VINS-Mono、ORB-SLAM3）精度高但VIBA计算量大，难以部署在资源受限的边缘设备上。
2. **端到端深度学习方法局限**：VINet、SelfVIO等直接回归位姿，精度和泛化性仍不及成熟优化框架。
3. **混合方法未解决根本瓶颈**：iSLAM、DPVO等引入学习模块增强特征匹配或优化，但仍受限于VIBA的计算瓶颈。
4. **现有关键帧选择策略低效**：传统策略需要先处理图像才能判断帧是否有用，无法在视觉计算之前做出跳过决策。
5. **固定融合权重不够灵活**：静态的EKF或固定增益融合无法根据运动强度和传感器可靠性动态调整视觉/惯性的信任程度。
6. **RL在里程计中的应用尚浅**：已有工作（Messikommer et al.）仅用RL优化VO内部启发式，未从"是否启动整个VO流水线"的高层调度角度切入。
