...
现有稳定性评估存在三个结构性缺陷：
1. **不强制预测不变性**：在计算稳定性时未检查扰动是否改变了模型预测类别。Lipschitz 连续性、max-sensitivity 等指标在扰动后预测类别改变时仍然比较归因，混淆了"模型敏感性"与"解释脆弱性"
2. **单标量度量**：将稳定性压缩为单一数值，无法区分空间位移、排序变化或显著区域不一致等不同失败模式
3. **仅评估加性噪声**：现有框架主要在 ε-ball 加性噪声下测试，忽略了实际系统中常见的几何变换、光度变化和压缩伪影
