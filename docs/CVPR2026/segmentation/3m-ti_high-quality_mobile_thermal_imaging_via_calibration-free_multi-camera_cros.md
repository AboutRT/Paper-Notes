...
1. **移动热成像的硬件瓶颈**：移动平台热传感器因小型化导致孔径缩小、像素尺寸受限，输出图像模糊且信息不足（典型分辨率仅 96×96）。
2. **单图超分的信息不足**：单幅热图像缺少足够的高频信息来恢复精细结构，尤其在大放大倍数下效果欠佳。
3. **RGB 引导方法依赖标定**：现有 RGB 引导的热图像 SR 方法需要精确的像素级跨相机标定，实际部署中标定过程繁琐且缺乏鲁棒性。
4. **跨模态域差异大**：RGB 与热红外成像原理根本不同，直接合并特征容易引入不真实的纹理细节。
5. **热红外数据集规模有限**：相比 RGB 领域，热红外数据集规模小、场景多样性不足，限制了网络训练和泛化。
6. **实际场景中的时空错位**：多相机系统在实际使用中不可避免地存在视差和时间不同步问题，现有方法对此缺乏鲁棒性。
