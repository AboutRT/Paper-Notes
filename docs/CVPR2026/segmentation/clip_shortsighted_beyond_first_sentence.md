...
1. **去除摘要句（Replacing the Summary Sentence）**:
    ...
    - 做什么：将短caption定义为长caption去掉首句后的内容 $C^{\text{no\_sum}} = [s_2, \dots, s_k]$
...
2. **句子子采样（Sentence Sampling）**:
    ...
    - 做什么：从去除首句后的caption中随机采样若干句子构成新的短caption $C^{\text{samp}} = [s_4, s_2]$
...
3. **前缀Token填充（Token Padding）**:
    ...
    - 做什么：将部分后续padding token移动到caption前面作为前缀padding $T^s_{\text{ours}} = [\mathtt{SOT}, \mathtt{PAD}_{\text{pre}}, s_4, s_2, \mathtt{EOT}, \mathtt{PAD}_{\text{post}}]$
...
1. **去除摘要句（Replacing the Summary Sentence）**:
    ...
    - 做什么：训练时将短 caption 定义为去除首句后的剩余内容 $C^{\mathrm{no\_sum}} = [s_2, \ldots, s_k]$
...
2. **句子随机采样（Sentence Sampling）**:
    ...
    - 做什么：从 $C^{\mathrm{no\_sum}}$ 中随机无放回采样 $n_{\mathrm{sampled}} = \mathcal{U}\{1, 2, \ldots, n_{\mathrm{sents}}-1\}$ 个句子，不保持原始顺序
...
3. **Token 前缀填充（Token Padding）**:
    ...
    - 做什么：将 token 序列末尾的 padding token 部分移到开头（SOT 之后），推迟有信息 token 的起始位置
