...
description: >-
  [CVPR 2026][图像分割][机器人分割] 本文提出 RobotSeg，第一个同时支持图像和视频的机器人分割基础模型，基于 SAM 2 引入结构增强记忆关联器（SEMA）、机器人提示生成器（RPG）和标签高效训练策略，仅需首帧标注即可训练，在自动模式下 Whole Robot 分割达到 85.1 J&F，比 SAM 2.1 微调版高 4.9 分，同时参数仅 41.3M（远小于现有 638M+ 方案）。
tags:
  - CVPR 2026
  - 图像分割
  - 机器人分割
...
1. **结构增强记忆关联器（SEMA）**:
    ...
    - 功能：将历史帧信息和结构先验注入当前帧特征以实现时序一致的关节机器人分割
    - 核心思路：双分支设计。**上分支**（时序关联）：将历史帧特征和掩码编码为记忆 $M_t$，当前帧特征 $F_t$ 依次通过自注意力、以 $M_t$ 为 KV 的交叉注意力和 MLP，得到时序增强特征 $F_t'$。**下分支**（结构增强）：用 Canny 滤波器提取当前帧边缘图 $E_t$，通过 $F_t^{edge} = F_t \odot (1 + E_t)$ 进行边缘调制，再过多尺度特征提取器和以 $M_t$ 为 KV 的交叉注意力得到结构图 $S_t = \sigma(\text{CrossAttn}(F_t^{ms}, M_t))$，最后用 $F_t'' = F_t' \odot (1 + \alpha S_t)$ 融合两个分支（$\alpha$ 为可学习权重）。
    - 设计动机：机器人的关节连接处是分割的难点——边缘信息提供了强先验，Canny 是零成本的结构探测器，多尺度感知能同时关注粗关节和细末端

2. **机器人提示生成器（RPG）**:

    - 功能：无需手动输入（点击/框）即可自动生成分割提示
    - 核心思路：生成两类 robot token——**class token** 从可学习 token bank 按目标类别（机器臂/夹爪/整体）选取，提供类级语义先验；**object token** 从历史记忆中通过分层聚类提取——先用 Farthest Point Sampling 初始化 $R$ 个宏观区域中心，K-Means 聚类得到区域掩码，再在每个区域内进一步分 $S$ 个微观子簇，拼接所有原型向量作为 object token。两类 token 共同输入 mask decoder 引导分割。
    - 设计动机：class token 提供"我要分割机器人"的语义先验，object token 提供"上一帧机器人长什么样"的外观线索。分层聚类（宏观-微观）既捕捉粗轮廓又保留末端细节。

3. **标签高效训练策略（Label-Efficient Training）**:

    - 功能：仅需首帧 GT 掩码即可训练视频分割模型
    - 核心思路：三级一致性损失。(a) **循环一致性** $\mathcal{L}_{cyc}$：前向传播从帧 0 到帧 $t$，再反向传播回帧 0，首帧两次预测掩码均与首帧 GT 用 focal+dice loss 监督。(b) **语义一致性** $\mathcal{L}_{sem}$：中间帧预测掩码内的平均特征应与首帧物体语义一致（余弦相似度最大化），防止模型学到"不变掩码"的捷径。(c) **Patch 一致性** $\mathcal{L}_{patch}$：利用 DINOv3 的 patch 相似度从首帧 GT 传播到中间帧生成伪标签，用 IoU loss 监督（掩码下采样 16x 匹配 patch 粒度）。总损失 $\mathcal{L}_{mask} = w_{cyc}\mathcal{L}_{cyc} + w_{sem}\mathcal{L}_{sem} + w_{patch}\mathcal{L}_{patch}$。
    - 设计动机：循环一致性利用唯一可用的首帧标注实现自监督，语义一致性约束中间帧不偏离目标语义，patch 一致性提供像素级伪监督——三者分别覆盖视频级、物体级、patch 级，形成完整的层次监督
