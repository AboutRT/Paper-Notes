...
1. **领域现状**: 当前视觉-语言模型（VLM）在场景理解和定性描述方面表现优异，但在定量空间推理（如计数、面积测量、距离计算）上表现很差，卫星图像领域尤为严重。
2. **现有痛点**: VLM 通过 vision encoder 将 1024×1024 图像压缩为 64×64 的 token 网格（256 倍压缩），这一过程从架构层面摧毁了定量分析所需的像素级空间索引。研究显示 vision encoder 导致了 40-60% 的 k-近邻散度。
3. **核心矛盾**: VLM 能"口若悬河"地描述一片森林，却无法可靠地数出其中的树木——模型的定性理解与定量分析之间存在根本性断裂。
4. **本文要解决什么？** 在卫星影像中，为气候监测、城市规划、灾害响应等需要精确数量化分析的应用提供可靠解决方案。
5. **切入角度**: 架构解耦——让语言模型只负责理解问题和生成代码，视觉分析完全交给分割模型在原始像素上操作。
6. **核心idea一句话**: 通过代码生成将语言理解与视觉分析解耦，使模型在像素分割掩码上直接执行几何运算，从而保持全程空间索引不压缩。
