...
description: "提出Omnivorous Vision Encoder，通过跨模态对齐损失和锚定损失在冻结DINOv2上训练轻量适配器，学习模态无关的特征空间，支持RGB/深度/分割图等多模态统一表示"
tags: ["DINOv2", "跨模态对齐", "视觉编码器", "模态不变性", "参数高效微调"]
---
...
1. **对称跨模态InfoNCE对齐（Symmetric Cross-Modal Alignment）**:
    ...
    - 做什么：在student输出空间中最大化同一场景不同模态的特征相似度
...
2. **锚定蒸馏损失（Anchoring Loss）**:
    ...
    - 做什么：约束student输出 $h_m$ 接近teacher输出 $h^*_m$，防止特征漂移或坍塌
...
3. **数据增强：自然色彩调色板 + 模态混合（Colorization + Modality Mixup）**:
    ...
    - 做什么：用对应RGB图片的颜色量化值给深度图和分割图着色，然后在模态之间进行alpha混合
