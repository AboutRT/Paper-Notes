...
**本文要解决什么？** 根据目标在图像中占据的面积比例，自适应选择合适的特征提取策略，同时高效进行多层特征交互。

**切入角度**：先在高层特征中估计目标区域比例，再以此引导低层特征的卷积核动态选择；中层特征交互用小波域分频做降维。

**核心idea一句话**：知道目标大概多大，再决定怎么看——区域比例引导的动态卷积核选择。
...
2. **DAD（动态自适应细节感知模块）**
    ...
    - 做什么：根据 PG 输出的区域比例，动态选择不同数量和大小的卷积核提取目标细节
...
3. **FCE（频率匹配上下文增强模块）**
    ...
    - 做什么：在中层特征间做高效跨层交互，避免全分辨率自注意力的高计算量和高低频混合问题
