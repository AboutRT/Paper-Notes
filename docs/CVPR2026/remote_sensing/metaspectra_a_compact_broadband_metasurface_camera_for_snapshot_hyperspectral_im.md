...
description: >-
  [CVPR 2026][遥感][超表面成像] MetaSpectra+ 提出超表面-折射透镜混合光学新范式，通过双层超表面实现色散独立可控，将多功能超表面带宽从10-100nm拓展到250nm，在17mm总光程下实现快照式高光谱+HDR/偏振多功能成像，KAUST基准PSNR达33.31dB。
tags:
  - CVPR 2026
  - 遥感
  - 超表面成像
...
**本文要解决什么？** (1) 如何将多功能超表面的工作带宽从几十nm拓展到250nm以覆盖整个可见光谱？(2) 如何在同一系统中独立控制每个通道的色散量——部分通道有可控色散用于光谱编码，部分通道消色差用于HDR/偏振？(3) 如何在保持紧凑性的同时降低F数？
...
**核心idea一句话**：用双层超表面分别负责分束和色散控制，加折射透镜负责成像，通过偏转向量的可加性实现各通道色散独立可调，从而在紧凑形态下实现宽带多功能高光谱成像。
...
1. **分束超表面 M0（随机交错多通道光束分裂）**:
    ...
    - 做什么：将入射准直光分裂、偏转到4个约33°偏角的独立光学通道
...
3. **多功能成像配置（零成本模态扩展）**:
    ...
    - 做什么：在消色差通道插入不同滤波器，零额外光学复杂度地扩展HDR或偏振成像能力
