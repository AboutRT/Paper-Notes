...
**本文要解决什么？** 显式利用几何结构作为跨模态关联的"物理锚点"，严格约束结构一致性同时容忍模态特定外观变化。

**切入角度**：从中间层特征提取梯度能量——既足够抽象以过滤低级噪声（如SAR散斑），又保留空间拓扑信息；在终端层解耦共享/专用特征并通过加法残差融合保持判别力。

**核心idea一句话**：用中间层梯度能量统计量作为尺度不变的结构描述子强制跨模态几何一致性，同时在终端层解耦并融合模态不变/模态专用特征。
...
1. **结构感知一致性学习(SCL)**:
    - **做什么**: 从ViT中间层(Block 6)提取跨模态几何不变的结构特征
...
2. **解耦特征学习与加法融合(DFL)**:
    - **做什么**: 将终端表示分解为模态不变共享特征和模态专用特征，以无参数方式融合
