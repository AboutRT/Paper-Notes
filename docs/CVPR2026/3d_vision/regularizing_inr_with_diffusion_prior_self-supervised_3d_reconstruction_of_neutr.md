...
1. **INR 后验均值估计器 (INR as DIS)**：
    - 功能：替代 DD3IP 中的 CG 求解器，作为扩散反向过程中后验均值的估计方法
    - 核心思路：INR 模型 $F_\phi$ 使用 SIREN 架构，将 3D 坐标网格 $S$ 映射为衰减系数值，同时接受 FBP 重建 $A^*y$ 作为辅助输入以加速收敛
    - 设计动机：DD3IP 证明了后验采样与 DIS 选择无关，INR 的连续表示天然支持分辨率无关重建，且其可微分特性便于与 CT 前向模型及扩散先验联合优化

2. **近端损失函数 (Proximal Loss)**：
    - 功能：在 INR 优化中同时施加数据保真约束和扩散先验约束
    - 核心思路：损失函数包含两项——数据保真项 $\text{MSE}(AF_\phi(S, A^*y), y)$ 确保投影一致性，近端项 $\rho \cdot \text{MSE}(\hat{x}_t, F_\phi(S, A^*y))$ 将 INR 输出拉向扩散去噪估计 $\hat{x}_t$
    - 设计动机：近端项为 INR 引入了扩散模型学到的图像先验，弥补了纯 INR 缺乏强先验的缺陷；参数 $\rho$ 控制先验影响强度，初始化阶段设 $\rho=0$ 仅做数据拟合

3. **噪声缩放初始化 (Noise Scaling)**：
    - 功能：控制扩散反向过程起点处信号与噪声的相对比例
    - 核心思路：$x_T = \sqrt{\alpha_T} A^*y + \sqrt{1-\alpha_T} \epsilon \cdot \omega$，引入可调参数 $\omega > 0$ 缩放注入噪声
    - 设计动机：$\omega$ 控制 FBP 低频成分相对于噪声的比例，间接调节 DD3IP 框架的正则化强度，不同稀疏度需要不同 $\omega$ 以获得最优重建
