...
description: >-
    [CVPR 2026][3D视觉] 提出GGPT框架，通过改进SfM管线获取稀疏几何引导，再用3D Point Transformer在三维空间直接融合refinement，实现跨架构跨数据集的显著泛化提升
tags:
    - CVPR 2026
    - 3D视觉
    - 3D重建
...
**本文要解决什么？** 在3D空间中将SfM的几何精度与前馈网络的稠密完整性有机结合，实现无需微调就能跨架构泛化的稀疏视角3D重建refinement。

**切入角度**：两阶段——先用改进SfM从输入RGB获取真实稀疏几何，再用3D Point Transformer在点云空间直接做注意力和残差修正。

**核心idea一句话**：在3D空间而非2D图像空间做几何融合refinement是跨域泛化的关键。
