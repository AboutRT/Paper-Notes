...
## 研究背景与动机
1. **领域现状**：场景级 3D 重建是计算机视觉的核心问题。优化式方法（3DGS、NeRF）质量高但需要逐场景迭代优化，耗时长。前馈 3DGS 方法通过单次前向推断实现跨场景泛化。
2. **现有痛点**：
...
3. **核心矛盾**：如何在前馈效率下实现几何一致、高保真的 3D 重建？
4. **本文切入角度**：从 3D 锚点出发而非 2D 像素——利用 MVS 预测的深度和位姿构建稀疏 3D 锚点，在锚点上预测高斯。
5. **核心 idea**：锚点对齐的高斯表示 + Gaussian Refiner 迭代精炼 = 更少高斯、更好质量、与输入分辨率/视图数无关。
...
3. **Gaussian Refiner（高斯精炼器）**：
   - **做什么**：通过渲染误差反馈精炼高斯属性，类似于可微渲染的一步"优化"。
   - **核心思路**：
