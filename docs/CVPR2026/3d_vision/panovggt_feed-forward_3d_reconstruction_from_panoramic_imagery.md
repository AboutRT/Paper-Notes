...
## 研究背景与动机
1. **领域现状**：DUSt3R、VGGT、$\pi^3$ 等前馈式3D重建模型在透视图像上取得了巨大成功，可以在单次前馈中联合推断深度、位姿和3D结构
2. **现有痛点**：
   - 这些模型本质上基于针孔投影假设，直接处理等距柱形全景图时会出现接缝、视差不一致和几何漂移
   - 将全景图分割为多个透视裁剪再拼接的做法会引入伪影
   - 现有全景数据集存在规模不足、标注不完整、视点重叠不够等问题
3. **核心矛盾**：全景图具有非针孔畸变和球面几何特性，现有前馈模型的位置编码、数据增强和几何推理都不适用
4. **本文要解决什么**：(a) 将前馈3D重建范式扩展到全景图像域；(b) 构建足够大规模的全景数据集支持训练
5. **切入角度**：通过球面感知位置编码和 $SO(3)$ 旋转增强，使 Transformer 学会在球面域进行有效几何推理
6. **核心idea一句话**：用球面位置编码 + 三轴旋转增强 + 随机锚点策略，将 VGGT/$\pi^3$ 的前馈重建范式扩展到全景图像
