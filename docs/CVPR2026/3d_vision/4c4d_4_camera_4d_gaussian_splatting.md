---
description: "【论文笔记】4C4D: 4 Camera 4D Gaussian Splatting 论文解读 | CVPR 2026 | arXiv 2604.04063 | 4D高斯溅射 | 提出 4C4D 框架，通过神经衰减函数（Neural Decaying Function）自适应控制高斯不透明度衰减，解决稀疏（仅4个相机）4D高斯溅射中几何与外观学习的不平衡问题，在多个数据集上达到SOTA。"
tags:
  - CVPR 2026
  - 4D高斯溅射
  - 稀疏视角
...
## 研究背景与动机
1. **领域现状**：4D动态场景的新视角合成需要密集相机阵列（数十到数百台），限制了日常使用。3DGS/4DGS在密集视角下表现出色。
2. **现有痛点**：极稀疏视角（如4台相机）下，4DGS严重失败。原因在于**优化偏差**：拟合外观（颜色）相对容易，但恢复准确几何（深度）在监督不足时极其困难。当前高斯公式无法平衡两者。
3. **核心矛盾**：稀疏视角下空间监督不足 → 几何学习不足 → 过拟合训练视点外观 → 新视点严重伪影。
4. **关键观察**：4DGS在训练视点上能准确复现外观，但深度几何一塌糊涂（见Fig.3），说明问题不在模型容量而在优化偏差。
5. **核心idea**：引入可学习的不透明度衰减函数，将优化梯度重新导向几何学习。
