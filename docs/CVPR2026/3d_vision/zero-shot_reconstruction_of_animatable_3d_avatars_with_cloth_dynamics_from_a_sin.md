---
description: "【论文笔记】Zero-Shot Reconstruction of Animatable 3D Avatars with Cloth Dynamics from a Single Image 论文解读 | CVPR2026 | arXiv 2603.14772 | 3D人体重建 | DynaAvatar 提出首个零样本框架，从单张图像重建具有运动依赖布料动态效果的可动画化3D人体Avatar，核心通过静态-动态知识迁移策略和光流引导的 DynaFlow 损失函数，在有限动态数据下实现了逼真的衣物动态建模，全面超越现有方法。"
tags:
  - CVPR2026
  - 3D人体重建
  - 可动画化Avatar
...
1. **领域现状**：单图可动画化3D人体Avatar重建是计算机视觉和图形学的核心目标。现有零样本方法（IDOL、LHM）主要依赖基于骨骼关节的刚性变换（LBS）来驱动动画，虽然能实现身体关节运动，但本质上无法建模非刚性布料动态。另一类个性化方法（ExAvatar、GaussianAvatar）虽能捕捉特定对象的衣物形变，但需要针对每人单独采集多视角视频并优化，无法推广到任意新人物。
...
3. **核心矛盾**：要学习运动依赖的布料动态需要大规模动态捕捉数据，但这类数据极其稀缺；同时传统图像重建损失在大幅布料形变场景下因局部感受野和颜色-几何耦合问题而监督失效。
...
5. **切入角度**：作者观察到大规模静态采集数据虽然缺乏时序形变信息，但包含丰富的人体几何和外观先验；同时利用光流可以建立渲染图像和真实图像之间跨越大形变的像素级对应关系，从而提供纯几何的位移监督。

6. **核心idea一句话**：通过静态预训练Transformer的LoRA微调实现知识迁移+光流引导的DynaFlow损失提供几何级形变监督，让单图Avatar在零样本下展现逼真的运动依赖布料动态。
...
2. **运动编码器（Motion Encoder）**:
   - **做什么**：将过去 1 秒（15帧）的运动历史编码为运动 token $\mathbf{T}_M$
...
3. **Dynamic Transformer（动态Transformer）**:
   - **做什么**：将运动信息注入静态特征，让Avatar展现运动依赖的布料动态
...
4. **静态-动态知识迁移（Static-to-Dynamic Knowledge Transfer）**:
   - **做什么**：利用大规模静态数据的预训练知识加速和提升动态形变学习
...
5. **Gaussian 解码器与动画渲染**:
   - **做什么**：将 Dynamic Transformer 输出转换为 3DGS 表示并进行动画和渲染
