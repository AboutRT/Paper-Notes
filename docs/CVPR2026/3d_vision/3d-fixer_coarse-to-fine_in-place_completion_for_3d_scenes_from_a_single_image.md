---
description: "【论文笔记】3D-Fixer: Coarse-to-Fine In-place Completion for 3D Scenes from a Single Image 论文解读 | CVPR 2026 | arXiv 2604.04406 | 单图3D场景生成 | 提出\"就地补全\"（in-place completion）新范式，将预训练物体级生成先验扩展到场景级，直接在原始位置对碎片化几何进行补全，无需显式位姿对齐，同时构建110K规模场景级数据集 ARSG-110K，大幅超越 MIDI 和 Gen3DSR 等基线。"
tags:
  - CVPR 2026
  - 单图3D场景生成
  - 就地补全
...
## 研究背景与动机
1. **领域现状**：从单张图片生成组合式3D场景是机器人、AR/VR等领域的核心任务。
2. **现有方法两大流派及痛点**：
   - **前馈生成**（如MIDI, SceneGen）：端到端高效但泛化性差，且多实例注意力复杂度随物体数二次增长。
   - **分而治之**（如Gen3DSR）：生成/检索单个物体再优化位姿对齐——泛化性好但优化过程耗时且易累积误差。
3. **核心矛盾**：如何在保持泛化性的同时避免耗时的位姿对齐？
4. **关键观察**：几何估计模型已能准确恢复可见部分的3D几何，其中既包含空间布局也包含各实例的可见部分。因此可以直接在原位补全不可见部分，无需先生成再对齐。
5. **核心idea**：不做"生成+对齐"，而是"就地补全"——以碎片化几何为空间锚点，用物体级生成先验在原位补全完整3D资产。
