...
**代码**: [GitHub](https://github.com/Hyggge/EgoMind) (有)  
**领域**: Multimodal / VLM  
...
1. **3D 先验方法的高成本**：大多数现有方法通过引入点云、深度图、BEV 表示、相机参数等显式 3D 输入来增强空间推理，但这些方法需要昂贵的数据采集、对齐和训练过程。例如 SpaceVista 需要 1M 训练样本，Struct-2D 需要 200K。
2. **纯 2D 方法的局限性**：不依赖 3D 先验的方法在多帧空间推理中表现不佳，原因有二：(a) 模型逐帧处理输入，未建模跨帧的连续时空变换关系，导致空间理解碎片化；(b) 模型只关注问题中显式提及的目标物体，忽略了连接不同帧观测所需的隐式"空间桥梁"物体。
