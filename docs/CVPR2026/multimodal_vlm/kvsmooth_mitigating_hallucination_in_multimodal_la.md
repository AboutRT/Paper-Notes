...
description: >-
  [CVPR 2026][多模态VLM][幻觉缓解] KVSmooth 提出免训练即插即用方法，通过注意力行熵引导的自适应EMA平滑KV-Cache，将LLaVA-1.5的CHAIR_S从41.8降至18.2同时F1从77.5提升到79.2
tags:
  - CVPR 2026
  - 多模态VLM
  - 多模态幻觉
...
**本文要解决什么？** 在不需要重训练、不牺牲召回率、几乎零额外开销的前提下，抑制解码中的语义漂移和幻觉。

**切入角度**：从隐状态动态演化视角出发，发现 attention sink 是幻觉的直接诱因，提出对 KV-Cache 做自适应平滑。

**核心idea一句话**：行熵量化 sink 程度 → 高 sink token 给更强 EMA 平滑 → 抑制语义漂移 → 消除幻觉。
...
2. **EMA Smoothing on KV-Cache**:
    ...
    - 做什么：对 KV-Cache 中的 Key 和 Value 向量施加指数移动平均平滑
...
3. **Entropy-Guided Coefficient Adaptation**:
    ...
    - 做什么：根据每个 token 的 sink 程度自适应调节平滑强度
