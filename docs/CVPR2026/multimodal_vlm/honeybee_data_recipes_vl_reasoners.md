...
description: >-
  [CVPR 2026][多模态][VLM推理] 系统性地研究了VL推理训练数据的设计空间（数据来源、干预策略、多维度缩放），基于洞察构建了250万样本的HoneyBee数据集，训练出的3B VLM在MathVerse上超越SOTA 7.8个百分点。
tags:
  - CVPR 2026
  - 多模态
  - VLM推理
...
1. **上下文来源排名**
    ...
    - 做什么：在统一实验框架下公平对比不同数据源对 VL 推理性能的影响
...
2. **数据干预策略筛选**
    ...
    - 做什么：严格测试 8 种感知/推理增强策略，找出真正有效的干预
...
3. **三维数据缩放**
    ...
    - 做什么：分别研究缩放图像数量、每图问题数（合成新问题）、每题 CoT 数量的效果
