...
1. **Prompt表达力不足**：多数方法仅用1-2个prompt（如正/负对），单视角对齐无法捕捉多样化、细粒度的语义线索，难以有效区分clean和noisy信号
2. **显式负标签僵硬**：为每张图像分配一个硬负标签（指定某个反类），这种固定的反类信号在噪声环境下常不准确或无信息量，反而干扰优化
3. **去噪粗糙**：依赖固定置信度阈值或无选择性的伪标签机制，要么漏检噪声样本，要么误改正确标签，导致错误传播
