...
1. **领域现状**：人脸识别系统面临照片打印、视频重放、3D 面具等欺骗攻击，FAS 技术用于增强系统可靠性。早期方法在域内表现好，但跨域泛化能力差。
2. **现有痛点**：近期基于 MLLM 的 FAS 方法（如 I-FAS）将二分类任务转为文本生成，但仅能捕获粗粒度语义线索（如面具轮廓、屏幕边框），对高质量伪造样本的细粒度视觉模式感知力不足。
3. **核心矛盾**：MLLM 对低级视觉特征存在"盲区"（blindness to low-level visual features），而 FAS 恰恰依赖这些细粒度特征来区分真假人脸。简短的文本描述进一步加剧了这一问题。
4. **本文要解决什么**：如何引导 MLLM 感知那些容易被忽略的细微欺骗线索？
5. **切入角度**：从传统 FAS 方法的成功经验出发——LBP、HOG、FFT 等基础视觉算子已被证明能有效提取细粒度 spoof 特征。将这些算子作为"外部工具"嵌入 MLLM 的 CoT 推理过程。
6. **核心 idea**：让 MLLM 像侦探一样，先直觉判断，再用工具深入调查——"From Intuition to Investigation"。
