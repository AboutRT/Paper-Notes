...
description: >-
  [CVPR 2026][文档解析] DocHumming 提出数据-训练协同设计框架，通过真实场景合成构建DocMix-3M+渐进学习与结构token加权训练，在1B参数MLLM上实现OmniDocBench 93.75分，Wild场景仅退化6.72分
tags:
  - CVPR 2026
  - 文档解析
  - 合成数据
...
**本文要解决什么？** 通过数据-训练协同设计释放端到端文档解析在真实场景下的潜力。

**切入角度**：同时解决数据瓶颈（大规模合成）和训练瓶颈（结构感知优化），而非只攻其一。

**核心idea一句话**：576K 布局模板 + 9M 原子元素合成 3M 页面级数据，配合短到长渐进训练和结构 token 加权损失，让 1B 模型达到 235B 模型水平。
...
2. **Document-Aware Training Recipe（文档感知训练策略）**:
    ...
    - 做什么：设计专门针对文档解析的训练策略，解决上下文长度跨度大和结构化输出不稳定的问题
