...
1. **评估端**：大多数 benchmark 仅涉及单图或一组图片，答案往往可从单一模态推断，无法测试真正的跨模态推理
2. **训练端**：虽然大量交错图文数据用于预训练，但其中真正需要互补跨模态推理的数据极少
3. **模型端**：即使 SOTA 模型（GPT-4o）在需要 CoT 推理时，也经常产生与视觉/文本证据脱节的幻觉
