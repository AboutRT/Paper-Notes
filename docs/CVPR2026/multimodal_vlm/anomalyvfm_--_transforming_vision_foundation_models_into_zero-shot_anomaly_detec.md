...
## 研究背景与动机
1. **领域现状**：零样本异常检测要求在未见过的物体类别上无需任何域内图像即可检测异常。当前 SOTA 方法（AnomalyCLIP、AdaCLIP 等）依赖 CLIP 等视觉-语言模型的高层概念知识。
2. **现有痛点**：
...
3. **核心矛盾**：VFM 有更强的视觉表征能力，但缺乏多样化训练数据和有效的深层适配方法来释放其潜力。
4. **本文切入角度**：同时解决数据和方法两个瓶颈——合成大规模多样化数据 + LoRA 深层适配。
