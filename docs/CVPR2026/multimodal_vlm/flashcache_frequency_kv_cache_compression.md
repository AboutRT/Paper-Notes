---
description: 提出FlashCache，从频域角度识别多模态KV Cache中的离群KV并优先保留，结合动态逐层预算分配实现80%内存节省和1.69倍加速
tags:
  - CVPR2026
  - 多模态VLM
  - KV Cache压缩
...
2. **动态预算分配模块（Dynamic Budget Allocation）**:
    ...
    - 做什么：为模型不同层自适应分配不同的 KV Cache 保留配额
