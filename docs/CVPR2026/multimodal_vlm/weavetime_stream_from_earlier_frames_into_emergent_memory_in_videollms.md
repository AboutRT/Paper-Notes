...
**本文要解决什么**：Time-Agnosticism 导致的两个耦合问题——**时序顺序歧义**（Temporal Order Ambiguity）和**过去-当前焦点盲区**（Past-Current Focus Blindness）。

**切入角度**：先教时序（训练时），再用时序（推理时）—— "first teach order, then use order"。

**核心idea一句话**：通过轻量级时序重建辅助任务让模型学会感知帧序，再用不确定性驱动的粗到细检索实现按需回溯。
