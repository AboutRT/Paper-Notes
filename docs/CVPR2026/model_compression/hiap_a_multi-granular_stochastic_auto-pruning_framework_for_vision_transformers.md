...
1. **宏观门控：决定大结构是否存在**
   - 做什么：对 attention head 和 FFN block 做开或关的决策。
...
2. **微观门控：在保留结构内部继续瘦身**
   - 做什么：对 active head 里的 value 维度和 active FFN 里的隐藏神经元做细粒度裁剪。
...
3. **解析化的可微代价建模**
   - 做什么：把可剪枝的 MACs 显式分解成几类基础成本，再用这些成本去惩罚门控。
...
5. **结构可行性约束，防止层坍塌**
   - 做什么：限制模型不能为了压预算而把某一层或某一类结构过早清空。
