...
1. **极端数据稀缺**：公开数据集仅 1,170 张声纳图像（含 668 个标注目标），标注获取极其困难
2. **巨大的域差距**：声纳图像基于声学回波成像，与 RGB 自然图像在成像机理、纹理统计上截然不同——颜色信息几乎无意义，关键线索来自声学亮/暗区域和海床纹理
3. **大模型并非万能**：直觉上应该用 DINOv3 等大规模预训练模型进行迁移，但声纳域的特殊性使得这一策略未必有效
