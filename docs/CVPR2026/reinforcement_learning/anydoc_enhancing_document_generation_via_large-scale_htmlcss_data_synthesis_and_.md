...
## 研究背景与动机
1. **领域现状**：日常工作中广泛使用各类文档（简历、演示文稿、报告等），人工设计高质量文档需要平衡结构、布局、视觉和风格多个原则。近年自动文档生成受到关注。
2. **现有痛点**：
...
3. **核心矛盾**：如何同时实现类别通用性、结构可编辑性和数据充足性？
4. **本文切入角度**：引入 HTML/CSS 作为统一文档表示——天然层次结构、强大布局机制（flexbox/grid）、可大规模合成。
