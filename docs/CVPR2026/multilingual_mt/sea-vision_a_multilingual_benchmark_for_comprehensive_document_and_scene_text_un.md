...
**本文要解决什么？** (1) 构建首个统一评估文档解析 + TEC-VQA 的东南亚多语言基准；(2) 设计解决视觉-文本错位的标注方法论；(3) 量化 MLLM 在低资源 SEA 语言上的真实能力。

**切入角度**：设计混合标注流水线（自动过滤 + MLLM 辅助标注 + 母语者验证），用重渲染策略将翻译后的文字"画回"图像，从源头消除视觉-文本错位。

**核心 idea 一句话**：通过重渲染保证可视文本与 QA 语言完全一致，构建覆盖 11 种东南亚语言、统一评估文档解析与场景文字 VQA 的高质量基准。
...
1. **文档解析四阶段标注流水线**
    ...
    - 做什么：从约 3M 网络爬取 PDF 中筛选高质量多语言页面并精标注
