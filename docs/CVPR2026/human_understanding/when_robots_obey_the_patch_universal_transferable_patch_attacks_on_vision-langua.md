...
1. **VLA 模型的快速发展**：Vision-Language-Action (VLA) 模型将视觉编码器、语言理解和动作头耦合，能够解析自然语言指令并在仿真/真实世界执行多步操作，代表包括 OpenVLA、π₀ 等。
2. **对抗脆弱性的实际危害**：在机器人场景中，视觉对抗攻击不仅会误导感知，还会级联传播为不安全动作——碰撞、任务约束违反等，后果远比分类错误严重。
3. **现有攻击的局限**：已有的 VLA 对抗补丁（如 RoboticAttack）假设白盒访问，补丁高度过拟合到单一模型/数据集/prompt 模板，在黑盒设置（未知架构、微调变体）下攻击效果急剧下降。
4. **通用可迁移攻击的空白**：跨模型族（OpenVLA、OpenVLA-oft、π₀）的通用可迁移补丁攻击几乎未被探索，现有评估因此可能高估安全性。
5. **跨模态瓶颈的利用**：VLA 模型中视觉-语言的跨模态对齐机制是可被利用的结构性弱点，但缺乏系统性研究。
6. **安全评估的现实需求**：实际部署中攻击者不会有白盒访问权限，需要在黑盒条件、不同视角、sim-to-real 迁移等现实约束下评估安全基线。
