...
description: >-
  [CVPR 2026][优化][optimizer] 从信号处理视角揭示固定动量系数在偏差-方差权衡上的固有缺陷，提出SGDF优化器，通过在线计算最优时变增益（基于最小均方误差原则）动态平衡梯度估计的噪声抑制和信号保持，在多种视觉任务上超越SGD动量和Adam变体。
tags:
  - CVPR 2026
  - 优化
  - optimizer
...
3. **幂次缩放（Power Scaling, $\gamma=1/2$）**
    ...
    - 功能：用 $K_t^\gamma$ 替代 $K_t$ 增强在噪声环境下的响应性
