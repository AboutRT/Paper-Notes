...
description: >-
  [CVPR2026][优化][联邦学习] SCOPE 用一个零训练的视觉语言几何打分器，把每个样本压缩成表示性、多样性和负类边界接近度三个标量，再由服务器只聚合这些轻量统计量形成全局共识，指导各客户端先删语义异常样本、再删多数类冗余样本，从而在强非 IID 和长尾联邦场景下兼顾精度、鲁棒性和极低通信开销。
tags:
  - CVPR2026
  - 优化
  - 联邦学习
...
1. **三维语义评分体系（RS / DS / $S_{neg}$）**:
    - 功能：在共享 CLIP 语义空间中，为每个样本从三个正交维度刻画其保留价值
    - 核心思路：表示性分数 $RS_i = v_{img,i} \cdot t_{c_i}$ 衡量样本与本类原型的对齐度；多样性分数通过正交投影残差 $DS_i = \|v_{img,i} - RS_i \, t_{c_i}\|_2$ 捕捉超出类原型的类内变化；边界接近度 $S_{neg,i} = \max_{j \neq c_i} v_{img,i} \cdot t_j$ 刻画与最近负类的混淆程度。三个分数联合使用，使后续筛选能区分噪声、冗余和有价值的边界样本
    - 设计动机：单一指标无法兼顾核心锚点保留、类内多样性和决策边界支撑。RS 保基础、DS 去重复、$S_{neg}$ 识边界，三者正交互补，为两阶段筛选提供完整的语义判据

2. **隐私保护的全局 Profile 构建**:
    - 功能：在不传输样本级 embedding 的前提下，建立全联邦统一的语义参考坐标和类别稀有度
    - 核心思路：客户端只上传每类的样本数及三个分数的均值和方差，服务器通过总方差定律（分解组内与组间方差）计算全局均值和方差，用于后续 Z-score 标准化。同时根据全局类频率构造稀有度权重 $W_c \propto (1/(F_c + \epsilon))^{\gamma}$，标记全局长尾结构
    - 设计动机：联邦场景中客户端间数据分布差异大，简单平均会低估异质性；显式分解方差才能得到可靠的"全局正常范围"，同时类级统计量保证了通信轻量和隐私安全

3. **两阶段结构化剪枝（Consensus Filter + Dynamic Balancing）**:
    - 功能：先删语义异常样本，再定向压缩头部类冗余，实现带全局公平约束的 coreset 构造
    - 核心思路：阶段一计算异常分数 $AS_i = \hat{Z}_{S_{neg},i} - \hat{Z}_{RS,i}$（Z-score 标准化后），删除前 $p_l$ 比例的高异常样本，即"更像负类而不像真类"的样本。阶段二定义冗余分数 $R_i = \hat{Z}_{RS,i} - \hat{Z}_{S_{neg},i} - \hat{Z}_{DS,i}$，筛出"典型、远离边界、缺乏新颖性"的样本，但只对目标类指标 $T_c = f_c / W_c$ 较高（本地频率高且全局不稀有）的类别执行删除前 $p_f$ 比例操作
    - 设计动机：两阶段解耦了两种不同目标：阶段一用语义对齐而非高 loss 来识别噪声，避免误删正常困难样本；阶段二将削减限制在头部类，保护尾类和边界样本，实现非均匀的结构化压缩
