...
## 方法详解
这篇论文的方法设计并不复杂，但思路很整齐。作者先把“什么样的样本值得留”拆成三个几何问题，然后让服务器只聚合这些问题的统计答案，最后客户端依据这个全局共识执行两阶段筛选。

直观地说，SCOPE 的逻辑不是去找“最难的样本”，而是把样本分成三类再分别处理：

- 明显语义不对劲、像噪声或错配的样本，先删。
- 明显属于头部类中心区域、重复度很高的样本，再删。
- 真正能支撑决策边界、保持类内多样性或保护尾类的样本，尽量留。

这和传统 loss-based 剪枝最大的区别在于：它不依赖训练动态，而依赖预训练 VLM 的语义几何结构。

### 整体框架
整体流程可以按四步理解。

第一步，每个客户端使用 MobileCLIP-S2 把图像编码到共享语义空间，同时用自然语言 prompt 为每个类别构造文本原型。随后对每个样本计算三个标量分数：表示性分数 RS、多样性分数 DS、负类边界接近度 $S_{neg}$。

第二步，客户端不上传样本级 embedding，只上传类别级统计量，包括每个类的样本数，以及三个分数在该类上的均值和方差。服务器据此构造全局 profile，既知道每个类在全局上有多稀有，也知道每个类在三个语义指标上的全局分布。

第三步，客户端拿到全局 profile 后，先做 Consensus Filter。这个阶段主要删除语义矛盾强的样本，也就是更像负类、却不太像真类的样本。

第四步，客户端再做 Dynamic Balancing。这个阶段不再全局无差别删样本，而是只在“本地很多且全局也偏常见”的类别里删最冗余的样本，借此压头部类、保尾部类。

最终得到的 coreset 被送去后续联邦训练，而 coreset 的构造本身是训练前、零训练、轻通信的。

### 关键设计
1. **表示性分数 RS**:
	- 做什么：判断一个样本是不是该类别的“标准代表”。
	- 核心思路：把图像 embedding $v_{img,i}$ 与其真实类别的文本原型 $t_{c_i}$ 做内积，得到 $RS_i = v_{img,i} \cdot t_{c_i}$。
	- 设计动机：联邦剪枝不能只保留边界样本，还要保留每个类最核心、最能稳定训练的锚点样本。RS 高说明这个样本和类别语义对得很准，是基础语义支撑点。

2. **多样性分数 DS**:
	- 做什么：判断一个样本是否仅仅是“标准样本的重复版本”，还是带有超出类别原型的额外结构信息。
	- 核心思路：先把图像 embedding 在文本原型方向上的分量拿掉，得到正交残差 $v_{res,i} = v_{img,i} - RS_i t_{c_i}$，再取其模长 $DS_i = ||v_{res,i}||_2$。
	- 设计动机：如果只看 RS，高 RS 样本往往都像“最标准的教材图”，但这些样本可能高度重复。DS 通过观察正交残差信息来刻画新颖性，值越高说明它除了像本类，还保留了更多类内变化。

3. **边界接近度 $S_{neg}$**:
	- 做什么：判断样本离其他类别的决策边界有多近。
	- 核心思路：对所有错误类别文本原型求最大相似度，$S_{neg,i} = \max_{j \neq c_i} v_{img,i} \cdot t_j$。
	- 设计动机：联邦模型若要学到稳健边界，必须保留一些“容易和别类混淆”的样本。$S_{neg}$ 高的样本不一定是噪声，也可能是边界支撑点，因此后续不能粗暴删除，而要和 RS、DS 联合判断。

4. **全局稀有度权重 $W_c$**:
	- 做什么：刻画某类别在全联邦范围内到底稀不稀有。
	- 核心思路：服务器根据全局类频率 $F_c$ 构造反比幂律权重 $W_c \propto (\frac{1}{F_c + \epsilon})^{\gamma}$。
	- 设计动机：单靠本地类频看不出全局长尾结构。$W_c$ 高说明这是全局尾类，需要保护；$W_c$ 低说明它本来就是头部类，更适合在后续冗余削减中被针对。

5. **全局异质性 profile**:
	- 做什么：给每个类别、每个语义指标建立全联邦统一的参考坐标。
	- 核心思路：服务器使用各客户端上传的类别级均值、方差和样本数，通过总方差定律计算每个类在 $RS$、$DS$、$S_{neg}$ 上的全局均值和全局方差，而不是简单平均。
	- 设计动机：联邦数据不仅均值不同，客户端间方差也很大。如果只平均局部统计，会低估异质性。作者显式分解组内和组间方差，本质上是在构造一个更可靠的“全局正常范围”。

6. **阶段一：Consensus Filter**:
	- 做什么：优先移除语义异常样本。
	- 核心思路：先按类把 $RS$、$DS$、$S_{neg}$ 用全局均值和方差做 Z-score 标准化，再裁剪到 $[0,1]$。然后构造异常分数 $AS_i = \hat{Z}_{S_{neg},i} - \hat{Z}_{RS,i}$，值越大表示“更像负类而不像真类”。客户端按 $AS_i$ 从高到低删除前 $p_l$ 比例样本。
	- 设计动机：这里抓住的是科学数据里很常见的“高置信噪声”问题。作者不直接拿高 loss 当噪声，而用语义对齐关系做判别，避免把正常但困难的边界样本误删。

7. **阶段二：Dynamic Balancing**:
	- 做什么：在保证边界和尾类不被破坏的前提下，压缩头部类内部的重复样本。
	- 核心思路：对中间保留集中的样本定义冗余分数 $R_i = \hat{Z}_{RS,i} - \hat{Z}_{S_{neg},i} - \hat{Z}_{DS,i}$。它偏好那些“非常典型、远离边界、又缺乏新颖性”的样本。然后作者再定义目标类指标 $T_c = f_c / W_c$，只对本地频率高且全局也不稀有的类别做冗余剪枝，并删掉其中冗余分数最高的前 $p_f$ 比例。
	- 设计动机：这一步很关键，因为它把“删谁”限制在头部类，避免所有类一起被平均删薄。换句话说，SCOPE 不是在做均匀压缩，而是在做带有全局公平约束的结构化压缩。

### 损失函数 / 训练策略
SCOPE 本身不是一个新训练损失，而是一个训练前的数据筛选框架。它的核心特点是“selection is training-free”。

训练阶段沿用常规联邦训练配置：

- 下游训练使用 SGD 和 cosine decay 学习率。
- 总通信轮数为 200。
- 报告结果取最后 10 个 global rounds 的 Top-1 accuracy 平均值。
- baseline 方法如 FedCS、FedCore、EL2N、Forgetting、GradND 都需要基于本地模型训练信号来做筛选。
- SCOPE 不需要在原始全量本地数据上先 warmup，因此在 coreset selection 这一步更便宜。

如果从优化角度看，SCOPE 等于在训练前先改变经验分布：

- 用异常筛选减少梯度偏差项。
- 用动态平衡减少 client drift。
- 用全局 rarity 约束阻止长尾分布被进一步破坏。

论文还给出非凸情形下的理论解释。结论不是说它改变了优化器，而是说这种数据构造能让最终联邦目标的异质性项和逼近误差项更小，因此收敛到更紧的 stationary point。
