---
description: "【论文笔记】Fighting Hallucinations with Counterfactuals: Diffusion-Guided Perturbations for LVLM Hallucination Suppression 论文解读 | 提出 CIPHER，一种无需训练的测试时幻觉抑制方法：离线阶段用扩散模型生成反事实图像构建 OHC-25K 数据集，通过 SVD 提取视觉幻觉子空间；推理阶段将隐状态投影到该子空间的正交补空间，在不修改模型参数、不增加推理开销的前提下显著降低 LVLM 的视觉幻觉。"
tags:
...
1. **文本诱导幻觉**：源自 LLM 的自回归生成偏好和语言先验
2. **视觉诱导幻觉**：源自弱视觉 grounding、模态对齐不足
