...
1. **领域现状**: 扩散模型水印技术从后处理（HiNet）→ 模型微调（Stable Signature）→ training-free 噪声空间注入（Tree-Ring, Gaussian Shading）不断演进。SEAL 进一步引入语义感知，使检测依赖图像内容而非固定全局模式。

2. **现有痛点**: 现有语义水印（如 SEAL）依赖单一全局语义绑定。CSI 攻击证明了攻击者可以做局部但全局连贯的语义编辑（如只改主体）来绕过全局水印验证，ASR 高达 81%。内容无关水印（Tree-Ring 等）在生成式伪造攻击下近乎全面崩溃（100% ASR）。

3. **核心矛盾**: 图像语义不是单一整体，而是由部分独立的语义因子组成。全局绑定无法区分"整体语义看起来一致但某个局部因子被恶意修改"的情况。

4. **本文要解决什么？** (1) 如何将水印与细粒度语义因子绑定而非全局？(2) 如何实现篡改不仅可检测还可定位？(3) 如何保证理论上的安全性保证？

5. **切入角度**: 利用扩散模型潜空间的空间可分解性——不同空间区域可以独立携带不同语义的水印信号。

6. **核心idea一句话**: 将图像语义解耦为四因子并各自绑定到噪声潜空间的独立空间分区，使局部语义编辑仅导致对应分区验证失败，实现篡改定位。
