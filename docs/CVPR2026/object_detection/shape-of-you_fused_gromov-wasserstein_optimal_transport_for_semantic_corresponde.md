---
description: "【论文笔记】Shape-of-You: Fused Gromov-Wasserstein Optimal Transport for Semantic Correspondence in-the-Wild 论文解读 | CVPR2026 | arXiv 2603.11618 | 语义对应 | 将语义对应问题重新建模为 Fused Gromov-Wasserstein (FGW) 最优传输问题，利用 3D 基础模型提供的几何结构约束来生成全局一致的伪标签，解决了传统最近邻匹配因局部性和 2D 外观歧义导致的几何不一致问题。"
tags:
  - CVPR2026
  - 语义对应
  - 最优传输
...
1. **语义对应的核心挑战**：在 in-the-wild 场景中，极端视角变化、光照差异和类内形状差异使得建立像素级语义对应极为困难，而全监督方法依赖昂贵的逐像素标注，可扩展性差。
2. **伪标签方法的固有缺陷**：当前无显式几何标注的方法（如 ASIC）依赖 DINO/SD 等基础模型的特征进行最近邻 (NN) 匹配生成伪标签，但 NN 匹配是局部操作，忽略了全局结构关系。
3. **2D 特征的几何歧义**：纯粹基于 2D 外观训练的模型无法反映物体真实的 3D 几何结构，导致语义上合理但几何上错误的对应（如对称部件、重复纹理区域的误匹配）。
4. **局部匹配无法保持结构一致性**：NN 匹配逐点独立决策，不保证匹配集合在全局上的几何连贯性，引入的训练噪声会降低模型性能。
5. **GW 计算瓶颈**：Gromov-Wasserstein 是二次规划的非凸问题，直接求解在计算上不可行，需要高效的近似方案。
6. **部分可见性与遮挡**：真实场景中物体常被遮挡或截断，平衡最优传输的硬边际约束（所有质量必须传输）不适用于这种部分匹配情况。
