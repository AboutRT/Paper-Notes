---
description: "【论文笔记】DA-Mamba: Learning Domain-Aware State Space Model for Global-Local Alignment in Domain Adaptive Object Detection 论文解读 | CVPR2026 | arXiv 2603.18757 | 域自适应目标检测 | 提出 DA-Mamba，一种 CNN-SSM 混合架构，通过 Image-Aware SSM（IA-SSM）和 Object-Aware SSM（OA-SSM）两个模块，以线性复杂度实现图像级和实例级的全局-局部域不变特征对齐，在四个域自适应检测基准上达到 SOTA。"
tags:
  - CVPR2026
  - 域自适应目标检测
  - 状态空间模型
...
1. **域自适应目标检测（DAOD）** 旨在将标注源域的检测器迁移到无标注目标域，核心在于学习域不变特征表示。
2. **CNN 局部性瓶颈**：现有 CNN-based DAOD 方法的卷积局部连通性限制了对齐范围——backbone 仅在滑动窗口内提取局部特征，检测头也无法建模所有实例间的空间/语义依赖，导致 region-to-region 和 object-to-object 对齐都局限于局部区域。
3. **Transformer 复杂度过高**：已有工作引入 Vision Transformer 捕获全局依赖，但自注意力的二次复杂度带来大量计算和内存开销；一些卷积注意力变体要么依赖全局参数共享（忽略域偏置），要么仍隐含二次复杂度。
4. **实例间空间-语义关系被忽视**：物体存在稳定的共现模式（如 rider 与 bicycle），且语义上有层次关联（cat/dog 同为动物），忽略这些全局依赖会削弱域对齐效果。
5. **Mamba/SSM 机遇**：状态空间模型（SSM）具有线性时间复杂度的长程依赖建模能力，为 CNN 检测器注入全局域信息提供了高效方案。
6. **需要全局-局部联合对齐**：单纯的局部对齐或全局对齐都不够，需要同时在图像级和实例级进行多粒度的全局-局部特征对齐。
...
- 插入 backbone 的 FPN 中，采用**双流管道**：卷积流提取局部域不变特征，SSM 流捕获全局视觉属性。
- 引入可学习的**图像级视觉提示** $\mathbf{v}^I \in \mathbb{R}^C$，广播到所有空间位置后与输入特征拼接，为双流注入域信息。
- 采用瓶颈结构（降维比 $r$）减少冗余，最后将双流输出拼接并上采样恢复原始维度。

**Object-Aware SSM（OA-SSM）**：

- 插入检测头中，同样采用双流管道，但设计了**像素级实例级视觉提示** $\mathbf{v}^O \in \mathbb{R}^{B,C,H,W}$。
- 通过卷积层将输入特征映射为逐像素的类别相似度矩阵 $\mathbf{W} \in \mathbb{R}^{B,H,W,K}$，与 CLIP 文本编码器生成的**类别原型** $\mathbf{E} \in \mathbb{R}^{K,C}$ 相乘得到实例级提示，将特征图划分为语义一致的实例区域。
- 实现对所有实例间空间共现和语义层次关系的全局建模。
