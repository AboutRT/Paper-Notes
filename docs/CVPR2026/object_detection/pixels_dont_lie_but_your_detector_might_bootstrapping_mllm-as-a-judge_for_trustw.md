...
1. **生成模型进步带来威胁**：Stable Diffusion、DALL·E 2、Imagen 等扩散/Transformer 模型生成的合成图像已接近真实，传统基于频域不一致或眨眼模式的检测方法在现代生成管线面前失效。
2. **分类准确率不够，需要可解释推理**：现有 deepfake 检测器仅关注分类准确率，但可靠的检测需要可解释且视觉可溯源（visually grounded）的推理，已有方法（如 SIDA、FakeShield）的文本解释常常与视觉证据脱节或存在幻觉。
3. **VLM 过度依赖文本先验**：研究表明视觉语言模型倾向于依赖文本先验和世界知识，而非真正观察图像内容，导致推理不可靠（例如即使图像中牛只有三条腿也预测四条腿）。
4. **传统文本指标无法衡量推理质量**：BLEU、ROUGE、METEOR、BERTScore 等指标只捕捉词汇重叠或语义相似度，无法评估推理的事实性和视觉溯源性，与人类判断相关性差。
5. **缺乏 OOD 基准**：现有数据集（FaceForensics++、DFDC）主要针对老一代生成模型，缺少对最新文本生成图像和图像编辑伪造的测试。
6. **缺乏推理监督的可扩展方案**：人类标注推理成本昂贵、难以大规模扩展，需要一种方法将有限的人类标注高效放大为大规模训练数据。
