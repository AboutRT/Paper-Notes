---
description: 发现跨域少样本目标检测中的注意力散光问题，提出受人类中央凹视觉启发的中心-周边原型框架PPR+NCM+TSA，在6个基准上达到SOTA
tags:
  - CVPR2026
  - 目标检测
  - 跨域少样本
...
**本文要解决什么？**：如何增强模型自身修复散光问题的内在趋势，使注意力从分散模式转变为聚焦的、以目标为中心的模式，从而在目标域实现精准检测。

**切入角度**：受人类视觉系统中央凹结构的启发——中央感知区域捕获高细节信息（前景），外周区域捕获低细节信息（背景），中心-周边对比机制维持注意力集中。据此设计三个模块分别增强中心区域表征、周边区域表征和两者之间的区分度。

**核心idea一句话**：通过类特定前景原型增强目标区域注意力（上调 $A_2, A_3$），通过统一背景原型抑制背景区域的伪前景响应（下调 $A_1$），再通过"not [class]"文本线索从跨模态角度强化前景-背景分离，三管齐下将散焦注意力转变为聚焦模式。
...
1. **正向模式精化（PPR）模块**:
    ...
    - 做什么：利用类特定原型增强前景区域的特征表示，模拟人类视觉的中央感知区域
...
2. **负向上下文调制（NCM）模块**:
    ...
    - 做什么：构建统一的背景原型来增强背景区域表征，模拟人类视觉的外周感知区域
...
3. **文本语义对齐（TSA）模块**:
    ...
    - 做什么：利用跨模态知识强化前景-背景的区分度，实现中心-周边的对比增强
