---
description: "【论文笔记】HiF-VLA: Hindsight, Insight and Foresight through Motion Representation for Vision-Language-Action Models 论文解读 | CVPR 2026 | arXiv 2512.09928 | VLA模型 | 提出 HiF-VLA 框架，通过运动向量（Motion Vector）作为紧凑时间原语，统一回顾（Hindsight）、洞察（Insight）和前瞻（Foresight）三种时间推理能力，实现 VLA 模型的双向时间扩展，在长时操作任务中以极低计算开销大幅超越基线。"
tags:
  - CVPR 2026
  - VLA模型
  - 运动表示
...
现有缓解时间近视的方法主要有两个方向：
1. **历史帧堆叠**：如 TraceVLA、Octo 等将多帧过去观测作为输入，但存在严重冗余——相邻帧高度相似，计算开销大且推理延迟高（Tab.3 显示 history=4 时延迟增加 3.15×）
2. **像素级子目标预测**：如 CoT-VLA、Seer 等预测未来视觉子目标，但易受局部失真和语义漂移影响
