...
**本文要解决什么？** 让具身agent从自身视角出发推断人类心理状态（信念、欲望、意图），并基于此做出主动的决策和行动。

**切入角度**：将认知科学的BDI框架（Belief-Desire-Intention）系统化地引入具身agent，构建三级六层的连续推理层级，并用结构化奖励函数（Mind-Reward）通过RL优化推理一致性。

**核心idea一句话**：用三级六层的Robot-Centric BDI推理层级将感知连接到行动，并用原子动作匹配的Mind-Reward通过GRPO优化推理链的一致性。
...
2. **Robot-Centric视角（区别于Role-Centric）**:
    - **做什么**: 要求agent同时推断自己的信念和人类的信念，形成双视角推理闭环
...
3. **Mind-Reward（原子动作匹配奖励）**:
    - **做什么**: 设计结构化奖励函数驱动GRPO优化，确保推理链从感知到行动的一致性
