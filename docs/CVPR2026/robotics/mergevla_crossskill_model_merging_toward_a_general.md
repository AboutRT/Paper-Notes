...
**本文要解决什么？** 设计一种"为合并而生"的 VLA 架构，让多个单任务专家可以高效合并为一个通才模型。

**切入角度**：先精确诊断失败的两个根因，再针对性设计解决方案——task mask 解决参数冲突，去自注意力解决架构不兼容。

**核心idea一句话**：通过稀疏激活的 task mask 抑制 LoRA 冲突参数 + 删除 action expert 的自注意力层消除任务依赖传播，使 VLA 模型"天生可合并"。
...
1. **Task Mask（解决 LoRA 参数冲突）**
    ...
    - 做什么：为每个任务构建 binary mask，选择性激活与该任务一致的合并参数，抑制冲突参数
...
2. **去自注意力的 Action Expert（解决架构不兼容）**
    ...
    - 做什么：重新设计 action expert 架构使其天生可合并
...
3. **Test-time Task Router（免训练任务推断）**
    ...
    - 做什么：在任务身份未知时自动判断当前任务并选择对应的 mask 和 expert head
