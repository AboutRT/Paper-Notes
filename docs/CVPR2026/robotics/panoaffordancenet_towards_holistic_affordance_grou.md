...
description: >-
  [CVPR 2026][机器人][功能可供性] 提出全景室内环境的整体功能可供区域定位任务，设计畸变感知频谱调制和全球面致密化头，构建首个全景功能可供性数据集360-AGD
tags:
  - CVPR 2026
  - 机器人
  - 功能可供性
...
**本文要解决什么？** (1) 如何在ERP畸变下保持局部交互细节和全局功能结构；(2) 如何从稀疏碎片化的初始激活恢复连续完整的功能区域；(3) 如何在极度稀疏（one-shot）标注下精确对齐语义和视觉区域。

**切入角度**：将问题分解为三个独立通道：频谱域处理畸变（高频+低频分别校正）、球面拓扑域处理碎片化（自相似性传播）、对比学习域处理语义漂移（区域-文本对齐）。

**核心idea一句话**：通过频谱畸变校正+球面致密化+多层级约束的三阶段设计，实现360°室内环境下的one-shot整体功能可供性定位。
...
2. **全球面致密化头（OSDH）**:
    ...
    - 做什么：将稀疏碎片化的初始功能区域激活恢复为拓扑连续的完整区域
...
3. **区域-文本对比损失（$\mathcal{L}_{RTC}$）**:
    ...
    - 做什么：建立视觉区域与功能语义概念之间的精确对应，抑制语义漂移
