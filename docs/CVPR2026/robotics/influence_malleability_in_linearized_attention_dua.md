...
description: >-
  [CVPR 2026][深度学习理论][线性化注意力] 通过NTK框架证明线性化注意力不会收敛到无限宽度核极限（需要宽度m=Ω(κ⁶)），并提出"影响可塑性"指标量化其双面效应：注意力比ReLU网络高6-9倍的数据依赖灵活性，既能降低近似误差也增加对抗脆弱性。
tags:
  - CVPR 2026
  - 深度学习理论
  - 线性化注意力
...
**本文要解决什么？** 线性化注意力是否收敛到无限宽度 NTK 极限？如果不收敛，这种非收敛行为对模型的训练数据依赖性意味着什么？

**切入角度**：设计无参数线性化注意力 $f^{att}(X) = XX^TX$，建立其与数据依赖 Gram 诱导核的精确对应，利用谱分析解释非收敛现象，并通过"影响可塑性"量化双面效应。

**核心 idea 一句话**：注意力的 power 与 vulnerability 同源于它对核体制的超越——数据依赖核带来灵活性的同时也带来脆弱性。
