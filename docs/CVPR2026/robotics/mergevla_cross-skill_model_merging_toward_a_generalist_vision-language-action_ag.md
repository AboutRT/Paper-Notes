...
description: >-
  [CVPR 2026][机器人][VLA模型合并] 首次系统诊断 VLA 模型不可合并的两大根因（LoRA 自私参数冲突 + 动作专家自注意力导致的任务耦合），提出 MergeVLA——通过任务掩码稀疏激活 LoRA、去自注意力动作专家、无训练测试时路由，将多个单技能 VLA 专家合并为一个通用 agent…
tags:
  - "CVPR 2026"
  - "机器人"
  - "VLA模型合并"
...
1. **任务掩码稀疏 LoRA**（解决 VLM 中 LoRA 参数冲突）
2. **去自注意力动作专家**（解决动作专家架构不兼容）
3. **无训练测试时任务路由**（解决推理时不知任务身份的问题）
...
1. 对每个候选任务 m，用掩码 $\mathbf{S}_m$ 运行 VLM 获取隐状态
2. 对合并动作专家第 l 块的值投影矩阵做 SVD，取前 $k_r=8$ 个右奇异向量构成主子空间
3. 将各任务隐状态投影到此子空间，计算响应强度 $r_m$
4. softmax 选择得分最高的任务，固定该掩码和 expert head 用于整个 episode
