...
1. **长残差连接 (Long Residual Connections)**：将浅层 block 的输出注入对称深层 block，打破输入同质化
2. **表示多样性损失 (Representation Diversity Loss)**：由正交损失、互信息最小化损失、特征离散损失三部分组成，显式约束各 block 学到不同特征
...
- **功能**：将第 $i$ 个 block 的输出连接到第 $(L-i)$ 个 block（$L$ 为总层数），形成对称的跳跃连接
