...
1. **系数不通用**：架构或数据管道改变后需重新估计
2. **MoE 引入新维度**：MoE 模型通过稀疏路由以固定 FLOPs 获得高容量，成为 Gemini 2.5 Pro、DeepSeek-V3、Qwen3 等旗舰模型的标准配置，但稠密模型的 scaling 前沿无法覆盖稀疏度这一维度
3. **损失≠性能**：相同预训练损失的模型在下游推理基准上可能表现迥异（GLM-4.5 Team 也有此观察）
4. **后训练和 TTC 的影响未知**：GRPO 和测试时计算是否能弥补预训练稀疏度选择的不足？
