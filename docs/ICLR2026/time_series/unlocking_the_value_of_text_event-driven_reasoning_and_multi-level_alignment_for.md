---
description: "【论文笔记】VoT: 事件驱动推理与多层对齐解锁文本价值用于时间序列预测 论文解读 | ICLR 2026 | arXiv 2603.15452 | 多模态时间序列预测 | 提出 VoT，一种通过事件驱动推理（利用 LLM 对外生文本进行结构化推理获取数值预测）和多层对齐（表征级内生文本对齐 + 预测级自适应频率融合）充分挖掘文本信息价值的多模态时间序列预测方法，在 10 个领域的真实数据集上全面超越现有方法。"
tags:
  - ICLR 2026
  - 多模态时间序列预测
  - 事件驱动推理
