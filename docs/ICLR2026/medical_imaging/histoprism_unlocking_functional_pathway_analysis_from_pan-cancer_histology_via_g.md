...
1. **领域现状**：从H&E切片预测空间基因表达是低成本替代测序的方向。现有方法（BLEEP/TRIPLEX/STEM/STPath）要么限于单癌种、要么评估仅看高变异基因(HVG)的Pearson相关。

2. **现有痛点**：(1) HVG评估忽略了基因间的功能协调——单基因好不等于通路级别好；(2) 多数方法受限于单癌种训练→泛化差；(3) STPath虽然泛癌但依赖基因-基因相关性假设+参数量巨大。

3. **切入角度**：提出GPC基准——在50个Hallmark+87个GO通路上评估功能一致性→比HVG更接近临床意义。
