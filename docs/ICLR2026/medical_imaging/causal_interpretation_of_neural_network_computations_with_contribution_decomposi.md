...
description: >-
  [ICLR2026][医学图像][neural network interpretability] 提出 CODEC（Contribution Decomposition），用 Integrated Gradients 计算隐藏层神经元对输出的贡献（而非仅分析激活），再用 Sparse Autoencoder 将贡献分解为稀疏模式（modes），实现比激活分析更强的因果可解释性和网络控制能力，并成功应用于 ResNet-50 和视网膜生物神经网络模型。
tags:
  - ICLR2026
  - 医学图像
...
2. **Sparse Autoencoder 分解（CODEC 核心）**:
    ...
    - 功能：将 $d$ 通道 × $n$ 图像的贡献矩阵分解为 $k$ 个稀疏模式
...
3. **贡献映射可视化（Contribution Mapping）**:
    ...
    - 功能：将 modes 映射回输入像素空间，展示哪些图像区域通过哪个 mode 驱动输出
...
4. **网络控制实验**:
    ...
    - 功能：用 CODEC 识别的关键通道进行定向ablation/preservation
