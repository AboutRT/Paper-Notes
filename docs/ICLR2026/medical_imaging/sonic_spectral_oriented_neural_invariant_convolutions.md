...
**本文要解决什么？** （1）如何在频域设计真正连续的、不依赖离散网格的卷积参数化？（2）如何在频域引入方向感知先验，同时保持极低的参数量？（3）如何让单一架构在 2D / 3D、不同分辨率之间无缝切换？
...
**核心 idea 一句话**：用 SSM 式的连续解析函数在频域中参数化方向选择性的全局卷积核，以低秩分解实现极端参数高效的全局感受野。
