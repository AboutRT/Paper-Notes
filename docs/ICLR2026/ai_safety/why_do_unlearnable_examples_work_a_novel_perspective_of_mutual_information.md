...
整个工作分三个递进阶段。第一阶段是**实证发现**：在多种 MI 估计器上系统测量所有主流 UE 方法的 MI 降低量，验证"MI 降低 ↔ 泛化下降"的强正相关（Spearman 相关 0.7818）。第二阶段是**理论推导**：在高斯混合假设下，证明 MI 上界中包含类内协方差 $\log\det\Sigma_Y$ 项，从而将难以直接优化的 MI 问题转化为可优化的协方差缩减问题。第三阶段是**方法设计**：提出 MI-UE，通过双层优化同时训练 shadow 模型和优化扰动 $\delta$，外层目标为最大化类内余弦相似度、最小化类间余弦相似度的 $\mathcal{L}_{mi}$ 损失。

### 关键设计

1. **MI 解释框架（实证验证）**:

    - 功能：为所有 UE 方法提供统一的有效性度量
    - 核心思路：分类模型 $f = h \circ g$（$g$ 为特征提取器，$h$ 为线性分类头）。对每种 UE 方法，用其生成的下毒数据训练 ResNet-18，然后在特征空间中计算 $I(g(X), g(X'))$。为避免单一估计器偏差，同时使用四种 MI 估计方法：Histogram、KDE、k-NN 和 MINE，配合 Sliced MI（SMI）将高维特征投影到一维后估计。实验覆盖了 EM、AP、NTGA、AR、REM、SEM、GUE、TUE 等 8 种 UE 方法。所有有效 UE 的 MI 均显著低于 Clean 和 Random 基线，且 MI Gap 越大、Acc Gap 越大。此外，在 Linear→2-NN→3-NN→LeNet-5→VGG-11→ResNet-18 不同深度模型上，MI 降低和准确率下降严格正相关——深度模型的特征提取器放大了扰动对 MI 的影响（error amplification 效应），线性模型中 $g$ 退化为恒等映射，扰动在小范数约束下几乎无法改变 $I(X, X')$
    - 设计动机：如果一个解释只在某一种估计方法下成立，说服力不足；四种独立估计器的一致趋势提供了强有力的实证支撑

2. **协方差缩减定理（Theorem 5.1）**:

    - 功能：将不可直接优化的 MI 降低问题转化为可优化的协方差最小化问题
    - 核心思路：假设对每个类别 $Y$，下毒特征 $g(X')|Y$ 近似服从高斯分布 $\mathcal{N}(\mu_Y, \Sigma_Y)$（KL 散度 $\leq \epsilon$）。则 MI 上界为 $I(g(X), g(X')) \leq \frac{d}{2}\log(2\pi e) + \frac{1}{2}\mathbb{E}_Y\log(\det\Sigma_Y) + H(g(X')|g(X)) + \mathbb{E}_Y C_Y\sqrt{\epsilon}$。其中第一项是常数，第三项在固定 UE 生成器 $\mathcal{G}$ 和训练算法 $\mathcal{A}$ 后也为常数，第四项是近似误差小量。因此，MI 上界的关键可变量是类内协方差行列式 $\det\Sigma_Y$——缩小它就降低 MI 上界
    - 设计动机：MI 估计本身在高维空间极其困难（已有方法存在严重统计偏差，SGD 优化 MI 有偏），直接优化 MI 不可行。协方差缩减定理将优化目标从"MI"降维到"类内特征散布程度"，使问题变得可处理

3. **MI-UE 损失函数**:

    - 功能：实现协方差缩减的具体优化目标
    - 核心思路：$\mathcal{L}_{mi}$ 由两项组成。**相似度项**：对 mini-batch 中的样本对，分子计算同类样本对的 $\exp(\cos/\tau)$ 之和，分母计算异类样本对的 $\exp(\cos/\tau)$ 之和，取 log 比值——最大化类内余弦相似度同时最小化类间相似度，自然压缩类内协方差且防止类坍缩。**距离项**：$\zeta \cdot \log(1 + \sum_k \|g(x_{b_j}+\delta_{b_j}) - g(x_{b_k}+\delta_{b_k})\|_2)$ 作为正则项增强鲁棒性。使用 $\log(1+\cdot)$ 形式稳定梯度。采用 batch normalization 不变的余弦度量而非纯欧氏距离，避免归一化层使欧氏优化失效
    - 设计动机：纯距离最小化在有 BN/LN 的网络中失效（归一化后特征被重新缩放）；类间排斥项防止所有类特征坍缩到同一点（这反而会恢复泛化能力）
...
- [InfoDecom: Decomposing Information for Defending Against Privacy Leakage in Split Inference](../../AAAI2026/ai_safety/infodecom_decomposing_information_for_defending_against_privacy_leakage_in_split.md)
