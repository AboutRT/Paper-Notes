...
description: >-
  [ICLR 2026][动作预期] 提出动作引导注意力 (AGA) 机制，用模型自身的动作预测序列作为注意力的 Query 和 Key（而非像素特征），结合自适应门控融合历史上下文和当前帧特征，在 EPIC-Kitchens-100 上实现从验证集到测试集的良好泛化，同时支持训练后的可解释性分析。
tags:
  - ICLR 2026
  - 动作预期
  - 注意力机制
...
1. **动作引导的 Query/Key**:
    ...
    - 功能：用预测的动作概率分布（而非视觉特征）构建注意力的 Query 和 Key
...
2. **自适应门控融合**:
    ...
    - 功能：逐元素门控融合历史注意力输出 $\tilde{h}_t$ 和当前帧特征 $e_t$
...
3. **训练后可解释性分析**:
    ...
    - 功能：通过前向/反向分析揭示模型学到的动作依赖关系和反事实证据
