...
description: >-
  [ICLR 2026][科学计量] 构建 Paper Copilot 跨数十个 AI 会议的同行评审持久数字档案与分析平台，通过三源混合数据收集（OpenReview API + 网页抓取 + 社区 opt-in）实现评审评分动态实时归档，对 ICLR 2017-2025 进行大规模纵向分析，揭示 2025 年评审从概率性分层转向近确定性分数驱动决策的结构性变化。
tags:
  - ICLR 2026
  - 同行评审
  - 评分动态
...
**本文要解决什么？** 构建统一的评审数据收集、归档和分析平台，支持跨会议纵向分析和实时评分动态追踪。

**切入角度**：三源混合数据策略最大化覆盖率，实时归档时间快照保存不可恢复的历史数据。

**核心idea一句话**：将分散、短暂的 AI 会议评审信息统一为持久化、结构化、可分析的数字档案，构建评审过程的"元科学基础设施"。
...
2. **评审动态时间快照归档**:
    - 做什么：实时归档评审分数在讨论/rebuttal 阶段的完整演变过程
...
3. **LLM 驱动的作者-机构元数据提取**:
    - 做什么：大规模自动提取论文作者的结构化元组 $(a_i, \mathcal{A}_i, e_i)$（姓名, 机构集合, 邮箱）
