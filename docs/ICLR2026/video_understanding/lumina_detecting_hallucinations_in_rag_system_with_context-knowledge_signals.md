---
description: "【论文笔记】LUMINA: Detecting Hallucinations in RAG System with Context-Knowledge Signals 论文解读 | ICLR 2026 | arXiv 2509.21875 | RAG幻觉检测 | 提出 Lumina 框架，通过\"上下文-知识信号\"检测RAG系统中的幻觉：用MMD度量**外部上下文利用**程度，用跨层token预测演化度量**内部知识利用**程度，无需超参调优即可泛化。"
tags:
  - ICLR 2026
  - RAG幻觉检测
  - 外部上下文利用
...
1. **超参依赖严重**：需要选择特定的attention head和transformer层来计算分数，选择过程需大量调优，且参数因数据集和模型而异
2. **缺乏验证**：虽展示了分数与幻觉的相关性，但未验证分数是否真正反映了外部上下文/内部知识的"利用程度"
