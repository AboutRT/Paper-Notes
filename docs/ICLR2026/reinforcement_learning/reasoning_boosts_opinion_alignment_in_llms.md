...
description: >-
  [ICLR2026][LLM对齐] 用GRPO强化学习训练LLM通过结构化推理从政治调查数据中学习个体观点对齐，SFT+GRPO在美/德/瑞三国数据集上一致超越ICL和ORPO基线，但揭示系统性意识形态偏差和Neutral预测困难。
tags:
  - ICLR2026
  - opinion_alignment
  - GRPO
...
**本文要解决什么？** 能否通过 RL 训练让 LLM 学会"先推理再回答"从而提升个体级政治观点对齐？

**切入角度**：将观点形成视为推理问题——借鉴 GRPO 在数学推理中的成功，迁移到政治推理场景。

**核心idea一句话**：政治调查数据 + GRPO 奖励正确立场 + SFT 热启动推理格式 = 推理式个体观点对齐。
...
3. **SFT 热启动 + 合成论证数据**
    - 做什么：用 Llama-70B 为每个政策问题生成支持/反对论证，构建 SFT 数据训练模型掌握推理格式
