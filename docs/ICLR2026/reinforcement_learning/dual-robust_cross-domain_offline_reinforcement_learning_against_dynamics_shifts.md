...
**本文要解决什么？** 设计一个统一框架，同时保证：(1) 安全地利用源域数据而不引入 OOD 动力学导致的 Q 值高估（train-time robustness）；(2) 学到的策略在部署环境动力学偏移时仍保持性能下界（test-time robustness）。
...
**核心idea一句话**：用一个 RCB 算子统一双重鲁棒性——对源域数据做鲁棒 Bellman backup、对目标域数据做标准 backup，并通过 Wasserstein 对偶将动力学不确定性转化为可操作的状态扰动。
