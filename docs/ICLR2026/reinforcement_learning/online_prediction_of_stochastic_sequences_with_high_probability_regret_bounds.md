---
description: "【论文笔记】Online Prediction of Stochastic Sequences with High Probability Regret Bounds 论文解读 | ICLR 2026 | arXiv 2602.16236 | 在线预测 | 重新审视有限时间范围 $T$ 下随机序列的通用预测经典问题，首次给出以高概率成立的消退遗憾界（形式为 $O(T^{-1/2}\delta^{-1/2})$），与已有的期望遗憾界 $O(T^{-1/2})$ 形式高度一致，并证明在不附加额外假设时 $\delta$ 的指数无法改进。"
tags:
  - ICLR 2026
  - 在线预测
  - 随机序列
...
1. **尾部风险不可控**：期望界只保证"平均"表现良好，但不排除在某些（可能概率不低的）实现路径上遗憾极大
2. **不适合风险敏感场景**：在安全关键、金融风控等应用中，需要的是"除了极小概率事件外，遗憾一定不超过某个阈值"这类更强的高概率保证
3. **理论空白**：是否能在不显著牺牲收敛率的情况下，将期望界提升为高概率界，这一问题此前尚未解决
