...
1. **单层语义局限**：现有方法要么只用高层描述（如 SemFew 生成类别描述），要么只用低层属性（如 ECER 生成具体实体），无法同时提供细粒度区分和整体类别理解
2. **静态融合模块**：现有方法使用固定的 MLP 融合跨模态信息，无法在不同网络深度自适应地调整视觉-语言对齐策略——浅层应关注局部细节、深层应强调全局语义
