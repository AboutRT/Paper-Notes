...
description: >-
  [ICLR 2026][图推理] 提出 GraphOmni 基准框架，在 7 种图类型 × 7 种序列化格式 × 9 种提示策略的 241K 查询上系统评估 11 个 LLM 的图论推理能力，揭示三维度间的复杂交互效应，并设计 RL 引导的组合搜索方法以 25% 成本保持约 90% 最优准确率。
tags:
  - ICLR 2026
  - 图推理
  - LLM基准测试
...
**本文要解决什么？** (1) 构建一个覆盖三维度完整笛卡尔积的大规模基准；(2) 系统量化图类型、序列化格式、提示策略之间的交互效应；(3) 提供一种自动化的最优组合搜索机制。

**切入角度**：作者的核心观察是"三个维度并非独立——一种对开源模型有效的序列化格式在闭源模型上可能反而有害"。因此必须做完整的网格评估(full factorial evaluation)才能得到可信结论。

**核心idea一句话**：通过 7×7×9 的全因子组合评估 + RL 引导的组合搜索，首次系统刻画 LLM 图推理中"图结构 × 文本表示 × 提示方式"三维交互的完整图景。

## 方法详解

### 整体框架

GraphOmni 由四个可插拔模块组成：**图论任务**（6 类经典图问题）→ **图生成器**（7 种图类型）→ **序列化格式**（7 种文本表示）→ **提示方案**（9 种指令策略）。每个查询是这四个维度的一个具体组合加上三级难度（Easy 5-10 节点 / Medium 10-20 节点 / Hard 20-30 节点），总计产生 241,726 个查询实例。评估对象包括 7 个开源模型（Llama-3、Llama-3.1、Mistral、Phi-4、Qwen-2.5 7B、Qwen-2.5 72B、Qwen-3 8B）和 4 个闭源模型（Claude-3.5、GPT-4o、GPT-4o-mini、Gemini-2.0、o4-mini）。框架设计为模块化和可扩展的，研究者可以方便地添加新的图生成器、序列化方法或提示策略。

### 关键设计

1. **6 类图论任务的能力分层**:

    - 这 6 个任务经过精心选择以覆盖不同推理能力维度：**局部属性** 包含连通性判断（测试局部链接理解）和环检测（测试循环模式识别）；**全局属性** 包含直径计算（需要计算所有节点对之间最短路径的最大值）和 BFS 遍历（测试有序序列生成）；**数值/路径** 包含最短路径（路径规划能力）和三角形计数（精确枚举三元组，最具挑战性）
    - 每个任务有专门的随机基线设计：例如连通性的随机基线约 67%（反映真实图多数连通的先验），环检测为 50%，三角形计数仅约 2%

2. **7 种图生成器的拓扑多样性**:

    - 覆盖 Erdős-Rényi 随机图的两个变体（ERM 固定边数 / ERP 概率边），二部 ER 图的两个变体（BERM/BERP 施加二部约束），Barabási-Albert 无标度图（BAG 优先连接 / BAF 森林变体），以及 Scale-Free 图（SF 度加权随机连接）
    - 这种设计确保了从均匀随机结构到幂律分布到层次树状结构的全面覆盖，且作者在附录中用统计检验证实这些生成器在 5-30 节点范围内确实产生了结构显著不同的图

3. **序列化-提示的全因子交叉**:

    - 7 种序列化格式从简洁到冗余排列：Edge Set/List（紧凑边表示）、Adjacency Set/List/Matrix（邻接表示）、GMoL/GMaL（结构化标记语言）。不同格式的 token 开销差异巨大（如 Adjacency Matrix 在密集图上 token 数远高于 Edge Set）
    - 9 种提示策略按指导程度分层：从 0-Shot（无任何提示）到 Algorithm（显式写出 BFS/Dijkstra 等算法步骤），中间穿插 CoT、K-Shot、Instruct 及其零样本变体和 LTM（Least-to-Most）
    - 关键是做完整笛卡尔积而非只比较局部组合，这使得 heatmap 分析成为可能

### RL 引导的最优组合搜索（RL-Opt）

受基准实验"没有通用最优组合"这一发现的启发，作者提出了一种实用的自动配置方法。核心思路是将"为特定任务选择最佳 (序列化格式, 提示策略, 模型) 组合"建模为序列决策问题：

- **状态空间**: 当前已选择的因子组合（如已选 Adjacency List 序列化、待选提示策略）
- **动作空间**: 在每一步从对应维度的候选中选择一个选项
- **奖励**: 选定组合在验证集上的准确率
- **搜索空间**: $E = 7 \times 9 \times 5 = 315$ 种组合（5 个开源模型）
- 使用 DQN 学习 Q 函数逼近最优策略，采用 $\varepsilon$-greedy 探索（$\varepsilon$ 从 1.0 衰减至 0.01）
- 目标是在只探索约 25% 组合（$\text{Cost} = k/K \approx 0.22$）的情况下，找到接近全局最优的组合（$\text{Rate} = acc^*/acc_{\max} \approx 0.90$）

### 训练与评估策略

评估采用二值准确率：对于定性任务（连通性、环检测）通过关键短语匹配判断正误；对于定量任务（三角形计数、直径）提取数值与真值比较；对于多解任务（BFS 遍历、最短路径）使用规则验证函数检查解的正确性。所有 few-shot 实验统一使用 $k=5$ 个示例。
