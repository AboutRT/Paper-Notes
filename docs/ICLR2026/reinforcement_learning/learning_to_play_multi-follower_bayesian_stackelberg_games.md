...
**本文要解决什么？** (1) 多追随者BSG的在线学习是否可行？(2) 在类型反馈和动作反馈两种设置下，最优遗憾界分别是多少？(3) 遗憾界能否避免对 $n$ 的指数依赖？
...
**核心idea一句话**：将领导者策略空间按追随者最佳响应几何分割为多面体区域，利用区域数关于 $n$ 多项式可控的性质，设计基于UCB和集中不等式的学习算法，实现遗憾界不随 $n$ 指数增长。
