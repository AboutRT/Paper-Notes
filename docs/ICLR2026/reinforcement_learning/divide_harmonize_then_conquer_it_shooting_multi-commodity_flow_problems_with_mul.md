...
1. **LP 求解器的扩展性瓶颈**：LP 求解复杂度约 $\mathcal{O}(d^{2.3729})$，当变量规模达到百万级时运行时间过长（小时级别），且需准确预测未来需求
2. **ML 方法的局限性**：(a) GNN/RL 等专用网络工程成本高、需反复调参；(b) 对未见环境泛化差；(c) 输出维度随节点数平方增长（$\mathcal{O}(|\mathcal{V}|^2)$），仍有维度诅咒
