...
description: >-
  [ICLR 2026][强化学习][CUDA optimization] 提出 CUDA-L1，一个基于对比强化学习（Contrastive RL）的三阶段流水线框架，将初始 CUDA 能力较弱的 LLM 训练为高效的 CUDA 优化器…
tags:
  - "ICLR 2026"
  - "强化学习"
  - "CUDA optimization"
...
- **功能**：利用 6 个现有 LLM（GPT-4o、o1、DeepSeek-R1/V3、Llama-405B、Claude 3.7）为 KernelBench 的 250 个任务生成 CUDA 代码变体。
...
- **功能**：模型迭代生成 CUDA 代码，评估可执行性和正确性，仅保留成功样本进行训练。
...
- **功能**：核心创新——将历史代码变体及其性能分数嵌入到 prompt 中，让 LLM 进行对比分析后生成更优代码。
...
- **功能**：从历史代码中选择 N=2 个代码作为对比样本放入 prompt。
