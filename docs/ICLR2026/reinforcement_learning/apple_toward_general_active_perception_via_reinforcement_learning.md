...
description: >-
  [ICLR 2026][强化学习][active perception] 提出APPLE——一种结合强化学习与监督学习的通用主动感知框架，将主动感知建模为POMDP，奖励函数设计为RL奖励减去预测损失，梯度自然分解为策略梯度和预测损失梯度两部分…
tags:
  - "ICLR 2026"
  - "强化学习"
  - "active perception"
...
1. **统一的奖励-损失设计**
    ...
    - **功能**：将奖励定义为RL任务奖励减去监督预测损失 $r_t = r_t^{RL} - \mathcal{L}_{pred}(\hat{y}_t, y)$
...
2. **Off-policy算法选择（SAC/CrossQ）**
    ...
    - **功能**：提出APPLE-SAC和APPLE-CrossQ两种变体
...
3. **共享ViViT骨干网络**
    ...
    - **功能**：使用Video Vision Transformer（ViViT）风格的架构作为策略网络和预测网络的共享特征提取器
