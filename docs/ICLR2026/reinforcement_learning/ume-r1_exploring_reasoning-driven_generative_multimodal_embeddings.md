...
description: >-
  [ICLR 2026][多模态] 提出 UME-R1，首次探索推理驱动的生成式多模态嵌入范式，通过两阶段训练（冷启动SFT + 强化学习）让嵌入模型先推理再生成表示，在 MMEB-V2 基准的 78 个任务上显著超越传统判别式嵌入模型。
tags:
  - ICLR 2026
  - 多模态
...
**设计1：双模态嵌入架构**
- **功能**：设计统一模板，让模型同时产出判别式和推理驱动的生成式嵌入
...
**设计2：冷启动数据集构建**
- **功能**：为 1.76M 个 query-target 对生成 CoT 标注
...
- [Exploring Large Action Sets with Hyperspherical Embeddings using von Mises-Fisher Sampling](../../ICML2025/reinforcement_learning/exploring_large_action_sets_with_hyperspherical_embeddings_using_von_mises-fishe.md)
