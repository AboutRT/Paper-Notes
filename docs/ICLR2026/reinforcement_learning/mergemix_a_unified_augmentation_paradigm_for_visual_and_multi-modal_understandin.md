...
1. **领域现状**：MLLM对齐分SFT（需标注+缺偏好建模）和RL（需奖励模型+不稳定）两条路线。偏好优化(DPO等)通过构建偏好对桥接两者，但如何高质量构建偏好对是核心问题。

2. **现有痛点**：(1) SeVa用随机增强(RandomCrop)构建"输者"→增强质量不可控→"输者"可能太差或太好；(2) DPO的margin是固定的→无法与数据质量关联；(3) 现有Mixup方法要么基于显著性（慢）要么随机（效果不稳）。

3. **切入角度**：Token Merge天然产出注意力聚类信息→用作Mixup的掩码→混合比与合并比关联→混合图像质量可控→混合比自然作为偏好margin。
