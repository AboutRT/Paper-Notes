...
1. **有界源分布的BC流策略**:
    ...
    - 功能：学习流策略 $\psi_{\theta_1}(t,z;s)$，将有界均匀分布 $q_{BC} = \mathcal{U}(\mathcal{B}_l^d)$ 映射到行为策略
...
2. **反射流噪声生成器**:
    ...
    - 功能：学习另一个流模型 $\psi_{\theta_2}(t,w;s): \mathcal{B}_l^d \to \mathcal{B}_l^d$ 生成超球内的多模态噪声
...
3. **策略蒸馏**:
    ...
    - 功能：将多步BC流策略蒸馏为单步映射 $\hat{\mu}_{\hat{\theta}_1}$
