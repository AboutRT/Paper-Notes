---
description: "【论文笔记】ToolTree: Efficient LLM Agent Tool Planning via Dual-Feedback Monte Carlo Tree Search and Bidirectional Pruning 论文解读 | ICLR 2026 | arXiv 2603.12740 | Tool Planning | 提出 ToolTree，一种基于 MCTS 的 LLM Agent 工具规划框架，通过执行前/后双阶段评估和双向剪枝机制，在固定计算预算下实现前瞻性工具选择，在 4 个 benchmark 上平均提升约 10%。"
tags:
  - ICLR 2026
  - Tool Planning
  - MCTS
...
1. **领域现状**：LLM Agent 在多步骤复杂任务中需要调用外部工具链（API、搜索、计算器等），核心挑战是工具规划——决定用什么工具、什么顺序、什么参数。
2. **现有痛点**：(a) 贪心方法（ReAct、CoT）逐步选择"当前最优"工具，缺乏前瞻，早期错误不可逆地向后传播；(b) 搜索方法（ToT、A*）展开多候选分支但分支因子随工具数指数增长，计算成本高且评估基于假设性思维而非实际执行结果。
3. **核心矛盾**：搜索方法虽然有前瞻性但计算代价大且评估不接地（evaluate hypothetical thoughts）；贪心方法高效但缺乏纠错能力。需要一种既有前瞻能力又基于实际执行反馈的方法。
4. **本文要解决什么？** 在固定计算预算下，如何让 Agent 进行前瞻性工具规划，同时保证效率？
5. **切入角度**：将 MCTS 的 selection-expansion-simulation-backpropagation 循环改造为适合工具调用的框架，执行前用 LLM 快速预评估筛选分支，执行后用实际输出评分修正策略。
6. **核心idea一句话**：双阶段评估（pre-execution 预判 + post-execution 实测）+ 双向剪枝（执行前砍低分支 + 执行后砍失败分支），让 MCTS 在工具规划场景下既高效又准确。
...
1. **先验增强的 UCT 选择 (Prior-Augmented Selection)**:
   - 做什么：在标准 UCT 公式中加入预评估分数 $r_{\text{pre}}$ 作为探索引导
...
2. **Pre-Evaluation（执行前预评估）**:
   - 做什么：在实际调用工具前，用 LLM 判定该工具在当前上下文中的适用性，输出 $r_{\text{pre}}(s,a) \in [0,1]$
...
3. **Post-Evaluation（执行后评估）**:
   - 做什么：工具执行后，用 LLM 评估实际输出的质量 $r_{\text{post}}(s,a) \in [0,1]$
...
4. **双向剪枝 (Bidirectional Pruning)**:
   - 做什么：执行前剪枝（pre-pruning）移除低 $r_{\text{pre}}$ 的候选并只保留 top-$K$；执行后剪枝（post-pruning）关闭低 $r_{\text{post}}$ 的分支
   - 设计动机：两阶段互补——前者基于预判减少候选，后者基于证据消除死胡同，共同将预算集中在最有前途的轨迹上

5. **确定性缓存 + 错误处理**:
   - 做什么：同一 rollout 内相同 (tool, args) 调用复用结果；失败调用附加错误 token
