...
1. **隐式推理链脆弱**：LLM 必须在内部桥接不同段落间的逻辑间隙，失败时导致幻觉
2. **现有 KG-RAG 方法依赖静态全局图**：如 GraphRAG 需要对全库建图，Wikipedia 2018 就需要数百万次 LLM 调用、数万美元成本
3. **全局图质量问题**：混合多个文档的证据，可能包含矛盾或模糊关系（如同一药物的正负关联）
