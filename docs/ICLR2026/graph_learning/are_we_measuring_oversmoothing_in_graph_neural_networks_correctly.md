...
1. **有效秩（Effective Rank）作为度量**
    ...
    - **功能**：使用特征矩阵奇异值的归一化熵定义有效秩 $\text{Erank}(X) = \exp\left(-\sum_i \hat{\sigma}_i \log \hat{\sigma}_i\right)$，其中 $\hat{\sigma}_i = \sigma_i / \sum_j \sigma_j$
...
2. **Dirichlet Energy失效的实证分析**
    ...
    - **功能**：在多种GNN架构和数据集上系统比较Erank与Dirichlet energy同准确率的相关性
    - **核心思路**：计算每种度量在不同深度下与节点分类准确率的Pearson/Spearman相关系数
    - **设计动机**：如果一个度量声称衡量过平滑，那它应当与模型性能高度负相关（过平滑越严重，性能越差）

3. **理论收敛证明**

    - **功能**：证明对包括GCN、GraphSAGE、GIN等在内的广泛GNN架构族，随着层数趋于无穷，特征矩阵的数值秩收敛到1
