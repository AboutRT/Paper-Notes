...
**本文要解决什么？** (1) 如何在不用神经网络的前提下从 SW 距离精确恢复 Wasserstein 距离？(2) 如何仅用极少量（10-100 对）的精确 Wasserstein 标注就训练出好的 estimator？(3) 能否与现有深度方法（Wormhole）结合实现加速？
...
**核心 idea 一句话**：用 6 种 SW 距离变体（3 个下界 + 3 个上界）作为特征，通过最小二乘线性回归拟合真实 Wasserstein 距离，闭式解训练、无需迭代、无需神经网络。
