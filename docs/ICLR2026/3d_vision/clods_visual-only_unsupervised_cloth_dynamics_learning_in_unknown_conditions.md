...
description: >-
  [ICLR 2026][3D视觉][布料动力学] CloDS 提出首个从多视角视频中无监督学习布料动力学的框架，通过 Spatial Mapping Gaussian Splatting 建立 2D 图像到 3D 网格的可微映射，结合双位置不透明度调制解决自遮挡问题…
tags:
  - "ICLR 2026"
  - "3D视觉"
  - "布料动力学"
...
2. **双位置不透明度调制 (Dual-Position Opacity Modulation)**:
    ...
    - 功能：动态控制每个 Gaussian 组件的不透明度 $\alpha_{i,t} = f_\theta(\mu_{i,t}^W, \mu_{i,t}^M)$
...
3. **逆渲染恢复 3D 标签**:
    ...
    - 功能：通过反向传播从 2D 图像中恢复 3D mesh 节点位置
