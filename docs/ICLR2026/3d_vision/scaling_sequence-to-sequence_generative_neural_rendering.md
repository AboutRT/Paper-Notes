...
1. **逐场景优化方法**（如 3DGS、NeRF）：需要大量输入视图和逐场景优化，计算成本高
2. **前馈推理方法**（如 PixelNeRF、LRM）：依赖显式 3D 表示（点云、三平面等），泛化能力受限
3. **生成方法**（如 Zero123++、SV3D）：采用扩散模型进行视角合成，但在少视图设置下质量不稳定
4. **3D 数据稀缺**：带相机标签的 3D 数据集规模远小于视频数据集
