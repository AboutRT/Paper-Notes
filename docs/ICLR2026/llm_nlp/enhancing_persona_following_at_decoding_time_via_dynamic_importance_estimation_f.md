...
1. **缺乏动态适应性**：心理学研究（如 CAPS 理论）表明人设对行为的影响是**场景依赖**的——同一个人在不同情境下激活不同的人格特质。但无论是 prompt 工程（直接提示、ICL、RAG）还是参数训练（SFT、LoRA），都将人设视为静态的
2. **数据依赖严重**：参数方法需要大量角色特定对话数据，而社会模拟中角色多样、人格复杂，数据收集极其困难
