...
description: >-
  [ICLR 2026][LLM/NLP][不确定性估计] 从 proper scoring rules 框架出发，证明最高概率输出序列的负对数似然（MSP）是理论上合理的不确定性度量，并提出 G-NLL——仅用一次贪心解码就能逼近该度量，在多个场景下匹配或超越需要多次采样的 SOTA 方法。
tags:
  - "ICLR 2026"
  - "LLM/NLP"
  - "不确定性估计"
...
1. **Zero-one Scoring Rule 导出 MSP**:
    ...
    - 功能：从 proper scoring rule 框架推导出最高序列概率（MSP）作为偶然不确定性度量
...
3. **采样复杂度理论分析（Theorem 1）**:
    ...
    - 功能：证明近似 $M(p(\bm{y}))$ 的采样复杂度远优于近似 $H(p(\bm{y}))$
