---
description: "【论文笔记】Meta-RL Induces Exploration in Language Agents 论文解读 | ICLR 2026 | arXiv 2512.16848 | Meta-RL | 提出 LaMer 框架，将元强化学习（Meta-RL）引入 LLM agent 训练，通过跨 episode 的奖励优化和基于反思的上下文策略适应，使语言智能体学会主动探索环境，在 Sokoban/MineSweeper/Webshop 上分别获得 11%/14%/19% 的绝对性能提升。"
tags:
  - ICLR 2026
  - Meta-RL
  - LLM Agent
...
1. **Prompting 方法**（Zero-shot、ReAct、Reflexion）：依赖冻结的 LLM，探索行为有限，性能天花板低
2. **标准 RL 训练**（PPO、GRPO、GiGPO）：每个 episode 独立采样，策略固定，无法在测试时通过试错进行适应
3. **离线蒸馏方法**：依赖离线数据，只能模仿而非主动探索；多聚焦于单轮推理而非多轮 agent 任务
