...
**本文要解决什么？** 在LVLM中，外部视觉线索改善推理的因果机制是什么？具体分解为：(1) 结构化输入是否诱导了显式的跨模态绑定标识符？(2) 这些标识符是否因果性地决定了模型预测？(3) 这种增强的绑定是否转化为下游任务（幻觉缓解、视觉推理）的实际收益？
...
**核心idea一句话**：简单的对齐外部线索（图像分区+符号标记）在LVLM内部诱导出Grounding IDs——因果性地驱动跨模态绑定的潜在标识符，从而解释并增强了外部线索的推理改善效果。
