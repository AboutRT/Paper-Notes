...
1. **端到端VLM**：隐式推理过程难以审计，在涉及空间关系、计数等复杂查询时容易产生幻觉
2. **组合式方法**（如ViperGPT、HYDRA）：提高了可解释性，但多数采用单Agent或手工设计的流水线
3. **多Agent方法**：各Agent被分配不相交角色并硬编码管道连接，无法处理错误传播，不支持功能重叠Agent间的竞争
4. **规则转移的僵化性**：手写规则转移函数随状态增长变得难以定义
