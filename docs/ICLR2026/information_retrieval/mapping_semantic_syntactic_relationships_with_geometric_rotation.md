...
description: >-
  [ICLR 2026][嵌入几何][可解释性] 提出RISE(Rotor-Invariant Shift Estimation)方法，将话语级语义-句法变换（否定、条件、礼貌）建模为嵌入空间超球面上的一致旋转操作，首次证明这些变换可跨7种语言(5个语系)和3种嵌入模型泛化，将线性表示假说从词级扩展到跨语言话语级。
tags:
  - ICLR 2026
  - 嵌入几何
  - 旋转不变性
...
1. **Rotor规范化与原型学习**：
    ...
    - 做什么：对每对中性-变换句子嵌入 $(n_i, v_i)$，计算将 $n_i$ 映射到参考方向 $e_1$ 的正交变换 $R(n_i)$，然后将所有变换的切向量在参考坐标系中取平均得到原型 $\vec{p}$。
...
2. **跨语言与跨模型迁移**：
    ...
    - 做什么：在某种语言A上学习的旋转原型，直接应用到语言B的嵌入上评估跨语言迁移；利用统计映射（Morris等的PCA+分布对齐）将text-embedding-3-large的原型迁移到bge-m3空间评估跨模型迁移。
...
3. **交换律验证与理论支撑**：
    ...
    - 做什么：证明连续应用多个RISE变换（如先否定再条件化 vs 先条件化再否定）的结果在一阶近似下交换。
