...
description: >-
  [ICLR 2026][选择性预测] 提出 MA-PaPSP 框架，通过外部检索数据集构建代理嵌入（k-NN 加权平均降低表示方差）+ 对比归一化评分（改善校准），无训练地为任意 VLM 提供可靠的"拒绝回答"能力，在图像描述、图文匹配、分类的选择性预测上全面优于 PaPSP 和 LLM-as-judge 基线。
tags:
  - ICLR 2026
  - 选择性预测
  - 信息检索
...
1. **Proxy Embeddings（代理嵌入）**:
    ...
    - **功能**: 将查询的嵌入替换为其 K 近邻嵌入的加权平均，得到更稳定的"代理嵌入"
...
2. **Contrastive Scores（对比评分）**:
    ...
    - **功能**: 用硬负例归一化相似度分数，得到校准的 [0,1] 区间置信度
