...
HUME 建立在 MTEB 之上，包含四个核心组件：(1) 任务特定标注界面（基于 Argilla），(2) 有原则的数据集采样（5 维度选择准则），(3) 标准化结果格式，(4) 与模型评估完全对齐的评估指标。人类和模型在完全相同的下采样实例上评估，使用相同的指标和协议。

### 关键设计

1. **数据集选择策略（5 维度覆盖）**:
    - 功能：确保发现具有泛化性
    - 核心思路：语言多样性（英/阿/俄/挪/丹，资源等级覆盖 1-5 级）、领域多样性（新闻/社交/百科/科学/论坛）、构建方式（人工+合成）、任务相关性、复杂度变化
    - 设计动机：避免仅在英语高资源数据集上得出偏颇结论

2. **标注流程与对齐设计**:
    - 功能：确保人类-模型比较的公平性
    - 核心思路：使用 Argilla 标注工具，样本量 20-50/task；英语任务配 2 位标注者以计算一致性，多语言任务由母语者标注；标注指令严格匹配原始任务定义（相同标签集、相同 1-5 量表）
    - 设计动机：消除标注指令差异导致的不可比性

3. **重排序代理设计**:
    - 功能：使信息检索任务对人类可行
    - 核心思路：直接检索需评估数千候选文档不可行，用重排序（评估 top-k 候选）作为语义等价代理
    - 设计动机：保留核心语义挑战的同时使人类评估可操作

### 损失函数 / 训练策略

作为评估框架，HUME 不训练模型。统计显著性通过 Wilson Score Intervals（分类准确率）和 Fisher $z$ 变换（相关性指标）计算 95% 置信区间。模型在 14/26 任务上落在人类置信区间之外 ($p<0.05$)。
...
- 新颖性: ⭐⭐⭐⭐ 首次系统化的嵌入任务人类基线，问题定义清晰但方法相对标准
- 实验充分度: ⭐⭐⭐⭐ 16 个数据集 × 13 个模型 × 9 个 LLM 全面评估，但样本量偏小
- 写作质量: ⭐⭐⭐⭐⭐ 分析深入透彻，洞察力强，结论有理有据
- 价值: ⭐⭐⭐⭐⭐ 直接改变 MTEB 排行榜的解读方式，已集成到开源框架
