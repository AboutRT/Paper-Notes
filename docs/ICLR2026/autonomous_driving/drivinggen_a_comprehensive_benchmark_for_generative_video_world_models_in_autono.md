...
1. **多样化评估数据集**:
    ...
    - 功能：构建覆盖极端天气（雪、雾、沙尘暴、洪水）、多时段（黎明、夜晚）、全球地理（北美/欧洲/亚洲/非洲/中东等）和复杂交互（密集交通、行人过马路、强行并线）的 400 样本数据集。
...
2. **Fréchet Trajectory Distance (FTD)**:
    ...
    - 功能：新提出的分布级轨迹评估指标，类比 FVD 但作用于轨迹空间。
...
3. **智能体异常消失检测**:
    ...
    - 功能：检测生成视频中车辆/行人的非物理消失现象（如不在视野边缘也不被遮挡就凭空消失）。
