...
description: >-
  [ICLR 2026][自动驾驶][扩散桥模型] BridgeDrive 提出用扩散桥（diffusion bridge）替代截断扩散来实现锚点引导的自动驾驶轨迹规划，保证前向/反向过程的理论对称性，在 Bench2Drive 闭环评估中成功率达到 74.99%（PDM-Lite）和 89.25%（LEAD）…
tags:
  - "ICLR 2026"
  - "自动驾驶"
  - "扩散桥模型"
...
1. **扩散桥公式化**:
    ...
    - 功能：将规划任务定义为从锚点 $x_T = y$ 到真实轨迹 $x_0 = x$ 的扩散桥，保证前后向过程对称。
...
2. **几何路径点表示 (Geometric Path Waypoints)**:
    ...
    - 功能：用等几何间距的路径坐标 + 标量速度 $(x^{\text{geo}}, v) \in \mathbb{R}^{N \times 2} \times \mathbb{R}$ 替代等时间间距的速度路径点 $x^{\text{temp}} \in \mathbb{R}^{N \times 2}$。
...
3. **锚点分类器**:
    ...
    - 功能：训练时使用最近邻锚点作真值，推理时用分类器 $h_\phi(z, \mathcal{Y})$ 预测最优锚点。
