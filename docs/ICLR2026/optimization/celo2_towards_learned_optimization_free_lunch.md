...
1. **元泛化（Meta-generalization）困难**: 在小规模任务上元训练的优化器往往无法泛化到大规模任务。VeLO 是此前最强的学习型优化器，尽管投入了 4000 TPU-month（约 10× GPT-3 训练量）的元训练计算，但仍未能泛化到超过 600M 参数的任务。
2. **元训练成本高昂**: VeLO 的 4000 TPU-month 计算量使得学习型优化器的研究迭代极为缓慢。
3. **稳定性不足**: 学习型优化器在超出训练分布时容易出现不稳定的训练动态，限制了实际采用。
