...
1. **MIP 方法**（Bertsimas & Dunn 2019）：可保证全局最优但计算不可扩展，搜索空间与样本量相关
2. **DPB 方法**（Zhang et al. 2023, 当前 SOTA）：可扩展到 200 万样本，但**必须将连续特征二值化**，损失了全局最优性且可能导致不必要的深树
3. **进化搜索**（evtree）：无最优性保证
4. **其他 MIP 变体**：限制树的大小或分裂数量以保证可扩展性
