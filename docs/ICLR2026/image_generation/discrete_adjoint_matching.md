...
DAM 的输入是一个预训练的基于 CTMC 的离散生成模型（如 LLaDA 扩散式 LLM）和一个奖励函数 $r(x)$（设 $g(x) = -r(x)$），输出是微调后的模型，使其生成更高奖励的样本。整体流程：（1）分析推导出最优 CTMC 速率 $u_t^\star(y,x)$ 的解析形式；（2）用 Dynkin 公式构造离散伴随变量 $\tilde{a}_t$ 作为最优速率的无偏估计器；（3）通过 Bregman 散度匹配框架训练参数化模型逼近最优速率；（4）利用 masked diffusion 的特殊结构和重要性采样降低方差。
...
- [DiFlowDubber: Discrete Flow Matching for Automated Video Dubbing via Cross-Modal Alignment and Synchronization](../../CVPR2026/image_generation/diflowdubber_discrete_flow_matching_for_automated_video_dubbing_via_cross-modal_.md)
