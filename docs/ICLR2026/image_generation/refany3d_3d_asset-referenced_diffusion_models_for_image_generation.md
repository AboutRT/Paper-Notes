...
现有的参考图像生成方法（如 IP-Adapter、OminiControl）依赖 2D 参考图像，无法有效利用 3D 资产。在实际应用中，创作者通常需要直接使用网格等 3D 资产作为参考来可视化物体在不同场景中的表现。

3D 资产参考生成面临三大挑战：
1. **一致性不足**：需要与 3D 资产的几何结构和纹理精确对齐
2. **视角受限**：单参考图像方法无法捕获物体的完整外观
3. **视角冲突**：多图像条件方法缺乏 3D 结构先验，导致跨视角不一致
