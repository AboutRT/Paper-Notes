...
description: >-
  [ICLR 2026][图像生成][solution function] 提出 Solution Flow Models (SoFlow)，直接学习速度 ODE 的解函数 $f(x_t, t, s)$（将 $t$ 时刻的 $x_t$ 映射到 $s$ 时刻的解）…
tags:
  - "ICLR 2026"
  - "图像生成"
  - "solution function"
...
1. **解一致性损失（无需 JVP）**:
    ...
    - 功能：保证 $f_\theta$ 满足 ODE 的传递性 $f(x_t, t, s) = f(f(x_t, t, l), l, s)$
...
2. **Flow Matching 损失（提供速度场+CFG）**:
    ...
    - 功能：解函数在 $s=t$ 附近的行为等价于速度场，因此可以同时训练速度预测
