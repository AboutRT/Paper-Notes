---
description: "【论文笔记】Zatom-1: A Multimodal Flow Foundation Model for 3D Molecules and Materials 论文解读 | ICLR 2026 | arXiv 2602.22251 | 基础模型 | Zatom-1是首个端到端全开源的基础模型，通过多模态流匹配(multimodal flow matching)统一了3D分子和材料的生成建模与属性预测，使用标准Transformer架构在欧几里得空间直接建模离散原子类型和连续3D几何，实现了跨化学域的正迁移学习。"
tags:
  - ICLR 2026
  - 基础模型
  - 流匹配
...
1. **领域现状**：AI驱动的化学建模已取得重大突破（AlphaFold等），但现有方法通常针对单一领域（分子或材料）和单一任务（生成或预测）进行优化，限制了表示共享和迁移学习。

2. **现有痛点**：(1) 分子和材料的生成模型分开训练，无法利用跨域数据的互补信息。(2) 稀疏图神经网络架构和手工设计的生成先验限制了扩展性和推理速度。(3) 生成和预测任务使用不同模型，无法共享表示。

3. **核心矛盾**：3D化学系统包含离散（原子类型）和连续（3D坐标、晶格参数）多种模态，如何在统一框架下同时建模这些模态？如何让生成预训练为下游预测任务提供好的初始化？

4. **本文要解决什么**：构建一个统一的基础模型，能同时进行3D分子和材料的生成建模和表示学习，实现跨域正迁移。

5. **切入角度**：将生成建模视为化学表示学习的理想预训练任务，采用标准Transformer架构 + 多模态流匹配，在环境全原子空间 $\mathbb{R}^3$ 中直接建模。

6. **核心idea一句话**：用多模态流匹配在统一的Transformer中联合建模离散原子类型和连续3D几何，预训练后可微调用于多任务属性预测。
