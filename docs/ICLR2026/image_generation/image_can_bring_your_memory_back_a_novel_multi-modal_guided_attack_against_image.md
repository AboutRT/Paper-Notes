...
description: >-
  [ICLR 2026][图像生成][机器遗忘攻击] Recall 提出首个多模态引导的攻击框架，通过在隐空间中优化对抗图像 prompt（仅需一张参考图像），配合原始文本 prompt 利用扩散模型的 image-conditioning 通道…
tags:
  - "ICLR 2026"
  - "图像生成"
  - "机器遗忘攻击"
...
1. **噪声初始化策略**：
    ...
    - 功能：构建初始图像 prompt $P_{img}^{init} = \lambda \cdot P_{ref} + (1-\lambda) \cdot \delta$，其中 $\delta \sim \mathcal{N}(0, I)$，$\lambda = 0.25$
    - 核心思路：掺入大量噪声（75%）扩大采样空间、增加生成多样性，同时保留少量参考信号提供概念种子
    - 设计动机：避免生成图像仅是参考图像的变换，确保输出多样且跟随文本 prompt

2. **迭代隐空间优化**：

    - 功能：沿 50 步 DDIM 时间步，每步做 20 次梯度迭代优化 $z_{adv}$
