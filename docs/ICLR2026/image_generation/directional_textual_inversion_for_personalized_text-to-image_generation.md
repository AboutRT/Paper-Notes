...
description: >-
  [ICLR 2026][图像生成][Textual Inversion] 本文发现 Textual Inversion (TI) 学到的 token embedding 存在范数膨胀（norm inflation）问题，导致复杂 prompt 的文本对齐下降…
tags:
  - "ICLR 2026"
  - "图像生成"
  - "Textual Inversion"
...
1. **超球面方向优化（Riemannian SGD）**:
    ...
    - 功能：在 $\mathbb{S}^{d-1}$ 上优化 embedding 方向，避免范数膨胀
...
2. **von Mises-Fisher (vMF) 方向先验**:
    ...
    - 功能：将方向优化视为 MAP 估计，引入 vMF 分布作为先验，防止语义漂移
...
3. **范数尺度选择**:
    ...
    - 功能：将 $m^\star$ 固定为预训练词汇表 embedding 的均值范数
...
- [\[CVPR 2025\] PersonaBooth: Personalized Text-to-Motion Generation](../../CVPR2025/image_generation/personabooth_personalized_text-to-motion_generation.md)
