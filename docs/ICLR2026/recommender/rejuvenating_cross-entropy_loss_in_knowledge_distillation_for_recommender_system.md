...
description: >-
  [ICLR 2026][推荐系统] 理论证明 CE 损失在推荐系统 KD 中最大化 NDCG 下界需满足"闭合性假设"（子集需包含学生 top 项目），但实际目标是蒸馏教师 top 项目的排序——两者冲突导致 vanilla CE 表现差。据此提出 RCE-KD：将教师 top-K 项目按是否在学生 top-K 中分两组，分别用精确 CE 和采样近似闭合性 CE，自适应融合权重随训练动态调整。
tags:
  - ICLR 2026
  - 推荐系统
  - cross-entropy
...
1. **理论基础：CE → NDCG 在 KD 中的推广**
    ...
    - 功能：证明 CE 损失与 NDCG 在推荐 KD 场景中的连接
...
2. **分裂策略**
    ...
    - 功能：将教师 top-K 按是否也在学生 top-K 中分为两组
...
3. **自适应采样策略（for $\mathcal{L}_2$）**
    ...
    - 功能：对 $(\mathcal{Q}_u^T)_2$ 中的每个项目 $i$，找到学生排名高于 $i$ 的所有项目并提高其采样概率，采样 $L$ 个与 $(\mathcal{Q}_u^T)_2$ 合并
