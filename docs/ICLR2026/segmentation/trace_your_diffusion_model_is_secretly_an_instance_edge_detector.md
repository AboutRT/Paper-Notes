---
description: >-
  [ICLR2026][分割] 发现扩散模型自注意力在去噪过程中短暂但可靠地展现实例级结构，提出TRACE框架利用IEP+ABDiv提取实例边缘并蒸馏为单步预测器，无监督实例分割提升+5.1 AP，tag监督全景分割超越点监督方法。
tags: [ICLR2026, 分割, 扩散模型, 实例边缘, 无监督分割, 自注意力, 全景分割]
---
...
**本文要解决什么**: 寻找一种无需标注的实例级信号源来补充语义特征的实例分离能力。

**切入角度**: 扩散模型在去噪过程中从噪声→实例结构→语义内容渐进演变——在特定时步，自注意力短暂但清晰地编码了实例边界。

**核心idea一句话**: 扩散模型的自注意力是隐藏的实例边缘标注器——跨边界的注意力分布剧烈散度变化就是实例边界信号。
