...
1. **问题定义**：图域适应(GDA)旨在将有标注源图(source graph)的知识迁移到无标注目标图(target graph)，解决跨域分布偏移问题，本文聚焦节点级GDA任务。

2. **现有范式**：GDA两大范式——(1) 模型导向方法(学习域不变表示，如MMD、对抗训练)，假设结构变化有限，在大结构差异下失效；(2) 数据导向方法(构建中间图桥接结构gap)，依赖离散对齐步骤，灵活但受限于步数固定。

3. **离散对齐的根本局限**：真实世界图的结构受社交动态、引用增长、知识扩散等复杂过程驱动，演化是连续且非线性的。固定步数的离散对齐无法逼近实际的变换过程，特别是面对无标注图时，结构和语义缺乏显式锚点可供对齐。

4. **连续演化的优势**：连续时间建模(1)将结构变化表示为平滑时序轨迹，灵活适配非线性异构拓扑；(2)语义信息沿变换路径连续演化，模型自动学习最优对齐轨迹。

5. **扩散模型的契机**：扩散模型在捕捉复杂分布变换方面已取得巨大成功，SDE框架可将跨图迁移表示为连续概率流——自然统一结构与语义适应。

6. **研究空白**：现有图扩散模型主要关注对称扩散过程(生成任务)，GDA需要非对称扩散(从源域到目标域的定向迁移)，此前无人探索将扩散引入GDA领域。
