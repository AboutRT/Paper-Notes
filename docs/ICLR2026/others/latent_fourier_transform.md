...
description: >-
  [ICLR2026][音乐生成][扩散模型] 在扩散自编码器潜在空间中应用傅里叶变换，通过频率域掩码实现对音乐不同时间尺度结构的细粒度控制
tags:
  - ICLR2026
  - 音乐生成
  - 扩散模型
...
**本文要解决什么？** 提供一个基于潜在频率域的新控制轴，让用户像使用均衡器调控音色一样调控音乐结构。

**切入角度**：利用傅里叶变换天然的"按频率分解"特性——不同频率分量正交、独立修改不互相干扰——将其应用到学到的潜在时间序列上。

**核心idea一句话**：在潜在空间做 DFT，用频率掩码控制音乐生成——"音乐结构的均衡器"。
...
1. **潜在空间离散傅里叶变换 (Latent DFT)**:
    - 做什么：对编码器输出的潜在时间序列 $\mathbf{z}$ 沿时间轴做 DFT，得到潜在频谱 $\mathbf{Z} = \text{DFT}(\mathbf{z})$
...
2. **随机相关对数频率掩码训练策略**:
    - 做什么：训练时对潜在频谱应用 0-1 随机掩码 $\mathbf{M}$，掩码的生成过程经过相关+对数变换，使得相邻频率 bin 倾向于同时被掩/保留，且高频 bin 组更宽
...
3. **扩散自编码器解码器**:
    - 做什么：条件扩散模型从掩码后的潜在表征 $\mathbf{z}_{\text{masked}}$ 生成完整音频
