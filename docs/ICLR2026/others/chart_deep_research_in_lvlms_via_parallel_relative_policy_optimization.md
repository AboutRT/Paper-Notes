...
description: >-
  [ICLR2026][其他][chart understanding] 提出 PRPO（Parallel Relative Policy Optimization），通过在奖励维度和数据类型两个层面做并行解耦优化，解决 GRPO 在多维奖励信号干扰和异构数据梯度冲突下的训练瓶颈；同时构建 MCDR-Bench，基于"错误唯一性原则"将主观生成评估转化为客观错误识别，实现图表深度研究能力的量化评估。
tags:
  - ICLR2026
  - 其他
  - chart understanding
...
1. **Reward-PRPO（奖励维度并行）**:
    ...
    - 功能：在奖励维度层面分解优化，每个维度独立计算 advantage
...
2. **Data-PRPO（数据类型并行）**:
    ...
    - 功能：按能力维度分区数据，每个分区内独立标准化 advantage
