...
1. **大多数理论聚焦于浅层（depth-2）模型**：无法完全解释更深网络中观察到的更强低秩偏好。为什么深度越大，模型越倾向于收敛到低秩解？已有工作（如 Menon, 2024）提出了这一开放问题但未解决。

2. **可塑性损失现象缺乏理论解释**：Kleinman et al. (2024) 发现，在矩阵补全中先用少量观测值预训练、再用更多观测值继续训练，反而会导致性能下降——这就是可塑性损失。深层网络不受此影响，但浅层网络受影响严重，其机制不明。
