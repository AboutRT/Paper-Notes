...
1. **序列长度爆炸**: CAD模型元数据（如STEP文件）通常超出模型token限制
2. **非自然语言输入**: CAD元数据是结构化技术数据，而非自然语言
3. **无法微调**: GPT/Gemini等高性能模型通常不提供微调接口
4. **细粒度推理困难**: 即使将STEP数据转为图像，现成模型仍频繁误识零件，因为任务需要对零件关系和属性进行精细推理
