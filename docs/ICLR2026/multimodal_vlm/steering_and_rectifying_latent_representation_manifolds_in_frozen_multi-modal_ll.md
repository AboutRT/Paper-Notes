...
1. **表示偏差 (Representational Bias)**：MLLM 在网页规模语料上预训练，其特征空间针对频繁的原型概念进行优化，对异常事件这类罕见、微妙的模式敏感度低
2. **上下文歧义 (Contextual Ambiguity)**：被动依赖孤立特征会产生混淆表示——视觉上相似但语义不同的事件（如正常跑步 vs. 逃跑）无法被有效区分
