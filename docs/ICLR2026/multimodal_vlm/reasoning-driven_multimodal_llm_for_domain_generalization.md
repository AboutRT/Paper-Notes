...
**阶段二 — SARR（Self-Aligned Reasoning Regularization）**：解决挑战二。MTCT 训练后，让模型自行生成推理链，仅保留 <code>&lt;CONCLUSION&gt;</code> 与 ground-truth 标签匹配的推理链作为新的监督信号，然后重新进行 MTCT 训练。迭代 $N$ 轮（实验设 $N=3$），逐步将 GPT-4o 风格的推理替换为模型自身风格，在语义丰富性与可优化性之间取得平衡。
