...
1. **领域现状**：MLLM(Qwen-VL/GPT-4o等)已能处理图像/视频/分子等多模态输入。自动提示优化(APO)通过迭代生成-评估-改进来自动发现好的提示。

2. **现有痛点**：APO方法全部限于文本提示→忽略了MLLM的多模态输入能力。文本不一定能高效传达某些信息（如描述鸟的独特羽毛纹理需要长文本，一张图片更直观）。

3. **核心矛盾**：将搜索空间从 $\mathcal{T}$ 扩展到 $\mathcal{T} \times \mathcal{M}$ →组合爆炸+跨模态一致性挑战+候选选择困难。

4. **切入角度**：(1) 对齐保持更新——用统一反馈同时指导文本和非文本更新；(2) 先验继承——父提示的性能是子提示的良好先验→warm-start贝叶斯选择。
