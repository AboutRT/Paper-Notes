...
description: >-
  [ICLR 2026][多模态][多模态提示] 首次将自动提示优化（APO）从纯文本空间扩展到多模态空间，提出 MPO 框架：通过对齐保持的联合探索（统一语义梯度同步驱动文本+非文本提示更新，配合 Generation/Edit/Mix 三种算子多样化搜索）和先验继承的贝叶斯 UCB 候选选择（利用父提示性能 warm-start 子提示的 Beta 先验），在图像/视频/分子共 10 个数据集上平均准确率达 65.1%，超越最强文本 APO 基线 ProTeGi 的 60.0%。
tags:
  - ICLR 2026
  - 多模态
  - 多模态提示
...
1. **对齐保持探索（Alignment-Preserving Exploration）**:
    ...
    - 功能：在扩大的 $\mathcal{T} \times \mathcal{M}$ 空间中生成跨模态一致的候选提示
    - 核心思路：分两步——**凝聚反向传播**和**联合多模态更新**。首先从当前提示 $(\mathbf{t}, \mathbf{m})$ 的失败集 $\mathcal{F}$ 出发，让 MLLM 一次性生成同时涵盖文本和非文本两个维度弱点的统一反馈 $\nabla_{\mathbf{p}} = (\nabla_{\mathbf{t}}, \nabla_{\mathbf{m}})$。然后基于该反馈，MLLM 同时输出更新后的文本提示 $\mathbf{t}'$ 和一个模态特定的文本条件 $\mathbf{c}$（描述非文本提示应如何调整），该条件 $\mathbf{c}$ 被传入模态生成器 $g$（如 text-to-image 模型 GPT-Image、text-to-video 模型 Wan2.1、或 text-to-molecule 模块），最终产出更新后的非文本提示 $\mathbf{m}' = g(\mathbf{c})$
    - 设计动机：用单一反馈信号同时驱动两个模态的更新，避免了独立优化导致的语义不对齐。实验中，联合更新的 DSG 对齐分数显著高于先文本后图像的 Sequential 策略

2. **三种探索算子（Generation / Edit / Mix）**:

    - 功能：系统化覆盖多模态搜索空间的不同区域，避免陷入局部最优
    - 核心思路：
        - **Generation**：从零生成全新的非文本提示 $\mathbf{m}' = g(\mathbf{c}_{\text{gen}}, \varnothing)$，不参考已有候选，适合优化早期或候选池偏置时跳出局部最优
        - **Edit**：在已有非文本提示基础上做细粒度微调 $\mathbf{m}' = g(\mathbf{c}_{\text{edit}}, \{\mathbf{m}\})$，保留已有好的结构只修改局部属性（如纹理、颜色），适合提示已经较强但需要微调的阶段
        - **Mix**：融合 $K$ 个父提示的互补优势 $\mathbf{m}' = g(\mathbf{c}_{\text{mix}}, \{\mathbf{m}_i\}_{i=1}^K)$，产生综合多个候选优点的平衡方案，避免过度依赖单一候选
    - 设计动机：消融实验表明三种算子各自擅长不同子任务（如 Mix 在 Grape 子任务上从 48.0% 提升至 65.1%），组合使用时 PlantVillage 平均达 76.4% 远超单算子最佳的 74.8%

3. **先验继承贝叶斯 UCB 选择（Prior-Inherited Bayesian UCB）**:

    - 功能：高效地从 $b^2$ 个候选中识别最优提示，减少评估预算浪费
    - 核心思路：基于经验发现父提示性能与子提示性能高度正相关（$r = 0.88$），将每个候选提示的期望得分建模为 Beta 分布 $\text{Beta}(\alpha_i, \beta_i)$，并将子提示的先验参数设为 $\alpha_i = \hat{\mu}_{\text{par}} \cdot S + 1$，$\beta_i = (1 - \hat{\mu}_{\text{par}}) \cdot S + 1$，其中 $\hat{\mu}_{\text{par}}$ 为父提示的后验均值，$S = 10$ 为先验强度。每轮选择 UCB 得分最高的候选进行评估，更新其后验参数，预算用尽后选期望得分最高者
    - 设计动机：标准 UCB 和均匀分配都将新候选视为无先验的独立臂，冷启动效率低下。通过继承父提示的先验知识，MPO 在达到相同性能时比均匀分配节省 70% 评估预算，比无先验 UCB 节省 42%。Proposition 3.1 证明了当先验比均匀先验更 informative 时，最佳臂识别代价不增
...
- [MMOne: Representing Multiple Modalities in One Scene](../../ICCV2025/multimodal_vlm/mmone_representing_multiple_modalities_in_one_scene.md)
- [OMGM: Orchestrate Multiple Granularities and Modalities for Efficient Multimodal Retrieval](../../ACL2025/multimodal_vlm/omgm_orchestrate_multiple_granularities_and_modalities_for_efficient_multimodal_.md)
