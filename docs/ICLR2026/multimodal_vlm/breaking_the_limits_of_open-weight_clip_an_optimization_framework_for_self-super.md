...
description: >-
  [ICLR 2026][多模态][CLIP] 本文提出 TuneCLIP，一个自监督微调（SSFT）框架，通过两阶段设计——先恢复优化器统计量（OSR）消除冷启动偏差，再用带margin的铰链全局对比损失（HGCL）缓解假负样本过度惩罚——在不使用任何标签的条件下持续提升已有开源 CLIP 模型的通用性能，在 ImageNet 及变体上提升最高 +2.5%，在 DataComp 基准上提升 +1.2%。
tags:
  - ICLR 2026
  - 多模态
  - CLIP
...
1. **Optimizer Statistics Recovery (OSR)**:
    ...
    - 功能：在不更新模型参数的情况下，通过正常的前向传播和梯度计算来"预热"优化器状态
...
2. **Hinged Global Contrastive Loss (HGCL)**:
    ...
    - 功能：在对比损失中引入 margin 阈值，超过一定相似度差距的负样本不再被惩罚
