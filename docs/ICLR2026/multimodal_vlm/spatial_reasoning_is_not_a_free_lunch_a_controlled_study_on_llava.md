...
description: >-
  [ICLR 2026][多模态][spatial reasoning] 通过在 LLaVA 框架下控制实验，系统性地研究图像编码器训练目标和 2D 位置编码对 VLM 空间推理能力的影响，发现编码器选择主导空间性能、AIMv2 编码器一致性最好，但 2D-RoPE 的改进不稳定，空间推理的失败根植于当前 VLM 流水线的核心设计选择。
tags:
  - ICLR 2026
  - 多模态
  - 多模态VLM
...
1. **编码器替换对比实验**:
    ...
    - 功能：隔离编码器训练目标对空间推理的影响
    - 核心思路：对比四种编码器——CLIP（对比学习，全局图像-文本对齐）、SigLIP（Sigmoid 损失的 CLIP 变体）、SigLIP2（SigLIP 改进版）、AIMv2（使用更密集或生成式训练目标的编码器）。所有编码器直接插入 LLaVA 框架，使用完全相同的训练数据和流程
    - 设计动机：不同训练目标会产生不同的视觉特征——全局对齐目标（CLIP/SigLIP）可能弱化局部空间信息，而密集/生成式目标（AIMv2）可能保留更多空间细节

2. **2D 旋转位置编码（2D-RoPE）**:

    - 功能：在多模态注意力层中显式保留 2D 空间结构
    - 核心思路：标准 1D-RoPE 只编码 token 的序列位置，将 2D 图像中的所有 patch 排成一行后编码。2D-RoPE 同时编码每个 patch 的水平和垂直索引，应用于多模态注意力的 query 和 key 投影。这样在图像-文本融合时，模型可以区分同一图像中不同行/列的 patch
    - 设计动机：图像展平为 1D 序列时丢失了 2D 结构信息，这是空间推理失败的另一个潜在原因。Qwen2-VL 已引入类似的多模态旋转嵌入，但缺乏隔离位置编码影响的系统性证据

3. **多基准空间推理评估**:

    - 功能：从多个角度全面评估空间推理能力
    - 核心思路：使用 7 个空间推理基准——MMVP（视觉感知）、CV-Bench 2D Overall（2D 空间理解）、TallyQA（计数）、GQA Overall（场景图推理）、VSR（视觉空间关系）、TopViewRS（俯视图推理）、CountBenchQA（计数QA）。同时评估前沿模型（LLaVA-NeXT、Qwen2.5-VL 等 7 个模型）作为上界参照
    - 设计动机：空间推理是多维能力，单一基准可能遗漏某些方面的差异
