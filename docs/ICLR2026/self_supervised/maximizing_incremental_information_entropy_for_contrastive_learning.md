...
3. **核心矛盾**：更强的数据增强引入更多信息变化，理论上有助于学到更鲁棒的特征；但过强的增强可能超出语义保持边界，破坏正样本对的语义一致性。缺乏一个统一框架来平衡"熵增生成"与"语义保持"。
4. **本文要解决什么？** 设计一个理论指导的对比学习框架，在最大化增强视图间的信息熵增益的同时保持语义一致性。
5. **切入角度**：将编码器重新定义为信息瓶颈，将优化目标从"互信息最大化"重构为"增量信息熵最大化"，分离出熵的生成与保留两个独立可优化的子目标。
6. **核心idea一句话**：用可学习变换自适应生成信息熵 + 用编码器正则化保留熵 → 突破静态增强和刚性不变性的双重限制。
