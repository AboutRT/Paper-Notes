...
1. **预生成而非后验**：在任何 token 生成之前就进行预测，不需要完整答案
2. **自由格式问答**：不限于选择题，适用于开放式问答
3. **简单线性探针**：使用 difference-of-means 方向而非复杂非线性模型，旨在验证线性可分性
4. **跨域泛化**：核心目标不是最大化预测精度，而是验证正确性是否作为**统一的线性特征方向**存在
