---
description: "【论文笔记】Plan and Budget: Effective and Efficient Test-Time Scaling on Reasoning LLMs 论文解读 | ICLR 2026 | arXiv 2505.16122 | test-time scaling | 提出 Plan-and-Budget 框架，通过将复杂查询分解为子问题并基于估计复杂度自适应分配 token 预算，实现推理 LLM 的高效测试时缩放——最高提升 70% 准确率、减少 39% token、E3 指标提升 193.8%。"
tags:
  - ICLR 2026
  - test-time scaling
  - 推理效率
...
**领域**: LLM 推理 / 测试时缩放  
**关键词**: test-time scaling, 推理效率, overthinking, token预算分配, 推理LLM
...
1. **Overthinking（过度思考）**：许多主流 LLM 即使面对简单查询也会生成冗长、离题的推理链。模型"想得太多"，产生了大量不必要的中间推理步骤，浪费了计算资源。

2. **固定预算的局面**：近期工作试图通过限制固定 token 预算来缓解 overthinking，但这种"一刀切"的策略会导致 **underthinking（思考不足）**——对于困难问题，固定预算可能不够用，导致推理不充分。

3. **问题难度异质性**：现实中的查询复杂度变化很大。一个简单的算术题和一个复杂的多步推理问题需要截然不同的计算资源，但现有方法缺乏合理的资源分配机制。

4. **缺乏理论基础**：关于如何最优地分配推理计算资源，缺乏形式化的理论框架。
...
3. **子问题分解（Plan 阶段）**:
   - 做什么：使用 LLM 自身或轻量辅助模型将原始查询拆分为多个子问题
...
4. **自适应预算调度（Budget 阶段）**:
   - 做什么：基于每个子问题的估计复杂度，动态分配 token 预算
