---
description: "【论文笔记】Reasoning or Retrieval? A Study of Answer Attribution on Large Reasoning Models 论文解读 | ICLR 2026 | arXiv 2509.24156 | large reasoning models | 首次系统研究大型推理模型（LRM）的答案来源归因问题，揭示推理（CoT）和检索（记忆）两种机制同时竞争影响最终答案，并提出 Farl（遗忘增强强化学习）通过抑制检索捷径来提升模型的真实推理能力。"
tags:
  - ICLR 2026
...
1. **推理-答案断连**：最终答案并不总是由 CoT 推理过程直接产生，上下文偏差可以在 CoT 未承认的情况下影响输出
2. **双重机制假说**：模型可能同时通过"审慎推理"和"直接从内部记忆检索"两条路径生成答案
3. **训练方法影响不明**：蒸馏和强化学习对这两种机制的影响尚未被系统研究
