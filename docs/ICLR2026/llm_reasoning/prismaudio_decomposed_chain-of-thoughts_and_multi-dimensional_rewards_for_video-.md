...
description: >-
  [ICLR 2026][LLM推理][Video-to-Audio] 首次将分解式 Chain-of-Thought 推理与多维度强化学习（RL）结合应用于视频到音频（V2A）生成，通过四个专门化的 CoT 模块（语义/时序/美学/空间）配合对应奖励函数，解决了目标纠缠问题…
tags:
  - "ICLR 2026"
  - "LLM推理"
  - "Video-to-Audio"
...
- **功能**：将单体推理分解为 Semantic CoT（语义）、Temporal CoT（时序）、Aesthetic CoT（美学）、Spatial CoT（空间）四个专门模块
...
- **功能**：提出混合 ODE-SDE 采样策略，大幅降低 GRPO 训练开销
