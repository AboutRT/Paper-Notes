...
**本文要解决什么？** (1) 如何在一个统一框架下融合两类异质证据，让它们在不同训练阶段各取所长？(2) 如何以极低计算开销实现隐式证据的在线估计？(3) 如何在证据不确定时自动平衡探索（评估未知任务）与利用（选择已知适中任务）？
...
**核心idea一句话**：把在线任务选择重构为非平稳贝叶斯推断，通过广义后验更新融合显式+隐式证据，用 Thompson 采样自动做探索-利用权衡。

## 方法详解

### 整体框架

BOTS 在每个训练 step 执行一个三阶段循环：**任务选择 → 模型训练与证据收集 → 后验更新**。输入是 $N$ 个训练任务的池子 $\{\mathcal{T}^k\}_{k=1}^N$，每个任务维护一个 Beta 后验 $\text{Beta}(\alpha_t^k, \beta_t^k)$ 表示当前模型在该任务上的成功概率信念。每步的流程是：从后验采样估计难度 → 选择接近目标难度 $p^*=0.5$ 的任务组成训练批次 $\mathcal{B}_t$ → 用 GRPO 等 RL 算法训练模型 → 收集被选任务的直接评估结果（显式证据）和未选任务的插值预测结果（隐式证据）→ 融合更新所有任务的后验参数。

### 关键设计

1. **贝叶斯难度建模与广义后验更新——核心融合机制**:

    - 功能：将每个任务的难度估计建模为 Beta 分布，并设计一个同时吸收显式和隐式证据的在线更新规则
    - 核心思路：每个任务 $\mathcal{T}^k$ 的模型成功概率 $p_t^k$ 建模为 $\text{Beta}(\alpha_t^k, \beta_t^k)$。后验更新采用广义形式：$\alpha_{t+1}^k = (1-\lambda)\alpha_t^k + \lambda\alpha_0^k + (1-\rho)s_t^k + \rho\tilde{s}_t^k$（$\beta$ 的更新对称）。这里 $s_t^k, f_t^k$ 是直接评估的显式成功/失败计数（仅对被选任务 $k \in \mathcal{B}_t$ 非零），$\tilde{s}_t^k, \tilde{f}_t^k$ 是结合了显式和隐式的伪计数——对被选任务等于显式计数，对未选任务则通过插值估计器 $\tilde{p}(k, \mathcal{B}_t)$ 生成。参数 $\lambda \in [0,1]$ 控制历史折扣（通过与先验 $\alpha_0, \beta_0$ 插值实现遗忘），$\rho \in [0,1]$ 控制显式/隐式证据的融合权重。作者证明了该更新规则保持 Beta 族的闭合性——更新后仍是 Beta 分布，无需近似推断
    - 设计动机：传统贝叶斯更新只能处理直接观测（显式证据），完全忽略未评估任务。通过引入伪计数机制，未选中的任务也能在每步获得难度更新，从根本上解决冷启动问题。$\lambda$ 的设计解决了非平稳性——模型能力在不断进化，旧的评估结果逐渐失效，需要通过向先验回归来"遗忘"过时信息。作者的理论分析表明，后验的等效样本量 $n_t = \alpha_t + \beta_t$ 在稳态时满足 $\rho n/\lambda \leq n_t - n_0 \leq n/\lambda$，即 $\lambda$ 和 $\rho$ 共同控制了估计的置信度

2. **超轻量插值插件——零开销隐式证据生成**:

    - 功能：利用两个预评估过的参考模型（一弱一强）来估计当前模型在未评估任务上的通过率，无需任何额外 rollout
    - 核心思路：假设已有弱模型通过率 $\bar{p}_w^k$ 和强模型通过率 $\bar{p}_s^k$（很多 RL 数据集如 GURU 已自带这些标签）。在每个 step，利用当前批次 $\mathcal{B}_t$ 的在线评估结果计算当前模型的相对能力系数 $\mu_t = (\bar{p}_t^{\text{ref}} - \bar{p}_w^{\text{ref}}) / (\bar{p}_s^{\text{ref}} - \bar{p}_w^{\text{ref}})$，即当前模型在弱-强参考模型之间的位置。为降低随机性，$\mu_t$ 用动量平滑 $\tilde{\mu}_t = \gamma \tilde{\mu}_{t-1} + (1-\gamma)\mu_t$。然后对任意未评估任务 $k$ 估计通过率为 $\tilde{p}(k, \mathcal{B}_t) = \text{clip}(\tilde{\mu}_t \cdot \bar{p}_s^k + (1-\tilde{\mu}_t) \cdot \bar{p}_w^k, 0, 1)$
    - 设计动机：相比 AFRL 的注意力核方法需要额外 rollout 评估锚点任务，插值法仅需一次性的参考模型评估且只涉及简单向量运算，训练时额外开销 $\leq 0.2\%$。线性插值虽然是强假设，但实验表明在弱-强模型之间的合理范围内足够准确，且 BOTS 框架本身对隐式证据的噪声具有鲁棒性（通过 $\rho$ 控制其影响权重）

3. **Thompson 采样任务选择——自动探索-利用平衡**:

    - 功能：从维护的后验分布中选择最有学习价值的任务组成训练批次
    - 核心思路：对每个任务从当前后验中采样 $\hat{p}_k \sim \text{Beta}(\alpha_t^k, \beta_t^k)$，计算效用 $\hat{u}_k = -|\hat{p}_k - p^*|$（$p^* = 0.5$），选择效用最高的任务。由于采样引入的随机性与后验方差成正比，不确定性高（评估次数少）的任务自然更容易被"意外"采样到极端值而被选中探索，后验收紧的任务则趋于稳定利用
    - 设计动机：贪心策略（直接用后验均值选择最接近 $p^*$ 的任务）看似高效，但会陷入局部最优——忽略那些因数据稀疏而后验不确定的潜在高价值任务。Thompson 采样在理论上有 Bayes regret bound 保证，且实现极其简单（一行代码：从 Beta 分布采样）。消融实验证实 Thompson 采样比贪心选择产生更稳定的任务选择行为
