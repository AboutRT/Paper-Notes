...
1. **冗余感觉输入**: 原始对话包含大量无关信息，直接处理浪费资源甚至损害上下文学习能力
2. **粗糙粒度组织**: 按固定窗口切分导致语义混淆，按单轮切分又频繁调用API，两者均不理想
3. **实时更新瓶颈**: 记忆更新在推理时执行，引入延迟；且由于读写依赖需串行处理
...
2. **Light2: 主题感知短期记忆**:
   - 做什么：将主题段落暂存于STM缓冲区，达到token阈值后调用LLM生成简洁摘要
