---
description: "【论文笔记】LoFT: Low-Rank Adaptation That Behaves Like Full Fine-Tuning 论文解读 | ICLR 2026 | arXiv 2505.21289 | LoRA | 提出 LoFT，一种通过对齐优化器内部动态（动量和二阶矩）与全参微调行为一致的低秩适配方法，由六个构建模块组成，在全秩极限下可精确恢复 AdamW，在多项基准上显著缩小 LoRA 与全参微调的性能差距。"
tags:
  - ICLR 2026
  - LoRA
  - 低秩适配
...
1. **性能差距持续存在**：经验研究表明 LoRA 与全参微调之间存在持久的性能差距
2. **收敛速度较慢**：LoRA 的优化动态与全参微调存在本质差异
3. **超参数敏感**：缩放因子 $\alpha$ 的设置对性能影响显著，调参成本高
...
3. **一阶矩（动量）校准（Building Block 3）**:
   - 做什么：在动量累积中加入校准矩阵 $C_V^k = (V_{k-1}^\top V_k)(V_k^\top V_k)^{-1}$
...
4. **二阶矩对齐（Building Block 4 & 5）**:
   - 做什么：利用 Khatri-Rao 积和 Kronecker 积的性质高效维护二阶矩的交叉项
