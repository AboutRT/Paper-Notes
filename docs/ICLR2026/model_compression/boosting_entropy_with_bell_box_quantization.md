...
description: >-
  [ICLR 2026][模型压缩][量化感知预训练] 提出 Bell Box Quantization (BBQ)，首个同时满足"信息论最优"(ITO) 和"计算高效"(compute-efficient) 的量化方法，核心洞察是学习的域无关性——量化器输出域不必与输入域相同…
tags:
  - "ICLR 2026"
  - "模型压缩"
  - "量化感知预训练"
...
3. **均匀量化 + 域变换 (Step 1c + Dequant)**：
    ...
    - 功能：将 ITO 量化值映射到计算高效数据类型（INT4/MX FP4）
