...
description: >-
  [ICLR 2026][模型压缩][参数高效微调] 提出 ABBA 适配器，将权重更新参数化为两个独立可学习的低秩矩阵的 Hadamard 积 $\Delta W = s(B_1A_1) \odot (B_2A_2)$…
tags:
  - "ICLR 2026"
  - "模型压缩"
  - "参数高效微调"
...
1. **Hadamard 积的双低秩参数化**:
    ...
    - 功能：将更新表达为两个独立低秩矩阵的 Hadamard（逐元素）积
...
2. **Khatri-Rao 高效实现 (Theorem 1)**:
    ...
    - 功能：用 Khatri-Rao 分解将 ABBA 转换为 LoRA 形式，避免实例化全尺寸矩阵
...
3. **SVD 初始化 + 秩稳定性**:
    ...
    - 功能：用 $W_0$ 的截断 SVD 初始化 $(B_1, A_1)$，$(B_2, A_2)$ 用标准 LoRA 初始化
