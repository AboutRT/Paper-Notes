...
1. **双编码器方案**（如 TokenFlow）：语义编码器 + 像素编码器，模型冗余且训练低效
2. **冻结 VFM + 潜在扩散解码器**（如 EMU2、BLIP3-o）：继承了理解能力，但语义编码器无法建模细粒度细节，且受限于预训练 VAE 天花板
3. **统一编码器微调方案**（如 VILA-U、UniTok）：初始化后在像素解码器上微调，但高层语义与低层重建目标冲突，导致理解能力退化
