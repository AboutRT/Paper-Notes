...
**本文要解决什么？** 系统性地检验"LLM 安全行为是否集中在特定线性子空间"这一基础假设。
...
**核心idea一句话**：通过四个精心设计的实验证明，安全相关的权重更新和激活模式与通用学习不可线性分离，子空间防御策略面临根本局限。
...
1. **平行投影实验（实验1：子空间是否编码安全？）**:
    ...
    - 做什么：测试将有用/有害微调更新投影到候选安全子空间顶部方向后的行为表现
...
2. **正交投影实验（实验2：能否移除有害子空间？）**:
    ...
    - 做什么：在混合（80%有用 + 20%有害）微调场景下，测试去除候选安全子空间方向能否选择性抑制有害性
...
3. **Mode Subspace Overlap (MSO) 分析（实验3：更新间的几何关系）**:
    ...
    - 做什么：直接比较有用更新、有害更新、安全更新之间的子空间重叠度
...
4. **激活空间分析（实验4：表示层面是否可分？）**:
    ...
    - 做什么：检查有害提示和有用提示在模型内部激活空间中是否占据不同区域
