...
description: >-
  [ICLR 2026][人体理解][自监督学习] 提出 SER（Soft Equivariance Regularization），通过在 ViT 中间层施加软等变正则化、在最终层保持不变性目标的层解耦设计，在不引入额外模块的情况下，为不变性 SSL 方法（MoCo-v3, DINO, Barlow Twins）带来一致的分类精度和鲁棒性提升。
tags:
  - ICLR 2026
  - 人体理解
  - 自监督学习
...
2. **解析特征空间群操作 $\rho_g$**:
    ...
    - 功能：在特征空间直接定义几何变换的操作，无需学习额外的变换网络
...
3. **批次分区与增强策略**:
    ...
    - 功能：将 mini-batch 分为 $b_1$（使用标准增强）和 $b_2$（使用可逆增强）
...
4. **[CLS] token 延迟插入**:
    ...
    - 功能：将 [CLS] token 的插入从输入层推迟到等变正则化层之后（即 $f^{(2)}$ 的输入）
