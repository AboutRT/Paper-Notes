...
1. **运动偏好被忽视**：现有 VLM-机器人研究主要关注任务规划（做什么），而非运动规划（怎么走）——路径的拓扑属性和风格偏好从未被系统研究
2. **空间推理能力未知**：VLM 是否真的能理解"远离/靠近某物体"、"曲线/直线路径"等空间约束，缺乏定量评估
3. **集成路径不明**：将 VLM 集成到运动规划 pipeline 中的最佳方式（图像查询策略）尚未探索
