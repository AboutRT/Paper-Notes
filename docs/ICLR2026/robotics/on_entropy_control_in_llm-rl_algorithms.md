...
1. **领域现状**：策略梯度方法（PPO/GRPO/DAPO）是LLM-RL的主流。传统RL中熵正则化（SAC/A3C/PPO）通过保持策略随机性防止过早收敛，效果显著。

2. **现有痛点**：实验发现熵正则化在LLM-RL中几乎无增益。Cui等人观察到不同熵系数对验证准确率影响微乎其微。这与机器人/游戏RL中的显著效果形成矛盾。

3. **核心矛盾**：理论上熵正则化有优化优势（改善收敛），但在LLM中引入的偏差 $O(H\log\frac{|\mathcal{A}|}{|\mathcal{A}_H^*(s_0)|^{1/H}})$ 随动作空间 $|\mathcal{A}|$ 和最优稀疏度增大而剧增。LLM词汇表~10万+、最优token极其稀疏→偏差远大于优化增益。

4. **切入角度**：既然全词汇表上的熵偏差太大，就在更小的合理token空间上计算截断熵——只鼓励在"合理候选"中探索而非在整个词汇表中。
