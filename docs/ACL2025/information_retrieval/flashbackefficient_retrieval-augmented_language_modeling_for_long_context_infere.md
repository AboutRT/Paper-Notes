...
description: >-
  [ACL 2025][检索增强生成] 针对检索增强语言模型(RALM)中因检索内容前置(prepending)导致 KV cache 反复重算的推理效率问题，提出 FlashBack，将检索内容后置(appending)以保留输入的 KV cache，并用 Marking Token + LoRA 微调适配新的上下文模式，在 Llama 2-7B 上实现最高 4 倍推理加速且 perplexity 持平。
tags:
  - ACL 2025
  - 检索增强生成
  - 信息检索
...
**代码**: [https://github.com/BIT-NLP-GROUP/FlashBack](https://github.com/BIT-NLP-GROUP/FlashBack)  
**领域**: 信息检索  
...
**领域现状**：RALM 通过将外部语料检索结果与输入拼接来增强 LLM 的生成能力。主流做法（如 In-Context RALM）将检索内容前置(prepend)到输入之前，每隔 $s$ 个 token 检索一次。

**现有痛点**：每次检索后，由于检索内容变化且位于输入前方，整个上下文的 KV cache 都必须丢弃并重新计算。随着输入长度 $T$ 增加，重算的 FLOPs 以 $O(T^2)$ 增长，严重拖慢推理速度。
...
2. **Marking Token**:
    ...
    - 功能：两个特殊 prompt token `<MARK_L>` 和 `<MARK_R>`，标记检索内容的左右边界
...
3. **LoRA 微调策略**:
    ...
    - 功能：用 LoRA 对 attention 层做参数高效微调，适配 appending 模式
