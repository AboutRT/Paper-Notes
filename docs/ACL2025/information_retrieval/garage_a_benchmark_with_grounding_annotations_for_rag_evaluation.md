---
description: "【论文笔记】GaRAGe: A Benchmark with Grounding Annotations for RAG Evaluation 论文解读 | ACL 2025 | arXiv 2506.07671 | RAG benchmark | GaRAGe 是一个包含 2366 个问题和超过 35K 条人工标注 grounding 段落的 RAG 基准，通过细粒度的 grounding 相关性标注，系统评估 LLM 在 RAG 场景下识别相关信息、拒绝回答和归因引用的能力。"
tags:
  - ACL 2025
  - RAG benchmark
...
1. **评估维度混淆**：多数基准将查询生成和答案生成合并评估，无法单独衡量 LLM 的信息筛选能力
2. **grounding 质量未标注**：现有基准中检索到的段落要么全部相关（不现实），要么相关性未知（无法精准评估），缺少对每条 grounding 的人工相关性标注
3. **答案形式过于简单**：大量基准使用短答案或多选题，与真实用户需要长文本带引用的场景脱节
4. **来源单一**：很少有基准同时包含公开 Web 和私有知识库的混合 grounding，无法模拟企业 RAG 场景
