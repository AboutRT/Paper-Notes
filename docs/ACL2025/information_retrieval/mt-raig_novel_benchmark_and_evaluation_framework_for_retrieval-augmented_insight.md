...
**封闭域假设**：现有方法在测试时提供预定义的gold表格，但实际场景中用户往往不知道哪些表格与查询相关，手动指定表格既昂贵又不现实

**单表格限制**：多数工作假设所有信息包含在单一表格中，无法处理需要跨多表综合洞察的用户需求

**评估方法粗糙**：现有自动评估指标（BLEU、ROUGE、G-Eval等）以粗粒度方式分析输出，难以检测洞察与多表格间的事实对齐和多跳查询的完整覆盖
