...
1. **事实核查方法过于简化**：将声明视为整体验证，即使有"大部分为真"等细粒度标签，仍无法揭示科学领域中缺乏研究共识的方面
2. **文档级立场检测粒度不足**：同一文档可能对声明的不同方面持不同立场（如支持安全性但反对分发便利性），文档级分类无法捕捉
3. **LLM 分类法生成缺乏语料库感知**：现有方法依赖模型预训练知识，忽略特定语料库中的领域讨论
4. **检索噪声影响推理**：语义相似的方面节点之间存在检索片段重叠问题
