...
1. **Token 嵌入聚类（词表压缩）**:
    ...
    - 做什么：对 LM head 中的 token 嵌入矩阵做 KMeans 聚类，将 ~32K 个 token 合并为 $k$ 个语义簇（$k$ = 4000 或 8000），用簇中心嵌入替换原始 token 嵌入
...
2. **双向注意力机制**:
    ...
    - 做什么：在微调阶段将 LLM 的因果注意力掩码替换为全连接注意力（双向）
...
3. **表示生成与 Pooling**:
    ...
    - 做什么：将 logits 通过 log-saturation 变换后做 max-pooling 得到最终嵌入
