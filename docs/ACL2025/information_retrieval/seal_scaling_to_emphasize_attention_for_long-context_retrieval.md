...
description: >-
  [ACL 2025][长上下文检索] 发现特定注意力头与长上下文检索性能正/负相关，提出SEAL方法通过学习头级(SEAL-H)和通道级(SEAL-C)可学习缩放因子来增强关键注意力组件，仅需50个合成样本和不到1小时微调，即可在LongEval/Needle-in-a-Haystack/RULER等基准上大幅提升检索准确率（如LongChat-7B在31K长度从32%提升至88%），且推理零开销。
tags:
  - ACL 2025
  - 长上下文检索
  - 注意力机制
...
1. **Head-wise Pruning 分析（动机验证）**:
   - 做什么：逐一 pruning LongChat-7B 的每个注意力头，观察对检索准确率的影响
...
2. **SEAL-H（Head-level Scaling）**:
   - 做什么：为每个注意力头学习一个标量缩放因子 $s_{l,h} \in \mathbb{R}$
...
3. **SEAL-C（Channel-level Scaling）**:
   - 做什么：为每个注意力头输出的每个隐藏维度通道学习一个缩放因子 $s_{l,h,c} \in \mathbb{R}^{d_h}$
