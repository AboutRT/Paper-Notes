...
description: >-
  [ACL 2025][信息检索][RAG robustness] Astute RAG 提出了一种对不完美检索具有鲁棒性的 RAG 方法，通过自适应生成 LLM 内部知识作为补充、带有来源标注的知识整合、以及基于可靠性的答案生成三个步骤，在 Gemini 和 Claude 上显著优于现有鲁棒 RAG 方法，且是唯一在最坏情况下（检索全部无用）不劣于无 RAG 基线的方法。
tags:
  - ACL 2025
  - 信息检索
  - RAG robustness
...
**本文目标** 让 RAG 在不完美检索下仍然可靠，解决内外部知识冲突

**切入角度**：用 LLM 自己的知识作为"第二来源"，与外部检索结果对照，通过来源标注的整合机制判断可靠性

**核心 idea**：将 LLM 内部知识显式生成为"passages"，与外部检索 passages 一起做带来源标注的知识整合，根据一致性和来源可靠性决定最终答案。
