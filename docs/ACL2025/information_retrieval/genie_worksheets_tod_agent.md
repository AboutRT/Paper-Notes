...
description: >-
  [ACL 2025][信息检索/RAG][任务导向对话] Genie 提出了一个可编程的知识密集型任务导向对话框架，通过声明式 Worksheet 规范定义 Agent 策略，将 LLM 限制在语义解析和回复生成两个角色，由算法化运行时系统强制执行策略，实现从 21.8% 到 82.8% 的真实任务完成率提升…
tags:
  - "ACL 2025"
  - "信息检索/RAG"
  - "任务导向对话"
...
## 研究背景与动机
**领域现状**：LLM 可以进行类人对话，但在实际部署中常出现幻觉、无法遵循条件逻辑、知识整合困难
...
**核心矛盾**：开发者需要对 Agent 行为的精确控制，但 LLM 天然不可靠、不遵循复杂逻辑指令

**本文目标**：如何在保持 LLM 对话自然性的同时，让 Agent 可靠地遵循开发者定义的策略

**切入角度**：将 LLM 和算法化系统解耦——LLM 只做解析和生成，策略执行交给确定性运行时

**核心 idea**：声明式 Worksheet 规范 + 算法化 Runtime 强制执行策略 + LLM 仅做 NLU/NLG
...
1. **Genie Worksheet (声明式规范语言)**:
    ...
    - 功能：开发者只需声明需要从用户收集的字段、类型、条件谓词和对应动作
...
2. **Genie Parser (LLM 语义解析)**:
    ...
    - 功能：将用户自然语言输入转为对话状态更新的 Python 语句
...
3. **Genie Runtime (算法化运行时)**:
    ...
    - 功能：确定性地执行 Agent 策略，生成形式化 Agent Acts (Report/Confirm/Say/Propose/Ask)
