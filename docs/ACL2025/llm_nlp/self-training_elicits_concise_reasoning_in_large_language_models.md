---
title: >-
  [论文解读] Self-Training Elicits Concise Reasoning in Large Language Models
description: >-
  [ACL 2025][LLM/NLP][简洁推理] 发现 LLM 输出分布中天然包含简洁推理路径，提出 FS-BoN（Few-shot 条件化 + Best-of-N 采样）自训练框架，从模型自身分布中筛选短且正确的推理样本进行微调，在 GSM8K 和 MATH 上跨 5 个模型族实现平均 30% token 缩减且不损准确率，效率为先前方法 Rational Metareasoning 的 2.4 倍。
tags:
  - ACL 2025
  - LLM/NLP
  - 简洁推理
...
**代码**: [https://github.com/TergelMunkhbat/concise-reasoning](https://github.com/TergelMunkhbat/concise-reasoning)  
**领域**: LLM/NLP  
...
**本文目标**：如何在不损害准确率的前提下，可靠地缩短 LLM 推理链中的冗余 token？子问题包括：(a) 模型是否具备简洁推理的潜在能力？(b) 如何高效提取简洁推理样本？(c) 如何将推理时的简洁能力内化为模型默认行为？

**切入角度**：作者对多个模型在 GSM8K 上采样 16 条推理路径，发现正确路径的归一化长度分布有大量质量落在平均长度以下——例如 DeepSeekMath-7B 有 8.37% 的正确解长度不到平均值的一半。这说明**简洁推理已经存在于模型的输出分布中**，只是默认解码不会选中它们。

**核心 idea**：通过 Few-shot 条件化引导 + Best-of-N 采样筛选，从模型自身分布中提取简洁正确的推理路径，再用标准微调将这种能力内化，实现零推理开销的简洁推理。
...
1. **朴素 Best-of-N 采样（Naive BoN）**:
    ...
    - 功能：对每个训练问题采样 $N$ 条推理路径，选最短的正确路径作为训练样本
...
2. **Few-shot 条件化采样（FS）**:
    ...
    - 功能：在采样推理路径时，用 8 个简洁推理示例作为 few-shot prompt 引导模型生成更短的推理
...
3. **Few-shot 条件化 BoN（FS-BoN）**:
    ...
    - 功能：在 few-shot 条件下再叠加 BoN 采样，双重缩减
...
4. **样本增强（Sample Augmentation）**:
    ...
    - 功能：对 FS 和 FS-BoN 方法，额外从朴素 BoN（不加 few-shot prompt）采 $N$ 条路径，与 FS/FS-BoN 的候选合并，选最短正确路径
