...
- **本文要解决什么**: LLM能否像人类一样，利用语法体貌（完成体 vs 未完成体）的语义差异来构建叙事情境模型、保持工作记忆焦点、并进行远距因果推理？
...
- **核心idea一句话**: 复用认知语言学已验证的叙事体貌实验材料，构建30种prompt变体的鲁棒测试管线，从语义→认知加工→语用三层递进地评估LLM与人类在体貌理解上的差距。
