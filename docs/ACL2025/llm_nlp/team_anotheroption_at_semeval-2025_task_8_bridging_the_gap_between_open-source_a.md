...
**本文要解决什么？** (1) 如何让多个开源模型协同工作以弥合与闭源模型的差距？(2) 如何在代码生成和语义理解之间实现互补？(3) 如何应对真实数据的噪声和格式不匹配？

**切入角度**：作者观察到不同模型和不同方法路径在不同类型问题上各有优势，单一模型的瓶颈可以通过多模型投票和编排来突破。与其追求单一更强的模型，不如设计智能的多路径管道 + 仲裁机制。

**核心 idea 一句话**：用多个开源 LLM 分别走 SQL、Python、E2E 三条路径生成候选答案，再由 Orchestrator 从中选择最可靠的，实现"群体智慧"超越单个闭源强模型。
