...
description: >-
  [ACL 2025][LLM/NLP][提示学习] 提出 ERM 方法，通过指导性元提示生成带详细解题过程的 exemplar 来增强 feedback 质量，并引入 Feedback Memory 和 Exemplar Factory 两种长期记忆机制来高效存储和复用历史反馈与示例，在多个任务上以约一半的优化步数超越了 SOTA prompt 优化方法。
tags:
  - ACL 2025
  - LLM/NLP
  - 提示学习
...
1. **Exemplar-Guided Reflection（指导性反思）**:
    ...
    - 功能：设计指导性元提示，引导 prompt optimizer 从错误样本中选出典型样本并提供详细解题过程（CoT 风格），再基于这些 exemplar 生成更有信息量的 feedback。
...
2. **Feedback Memory（反馈记忆）**:
    ...
    - 功能：存储历史 feedback 并为每条分配优先级分数，周期性检索高优先级 feedback 指导 prompt 优化。
...
3. **Exemplar Factory（示例工厂）**:
    ...
    - 功能：存储、评估和检索 exemplar，推理时选择最优 exemplar 拼接到 prompt 中增强预测。
