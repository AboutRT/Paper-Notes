...
## 研究背景与动机
**领域现状**：当前 LLM 对齐主要依赖训练时方法（RLHF、DPO 等），通过人类偏好数据微调模型参数来嵌入对齐目标
...
**核心矛盾**：训练时对齐是一次性、静态的嵌入过程，无法适应动态变化的对齐需求，也无法在推理时提供强制性保证

**本文目标** 如何在解码阶段灵活、可靠地施加可定制的对齐约束

**切入角度**：将文本生成视为启发式搜索问题，alignment objectives 作为搜索启发式函数

**核心 idea**：把对齐从训练时移到解码时，通过 A* 搜索 + reward model 启发式引导每步 token 选择
...
1. **起始状态适应（Start-state Adaptation）**:
    ...
    - 功能：通过 alignment prompt $p_a$ 修改输入，改善生成的初始搜索方向
...
2. **动作选择与 Lookahead（Action Selection）**:
    ...
    - 功能：在每步解码时从 top-k 候选 token 中选最优
