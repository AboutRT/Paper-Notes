...
1. **领域现状**: 基于多模态 LLM 的 UI Agent（如 UFO）能够直接操作应用界面完成用户任务，成为研究热点。然而现有应用的 UI 是为**人类**设计的，不适合 Agent 高效操作。
2. **现有痛点**:
   - **高延迟**: 每步 UI 交互需要一次 LLM 推理，多步操作累积延迟严重
   - **低可靠性**: 长链 UI 交互中 LLM 易产生幻觉，错误逐步累积
   - **泛化困难**: LLM 对预训练阶段未见过的 UI 控件难以正确交互
3. **核心矛盾**: 现有 UI 是 HCI（人-计算机交互）范式的产物，用于 HACI（人-Agent-计算机交互）范式效率低下。类比蒸汽时代到电力时代的工厂改造——不能只替换动力源，需要重新设计整个流程。
4. **本文要解决什么**: 如何让 LLM Agent 高效、可靠地完成应用操作任务。
5. **切入角度**: API 调用比 UI 操作更高效——一次 API 调用可替代多步 UI 交互（如"插入 2×2 表格"从 UI 的"Insert→Table→2×2"三步变为一次 API 调用）。
6. **核心idea**: Agent 应**优先**调用 API，仅在 API 不可用时才退回 UI 操作；框架应能自动探索应用并构建新 API。
