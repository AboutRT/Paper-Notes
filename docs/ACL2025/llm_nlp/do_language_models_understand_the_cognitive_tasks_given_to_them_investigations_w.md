...
**本文要解决什么？** 系统区分 LLM 在 N-back 任务中的性能下降到底是由任务理解不足还是工作记忆限制导致的。

**切入角度**：对多个不同性能层级的开源模型进行 N-back 测试，引入反事实度量（counterfactual measures）分析模型实际执行的是哪个 m-back 任务，并通过注意力分析验证任务理解。

**核心idea一句话**：LLM 在 N-back 任务上的差异主要反映任务理解能力的差异，而非工作记忆容量的差异。
...
1. **反事实度量体系（Counterfactual Measures）**:
    ...
    - 做什么：在给定 n-back 指令的情况下，同时评估模型对所有可能的 m-back（$m \neq n$）任务的响应概率
...
2. **三层级模型分类（Three Performance Tiers）**:
    ...
    - 做什么：将模型按 2-back 和 3-back 检索准确率分为 T1（>80%）、T2（~50%）、T3（≤20%）三个层级
...
3. **任务集维持分析（Task Set Maintenance）**:
    ...
    - 做什么：追踪模型在 24 步 trial 中随时间的任务一致性变化
...
4. **课程学习策略（In-context Curriculum Learning）**:
    ...
    - 做什么：在测试 n-back 前，先提供从 1-back 到 (n-1)-back 的完整演示序列
...
5. **注意力模式分析（Attention Analysis）**:
    ...
    - 做什么：计算每个检索 token 对正确源 token（n 步前）的平均注意力（MRAT）
...
**切入角度**：从独特的理论框架或方法论出发。
**核心idea一句话**：用 N-back 任务系统分析 LLM 认知任务理解能力。
