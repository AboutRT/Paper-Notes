...
1. **Profile 与对话的偏差（Bias）**：训练时使用预定义的完整角色 profile 来指导特定场景的对话生成，但单个场景的对话通常只能体现 profile 的部分特质。例如，赫敏的说话风格被定义为四类，但某场景的对话可能只体现了其中两类。在 HPD 数据集中，83.2% 的对话与预定义 profile 存在偏差。这种偏差会误导模型训练。

2. **缺乏细粒度对齐**：模型仅通过对话任务学习从 profile 到对话的粗略映射，无法在句子级别理解"性格特质如何体现在具体对话中"，导致角色扮演缺少深度。
