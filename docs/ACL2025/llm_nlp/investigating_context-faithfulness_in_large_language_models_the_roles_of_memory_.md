---
description: "【论文笔记】Investigating Context-Faithfulness in Large Language Models: The Roles of Memory Strength and Evidence Style 论文解读 | ACL 2025 | arXiv 2409.10955 | context faithfulness | 通过测量 LLM 对同一问题不同释义的回答一致性来量化\"记忆强度\"，发现 LLM 对外部证据的接受度与记忆强度高度负相关，且改写式证据比重复或详细证据更有效。"
tags:
  - ACL 2025
...
1. **领域现状**：RAG 通过引入外部信息增强 LLM，但当外部信息与模型内存知识冲突时，LLM 常忽略外部证据。
2. **现有痛点**：(1) 先前工作提供长上下文做测试，无法区分"忽略证据"和"无法理解长文"；(2) 不同 LLM 知识量不同，在同一数据集上测试不公平。
3. **核心矛盾**：如何公平且准确地测量 LLM 的上下文忠实度？需要一个模型感知的评估框架。
4. **本文要解决什么？** (1) 量化记忆强度并分析其对上下文忠实度的影响；(2) 研究证据呈现方式对说服力的影响。
5. **切入角度**：用释义一致性衡量记忆强度——回答不同表述的同一问题时越一致，记忆越强。
6. **核心idea一句话**：LLM 的上下文忠实度是记忆强度的函数——记忆越强越拒绝外部证据；释义式证据比重复或扩展更有说服力。
