...
**本文要解决什么？** → 该领域缺乏对 Agent 架构设计、应用场景和评估框架的全面系统性综述。

**切入角度**：以"距离 Baymax 还有多远"为隐喻切入，从理想化的全能医疗 AI 助手倒推当前技术的差距与挑战。

**核心 idea 一句话**：从架构、应用和评估三个维度全面审视医学 LLM Agent 的能力边界和改进方向，用 Baymax 愿景锚定技术发展的长期目标。
...
1. **临床规划——从任务分解到自适应协作**:
    ...
    - 做什么：将复杂医学任务分解为可执行的子任务，支持 Agent 与临床工具和数据库的交互
