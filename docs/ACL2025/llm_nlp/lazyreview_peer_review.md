...
description: >-
  [ACL 2025][LLM/NLP][同行评审] 构建首个标注"懒惰思维"细粒度类别的同行评审数据集 LazyReview，通过三轮迭代标注协议使 Cohen's κ 从 0.31 提升到 0.52；发现 LLM 零样本下检测困难，但指令微调后性能提升 10-20 个百分点，且经懒惰思维反馈修改的评审显著更全面和可操作。
tags:
  - ACL 2025
  - LLM/NLP
  - 同行评审
...
**本文要解决什么？** (1) 如何构建一个标注一致性足够高的懒惰思维检测数据集？(2) LLM 能否自动检测评审中的懒惰思维？(3) 懒惰思维反馈对评审质量提升有多大帮助？

**切入角度**：作者从认知心理学中的启发式偏误理论出发，将评审中的懒惰思维视为信息过载和时间压力下的认知捷径。通过迭代改进标注指南（合并 ARR + EMNLP 指南 + 正例增强）来克服标注歧义，然后用指令微调弥补 LLM 的零样本不足。

**核心idea一句话**：通过三轮迭代标注协议构建高质量懒惰思维数据集，再用指令微调让 LLM 学会检测评审中的认知偏误。
...
1. **三轮迭代标注协议**:
    ...
    - 做什么：从 ARR-22 的 684 篇评审中提取"弱点"部分的评审片段，先用 GPT-4 预筛选候选段落（precision 0.74, recall 1.00），然后由 NLP 博士生进行三轮标注
...
2. **双粒度任务建模**:
    ...
    - 做什么：将懒惰思维检测分为粗粒度（二分类：是否存在懒惰思维）和细粒度（18 类多分类），并分别评估
...
3. **混合数据指令微调**:
    ...
    - 做什么：用 LoRA 对 7 款 LLM 进行指令微调，探索不同数据混合配方的效果
