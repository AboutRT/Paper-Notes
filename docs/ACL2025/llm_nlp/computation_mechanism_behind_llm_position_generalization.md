...
**本文目标** 揭示 LLM 注意力机制中位置和语义信息的计算处理机制，并用此解释位置泛化现象。

**切入角度**：直接分析注意力 logit 矩阵的结构，发现其可以分解为位置轴和语义轴的简单加法。

**核心 idea**：LLM 的注意力 logit 学习了位置和语义的解耦加法分解——这是位置泛化的计算基础。
