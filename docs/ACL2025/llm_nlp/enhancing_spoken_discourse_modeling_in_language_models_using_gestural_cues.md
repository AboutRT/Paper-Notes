...
description: >-
  [ACL 2025][LLM/NLP][gesture modeling] 提出将手势动作序列（3D 人体运动数据）通过 VQ-VAE 编码为离散 gesture token，再经特征对齐映射到语言模型输入空间，用于增强口语篇章建模…
tags:
  - "ACL 2025"
  - "LLM/NLP"
  - "gesture modeling"
...
**本文目标** 探索联合建模手势与语言能否提升语言模型的口语篇章理解能力。

**切入角度**：设计三个有语言学依据的文本填充任务来评估手势对篇章建模的贡献。

**核心 idea**：用 VQ-VAE 将 3D 手势序列离散化为 token，通过特征对齐映射到语言模型空间，从而让语言模型在口语篇章建模中利用手势信息。
