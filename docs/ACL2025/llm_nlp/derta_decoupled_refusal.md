---
title: >-
  [论文解读] Refuse Whenever You Feel Unsafe: Improving Safety in LLMs via Decoupled Refusal Training
description: >-
  [ACL 2025][LLM/NLP][LLM安全] 发现标准安全微调数据存在"拒绝位置偏差"——模型只学会在回答开头拒绝，中途发现不安全时无法中断。提出 DeRTa（Decoupled Refusal Training）…
tags:
  - "ACL 2025"
  - "LLM/NLP"
  - "LLM安全"
...
**本文目标** 让 LLM 能在生成回答的任何位置感知到不安全并中断转向拒绝。

**切入角度**：在安全训练数据中显式构造"先有害后安全"的转换序列——让模型学会"从有害到安全"的转换技能，而非仅学习"直接安全"的模式。

**核心 idea**：让模型学会"中途刹车"——从有害输出的任意位置转向安全拒绝。
...
2. **Reinforced Transition Optimization (RTO)**:
    ...
    - 功能：在有害回复的每个 token 位置上都强化"转向安全"的能力
