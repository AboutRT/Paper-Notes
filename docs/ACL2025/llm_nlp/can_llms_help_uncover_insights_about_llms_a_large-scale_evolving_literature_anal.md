...
**领域**: llm_nlp  
**关键词**: 文献分析, 自动数据抽取, LLM评估, Chain-of-Thought, In-context Learning, 提示策略
...
1. **LLM研究爆发式增长**：LLM相关实证研究数量激增，"综述的综述"已出现，单个研究者无法手动审阅所有文献
2. **跨研究综合困难**：不同论文使用不同模型、数据集和提示配置，使得跨研究发现的聚合分析极为困难
3. **手动文献分析耗时巨大**：人工从论文中提取实验数据（识别表格→抽取属性→汇总）每张表平均耗时7分50秒，全量约需350小时
4. **发现快速过时**：LLM领域进展极快，基于已有研究的静态分析会迅速过时，需要可持续更新的分析方案
5. **已有工作局限**：Sprague et al. (2024) 对CoT的手动分析规模有限、仅关注CoT提示、依赖人工抽取
6. **本文切入点**：用LLM加速数据抽取，构建结构化可更新数据集，实现大规模自动化文献分析，同时揭示新的提示策略洞见
