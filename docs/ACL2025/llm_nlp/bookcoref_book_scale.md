---
title: >-
  [论文解读] BookCoref: Coreference Resolution at Book Scale
description: >-
  [ACL 2025][LLM/NLP][共指消解] 提出首个书级别共指消解基准BookCoref，通过角色链接+LLM过滤+窗口扩展的自动标注管线，在50本完整小说上生成高质量银标注数据，平均文档长度超过20万tokens。
tags:
  - "ACL 2025"
  - "LLM/NLP"
  - "共指消解"
...
**本文目标**: 设计可靠的自动标注管线，构建首个书级别共指消解训练和评估资源。

**切入角度**: 利用人物名单初始化共指簇，通过LLM过滤提高精度，再用窗口化CR模型扩展到代词和其他指称。

**核心 idea**: 通过角色链接→LLM过滤→窗口级CR扩展→分组窗口扩展四步管线，实现书级别的高质量自动共指标注。
