...
description: >-
  [ACL 2025][LLM/NLP][可解释性] 提出 COSMIC（Cosine Similarity Metrics for Inversion of Concepts），一种完全不依赖模型输出 token 的自动化方向选择框架，通过余弦相似度目标在 LLM 激活空间中识别拒绝方向，在对抗性完全拒绝和弱对齐模型场景下均能有效提取和引导拒绝行为。
tags:
  - ACL 2025
  - LLM/NLP
  - 可解释性
...
**本文要解决什么？** 设计一种完全不依赖模型输出的方向选择框架，能在任意对齐条件下自动识别拒绝方向。

**切入角度**：既然拒绝行为编码在激活空间的方向中，那么一个好的方向应该能让干预后的激活在内部表示层面发生"概念反转"——把有害提示的激活变得像无害提示，反之亦然。这种反转可以用余弦相似度来度量，完全不需要看输出。

**核心idea一句话**：用激活空间中干预前后的余弦相似度（概念反转程度）代替输出 token 匹配来选择最优拒绝方向。
...
2. **低相似度层选择 + 余弦相似度评分**:
    ...
    - 做什么：选择评估层集合 $\mathcal{L}_{low}$ 并对每个候选方向打分
