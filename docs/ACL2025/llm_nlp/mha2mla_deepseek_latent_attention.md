---
title: >-
  [论文解读] MHA2MLA: Towards Economical Inference by Enabling DeepSeek's Multi-Head Latent Attention in Any Transformer-based LLMs
description: >-
  [ACL 2025][LLM/NLP][注意力机制] MHA2MLA 首次提出将已训练好的 MHA 模型高效迁移到 DeepSeek 的 MLA 架构的方法，通过贡献度感知的 partial-RoPE 移除和联合 SVD 低秩近似，仅用 0.6%-1% 的训练数据即可恢复性能…
tags:
  - "ACL 2025"
  - "LLM/NLP"
  - "注意力机制"
...
## 研究背景与动机
**领域现状**：DeepSeek 提出的 MLA 通过将 KV cache 压缩为低秩潜向量显著降低了推理内存，但需要从头预训练。现有 MHA/GQA 模型（如 Llama、Qwen）无法直接受益
...
**核心矛盾**：MLA 的推理效率优势只有从头训练的模型才能享有，已有大量 MHA 模型的投资无法复用

**本文目标** 让已训练好的 MHA 模型以极低数据成本迁移到 MLA 架构

**切入角度**：MLA 的关键架构差异是 partial-RoPE（位置和非位置维度分离）和 KV 低秩压缩，分别用贡献度分析和 SVD 初始化来弥合

**核心 idea**：移除 MHA 中贡献最低的 RoPE 维度 → 对非 RoPE 维度做联合 SVD 压缩 → 少量微调恢复性能
...
1. **贡献度感知 Partial-RoPE 移除**:
    ...
    - 功能：选择性地从 QK 的频率子空间中移除 RoPE，将其转为 NoPE（无位置编码）
...
2. **联合 SVD 低秩近似（SVDjoint）**:
    ...
    - 功能：将移除 RoPE 后的 Key（NoPE 部分）和 Value 的投影矩阵联合压缩为低秩潜向量
...
- [\[ACL 2025\] Supervised Fine-Tuning Achieves Rapid Task Adaption Via Alternating Attention Head Activation Patterns](analyzing_the_rapid_generalization_of_sft_via_the_perspective_of_attention_head_.md)
