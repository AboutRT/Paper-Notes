...
**本文目标** 如何将约束解码的自动机从非确定性 PDA 转为确定性 PDA（DPDA），消除运行时探索开销。

**切入角度**：LR(1) 文法天然是确定性上下文无关语言（DCFL）的子集，理论上可以被 DPDA 识别。关键是设计从 LR(1) 状态转移图到 DPDA 的高效构造算法。

**核心 idea**：用前缀条件边为每条转移附加栈匹配条件，使所有转移在预处理时完全确定。
