...
**代码**: [https://scalequest.github.io](https://scalequest.github.io)  
**领域**: LLM/NLP  
...
1. **QFT (Question Fine-Tuning) — 激活出题能力**
    ...
    - **功能**：用约 15K 数学问题（不含解答、仅问题文本 + EOS token）微调解题模型（Qwen2-Math-7B-Instruct），让模型学会"只生成问题后就停止"。
...
2. **QPO (Question Preference Optimization) — 提升问题质量**
    ...
    - **功能**：QFT 模型生成 10K 问题，用外部 LLM 分别从可解性和难度两个方向优化问题，构造(优化后, 原始)偏好对，用 DPO 训练模型。
...
3. **多维过滤 + 奖励模型选解答**
    ...
    - **功能**：语言过滤（去除约 20% 非英文问题）→ 可解性过滤（Qwen2-Math 判断问题是否有意义、条件是否充分）→ 难度采样（基于 fail rate 训练的难度评分器过滤过简单问题）→ 每题生成 5 个解答、用 InternLM2-7B-Reward 选最高分解答。
