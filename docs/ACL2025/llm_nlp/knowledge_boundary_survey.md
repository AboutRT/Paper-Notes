...
**领域现状**: LLM在参数中存储大量知识，但在记忆和利用某些知识方面仍存在局限，导致不真实、不准确的回复。**现有痛点**: Know-Unknown Quadrant概念性强但缺乏形式化；已有形式化定义仅关注特定LLM。**核心矛盾**: 对LLM知识边界缺乏清晰统一的定义，阻碍了系统性的识别和缓解策略。**本文要解决什么**: 提供全面的形式化知识边界定义并系统综述相关研究。**切入角度**: 从知识是否被人类已知、是否嵌入参数、是否可实证验证三个维度定义边界。**核心idea**: 将知识分为PAK（无论怎么问都会）、PSK（看怎么问）、MSU（模型不会但人类知道）、MAU（人类也不知道）四类。
