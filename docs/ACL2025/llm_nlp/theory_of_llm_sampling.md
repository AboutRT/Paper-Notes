...
description: >-
  [ACL 2025][LLM理论][认知科学] 提出LLM采样行为理论——LLM的响应采样同时受描述性成分(统计规范)和规范性成分(隐式理想)的双重驱动，类似人类决策中的System-1启发式，在500个概念和15个模型上验证，揭示了LLM在医疗等场景中可能产生的偏差。
tags:
  - ACL 2025
  - LLM理论
  - 认知科学
...
1. **虚构概念实验 (Novel Concept "Glubbing")**:
    - 做什么：引入LLM从未见过的虚构概念"glubbing"，附带100个样本(来自高斯分布 $C_\mu=45$)和对应评分(A+到D-)
