---
title: >-
  [论文解读] Training Language Model to Critique for Better Refinement
description: >-
  [ACL 2025][LLM/NLP][Critique-Refinement Loop] 提出 Refinement-oriented Critique Optimization（RCO），以"批判效用"（Critique Utility, CU）——即批判导致的精炼改善比例——作为奖励信号训练 critic 模型，通过 DPO 变体的 MSE 目标函数优化，无需直接评估批判质量；在对话生成、摘要、问答、数学推理、代码生成 5 个任务上，RCO 训练的 7B/13B critic 模型在 CU 和 RQS 指标上显著超过 70B 基线模型和 DPCO 方法。
tags:
  - ACL 2025
  - LLM/NLP
...
**领域现状**：LLM 的批判能力（critique ability）是自动评估和自我改进的关键。近期工作通过人工标注批判数据 + SFT/RLHF 训练 critic 模型，取得一定进展。

**现有痛点**：(a) 现有方法训练 critic 模型是为了评估，而非为了驱动精炼改善——批判和精炼是割裂的；(b) 直接评估批判质量困难且主观——什么算"好的"批判缺乏客观标准；(c) 人工标注批判偏好成本高且质量不稳定。

**核心矛盾**：好的批判应该是能带来好的精炼结果的批判，但现有方法没有建立批判质量与精炼效果之间的因果链。

**本文目标** 设计以精炼效果为导向的 critic 训练方法——让批判质量直接由其带来的改善程度定义。

**切入角度**：构建一个闭环——critic 生成批判 → actor 基于批判精炼 → 评估精炼 vs 原始回答的偏好 → 偏好作为 critic 的奖励。

**核心 idea**：用精炼改善率（CU = 精炼优于原始的概率）作为 critic 模型的奖励信号，通过 DRO-style MSE 目标训练。
