...
description: >-
  [ACL 2025][指令跟随] 发现当输入文本本身也是指令形式时，LLM会被"指令干扰"误导，忽略用户真实意图。提出DIM-Bench基准（4种指令任务×5种输入任务=20类2000实例），实验表明包括GPT-4o在内的6个LLM均无法完全抵抗指令干扰，尤其在QA输入下准确率接近0。三种提示策略（直接提示/CoT/后缀指令）均无法根本解决问题。
tags:
  - ACL 2025
  - 指令跟随
  - 指令干扰
...
4. **本文要解决什么？** (1) 系统性定义和分类"指令干扰"现象；(2) 构建评估基准；(3) 测试主流 LLM 的抗干扰能力；(4) 探索缓解策略的有效性。

5. **切入角度**：从数据处理的实际需求出发——翻译、改写、校对、风格转换这四类常见任务，遇到推理题、代码、数学题、偏见检测题、阅读理解题这五类"看起来像指令"的输入时，LLM 会如何表现？

6. **核心 idea 一句话**：指令跟随能力是双刃剑——当输入数据像指令时，LLM 无法区分优先级，这是一个被忽视但影响实际应用的系统性漏洞。
...
1. **指令任务选择（4种）**:
   - 做什么：选择改写（rewriting）、校对（proofreading）、翻译（translation）、风格转换（style transfer）作为指令任务
...
2. **输入任务选择（5种干扰源）**:
   - 做什么：选择推理（ARC 数据集）、代码生成（Code Alpaca）、数学推理（GSM8K + MATH）、偏见检测（BBQ）、问答（NarrativeQA）作为输入数据
...
3. **双重评估方法**:
   - 做什么：结合 LLM-judge 评估和基于长度差异的自动评估
