...
## 研究背景与动机
**领域现状**：文法推理在 NLP 和软件工程中有广泛应用（解析器生成、LLM 结构化输出约束等）。

**现有痛点**：LLM 在大多数任务上表现出色，但从少量正负例推理和生成 BNF 文法的能力尚未充分探索。

**核心矛盾**：传统文法推理需要大量特征例子，但 LLM 能否利用预训练知识从极少示例中推理文法？

**本文目标** 评估并提升 LLM 的 few-shot 文法生成能力。

**切入角度**：将遗传算法的搜索能力与 LLM 的生成能力结合。

**核心 idea**：用 LLM 初始化种群和驱动变异，用 BNF 解析器提供精确的适应度评分。
