...
1. **上下文长度受限**：Transformer 的最大上下文长度限制了可使用的 demonstration 数量，而更多的 demonstration 通常能带来更好的性能。
2. **二次复杂度**：随着 demonstration 数量增加，self-attention 的计算和内存开销呈二次增长。
3. **对 demonstration 选择敏感**：ICL 的效果高度依赖于选择了哪些 demonstration，不同选择可能导致截然不同的结果。
4. **缺乏质量区分**：传统 ICL 将所有 demonstration 等权重对待，无法区分高质量和低质量（甚至噪声）示例。
