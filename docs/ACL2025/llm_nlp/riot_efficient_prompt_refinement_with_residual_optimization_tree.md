...
### 两大挑战
1. **多样性不足**：现有方法（OPRO、TextGrad、ProTeGi）每轮仅生成一个候选 prompt，限制了搜索空间的广度；APE 虽然并行生成多个候选，但缺乏迭代精炼机制
2. **语义漂移**（Semantic Drift）：迭代优化 prompt 时，针对某个任务的优化可能损害先前任务的表现，类似于持续学习中的稳定性-可塑性困两难
