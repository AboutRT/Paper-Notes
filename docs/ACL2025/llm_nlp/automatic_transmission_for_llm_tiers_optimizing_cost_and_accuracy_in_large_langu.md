...
**本文要解决什么？** 设计无需训练的 LLM 层级自动选择框架，实现准确率和成本的 Pareto 最优。

**切入角度**：用类似汽车自动变速箱的理念——根据任务难度（路况）自动选择合适的档位（模型层级），配合自我验证和自动升级机制。

**核心idea一句话**：通过历史推理记录构建准确率估计器选择初始层级，再用 Judge 自动验证和升级，实现无训练的成本-准确率优化。
...
1. **准确率估计器（Accuracy Estimator）**:
    ...
    - 做什么：在无标注数据的情况下，估计各层级对当前问题的回答准确率
...
2. **伪标签机制（Pseudo-labeling）**:
    ...
    - 做什么：在没有人工标注的情况下，为历史推理记录生成正确性标签
