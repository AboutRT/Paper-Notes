---
title: >-
  [论文解读] HumT DumT: Measuring and Controlling Human-like Language in LLMs
description: >-
  [ACL 2025][LLM/NLP][human-like tone] 提出基于 GPT-2 对数概率比的文本人类化语气度量 HumT 及其社会感知泛化版 SocioT，在 40 万+偏好样本上发现用户普遍偏好更低人类化的 LLM 输出且人类化语气与社交亲近（r=0.87）、低地位（r=-0.80）、女性化（r=0.47）强相关，进而通过仅 500 对偏好数据的 DPO 微调（DumT）有效降低人类化程度而不损模型性能。
tags:
  - ACL 2025
  - LLM/NLP
  - human-like tone
...
**本文目标**：构建度量→理解→控制人类化语气的完整闭环：(a) 如何量化任意文本的人类化程度？(b) 用户真正偏好什么？(c) 人类化语气与哪些社会感知维度相关联？(d) 如何系统降低人类化程度？
...
**核心 idea**：用基于代词生命性（animacy）的对数概率比指标一箭三雕——度量人类化程度、预测用户偏好、量化社会感知维度
...
1. **HumT — 基于代词生命性的人类化度量**
    ...
    - 功能：给定任意文本 s，输出标量分数量化其"听起来像人说的"程度
...
2. **SocioT — 社会感知多维度度量**
    ...
    - 功能：将 HumT 的框架泛化到温暖、社会地位、社交距离、性别四个社会感知维度
...
3. **DumT — 基于 HumT+DPO 的人类化控制**
    ...
    - 功能：系统性降低 LLM 输出的人类化程度，同时维持甚至提升模型性能
