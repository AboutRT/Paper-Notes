...
### 核心观察
1. **显著权重的重要性**：先前研究表明，LLM 中存在一小部分"显著权重"（weight outliers），它们对量化（PTQ）和剪枝的效果有显著影响。Dettmers 等人发现输入激活中的极少数 outlier 对模型性能影响巨大。
2. **噪声注入的正则化效果**：扰动梯度下降（PGD）通过在梯度步骤前/后注入噪声，能帮助模型逃离鞍点和局部最优，起到稳定收敛和防止过拟合的作用。
3. **量化噪声与高斯噪声的等价性**：量化感知训练（QAT）中的量化噪声注入（QNI）与高斯噪声注入（GNI）在数学形式上具有等价性，因此 GNI 也可以增强模型在后续量化中的鲁棒性。
