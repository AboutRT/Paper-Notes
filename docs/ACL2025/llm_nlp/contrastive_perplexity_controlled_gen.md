---
title: >-
  [论文解读] Contrastive Perplexity for Controlled Generation: An Application in Detoxifying Large Language Models
description: >-
  [ACL 2025][LLM/NLP][可控生成] 提出基于原型对比困惑度（Contrastive Perplexity, CP）的框架，通过构造语义相似但毒性属性不同的正负样本对，在困惑度空间中进行对比学习来微调 LLM，实现显著的毒性降低（Mistral-7b 毒性从 33.1% 降至 4.3%）且几乎不影响下游任务性能。
tags:
  - ACL 2025
  - LLM/NLP
  - 可控生成
...
**本文目标** 如何让 LLM "提升用词而非提高音量"——不是回避敏感话题，而是学会用非毒性方式表达同样的语义。

**切入角度**：作者观察到毒性/非毒性表达的语义高度相近，可以构造"硬负样本"（语义相似但毒性不同的对），利用困惑度作为可解释的度量指标进行对比学习。

**核心 idea**：用对抗性改写生成语义相似的毒性硬负样本，通过原型对比困惑度损失拉近正样本、推远负样本。
...
1. **硬负样本生成**:
    ...
    - 功能：利用无审查 LLM（Vicuna-13B uncensored）为每条非毒性样本生成语义相似但有毒的对抗性改写
...
2. **原型对比困惑度（Prototype-based Contrastive Perplexity）**:
    ...
    - 功能：在困惑度空间中进行对比学习，让正样本的困惑度聚集在原型均值附近，负样本的困惑度远离原型
...
3. **负样本加权机制**:
    ...
    - 功能：通过权重超参 $\alpha$ 调节负样本在对比损失中的影响力
