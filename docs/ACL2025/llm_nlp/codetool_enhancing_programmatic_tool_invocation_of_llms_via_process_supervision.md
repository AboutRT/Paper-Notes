...
1. **JSON/文本格式的局限**：当前工具调用主要使用 JSON 或文本格式，在处理请求密集型任务时 token 消耗大、推理路径长，且容易被截断导致关键信息丢失
2. **缺乏中间步骤监督**：现有代码方式（如一次性生成完整代码）无法检测和纠正复杂场景中的中间步骤错误
3. **过程奖励可靠性差**：已有的过程奖励方法（如 StepTool）依赖强 LLM 生成奖励信号，客观性和可靠性存疑
