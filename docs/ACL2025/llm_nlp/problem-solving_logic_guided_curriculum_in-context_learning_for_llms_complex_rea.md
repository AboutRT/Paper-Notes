...
1. **领域现状**：ICL 通过 few-shot 示例增强 LLM 推理能力，关键在于示例选择和排序。
2. **现有痛点**：现有方法依赖文本相似度或困惑度等表面特征，无法反映解题逻辑联系。
3. **核心矛盾**：如何度量解题过程相似度而非文本相似度？
4. **本文要解决什么？** 用 QDMR 分析的解题逻辑来选择和排序示例。
5. **切入角度**：QDMR 将复杂问题分解为操作符序列（13种），即解题逻辑。
6. **核心idea一句话**：选解题逻辑相似的示例 + 按步骤数从易到难排序 = 课程式 ICL。
