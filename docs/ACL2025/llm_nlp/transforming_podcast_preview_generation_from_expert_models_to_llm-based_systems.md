...
1. **领域现状**：播客等长内容的发现和评估对用户来说时间成本高，预览片段是帮助用户快速判断内容是否感兴趣的有效方式。
2. **现有痛点**：传统播客预览系统（Legacy ML）依赖复杂的特征工程管线，需要整合话题分析、情感分析、广告检测、语音事件检测、句子边界检测、排序等多个专家模型，维护和迭代成本极高。
3. **核心矛盾**：传统系统每新增一个需求或调整标准，都需要重新训练或调整多个模型的权重和聚合逻辑，迭代周期长、灵活性差。
4. **本文解决**：用单一 LLM + few-shot prompt 替代整个多模型管线，通过 prompt 迭代替代特征工程，大幅简化架构。
5. **切入角度**：利用 LLM 的长文本理解能力和结构化推理，直接从转录文本中选出最佳预览片段，并生成元数据（话题标签、推荐理由等）。
6. **核心idea一句话**：LLM + 句子索引 + few-shot prompt 可以取代传统复杂特征工程管线，更快更好地生成播客预览。
