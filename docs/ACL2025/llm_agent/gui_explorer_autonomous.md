---
title: >-
  [论文解读] GUI-explorer: Autonomous Exploration and Mining of Transition-aware Knowledge for GUI Agent
description: >-
  [ACL 2025][LLM Agent][GUI agent] 提出 GUI-explorer，一个无需训练的 GUI agent，通过自主探索收集功能感知的交互轨迹，并以无监督方式从状态转换三元组中挖掘 transition-aware 知识…
tags:
  - "ACL 2025"
  - "LLM Agent"
  - "GUI agent"
...
1. **Function-aware Task Goal Generator**:
    ...
    - **功能**: 自动从应用环境中提取"探索锚点"（Exploration Anchors），如 APK manifest 中声明的功能模块（如"PaymentActivity"），基于当前截图和锚点构建探索任务
...
2. **Transition-aware Knowledge Extractor**:
    ...
    - **功能**: 对每个状态转换三元组 (o_i, a_i, o_{i+1})，提取 UI 元素的视觉 patch（key）和操作知识描述（value），构建多模态知识向量库
...
3. **Continuous Knowledge Refinement**:
    ...
    - **功能**: 通过余弦相似度判断新知识条目与现有条目的关系——key 相似度 ≥ 0.99 且 value 相似度 ≤ 0.1 时合并（表示同一 UI 元素的新功能描述）；否则添加新条目
    - **核心思路**: 知识库可持续更新，不会因为重复探索而产生冗余
    - **设计动机**: 适应应用频繁更新的需求，实现渐进式知识积累

4. **Dynamic Guidance**:

    - **功能**: 在执行时，对当前截图的 UI 元素进行视觉-语义检索匹配知识库（使用 SigLIP 作为嵌入模型），然后通过 MLLM-based 排序器按与任务指令的相关性排序，构建动态指导 prompt
