...
**领域现状**：Web Agent（如 ReACT）通过 LLM 执行网页导航和交互任务，但约 60% 的失败源于导航错误——Agent 无法找到正确的目标页面。
...
**核心矛盾**：Web 环境复杂但 Agent 每次都从零开始探索，历史经验无法有效复用

**本文目标** 构建 Web 环境的结构化表示（"地图"），让 Agent 基于已知信息做决策

**切入角度**：受人类认知和机器人探索研究的启发——人类通过记忆和反思迭代改进策略，不是每次都从头来

**核心 idea**：用 replay buffer 构建 Web 环境的有向图"地图"（Known MDP），用 A* 搜索替代盲目导航；用反思机制学习执行错误，将纠正后的轨迹存入记忆供未来检索。
...
3. **Reflective Memory（反思记忆）**：
    ...
    - 功能：键值存储，查询向量为 key，纠正后的轨迹+反思为 value
