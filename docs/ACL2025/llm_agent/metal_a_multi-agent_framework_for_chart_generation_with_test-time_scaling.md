...
### 现有不足
1. **单模型能力不足**：即使是 GPT-4o 等 SOTA VLM，在直接生成图表代码时也难以准确解读和复现图表中复杂的视觉元素（颜色、布局、文本、类型）。
2. **现有方法效果有限**：Best-of-N 和 Hint-enhanced Prompting 相比直接 prompting 改进微弱。
3. **核心挑战**：图表生成需要同时具备强视觉设计能力和精确编码能力，这种复杂的多模态推理过程超出单模型或单智能体能力。
