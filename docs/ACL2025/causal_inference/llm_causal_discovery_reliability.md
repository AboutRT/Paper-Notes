...
description: >-
  [ACL 2025][因果发现] 利用开源 LLM（OLMo、BLOOM）可访问的预训练语料库，实证验证了"因果鹦鹉"假说——LLM 识别因果关系的能力与预训练数据中该关系的出现频率高度相关（Spearman r=0.9），且错误因果关系的存在和上下文变化都会显著影响预测可靠性。
tags:
  - ACL 2025
  - 因果发现
  - LLM可靠性
...
1. **频率-性能相关性分析（RQ1）**:
    ...
    - 功能：量化预训练数据中因果关系出现频率与 LLM 预测准确率的相关性
