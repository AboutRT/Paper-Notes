...
1. **"遗忘"问题**：SFT训练后模型会丢失文本LLM的已有能力。即使冻结LLM权重也无法完全避免
2. **数据瓶颈**：要防止遗忘需要大量多样化的标注语音指令数据，但这种数据目前极度稀缺
3. **代表性不足**：现有语音数据往往只来自少量说话人，加剧了语音处理中的偏差问题
4. **开源困难**：最先进的Speech LLM训练细节不公开，难以复现
