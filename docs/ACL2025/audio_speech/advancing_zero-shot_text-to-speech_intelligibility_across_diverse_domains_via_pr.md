...
1. **绕口令（Tongue Twisters）**：快速发音转换导致语音含糊
2. **重复词（Repeated Words）**：模型倾向于跳过或合并重复的词
3. **中英混合（Code-Switching）**：在一句话中切换中英文时，发音和韵律紊乱
4. **跨语言合成（Cross-Lingual）**：用中文参考声音说英文（或反过来）时效果差
...
2. **三类偏好对构造策略**:
   - 做什么：设计三种互补的偏好对构造方法，最大化对齐信号的信息量
   - 核心思路：
