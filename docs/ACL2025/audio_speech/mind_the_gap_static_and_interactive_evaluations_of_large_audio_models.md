...
1. **静态基准的局限性**：现有评估框架（如 AIRBench、AudioBench）从传统 ASR 任务扩展而来，使用参考答案的静态指标（WER、准确率），无法反映真实用户需求
2. **文本领域的启示**：在文本 LLM 领域，MMLU、AlpacaEval 等基准与 Chatbot Arena 的交互式评估高度相关（$\rho > 0.8$），但语音领域是否如此完全未知
3. **缺乏交互式数据**：此前从未有工作收集过 LAM 的用户偏好数据
