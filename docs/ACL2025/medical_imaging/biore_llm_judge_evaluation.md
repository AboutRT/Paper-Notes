...
**领域现状**：LLM 在生物医学关系抽取（BioRE）中已展现出强大的零样本能力，但评估仍依赖昂贵的人工标注。LLM-as-Judge 范式在通用 NLP 任务中已证明有效，但在生物医学领域尚未深入探索。

**现有痛点**：(1) LLM 生成的回答经常包含金标准的同义词或缩写（如"dex" vs "dexamethasone"），传统精确匹配指标无法正确评估；(2) 人工评估成本高、耗时长、不可扩展；(3) 直接将 LLM-Judge 应用到生物医学关系抽取时，准确率出奇地低（通常 <50%）。

**核心矛盾**：LLM 生成器的自由格式输出导致 LLM 评判者难以准确解析关系，而生物医学领域的专业术语和命名规范进一步加剧了这一问题。

**本文目标** (1) 量化 LLM-Judge 在 BioRE 评估中的实际能力；(2) 找出失败原因；(3) 提出改进方案使 LLM-Judge 更可靠。

**切入角度**：从 LLM 生成器的输出格式入手——非结构化文本是评判困难的根因。

**核心 idea**：通过将 LLM 生成器的输出规范为 JSON 结构化格式，显著提升 LLM-Judge 在 BioRE 评估中的准确率，结合域适应微调进一步增强效果。
...
1. **结构化输出格式（Structured Output Formatting）**:
    ...
    - 功能：要求 LLM-Generator 以 JSON 格式输出关系抽取结果，而非自由文本
...
2. **域适应（Domain Adaptation via Transfer Learning）**:
    ...
    - 功能：在缺乏目标领域人工评估数据时，用其他数据集的人工标注数据微调 LLM-Judge
...
- [\[ACL 2025\] Are LLMs Effective Psychological Assessors? Leveraging Adaptive RAG for Interpretable Mental Health Screening](are_llms_effective_psychological_assessors_leveraging_adaptive_rag_for_interpret.md)
