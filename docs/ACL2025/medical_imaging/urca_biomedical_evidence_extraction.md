...
description: >-
  [ACL 2025][医学图像][evidence extraction] 本文提出 URCA（Uniform Retrieval Clustered Augmentation）框架，通过均匀检索+聚类+知识提取的 RAG 流程，从 RCT 研究全文中自动提取与临床问题相关的科学证据结论…
tags:
  - "ACL 2025"
  - "医学图像"
  - "evidence extraction"
...
**本文目标** (1) 形式化定义"文档级科学证据抽取"任务；(2) 构建包含矛盾结论的基准数据集；(3) 提出适合多源证据综合的 RAG 框架。

**切入角度**：利用 Cochrane 系统综述中的 forest plot 作为标注来源——forest plot 本身就包含了每个研究对特定问题的结论标注。

**核心 idea**：通过均匀分配检索配额给各源论文、聚类检索结果、逐簇提取知识，解决多源RAG中的信息偏斜问题。
...
1. **均匀检索（Uniform Retrieval）**:
    ...
    - 功能：将检索配额 $k$ 均匀分配给每篇论文，而非全局 top-k
...
2. **聚类与知识提取（Clustering & Knowledge Extraction）**:
    ...
    - 功能：对检索到的段落做 UMAP 降维 + GMM 聚类，然后用 LLM 从每个簇中提取与 $q$ 相关的知识
...
3. **答案终结（Answer Finalization）**:
    ...
    - 功能：将所有簇提取的知识 $\langle D_1, ..., D_n \rangle$ 作为上下文，让 LLM 生成最终结论
