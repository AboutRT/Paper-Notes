...
具体而言，现有评估方法存在两大研究空白：
1. **评估方法不统一**：zsRE用逐token的greedy decoding判断，CounterFact用序列对数似然的排名，MQuAKE和RippleEdits用生成文本中的字符串匹配。不同方法是否会导致不同的编辑器排名？
2. **批量编辑的副作用**：随着编辑数量的增加，编辑器如何影响模型在通用任务上的表现？这需要一个系统性的研究框架。
