...
description: >-
  [ACL 2025][LLM对齐][implicit bias] 发现 LLM 对齐训练的矛盾效应：对齐成功消除了显式偏见（Llama 3 70B 降至 8.13%），但反而放大了隐式偏见（从 64.1% 升至 91.4%），机制是对齐使模型在歧义上下文中不再表征种族概念（"种族盲视"）…
tags:
  - "ACL 2025"
  - "LLM对齐"
  - "implicit bias"
...
- **功能**：在模型推理时注入种族感知激活，测试"增强意识 -> 减少偏见"
