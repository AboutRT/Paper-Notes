...
description: >-
  [ACL 2025][AI安全][蛋白质语言模型] KPO通过构建蛋白质安全知识图谱(PSKG)并利用图剪枝+DPO微调，使蛋白质语言模型生成的有害序列相似度降低50-80%，同时保持甚至提升功能性
tags:
  - ACL 2025
  - AI安全
  - 蛋白质语言模型
...
**本文要解决什么？** 如何在微调PLM时引入安全性约束，使其在保持功能性的同时最小化生成有害蛋白质的概率？
...
**核心idea一句话**：构建蛋白质安全知识图谱(PSKG)编码有害/安全蛋白质的生化关系，通过图结构+嵌入相似性挖掘高质量偏好对，用DPO引导PLM远离有害序列空间。
...
1. **蛋白质安全知识图谱(PSKG)构建**:
    ...
    - 做什么：构建编码有害蛋白质 $P_H$ 和安全蛋白质 $P_B$ 关系的知识图谱
...
2. **加权指标图剪枝**:
    ...
    - 做什么：将大规模PSKG裁剪为保留最关键节点的紧凑子图，显著降低计算开销
...
3. **基于图+嵌入的偏好对构造与DPO微调**:
    ...
    - 做什么：从裁剪后的PSKG中找出与每个有害蛋白质最"相似但安全"的蛋白质，构造偏好对进行DPO微调
