...
description: >-
  [ACL 2025][LLM对齐][扩散模型] 提出 DiffPO，将 LLM 对齐重新建模为句子级扩散去噪过程，通过 parallel decoding 实现高效推理时对齐，作为即插即用模块可增强任意底座模型的对齐质量。
tags:
  - "ACL 2025"
  - "LLM对齐"
  - "扩散模型"
...
**本文目标**: 实现一种高效、模型无关的对齐方法，一次训练即可应用于多个底座模型，同时减少推理延迟。

**切入角度**: 受扩散模型全局可控性启发，将对齐过程类比为句子级去噪：从未对齐的句子 y(0) 逐步修正到对齐的句子 y(T)，每步做句子级预测而非 token 级生成。

**核心 idea**: LLM 对齐 = 句子级扩散去噪过程，用一个 DiffPO 模型做 plug-and-play 对齐增强。
