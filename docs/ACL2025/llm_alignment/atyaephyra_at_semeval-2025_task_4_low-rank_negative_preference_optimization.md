...
**代码**: [https://github.com/XelfXendr/peft_unlearning](https://github.com/XelfXendr/peft_unlearning)  
**领域**: LLM对齐 / 机器遗忘  
...
**本文目标** 将 NPO 与 LoRA 结合，利用 LoRA 的结构优势在零额外内存开销下实现高质量遗忘。

**切入角度**：关键洞察——LoRA 中原始权重被冻结保留，需要计算原始模型输出时只需禁用 LoRA 层即可，无需额外存储模型副本。

**核心 idea**：LoRA 中原始权重天然保留，禁用 LoRA 即可获取参考模型的输出分布，从而零开销实现 NPO + KL 正则化遗忘。
...
1. **NPO + LoRA 组合**:
    ...
    - 功能：用 NPO 损失替代简单梯度上升来遗忘，同时用 LoRA 做参数高效微调
...
2. **KL 散度正则化 ($\mathcal{K}_{RT}$)**:
    ...
    - 功能：在保留集上最小化当前模型与原始模型输出分布的 KL 散度
