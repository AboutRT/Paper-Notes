...
1. **高质量 agent 指令难以获取**：人工标注成本过高，直接用 LLM 生成往往质量低、不切实际（指令无法关联到具体工具，或缺少执行必需信息）
2. **安全-效用平衡难以把握**：简单的安全训练容易导致模型对良性请求也过度拒绝
