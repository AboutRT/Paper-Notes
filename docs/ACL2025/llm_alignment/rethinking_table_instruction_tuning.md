...
description: >-
  [ACL 2025][LLM对齐][table understanding] 系统消融表格指令微调中被忽视的超参数选择（学习率、数据量、epoch），揭示现有表格 LLM 因学习率过大（2e-5）导致通用能力严重退化（MMLU 降 14 分、AI2ARC 降 21 分）…
tags:
  - "ACL 2025"
  - "LLM对齐"
  - "table understanding"
...
**领域**: 对齐RLHF / 指令微调  
**关键词**: table understanding, 指令微调, hyperparameter analysis, 数据效率, 灾难性遗忘
...
**本文目标**：通过系统消融找到表格指令微调中"保持通用能力 + 提升表格能力"的最优策略，证明小学习率 + 少量数据可以同时实现两个目标。

**切入角度**：不追求更多数据，而是系统探索学习率、数据量、epoch 数、多任务协同等超参数对表格性能和通用能力的影响。在 5 个不同基座模型上验证结论的普适性。

**核心 idea**：表格指令微调中，过大的学习率是通用能力退化和域外泛化差的根本原因；精心选择超参数（lr=1e-6, 2600 条数据, 2 epoch）远优于海量数据暴力训练。
...
- **功能**：在 5 个学习率（1e-7、5e-7、1e-6、5e-6、1e-5）× 多个数据量上做网格搜索，同时监测表格任务和通用任务的性能。
...
- **功能**：固定学习率，观察数据量从 50 到 1500 的性能变化曲线。
