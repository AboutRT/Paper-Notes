...
**本文目标** 如何让语音编码器直接产出与文本 LM 同等富度的语义和心理学表征？

**切入角度**：直接对齐潜在空间——用 SBERT 嵌入做教师，Whisper 嵌入做学生，对比学习拉近。

**核心 idea**：Whisper + NCE 对比损失 → 对齐到 SBERT 语义 + 心理学维度 = 无需文本 LM 的心理学语音表征。
...
1. **语义对齐（WhiSA）**:
    ...
    - 功能：将 Whisper 音频嵌入与 SBERT 文本嵌入对齐
...
2. **心理学对齐（WhiSPA）**:
    ...
    - 功能：在语义对齐基础上注入 10 维心理学特征（PsychEmb）
