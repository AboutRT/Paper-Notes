...
description: >-
  [ACL 2025][食谱推荐] 提出 KERL 统一食品推荐系统，结合 FoodKG 知识图谱和 Phi-3-mini 多 LoRA 微调，实现个性化食谱推荐（F1=0.973）、食谱生成和微量营养素估算三个功能，大幅超越基线 LLM 和传统嵌入方法。
tags:
  - ACL 2025
  - 食谱推荐
  - 推荐系统
...
**代码**: [https://github.com/mohbattharani/KERL](https://github.com/mohbattharani/KERL)  
**领域**: 推荐系统 / 知识图谱  
...
**本文目标** 如何将食品 KG 的结构化知识有效注入 LLM，构建一个能同时满足偏好约束的推荐+食谱+营养三合一系统？

**切入角度**：用 SPARQL 从 KG 检索子图作为 LLM 上下文，Multi-LoRA 为三个子任务各训练一个轻量适配器。

**核心 idea**：KG 子图检索 + Multi-LoRA 微调 Phi-3-mini = 统一的个性化食品推荐系统。
...
1. **KG 子图检索与 LLM 增强推荐（KERL-Recom）**:
    ...
    - 功能：从自然语言查询中提取实体（标签如 "vegetarian"、食材如 "spinach"），构造 SPARQL 查询检索 FoodKG 子图，将子图序列化为文本作为 LLM 上下文
