...
3. **核心矛盾**: LLM 具有强大的 next-token prediction 能力，但现有推荐框架未能直接利用这一能力，反而要求 LLM 完成更困难的多 token 标题生成任务。

4. **本文要解决什么**: 如何设计一个框架让 LLM 直接利用 next-token prediction 进行推荐，同时解决大规模物品空间下嵌入表的内存效率问题。

5. **切入角度**: 借鉴领域自适应中的词表扩展技术，为每个物品分配唯一 token，将推荐转化为单 token 预测问题。

6. **核心 idea 一句话**: 扩展 LLM 词表使每个物品对应一个唯一 token，通过 next-token prediction 的 logits 直接推荐，用哈希压缩解决嵌入表内存瓶颈。
