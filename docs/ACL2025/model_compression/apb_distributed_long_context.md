...
description: >-
  [ACL 2025][模型压缩][long-context inference] APB 提出了一种分布式长上下文推理框架，通过在序列并行框架中引入本地 KV cache 压缩和跨 GPU 传递压缩上下文块的机制…
tags:
  - "ACL 2025"
  - "模型压缩"
...
## 研究背景与动机
**领域现状**：随着 LLM 支持的上下文长度不断增加（Llama-3.1 128K、Claude-3.5 200K、MiniMax-01 4M），长上下文推理的 prefill 阶段成为关键瓶颈，注意力计算复杂度为 $O(n^2)$
...
**核心矛盾**：现有 KV cache 压缩方法需要全局注意力分数来判断哪些 KV 重要，但序列并行中每个 host 只持有部分上下文，无法获取全局视图

**本文目标** 设计一种在序列并行框架内同时减少计算量和通信开销的近似注意力机制，且随 GPU 数量增加仍保持稳定性能

**切入角度**：使用本地 KV cache 压缩（无需全局视图）+ 压缩后跨 GPU 传递关键 KV 对

**核心 idea**：每个 GPU 独立压缩本地 KV cache，然后只传递压缩后的关键上下文给其他 GPU，实现计算和通信的双重减少
...
2. **本地 KV Cache 压缩（Block Compression）**:
    ...
    - 功能：在每个 host 上独立压缩本地 KV cache，保留最关键的 $l_p$ 个 KV 对
...
3. **AllGather 通信 + Passing Block 构建**:
    ...
    - 功能：收集所有 host 的压缩 KV cache，为每个 host 构建包含前序所有压缩上下文的 passing block
...
4. **定制 FlashAttn Kernel + 修改的注意力掩码**:
    ...
    - 功能：使用修改的 attention mask 在 anchor + passing + local 三部分拼接的 KV 上计算注意力
