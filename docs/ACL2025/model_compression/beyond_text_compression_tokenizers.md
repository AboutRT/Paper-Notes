...
description: >-
  [ACL 2025][模型压缩][tokenizer evaluation] 本文系统评估了 6 种 tokenizer 在 350M 和 2.7B 参数模型上的影响，发现 tokenizer 选择对英文任务影响极小但对多语言任务（如机器翻译）有显著且跨尺度一致的影响…
tags:
  - "ACL 2025"
  - "模型压缩"
...
1. **Scaling Consistency 实验设计**：
    ...
    - 功能：隔离 tokenizer 影响——所有模型使用相同架构、数据、训练配置，唯一变量是 tokenizer
...
3. **两阶段预测框架**：
    ...
    - 功能：组合多个内在指标形成可靠的 tokenizer 评估框架
