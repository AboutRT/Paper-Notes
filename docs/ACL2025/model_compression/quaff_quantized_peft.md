...
**本文目标** 解耦权重和激活量化的依赖关系，使 INT8 WAQ 在微调场景下既高效又准确。

**切入角度**：观察到异常通道的空间位置在微调期间保持稳定（OSSH），因此只需预先识别这些通道并定向处理即可。

**核心 idea**：利用激活异常通道空间稳定性，仅对少量（<5%）不变通道做动态缩放，解耦权重-激活量化依赖。
...
1. **Outlier Spatial Stability Hypothesis (OSSH)**:
    ...
    - 功能：提出并验证假设——微调期间，激活异常通道的空间位置保持稳定
...
2. **解耦 WAQ 公式**:
    ...
    - 功能：将通道缩放 $Y = \hat{X}(sW)$ 分解为静态项和动态项
