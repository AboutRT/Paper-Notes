...
**本文目标** 在保持 LoRA 低秩效率的前提下，实现联邦 LoRA 的精确聚合。

**切入角度**：将聚合误差（理想更新与实际更新之差）作为残差项吸收到已经是高秩的预训练冻结权重矩阵中，无需额外训练。

**核心 idea**：把"均值的乘积"与"乘积的均值"之间的残差误差加到冻结基础权重上，既保持 LoRA 低秩训练又实现精确聚合。
