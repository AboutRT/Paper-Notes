...
**本文目标** 统一 UQ 和 BCQ，同时获得两者的优势。

**切入角度**：分析发现 UQ 的优化性来源于其变换过程 $\mathcal{T}$，BCQ 的表现力来源于其映射过程 $\mathcal{M}$——两者可以组合。

**核心 idea**：将 UQ 的可微变换过程嵌入 BCQ 的非均匀映射过程中，统一后通过合并定理消除部署时的额外开销。
...
1. **UniQuan 统一框架**:
    ...
    - 功能：将 UQ 的变换过程和 BCQ 的映射过程组合成统一量化方案
...
2. **统一初始化（Unified Initialization）**:
    ...
    - 功能：为 UniQuanF 中 FlexRound 和 Alternating 的参数提供联合初始化
...
4. **统一定理（Unification Theorem）**:
    ...
    - 功能：证明优化后的 UniQuanF 可以等价转换为标准 BCQ 格式，消除部署额外开销
