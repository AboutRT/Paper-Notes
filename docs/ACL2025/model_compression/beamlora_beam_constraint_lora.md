...
description: >-
  [ACL 2025][模型压缩][LoRA] BeamLoRA 发现 LoRA 模块中不同 rank 的重要性存在显著差异且随训练动态演变，受 beam search 启发，提出在训练过程中动态评估 rank 重要性、剪枝不重要的 rank 并将参数空间扩展给重要 rank，在固定总 rank 下提升性能…
tags:
  - "ACL 2025"
  - "模型压缩"
  - "LoRA"
...
## 研究背景与动机
**领域现状**：LoRA 是最流行的 PEFT 方法，通过低秩矩阵 $BA$ 近似权重更新。现有改进如 AdaLoRA（跨模块分配 rank）、DoRA（方向/幅度解耦）

**现有痛点**：现有方法将每个 rank 视为同质单元，为所有 rank 分配相同的参数空间，但实验发现不同 rank 的重要性（$\Delta w_i = b_i a_i$ 的 Frobenius 范数）差异极大——剪枝低重要性 rank 几乎不影响性能，而少数高重要性 rank 决定了大部分效果

**核心矛盾**：重要 rank 的优化空间被限制在一行一列，而不重要的 rank 浪费了同等参数预算

**本文目标** 在固定 rank 总数下，将参数空间从不重要 rank 重新分配给重要 rank

**切入角度**：将 LoRA 模块视为 beam search 中的 beam——每个 rank 是一个子解，训练过程是搜索最优子解组合

**核心 idea**：用可学习 score 向量评估 rank 重要性，周期性剪枝低分 rank 并复制高分 rank 的参数来扩展其空间
...
1. **可学习重要性评估**:
    ...
    - 功能：在 $B$ 和 $A$ 之间插入 score 向量，通过 softmax 归一化后缩放每个 rank 的输出
...
2. **剪枝与扩展**:
    ...
    - 功能：周期性将低重要性 rank 的参数空间转移给高重要性 rank
