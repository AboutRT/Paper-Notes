...
1. **领域现状**：LLM Agent 需要调用外部工具（API/函数）来完成复杂任务。当前模型在单步 FC 上表现不错，但在 **多轮、多步骤**交互中仍面临挑战。
2. **多轮 FC 的三大挑战**：
...
3. **数据瓶颈**：BFCL-v3 上最佳开源模型多轮成功率仅 ~10%，现有模型在多轮场景上的低性能导致难以收集高质量训练轨迹
4. **核心动机**：设计一个有原则的数据合成管道，从图的视角构建可靠的多轮 FC 训练数据
