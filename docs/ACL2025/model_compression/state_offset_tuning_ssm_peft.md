...
description: >-
  [ACL 2025][模型压缩][state space model] 针对 SSM（如 Mamba）提出 State-offset Tuning，一种新的"状态基"PEFT 方法家族，通过在每个时间步直接注入可训练的状态偏移量 $h'$ 替代 Prefix-Tuning 的虚拟 token…
tags:
  - "ACL 2025"
  - "模型压缩"
  - "state space model"
...
1. **Prefix-Tuning 在 SSM 上的局限性分析**:
    ...
    - 功能：数学证明 Prefix-Tuning 等价于 Initial State Tuning
...
2. **State-offset Tuning (h-offset)**:
    ...
    - 功能：在每个时间步的 SSM 输出上加一个与位置无关的可训练偏移
