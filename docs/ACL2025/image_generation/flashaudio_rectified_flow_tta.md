---
title: >-
  [论文解读] FlashAudio: Rectified Flows for Fast and High-Fidelity Text-to-Audio Generation
description: >-
  [ACL 2025][图像生成][文本转音频] 将整流流（Rectified Flow）引入文本转音频生成，通过双焦采样器优化时间步分布、不混溶流减少数据-噪声总距离、锚定优化修正 CFG 引导误差，实现单步生成 FAD=1.49 超越百步扩散模型，生成速度达实时 400 倍。
tags:
  - "ACL 2025"
  - "图像生成"
  - "文本转音频"
...
**本文目标** 将整流流应用于 TTA，解决其训练中时间步分配不佳、噪声-数据配对次优、CFG 误差放大等实际问题。

**切入角度**：整流流已在图像生成（Stable Diffusion 3/InstaFlow）和 TTS 中成功应用，但 TTA 领域尚未探索。从预训练流匹配模型初始化可加速收敛。

**核心 idea**：用整流流学直线路径实现快速模拟，配合三项训练优化使单步生成质量超越百步扩散模型。
