...
1. **相似度估计器失效**：在现代 GEC 系统的输出上，PLM 无法准确捕捉语义变化。例如 "healty→healthy" 的拼写纠错反而导致相似度降低被错误过滤，而 "like→dislike" 这种语义反转却获得高相似度
2. **质量估计器不够强**：原始 IMPARA 使用的 vanilla PLM 缺乏对语法错误的细粒度感知能力
