...
1. **领域现状**：多 LLM 协作（如多智能体辩论）近年受到关注，但现有方法主要依靠 prompting 开箱即用的 LLM，期望它们"天然"具备协作能力。
2. **现有方法的不足**：
...
3. **核心动机**：需要一个交互式训练环境，让多个 LLM 同时训练、动态优化各自策略，从而显式地学习协作行为。博弈论分析表明，共同训练的智能体能够达到展现合作行为的均衡点。
