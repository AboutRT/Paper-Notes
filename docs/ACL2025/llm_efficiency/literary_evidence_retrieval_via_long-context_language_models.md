---
description: "【论文笔记】Literary Evidence Retrieval via Long-Context Language Models 论文解读 | ACL 2025 | arXiv 2506.03090 | 长上下文理解 | 将 RELiC 数据集改造为长上下文文学证据检索 benchmark（292 个高质量样本），要求模型在完整小说文本（45k-125k tokens）中为文学评论找到缺失引用；Gemini Pro 2.5 以 62.5% 准确率首次超越人类专家（55%），但最佳开源模型 DeepSeek-R1 仅 29.1%，揭示了闭源/开源模型在解释性推理上的巨大鸿沟。"
tags:
  - ACL 2025
  - 长上下文理解
  - 文学证据检索
...
1. **长上下文评测的不足**：现有长上下文 benchmark（如 Needle-in-Haystack）主要测试简单信息检索，无法衡量模型对长文本的深度理解和推理能力。
2. **文学分析的独特需求**：文学证据检索要求模型同时具备对叙事的全局理解（global narrative reasoning）和对具体段落的精细阅读能力（close textual examination），是检验长上下文真正理解能力的理想测试场景。
3. **RELiC 数据集的局限**：原始 RELiC 数据集（Thai et al., 2022）包含 78k 条文学分析摘录，但数据较嘈杂，存在 OCR 错误、引用泄露、数据污染等问题，不适合直接用于 LLM 评测。
4. **核心动机**：在长上下文 LLM 能处理百万 token 的今天，探索它们对文学小说的理解到底达到了什么水平。
