...
当前长上下文训练面临多重挑战：
1. **计算资源需求巨大**：Ring Attention 原始工作报告 512K 序列需 16×A100（80GB）才能训练 7B 模型
2. **大模型不等于强长上下文**：Llama-3.1-8B 在 RULER 上反而超过 70B 版本
3. **隐蔽的数值精度问题**：bfloat16 在大位置索引下的精度损失未被充分认知
