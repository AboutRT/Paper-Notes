...
description: >-
  [ACL 2025][社会计算][misinformation] 构建了目前最大的误信息评估基准 MisBench（1034 万条误信息），从知识冲突类型和文本风格两个维度系统分析 LLM 对误信息的行为和偏好，并提出 RtD 方法结合外部知识源提升误信息检测能力。
tags:
  - "ACL 2025"
  - "社会计算"
  - "misinformation"
...
**本文目标** (1) 构建大规模多维度误信息基准。(2) 系统分析 LLM 对不同类型/风格误信息的响应模式。(3) 提出改进 LLM 误信息检测的方法。

**切入角度**：从知识冲突（事实冲突、时间冲突、语义冲突）和文本风格（6 种写作风格）两个正交维度构建误信息，用 Wikidata 一跳和多跳关系保证覆盖广度。

**核心 idea**：构建 1034 万条覆盖 3 种冲突类型 × 6 种文本风格的误信息基准，揭示 LLM 对不同形式误信息的差异化脆弱性。
...
2. **六种文本风格化**:
    ...
    - 功能：将每条误信息转化为维基百科条目、新闻报道、科学文献、博客、技术语言、自信语言六种风格。
...
3. **Reconstruct to Discriminate (RtD)**:
    ...
    - 功能：利用 LLM 识别上下文不一致性的能力，结合外部知识源重建关键实体的证据文本来辨别误信息。
