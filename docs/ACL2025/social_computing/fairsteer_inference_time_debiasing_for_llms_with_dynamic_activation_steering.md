...
1. **领域现状**：LLM 会从训练语料中继承种族、性别、年龄等社会偏见，对弱势群体产生负面影响。去偏是 AI 对齐的重要组成部分。
2. **现有痛点**：Prompt 方法（如 CAL）对措辞敏感、效果不稳定；微调方法（投影、对比学习、强化学习）需要大量计算资源和标注数据，还有灾难性遗忘风险；解码策略方法（如约束搜索、重排序）会降低输出多样性且多针对旧模型。
3. **核心矛盾**：如何在不重训、不依赖复杂 prompt 设计的前提下，精准地在推理阶段缓解偏见，同时保持模型原有能力？
4. **本文要解决什么**：设计一种推理时的去偏框架，仅在检测到偏见时进行干预，避免对无偏输出的破坏。
5. **切入角度**：基于线性表示假说——真值、情感、幽默等语义特征在 LLM 激活空间中编码为线性可分方向——验证公平性特征是否同样线性可分。
6. **核心 idea**：偏见特征在中间层激活空间中呈线性可分（>90% 分类准确率），可以通过几何干预——沿去偏方向平移激活向量——实现去偏。
...
- **做什么**：在每层训练一个轻量线性分类器 $C^l$，判断最后一个 token 的激活向量 $\mathbf{a}^l$ 是否对应偏见输出。
...
- **做什么**：计算一个几何上可解释的干预方向，捕获偏见→无偏的激活空间偏移。
