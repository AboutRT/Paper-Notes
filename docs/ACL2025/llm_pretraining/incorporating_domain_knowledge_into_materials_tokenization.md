...
1. **领域现状**：语言模型在材料科学中的应用日益增多（MatSciBERT, BatteryBERT 等），但这些模型沿用了通用 NLP 的频率驱动分词方法（BPE、WordPiece）。
2. **现有方法的不足**：
   - 材料相关术语（化学式、材料名称）在语料中出现频率极低，而频率驱动分词优先保留高频词
   - 低频的材料概念被拆分成语义无关的子词。例如 "germanium"（锗元素）被拆为 "german" + "ium"，完全丧失化学含义
   - 之前的子词分词改进方法如 SAGE、PickyBPE 面向通用领域，未针对材料科学场景
3. **核心动机**：需要在分词训练阶段就引入领域知识，确保材料概念在分词时保持结构和语义完整性，而不仅仅依赖词频统计。
