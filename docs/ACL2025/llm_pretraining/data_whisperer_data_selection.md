...
description: >-
  [ACL 2025][data selection] Data Whisperer 提出一种无需训练的注意力加权 few-shot ICL 数据选择方法，利用预训练模型自身的 ICL 能力和注意力分数为训练样本打分，仅用 10% 数据即可超越全量微调性能，同时比现有方法快 7-20 倍。
tags:
  - ACL 2025
  - data selection
  - in-context learning
...
## 研究背景与动机
**领域现状**：LLM 微调需要在任务特定数据上训练，数据集越来越大，数据选择（coreset selection）成为平衡性能和计算成本的关键问题
...
**核心矛盾**：数据选择本身的开销应该远小于微调开销，但现有方法的 Selection-to-Tuning Ratio (STR) 普遍 > 1，选择时间甚至超过了微调时间

**本文目标** 设计一种 training-free 的数据选择方法，在保证选择质量的同时大幅降低选择时间

**切入角度**：ICL 在理论上等价于隐式微调（ICL ≈ implicit fine-tuning），因此可以用 ICL 的表现来预测微调的效果

**核心 idea**：用 few-shot ICL 中各示例的注意力加权得分来评估每个训练样本对任务的贡献价值
...
1. **Selection-to-Tuning Ratio (STR) 指标**:
    ...
    - 功能：定义 $\text{STR} = t_p(\tau, \rho) / t_{ft}$，量化选择方法的效率
    - 核心思路：STR < 1 意味着选择时间低于全量微调时间，才有实际价值
    - 设计动机：揭示了现有方法的关键缺陷——大多数方法 STR > 1，比直接全量训练还慢

2. **Few-shot ICL 打分机制**:

    - 功能：每次从数据集中采样 $n_d$ 个 demonstration 和 $n_q$ 个 query，用预训练模型做 ICL 推理
...
3. **Context-Aware Attention Weighting**:
    ...
    - 功能：用自注意力分数加权 demonstration 的得分，消除 ICL 中的位置偏差
...
4. **Weak-to-Strong 策略**:
    ...
    - 功能：用同系列更小的模型做 ICL 打分（如用 Qwen-2.5-3B 为 Qwen-2.5-7B 打分）
