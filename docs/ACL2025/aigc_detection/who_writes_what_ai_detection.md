...
**本文目标** (a) 作者属性是否以及如何影响AI文本检测准确率？(b) 哪些属性影响最大、最一致？(c) 不同检测器对属性偏差的敏感度有何差异？

**切入角度**：利用ICNALE学习者语料库（自带丰富元数据标注）构造人写+AI生成的平行文本，在out-of-domain条件下评估多种检测器，用多因素统计分析隔离各属性的独立影响。

**核心 idea**：将"谁在写"这一社会语言学维度引入AI文本检测评估，用多因素WLS+ANOVA框架量化作者属性导致的检测偏差。
...
1. **67K平行数据集构建**:
    ...
    - 功能：构建一个同时包含人写文本和AI生成文本、且每篇文本都带有详细作者属性标注的数据集
...
2. **多检测器Out-of-Domain评估**:
    ...
    - 功能：用9个不同类别的现成检测器在构建的数据集上进行zero-shot评估
...
3. **多因素WLS + Type II ANOVA偏差分析框架**:
    ...
    - 功能：量化各作者属性对检测准确率的独立影响，同时控制其他属性的混杂效应
