...
- **本文要解决什么**：构建一个覆盖全球敏感话题的多语言问答数据集，并通过严格的统计假设检验量化不同语言之间的伦理偏差，验证该偏差在多个LLM中是否普遍存在。
- **切入角度**：从Human Rights Watch的17个人权话题出发生成敏感问题，扩展至6种语言，分别对回答的拒绝行为（离散）和内容分布（连续）设计两种互补的统计检验。
- **核心idea一句话**：用McNemar检验和PERMANOVA检验两种统计假设检验方法，在控制所有其他变量的前提下，验证"仅改变语言不应导致回答差异"这一零假设在几乎所有语言对和模型组合中被拒绝。
