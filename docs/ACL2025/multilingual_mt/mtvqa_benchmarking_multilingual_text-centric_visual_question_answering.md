...
1. **语言覆盖偏差**：现有 TEC-VQA 基准几乎只关注英文和中文等高资源语言，低资源语言（阿拉伯语、泰语、越南语等）被严重忽视
2. **翻译方案的根本缺陷**：之前的多语言 VQA 工作（xGQA、MaXM）通过翻译引擎扩展问答对，但在 TEC-VQA 场景下产生严重的"视觉-文本不对齐"问题——翻译只处理了问答文本，忽略了图像中的视觉文本
