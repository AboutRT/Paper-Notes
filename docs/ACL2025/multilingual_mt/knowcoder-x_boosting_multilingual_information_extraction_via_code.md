...
- **本文要解决什么**: 增强 LLM 在 IE 任务中的跨语言对齐能力，使得在中英文训练的模型能迁移到 29 种未见过的语言
- **切入角度**: 利用代码（Python 类）统一多语言 schema 表示，并通过翻译实例预测任务进行跨语言对齐训练
- **核心 idea 一句话**: 用 Python 类统一多语言 IE schema + 跨语言对齐指令微调 = 强大的零样本跨语言 IE 迁移
...
- **做什么**: 将所有语言的 IE schema 映射为统一的 Python 类定义
...
#### 4. ParallelNER 数据集
- **做什么**: 构建高质量中英 NER 平行数据集，共 257,190 样本
