...
1. **领域现状**：即使未显式使用平行数据训练，LLM 也展现出翻译能力——被归因于训练数据中的偶然双语信号。一些主流多语言 LLM（如 BLOOM）选择不使用平行数据。
2. **现有痛点**：平行数据对 encoder 模型有效已被证明（如 XLM），但 decoder-only LLM 是否也受益？最优使用策略（何时加入、加多少）未知。
3. **核心矛盾**：decoder LLM 通常忽略平行数据——这是否是个错误决策？
4. **本文要解决什么？** 通过控制实验系统回答平行数据的价值和最优使用方式。
5. **切入角度**：固定总训练量，对比 7 种不同数据策略（无平行/单语替代/平行前置/分散/末置等）。
6. **核心idea一句话**：平行数据放在训练末期对多语言能力的提升最大；放在开始会导致严重的灾难性遗忘。
