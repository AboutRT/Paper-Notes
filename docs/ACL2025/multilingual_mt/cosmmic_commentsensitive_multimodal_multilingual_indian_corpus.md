...
description: >-
  [ACL 2025][多语言] 构建首个面向印度语言的评论感知多模态多语言数据集COSMMIC（9种语言、4,959篇文章-图像对、24,484条评论）。
tags:
  - ACL 2025
  - multilingual
  - 多模态
...
**领域现状**: 读者评论包含对文章的反应/情感/补充信息，可增强摘要和标题生成质量，但现有数据集几乎都是英语。**现有痛点**: 无面向印度语言的评论感知多模态数据集；现有多语言摘要资源未结合读者评论和配图。**核心矛盾**: 印度有数十亿互联网用户使用多种语言——但NLP资源严重匮乏。**本文要解决什么**: 构建首个同时覆盖多语言+多模态+评论感知的印度语言数据集。**切入角度**: 从印度主流新闻网站爬取文章+图像+评论。**核心idea**: 评论不仅是噪声——经过质量过滤后可提供有价值的上下文信号增强生成任务。
...
1. **9语言数据爬取与标准化**:
    - 做什么：从印度主流新闻网站爬取9种语言的文章-图像-评论三元组
