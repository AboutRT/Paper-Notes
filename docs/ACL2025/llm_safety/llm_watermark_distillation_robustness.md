---
title: >-
  [论文解读] Can LLM Watermarks Robustly Prevent Unauthorized Knowledge Distillation?
description: >-
  [ACL 2025][LLM安全][LLM水印] 本文首次系统研究 LLM 水印在防止未授权知识蒸馏中的鲁棒性，提出三种水印去除攻击（无目标/有目标释义 + 推理时水印中和），发现有目标释义和水印中和可以彻底去除继承的水印，其中水印中和在保持知识迁移效率的同时实现零额外训练开销的水印去除。
tags:
  - "ACL 2025"
  - "LLM安全"
  - "LLM水印"
...
**领域现状**：OpenAI、Anthropic 等主要 LLM 服务商在使用条款中禁止用其输出训练竞争模型。水印技术（如 KGW、Google 的 SynthID-Text）被视为监控未授权知识蒸馏的有前景方案：水印具有"放射性"——在水印教师模型输出上训练的学生模型会继承可检测的水印模式。

**现有痛点**：虽然水印放射性已被验证（p 值可低至 $10^{-30}$），但对抗性攻击者是否能在保持知识迁移的同时去除继承水印，这一关键安全问题尚未被系统研究。

**核心矛盾**：水印的设计需要鲁棒到即使被蒸馏也能被追踪，但攻击者可以利用释义模型或推理时干预来破坏水印统计特征。

**本文目标** 系统评估 LLM 水印作为知识产权保护机制对抗性攻击下的鲁棒性，并提出更有效的攻击方法。

**切入角度**：分析水印放射性的关键因素（前缀频率和窗口大小），据此设计水印窃取技术，然后利用窃取的规则进行有针对性的水印去除。

**核心 idea**：通过对比蒸馏前后学生模型的 token 概率分布来窃取水印规则，再用逆水印操作在训练数据释义或推理时解码阶段彻底去除水印。
