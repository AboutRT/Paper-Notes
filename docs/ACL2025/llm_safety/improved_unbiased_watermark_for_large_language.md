...
**代码**: [https://github.com/RayRuiboChen/MCMark](https://github.com/RayRuiboChen/MCMark)  
**领域**: AI安全  
...
**本文目标** 在严格保持无偏性的前提下，大幅提升可检测性和鲁棒性。核心想法是：从 2 个通道推广到 $l$ 个通道。

**切入角度**：作者引入"分布通道"（Distribution Channel）这一抽象概念，将水印问题形式化为一个带约束的优化问题——最大化在水印键选中通道下生成对应段 token 的总概率，同时约束每个通道必须是有效概率分布且期望等于原始分布。

**核心 idea**：将词表均分为 $l$ 段，构建 $l$ 个分布通道，每个通道提升对应段的 token 概率，通过求解闭式最优解实现在保持无偏性前提下可检测性最大化。
...
1. **分布通道（Distribution Channel）概念**：
    ...
    - 功能：将水印后的分布空间抽象为一组"通道" $\{P_1, \dots, P_l\}$，每个通道是一个完整的概率分布
...
3. **水印键设计与 n-gram 策略**：
    ...
    - 功能：用秘密密钥 $\mathsf{sk}$ 和前 $n$ 个 token 的 n-gram 作为伪随机种子选择通道
