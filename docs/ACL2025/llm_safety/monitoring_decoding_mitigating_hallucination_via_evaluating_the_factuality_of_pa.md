---
description: "【论文笔记】Monitoring Decoding: Mitigating Hallucination via Evaluating the Factuality of Partial Response during Generation 论文解读 | ACL 2025 | arXiv 2503.03106 | 幻觉缓解 | 提出 Monitoring Decoding (MD) 框架，在生成过程中动态监控部分响应的事实性，通过监控函数识别易产生幻觉的 token 并利用树搜索策略选择性地修正这些关键 token，从而在保持效率的同时显著提升事实准确性。"
tags:
  - ACL 2025
  - 幻觉缓解
  - 解码策略
...
1. **全长采样效率低**：Best-of-N (BoN) 策略和自一致性方法需要生成多个完整响应，引入显著的延迟开销。
2. **过度自信问题**：模型对幻觉 token 可能表现出极高的置信度，导致多次采样仍然生成相同的错误输出。自一致性高并不等于事实正确。
3. **关键发现**：作者观察到，通常只有少数关键 token 导致幻觉，只需替换这些 token（如将 "24" 替换为 "It"）就可以将错误响应转变为正确响应。这意味着不需要重新采样整个响应。
...
3. **树搜索修正 (Tree-based Revision)**：
   - 做什么：对被拒绝的 m 个 token 逐个进行树搜索式重新生成
