...
description: >-
  [ACL 2025][LLM安全][幻觉缓解] 提出NOVA框架通过内部一致性探测(ICP)和语义等价识别(SEI)筛选与LLM已有知识一致的指令数据，避免训练不熟悉的数据导致幻觉，在仅用5%数据的情况下同时提升指令遵循能力和大幅降低幻觉。
tags:
  - ACL 2025
  - LLM安全
  - 幻觉缓解
...
**本文要解决什么**：在指令微调阶段，同时实现"遵循指令"和"减少幻觉"——筛选出既高质量又知识对齐的指令数据。
...
3. **专家对齐质量奖励模型**:
    - 做什么：用3751条专家标注偏好数据训练reward model，评估数据质量
