---
description: "【论文笔记】UAQFact: Evaluating Factual Knowledge Utilization of LLMs on Unanswerable Questions 论文解读 | ACL 2025 | arXiv 2505.23461 | 不可回答问题 | 提出双语不可回答问题数据集UAQFact（13,970题），每个问题附带知识图谱事实知识，定义三个评估任务分别衡量LLM区分不可回答问题（UAQ）与可回答问题（ABQ）、利用内部/外部事实知识处理UAQ的能力，实验揭示即使LLM已存储相关知识也难以有效利用。"
tags:
  - ACL 2025
  - 不可回答问题
  - 事实性知识
...
### 已有工作不足
1. **缺乏事实知识支撑**：现有UAQ数据集（SelfAware、FalseQA、UnknownBench等）的问题来自网络抓取、头脑风暴或实体替换，仅提供答案/标签而不包含支撑性事实知识，无法评估LLM利用知识处理UAQ的能力
2. **仅支持英语**：已有数据集均为英文，无法评估跨语言泛化能力
3. **评测维度单一**：已有数据集仅支持UAQ/ABQ二分类任务，缺少对内部知识利用和外部知识利用的深度评估
