...
## 研究背景与动机
**领域现状**: PEFT（特别是LoRA）已成为LLM下游任务适配的主流方法，但实际场景中训练数据常含噪声标签（noisy labels），现有方法对此关注不足。

**现有痛点**: 主流LNL方法基于"小损失"策略用训练损失选择干净样本，但选出的样本又影响后续训练的损失计算，形成恶性循环（catch-22）——准确的样本选择需要好的泛化能力，而好的泛化能力又需要准确的样本选择。

**核心矛盾**: 样本选择依赖正在训练的模型（in-training model），导致选择偏差随训练累积放大，特别在高噪声场景下表现退化严重。

**本文目标**: 将噪声标签检测（样本选择）与分类模型训练完全解耦，避免恶性循环。

**切入角度**: 利用PEFT模块的有限容量特性，设计双LoRA分别记忆干净和噪声样本。

**核心 idea**: 用两个LoRA分别吸收干净数据和噪声数据的记忆，以此构建可学习的噪声标签检测阈值。
