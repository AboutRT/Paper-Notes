...
### 现有不足
1. **数据匮乏**：现有 MCR 数据集主要集中于人机对话（如 J-CRe3）、电影叙述（如 MPII-MD）或图像描述（如 CIN），无法充分反映真实社交媒体中多模态交互的复杂性和多样性。
2. **语言覆盖偏向英/日**：大多数研究聚焦英语，中文 MCR 研究几乎空白。
3. **隐式视觉线索挑战**：在社交媒体真实对话中，说话者往往省略对可见物体外观或位置的描述，使得模型难以从文本中提取视觉线索来定位被提及的对象。
