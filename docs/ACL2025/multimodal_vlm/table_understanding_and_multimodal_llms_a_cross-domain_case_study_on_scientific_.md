...
1. **数据源偏向**: 大多数表格理解（Table Understanding, TU）研究只关注非科学来源的表格（如 Wikipedia、金融报告），而对科学文献中的表格关注极少。科学表格包含大量专业术语、复杂缩写和密集数值，需要领域知识和更强的算术推理能力。
2. **模态影响未知**: 表格可以用图片（PNG/JPEG）或多种文本格式（HTML、XML、LaTeX、Dict）表示，但不同表示模态对模型性能的影响尚不清楚。
3. **可解释性缺失**: 现有 TU 研究很少从可解释性角度分析模型如何利用表格中的上下文信息做决策。
