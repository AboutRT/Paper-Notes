...
现有解决方案的不足：
1. **数据增强方法**：自动合成训练样本容易引入偏差（bias），甚至导致模型崩溃（model collapse），模型倾向于忘记人类生成数据的真实分布
2. **基于相似度的数据选择**：利用 n-gram、任务指令或梯度的特征匹配外部数据，但要么依赖外部数据与目标任务的表面形式高度一致，要么对大规模外部数据集进行反向传播计算量过大
