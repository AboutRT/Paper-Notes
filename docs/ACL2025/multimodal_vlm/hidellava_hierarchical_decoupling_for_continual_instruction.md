---
title: >-
  [论文解读] HiDe-LLaVA: Hierarchical Decoupling for Continual Instruction Tuning of Multimodal Large Language Model
description: >-
  [ACL 2025][多模态VLM][持续学习] 通过 CKA 分析发现 MLLM 顶层学任务特异信息而其余层学通用知识，提出 HiDe-LLaVA：顶层 LoRA 做 MoE 式任务特异扩展（双模态锚点匹配）+ 其余层 LoRA 做均匀融合，在新构建的无信息泄露基准 UCIT 上比最佳基线提升 5.8%。
tags:
  - "ACL 2025"
  - "多模态VLM"
  - "持续学习"
...
**现有痛点**：(1) 现有基准 CoIN 存在**信息泄露**——下游数据集与 LLaVA SFT 阶段重叠，multi-task 与 zero-shot 差距仅 13.19%；(2) 现有方法（MoELoRA、O-LoRA）缺乏理论指导，不清楚应该在哪些层做任务隔离、哪些层做知识共享。

**核心矛盾**：全部扩展（expand all layers）性能好但参数量爆炸（5.2×）；全部融合（merge all layers）参数少但性能下降。需要找到精确的"哪些层该隔离、哪些该共享"的分界点。

**本文目标**：(1) 修正现有基准的信息泄露问题；(2) 基于层级分析提出高效的持续学习方法。

**切入角度**：用 CKA 相似度分析模型各层在不同任务上的输出差异，发现仅顶层（layer 32）表现任务特异性，其余层高度相似。

**核心 idea**：仅在顶层做 MoE 式任务特异扩展，其余 31 层直接均匀融合 LoRA，以最小参数开销实现最佳持续学习效果。
...
1. **CKA 驱动的层级解耦**:
    ...
    - 功能：分析 6 个指令微调数据集上 LLaVA 各层的 CKA 相似度
...
2. **任务特异扩展（顶层 MoE）**:
    ...
    - 功能：训练时为每个任务提取双模态特征锚点（视觉用 LLaVA 视觉编码器均值，文本用 CLIP 文本编码器均值）
...
3. **任务通用融合（其余层）**:
    ...
    - 功能：将所有任务在第 1-31 层的 LoRA 参数均匀融合 $\bar{E}_T = \sum_{i=1}^T \epsilon_i E_i$，系数 $\epsilon_i = 1.0$
