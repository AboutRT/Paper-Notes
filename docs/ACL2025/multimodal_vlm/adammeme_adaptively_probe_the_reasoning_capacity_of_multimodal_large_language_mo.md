---
description: "【论文笔记】AdamMeme: Adaptively Probe the Reasoning Capacity of Multimodal Large Language Models on Harmfulness 论文解读 | ACL 2025 | arXiv 2507.01702 | meme理解 | 提出AdamMeme——一个基于多智能体协作的自适应评估框架，通过迭代生成更具挑战性的meme样本来探测多模态大语言模型(mLLM)在有害内容理解上的推理能力和特定弱点。"
tags:
  - ACL 2025
  - meme理解
  - 有害性检测
...
1. **静态数据集**：评估基于固定的数据集（如Hateful Memes），无法跟上网络meme的动态演变
2. **模型无关性**：所有模型使用相同的测试集，无法针对特定模型进行深度探测
3. **仅关注准确率**：只看对/错比例，无法提供模型弱点的细粒度分析
4. **缺乏挑战性**：简单样本占多数，难以区分模型间的真实能力差异
...
1. **有害性挖掘Agent（Harmfulness Mining）**:
   - 做什么：从原始meme数据中识别有害性属性，并系统性地分解有害性的细粒度维度
...
2. **多维度模型评分（Model Scoring）**:
   - 做什么：从多个角度评估目标mLLM对meme有害性的理解能力
...
3. **迭代精炼Agent（Iterative Refinement）**:
   - 做什么：根据模型的错误模式，自适应地调整评估数据的难度和分布
