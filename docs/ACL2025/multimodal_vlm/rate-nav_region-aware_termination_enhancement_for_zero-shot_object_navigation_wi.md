...
1. **冗余探索**：传统方法要求完全搜索当前区域后才移动到下一个区域。但作者观察到，**探索步数与探索率之间存在边际递减效应**——前 5 步可探索区域的 55%，但后续每步边际收益急剧下降。

2. **探索重复失败**：由于视觉感知精度有限，一个区域虽然大部分已被探索，但因小块未知区域触发重复边界设定，导致反复搜索同一区域。

3. **缺乏探索终止策略**：现有研究关注语义地图构建和目标方向预测，但"何时终止当前区域探索"这一关键问题被严重忽视。
