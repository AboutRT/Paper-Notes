...
**本文要解决什么？** 诊断并缓解 MLLM 长链推理中的视觉遗忘现象。

**切入角度**：通过"渐进式图像移除"实验量化视觉遗忘程度，再设计训练+推理双阶段的视觉再注入方案。

**核心 idea 一句话**：在推理链的关键阶段重新注入压缩后的视觉 token，模拟人类"回看图像"行为来保持视觉注意力。
