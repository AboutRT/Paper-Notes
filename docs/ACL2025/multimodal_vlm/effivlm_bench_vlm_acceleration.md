---
description: >-
  [ACL 2025][多模态] 提出 EffiVLM-Bench，一个统一评估框架，系统性地评估大型视觉语言模型(LVLM)的免训练加速方法，涵盖 token 压缩和参数压缩两大类，从性能、泛化性、忠实度和效率四个维度进行全面对比分析。
tags:
  - ACL 2025
  - 多模态
  - VLM加速
...
**本文要解决什么？** → 为 LVLM 免训练加速领域提供一个统一、全面、可扩展的评估基准。

**切入角度**：定义四个正交评估维度（性能/泛化/忠实度/效率），覆盖最新模型架构和多样任务场景，系统对比主流加速方法。

**核心 idea 一句话**：用四维度统一框架终结 LVLM 加速方法各自为政的评估乱象，通过 Pareto 前沿分析为实际部署提供选型依据。
