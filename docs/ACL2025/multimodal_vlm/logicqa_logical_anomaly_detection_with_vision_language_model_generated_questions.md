...
1. **重建式方法**（AutoEncoder 等）：需要大量正常图像训练，在少样本场景下不适用
2. **记忆库方法**（PatchCore 等）：依赖预训练视觉模型的特征库，需要昂贵的微调且不提供解释
3. **LogicAD**（Jin et al., 2025）：使用 VLM 但依赖类别特定的引导链式思维提示，需要为每类异常精心设计 prompt
4. **LogiCode**（Zhang et al., 2024）：用 LLM 生成 Python 逻辑约束，但依赖详细的人工标注
