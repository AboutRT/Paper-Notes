...
description: >-
  [ACL 2025][代码智能] 提出 CodeDPO，通过 PageRank 启发的自验证评分机制从自生成代码中构造高质量偏好对（93K 正确性 + 21K 效率），DPO 训练后在 8 个代码模型上 HumanEval 平均提升 10+ 分，同时提升代码执行效率 1.25-1.45×。
tags:
  - ACL 2025
  - 代码智能
  - DPO
...
**代码**: [https://anonymous.4open.science/r/CodeDPO/](https://anonymous.4open.science/r/CodeDPO/)  
**领域**: LLM对齐 / 代码生成  
...
**本文目标**：如何从自生成代码和测试中构造高质量偏好对，同时优化正确性和效率？

**切入角度**：借鉴 PageRank 的互参照思想——通过更多测试的代码更可信，被更多代码通过的测试更可靠。

**核心 idea**：用 PageRank 式迭代自验证评分取代简单的通过/未通过判定，构造高质量偏好对做 DPO。
...
1. **PageRank 式自验证评分 (Self-Validation Score)**:
    ...
    - 功能：对每道题的 15 个代码和 15 组测试用例，构造二部图——代码节点和测试节点之间的边表示"通过"关系
...
3. **RPO 训练策略**:
    ...
    - 功能：使用 RPO 格式 loss = 加权 SFT loss + 原始 DPO loss
