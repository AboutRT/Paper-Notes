...
1. **依赖 Ground Truth 终止**：CodeAct 假设 GT 已知并用于匹配终止条件，在真实在线场景中不可行
2. **碎片化思考低效**：逐步生成代码块导致逻辑链断裂，随着轮次增加，重复整合历史上下文引发上下文过载和幻觉累积
3. **缺乏多路径探索**：仅沿单一推理路径交互，无法探索多种解题策略
4. **轨迹数据难以复用**：多轮交互产生的轨迹无法直接组合为完整程序用于 SFT，且因缺乏过程监督而不适合强化学习
