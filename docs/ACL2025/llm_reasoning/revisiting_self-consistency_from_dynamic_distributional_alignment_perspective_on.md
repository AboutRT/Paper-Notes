...
description: >-
  [ACL 2025][LLM推理][self-consistency] 将 Self-Consistency 重新理解为采样分布与真实答案分布的动态对齐问题，揭示温度不仅控制采样随机性还直接塑造真实答案分布，据此提出置信度驱动的三阶段动态温度调节机制（FSD 阈值理论推导）…
tags:
  - "ACL 2025"
  - "LLM推理"
  - "self-consistency"
...
**本文目标**：(1) 温度如何影响 SC 的收敛行为和最终精度？(2) 能否根据实时置信度动态调整温度以加速收敛并探索更优分布？

**切入角度**：从分布对齐的视角出发——SC 的本质是让采样分布的 top-1 答案对齐到真实分布的 top-1 答案，温度参数决定了这个对齐过程的效率和终点。

**核心 idea**：用实时置信度（FSD）驱动温度动态调节，低置信时降温收敛、高置信时升温探索，实现采样分布与真实分布的动态同步。
...
1. **分布对齐理论框架**:
    ...
    - 功能：从概率论角度重新理解 SC 为什么有效以及温度如何影响它
...
2. **置信度驱动的动态温度调节机制**:
    ...
    - 功能：根据当前已采样结果的 First-Second Distance (FSD) 自适应调节温度
