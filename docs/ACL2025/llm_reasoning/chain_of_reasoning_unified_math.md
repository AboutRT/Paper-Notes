---
title: >-
  [论文解读] Chain-of-Reasoning: Towards Unified Mathematical Reasoning in Large Language Models via a Multi-Paradigm Perspective
description: >-
  [ACL 2025][LLM推理][数学推理] 提出 Chain-of-Reasoning（CoR）框架，将自然语言推理（NLR）、算法推理（AR）和符号推理（SR）三种范式统一在一个推理链中…
tags:
  - "ACL 2025"
  - "LLM推理"
  - "数学推理"
...
**本文目标** 设计一个统一框架，让 LLM 在一次推理中依次使用 NLR、AR、SR 三种范式，各取所长，实现跨任务的综合数学能力。

**切入角度**：类比软件开发中模块化思想——每种范式是一个模块，CoR 让它们以链式组合方式协作，前一范式的输出为后一范式提供参考。

**核心 idea**：用三种推理范式的链式组合替代单一范式，以渐进训练策略让模型逐步掌握NLR→AR→SR，实现数学任务的统一求解。
...
1. **多范式推理数据集 MPM（Multi-Paradigm Mathematical）**:
    ...
    - 功能：为每个数学问题构建包含三种范式推理路径的训练数据 $<x, NLR, SR, AR, y>$
...
3. **顺序多范式采样 SMPS（Sequential Multi-Paradigm Sampling）**:
    ...
    - 功能：推理时在范式级别（而非 token 级别）进行采样，组合多条推理路径
...
- [\[ACL 2025\] Chain-of-Reasoning: Towards Unified Mathematical Reasoning in Large Language Models](chain_of_reasoning_unified_math.md)
