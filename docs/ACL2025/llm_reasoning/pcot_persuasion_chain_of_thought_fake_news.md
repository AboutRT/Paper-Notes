...
description: >-
  [ACL 2025][LLM推理][假新闻检测] 提出 PCoT（说服增强的思维链）方法，通过两阶段推理——先让 LLM 分析文本中的说服策略，再结合说服分析结果判断是否为虚假信息——在零样本设置下，5 个 LLM 和 5 个数据集上平均提升 15% 的检测 F1。
tags:
  - ACL 2025
  - LLM推理
  - 假新闻检测
...
**代码**: [https://github.com/ArkadiusDS/PCoT](https://github.com/ArkadiusDS/PCoT)  
**领域**: LLM推理  
...
**领域现状**：虚假信息检测是媒体素养的关键任务。传统方法依赖监督学习（如 BERT 微调），但面临标注数据稀缺和泛化困难。LLM 的零样本检测开始展现竞争力。

**现有痛点**：(a) 零样本 LLM 检测直接让模型判断真假，缺乏推理过程，性能不够稳定；(b) 监督方法在跨数据集/跨领域场景下泛化差；(c) 虚假信息与说服/操纵手法密切相关，但现有检测方法不利用这一关键线索。

**核心矛盾**：心理学研究已证明，了解说服谬误的人能更好地识别虚假信息。能否将这种"说服知识"注入 LLM 以提升检测能力？

**本文目标** 设计一种零样本方法，利用说服策略知识增强 LLM 的虚假信息检测推理过程。

**切入角度**：受心理学启发——人类通过识别说服手法来辨别虚假信息。将这种认知过程建模为两阶段 CoT：先检测说服策略，再基于说服分析判断真假。

**核心 idea**：用说服策略分析作为 CoT 的中间推理步骤，为 LLM 的虚假信息检测提供可解释的推理桥梁。
...
1. **说服检测阶段（Persuasion Detection Step）**:
    ...
    - 功能：让 LLM 分析文本中是否存在 6 种说服策略，并为每种生成解释
...
2. **虚假信息检测阶段（Disinformation Detection Step）**:
    ...
    - 功能：结合原始文本和第一阶段的说服分析结果，让 LLM 做最终的真假判断
