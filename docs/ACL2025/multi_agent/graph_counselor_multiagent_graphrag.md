...
description: >-
  [ACL 2025][LLM/NLP][图检索增强生成] Graph Counselor 提出了一个多智能体协作的 GraphRAG 推理框架，通过 Planning/Thought/Execution 三个 Agent 自适应提取图结构信息，并引入多视角自反思机制纠正推理偏差，在多个图推理任务上超越现有方法。
tags:
  - ACL 2025
  - LLM/NLP
  - 图检索增强生成
...
**本文目标**：设计灵活的图信息提取和自纠正推理框架

**切入角度**：将图推理分解为规划→思考→执行三步，让不同 Agent 各司其职

**核心 idea**：三个专用 Agent 自适应提取图信息 + 多视角自反思纠正推理偏差
...
2. **Self-Reflection with Multiple Perspectives (SR)**:
    ...
    - 功能：检查推理结果的逻辑一致性，纠正语义和图结构之间的对齐偏差
