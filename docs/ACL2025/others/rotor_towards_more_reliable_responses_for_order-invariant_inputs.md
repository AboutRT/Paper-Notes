...
现有零样本顺序不变方法存在两个关键局限：
1. **训练-推理分布不匹配**：PCW 完全隔离段间注意力，PINE 为每个 query token 动态重新分配位置 ID，导致位置编码分配与预训练时的分布差距太大
2. **无法适应混合输入**：实际问题（如 MMLU）既包含顺序无关的选项（A、B、C），也包含顺序敏感的选项（如"None of the above"），现有方法用单一策略处理两者
...
1. **全局排序（Global Ordering）**
   - 做什么：为所有输入段确定一个统一的排列顺序，而非 PINE 的逐 query 排列
   - 核心思路：
     - 提供三种全局排序算法：
...
2. **循环排列（Circular Arrangement）**
   - 做什么：在因果 LM 中模拟双向注意力，让每个段都能"看到"其他所有段
   - 核心思路：
