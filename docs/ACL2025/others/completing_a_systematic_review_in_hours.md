---
title: >-
  [论文解读] Completing A Systematic Review in Hours instead of Months with Interactive AI Agents
description: >-
  [ACL 2025][systematic review] 提出 InsightAgent，一个以人为中心的交互式多 Agent 系统，通过语义聚类分区、多 agent 并行阅读和实时用户交互，将医学系统综述的撰写时间从数月缩短到约 1.5 小时，达到人类撰写质量的 79.7%。
tags:
  - "ACL 2025"
  - "systematic review"
  - "multi-agent"
...
**本文目标** 设计一个人-AI 协作的系统综述框架，让临床专家能实时监控和引导 AI agent 的文献阅读和综合过程。

**切入角度**：借鉴系统综述方法学中"多审稿人策略"的理念，将文献库语义分区后派多个 agent 并行处理，并提供直觉可视化界面让用户实时干预。

**核心 idea**：通过语义聚类分区 + 多 agent 并行阅读 + 可视化人机交互，实现高效高质的系统综述自动化。
...
2. **多 Agent 并行阅读与证据综合 (Stage 2)**:
    ...
    - 功能：每个 agent 被分配一个文献簇，从最相关文章开始向外探索，筛选相关文献并增量式生成综合摘要。
