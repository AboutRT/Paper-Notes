...
1. **缺乏系统性回顾**：过去几年KG研究分散在多条技术路线上（Seq2Seq架构、预训练模型、生成策略等），但至今没有人对这些工作进行系统梳理和对比分析，导致领域现状不清晰。

2. **基准数据集高度相似**：KG领域常用的几个benchmark（如KP20k、Inspec、Krapivin、NUS、SemEval等）之间存在令人担忧的高度相似性。这些数据集大多来源于学术论文，领域分布和文本风格高度重叠，导致模型可能只是在重复学习而非真正泛化。

3. **评估指标计算不一致**：不同论文在计算F1@K、F1@M等指标时存在显著不一致——词干化（stemming）处理方式不同、截断策略不同、present/absent keyphrase的划分标准不同，这些差异导致跨论文的性能对比不可靠，部分方法的性能被高估。

4. **预训练模型可获取性差**：尽管近年来基于PLM的方法表现优异，但大多数论文未开放预训练模型，阻碍了后续研究的进展和公平对比。
...
2. **评估指标不一致性分析**:
   - 做什么：梳理不同论文中F1@K、F1@M等指标的计算细节差异
   - 核心思路：对比分析以下方面的实现差异：
...
3. **PLM-based强基线模型**:
   - 做什么：训练并发布一个基于预训练语言模型的KG模型作为可复现的强基线
