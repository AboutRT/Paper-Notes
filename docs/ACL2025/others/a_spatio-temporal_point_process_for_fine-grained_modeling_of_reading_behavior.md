...
**本文要解决什么？** (1) 能否直接建模原始扫视路径的时空动态，避免聚合带来的信息损失？(2) surprisal 等预测因子在细粒度建模中是否仍然有效？

**切入角度**：作者观察到阅读过程本质上是一个时间和空间上交替的事件序列——注视和扫视不断交替。这天然适合用时空点过程来建模。每一次新注视的发生概率受到之前所有注视的激励（self-excitation），这正是 Hawkes 过程的核心特征。

**核心 idea 一句话**：用时空 Hawkes 过程联合建模注视的时间、位置和持续时间，从而在不聚合数据的情况下直接分析 surprisal 等因子对细粒度眼动的影响。
...
3. **卷积式溢出效应的持续时间模型**:
    ...
    - 做什么：建模注视持续时间，同时捕捉跨注视的认知加工溢出（spillover）效应
...
**本文要解决什么？** (1) 如何在不损失信息的前提下，统一建模注视的时间、空间和持续时间三个维度？(2) 对原始 scanpath 进行建模时，surprisal 等预测因子的效应大小是否与聚合数据一致？

**切入角度**：作者将阅读过程形式化为时空标记点过程——注视和扫视的交替序列天然适合点过程框架。利用 Hawkes 过程的自激性质可以捕捉先前注视对后续注视在时空上的影响。

**核心idea一句话**：用 Hawkes 过程建模扫视的时空分布、对数正态分布建模注视持续时间，构成统一的标记时空点过程，直接对原始 scanpath 建模。
