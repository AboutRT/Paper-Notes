...
1. **只能排序，无法量化**：现有方法（如知识神经元发现方法）主要是按重要性对神经元排名，但无法量化神经元激活值变化与模型输出变化之间的精确关系。这限制了知识编辑等应用场景——如果你不知道修改一个神经元会对输出产生多大影响，就无法精确控制模型行为。

2. **计算成本高昂**：现有方法需要反复修改激活值进行推理、或进行大量张量运算，导致无法对所有神经元进行大规模分析，尤其是在 Llama2-70B 这种大模型上。
