---
description: "【论文笔记】Memorization: A Close Look at Books 论文解读 | ACL 2025 | arXiv 2504.12549 | LLM 记忆化 | 系统研究 Llama 3 系列模型对完整书籍的记忆化程度，发现书籍提取率与其流行度（训练数据重复度代理）高度正相关，并通过 LoRA 微调揭示指令微调的抗反刍缓解措施仅涉及极少量集中在底层 transformer block 的权重变化。"
tags:
  - ACL 2025
  - LLM 记忆化
  - 训练数据提取
...
1. **能否提取完整作品？** 现有研究主要关注短文本片段（如电话号码、邮箱）的提取，但对版权诉讼更重要的是：LLM 是否记住了整本书？
2. **什么决定了记忆化程度？** 训练数据的重复度与记忆化之间的定量关系在完整作品层面尚未被研究。
3. **对齐能否真正消除记忆化？** Nasr et al. (2025) 已证明可通过微调"撤销"对齐模型的反反刍措施，但其背后的权重变化模式仍不清楚。
