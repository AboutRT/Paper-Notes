...
description: >-
  [ACL 2025][计算语言学] 在12种语言上训练GPT-2，测试LM区分可能语言与不可能语言的能力，发现LM展现出部分类人学习偏好但不完全一致——能区分单语言内的可能vs不可能变体，但无法跨语言完全分离；在名词短语词序实验中，困惑度无法区分但泛化测试能区分类型学上已证实vs未证实的词序。
tags:
  - ACL 2025
  - 计算语言学
  - 语言习得
...
**本文要解决什么**：(1) LM是否能跨语言区分可能vs不可能语言？(2) LM是否展现类型学偏好，即是否更容易学习类型学上常见的词序？
...
2. **不可能语言构造**:
    - 做什么：对每种自然语言生成多种"不可能"变体——确定性打乱(3种seed)、局部窗口打乱(w=2,3,5,10)、完全逆序、奇偶重排
...
3. **NP词序泛化测试 (ΔGenScore)**:
    - 做什么：提出ΔGenScore指标，测试在不同NP词序上训练的模型的泛化能力
