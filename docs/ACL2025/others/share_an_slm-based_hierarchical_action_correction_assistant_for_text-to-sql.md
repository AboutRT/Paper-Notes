...
1. **计算开销高**：传统自纠正依赖 LLM 的递归自调用（如 GPT-4o 多次调用），计算成本呈乘法级增长
2. **错误定位困难**：LLM 难以对声明式 SQL 进行有效的错误检测和修正——SQL 是声明式的，隐藏了底层推理路径。模型无法"看到"自己生成 SQL 时的推理过程，因此难以精确定位错误
...
1. **Base Action Model (BAM)**：将 SQL 转换为动作轨迹
   ...
   - 做什么：接收初始 SQL，输出类似 pandas API 的步骤化操作序列
...
2. **Schema Augmentation Model (SAM)**：修正 Schema 链接错误
   ...
   - 做什么：对动作轨迹中的表名、列名等 Schema 元素进行纠正
...
3. **Logic Optimization Model (LOM)**：修正推理逻辑错误
   ...
   - 做什么：在 Schema 正确的前提下，修正动作轨迹中的逻辑推理错误
