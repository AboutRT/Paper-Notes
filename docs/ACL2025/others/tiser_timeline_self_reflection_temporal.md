...
**本文目标** 如何让 LLM 在推理时显式构建时间线，并通过自反思来检测和修正时间推理中的不一致？

**切入角度**：受 test-time scaling 启发，通过延长推理轨迹来捕获复杂时间依赖——但不是简单地更长，而是结构化为推理→时间线→反思三个阶段。

**核心 idea**：让LLM在推理时显式构建事件时间线作为"脚手架"，然后将推理结果对照时间线进行自反思修正，从而大幅提升时间推理准确率。
