...
description: >-
  [ACL 2025 (Findings)][文本检测] 基于信息论中的通用压缩原理，提出 MOSAIC——多 LLM 集成的 AI 生成文本检测方法，通过 Blahut-Arimoto 算法为多个 detector LLM 计算最优组合权重，构建混合分布作为观察者…
tags:
  - "ACL 2025 (Findings)"
  - "文本检测"
  - "ensemble LLM"
...
1. **最优混合分布 (Blahut-Arimoto)**
    ...
    - 功能：为 $K$ 个 detector LLM 计算位置自适应的最优权重 $\mu^*(m|\mathbf{y}_{<t})$
...
2. **MOSAIC 评分**
    ...
    - 功能：每个 token 计算 surprisal 与期望交叉熵的差
