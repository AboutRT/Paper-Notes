...
长篇故事评估面临三大挑战：
1. **数据标注限制**：人工评估是金标准，但对超过10万token的故事进行标注在时间和认知上都不可行
2. **评价标准不一致**：现有工作使用各自预定义的标准进行评估，没有统一标准，且这些标准是否反映读者的真实偏好尚不清楚
3. **长文本处理挑战**：书籍级故事常常超过大多数LLM的128K token上下文限制，即使在限制内，处理如此长的上下文对模型仍然是挑战
