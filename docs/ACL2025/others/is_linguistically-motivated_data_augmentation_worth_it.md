...
1. **简单方法出人意料地有效**：随机扰动（删词、插入噪声等）——即使产生无意义或不合语法的句子——竟然也能让模型受益
2. **语言学方法成本高**：需要领域语言学专家，且实现更复杂
3. **缺乏系统比较**：此前没有系统实证研究对比这两类策略，开发者无法做出知情决策
4. **任务难度不同**：对于序列到序列任务（翻译、形态标注），标签也是不受限序列，确保标签有效性远比分类任务困难
