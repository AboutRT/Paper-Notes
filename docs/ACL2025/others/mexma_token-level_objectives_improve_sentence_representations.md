...
1. **预训练阶段使用 token 级目标**：CLSE 通常基于 XLM-RoBERTa、NLLB 等预训练编码器，这些模型通过掩码语言建模（MLM）等 token 级目标更新每个 token 的表示
2. **微调阶段仅使用句子级目标**：CLSE 训练时（如 LaBSE 的对比学习、SONAR 的翻译瓶颈）仅通过句子表示更新编码器，不再有 token 级目标
3. **后果**：这导致 token 级信息（尤其是词汇信息）退化，进而影响句子表示质量
