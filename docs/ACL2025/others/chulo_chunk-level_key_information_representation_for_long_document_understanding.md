---
description: "【论文笔记】ChuLo: Chunk-Level Key Information Representation for Long Document Understanding 论文解读 | ACL2025 | arXiv 2410.11119 | 长文档理解 | ChuLo 的核心不是单纯把长文档切小，而是先在全文范围内找出最关键的语义短语，再把这些关键信息重新注入每个 chunk 的表示里，从而在只用紧凑块表示的前提下，同时保住全局语义和细粒度 token 信息。"
tags:
  - ACL2025
  - 长文档理解
  - 分块表示
...
### 关键设计
1. **固定长度非重叠切块**
	- 做什么：把长度为 $l_D$ 的文档 $D=(t_0,t_1,\dots,t_{l_D-1})$ 切成若干个长度为 $n$ 的 chunk，chunk 数量为 $m=\lceil l_D / n \rceil$，最后一个不足长度的 chunk 用 `[PAD]` 补齐。
...
2. **Semantic Keyphrase Prioritization, SKP**
	- 做什么：从全文中抽取候选关键词短语，并对这些短语按“语义重要性”排序。
...
3. **关键词加权的 chunk 表示**
	- 做什么：把原文中属于 keyphrase 的 token 标记为 $T_k$，其他 token 标记为 $T_{nk}$，然后对一个 chunk 内所有 token embedding 做加权平均。
	- 公式：
	  $$
	  w_t=
	  \begin{cases}
...
4. **块级 Transformer 与任务头**
	- 做什么：将 chunk embedding 序列输入 chunk attention module，再做分类或序列标注。
