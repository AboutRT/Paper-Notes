...
description: >-
  [ACL 2025][不确定性表达] 定义"长文本不确定性生成"（LoGU）任务，识别不确定性抑制和不确定性错位两个子挑战，提出基于分解的数据构造框架和 SFT+DPO 两阶段训练流水线，使 LLM 在长文本生成中对不确定事实显式表达不确定性，在三个数据集上将 Llama3-8B 的事实准确率从 51.9% 提升到 71.6%，错误声明数从 20.4 降到 5.81。
tags:
  - ACL 2025
  - 不确定性表达
  - 其他
...
## 研究背景与动机
**领域现状**：LLM 在不确定时仍然会自信地生成错误信息（幻觉）。现有不确定性估计研究主要集中在短文本 QA（如给出一个置信度分数），不适用于长文本场景。

**现有痛点**：(a) 短文本不确定性方法给整个回答一个统一的不确定性评分，但长文本包含多个 claim，有的正确有的错误，需要细粒度处理；(b) LLM 存在"不确定性抑制"——倾向于给出自信回答而非承认不知道；(c) 现有方法的不确定性表达常常模糊笼统（如"我不确定他的早年生活"），而非针对具体事实。

**核心矛盾**：理想的 LLM 应在知道的事实上自信生成，在不知道的事实上明确表达不确定性——但这需要模型同时具备自知能力和细粒度表达能力。

**本文目标** 如何让 LLM 在长文本中为每个 atomic claim 做出正确的确定/不确定判断并自然表达。

**切入角度**：将长文本分解为原子声明，逐个事实核查，然后有选择地将不正确声明改写为不确定性表达，重组回连贯文本。

**核心 idea**：基于分解-核查-改写-重组流程构造训练数据，SFT 解决不确定性抑制，DPO 解决不确定性错位。
