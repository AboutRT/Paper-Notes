...
1. **现实中的表格数据绝大多数是关系型的**——由多张相互关联的表组成（如数据库中的主表和从表），但现有方法仅能生成单张独立表
2. **基于学习的方法**（如 Synthetic Data Vault、GNN+扩散模型）需要真实世界数据集作为基础提取统计和关系模式，而可访问的真实关系型数据集稀缺
3. **TabPFN 的 SCM 方法**虽然独立于真实数据、能自然建模因果依赖，但被限制在单表场景
