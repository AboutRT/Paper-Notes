...
核心挑战：
1. **几何组件对齐困难**：3D 对象由多个组件组成，每个对应程序中特定代码块（含子程序、控制流、布尔运算），模型需要将视觉组件与代码块关联
2. **隐藏内部组件**：3D 对象可能包含不可直接视觉检查的内部结构（如内六角孔），需要程序化分析
3. **几何操作映射**：精确纠正需要将几何操作（旋转、平移）映射到对应的代码修改
4. **MLLM 表现不佳**：GPT-4o 等先进模型在 CAD 审查上效果有限，主要因为无法有效对齐代码与视觉信息
