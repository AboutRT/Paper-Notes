...
description: >-
  [ACL 2025][代码生成] 构建 AceCode-87K（87K 编码题 + 138 万自动合成测试用例），训练代码专用 Reward Model（7B 超越 340B Nemotron），Best-of-N 提升 Llama-3.1-8B 平均 8.9 分…
tags:
  - "ACL 2025"
  - "代码生成"
  - "强化学习"
...
## 研究背景与动机
**领域现状**：代码生成模型主要依赖 SFT 提升，RL 的潜力尚未被充分挖掘。数学领域的 RL 成功（如 DeepSeek-R1）暗示代码领域也可以受益。

**现有痛点**：(1) 代码评估需要执行测试用例（不像数学可直接字符串匹配），缺乏可靠奖励信号；(2) 大规模带测试用例的编码数据集稀缺——APPS/TACO 依赖人工标注，之前 RL 工作限于 <5K 样本。

**核心矛盾**：现有通用 Reward Model（如 Skywork）不能泛化到代码评估，而人工标注测试用例成本太高无法规模化。

**本文目标**：构建自动化管线从代码数据集生成大规模可靠测试用例 → 训练代码专用 RM → 用 RM 做 Best-of-N 或 RL。

**切入角度**：用 GPT-4o-mini 批量"想象"测试用例 + 强代码模型执行过滤 = 低成本高质量大规模测试数据。

**核心 idea**：全自动测试用例合成 + 执行过滤 → 代码 RM 训练 → RL/Best-of-N，将代码生成的 RL 扩展到 87K 规模。
...
1. **自动测试用例合成 + 过滤 (AceCode-87K)**:
    ...
    - 功能：从 124K Python 函数（Magicoder-Evol/OSS/StackPy）出发，GPT-4o-mini 重写为 LeetCode 风格 + 生成 ~20 个测试用例
...
2. **偏好对构造与 RM 训练**:
    ...
    - 功能：对每道题采样 16 个程序，按测试通过率排序，构造偏好对
...
3. **RL 训练 (Reinforcement++)**:
    ...
    - 功能：用 AceCode-RM 或二值规则奖励（全部测试通过=1，否则=0）做 RL
