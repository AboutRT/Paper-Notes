---
description: "【论文笔记】Understanding Common Ground Misalignment in Goal-Oriented Dialog: A Case-Study with Ubuntu Chat Logs 论文解读 | ACL 2025 | arXiv 2503.12370 | Common Ground | 本文通过在 Ubuntu IRC 技术支持对话中标注\"对话摩擦\"（conversational friction），实证揭示了共识基础（common ground）的失配与任务成功率之间的显著关联，并发现 LLM 能识别显式的对话摩擦但难以处理需要语用或领域推理的隐式摩擦。"
tags:
  - ACL 2025
  - Common Ground
  - 对话摩擦
...
1. **可观察性问题**：当对话顺利进行时，很难知道参与者的 common ground 包含什么
2. **现有研究局限**：大多数研究通过限制对话场景（如 Minecraft 建造任务）来推断 common ground
3. **LLM 作为对话伙伴/中介的需求**：LLM 越来越多地被用于对话，但它们是否能追踪 common ground 尚不清楚
