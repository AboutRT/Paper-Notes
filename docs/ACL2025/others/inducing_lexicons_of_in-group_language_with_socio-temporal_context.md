...
1. **手工构建昂贵且容易过时**: 例如 Rowe & Saif (2016) 在研究 ISIS 语言时使用了 7 年前的词典
2. **计算方法忽视社会和时间维度**: Lucy & Bamman (2021) 用上下文词嵌入，Farrell et al. (2020) 用话题模型，但都只利用语言信息，忽视了群体的动态社会结构
...
**做什么**: 在每个时间步 $t$，联合分解用户-内容矩阵 $C_t$ 和用户-用户邻接矩阵 $A_t$，得到动态的用户嵌入 $U_t$ 和词嵌入 $W_t$。
