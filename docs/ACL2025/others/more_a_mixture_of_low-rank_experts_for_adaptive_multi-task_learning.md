...
1. **不同任务需要不同秩**：作者通过实验发现（Table 1），T5-base 在 MRPC 上最优秩为 r=1，而 CoLA 上最优秩为 r=8，RTE 上为 r=4。固定秩无法适配所有任务。
2. **搜索最优秩代价高**：为每个任务独立搜索 + 训练多个 LoRA 在计算和存储上都不经济。
3. **现有方案的不足**：
...
1. **低秩专家 (Low-Rank Experts)**：
   - 做什么：将 LoRA 中的每个秩 $r_i \in [1, r]$ 视为一个独立的"专家"
...
2. **自适应秩选择器 (Adaptive Rank Selector)**：
   - 做什么：根据任务嵌入 $\mathbf{e}_t$ 输出秩的概率分布 $\mathbf{p}_t = \text{softmax}(\mathbf{W}_g \mathbf{e}_t + \mathbf{b}_g)$
...
3. **任务嵌入与对比学习优化**：
   - 做什么：为每个任务分配可学习的嵌入向量 $\mathbf{e}_t$
...
4. **平衡数据采样 (Balanced Data Sampling)**：
   - 做什么：为每个数据集分配采样权重 $\phi_t = \exp(|\mathcal{D}_t| / \sum_i |\mathcal{D}_i|)$
