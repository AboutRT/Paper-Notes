...
description: >-
  [ACL 2025][friction alignment] 提出摩擦对齐框架 FAAF（Frictional Agent Alignment Framework），通过双策略（frictive state policy + intervention policy）目标函数…
tags:
  - "ACL 2025"
  - "friction alignment"
...
3. **从双策略到单策略的推导**:
    ...
    - **功能**: 通过 Lagrangian 推导将双策略的闭式解合并为单一参数化策略的 ℓ₂ 损失
