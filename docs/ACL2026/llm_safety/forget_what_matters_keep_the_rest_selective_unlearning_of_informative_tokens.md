...
ETW 在标准的遗忘训练流程中引入 token 级权重：对遗忘集使用加权梯度上升损失，对保留集使用标准交叉熵损失。权重由模型预测分布的熵值决定——高熵 token 获得更大的遗忘惩罚。

### 关键设计

1. **熵引导的 token 权重计算**:

    - 功能：为每个 token 计算信息量权重，指导遗忘强度
    - 核心思路：计算每个位置 $i$ 的预测分布熵 $H(y_i | y_{<i}, \mathbf{x}; \hat{\boldsymbol{\theta}})$，然后归一化使权重之和等于序列长度 $n$：$\omega_i^{\text{ETW}} = n \cdot H_i / \sum_j H_j$
    - 设计动机：相比仅关注 ground-truth 置信度，熵值反映了概率质量在所有候选 token 上的分布情况。即使两个 token 的 GT 置信度相同，其熵值差异仍可达到 $H_{\max} - H_{\min}$ 的范围

2. **Stop-gradient 权重计算**:

    - 功能：确保权重计算不影响梯度更新
    - 核心思路：使用 stop-gradient 副本 $\hat{\boldsymbol{\theta}}$ 计算熵值，通过温度 $T$ 的 softmax 得到概率分布
    - 设计动机：解耦权重计算和遗忘训练过程，避免权重计算引入额外梯度干扰

3. **归一化保持损失尺度不变**:

    - 功能：在 token 间重新分配遗忘强度而不改变总量
    - 核心思路：权重归一化后 $\sum_i \omega_i = n$，保持与无加权 GA 相同的整体遗忘损失尺度
    - 设计动机：避免额外引入超参数来控制整体遗忘强度，使 ETW 可以直接替换现有方法中的均匀权重
