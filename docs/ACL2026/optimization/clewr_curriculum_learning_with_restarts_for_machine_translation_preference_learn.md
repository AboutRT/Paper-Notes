...
1. **基于多指标的难度评分**:
    ...
    - 功能：为每个偏好三元组 $(x, y_w, y_l)$ 计算难度分数
    - 核心思路：计算选中翻译 $y_w$ 和拒绝翻译 $y_l$ 之间的 BLEU、COMET-22、METEOR 三个指标，归一化后取平均作为相似度分数 $s$。相似度高的样本（选中和拒绝翻译差异小）为困难样本，相似度低的为简单样本
    - 设计动机：单一指标可能有偏差，综合三个互补的翻译评估指标提供更鲁棒的难度估计

2. **Epoch 级重启机制**:

    - 功能：在每个 epoch 重启易到难的课程排序
    - 核心思路：数据排序在训练前一次性完成，训练时每个 epoch 按相同的易到难顺序遍历所有数据（不打乱），确保每个 epoch 模型都能从简单样本开始复习
    - 设计动机：传统课程学习只遍历一次容易导致灾难性遗忘，通过每个 epoch 重启课程，模型在每轮都能重新巩固简单样本的知识

3. **CLewR-z 变体（基于 ARPO 距离的排序）**:

    - 功能：利用 ARPO 的自适应距离函数 $z_\theta$ 替代外部指标进行排序
    - 核心思路：ARPO 中的 $z_\theta(y_w, y_l)$ 编码了选中和拒绝响应之间的对数似然差异，用 $s = -z_\theta$ 作为课程分数。同时提出增强版 ARPO，将 $z_\theta$ 与 BLEU、COMET 距离加权组合
    - 设计动机：利用模型内部的距离信号（而非外部指标）进行排序，使课程排序与优化目标更一致
