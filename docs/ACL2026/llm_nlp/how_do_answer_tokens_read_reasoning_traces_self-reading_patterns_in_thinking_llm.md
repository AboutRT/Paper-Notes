...
description: >-
  [ACL 2026][LLM/NLP][推理模型] 本文发现推理 LLM（如 DeepSeek-R1）在定量推理中存在"良性自读"模式——答案 token 对推理痕迹的注意力呈现前移漂移（沿推理链逐步推进）和语义锚点集中（反复回顾关键步骤），且此模式与正确性强相关；基于此提出 SRQ（自读质量）驱动的免训练激活引导方法，在多个基准上提升准确率最高 2.6%。
tags:
  - ACL 2026
  - LLM/NLP
  - 推理模型
...
三阶段：(1) 自读分析——在 GSM8K 上分析三个推理 LLM 的答案-推理注意力模式，识别良性自读特征；(2) SRQ 得分设计——从几何维度（控制策略的过程结构）和语义维度（监控质量的内容锚定）量化自读质量；(3) 激活引导——用不同 SRQ 得分的样本构建引导向量，推理时注入以促进良性自读。

### 关键设计

1. **良性自读模式识别**:

    - 功能：发现答案 token 阅读推理痕迹的结构化模式
    - 核心思路：计算每个答案 token 对推理 token 的注意力分布的加权平均位置（质心），归一化到 [0,1]。正确样本中，质心轨迹呈清晰的对角线模式——随答案生成推进，阅读焦点沿推理链前移。同时，注意力反复集中于"语义锚点"（问题约束、解题计划、反思、最终结论）。错误样本中，注意力分散且不规则
    - 设计动机：推理 token 实现"对象层"计算，答案 token 实现"元层"操作——控制（推进阅读焦点）和监控（回顾证据）。这与经典认知理论（Nelson 1990, Koriat 1997）的元认知框架一致

2. **SRQ（自读质量）得分**:

    - 功能：量化自读模式的质量，用于样本选择和引导向量构建
    - 核心思路：几何维度——度量注意力质心轨迹的前移程度和平滑度（是否沿对角线推进）。语义维度——度量注意力是否集中于关键语义步骤（约束、计划、结论）。两个维度组合为 SRQ 得分
    - 设计动机：仅用几何维度可能选到"平滑但语义无意义"的样本，仅用语义维度可能选到"锚定正确但过程混乱"的样本。两者互补确保选到真正高质量的自读样本

3. **SRQ 驱动的激活引导**:

    - 功能：免训练地推动模型在推理时采用更好的自读模式
    - 核心思路：选择高 SRQ 和低 SRQ 的样本，提取中间层激活差异构建引导向量。推理时将引导向量添加到隐藏状态中，推动模型远离混乱阅读、趋向有序阅读。不修改模型参数，不需要额外训练
    - 设计动机：良性自读与正确性关联（人工验证 159/171 正确样本展示良性自读），因此推动模型采用良性自读模式应能提升准确率

### 损失函数 / 训练策略

完全免训练。引导向量从对比样本的激活差异中提取，推理时添加到目标层的隐藏状态中。
