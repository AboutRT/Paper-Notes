...
1. **毒性宪法与自举批评-修正**:
    ...
    - 功能：自动化生成结构化、多维度的毒性数据
    - 核心思路：定义四维度毒性宪法（法律伦理、社会偏见、行为后果、信任欺骗），每个维度有明确的行为目标。基础策略 $\pi_\theta$ 同时担任批评者和修正者，对每个提示进行 $K=4$ 轮迭代：先批评当前回复在毒性强度/结构完整性/类别对齐上的不足，再据此修正。多轮迭代鼓励组合性和推理驱动的有害行为，而非表面关键词级毒性
    - 设计动机：单步重写只能增强表面毒性，多轮批评-修正能逐步构建复杂、逻辑严密的有害叙事，更好地暴露模型的潜在危险能力

2. **概率截断（Probability Clamping）**:

    - 功能：在奖励模型训练中稳定对抗优化，防止语义崩溃
    - 核心思路：标准 Bradley-Terry 偏好概率 $P = \sigma(r_\phi(R_c) - r_\phi(R_r))$ 在对抗设置下容易饱和到0或1（因为模型对毒性关键词给予极端分数），导致梯度消失、奖励模型过度自信。概率截断将 $P$ 限制在 $[\epsilon_{\min}, \epsilon_{\max}]$ 区间：$P_{\text{clamped}} = \text{clamp}(P, 0.4, 0.6)$。这确保优化停留在非饱和区域，防止策略漂移到不连贯的高奖励局部最优
    - 设计动机：对抗奖励景观尖锐且非光滑，直接优化导致奖励黑客——模型生成重复的毒性关键词但忽略上下文逻辑。截断相当于一个"逻辑锚"，平坦化奖励景观

3. **毒性偏好建模**:

    - 功能：训练毒性感知的奖励模型为策略优化提供信号
    - 核心思路：利用多轮批评-修正产生的中间回复，由更强的参考模型（Llama3-70B）评分并排序，构建偏好对 $\langle R_c, R_r \rangle$。独立的奖励模型 $r_\phi$ 在这些偏好对上训练，结合概率截断后为 PPO 策略优化提供稳定信号
    - 设计动机：将多轮迭代的中间产物充分利用为偏好数据，而非仅使用最终输出
