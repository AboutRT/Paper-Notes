...
description: >-
  [ACL 2026][标签投影] 提出 LabelPigeon，一种基于 XML 标签的联合翻译与标签投影方法，通过在高质量 XML 标记平行语料上微调 NLLB-200 翻译模型，在 11 种语言上超越所有基线并主动提升翻译质量，在下游跨语言 NER 任务中实现最高 +40.2 F1 的提升。
tags:
  - ACL 2026
  - 标签投影
  - XML标记
...
LabelPigeon 的流程非常简洁：(1) 在待翻译的标注文本中用字母序 XML 标签（`<a>`, `<b>` 等）标记所有 span；(2) 用微调后的 NLLB-200 3.3B 模型进行翻译；(3) 用标准 XML 解析器从译文中提取标签。整个推理只需一次模型前向传播，无额外计算开销。

### 关键设计

1. **XML 标签替代方括号**:

    - 功能：提供源语言和目标语言标注 span 之间的精确对应关系
    - 核心思路：XML 标签天然支持命名属性（如 `<a>...</a>` 在源文和译文中一一对应），可以处理嵌套和重叠 span（如 `<a><b>...</b>...</a>`），还可承载语义信息（如 `<PER>` 表示人名）。方括号无法提供对应关系——EasyProject 需要额外的模糊字符串匹配来建立映射，不仅耗时且在嵌套时容易出错
    - 设计动机：结构化文档翻译领域已有丰富的 XML 标记翻译实践和数据，选择 XML 可以直接利用这些资源

2. **高质量真实数据微调**:

    - 功能：通过真实的 XML 标记平行语料训练模型学会在翻译时保持标签结构
    - 核心思路：使用 Salesforce Localization XML MT 数据集，包含英语与七种语言的约 10 万对 XML 标记平行句子。将原始 UI/样式标签替换为通用字母序标签（`<a>`, `<b>` 等），去除无标签样本后每语言对约 2.5 万样本。经消融实验选择在英-德、英-俄、英-中三个高资源语言对上训练，总计约 15 万训练样本（含双向翻译），在单张 A100 上 5.5 小时完成一个 epoch
    - 设计动机：先前方法（EasyProject）使用合成生成数据，而本文利用已有的高质量真实数据避免了合成数据的噪声和分布偏移

3. **标签感知翻译的优势论证**:

    - 功能：理论和实证证明联合翻译+标签投影优于分离方法
    - 核心思路：通过三个最小示例说明分离翻译的问题：(a) 翻译可能将标签 span 拆分到句子不同位置（如马拉雅拉姆语）；(b) 目标语言可能省略标签对应的词（如日语的代词省略）；(c) 翻译可能产生标签归属歧义（如法语）。标签感知翻译能引导模型选择保持 span 连续且完整的译法
    - 设计动机：分离方法假设翻译后可以可靠地重建标签映射，但翻译过程中的语言学变化使这个假设经常失效
...
- [\[ACL 2025\] Rethinking Evaluation Metrics for Grammatical Error Correction: Why Use a Different Evaluation Process than Human?](../../ACL2025/multilingual_mt/rethinking_evaluation_metrics_for_grammatical_error_correction_why_use_a_differe.md)
