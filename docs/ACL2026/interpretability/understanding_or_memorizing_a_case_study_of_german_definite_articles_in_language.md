...
选取德语定冠词系统（3 性 × 4 格 = 12 个格位，6 种冠词形式）作为受控实验系统。对每对性-格格位之间的冠词转换（如阳性主格↔阴性主格），使用 Gradiend 学习一维特征方向。然后从三个角度分析：编码器值分布、干预后冠词概率变化、以及参数空间中更新方向的重叠。

### 关键设计

1. **Gradiend 梯度特征学习**:

    - 功能：为特定冠词转换学习一个压缩的一维特征方向
    - 核心思路：给定两个性-格格位 $z_1, z_2$ 之间的冠词转换（如阳性主格 der ↔ 阴性主格 die），收集两个方向的梯度（事实目标梯度 $\nabla^F$ 和替代目标梯度 $\nabla^A$），训练编码器将梯度差 $\nabla^\Delta = \nabla^F - \nabla^A$ 压缩为标量 $h \in [-1, 1]$（+1 对应一个方向，-1 对应另一个方向），解码器再从标量重构梯度。所有非目标格位的训练数据作为恒等对（factual = alternative），强制 $h \approx 0$。
    - 设计动机：一维瓶颈确保学到的是最主要的更新方向，恒等对约束确保学到的方向特定于目标转换而非全局扰动。

2. **三种分析视角**:

    - 功能：从互补角度判断规则编码 vs 记忆
    - 核心思路：(a) 编码器分析——检查非目标格位的梯度是否也被编码到非零值（规则预测中应为零）；(b) 概率干预——将学到的方向施加到模型参数后，检查冠词概率变化是否仅限于目标格位（LR）、系统性扩展到相同性/格维度（GR）、还是溢出到共享表层冠词的不相关格位（SO）；(c) Top-k 参数重叠——比较不同转换的最重要参数是否大量重叠。
    - 设计动机：三种分析分别从表示空间、功能行为和参数空间提供互补证据，任何单一视角都可能有混淆因素。

3. **学习率选择与语言建模保持**:

    - 功能：确保干预效果不是由模型退化导致的伪信号
    - 核心思路：施加干预时扫描多个学习率 $\alpha$，仅保留在中性数据集上保持 99% 以上语言建模得分的候选，选择在目标数据集上最大化目标冠词概率的 $\alpha^*$。同时报告 SuperGLEBer 基准分数确认模型整体能力未受损。
    - 设计动机：大幅参数更新可能通过破坏语言建模能力来改变预测，而非反映真正的语法机制。

### 损失函数 / 训练策略

Gradiend 使用 MSE 重构损失 $\|\text{dec}(\text{enc}(\nabla^A W_m)) - \nabla^\Delta W_m\|_2^2$ 训练，将替代目标梯度作为输入（因为事实目标梯度通常接近零，不够信息丰富）。
