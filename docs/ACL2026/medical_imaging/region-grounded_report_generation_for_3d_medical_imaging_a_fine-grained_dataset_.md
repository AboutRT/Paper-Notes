...
1. **双流编码器与交叉注意力融合**:
    ...
    - 功能：分别提取 CT 的解剖结构信息和 PET 的代谢活动信息，然后通过交叉注意力进行跨模态融合
    - 核心思路：两个独立的 CT-ViT 3D 编码器分别提取 $F_{CT}$ 和 $F_{PET} \in \mathbb{R}^{B \times N \times D}$，然后通过双向交叉注意力融合：$\tilde{F}_{CT} = F_{CT} + \text{CA}(F_{CT}, F_{PET})$，最终取平均并经多尺度金字塔网络得到融合表示
    - 设计动机：CT 捕捉组织边界和器官形态，PET 量化代谢活动（葡萄糖摄取模式），两者提供互补信息。分流设计避免了过早融合导致的模态特征淹没

2. **GATv2 图关系建模**:

    - 功能：捕捉 ROI 之间的空间和形态学依赖关系
    - 核心思路：构建以 ROI 为节点的图，边基于两个标准建立：空间邻近度（质心间几何距离 $d_{ij} < \tau_d$）和形态学相似度（特征余弦相似度 $s_{ij} > \tau_s$）。边特征编码空间关系（距离、相对方向、体积比）和形态学关系（相似度、CT/PET 平均强度），通过 GATv2 注意力机制进行消息传播
    - 设计动机：孤立地处理每个 ROI 会遗漏关键的临床关系——空间相邻的病灶可能暗示局部侵犯，远处具有相似代谢模式的病灶可能提示转移。图建模可以同时捕捉这两种类型的关系

3. **临床评估指标（RoI Coverage 和 RoI Quality Index）**:

    - 功能：超越传统 NLP 指标，从临床角度评估报告质量
    - 核心思路：RoI Coverage 通过匈牙利匹配计算预测 ROI 与真实 ROI 的配对，得到 Precision/Recall/F1。RoI Quality Index (RoIQ) 对匹配的 ROI 对进行属性级准确率评估，使用公式 $\text{RoIQ} = \sqrt{S_{\text{region}} \cdot S_{\text{lesion}}} \times \frac{1}{|\mathcal{A}|}\sum S_k$，通过几何均值对核心属性（解剖区域和病灶类型）施加非线性惩罚
    - 设计动机：传统的 BLEU/ROUGE 只衡量词汇重叠，无法评估报告的临床正确性。RoIQ 的层次化设计确保了核心属性错误（定位或病灶类型错误）不会被次要属性的高分补偿
