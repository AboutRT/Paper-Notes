...
description: >-
  [NeurIPS 2025][时间序列][实例级修正] PIR 提出实例感知的事后修正框架——通过不确定性估计识别预测失败实例，用局部修正（协变量+外生变量 Transformer）和全局修正（检索相似训练实例加权平均）的残差组合，作为即插即用模块使 SparseTSF MSE 降低 25.87%…
tags:
  - "NeurIPS 2025"
  - "时间序列"
  - "实例级修正"
...
**代码**: [https://github.com/icantnamemyself/PIR](https://github.com/icantnamemyself/PIR)  
**领域**: 时间序列预测  
...
**本文目标** 设计一个模型无关的后处理模块，自动识别预测失败实例并进行针对性修正。

**切入角度**：先估计预测不确定性，不确定性高的实例用两种互补方式修正——局部（利用当前时间步的协变量和外生变量）和全局（检索历史相似实例）。

**核心 idea**：不确定性估计识别失败实例 → 局部 Transformer 修正（协变量+外生变量）+ 全局检索修正（相似训练实例加权）→ 不确定性自适应融合为最终预测。
...
- [\[ICML 2025\] TIMING: Temporality-Aware Integrated Gradients for Time Series Explanation](../../ICML2025/time_series/timing_temporality-aware_integrated_gradients_for_time_series_explanation.md)
