...
1. **领域现状**: 自回归模型在时间序列和 NLP 领域广泛应用，性能依赖于捕获数据中的时间模式。但识别这些模式仍严重依赖人类专家经验。
...
3. **核心矛盾**: 好的预测需要两个条件——(1) 数据包含可利用的模式；(2) 模型能识别和利用这些模式。现有框架缺乏区分这两者的工具。

4. **本文要解决什么**: (i) 序列数据的最小可达风险是什么？(ii) 能否区分性能差是因为模型不好还是数据不可预测？

5. **切入角度**: 从**预测信息** $\mathbf{I}(X_{\text{past}}; X_{\text{future}})$——过去与未来之间的互信息入手，建立信息论学习曲线。

6. **核心idea一句话**: 用预测信息的离散导数（通用学习曲线 $\Lambda(k)$）作为"增加一步历史能减少多少不确定性"的度量，将其与自回归模型的最小可达风险联系起来。
