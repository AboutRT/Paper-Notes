...
**领域现状**: 电离层（约50–1500 km 高空）是近地空间关键组成部分，其变化直接影响 GNSS 导航精度、高频通信和航空运营。JPL 提供 15 分钟时间分辨率的全球电离层 TEC 地图（GIM）。

**现有痛点**: 传统经验模型如 IRI 和物理模型如 GITM 存在固有局限性，已被广泛记录；现有 ML 方法多基于 XGBoost、MLP 或受限于局部区域的 BiLSTM，无法处理全球尺度的多源异构数据。
