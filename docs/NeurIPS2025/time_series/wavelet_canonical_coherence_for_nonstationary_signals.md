...
1. **平稳性假设**：经典的典型相干分析（Brillinger 1981）基于弱平稳假设，即统计特性（期望、协方差、谱特征）需随时间恒定。但实际信号（如 EEG/LFP）几乎都是非平稳的。

2. **缺乏时间分辨率**：傅里叶域的典型相干分析忽略了时间动态，无法捕捉瞬态的频率特定交互。现有方法主要关注网络内的逐对（pairwise）相干性，缺少组间（between-groups）的全局相干性度量。

3. **无先验工作**：据作者所知，此前没有将经典典型相干扩展到小波域以度量两组多变量非平稳时间序列间时变典型相干的工作。
...
1. **局部小波谱矩阵（LWS Matrix）**
   ...
   **做什么**：在每个尺度 $j$ 和时间位置 $u$ 处，量化各通道间的局部频谱贡献。
...
2. **小波典型相干（WaveCanCoh）定义**
   ...
   **做什么**：在每个尺度 $j$ 和时间 $u$ 处定义两组信号间的最大线性依赖。
