...
1. **领域现状**：LLM通常在顺序文本上用因果（自回归）掩蔽训练。但许多数据天然具有空间结构（如棋盘、分子、图像），线性化到1D序列可能丢失空间关系。

2. **现有痛点**：(a) 不清楚因果掩蔽在空间数据上是否仍有效；(b) 线性化（如PGN棋步记录）丢失了即时的空间信息；(c) 双向注意力理论上更适合空间数据但训练复杂度高。

3. **核心矛盾**：因果掩蔽天然假设1D序列，但空间数据是2D/3D的——如何在保留因果掩蔽训练范式的同时利用空间结构？

4. **本文要解决什么**：比较空间编码（FEN棋盘状态）vs 序列编码（PGN棋步）在因果/双向掩蔽下的表现。

5. **切入角度**：用国际象棋作为controlled testbed——FEN编码保留空间棋盘结构，PGN编码是传统的线性化序列。ChessBench提供150亿Stockfish标注位置。

6. **核心idea一句话**：空间编码（FEN）+ 因果掩蔽优于序列编码（PGN）+ 因果掩蔽，说明保留空间结构比选择掩蔽策略更重要。
