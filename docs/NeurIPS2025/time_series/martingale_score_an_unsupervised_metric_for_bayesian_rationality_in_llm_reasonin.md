---
description: "【论文笔记】Martingale Score: An Unsupervised Metric for Bayesian Rationality in LLM Reasoning 论文解读 | NeurIPS 2025 | arXiv 2512.02914 | Martingale Score | 提出 Martingale Score 作为无监督度量指标，基于贝叶斯统计中的鞅性质(Martingale property)来量化 LLM 推理过程中的信念固化(belief entrenchment)现象，发现该现象普遍存在且与准确率下降显著相关。"
tags:
  - NeurIPS 2025
  - Martingale Score
  - 信念固化
...
1. **领域现状**: LLM 推理技术（CoT、强化推理等）快速发展，但推理过程是否真正"寻求真相"尚不明确。
2. **现有痛点**: 现有评估方法主要基于结果准确率（outcome-based），无法评估推理过程的质量；且在没有 ground truth 的开放领域（如价值判断、学术评审）中无法使用。
3. **核心矛盾**: LLM 推理可能产生"信念固化"——系统性地将信念更新偏向先验观点而非新证据，但在单一案例中难以区分合理更新与偏见。
4. **本文要解决什么**: 提出一个无需 ground truth、领域无关的推理过程质量度量。
5. **切入角度**: 利用贝叶斯统计中的鞅性质——理性信念更新的方向不应可从先验中预测。
6. **核心idea**: 如果模型的信念更新可以从先验信念可靠预测，则违反了鞅性质，表明存在信念固化。
