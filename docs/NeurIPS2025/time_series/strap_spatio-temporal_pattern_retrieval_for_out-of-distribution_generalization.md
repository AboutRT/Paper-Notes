...
- 时空图神经网络（STGNN）在交通预测、气候预测等领域取得成功，但在**流式数据**中面临严重泛化挑战
- **STOOD 问题**（时空分布外）包含三类偏移：空间结构变化（节点增删）、时间动态变化（趋势漂移）、时空联合变化
- 现有方法的局限：
    - Backbone 方法（直接应用/重训练）：灾难性遗忘
    - 架构方法（TrafficStream 等）：稳定性-可塑性权衡困难
    - 正则化方法（EWC）：过度约束适应性
    - 回放方法：无法区分相关和无关历史知识
- **根本问题**：如何识别历史知识中对当前预测最有价值的部分？
