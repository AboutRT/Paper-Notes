...
1. **领域现状**：金融预测需要同时考虑宏观经济、行业趋势、技术指标、市场情感等多个维度，单一模型难以全面覆盖。
2. **现有痛点**：(1) 传统时序模型缺乏推理能力；(2) 单一 LLM 在多维度分析中容易遗漏关键因素；(3) 金融数据的多源异构性使单一模型难以处理。
3. **核心矛盾**：全面性 vs 专业性——一个模型要么广而浅，要么窄而深。
4. **切入角度**：多 agent 分工——每个 agent 专注一个分析维度，最终由协调 agent 综合。
