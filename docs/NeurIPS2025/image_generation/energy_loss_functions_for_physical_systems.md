...
1. **MSE 对应非物理能量**：MSE 等价于以数据为中心的各向同性谐振势 $E = \|\hat{y} - y\|^2$——这描述的是外力将粒子拉回固定点，而非粒子间相互作用
2. **对称性破坏**：物理配置通常在刚体运动 SE(d) 下等价，但 MSE 会惩罚通过旋转/平移与目标等价的正确预测
3. **MSE 最小化器偏差**：当存在多个对称等价目标时，MSE 的最小化器是所有目标的均值——这不是任何合法物理配置
