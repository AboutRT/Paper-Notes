...
description: >-
  [NeurIPS 2025][图像生成][image deblurring] 提出 BlurDM，将运动模糊的物理形成过程（连续曝光导致渐进模糊累积）集成到扩散模型——双扩散前向（同时加噪声+模糊）+ 双去噪去模糊反向，作为隐空间先验生成器一致性增强 4 种去模糊方法在 4 个数据集上的效果…
tags:
  - "NeurIPS 2025"
  - "图像生成"
  - "image deblurring"
...
2. **双去噪去模糊反向过程**:
    ...
    - 功能：学习两个估计器——模糊残差估计器 $e^\theta$ 和噪声估计器 $\epsilon^\theta$，同时执行去模糊和去噪
