...
1. **LLM方法**（如LLaMA-2微调）：擅长处理离散的原子类型信息，组成有效性高（~93%），但由于有限精度编码的限制，在连续数据（坐标和晶格）上表现较弱，结构有效性偏低（~96%）。
2. **去噪模型**（如CDVAE、DiffCSP）：天然适合处理连续变量并保持几何等变性，结构有效性高（~100%），但在离散原子类型预测上不够准确，组成有效性偏低（~83-87%）。
