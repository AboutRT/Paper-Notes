...
1. **基于参数模板的方法**（PaMIR、ICON、ECON、SiTH、SIFU、TeCH 等）依赖 SMPL/SMPL-X 等人体参数模型作为几何先验，但这些模型无法表示宽松衣物的 3D 细节，且在姿态/形状估计不准时产生"弯腿"等严重伪影。
2. **无模板方法**（PIFu、PIFuHD、PHORHUM 等）虽摆脱了参数约束，但因缺乏充分的人体特定先验，在纹理一致性和几何细节方面仍然不佳。
3. **通用 image-to-3D 方法**（Zero-1-to-3、Magic123 等）在通用物体上表现不错，但缺少人体特异性先验，导致人体几何不准确、面部和衣物细节丢失。
