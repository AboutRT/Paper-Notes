...
1. **不切实际的假设**：要求对所有个人图像（包括已分享的、新创建的）施加扰动，这在实践中不可能做到
2. **易被绕过**：即使图像被扰动，攻击者只需混入少量干净图像或施加简单变换（翻转、模糊）就能削弱保护效果
3. **用户负担重**：数据投毒需要技术专长，普通用户难以操作
4. **不适合服务商**：GDPR等隐私法规要求服务提供商承担隐私保护义务，但用户级的数据投毒方法天然不适合提供商侧部署
