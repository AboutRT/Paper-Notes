...
TDA 方法分两类：
1. **黑盒方法**（模型不可访问）：基于相似度的协同归因，使用外部编码器计算生成输出与训练数据的相似度（如 CLAP、CLEWS）。简单实用但依赖编码器视角，不一定反映生成模型内部行为。
2. **白盒方法**（可访问模型参数）：基于反事实推理，"如果移除某训练样本，模型预测会如何变化？"最直接的方法是 leave-one-out retraining（计算不可行），influence function 可近似变化但在大模型上也有局限。
