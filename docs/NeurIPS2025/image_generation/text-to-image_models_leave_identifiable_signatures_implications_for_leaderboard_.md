---
description: "[NeurIPS2025] 揭示文生图模型在生成图像中留下可识别签名，使得投票式排行榜中通过CLIP嵌入分类即可高精度模型去匿名化，威胁排名安全"
tags:
  - NeurIPS2025
  - Image Generation
...
**核心 idea 一句话**：利用 T2I 模型生成图像的低模内多样性和高模间差异性，在 CLIP 嵌入空间中通过无需训练的质心分类即可高效去匿名化排行榜上的模型，揭示 T2I 排行榜面临的严重安全威胁。
...
1. **质心去匿名化算法**:
    ...
    - 做什么：在 CLIP 嵌入空间中通过最近质心匹配识别排行榜图像的生成模型
...
2. **提示级可区分性度量（Distinguishability Metric）**:
    ...
    - 做什么：量化每个提示下不同模型的生成在嵌入空间中的可分离程度，用于识别最容易暴露模型身份的提示
...
3. **一对多分类与目标攻击**:
    ...
    - 做什么：评估攻击者针对特定模型（而非识别所有模型）的去匿名化能力
