---
description: >-
  NeurIPS2025论文提出ALE-Bench，首个面向分数制算法竞赛(AtCoder Heuristic Contest)的AI基准，收集40道NP-hard优化赛题，提供Session交互接口支持LLM/Agent迭代改进，并设计ALE-Agent原型（领域知识注入+多样性搜索），评估22个前沿模型发现最强AI(o3-high)仅达人类平均水平，暴露出AI在跨问题一致性和长时间推理上的显著差距。
tags:
  - NeurIPS 2025
  - benchmark
...
1. **Session交互系统**:
    - 做什么：为AI提供模拟真实AHC比赛的完整交互环境，支持四种操作（查看题目、测试运行、可视化、最终提交）
...
2. **ALE-Agent（算法工程专用Agent）**:
    - 做什么：在ALE-Bench框架上构建的强baseline Agent，通过领域知识注入和多样性搜索两个技术显著提升LLM的算法工程能力
...
3. **评估指标体系**:
    - 做什么：提供细粒度（单题Performance）和聚合（average performance、rating）两层评估指标，支持AI与人类的公平比较
