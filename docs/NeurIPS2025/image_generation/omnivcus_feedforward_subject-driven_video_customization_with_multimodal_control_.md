...
1. **多主体数据构建困难**：ConceptMaster 仅支持有限类别，Video Alchemist 依赖稀缺的高质量文本-视频对，数据规模有限
2. **推理时主体数量受限**：训练视频中主体数量有限（通常1-2个），如何在推理时组合更多主体尚未被充分探索
3. **多模态控制条件缺失**：如何将深度图、分割掩码、相机轨迹、文本编辑指令等控制信号融入主体驱动定制，仍是开放问题
