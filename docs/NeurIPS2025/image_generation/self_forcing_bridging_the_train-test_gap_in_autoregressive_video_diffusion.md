...
1. **Teacher Forcing (TF)**：训练时用真实帧作为上下文去噪下一帧，但推理时必须依赖自己之前生成的不完美帧——训练从未见过自己的错误输出，误差不断累积。
2. **Diffusion Forcing (DF)**：训练时给上下文帧加独立噪声，虽然覆盖了"干净上下文+噪声当前帧"的推理场景，但训练输出本身仍不属于模型推理时的真实分布。
