...
**本文目标** 设计一个在比特级操作的水印方案，满足：(a) 不降低图像质量，(b) 可靠检测，(c) 抵抗多种去除攻击，(d) 具有放射性。

**切入角度**：观察到 Infinity 模型中 bit 级重合率（77.4%）远高于 token 级（2.4%），因此在 bit 级嵌入水印信号比 token 级更鲁棒。

**核心 idea**：将 LLM 的 green/red list 水印思想从 token 级下沉到 bit 级，利用比特级自回归图像模型的生成过程嵌入可检测信号。
