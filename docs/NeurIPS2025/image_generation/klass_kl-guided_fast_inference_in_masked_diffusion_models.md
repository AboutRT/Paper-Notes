...
**代码**: [GitHub](https://github.com/shkim0116/KLASS) (有)  
**领域**: 图像生成 / 离散扩散模型采样  
...
1. **领域现状**：掩码扩散模型（MDM）在语言生成、图像生成、分子生成等任务上展现了竞争力，如 LLaDA、DREAM 等大规模模型已具备推理能力。
2. **现有痛点**：MDM 的采样过程依赖迭代去掩码，通常采用固定的 Top-k 或随机采样策略，每步仅解掩很少 token，导致推理速度慢。
3. **核心矛盾**：加速采样（一次解掩更多 token）与生成质量之间存在权衡——过早解掩不稳定的 token 会降低准确率。
4. **本文要解决什么**：如何在不需要额外训练或外部 planner 的前提下，安全地并行解掩多个 token 以加速生成。
5. **切入角度**：利用模型自身的信号（KL 散度 + 置信度）来判断 token 是否"稳定"，稳定的 token 可以安全地提前解掩。
6. **核心 idea**：KL 散度低 + 置信度高的 token 是"稳定"的，可被并行解掩；不正确的 token 在扩散过程中无法保持动态稳定性。
