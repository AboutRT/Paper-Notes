...
description: >-
  [NeurIPS 2025][图像生成][扩散模型] 提出 DWGF（Diffusion-regularized Wasserstein Gradient Flow），将隐空间扩散模型的后验采样问题严格形式化为 KL 散度在 Wasserstein-2 空间上的正则化梯度流…
tags:
  - "NeurIPS 2025"
  - "图像生成"
  - "扩散模型"
...
**领域现状**：图像逆问题（修复、超分辨率、去模糊等）需要强大的先验。近年来扩散模型成为 SOTA 先验，通过修改采样过程引导后验分布。常用方法包括 DPS（梯度引导）、RED-Diff（变分推断）、SMC 方法等。

**现有痛点**：大多数方法针对像素空间扩散模型设计，但实际中计算效率更高的隐空间扩散模型（如 Stable Diffusion）定义的是隐变量 $z_0$ 的先验而非 $x_0$ 的先验，将像素空间方法适配到隐空间并不直接。现有隐空间方法（PSLD、RLSD）要么图像质量不够好，要么缺乏严格的理论基础。

**核心矛盾**：隐空间 LDM 的先验定义在 $z_0$ 上，但观测约束（似然函数）定义在 $x_0$ 上，两个空间通过非线性编码-解码器相连，如何在隐空间做后验采样需要仔细处理。

**本文目标**
   - 如何从第一性原理出发，严格推导出用隐空间扩散模型求解逆问题的方法？
   - 如何在隐空间中同时考虑数据似然和扩散先验？

**切入角度**：将后验采样视为概率测度空间上的优化问题——在 Wasserstein-2 空间中最小化近似后验与真实后验之间的 KL 散度，加上基于扩散模型的正则项。

**核心 idea**：将隐空间后验采样形式化为正则化 Wasserstein 梯度流，推导出粒子 ODE 系统，用 Adam 优化器模拟梯度流轨迹。
