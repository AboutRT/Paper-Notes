...
**领域现状**：局部固有维度（LID）是信号处理和学习理论中的基本量，决定了数据的可压缩性和学习效率。高维复杂数据的 LID 估计历来困难。近年来发现扩散模型可以通过得分估计的谱或密度估计的噪声响应来捕获数据的 LID，产生了 FLIPD 和 Normal Bundle 等方法。

**现有痛点**：现有参数化 LID 估计方法需要大量计算——FLIPD 需要计算散度（涉及梯度），Normal Bundle 方法需要大量前向传播，在计算和内存受限场景下适用性有限。

**核心矛盾**：高精度的 LID 估计与计算效率之间存在权衡——准确方法计算量大，简单方法精度不够。

**本文目标**
   - DSM loss 与 LID 之间是否存在精确的数学关系？
   - 能否直接用 DSM loss 作为更高效的 LID 估计器？

**切入角度**：从信息论（熵功率不等式）出发，分析去噪误差在流形切空间和法空间上的不同表现，证明 DSM loss 的下界就是 LID。

**核心 idea**：噪声沿流形切方向不可预测（贡献 MSE≥1/维）、沿法方向可被完美去噪（贡献 MSE=0），因此 DSM loss 的下界等于切空间维度，即 LID。
