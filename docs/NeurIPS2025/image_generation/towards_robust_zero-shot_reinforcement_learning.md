---
description: "【论文笔记】Towards Robust Zero-Shot Reinforcement Learning 论文解读 | NeurIPS 2025 | arXiv 2510.15382 | 零样本强化学习 | 提出BREEZE框架，通过行为正则化、任务条件扩散策略和注意力增强表示建模，系统性解决FB-based零样本RL中的OOD外推误差和表达力不足问题，在ExORL和D4RL Kitchen上实现最优或接近最优的鲁棒零样本泛化性能。"
tags:
  - NeurIPS 2025
  - 零样本强化学习
  - Forward-Backward表示
...
1. **后继度量估计偏差严重**：$M^{\pi_z}$ 理论上是正值（表示未来状态占用），但实际学到的 $F^\top B$ 包含大量无效负值和规模失配的极端值。这些不准确的表示传导到 $Q$ 值估计，导致系统性的预测偏差。

2. **表达力不足**：学习涵盖所有可能任务向量 $z$ 的后继度量和策略需要高表达力模型，但现有FB方法在表示网络和策略上的表达力均不够。简单放大MLP网络规模并不能带来改善，说明问题在于架构设计本身。
