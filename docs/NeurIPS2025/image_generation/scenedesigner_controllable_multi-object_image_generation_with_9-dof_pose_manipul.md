...
1. **2D 控制主导**：大多数方法（GLIGEN、InstanceDiffusion）只能处理 2D 边界框，无法表达 3D 属性
2. **朝向控制不足**：LOOSECONTROL 使用 3D 边界框但缺少朝向信息，同一边界框可能对应正面或背面物体
3. **多物体场景困难**：Continuous 3D Words 和 COMPASS 只能处理单物体且视觉风格不真实
4. **方法兼容性差**：ORIGEN 依赖一步生成模型，无法与主流多步扩散框架兼容
