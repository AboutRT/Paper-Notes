...
1. **基于 do-calculus 的符号方法**：可以判断因果查询是否可识别，但不直接提供数值估计
2. **前门/后门准则方法**：只能处理特定混淆结构，不通用
3. **VAE/因果表示学习方法**：需要为每个因果查询单独训练，扩展性差
4. **因果 Normalizing Flow（如 VACA）**：不处理隐藏混淆因子，假设所有变量可观测
