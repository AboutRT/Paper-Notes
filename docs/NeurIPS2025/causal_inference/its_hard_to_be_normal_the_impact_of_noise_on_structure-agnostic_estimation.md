...
description: >-
  [NeurIPS 2025][DML] 证明 Double Machine Learning (DML) 在高斯处理噪声下是极小极大最优的（$O(\epsilon^2 + n^{-1/2})$），但在非高斯噪声下变得次优；提出 Agnostic Cumulant-based Estimation (ACE) 利用高阶累积量达到 $r$ 阶不敏感性 $O(\epsilon^r + n^{-1/2})$。
tags:
  - NeurIPS 2025
  - DML
  - 极小极大最优
...
**本文目标**：完整刻画处理噪声分布对结构无关因果估计的影响——什么时候 DML 最优（不可逾越），什么时候可以更好？

**切入角度**：Robinson (1988) 的部分线性模型 $Y = \theta T + g(X) + \epsilon$——最经典的因果推断设置。处理变量 $T$ 的噪声分布（高斯 vs 非高斯）决定了可达到的最优速率。

**核心 idea**：高斯噪声是不可逾越的障碍（DML 最优），非高斯噪声可以用累积量估计器实现任意高阶不敏感性。
