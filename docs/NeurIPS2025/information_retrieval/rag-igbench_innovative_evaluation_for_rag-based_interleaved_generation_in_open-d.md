...
**代码**: https://github.com/USTC-StarTeam/RAG-IGBench  
**领域**: 多模态VLM  
...
1. **领域现状**：交错图文生成（interleaved image-text generation）需要模型同时产出文本和图像，是实际应用（如内容创作、可视化故事讲述）的核心需求
2. **现有痛点**：
   - 端到端生成方法（如 Chameleon）虽然统一了文本和图像处理，但在遵循复杂指令方面能力有限
   - 现有评估框架要么只评估单模态指标（如 FID 只评图像），要么依赖 MLLM 打分（GPT-4o based），后者引入模型偏差和不稳定性
   - 缺乏高质量的开放域交错图文生成数据集
3. **核心矛盾**：如何在不依赖模型偏差的前提下全面评估交错图文内容的质量
4. **切入角度**：采用 RAG 框架——MLLM 从检索到的文档中选择图像并嵌入文本中，而非生成图像
...
2. **图像质量评估（Edit Distance + Kendall Score）**：
   - 做什么：评估模型选择的图像序列与 ground truth 的匹配度
