---
description: "【论文笔记】MIR-Bench: Can Your LLM Recognize Complicated Patterns via Many-Shot In-Context Reasoning? 论文解读 | NeurIPS 2025 | arXiv 2502.09933 | Many-Shot ICL | 提出 MIR-Bench，首个大规模多样化的 many-shot 上下文推理基准，通过从编程题中自动生成输入输出对来测试 LLM 的模式识别能力，发现 LLM 在 many-shot 场景下存在注意力分散导致的性能饱和现象，且转导推理普遍优于归纳推理。"
tags:
  - NeurIPS 2025
  - Many-Shot ICL
  - 模式识别
...
1. **领域现状**: 从示例中识别模式并泛化到新实例是通用智能的基础能力，被心理学和AI研究广泛关注。LLM 上下文长度不断扩展（128K→2M甚至无限），催生了 many-shot ICL 新范式——使用数百到数千个示例进行测试时任务学习，无需昂贵的微调。
2. **现有痛点**: 现有模式识别基准（ARC、WILT、KORBench等）都聚焦 few-shot（通常<10个示例），缺乏对从长上下文中整合大量信息能力的评估。另一方面，many-shot ICL 评测多局限于分类任务，而主流长上下文评测（如 NIAH）只是检索问题，不需要从多条线索中提取复杂模式。两个社区都存在空白。
3. **核心矛盾**: 某些现实世界问题的底层规则本质上太复杂或模糊，少量示例无法确定（如3个点无法区分圆和二次曲线），但300个示例就能清晰推断。LLM 应该能处理这种长上下文、多示例的推理，然而目前没有合适的基准来衡量这一能力。
4. **本文要解决什么？**: 构建首个大规模、多样化的 many-shot 模式识别推理基准，同时填补模式识别社区缺乏 many-shot 评测和长上下文社区缺乏复杂推理任务这两个空白。
5. **切入角度**: 利用编程题的解题函数作为底层模式来源——编程题天然具有多样的输入输出类型和难度级别，且解题函数可以自动生成大量输入输出对，避免数据泄漏。
6. **核心idea一句话**: 将入门级编程题的解题函数转化为模式识别挑战，让 LLM 仅从大量输入输出对中推断底层函数，构建涵盖6930个问题、支持4到2048个示例的系统化评测。
...
#### 超集保证设计
- **做什么**: 确保更多 shot 的测试用例是更少 shot 的超集
