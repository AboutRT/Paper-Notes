...
**本文要解决什么？**: 构建首个大规模、多样化的 many-shot 模式识别推理基准，同时填补模式识别社区缺乏 many-shot 评测和长上下文社区缺乏复杂推理任务这两个空白。

**切入角度**: 利用编程题的解题函数作为底层模式来源——编程题天然具有多样的输入输出类型和难度级别，且解题函数可以自动生成大量输入输出对，避免数据泄漏。

**核心idea一句话**: 将入门级编程题的解题函数转化为模式识别挑战，让 LLM 仅从大量输入输出对中推断底层函数，构建涵盖6930个问题、支持4到2048个示例的系统化评测。
