...
**领域现状**：RAG 通过引入外部文档增强 LLM 的事实准确性，已成为主流范式。但 RAG 系统高度依赖外部语料库，天然易受语料库投毒（corpus poisoning）和检索污染（retrieval contamination）攻击。
...
**核心矛盾**：激进过滤会丢失有价值信息降低召回，宽松过滤则保留毒化内容降低可靠性——在信息保留与安全过滤之间存在根本性的 trade-off。

**本文目标**：(1) 用细粒度语义信息替代粗粒度统计过滤，精准识别投毒文档；(2) 在推理阶段增加冲突检测，确保最终生成基于一致且可信的知识。

**切入角度**：引入结构化语义提取模块 EIRE，从文档中抽取实体、意图和关系三元组，构建语义图。正常文档的语义图结构连通密集，投毒文档的语义图则稀疏断裂——这一结构性差异可用于精准识别。

**核心 idea**：将结构化语义信息（实体-意图-关系）引入 RAG 防御的检索和推理两个阶段，实现细粒度投毒检测与冲突消解。
