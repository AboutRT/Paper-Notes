...
3. **核心矛盾**：如何同时实现高表达力、长程依赖捕获和高效并行计算？
4. **切入角度**：SSM（状态空间模型）在离散序列建模中已展示高效并行+长程依赖优势，但直接应用于 MTPP 面临事件间断性（跳跃输入）问题。
5. **核心idea**：将 Hawkes 过程的跳跃微分方程与 SSM 的状态递推统一，构建既保留事件序列归纳偏置又能用并行扫描高效计算的 LLH 层。
