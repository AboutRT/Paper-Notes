...
1. **忽略场景上下文**：现有 3D 检索方法（如 ULIP、OpenShape）主要关注物体级别的几何特征，不考虑空间关系、语义一致性和风格协调性
2. **缺乏标准检索范式**：与 NLP 领域成熟的 DPR 双编码器架构不同，3D 资产检索缺乏专门的标准化框架
3. **单模态查询限制**：大多数方法仅支持单一模态（3D→3D 或 文本→3D），无法处理多模态组合查询
