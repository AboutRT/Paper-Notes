---
description: 提出Hybrid-Balance GFlowNet框架，统一局部（DB）和全局（TB）优化用于求解车辆路径问题，配合depot引导推理显著提升性能
tags:
- GFlowNet
- vehicle-routing
