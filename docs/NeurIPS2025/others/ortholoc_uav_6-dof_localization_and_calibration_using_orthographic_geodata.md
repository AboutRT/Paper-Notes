---
description: "OrthoLoC提出首个基于正射地理数据的UAV 6-DoF定位数据集，包含16425张图像覆盖两国47个区域，并引入AdHoP匹配改进技术，将特征匹配提升95%、平移误差降低63%。"
tags: [UAV-localization, visual-localization, geodata, 6-DoF, feature-matching]
---
...
**本文要解决什么？** (1) 建立首个大规模UAV-地理数据对齐基准支持6-DoF定位评估；(2) 解决透视-正射域差异的匹配残差问题。

**切入角度** 构建配对数据集解耦检索和定位评估+利用地面近似平面假设做单应性预处理缩小域差异。

**核心idea一句话** 用DOP+DSM这种轻量政府公开数据替代昂贵的3D模型/图像数据库进行UAV 6-DoF定位。
...
1. **OrthoLoC数据集构建**:
    - 做什么：建立首个支持6-DoF UAV定位评估的大规模地理数据对齐基准
...
3. **AdHoP（自适应单应性预处理）**:
    - 做什么：缩小透视UAV图像和正射DOP间的域差异以改善特征匹配
