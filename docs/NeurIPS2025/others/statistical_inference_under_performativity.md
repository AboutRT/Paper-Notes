...
**本文要解决什么？** （1）为批量重复风险最小化（RRM）建立中心极限定理；（2）提供数据驱动的协方差估计方法（不再假设结构信息已知）；（3）将 PPI 推广到表演性设置，用少量标注数据+大量未标注数据获得更好的推断。

**切入角度**：从批量（batch）RRM入手而非单样本在线更新，更贴近政策制定的实际场景。通过创新的梯度无关得分匹配方法和策略扰动技巧来估计分布映射的梯度。

**核心idea一句话**：通过递推分析误差传播建立动态设置下的CLT，并用策略扰动+得分匹配绕过分布映射梯度的不可观测性，实现端到端统计推断。
