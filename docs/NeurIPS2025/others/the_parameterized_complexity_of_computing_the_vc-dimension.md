...
**本文要解决什么**：(1) 朴素穷举 $2^{O(|V|)}$ 是否真的不可避免？(2) 哪些超图/图的结构参数能让VC维计算变为固定参数可处理？(3) 对所有主流结构参数给出完整的复杂性分类。

**切入角度**：从参数化复杂性的视角出发，系统考察解大小、最大度、维度、退化度、超树宽、横贯数、树宽、顶点覆盖数等参数，给出算法或硬度结果，绘制完整景观图。特别地，引入 Gen-VC-Dimension 统一集合系统VC维和图VC维。

**核心idea一句话**：利用 shattered 集在树分解中的结构性质（要么局部于某个 bag，要么大小受限于 $O(\log \text{tw})$），将VC维计算转化为树分解上的模式图匹配动态规划。
