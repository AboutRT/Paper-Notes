---
description: "【论文笔记】Coreset for Robust Geometric Median: Eliminating Size Dependency on Outliers 论文解读 | NeurIPS 2025 | arXiv 2510.24621 | Coreset | 首次消除鲁棒几何中位数 coreset 大小对异常值数 m 的依赖：在 n ≥ 4m 条件下，d=1 时实现最优 coreset 大小 tildeΘ(ε^-1/2 + fracmnε⁻¹)，高维时实现 tildeO(ε⁻²minε⁻², d)，核心技术是新颖的**非逐分量误差分析**。"
tags:
  - NeurIPS 2025
  - Coreset
  - 鲁棒几何中位数
...
1. **领域现状**: 几何中位数（Fermat-Weber 问题）在计算几何中的核心地位不言而喻。鲁棒版本允许去除 $m$ 个异常值以抵抗噪声/对抗干扰，目标函数为 $\text{cost}^{(m)}(P, c) = \min_{|L|=m} \sum_{p \in P \setminus L} \|p - c\|$。
2. **现有痛点**: 所有已知 coreset 大小都包含 $O(m)$ 项——因为现有方法将异常点全部保留在 coreset 中。当 $m = \Theta(n)$（如 PageBlocks 数据集 $m \approx 0.1n$）时，coreset 失去压缩意义。
3. **核心问题**: 能否消除 coreset 大小中的 $O(m)$？先验结论似乎是否定的（已有 $\Omega(m)$ 下界），但该下界依赖 $m = n-1$ 的极端情形。
4. **本文切入角度**: 证明 $n - m = \Omega(n)$ 是消除 $O(m)$ 的充要条件，并在此条件下提出新算法和全新的非逐分量误差分析。
