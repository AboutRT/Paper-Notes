---
description: "【论文笔记】Depth-Bounds for Neural Networks via the Braid Arrangement 论文解读 | NeurIPS 2025 | arXiv 2502.09324 | ReLU网络深度下界 | 本文证明了在 $\mathcal{B}_d^0$-conforming 约束下，ReLU 网络精确表示 $\max\{0, x_1, \ldots, x_d\}$ 需要 $\Omega(\log \log d)$ 层——这是首个不限制权重的非常数深度下界；同时证明 rank-(3,2) maxout 网络可以计算 7 个数的最大值，说明标准上界不紧。"
tags:
  - NeurIPS 2025
  - ReLU网络深度下界
  - 分段线性函数
...
1. **领域现状**：ReLU 网络可以精确表示所有连续分段线性（CPWL）函数。Arora et al. (2018) 证明 $\lceil \log_2(d+1) \rceil$ 层足够。Bakaev et al. (2025) 改进到 $\lceil \log_3(d-1) \rceil + 1$ 层。但深度**下界**几乎空白。
2. **核心开放问题**：精确表示 $\mathbb{R}^d$ 上所有 CPWL 函数最少需要多少层？目前最好的无条件下界仅为 2。
3. **关键简化**：由 Wang & Sun (2005) 知，任意 CPWL 函数可化为 $\max$ 函数的表示——因此问题归结为：计算 $d$ 个数的最大值需要多深的网络？
4. **研究路线**：限制 breakpoints（断点）位于 braid 排列上——即 $\mathcal{B}_d^0$-conforming。这一限制虽非完全一般，但提供了可分析的框架。
