...
1. **ACC 方法的先验概率偏移假设不适用**：经典 ACC 假设测试集与训练集之间只存在先验概率偏移（prior probability shift），但在图数据中，由于图结构的影响，实际上存在**协变量偏移（covariate shift）**
2. **节点聚类方法的局限**：此前图量化只能通过节点聚类方法解决，无法利用标签信息
3. **非同质性边的影响**：当图中存在大量非同质性边（异质性连接）时，量化精度下降
