---
title: >-
  [论文解读] AgentChangeBench: A Multi-Dimensional Evaluation Framework for Goal-Shift Robustness
description: >-
  [NeurIPS 2025][LLM Agent][benchmark] AgentChangeBench 是首个系统评估 LLM agent 在对话中途目标切换时适应能力的 benchmark：315 基础任务 × 9 变体 = 2835 序列，覆盖 3 个企业领域（银行/零售/航空）和 5 种 user persona，引入 GSRT（目标切换恢复时间）等 4 个互补指标，揭示高 pass@k 掩盖的效率和鲁棒性差距——如 GPT-4o 航空恢复率 92.2% 但零售冗余率达 89.1%。
tags:
  - NeurIPS 2025
  - LLM Agent
  - benchmark
...
**本文目标** 构建显式包含"目标切换序列"的 benchmark + 多维评估指标，量化 agent 在动态目标下的适应能力。

**切入角度**：将每个任务定义为有序目标序列（如 ["authentication","transactions","dispute"]），用 persona 化 user simulator 在对话中自然触发目标切换，然后用 GSRT 多阶段指标量化恢复过程。

**核心 idea**：用显式目标序列 + 四维指标（成功率/效率/冗余率/恢复时间）替代 pass@k，揭示 agent 在动态目标下的真实鲁棒性。
...
2. **目标序列设计**：
    ...
    - 功能：每个任务的 JSON schema 中显式声明 `goal_shifts: {required_shifts: k, goals: [g1,...,g{k+1}]}`
