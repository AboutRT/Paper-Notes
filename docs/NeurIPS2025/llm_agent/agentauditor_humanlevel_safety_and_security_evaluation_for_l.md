---
description: 免训练的记忆增强推理框架，通过结构化语义特征提取和多阶段检索增强生成来评估LLM agent的安全性和安全威胁
tags:
- LLM-agent
- safety-evaluation
- RAG
...
1. **自适应结构化语义特征提取（Adaptive Structured Semantic Feature Extraction）**:
    ...
    - 做什么：对每条 agent 交互记录 $r_i$，使用 LLM 自动提取结构化三元组 $(s_i, t_i, b_i)$，其中 $s_i$ 是场景类型（如"智能家居控制"、"金融助手"、"网页浏览"），$t_i$ 是风险类型（如"隐私泄露"、"财产损失"、"物理伤害"），$b_i$ 是行为模式（如"未经授权的数据访问"、"操作累积风险"）。同时为每条记录生成完整的 CoT 推理轨迹——详细说明"为什么这条记录是安全/不安全的"推理过程。
...
2. **代表性记忆选择——PCA + FINCH 聚类**:
    ...
    - 做什么：从全部带标注的交互记录中，通过无监督聚类选出一小批"最具代表性"的记录作为经验记忆单元，压缩信息量同时保留多样性。
...
3. **多阶段上下文感知检索增强生成（Multi-Stage Context-Aware RAG）**:
    ...
    - 做什么：对新的待评估交互记录，执行两阶段检索从经验记忆库中找到最相关的 k=3 条记忆单元及其 CoT 推理轨迹，将这些推理经验注入 LLM 评估器的 prompt 中。
