...
1. **领域现状**：推测解码已成为降低 LLM 推理延迟的标准技术，draft 模型+验证器的组合广泛采用。

2. **现有痛点**：
   - 传统推测解码针对多样化请求优化，但 Agent 工作负载是**重复推理**（多 agent 管线、自优化循环）
   - Draft 模型需学习 diverse 任务分布，难以捕捉 Agent 场景的重复性

3. **核心矛盾**：Agent 推理中存在大量可缓存的长 token 序列，现有方法未充分利用。

4. **切入角度**：不需训练 draft 模型，用后缀树精确匹配历史序列，自适应决定推测长度。

5. **核心idea一句话**：后缀树缓存 prompt 和前序输出中的长 token 序列，无训练、极端推测。
