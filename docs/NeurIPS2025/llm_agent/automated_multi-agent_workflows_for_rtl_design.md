...
1. **微调成本高**：现有方法 [RTLCoder, VeriThoughts] 依赖昂贵的任务特定微调，需要大量 GPU 预算和数万个训练样本
2. **推理成本高**：大型推理模型（如 o4）虽无需微调，但将计算负担转移到了推理阶段
3. **工作流设计人工化**：现有的多智能体工作流方法主要面向 QA 和数学任务，对专业领域（如 RTL 设计）存在适用性鸿沟
