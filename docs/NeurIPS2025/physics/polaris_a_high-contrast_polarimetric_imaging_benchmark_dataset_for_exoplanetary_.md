...
description: >-
  [NeurIPS 2025][物理学][高对比度成像] 构建首个系外行星偏振成像ML基准数据集POLARIS（921张VLT/SPHERE/IRDIS偏振图像+75,910张预处理曝光），提出Diff-SimCLR框架（扩散模型增强对比学习），在参考星vs目标星分类任务上达到93%准确率，仅需<10%手动标注…
tags:
  - "NeurIPS 2025"
  - "物理学"
  - "高对比度成像"
...
**本文目标** (i) 构建首个大规模系外行星HCI偏振基准数据集；(ii) 用少量标注+大量无标注数据实现自动参考星/目标星分类；(iii) 验证分类结果可用于下游背景重建。

**切入角度**：偏振差分成像（PDI）产物$Q_\phi$可直接反映尘埃盘结构——无盘信号即为参考星，天然提供弱监督信号。

**核心 idea**：用扩散模型生成的潜空间轨迹增强对比学习的特征表示，在仅96个标注样本下实现93%分类准确率。
...
1. **POLARIS数据集构建**：
    ...
    - 功能：从ESO档案获取2014-2024全部公开SPHERE/IRDIS偏振观测，统一预处理
...
3. **多模型基线评估**：
    ...
    - 功能：系统评估统计、生成、LVLM三类模型在POLARIS上的表现
...
- [\[ICML 2025\] Gravity-Bench-v1: A Benchmark on Gravitational Physics Discovery for Agents](../../ICML2025/physics/gravity-bench-v1_a_benchmark_on_gravitational_physics_discovery_for_agents.md)
