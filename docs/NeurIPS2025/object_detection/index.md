...
**[Any Large Language Model Can Be A Reliable Judge Debiasing W](any_large_language_model_can_be_a_reliable_judge_debiasing_w.md)**
...
**[Cq-Dino Mitigating Gradient Dilution Via Category Queries For Vast Vocabulary Ob](cq-dino_mitigating_gradient_dilution_via_category_queries_for_vast_vocabulary_ob.md)**
...
**[Delving Into Cascaded Instability A Lipschitz Continuity View On Image Restorati](delving_into_cascaded_instability_a_lipschitz_continuity_view_on_image_restorati.md)**

:   提出首个面向动态场景的多帧去闪烁（MFFR）基准数据集 BurstDeflicker，通过 Retinex 合成、真实静态采集与绿幕合成三种互补策略构建大规模训练/测试数据，显著提升闪烁去除模型在真实动态场景中的泛化能力。

**[Detectiumfire A Comprehensive Multi-Modal Dataset Bridging Vision And Language F](detectiumfire_a_comprehensive_multi-modal_dataset_bridging_vision_and_language_f.md)**
...
**[Detree Detecting Human-Ai Collaborative Texts Via Tree-Structured Hierarchical R](detree_detecting_human-ai_collaborative_texts_via_tree-structured_hierarchical_r.md)**

:   提出 DETree 框架，通过构建层次亲和树（HAT）建模不同人机协作文本生成过程之间的层次关系，并设计树结构对比损失（TSCL）对齐表示空间，在混合文本检测和 OOD 场景下取得了显著优势。

**[Diffusion-Classifier Synergy Reward-Aligned Learning Via Mutual Boosting Loop Fo](diffusion-classifier_synergy_reward-aligned_learning_via_mutual_boosting_loop_fo.md)**
...
**[Dithub A Modular Framework For Incremental Openvocabulary Ob](dithub_a_modular_framework_for_incremental_openvocabulary_ob.md)**
...
**[Dynamic Features Adaptation In Networking Toward Flexible Training And Explainab](dynamic_features_adaptation_in_networking_toward_flexible_training_and_explainab.md)**

:   提出 DAFI（Drift-Aware Feature Importance）算法，利用分布漂移检测动态切换 SHAP/MDI 两种特征重要性方法，结合自适应随机森林（ARF）实现通信网络场景下特征动态增加时的灵活训练与高效可解释推理。

**[Flexevent Towards Flexible Event-Frame Object Detection At Varying Operational F](flexevent_towards_flexible_event-frame_object_detection_at_varying_operational_f.md)**
...
**[Pandapose 3D Human Pose Lifting From A Single Image Via Propagating 2D Pose Prio](pandapose_3d_human_pose_lifting_from_a_single_image_via_propagating_2d_pose_prio.md)**
...
**[Recon-Gs Continuum-Preserved Gaussian Streaming For Fast And Compact Reconstruct](recon-gs_continuum-preserved_gaussian_streaming_for_fast_and_compact_reconstruct.md)**

:   提出 ReCon-GS，通过连续性保持的 Gaussian 流式处理实现增量式 3D 重建，在保持渲染质量的同时大幅减少存储需求和训练时间，支持大规模场景的实时重建。

**[Recon Region-Controllable Data Augmentation With Rectification And Alignment For](recon_region-controllable_data_augmentation_with_rectification_and_alignment_for.md)**
...
**[Sample Complexity Of Distributionally Robust Average-Reward Reinforcement Learni](sample_complexity_of_distributionally_robust_average-reward_reinforcement_learni.md)**
...
**[Towards Effective Federated Graph Foundation Model Via Mitigating Knowledge Enta](towards_effective_federated_graph_foundation_model_via_mitigating_knowledge_enta.md)**
...
**[You Can Trust Your Clustering Model A Parameter-Free Self-Boosting Plug-In For D](you_can_trust_your_clustering_model_a_parameter-free_self-boosting_plug-in_for_d.md)**
