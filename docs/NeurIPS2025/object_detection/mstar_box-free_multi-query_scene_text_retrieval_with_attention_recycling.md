...
1. **领域现状**：场景文本检索（Scene Text Retrieval）旨在根据查询从图像集中搜索包含相关文本的图像，在签名检索、关键帧提取等领域有广泛应用。近年来在准确文本定位的辅助下取得了显著进展。
2. **现有痛点**：(1) 现有方法通常需要昂贵的边界框标注用于训练（word级、text-line级等多种标注），成本高昂；(2) 大多数方法采用定制化检索策略，难以统一处理多种查询类型（单词、短语、组合、语义查询）。
3. **核心矛盾**：视觉语言模型（VLM）在大规模无框预训练中表现强大，但倾向于关注显著视觉概念而忽略细粒度的场景文本实例（如图像中的小字）。
4. **本文要解决什么**：实现无边界框监督的场景文本检索，同时统一多种查询类型。
5. **切入角度**：利用 VLM 的注意力机制，通过注意力回收（Attention Recycling）逐步从高注意力区域转移到被忽视的区域。
6. **核心idea一句话**：通过渐进式遮蔽高注意力区域迫使模型关注不显著文本，并通过风格指令统一多种查询类型。
...
1. **Progressive Vision Embedding (PVE)**:
   - **做什么**：渐进式提取视觉嵌入，将注意力从显著区域转移到不显著的细节文本区域
