...
description: >-
  [NeurIPS 2025][目标检测][visual attribute reliance] 提出一个自反思 agent 框架，通过迭代的假设生成-测试-验证-反思循环来自动检测视觉模型中的属性依赖（如 CLIP 识别 teacher 依赖教室背景、YOLOv8 检测行人依赖人行横道）…
tags:
  - "NeurIPS 2025"
  - "目标检测"
...
**代码**: [https://christykl.github.io/saia-website/](https://christykl.github.io/saia-website/)  
**领域**: 目标检测  
...
**本文目标** 给定一个预训练视觉模型和目标概念，自动识别影响模型预测的视觉属性。

**切入角度**：将属性发现视为科学发现过程——agent 自主提出假设、设计实验（生成测试图像）、观察结果、反思并修正假设。

**核心 idea**：用多模态 LLM 实现的自反思 agent 像科学家一样迭代地假设-实验-验证-反思，自动发现视觉模型的属性依赖。
...
3. **自我评估（Self-Evaluation）**:
    ...
    - 功能：在无 ground-truth 的情况下评估发现的可靠性。
