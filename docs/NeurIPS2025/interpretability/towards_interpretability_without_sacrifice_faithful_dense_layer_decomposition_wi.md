...
**本文目标** 在保持高稀疏度的同时，忠实地重建原始 MLP 层的功能。

**切入角度**：从神经元级稀疏性转向**层级稀疏性**——每次选择少量全秩线性变换（专家子层），每个专家的表达能力远强于单个神经元。

**核心 idea**：用 Hadamard 乘积参数化的张量分解构造大量参数高效的满秩专家子层，稀疏激活 $K$ 个即可忠实重建原始 MLP。
...
3. **GLU 扩展**：
    ...
    - 功能：推广到现代 LLM 的 Gated Linear Unit 架构
