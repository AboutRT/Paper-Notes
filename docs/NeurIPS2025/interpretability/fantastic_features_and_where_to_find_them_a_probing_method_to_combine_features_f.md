...
1. **PEFT 方法**（如 LoRA、Adapter）仅适配单模型，且需要反向传播通过 backbone，多模型组合计算代价高
2. **蒸馏方法**（如 RADIO）需将多个 FM 蒸馏到学生模型，代价高、易出现 "mode switch" 问题
3. **现有 probing 方法**（Head2Toe、SMP）需要数据集特定的超参调优、对 feature map 做平均池化丢失空间信息、扩展性差
