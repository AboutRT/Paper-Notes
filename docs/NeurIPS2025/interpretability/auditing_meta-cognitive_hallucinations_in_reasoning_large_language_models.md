...
description: >-
  [NeurIPS 2025][可解释性][hallucination] 系统性审计推理大模型（RLLM）中幻觉的产生与传播机制，发现长 CoT 中的反思（reflection）会通过元认知偏差放大幻觉而非纠正它，即使在幻觉源头进行干预也难以改变最终结果（chain disloyalty）…
tags:
  - "NeurIPS 2025"
  - "可解释性"
  - "hallucination"
...
**本文目标** (1) 如何在可控环境中复现和分类幻觉？(2) 反思机制如何与幻觉交互？(3) 干预能否有效纠正幻觉？(4) 现有检测方法是否可靠？

**切入角度**：构建基于 RFC 文档的受控知识域，将幻觉分为 Type I（训练中见过但未学会）和 Type II（训练中未见过），通过审计 CoT 轨迹中的知识流、反思模式和元认知置信度来分析幻觉机制。

**核心 idea**：在黑箱设置下通过 CoT 轨迹审计实现可解释的长链幻觉归因，揭示反思机制在幻觉传播中的双面性。
...
2. **CoT 知识流建模**:
    ...
    - 功能：将长 CoT 形式化为推理图，追踪知识如何在步骤间流动。
...
3. **元认知置信度模型**:
    ...
    - 功能：建模反思过程中模型对自身知识状态的主观评估（而非事实正确性）。
...
4. **RFC 受控知识域**:
    ...
    - 功能：基于 RFC 文档构建满足 bounded scope 和 verifiability 的知识环境。
