---
description: "AdaptGrad提出自适应梯度平滑方法，通过分析SmoothGrad中超范围采样导致的额外噪声，自适应调整每个像素的采样方差，几乎消除梯度噪声并生成更清晰的显著性图。"
tags: [interpretability, gradient-smoothing, saliency-map, explainability, SmoothGrad]
---
...
**本文要解决什么？** 从理论上分析并最小化SmoothGrad的额外噪声，实现几乎无噪的梯度平滑。

**切入角度** 将SmoothGrad重新理解为蒙特卡洛近似的卷积操作，推导出额外噪声的解析表达式，据此设计自适应的采样方差。

**核心idea一句话** 根据每个输入像素到数据范围边界的距离自适应计算采样方差，确保超范围采样概率不超过预设上限 $1-c$。
...
3. **与其他方法的组合**:
    - 做什么：AdaptGrad可替代SmoothGrad与任意梯度方法组合
