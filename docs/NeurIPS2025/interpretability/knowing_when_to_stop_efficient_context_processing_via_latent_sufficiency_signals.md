...
1. **领域现状**：LLM 在推理时对输入上下文进行无差别处理，每个 token 获得相同的计算优先级，无论其与任务的实际相关性如何。
2. **现有痛点**：现有上下文压缩方法（LLMLingua系列、RAG）依赖预设的固定压缩率或固定检索文档数；这种"一刀切"方式无法适应不同输入的信息密度差异，容易导致信息丢失或计算浪费。
3. **核心矛盾**：人类阅读时会根据已获取信息的充分性动态决定何时停止，而LLM缺乏这种自适应能力；同时"lost-in-the-middle"现象表明冗余上下文反而降低准确率。
4. **本文要解决什么？** 让LLM能自主评估上下文是否充分，动态决定截断位置，在不预设压缩率的情况下实现效率与性能双赢。
5. **切入角度**：通过分析模型内部激活发现，特定注意力头天然编码了信息充分性信号，可通过线性探针轻量检测。
6. **核心idea一句话**：模型自身的注意力头激活中内嵌了"够不够用"的判断信号，用探针读取这个信号就能实现动态早停。
...
4. **大模型自提示替代方案（Self-Prompting）**:
   - 做什么：对14B+大模型，通过元提示让模型自行判断上下文是否充足
