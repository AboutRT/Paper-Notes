...
**代码**: 无  
**领域**: 可解释AI / 图像分类  
**关键词**: 可解释机器学习, 层次化解释, Conformal Prediction, 图像分类, 人机互补
...
1. **领域现状**：深度学习在医疗诊断、自动驾驶等安全关键领域愈发普及，可解释模型（interpretable-by-design）是实现可信 AI 的重要方向。QPM 系列方法通过紧凑的二值类别表示（每个类仅用少量特征）提供全局可解释性，能对比地解释类间差异。
...
3. **核心矛盾**：可解释性与准确率之间的 trade-off——更紧凑的表示更易解释但准确率受限；CP 方法生成的集合缺乏语义连贯性
...
5. **切入角度**：将特征检测与层次化类别结构结合，利用 QPM 的二值特征分配自然形成类别层次树，在该树上进行遍历即可得到语义连贯的集合预测。

6. **核心idea**：在 QPM 框架中引入层次化约束和 Feature Grounding Loss，使特征分配自然形成可遍历的类别层次结构，将 CP 融入这一层次结构，实现可解释的校准集合预测。
