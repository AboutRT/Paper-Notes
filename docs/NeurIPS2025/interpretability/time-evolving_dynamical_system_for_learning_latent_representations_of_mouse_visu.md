...
1. **领域偏向运动皮层**: 多数LVM研究聚焦运动脑区的神经数据（如预规划运动），对**视觉皮层**的研究很少
2. **忽略时间关系**: 自然视觉刺激本质上是高维且时间依赖的，但多数LVM未显式建模神经活动的时间关系
3. **视觉特性未充分利用**: 视觉神经活动包含刺激相关成分和内部状态成分，现有方法缺乏针对这些特性的设计
...
**做什么**: 反映动物的内部动态状态（注意力、觉醒度等），包含高变异性和噪声。
