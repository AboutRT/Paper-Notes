...
description: >-
  [NeurIPS 2025][可解释性][veracity probing] 提出 sAwMIL（稀疏感知多实例学习）三类探测框架，结合 MIL 和保形预测，将 LLM 内部激活分类为 true/false/neither，揭示真假信号并非简单的双向对称编码，而是跨越多维子空间的分布式表征。
tags:
  - "NeurIPS 2025"
  - "可解释性"
  - "veracity probing"
...
**核心 idea**：用多实例学习让探针自动发现句子中承载真实性信号的关键位置，配合保形预测量化不确定性，实现三类分类
...
1. **稀疏感知多实例学习 (sAwMIL)**：
    ...
    - 功能：将整个句子视为一个"bag"，每个 token 的激活是一个"instance"，自动识别哪些 token 承载了真实性信号
...
2. **句内标签机制（Intra-bag Labels）**：
    ...
    - 功能：将句子分为 pre-actualized 部分 $\boldsymbol{x}^p$（标签0）和 actualized 部分 $\boldsymbol{x}^a$（标签1）
