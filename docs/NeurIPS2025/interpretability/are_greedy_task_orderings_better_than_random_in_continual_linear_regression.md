...
持续学习（Continual Learning）中，模型需要按顺序学习多个任务，任务的呈现顺序对最终性能有显著影响。先前研究主要关注随机排序（random ordering），包括有放回和无放回两种形式，已证明其平均损失在 $k$ 次迭代后以 $\mathcal{O}(1/\sqrt{k})$ 收敛。然而，贪心排序——即每次选择与当前任务最不相似的下一个任务——虽然直觉上更优，但缺乏严格理论分析。

本文的核心动机是：
1. **贪心排序是否总是优于随机排序？** 直觉告诉我们多样化任务序列应该更好，但理论保证缺失
2. **允许任务重复与否对贪心排序有何影响？** 这涉及实际应用中的数据可用性约束
3. **Kaczmarz方法的理论工具如何迁移到持续学习分析？** 这是一个跨领域的方法论贡献
