...
description: >-
  [NeurIPS 2025][可解释性][scaling laws] 提出Skills Scaling Laws (Sloth)，通过假设LLM性能由低维潜在技能（如推理、指令遵循）驱动，利用benchmark间的相关性构建跨模型家族的缩放定律，用少量家族数据即可预测大模型在多个benchmark上的表现。
tags:
  - "NeurIPS 2025"
  - "可解释性"
  - "scaling laws"
...
## 研究背景与动机
**领域现状**：传统缩放定律（如Chinchilla）预测loss而非benchmark准确率，且在不同模型家族间泛化性差。

**现有痛点**：家族内缩放定律需要训练不同大小的模型（3-5个），成本高；跨家族缩放定律准确度差。

**核心矛盾**：不考虑家族信息→不准确；考虑家族信息→参数太多，需要大量训练数据。

**本文目标**：用最少的家族数据（甚至1个模型），预测该家族更大模型在多个benchmark上的表现。

**切入角度**：不同benchmark的分数是相关的（因为都反映了底层的"技能"），利用这种相关性可以减少参数。

**核心 idea**：将缩放定律建立在低维"技能空间"上而非直接在benchmark空间上，通过因子分析共享参数。
...
1. **潜在技能分解**:
    ...
    - 功能：将J个benchmark的分数分解为d个低维潜在技能的线性组合
