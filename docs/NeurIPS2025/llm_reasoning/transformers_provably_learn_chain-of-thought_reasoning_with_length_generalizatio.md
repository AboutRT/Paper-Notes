...
**本文目标**：为 Transformer 通过梯度下降学习 CoT 推理提供严格的收敛和长度泛化保证，且任务复杂度达到 $\mathsf{NC}^1$ 级别。

**切入角度**：选择 LEGO（基于群操作的状态追踪）作为分析对象——它是分离 $\mathsf{TC}^0$ 和 $\mathsf{NC}^1$ 的标准理论基准，且具有丰富的代数结构可以利用。

**核心 idea**：通过"注意力集中"机制将 attention 层的检索鲁棒性与状态追踪任务的代数结构联系起来，证明群结构决定了长度泛化的程度。
...
1. **注意力集中（Attention Concentration）机制**:
    ...
    - 功能：描述 attention 层如何实现对相关上下文的精确检索
