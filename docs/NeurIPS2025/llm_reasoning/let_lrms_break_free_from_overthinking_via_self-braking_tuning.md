...
**本文目标** 能否让 LRM 自主识别冗余推理并适时停止？即让模型内化一个"制动机制"，像人类一样在达到足够确信时自然结束推理。
...
**核心 idea**：用量化指标定位冗余推理的起始点，构造自适应长度的训练数据并配合制动提示（braking prompt），让模型通过 SFT 学会自主终止过度推理。
...
1. **推理轨迹的结构化分析**
    ...
    - 功能：将 LRM 的推理轨迹分解为 Foundation Solution（首次系统性解题）和多个 Evolution Solution（后续反思/验证/替代方案）
...
3. **SBT-E（Exact 截断策略）**
    ...
    - 功能：对每条轨迹统一保留 Foundation Solution + 1 个 Evolution Solution，然后截断
...
4. **SBT-D（Dynamic 动态截断策略）**
    ...
    - 功能：逐步分析推理轨迹，根据每步的 overthink score 动态决定截断点
...
5. **制动提示机制（Braking Prompt）**
    ...
    - 功能：在截断点插入自然语言的制动语句，如 "Wait, I've gotten the same answer multiple times, time to end the thinking."
