...
**本文要解决什么**：设计在极弱假设下具有**可证明缩放律**（provable scaling law）的测试时计算算法——即成功率可随计算量增长被提升至任意接近 100%，且仅需黑盒 LLM 本身。
...
**核心idea一句话**：生成 $N$ 个候选解 + 用 LLM 自身进行两两比较的锦标赛/联赛聚合 = 无需外部验证器的可证明测试时缩放。
