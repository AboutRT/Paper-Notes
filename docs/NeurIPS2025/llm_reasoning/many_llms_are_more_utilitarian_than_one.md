---
description: >-
  NeurIPS 2025，研究LLM多Agent系统中的"功利主义增强"现象——6款LLM在群组讨论道德困境时一致性地比单独判断更倾向功利主义（接受伤害少数人以利多数），尤其在涉及直接伤害的个人困境中，揭示多Agent对齐的新挑战。
tags:
  - NeurIPS 2025
  - LLM推理
  - 多Agent系统
...
**本文要解决什么？** LLM在群组讨论中是否也会产生功利主义增强？这种增强的机制是什么？能否被缓解？

**切入角度**：直接借用心理学中成熟的实验范式（Greene道德困境集、Oxford功利主义量表、CNI模型）来研究LLM群组。

**核心idea一句话**：LLM多Agent系统在道德讨论后系统性地向功利主义方向偏移——单个模型安全不等于群组安全。
...
1. **经典道德困境实验 (Greene dilemmas)**:
    - 做什么：测量LLM在Solo和Group条件下对道德违规的接受度变化
...
2. **CNI模型分析功利主义机制**:
    - 做什么：分解功利主义增强的来源——是对结果更敏感(C)、对规范更不敏感(N)、还是对行动更偏好(I)
...
3. **缓解策略探索**:
    - 做什么：测试模型多样性、自我反思和道德框架预设对功利主义增强的影响
