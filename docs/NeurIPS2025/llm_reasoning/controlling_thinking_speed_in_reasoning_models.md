...
**本文目标**：(1) 如何在推理过程中实现快/慢思考的平滑切换，(2) 何时切换才能最优地平衡效率和准确率。
...
**核心 idea**：用 PCA 从 LRM 隐藏层中提取"快→慢"思考的 steering vector，在推理时通过加/减该向量实现 token 级别的思考速度连续控制，并用早晚层 logit 散度作为实时难度信号驱动自适应调速。
...
2. **推理时表示控制（Representation Controlling）**：
    ...
    - 功能：在每个 token 生成时注入 steering vector 来调节思考速度
