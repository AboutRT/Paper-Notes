...
1. **领域现状**：组合零样本学习（CZSL）要求模型识别由已知属性和物体重新组合而成的未见过的属性-物体组合（如"棕色的奶酪"）。基于 CLIP 的方法（如 CSP、Troika、CDS-CZSL）通过 prompt tuning 微调 VLM 取得了显著进展
2. **现有痛点**：测试时标签空间会包含训练时未见过的组合，导致标签分布偏移。模型参数和类原型在训练后冻结，无法利用测试数据适应新分布
3. **核心矛盾**：训练只见过部分组合，但测试要求在完整组合空间上判别——这是一个根本性的分布偏移问题
4. **切入角度**：在测试时利用无标签数据流式累积知识，同时更新文本和视觉两个模态的原型
5. **核心 idea**：设计可学习的 Knowledge Accumulation Module (KAM) 调整原型偏移量，配合优先队列存储高置信度历史图像构建视觉原型
