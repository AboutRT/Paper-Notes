---
description: "【论文笔记】Multimodal Negative Learning 论文解读 | NeurIPS 2025 | arXiv 2510.20877 | 多模态融合 | 提出多模态负学习（MNL）范式，通过让强势模态引导弱势模态抑制非目标类别（而非强制对齐目标类别），稳定决策空间并保留模态特有信息，理论上收紧了多模态融合的鲁棒性下界。"
tags:
  - NeurIPS 2025
  - 多模态融合
  - 模态不平衡
...
1. **模态特有信息被压制**：弱模态被迫向强模态靠拢，自身独特互补信息丢失
2. **错误传播**：如果强模态在某些样本上也预测错误，弱模态会"盲目跟随"导致整体性能下降
3. **过对齐崩溃**：作者通过统计分析发现，经过 KL 引导对齐训练后，原本弱模态预测正确但强模态预测错误的样本，反而变成了错误预测
