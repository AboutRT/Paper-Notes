...
description: >-
  [NeurIPS 2025][多模态][visual instruction tuning] 提出 L2T（Learning to Instruct），仅通过将训练损失扩展到指令序列（不再只在回答上计算 loss）来改善视觉指令调优——无额外数据和几乎零计算开销，在 16 个多模态基准上获得高达 9% 的相对提升，captioning 提升 18%，同时缓解幻觉。
tags:
  - NeurIPS 2025
  - 多模态
...
**本文目标**：用最小的改动让 MLLM 更好地利用视觉信息，减少对语言捷径的依赖。

**切入角度**：如果模型也需要预测指令本身（如"描述这张图片"），那它必须理解图像内容才能知道什么样的指令是合理的——这迫使模型更关注视觉输入。

**核心 idea**：把 loss mask 从仅回答扩展到指令+回答，让模型同时学会"问什么"和"答什么"，zero-cost 正则化。
