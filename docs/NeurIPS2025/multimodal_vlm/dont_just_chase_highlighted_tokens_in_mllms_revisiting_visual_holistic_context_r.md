...
1. **信息冗余**: 基于注意力评分的选择倾向于保留语义相似的相邻 token，在高剪枝率下导致信息重复
2. **位置偏差**: Transformer 的位置编码使序列首尾位置的 token 获得更高注意力权重，但图像目标通常位于中心区域
3. **注意力分散**: 文本-视觉交叉注意力分散在大量 token 上，top 20% token 仅占总注意力的 40%
