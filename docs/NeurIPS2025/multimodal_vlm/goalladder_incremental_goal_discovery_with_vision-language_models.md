...
1. **嵌入方法**（如 VLM-RM 使用 CLIP）：将任务描述和观察嵌入同一空间，用余弦相似度作奖励。但 CLIP 训练数据与目标环境不匹配，导致奖励函数噪声大
2. **偏好方法**（如 RL-VLM-F）：用 VLM 比较轨迹片段生成偏好标签，再训练奖励函数。虽比嵌入方法更准确，但 VLM 的错误判断会污染偏好数据集，且需大量 VLM 查询
