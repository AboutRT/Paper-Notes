...
1. **计算开销大**：TPT（Test-Time Prompt Tuning）及其变体需要**反向传播通过大型文本编码器**来更新prompt参数，导致推理时间和内存消耗显著增加。TPT每个样本需要0.75秒和17.6GB内存。

2. **需要修改模型内部结构**：基于LoRA等参数高效微调的方法（如TTL）需要访问和修改模型的内部架构（注意力层），偏离了真正的黑盒范式，不适用于专有模型或固定结构模型。

3. **无约束的表征偏移**：TPS（Test-Time Prototype Shifting）虽然在嵌入空间直接学习偏移向量避免了编码器反向传播，但其偏移是**无约束的高维向量**——在高维空间中的任意方向偏移可能偏离语义有意义的区域，对噪声敏感。
