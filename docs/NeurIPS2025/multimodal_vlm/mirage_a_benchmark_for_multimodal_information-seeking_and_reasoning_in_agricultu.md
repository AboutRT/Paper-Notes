...
- **做什么**：构建 MIRAGE 基准，包含两大评估任务——MMST（多模态单轮）和 MMMT（多模态多轮），覆盖实体识别、管理建议生成和澄清决策。
...
#### MMST 单轮推理任务
- **做什么**：给定用户查询 + 图片 + 元数据，模型需生成结构化响应，包括实体识别（ID）和管理建议（MG）。
...
#### MMMT 多轮决策任务
- **做什么**：在多轮对话中，模型需判断在当前上下文下应澄清（Clarify）还是回答（Respond），并生成对应话语。
...
#### 评估框架
- **做什么**：采用多推理 LLM 集成评判方案，替代单模型单次评估。
