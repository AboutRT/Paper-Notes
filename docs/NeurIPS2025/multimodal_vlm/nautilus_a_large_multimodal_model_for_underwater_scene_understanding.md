---
description: "【论文笔记】Nautilus: A Large Multimodal Model for Underwater Scene Understanding 论文解读 | NeurIPS 2025 | arXiv 2510.27481 | 水下场景理解 | 构建了首个支持八种水下场景理解任务的大型多模态模型 Nautilus，通过物理先验驱动的视觉特征增强（VFE）模块显式修复水下图像退化，提升 LMM 在水下环境中的鲁棒性。"
tags:
  - NeurIPS 2025
  - 水下场景理解
  - 大型多模态模型
...
1. **领域现状**：水下场景理解对海洋探索至关重要，涵盖目标检测、计数、图像描述等多粒度任务。现有水下方法多为单任务设计，通用 LMM 直接应用于水下场景效果差。
2. **现有痛点**：(1) 通用LMM面临空中-水下域偏移问题；(2) 水下光散射和吸收导致严重图像退化；(3) 缺乏大规模水下多任务指令微调数据集。
3. **核心矛盾**：水下场景需要多粒度（图像级、区域级、目标级）的综合理解，但既缺数据又缺针对水下退化的有效处理机制。
4. **本文要解决什么**：构建覆盖八种任务的水下指令微调数据集 NautData，并设计能显式处理水下图像退化的 LMM。
5. **切入角度**：利用水下成像物理模型的先验知识，在特征空间而非像素空间进行图像增强。
6. **核心idea**：基于暗像素先验量化后向散射影响、利用深度信息恢复光吸收衰减，构建即插即用的视觉特征增强模块。
