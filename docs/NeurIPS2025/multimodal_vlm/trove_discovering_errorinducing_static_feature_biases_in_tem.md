---
description: "TRoVe提出自动化方法发现时序视觉语言模型中导致错误的静态特征偏差，通过双分数机制在101个合成模型上比最近基线高28.6%，并在7个真实VLM上揭示未知偏差。"
tags: [VLM, temporal-understanding, bias-discovery, robustness, video-understanding]
---
...
**本文要解决什么？** 自动发现已训练时序VLM中学到的、会导致系统性预测错误的静态特征偏差。

**切入角度** 将问题分解为两个可量化的子问题：(1) 某特征的存在是否导致特定类别错误增加？(2) 模型是否在该特征上学到了偏差？

**核心idea一句话** 将多图像序列分解为单帧→聚类提取候选特征→用ECS+SBS双评分定位导致错误的静态偏差。
