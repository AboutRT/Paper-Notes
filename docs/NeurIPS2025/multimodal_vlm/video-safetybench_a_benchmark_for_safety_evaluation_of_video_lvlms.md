...
1. **现有基准聚焦于静态图像**：FigStep、MM-SafetyBench、HADES、VLSBench 等均只考虑图像-文本输入，忽略了视频的时序动态特性可能引入的独特安全风险（如随时间演化的有害动作）
2. **视频输入扩大了攻击面**：相比单帧图像，视频的连续帧序列在安全对齐上面临更大挑战，攻击者可利用时序信息绕过安全机制
3. **评估指标不足以处理边界情况**：现有自动评判模型对不确定或边界有害输出的判断能力有限，缺乏与人类判断对齐的校准机制
