...
**本文要解决什么？** （1）如何定量度量 VLM 中 SAE 神经元的单义性？（2）哪些 SAE 设计因素对单义性贡献最大？（3）单义性神经元能否用于控制多模态大模型的输出？

**切入角度**：图像不同于文本——单张图像无需上下文即可直接激活神经元。因此可以通过衡量高激活图像之间的语义相似性来评估神经元的单义性。

**核心idea一句话**：提出基于激活加权图像相似度的 MonoSemanticity score，系统评估 VLM 上 SAE 的单义性，并利用单义神经元实现对多模态 LLM 的无监督概念级引导。
