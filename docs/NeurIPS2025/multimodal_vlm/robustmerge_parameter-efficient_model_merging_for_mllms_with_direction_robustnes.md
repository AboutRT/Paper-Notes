...
1. **分布不同**：FFT参数分布窄且集中（均值附近），符号冲突确实是主要问题；而LoRA参数分布明显更宽，B矩阵近似高斯、A矩阵近似均匀分布
2. **奇异值差异悬殊**：LoRA模块中存在显著的头部-尾部奇异值差距，大奇异值对应的方向在合并时天然鲁棒，而小奇异值的方向极易受到干扰
3. **核心矛盾**：对PEFT而言，问题不是符号冲突而是**方向不稳定性**——合并时小奇异值对应的任务特异性知识方向被改变，导致性能崩溃
