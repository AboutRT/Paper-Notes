---
description: "【论文笔记】Valid Inference with Imperfect Synthetic Data 论文解读 | NeurIPS 2025 | arXiv 2508.06635 | synthetic data | 提出基于广义矩估计（GMM）的无超参数框架，将 LLM 生成的不完美合成数据与真实数据结合进行统计有效推断，当合成数据残差与真实数据残差相关时可显著降低估计方差，且在最坏情况下（合成数据完全无信息）也不会损害估计质量。"
tags:
  - NeurIPS 2025
  - synthetic data
...
1. **代理标注（Proxy）**：用模型为无标签数据预测标签/协变量
2. **合成模拟（Synthetic）**：用模型生成全新的合成样本（如模拟问卷调查响应）
