...
description: >-
  [NeurIPS 2025][LLM/NLP][in-context learning] 分析了线性 Transformer 在噪声线性动力系统上的 ICL 近似能力：$O(\log T)$ 深度可达到 $O(\log T / T)$ 测试误差（接近最小二乘估计器）…
tags:
  - "NeurIPS 2025"
  - "LLM/NLP"
  - "in-context learning"
...
1. **深层 Transformer 上界（Theorem 1）**：
    ...
    - 功能：构造 $O(\log T)$ 深的线性 Transformer 实现近似最小二乘预测
...
2. **单层 Transformer 下界（Theorem 2）**：
    ...
    - 功能：证明单层线性注意力对动力系统的测试 loss 有不可消除的正下界
