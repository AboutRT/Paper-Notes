...
1. **LLM 评估日益昂贵**：前沿模型越来越复杂，标注成本高且评估数据可能泄露到训练集，需要持续动态地收集新评估数据
2. **Active testing 的瓶颈**：现有方法需要反复梯度训练代理模型、对所有池化数据做代理/目标模型推理，计算成本限制了其在 LLM 上的应用
3. **核心问题**：如何在保持 active testing 有效性的同时大幅降低计算成本，使其可扩展到 70B 级 LLM？
