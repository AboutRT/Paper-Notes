...
2. **整体意见优化**：整体表达意见 $f(\bm{R}, \bm{s}) = \bm{1}^\top \bm{M} \bm{s} = \sum_v \rho_v s_v$ 是所有 agent 均衡意见之和，其中 $\rho_v$ 为节点 $v$ 的**结构中心性**——衡量节点内部意见对全局意见的影响力
3. **传统方法的瓶颈**：精确计算结构中心性需要矩阵求逆，复杂度 $O(n^3)$，对百万级网络不可行
4. **已有工作的局限**：此前的意见优化主要通过修改抵抗系数、调整表达意见或改变网络结构实现，**通过修改内部意见来最大化整体意见**这一问题在算法设计上尚未得到高效解决
