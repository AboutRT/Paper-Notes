...
1. **块级递归（Inter-chunk Recurrence）**：
    ...
    - 功能：递归计算块级内存状态 $C_k = \bar{g}_k C_{k-1} + (\bar{a}_k \odot K^{(k)})^T V^{(k)}$
...
2. **块内并行与 Tiling（Intra-chunk Parallel）**：
    ...
    - 功能：对块内的矩阵乘法进行融合和 tiling，支持任意大块大小
...
3. **mLSTMsig：sigmoid 输入门变体**：
    ...
    - 功能：将 mLSTM 的指数输入门改为 sigmoid：$C_t = \sigma(\tilde{f}_t) C_{t-1} + \sigma(\tilde{i}_t) k_t v_t^T$
