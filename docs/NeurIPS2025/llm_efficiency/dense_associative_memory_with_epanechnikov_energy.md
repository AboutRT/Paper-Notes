...
description: >-
  [NeurIPS 2025][LLM效率][associative memory] 提出基于 Epanechnikov 核的 log-sum-ReLU（LSR）能量函数替代传统的 log-sum-exp（LSE），在 Dense Associative Memory 中首次实现了"精确记忆所有模式 + 同时涌现新的创造性局部极小"的共存，且保持指数级记忆容量。
tags:
  - NeurIPS 2025
  - LLM效率
  - associative memory
...
**本文目标** 寻找一种能量函数，使 DenseAM 同时具备精确记忆和涌现性。

**切入角度**：能量函数与概率密度函数的对偶关系——$\exp[-E(\mathbf{x})]$ 是核密度估计。在 KDE 理论中，Epanechnikov 核比高斯核有更优的估计效率，对应 $F(x) = \text{ReLU}(1+x)$。

**核心 idea**：用最优 KDE 核（Epanechnikov/ReLU）替代高斯核（exp）作为分离函数，得到同时支持精确记忆和涌现的 LSR 能量。
...
1. **LSR 能量函数定义**:
    ...
    - 功能：定义新的能量函数 $E_\beta^{\text{LSR}}(\mathbf{x}) = -\frac{1}{\beta}\log(\epsilon + \sum_\mu \text{ReLU}(1 - \frac{\beta}{2}\|\mathbf{x} - \boldsymbol{\xi}_\mu\|^2))$
...
2. **精确检索与指数容量（Theorem 1 & 2）**:
    ...
    - 功能：证明在适当 $\beta$ 下，所有原始记忆都可精确检索，且单步梯度下降即可完成
...
3. **全局涌现性（Global Emergence）**:
    ...
    - 功能：定义并证明 LSR 能量具有"全局涌现"——所有原始记忆是局部极小，同时存在新的局部极小（涌现记忆）
