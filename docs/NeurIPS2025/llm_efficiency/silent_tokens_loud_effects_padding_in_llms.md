---
title: >-
  [论文解读] Silent Tokens, Loud Effects: Padding in LLMs
description: >-
  [NeurIPS 2025][LLM效率][Padding Token] 系统性研究了padding token在未被正确掩码时对LLM的影响，发现即使少量padding也会漂移隐层表示、降低生成质量、不可预测地改变偏见，而128个padding token可将Llama-3.1-8B的有害提示攻击成功率从8%飙升到77.5%，本质上实现了jailbreak。
tags:
  - NeurIPS 2025
  - LLM效率
  - Padding Token
...
**本文目标** 从四个维度（激活表示、生成质量、社会偏见、安全性）系统量化padding token对LLM行为的影响。

**切入角度**：控制实验设计——在输入前面prepend不同数量的padding token（0到128个），故意让它们参与计算（模拟常见的mask遗漏错误），观察模型行为变化。

**核心 idea**：Padding不是无害的，它会漂移激活空间、降质量、改偏见、破对齐，本质上是一个被低估的部署鲁棒性风险。
