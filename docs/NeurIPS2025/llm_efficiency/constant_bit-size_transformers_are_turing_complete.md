...
1. **Post Machine 作为中间桥梁**:
    ...
    - 功能：将图灵机先转化为等价的 Post Machine（PM），PM 是配备队列的有限自动机
...
2. **常数精度单层 Transformer 构造**:
    ...
    - 功能：用 s(n) 大小的 context window 作为 PM 的队列，构造一个单层、单头、hardmax attention 的 Transformer
...
3. **队列大小固定化**:
    ...
    - 功能：将 PM 调整为队列大小始终等于 s(n)，确保 context window 大小固定
