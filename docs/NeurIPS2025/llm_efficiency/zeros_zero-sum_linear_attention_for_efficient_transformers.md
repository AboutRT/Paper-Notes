...
description: >-
  [NeurIPS 2025][LLM效率][零和注意力] 通过移除 softmax 的零阶均匀项 $1/t$，构建零和权重的线性注意力机制 ZeroS，突破凸组合只能做加法混合的限制，支持单层内的差分/对比操作，在保持 $O(Nd^2)$ 线性复杂度的同时，在多个序列建模基准上匹配甚至超越标准 softmax 注意力。
tags:
  - NeurIPS 2025
  - LLM效率
  - 零和注意力
...
**本文目标**：(a) 能否允许注意力权重取负值同时保持数值稳定？(b) 能否让线性注意力在单层内就支持对比操作，弥合与 softmax 的性能差距？
...
1. **Softmax 零阶移除与零和权重构建**
    ...
    - 功能：将 softmax 分解为零阶（$1/t$）、一阶偏移（$\delta_{t,i}/t$）和高阶残差（$\varepsilon_{t,i}$），去掉零阶项后用可学习门控混合剩余项
...
2. **径向-角向解耦（Radial-Angular Decoupling）**
    ...
    - 功能：将最终注意力权重分解为径向部分 $r_{t,i}$（来自零和 softmax）和角向部分 $\cos\theta$（query-key 方向相似度）
...
3. **偏差 logits 设计**
    ...
    - 功能：为每个步骤 $i$ 计算一个标量 logit $s_i$（不依赖当前步 $t$）
