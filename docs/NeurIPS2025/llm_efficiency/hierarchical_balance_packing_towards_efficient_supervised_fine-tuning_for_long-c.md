---
title: >-
  [论文解读] Hierarchical Balance Packing: Towards Efficient Supervised Fine-tuning for Long-Context LLM
description: >-
...
**代码**: [https://github.com/ModelTC/HBP](https://github.com/ModelTC/HBP)  
**领域**: LLM效率  
...
**本文目标** (a) 如何自动确定最优的多级打包分组？(b) 如何将样本分配到最优分组并均衡注意力计算？(c) 如何设计动态训练流水线适配多级输入？

**切入角度**：作者提出两个新指标 ABR（注意力均衡比）和 CR（通信比）来量化朴素打包的低效，发现不同长度的最优 SP/GC 策略差异巨大（32K 最优 SP=8，128K 最优 SP=8 但 GC 层数不同），因此应分级处理。

**核心 idea**：将单级打包替换为多级层次打包，每级配置最优 SP/GC 策略，物理隔离长短数据以消除通信浪费和注意力不均衡。
...
1. **层次分组自动选择 (Algorithm 1)**:
    ...
    - 功能：自动确定最优的打包长度集合 $L_p = \{l_1, l_{best}, l_2, l_{max}\}$
...
2. **均衡打包 (Algorithm 2)**:
    ...
    - 功能：将样本分配到各层级分组，确保 ABR、CR、DBR、PR 四个指标同时优化
...
3. **稳定损失归一化器 (Stable Loss)**:
    ...
    - 功能：用全局平均 token 数 $T_{ave}$ 归一化损失，替代 Token-Mean 或 Sample-Mean
