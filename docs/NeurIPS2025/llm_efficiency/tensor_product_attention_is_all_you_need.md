...
## 研究背景与动机
**领域现状**：LLM 长序列推理的瓶颈在于 KV 缓存 $O(T \cdot h \cdot d_h)$ 线性增长，严重限制实际上下文窗口

**现有痛点**：MQA/GQA 通过头共享减少 KV 缓存但灵活性受限；MLA（DeepSeek）用压缩表示但与 RoPE 集成困难，需额外位置编码参数

**核心矛盾**：需要在不损失模型容量的前提下大幅压缩 KV 缓存——现有方法要么牺牲性能，要么引入额外复杂度

**切入角度**：对激活值（而非权重）做动态低秩分解，为每个上下文构建轻量级因子表示

**核心 idea**：$Q_t = \frac{1}{R} A_Q(x_t)^\top B_Q(x_t)$ — 张量积分解让 KV 缓存只需存储低秩因子
