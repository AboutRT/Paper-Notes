...
1. **领域现状**：表格数据是医疗、金融等领域的主导数据格式，梯度提升决策树（GBDT/XGBoost/CatBoost）在表格数据上通常优于深度神经网络，是实际应用的首选。
2. **现有痛点**：(1) 神经网络存在光谱归纳偏置（spectral bias），倾向于拟合平滑的低频函数，难以捕获表格数据中复杂的高频模式；(2) 自监督学习（SSL）在表格域面临数据增强困难——常见增强如噪声注入或特征值交换容易破坏关键特征关系；(3) 掩码自编码器（MAE）在异质表格数据上也有局限。
3. **核心矛盾**：低标签场景下 SSL 意义重大，但表格数据缺乏有效的增强策略，传统增强方法容易生成不真实样本。
4. **本文要解决什么**：在低标签表格数据上学习有效的自监督表征，使下游分类/回归超越传统方法。
5. **切入角度**：从模型增强（model-based augmentation）取代数据增强——利用树模型的归纳偏置引导神经网络学习更好的表征。
6. **核心idea一句话**：将 OSDT 编码器作为训练时的"模型增强器"，通过共享解码器和对齐损失把树模型的表格友好归纳偏置传递给神经网络编码器。
...
2. **Stochastic Gating Network (SGN)** 作为样本级增强:
   - **做什么**：为每个输入样本学习一个特征掩码，实现样本级的特征选择
