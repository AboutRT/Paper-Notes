...
## 研究背景与动机
1. **领域现状**: 大规模基础模型的性能高度依赖训练数据质量。当前数据筛选主要依赖手工启发式规则（如语言检测、标点过滤、n-gram 去重等）和人工调节粗粒度的数据混合比例。
2. **现有痛点**: 手工规则难以捕捉细粒度的数据质量差异；面对合成数据等新来源，人类直觉难以有效判断数据价值；手动调参成本高且不可扩展。
3. **核心矛盾**: 数据规模在爆炸式增长（尤其合成数据），但数据筛选方法仍停留在人工启发式阶段，无法自动化地、端到端地优化"什么数据对训练有用"。
4. **本文要解决什么？**: 如何自动学习每个数据点对模型训练的价值，从而实现细粒度、可扩展的数据筛选。
5. **切入角度**: 将数据筛选建模为双层优化（bilevel optimization）问题，用元梯度直接优化一个评分网络（DataRater），使经过评分过滤后的数据集能最大化地提升模型在验证集上的训练效率。
6. **核心idea一句话**: 用元学习训练一个评分模型，让数据自己"说出"自己的价值。
