---
description: "【论文笔记】Soft Task-Aware Routing of Experts for Equivariant Representation Learning 论文解读 | NeurIPS 2025 | arXiv 2510.27222 | equivariant representation | 提出 STAR（Soft Task-Aware Routing），通过 MoE 路由机制协调不变性和等变性表示学习任务间的共享与专属信息，减少冗余特征学习，提升下游任务迁移性能。"
tags:
  - NeurIPS 2025
...
1. **冗余特征学习**：当前方法（如 EquiMod）使用两个独立的投影头分别处理不变性和等变性目标，隐含假设两个任务独立。但实际上它们是内在相关的——理解语义类别有助于推断光照方向，理解光照也有助于判断语义（"陨石坑错觉"例子：同一月球表面因光照不同看起来像陨石坑或穹顶）
2. **信息浪费**：独立投影头导致两个分支重复捕获共享信息，造成模型容量的低效利用
3. **梯度质量差**：冗余特征学习使投影头收敛慢，导致传递到骨干网络的梯度信号质量不佳
