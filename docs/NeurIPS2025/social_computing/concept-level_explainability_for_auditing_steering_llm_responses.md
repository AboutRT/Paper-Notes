...
**代码**: [https://github.com/k-amara/ConceptX](https://github.com/k-amara/ConceptX)  
**领域**: 社会计算  
...
**本文目标**：如何在概念层面而非 token 层面做归因？如何让归因结果不仅能解释模型行为，还能指导 prompt 编辑来引导 LLM 输出？

**切入角度**：只对输入中语义丰富的"概念"（ConceptNet 中度数高的内容词）做归因，用语义相似度替代 token 重合度作为价值函数，支持灵活的解释目标（如性别偏见、有害性）。

**核心 idea**：用 ConceptNet 知识图谱提取语义概念，通过基于语义相似度的 Shapley 值归因识别驱动 LLM 输出的关键概念，再通过编辑这些概念来引导输出。
