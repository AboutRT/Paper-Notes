---
title: >-
  [论文解读] Auto-Search and Refinement: An Automated Framework for Gender Bias Mitigation in LLMs
description: >-
  [NeurIPS 2025][性别偏见缓解] 提出 FaIRMaker 框架，通过"自动搜索+精化"范式先用梯度优化找到去偏见触发词（Fairwords），再训练 seq2seq 模型将其转化为可读指令，在开源和闭源 LLM 上有效缓解性别偏见同时保持甚至提升任务性能。
tags:
  - NeurIPS 2025
  - 性别偏见缓解
  - 自动提示搜索
...
## 研究背景与动机
**领域现状**：LLM 预训练中编码了社会偏见（尤其是性别偏见），现有缓解方法包括参数修改（微调/模型编辑）和指令引导（手动设计去偏见 preamble）两大类。

**现有痛点**：参数修改方法资源密集、不适用于闭源模型；手动设计的去偏见指令（如 counterfactual preamble CF-D）虽然降低偏见，但会显著损害正常任务性能——因为插入的性别相关描述干扰了模型对原始查询的理解。

**核心矛盾**：自动化梯度搜索方法（如 GCG trigger）可以在更大的搜索空间中找到有效的去偏见 token，但生成的是无意义 token 序列，不可解释且不可迁移到闭源模型。

**本文目标** 设计一个同时满足三个条件的方法：(1) 自动化，(2) 兼容开源和闭源模型，(3) 保持正常任务性能。

**切入角度**：将梯度搜索和手动设计的优势结合——先自动搜索触发词（大搜索空间），再用 seq2seq 模型精化为可读指令（可迁移性）。

**核心 idea**：用 GCG 自动搜索去偏见 trigger token，再训练 refiner 将其转化为自然语言指令。
...
1. **Fairwords 自动搜索与过滤**:
    ...
    - 功能：在偏好数据集上用 GCG 优化器搜索通用触发词，使其附加到性别相关查询后能提升 chosen response 概率并降低 rejected response 概率
...
2. **ChatGPT 辅助精化**:
    ...
    - 功能：用 ChatGPT 进行"逆向推理"，分析 Fairwords 为何有效，将其转化为人类可读的指令
...
3. **Seq2seq Refiner 训练**:
    ...
    - 功能：训练 Llama3.2-3b-instruct 作为 refiner，自动将 Fairwords + query 转化为精化指令
...
- [GG-BBQ: German Gender Bias Benchmark for Question Answering](../../ACL2025/social_computing/gg-bbq_german_gender_bias_benchmark_for_question_answering.md)
