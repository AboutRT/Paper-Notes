...
**代码**: https://github.com/mts-ai/ReplaceMe  
**领域**: 模型压缩  
...
1. **领域现状**：结构化剪枝是 LLM 压缩的主要方法之一，但大多数方法需要后剪枝重训练（"healing"过程），计算开销大
2. **现有痛点**：
...
3. **核心矛盾**：删除层 = 假设层是恒等映射，但实际上层做了非平凡变换
4. **切入角度**：用线性变换**近似**被删除层的变换，比恒等映射精确得多
5. **核心 idea**：选择影响最小的连续 Transformer 块组，用线性变换 $\mathbf{T}$ 近似其输入-输出映射，然后将 $\mathbf{T}$ 融合到相邻 MLP 的 down-projection 权重中，最终模型结构不变、参数不增
...
2. **线性变换估计——L2 目标**：
   - 做什么：求解 $\mathbf{T}^* = \arg\min_\mathbf{T} \|\mathbf{M}_i \cdot \mathbf{T} + \mathbf{Y}_i - \mathbf{L}_{i+n}\|_2^2$
...
3. **线性变换估计——余弦目标**：
   - 做什么：$\mathbf{T}^* = \arg\min_\mathbf{T} \sum_k (1 - \frac{(\mathbf{M}_{i,k} \cdot \mathbf{T} + \mathbf{Y}_{i,k})^\top \mathbf{L}_{i+n,k}}{\|\cdot\| \|\cdot\|})$
...
4. **权重融合**：
   - 做什么：$\mathbf{T}^*$ 与第 i 层 MLP 的 down-projection 矩阵相乘，得到新的 down-projection 矩阵
