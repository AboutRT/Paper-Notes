...
description: >-
  [NeurIPS 2025][模型压缩][LLM安全] 本文系统地研究了 model abliteration（一种推理时激活空间编辑攻击）对不同数据驱动安全预训练阶段的影响，发现仅依赖 refusal 训练的安全机制极易被攻破，而 **组合多种安全信号**（safe-only 过滤 + 改写 + metatag + refusal）可使安全行为分散到更广泛的表征空间、从而更难被单一方向投影移除。
tags:
  - NeurIPS 2025
  - 模型压缩
  - LLM安全
...
**本文目标** 哪些数据驱动的安全训练策略能在 abliteration 攻击下幸存？具体哪些"安全成分"能让安全信号分布得更广、更难被移除？

**切入角度**：利用 Safety Pretraining 项目发布的 SmolLM2-1.7B 的 7 个粒度化检查点（每个检查点隔离一种安全数据策略），构建"原始 vs 被攻击"的配对对比实验。

**核心 idea**：通过检查点级分析证明，组合式数据安全策略（过滤+改写+标签+拒绝）比单一拒绝训练在激活编辑攻击下鲁棒得多。
...
1. **Model Abliteration 攻击**:
    ...
    - 功能：移除模型激活空间中的"拒绝方向"，使模型不再拒绝有害请求
...
2. **粒度化安全预训练检查点**:
    ...
    - 功能：基于 SmolLM2-1.7B，隔离 6 种数据策略的影响
