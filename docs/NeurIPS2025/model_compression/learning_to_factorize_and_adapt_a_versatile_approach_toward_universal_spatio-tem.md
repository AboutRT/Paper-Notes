---
description: "【论文笔记】Learning to Factorize and Adapt: A Versatile Approach Toward Universal Spatio-Temporal Foundation Models 论文解读 | NeurIPS 2025 | arXiv 2601.12083 | 时空基础模型 | 提出 FactoST-v2，一个因式分解的时空基础模型框架，将通用时间预训练与领域特定空间适配解耦，以线性复杂度实现跨领域零样本/少样本/全样本时空预测。"
tags:
  - NeurIPS 2025
  - 时空基础模型
  - 时间序列预测
...
1. **领域现状**：时空（ST）基础模型旨在从多种领域数据中学习通用表示，实现跨数据集泛化预测。现有方法如 UniST、OpenCity 采用联合时空预训练范式。
2. **现有痛点**：联合预训练面临"模式不匹配"挑战——时间动态具有跨域不变性（周期性、趋势等），但空间关联高度依赖特定拓扑（路网 vs 电网），强行联合建模导致二次复杂度 $\mathcal{O}(N^2T^2)$ 和负迁移。
3. **核心矛盾**：空间和时间模式的性质根本不同——时间模式领域不变，空间模式领域特定——但现有方法迫使二者联合学习。
4. **本文要解决什么**：设计一个兼顾泛化性和效率的时空基础模型框架。
5. **切入角度**：提出"模式因式分解假说"——有效的时空泛化需要解耦领域不变的时间动态和领域特定的空间上下文。
6. **核心 idea**：先预训练通用时间 backbone，再用轻量适配器注入空间感知，实现线性复杂度的跨域迁移。
