...
**代码**: https://github.com/SAI-Lab-NYU/QSVD  
**领域**: 多模态VLM / 模型压缩  
...
**核心痛点**：
1. **KV缓存占用巨大**：Multi-Head Attention中Key和Value矩阵随序列长度线性增长，成为推理速度的主要瓶颈
2. **现有SVD压缩方案效率不足**：传统方法对Q、K、V矩阵分别做SVD，产生3组独立的下投影矩阵，参数和计算冗余
3. **量化与SVD的兼容性差**：SVD分解后的中间表示 $C_{qkv}$ 存在严重的通道级异常值，阻碍低精度量化
