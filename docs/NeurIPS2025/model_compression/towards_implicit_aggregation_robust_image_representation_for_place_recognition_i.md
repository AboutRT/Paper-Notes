...
1. **结构冗余**：两阶段过程（特征提取 + 聚合）引入了不必要的结构复杂度。聚合器本身需要大量参数（如 BoQ 需 8.6M 额外参数）。
2. **一次性聚合无法修正**：传统聚合器对 patch 特征做一次性聚合即输出、没有机会进行修正和精化，错失了渐进式优化的可能。
3. **聚合器设计困难**：NetVLAD 丢失位置信息，SALAD 需要 Sinkhorn 迭代，BoQ 引入额外 encoder block 和 cross-attention 层。
