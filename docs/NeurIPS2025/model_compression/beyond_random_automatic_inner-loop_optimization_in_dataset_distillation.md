---
title: >-
  [论文解读] Beyond Random: Automatic Inner-Loop Optimization in Dataset Distillation
description: >-
  [NeurIPS 2025][模型压缩][数据蒸馏] 提出 AT-BPTT（自适应截断 BPTT），将 DNN 训练分为早/中/晚三阶段并自适应调整截断策略和窗口大小，在 CIFAR-10/100/Tiny-ImageNet/ImageNet-1K 上平均提升 3-17%，同时实现 3.9× 加速和 63% 内存节省。
tags:
  - NeurIPS 2025
  - 模型压缩
  - 数据蒸馏
...
**现有痛点**：轨迹匹配方法（MTT/FTD/DATM）需要通过 BPTT 展开 $T$ 步训练过程来优化合成数据。完整展开 $T$ 步的计算和内存开销巨大，因此实践中用**随机截断**（RaT-BPTT）只展开随机选取的 $S$ 步。但随机选择忽略了 DNN 训练不同阶段的学习动态差异。

**核心矛盾**：DNN 训练呈现明确的阶段性——早期学粗粒度特征、中期学判别特征、晚期精细调整。随机截断对所有阶段一视同仁，无法匹配这种非均匀的学习动态。

**本文目标**：设计与 DNN 学习阶段自适应对齐的截断策略，在保持计算效率的同时提升蒸馏质量。

**切入角度**：观察到梯度范数在训练过程中非均匀变化——早期梯度大（学基本特征时）、晚期梯度小（微调时）。可以据此自适应分配截断位置和窗口大小。

**核心 idea**：根据梯度范数自适应选择 BPTT 截断位置（早期选大梯度步、晚期选小梯度步）并动态调整展开窗口宽度，配合低秩 Hessian 近似降低计算成本。
...
1. **三阶段自适应截断策略**:
    ...
    - 功能：将训练分为早/中/晚三阶段，每阶段用不同概率分布选择截断位置
    - 核心思路：
