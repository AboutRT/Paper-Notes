...
**领域现状**：长上下文处理因 self-attention 的二次复杂度面临计算瓶颈。上下文压缩分为：显式方法（直接删除不重要文本）和隐式方法（将上下文编码为紧凑向量/gist token）。

**现有痛点**：(a) **显式方法**（如 LongLLMLingua）保留全局语义但破坏局部连贯性——摘要粒度越细性能越差；(b) **隐式方法**（如 AutoCompressor, ICAE）存在位置偏差——中间和早期信息被遗忘（"lost in the middle"）；(c) 递归压缩（如 Activation Beacon）用固定分段不考虑信息密度差异，且单向压缩导致信息逐步退化。

**核心矛盾**：如何同时保留全局+局部、均匀跨位置的语义信息？

**切入角度**：借鉴认知科学中的层次化信息处理——用树结构在广度（跨上下文覆盖）和深度（细粒度）间取得平衡。

**核心 idea**：信息密度自适应分段 + 二叉语义树层次聚合 + 双向注意力消除位置偏差。
...
2. **语义二叉树构建**:
    ...
    - 功能：将叶 gist token 自底向上聚合为层次化语义表示
