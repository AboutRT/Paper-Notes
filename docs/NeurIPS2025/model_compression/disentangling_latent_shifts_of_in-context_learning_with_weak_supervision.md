...
1. **领域现状**：In-Context Learning (ICL) 让 LLM 通过 prompt 中的少量标注示例实现 few-shot 学习，无需参数更新，是低资源场景下的核心适配机制。
2. **现有痛点**：(a) ICL 对示例的选择和排列高度敏感，导致预测不稳定；(b) 多示例需要长上下文，推理效率低且受上下文窗口限制；(c) 示例数量超过阈值后性能反而下降——ICL 的可扩展性差。
3. **核心矛盾**：现有解耦方法（ICV、Batch-ICL）直接操纵注意力头或隐藏状态来分离示例的影响，但依赖线性注意力近似，忽略了 FFN 层、激活函数、残差连接等关键架构组件。
4. **本文要解决什么？** 如何在不修改模型内部状态的前提下，将 ICL 示例的效果"参数化"为可复用的紧凑表示？
5. **切入角度**：从功能性视角看 ICL——关注模型最终输出而非中间状态。ICL 输出本身就是示例效果的完整体现，可作为弱监督信号。
6. **核心 idea 一句话**：用 ICL 预测作为伪标签训练轻量适配器，将示例的潜在偏移编码为可复用参数。
