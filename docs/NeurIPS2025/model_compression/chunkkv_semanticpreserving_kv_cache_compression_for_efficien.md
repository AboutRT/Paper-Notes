---
title: >-
  [论文解读] ChunkKV: Semantic-Preserving KV Cache Compression for Efficient Long-Context LLM Inference
description: >-
  [NeurIPS 2025][模型压缩][KV cache compression] ChunkKV 将 KV cache 压缩的基本单元从离散 token 提升为语义 chunk（连续 token 组），通过 chunk 级 attention score 聚合来选择保留哪些语义完整的片段，并利用 chunk 带来的高跨层索引相似性实现 layer-wise index reuse，在 10% 压缩率下比 SnapKV/PyramidKV 提升最高 8.7%，吞吐量提升 26.5%。
tags:
  - NeurIPS 2025
  - 模型压缩
...
**领域现状**：长上下文 LLM 推理中，KV cache 消耗高达 70% GPU 内存。以 7B 模型为例，单个 token 的 KV cache 约占 0.5MB，10K token prompt 就需要约 5GB 显存。现有压缩方法（H2O、SnapKV、PyramidKV）基于 token 级 attention score 评估重要性，选择性丢弃低分 token。

**现有痛点**：token 级重要性评估忽略了 token 之间的语义依赖关系。Figure 1 的例子清楚展示了问题——对于 "turaco 吃什么"，token 级方法保留了与问题最相关的个别词（"turaco", "eat", "bamboo"），但丢失了这些词的主语/谓语/宾语上下文，导致语义碎片化。

**核心矛盾**：自然语言中的完整语义通常以连续序列形式出现（主-谓-宾结构、子句、短语），而 token 级压缩打破了这种连续性。

**本文目标**：在 KV cache 压缩中保留完整的语义信息，同时不增加（甚至减少）计算开销。

**切入角度**：将连续 token 分组为 chunk（默认 chunk size=10），以 chunk 为单位计算重要性、保留或丢弃。保留的 chunk 包含完整的主-谓-宾结构，不会出现语义碎片。进一步发现 chunk 级保留的索引跨层相似性远高于 token 级（57.74% vs 27.95% Jaccard similarity），由此自然衍生出 layer-wise index reuse 加速。

**核心 idea**：用语义 chunk 替代离散 token 作为 KV cache 压缩的原子单元，保留完整语义结构并利用跨层索引一致性实现无损加速。
...
2. **Layer-Wise Index Reuse**：
    ...
    - 功能：跨相邻 Transformer 层复用压缩后的 KV cache 索引
