...
1. **领域现状**: 语言模型在复杂推理任务中仍面临挑战。近期研究（如Stream of Search, SoS）表明，训练模型模仿搜索轨迹（包含探索和回溯的完整决策过程）比仅训练最终解答能获得更好的泛化性能。
2. **现有痛点**: SoS训练使用的搜索轨迹通常是噪声的、次优的，导致测试时计算资源被大量浪费在冗余探索和重复回溯上。标准的ReST只是过滤成功轨迹进行微调，没有利用最优解的结构信息。
3. **核心矛盾**: 最优解虽然能提供有价值的引导，但直接用它们做行为克隆泛化能力差；而搜索轨迹虽然泛化性好但效率低。如何兼顾两者优势？
4. **本文要解决什么**: 如何生成高质量的搜索轨迹数据，使模型内化高效的搜索策略，在有限token预算下最大化问题求解准确率。
5. **切入角度**: 借鉴jump-start RL的双策略框架思想，用最优解作为路标指导搜索过程。
6. **核心idea一句话**: 最优解不适合直接模仿，但可以作为step-by-step的路标，渐进式地融入模型的搜索过程以生成高质量轨迹。
...
1. **子目标增强算法 (Subgoal Augmentation)**:
   - **做什么**: 将最优解分解为一系列子目标节点，逐步融入模型自生成的搜索轨迹中
...
2. **操作级强化学习 (Operation-level RL)**:
   - **做什么**: 在Guided-ReST微调基础上使用PPO进一步优化
