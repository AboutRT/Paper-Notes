...
description: >-
  [NeurIPS 2025][优化/理论][联邦学习] 提出 FedLUAR：基于梯度-权重比的层级优先级度量选择低优先级层复用上一轮梯度（而非丢弃），在仅 17% 通信开销下保持与 FedAvg 几乎相同的精度。
tags:
  - "NeurIPS 2025"
  - "优化/理论"
  - "联邦学习"
...
**核心 insight**：梯度绝对值小的层不一定对模型影响小——应关注梯度与权重的**比值**。梯度大但权重也大时，对层输出的影响很有限。
