...
**本文要解决什么？** (1) 能否设计一个通用框架，直接复用已有精确算法来解决噪声问题？(2) 能否在拟阵约束下达到最优 $(1-1/e)$-近似比？(3) 能否首次处理非单调子模函数的噪声最大化？

**切入角度**：先前工作使用*确定性*代理函数来平滑噪声，但确定性代理不能在所有情况下保证对真实函数的良好近似。本文的关键洞察是使用*随机*代理函数——通过随机采样平滑集，在期望意义下实现对真实函数的统一近似。

**核心idea一句话**：构造一个随机代理函数来去噪，然后将任何"鲁棒"的精确值算法作为黑盒在去噪后的代理函数上运行，自动继承原算法的近似保证。
