...
1. **领域现状**：LLM 复杂推理通常在 MDP 框架下进行：定义状态空间和动作空间，用 MCTS 等搜索策略寻找最优推理路径。RAP 用自动生成的子问题作为动作，rStar 手工定义 5 种动作。

2. **现有痛点**：(a) 手工定义的动作空间（如 rStar 的 5 种动作）过于领域特异，缺乏可扩展性；(b) 自动生成的动作空间（如 RAP 的实时子问题）冗余度高，穷举搜索计算量大。

3. **核心矛盾**：好的动作空间需要同时满足两个矛盾属性——**可扩展性**（从数据自动学习，跨领域通用）和**紧凑性**（每步只保留少量高价值候选动作）。

4. **本文要解决什么？** 如何自动构建一个既通用又紧凑的动作空间用于 LLM 推理。

5. **切入角度**：将动作空间构建视为子集选择问题，利用子模函数的递减边际效益性质确保选出的子集兼顾效用和多样性。

6. **核心idea一句话**：用子模函数 + 贪心算法从大型代理动作空间中每步动态选出最优的 5 个动作。
...
2. **子模函数设计**:
   - 做什么：定义评分函数 $F(\mathcal{A}_t; s_t) = \alpha f_\text{util}(\mathcal{A}_t; s_t) + \beta f_\text{div}(\mathcal{A}_t)$
   - 核心思路：
...
3. **嵌入模型训练（Q-learning）**:
   - 做什么：训练嵌入函数 $\mathbf{e}(\cdot)$ 使 $\mathbf{e}(s_t)^T \mathbf{e}(a)$ 近似 Q 值
