---
title: >-
  [论文解读] From Linear to Nonlinear: Provable Weak-to-Strong Generalization through Feature Learning
description: >-
  [NeurIPS 2025][优化][弱到强泛化] 本文首次在非线性特征学习设定（线性 CNN → 两层 ReLU CNN）下严格分析了 weak-to-strong 泛化现象，揭示了数据匮乏和数据丰富两种机制下的不同行为：前者通过良性过拟合实现泛化（或因有害过拟合失败），后者通过早停的标签纠正实现泛化（但过训练会退化）。
tags:
  - NeurIPS 2025
  - 优化
  - 弱到强泛化
...
**代码**: 无  
**领域**: 学习理论 / 超对齐  
**关键词**: 弱到强泛化, 超级对齐, benign overfitting, 特征学习, CNN
...
2. **数据匮乏机制：良性/有害过拟合的临界条件**
    ...
    - 功能：当 $n_{\text{st}}$ 较小时（噪声记忆主导），分析强模型的过拟合行为
...
3. **数据丰富机制：早停的标签纠正**
    ...
    - 功能：当 $n_{\text{st}}$ 充足时（信号学习主导），分析训练早期阶段的泛化行为
...
4. **ReLU 非线性的关键角色**
    ...
    - 功能：利用 ReLU 的正半部分激活特性同时学习困难信号的两种符号
