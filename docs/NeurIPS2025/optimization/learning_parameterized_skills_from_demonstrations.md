---
description: "【论文笔记】Learning Parameterized Skills from Demonstrations 论文解读 | NeurIPS 2025 | arXiv 2510.24095 | 参数化技能 | 提出 DEPS，一种端到端从专家示范中发现参数化技能的算法，通过三层层次策略（离散技能选择→连续参数选择→底层动作）和信息瓶颈设计，学习可解释且可泛化的技能抽象，在LIBERO和MetaWorld上显著优于基线。"
tags:
  - NeurIPS 2025
  - 参数化技能
  - 示范学习
...
1. **领域现状**: 标准RL应用于长时域序列决策问题时未能利用内在行为模式，导致样本效率低。Options框架旨在发现模块化、时间延展的技能，但已有方法要么学习纯离散技能要么学习纯连续技能。
2. **现有痛点**: (i) 离散技能缺乏灵活性，难以泛化到新情境；(ii) 连续技能结构性差、难以解释；(iii) 现有参数化技能方法（如da Silva等）需要标注的任务参数，或依赖VLMs预训练聚类（如LOTUS、EXTRACT），假设相同技能发生在视觉相似的环境中；(iv) 潜变量模型易出现退化——底层策略直接记忆行为，不学习有意义的技能抽象。
3. **核心矛盾**: 如何在不依赖额外标注或预定义技能库的前提下，端到端地从示范中发现既有离散结构又可连续调制的参数化技能？
4. **本文要解决什么**: 从多任务专家示范中自动发现参数化技能，使学到的技能能快速泛化到未见任务。
5. **切入角度**: 将技能建模为参数化轨迹流形，通过极端的状态压缩（压缩到1维）迫使潜变量编码有意义的技能信息。
6. **核心idea一句话**: 通过将观测空间压缩到1维"索引"来创造信息不对称，迫使离散技能和连续参数承载关键的任务语义信息。
...
3. **信息不对称设计**:
   - **做什么**: 高层策略接收完整观测（图像+本体感觉），底层子策略只接收压缩的本体感觉状态
