...
## 研究背景与动机
1. **问题场景**：很多实际任务中精确标签昂贵或不可得——医疗测量费用高、传感器仅记录离散时刻、债券定价仅有买卖价差——但区间上下界往往容易获取。
2. **现有方法局限**：Cheng et al. (2023a) 分析了投影损失方法，但依赖两个强假设：(a) 可实现性（$f^* \in \mathcal{F}$）；(b) ambiguity degree $< 1$（无穷多区间的交集能恢复真实标签）。然而对回归问题，即使 $[y-\epsilon, y+\epsilon]$ 这样简单的区间 ambiguity degree 就等于 1。
3. **核心矛盾**：(a) 之前结论仅为渐进性的，缺乏有限样本保证；(b) 平滑假设类在区间学习中的作用未被充分利用。
4. **切入角度**：平滑（Lipschitz）假设类使得函数值在相近输入处不能差异太大，从而能"去噪"原始区间——将宽区间缩窄为更紧的有效区间（Fig. 2-3）。
