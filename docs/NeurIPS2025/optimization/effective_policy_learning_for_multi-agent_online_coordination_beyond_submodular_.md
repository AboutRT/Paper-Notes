...
description: >-
  [NeurIPS 2025][优化/理论][multi-agent coordination] 提出 MA-SPL 和 MA-MPL 两个多智能体在线协调算法，通过"基于策略的连续扩展"技术突破次模性限制，首次在次模和弱次模目标函数上均实现最优 $(1 - c/e)$ 近似比…
tags:
  - "NeurIPS 2025"
  - "优化/理论"
...
**领域现状**：领域现状**：多智能体在线协调（MA-OC）要求多个分布式智能体在线地协作最大化时变集函数目标（如 UAV 目标追踪、区域监控）。现有方法大多限于次模目标函数。

**现有痛点**：(a) 标准多线性扩展（multilinear extension）只对次模函数保证无损舍入，对非次模函数失效；(b) 弱次模（weakly submodular）和 $\alpha$-弱递减收益（weakly DR）在线场景从未被研究。

**核心矛盾**：如何在去中心化、仅局部反馈的约束下，对超越次模的目标函数实现可证明的近似保证？

**切入角度**：设计"基于策略的连续扩展"——对任意集函数（不限次模）都能保证无损舍入，绕过多线性扩展对次模性的依赖。

**核心 idea**：在连续空间优化策略参数，通过新的连续扩展将离散优化转为连续优化，再通过无损舍入恢复离散解。
