...
1. **领域现状**：联邦学习（FL）允许多客户端协作训练而不共享原始数据。FedAvg 是最流行的算法，但存在根本性的收敛偏差问题。
2. **现有痛点**：(a) 多步本地更新 + 非 i.i.d. 数据导致**客户端漂移**——FedAvg 的不动点偏离真实全局最优解，偏差为 $\|x^o - x^\star\|^2 = \Omega((\tau-1)\eta)\|x^\star\|^2$；(b) 任意客户端参与引入额外目标偏差——全局模型收敛到参与概率加权的扭曲目标的稳定点，而非问题 (1) 的真实最优解；(c) 现有方法需**衰减学习率**来缓解偏差，导致收敛速度变慢。
3. **核心矛盾**：在任意（未知）客户端参与概率下，能否用**常数学习率**实现精确收敛到全局最优解？
4. **本文要解决什么？** 设计一个在任意客户端参与下仍能精确收敛（线性速率）的 FL 算法，且不需要有界异质性假设或学习参与概率。
5. **切入角度**：将 FL 的三个核心操作（客户端参与、本地更新、模型聚合）统一表示为随机矩阵乘法，从去中心化优化视角重新设计 FL 算法。
6. **核心 idea 一句话**：用行随机矩阵建模 pull 操作、列随机矩阵建模 push 操作，通过 push-pull 对偶策略同时消除客户端漂移和参与偏差。
...
3. **FOCUS 算法的 Push-Pull 机制**:
   - 做什么：通过对偶变量跟踪全局梯度，消除本地更新和非均匀参与引入的偏差
   - 核心思路：
