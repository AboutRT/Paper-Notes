...
1. **领域现状**：博弈中在线学习算法的收敛性是博弈论和机器学习的基础问题。经典的 no-regret 算法（如 MWU）保证平均迭代收敛到 Nash 均衡，但实际策略（last-iterate）可能发散、循环或混沌。
2. **现有痛点**：
...
3. **核心矛盾**：average-iterate 收敛仅需 no-regret 性质即可（简单直接），而 last-iterate 收敛要求专门设计——能否弥合这一鸿沟？
4. **切入角度**：效用函数的线性性意味着 $u_i(\cdot, \bar{x}_{-i}) = \frac{1}{t}\sum_{k=1}^t u_i(\cdot, x_{-i}^k)$，因此从观测到的混合策略反馈可以恢复原始策略的反馈
5. **核心idea**：设计一个通用规约 A2L，让每个玩家输出内部策略的运行均值作为实际策略，通过线性性恢复内部策略的效用反馈，从而将 last-iterate 问题转化为 average-iterate 问题
...
2. **A2L-OMWU (梯度反馈)**:
   - 做什么：将 A2L 应用于 OMWU 算法实现最优 last-iterate 收敛
...
3. **A2L-OMWU-Bandit (bandit 反馈, Algorithm 2)**:
   - 做什么：在仅观测单动作收益（非全效用向量）时实现 last-iterate 收敛
