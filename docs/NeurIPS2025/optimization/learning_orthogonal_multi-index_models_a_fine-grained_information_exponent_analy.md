---
title: >-
  [论文解读] Learning Orthogonal Multi-Index Models: A Fine-Grained Information Exponent Analysis
description: >-
  [NeurIPS 2025][优化/理论][多索引模型] 证明正交多索引模型 $f_*(\mathbf{x}) = \sum_{k=1}^P \phi(\mathbf{v}_k^* \cdot \mathbf{x})$ 可通过两阶段在线 SGD 以 $\tilde{O}(dP^{L-1})$ 样本复杂度学习（…
tags:
  - "NeurIPS 2025"
  - "优化/理论"
  - "多索引模型"
...
**本文目标** 严格证明上述直觉：联合利用不同阶 Hermite 分量的两阶段 SGD 可以实现 $\tilde{O}(dP^{L-1})$ 样本复杂度。

**切入角度**：将学习过程分为子空间搜索阶段（由 2 阶项驱动）和方向恢复阶段（由 $L$ 阶项驱动），分析每阶段梯度流的动力学。

**核心 idea**：先用 2 阶 Hermite 项 $O(d)$ 成本恢复子空间，再在低维子空间中用 $L$ 阶项 $O(P^{L-1})$ 成本恢复方向 = $\tilde{O}(dP^{L-1})$ 总复杂度。
...
1. **阶段 1：子空间恢复（2 阶项驱动）**:
    ...
    - 功能：用 $\tilde{O}(dP)$ 样本将网络的 $P$ 个神经元方向与目标子空间 $\text{span}\{\mathbf{v}_k^*\}$ 对齐
...
2. **阶段 2：方向恢复（$L$ 阶项驱动）**:
    ...
    - 功能：在已恢复的 $P$ 维子空间内用 $\tilde{O}(P^{L-1})$ 样本恢复每个方向 $\mathbf{v}_k^*$
