...
1. **缺少长程规划能力**：现有VLA模型如OpenVLA、TraceVLA虽在短程技能上表现良好，但面对需要多步推理的长程操作任务时力不从心，因为它们没有中间推理步骤来分解复杂目标。

2. **CoT监督数据昂贵且易过拟合**：ECoT和RAD等方法尝试引入链式思维(CoT)推理，但依赖离线MLLM生成的CoT标注数据进行有监督微调。高质量推理轨迹的生成成本高昂，且模型容易过拟合到特定的视觉场景或推理模式。

3. **现有RL推理缺乏动作对齐**：Video-R1等工作将RL应用于VLM推理，但使用的是QA式准确率奖励，这种奖励信号无法支撑长程规划，也难以在推理和真实动作执行之间建立联系。
