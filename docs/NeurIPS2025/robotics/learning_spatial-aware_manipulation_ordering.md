...
1. **启发式流水线**（如检测后手工优化）：泛化能力差，当空间关系在不同场景间变化时容易崩溃
2. **两阶段框架**（先检测后用 VLM 推理顺序）：推理延迟高（通常数秒），无法满足实时部署要求
3. **VLM 直接推理**：即使给予特权信息（物体真值位姿），GPT-4o 在困难场景中成功率仅 71.4%，Gemini-2.5 仅 78.5%
