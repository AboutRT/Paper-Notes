...
1. **时间尺度过短**：LIBERO-Long和RoboCasa等"长程"基准通常只有2-5个子任务，不超过500步动作，难以测试记忆维持、时间抽象等真正的长程推理需求。

2. **缺乏动态场景和记忆需求**：真实世界中物体会被移动、遮挡、状态改变，机器人需要记住之前探索的柜子里有什么、在哪里放过东西。现有基准几乎没有这类设计。

3. **评估维度单一**：大多基准仅用二值任务成功率评估，无法区分规划能力、感知判断、记忆利用等具体认知维度的表现。
