...
**核心矛盾**：LLM灵活但不鲁棒——即使是o3-mini这样的大reasoning模型在复杂ToM中也会犯系统性错误（长上下文遗忘、递归推理崩溃）；BIP鲁棒但不灵活——需要人工为每个领域设计agent模型。**核心idea一句话**：让LLM自动发现合适的agent模型结构，然后在该模型上做自动化的贝叶斯推理——模型发现的灵活性+贝叶斯推理的鲁棒性=可扩展的开放式机器ToM。
