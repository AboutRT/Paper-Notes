...
1. **最优提示的理论条件**：在什么条件下，存在一个提示使得被提示的预训练预测器在目标任务上达到（接近）贝叶斯最优？
2. **软提示 vs 硬提示**：为什么软前缀（实数向量序列）比硬token序列更有效？其机制是什么？
3. **提示调优的根本局限**：在什么情况下提示调优不可能成功，需要权重调优？
