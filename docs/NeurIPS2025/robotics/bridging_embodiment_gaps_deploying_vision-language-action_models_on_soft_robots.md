...
description: >-
  [NeurIPS 2025][机器人][软体机器人] 本文首次系统地将 VLA 模型部署到软体连续体机械臂上，提出了结构化微调与部署流水线，在自定义软体机器人 Embuddy 上评估了 OpenVLA-OFT 和 π₀ 两个 SOTA VLA 模型，发现开箱即用的策略因实体差异完全失败，但通过针对性微调可使软体机器人在操纵任务上达到与刚性 UR5 机械臂相当的成功率，证明了 VLA 与软体机器人结合实现安全人机交互的可行性。
tags:
  - NeurIPS 2025
  - 机器人
  - 软体机器人
...
**本文要解决什么？** 本文聚焦于三个核心问题：（1）VLA 策略在没有微调的情况下能否直接迁移到软体机器人？（2）如果不能，通过什么样的微调流水线可以弥合这一实体差距？（3）不同架构的 VLA 模型（OpenVLA-OFT vs. π₀）在软体平台上的表现对比如何？
...
**核心idea一句话**：通过构建首个软体机器人示范数据集和结构化微调流水线，证明 VLA 模型可以在少量数据微调后成功部署到软体连续体机械臂上，在操纵任务中达到与刚性机器人相当的性能，同时保留软体平台的内在安全特性。
...
1. **软体连续体机器人平台 Embuddy**:
    ...
    - 做什么：作为实验的核心物理平台，Embuddy 是一个自定义设计的连续体机器人臂，用于验证 VLA 在柔顺平台上的可行性。
...
2. **基于分段常曲率模型的遥操作与逆运动学**:
    ...
    - 做什么：将人类操作者的笛卡尔空间指令映射到软体机器人的腱长度控制信号，实现流畅的遥操作数据采集。
...
3. **面向两种 VLA 架构的差异化微调策略**:
    ...
    - 做什么：分别为 OpenVLA-OFT 和 π₀ 设计适配其架构特点的微调方案，在计算效率和精度之间取得平衡。
    - 核心思路：
