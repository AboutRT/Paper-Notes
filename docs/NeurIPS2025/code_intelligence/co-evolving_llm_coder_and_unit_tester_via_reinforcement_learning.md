...
description: >-
  [NeurIPS 2025][自演化] 提出CURE框架，通过单元测试生成器与代码生成器的相互监督和共同进化，在无需ground-truth代码的情况下显著提升LLM代码生成能力。
tags:
  - NeurIPS 2025
  - 自演化
  - RL
...
**本文要解决什么？** 这篇论文要回答一个核心问题：**单元测试生成器和代码生成器能否在完全没有 ground-truth 代码的情况下有效共同进化？** 具体分解为三个子问题：（1）如何设计无监督的奖励信号让两者互相学习？（2）如何保证奖励的精确度，避免生成平凡或错误的单测？（3）如何在 long-CoT 模型上应用这一框架并保证推理效率？
...
**核心idea一句话**：用代码和单测之间的交叉执行矩阵构建自博弈奖励，通过理论推导的奖励精度目标 $\mu$ 来指导单测生成器的优化方向，实现无监督的 coder-tester 共同进化。
...
1. **奖励精度（Reward Precision）的理论推导**:
    ...
    - 做什么：定义并分析"奖励精度"——即一组生成的单测能多准确地区分正确代码和错误代码——作为优化单测生成器的理论目标。
...
2. **基于执行矩阵的个体奖励估计**:
    ...
    - 做什么：从执行矩阵中为每个生成的单元测试估计其个体贡献的奖励值 $\mathcal{R}_{u_k}^{\star}$。
...
3. **协同进化的策略优化**:
    ...
    - 做什么：使用 GRPO 风格的策略梯度方法交替优化代码生成和单测生成两个目标。
...
4. **长链推理模型的效率优化**:
    ...
    - 做什么：为 long-CoT 推理模型设计响应长度引导的奖励变换，在不损失单测质量的前提下大幅缩短生成长度。
