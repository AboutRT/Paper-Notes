...
**本文目标**：提供一个靶向诊断工具，系统评估 MLLM 的尺度不变性识别、几何变换推断、递归结构抽象、组合推理和分支递归等层级能力。

**切入角度**：分形是理想测试对象——由 IFS（迭代函数系统）中少数几个压缩映射通过简单递归生成极其复杂的自相似模式，要求模型同时具备视觉感知和数学抽象能力。

**核心 idea**：用分形程序合成作为诊断探针，测试 MLLM 能否从图像推断递归生成规则，而非记忆模式。
...
2. **MinimalTurtle 受限接口**:
    ...
    - 功能：限制模型只能使用 4 条命令（move, turn, pen up/down）
