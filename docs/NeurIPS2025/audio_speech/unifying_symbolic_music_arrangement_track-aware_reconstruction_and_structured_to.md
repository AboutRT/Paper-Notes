---
description: "【论文笔记】Unifying Symbolic Music Arrangement: Track-Aware Reconstruction and Structured Tokenization 论文解读 | NeurIPS 2025 | arXiv 2408.15176 | 音乐编排 | 提出一个统一的符号音乐编排框架，通过段级自监督重建目标（解耦内容和乐器风格）和新的多轨token化方案REMI-z，使单个预训练模型能够处理乐队编排、钢琴缩编和鼓编排等多种编排任务，并在三个典型任务上超越了任务特定的SOTA。"
tags:
  - NeurIPS 2025
  - 音乐编排
  - 符号音乐
...
1. **缺乏跨任务通用性**：每种编排任务使用专用架构和训练方案，无法共享音乐知识
2. **数据瓶颈**：端到端方法依赖平行数据集（同一作品的不同编排版本），这类数据极度稀缺
3. **token化的局限**：现有REMI+等严格时序化方案将不同乐器的音符交错排列，导致内容碎片化，难以实现乐器级别的控制
