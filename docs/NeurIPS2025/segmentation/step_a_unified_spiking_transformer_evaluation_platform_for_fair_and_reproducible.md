...
1. **实现不标准化**：各模型使用不同的框架（SpikingJelly、BrainCog、BrainPy 等），超参数调优和复现困难
2. **组件级分析缺乏**：STs 由多个相互作用的组件（脉冲编码器、神经元模型、代理梯度、注意力模块、MLP 头）组成，但各模块的贡献尚未被系统探索
3. **能效评估不公平**：SNN 与量化 Transformer 的直接能效比较稀缺，现有能耗模型忽略了位宽稀疏性和内存访问成本
4. **缺乏统一平台**：没有一个平台可以在分类、分割、检测等任务上统一评估 STs
