...
1. **领域现状**：3D人体重建近年快速发展——3DGS实现实时渲染，大规模重建Transformer（LRM、GRM）实现前馈泛化。但3D人体语义分割（身体部位分割）仍是空白。
2. **现有痛点**：
...
3. **核心idea**：构建统一框架让3D重建和3D语义分割相互增益——共享3D Gaussian参数，重建任务提供几何约束，分割任务提供语义正则化。
