---
description: "【论文笔记】Re-coding for Uncertainties: Edge-awareness Semantic Concordance for Resilient Event-RGB Segmentation 论文解读 | NeurIPS 2025 | arXiv 2511.08269 | Event-RGB融合 | 提出 Edge-awareness Semantic Concordance（ESC）框架，利用语义边缘作为异质 Event 和 RGB 模态的中间桥梁，通过边缘字典的离散潜空间建模实现跨模态特征对齐和不确定性优化，在极端条件下超越 SOTA 2.55% mIoU。"
tags:
  - NeurIPS 2025
  - Event-RGB融合
  - 语义边缘
...
1. **领域现状**：极端条件下（低光照、剧烈运动）RGB 信息严重损失，Event 相机以高动态范围和高时间分辨率提供互补信息。
2. **现有痛点**：Event 和 RGB 天然异质——特征级不匹配和劣化优化问题严重。现有多模态方法采用朴素融合策略，无法处理模态失衡和模态失败情况。
3. **核心矛盾**：如何在两个根本不同的模态之间建立统一的表示空间。
4. **切入角度**：发现 Event 数据天然聚集在语义边缘区域（统计验证），而 RGB 梯度也揭示边缘线索——语义边缘是两者共同的中间表示。
5. **核心 idea**：用 VQ-VAE 建立边缘字典（离散潜空间），通过 re-coding 实现双向特征转换和分布对齐。
