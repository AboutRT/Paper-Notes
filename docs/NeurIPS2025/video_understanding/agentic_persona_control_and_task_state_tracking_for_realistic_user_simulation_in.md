...
description: >-
  [NeurIPS 2025 (Workshop on SEA)][视频理解][多agent框架] 提出三 agent 协作框架用于逼真的用户模拟——User Agent（协调）+ State Tracking Agent（结构化任务状态）+ Message Attributes Generation Agent（基于 persona 和状态的行为属性控制），在餐厅点餐场景中综合仿真质量（CRRS）提升 102.6%，persona 保持度 +19.9%，行为自然度 +284.5%，且核心发现：无状态感知的行为控制导致 BVS=0（完全刚性）。
tags:
...
**领域现状**：对话式 AI 需要大规模测试，LLM 驱动的用户模拟是替代人工测试的方案，但单模型方法难以平衡 persona 一致性、任务准确性和行为自然度。

**现有痛点**：（a）单 LLM 同时承担状态追踪、行为建模和回复生成，职责过载导致 persona 漂移；（b）静态测试集无法捕捉多轮对话的动态性；（c）缺乏可解释的决策追踪。

**核心矛盾**：模拟需同时满足三个相互制约的目标——严格 persona（一致性）、准确任务完成（状态追踪）、自然的行为变化（多样性）。

**本文目标** 通过分解职责到专用 agent，让每个 agent 负责一个核心维度，协同实现高质量用户模拟。

**切入角度**：类比人类认知——工作记忆（State Tracking 追踪进度）、行为规划（MAG 根据 persona 和状态决定how）、语言生成（User Agent 综合产出回复）。

**核心 idea**：三个专用 agent 分别负责"进度追踪/行为决策/回复生成"，通过结构化协议协同实现可解释、可控、逼真的用户模拟。
