...
**本文要解决什么**：(1) 量化背景偏差在三类模型范式中的严重程度；(2) 为分类模型设计保留背景上下文的偏差缓解架构；(3) 为VLLM探索基于prompt的轻量缓解方案。

**切入角度**：从"人体分割"和"prompt设计"两个正交维度入手，分别针对分类模型和VLLM提出适配的缓解策略。

**核心idea一句话**：通过双分支架构融合原始视频与分割人体视频来平衡上下文利用与偏差抑制，同时通过自动化prompt tuning引导VLLM聚焦人体动作。
