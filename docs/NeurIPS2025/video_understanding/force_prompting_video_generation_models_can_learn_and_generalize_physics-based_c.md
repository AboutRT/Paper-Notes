...
1. **领域现状**：视频生成模型（Sora/CogVideoX/Wan2.1）视觉质量和运动真实性进步巨大，但主要依赖文本和图像输入，缺乏物理交互精确控制。可控方向主要是相机控制和轨迹控制。
2. **现有痛点**：(a) 轨迹控制需预先指定每帧像素位置，无法处理全局现象（风/流体）；(b) 轨迹与力是不同物理量——相同力对不同质量物体产生不同位移；(c) 物理模拟器方法需要3D几何或推理时运行模拟器。
3. **核心矛盾**：获取高质量力-视频配对训练数据极其困难。
4. **本文要解决什么？** 用极少量合成物理模拟数据教会预训练视频生成模型理解力控制信号。
5. **切入角度**：假设SOTA视频模型已编码了关于视觉动力学的强先验，合成数据只需"激发"而非"教会"模型。
6. **核心idea一句话**：视频生成模型可以从极少量合成数据学习以物理力为条件的生成，并展现跨物体/材质/几何的惊人泛化。
