---
description: "【论文笔记】Open-World Drone Active Tracking with Goal-Centered Rewards 论文解读 | NeurIPS 2025 | arXiv 2412.00744 | Drone Active Tracking | 提出首个开放世界无人机主动跟踪基准 DAT（24 个城市级场景、高保真动力学仿真），以及基于目标中心奖励函数和课程学习的强化学习跟踪方法 GC-VAT，在仿真器上达到约 72% 的跟踪成功率。"
tags:
  - NeurIPS 2025
...
1. **领域现状**: 视觉主动跟踪（VAT）旨在通过控制运动系统自主跟随目标，广泛应用于无人机监控和安防。基于强化学习的 VAT 方法将视觉跟踪和控制集成在统一框架中，避免了流水线方法的手动标注和额外调优。
...
3. **核心矛盾**: 真实无人机跟踪需要在复杂开放世界中运作，但现有仿真环境和奖励设计均无法满足此需求——距离度量的奖励在俯视视角下会产生误导。

4. **本文要解决什么**: (1) 构建一个真实、全面的无人机主动跟踪基准；(2) 设计适用于非固定视角的奖励函数和高效训练策略。

5. **切入角度**: 从射影几何出发，设计基于偏差度量（deviation metric）的奖励函数来替代欧氏距离奖励，并结合课程学习。

6. **核心 idea 一句话**: 用图像中心投影的归一化偏差度量替代欧氏距离，使奖励在任意视角下都能正确反映目标位置。
