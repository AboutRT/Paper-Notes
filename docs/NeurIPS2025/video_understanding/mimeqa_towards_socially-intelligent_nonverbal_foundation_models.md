...
1. **领域现状**: 社交智能AI日益重要，当前研究主要集中在纯语言数据和任务（如社交对话、问答），或以语言为主导的多模态数据。VideoLLMs在Ego4D、Video-MME等基准上表现优异。
2. **现有痛点**: (i) 现有基准严重依赖语言模态，非语言信号（肢体语言、手势、表情）被当作次要信息；(ii) 这导致模型在语言理解上进步但在非语言社交理解上严重不足；(iii) 部分基准甚至在无视频输入时也能取得不错成绩，说明存在严重的语言偏差。
3. **核心矛盾**: 真正的社交智能需要同时理解语言和非语言信号，但当前模型和评估体系都过度偏向语言，缺乏对非语言社交认知的有效测量手段。
4. **本文要解决什么**: 如何系统评估AI模型的非语言社交推理能力？
5. **切入角度**: 利用哑剧视频——这种完全不依赖语言，纯粹通过手势和动作传达信息的艺术形式——作为评估载体。
6. **核心idea一句话**: 哑剧表演完全排除了语言线索，迫使AI模型纯粹依靠对人体动作、手势和社交互动的理解来回答问题。
...
3. **评估设计**:
   - **做什么**: 使用GPT-4o作为LLM-as-a-judge自动评估开放式回答
