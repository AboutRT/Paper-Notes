...
1. **数据稀疏**：用户在异地通常只有极少甚至没有历史签到记录，传统推荐模型无法有效学习
2. **偏好二元性**：用户偏好包含稳定的静态兴趣（长期行为模式）和随情境变化的动态偏好（时间、位置敏感），现有方法将二者混为一谈
3. **偏好转移**：家乡签到偏好与异地行为可能存在漂移（interest drift），如何有效迁移家乡知识是关键
