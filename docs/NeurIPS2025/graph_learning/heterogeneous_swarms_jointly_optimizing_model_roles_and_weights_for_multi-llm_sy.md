...
1. **Fixed-weight系统**（如LLM辩论/多智能体）：使用静态黑盒LLM，通过文本交互上下文化角色，模型本身无法针对任务适配，权重不变导致灵活性不足。
2. **Fixed-role系统**（如AgentVerse/MetaGPT）：通过手工设计的prompt赋予LLM固定角色，扩展到新任务/领域时需要大量prompt工程，且角色分配依赖先验知识。
