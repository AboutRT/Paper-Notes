...
1. **标注谱数据稀缺**：高质量的实验质谱-分子配对数据获取成本高昂，限制了数据驱动模型的训练。

2. **质谱信号复杂多变**：同一分子在不同碰撞能量、加合物类型或仪器设置下的谱图差异巨大（如 Figure 1 所示），甚至相同条件下也有波动。这使得直接在原始谱上预训练非常困难。
...
1. **统一词表（Unified Vocabulary）**
   ...
   **做什么**：将质谱（以指纹表示）和分子结构统一到同一 token 空间。
