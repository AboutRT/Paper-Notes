---
description: "【论文笔记】From Objects to Anywhere: A Holistic Benchmark for Multi-level Visual Grounding in 3D Scenes 论文解读 | NeurIPS 2025 | arXiv 2506.04897 | 3D视觉定位 | 提出 Anywhere3D-Bench，首个涵盖区域/空间/物体/部件四个层级的 3D 视觉定位基准，揭示即使最强的 Gemini-2.5-Pro 和 o3 在空间级任务上仅达约 30% 准确率、部件级约 40%，远低于人类的 95%。"
tags:
  - NeurIPS 2025
  - 3D视觉定位
  - 多层级定位
...
1. **领域现状**：3D 视觉定位（visual grounding）已在物体级别取得显著进展，现有基准（ScanRefer、Nr3D、Sr3D）和方法主要关注将语言表达对齐到场景中的物体。
2. **现有痛点**：(a) 现有基准仅限于物体级别，忽略了物体之外的空间区域（如"把台灯放在扶手椅旁边"需要推理未被占据的空间）；(b) 对物体部件级（拉开抽屉）和活动区域级（协同学习区）的定位未被评估；(c) 现有基准很少测试模型对定量尺寸和距离的推理能力。
3. **核心矛盾**：人类能自然地在 3D 场景中"任意位置"进行视觉定位（估计尺寸、空间关系、物体部件），但当前模型和基准都局限于物体级语义。
4. **本文要解决什么**：构建一个全面评估模型在四个层级（area/space/object/part）3D 视觉定位能力的基准。
5. **切入角度**：设计分层的表达类型（活动→空间→物体→部件），每个层级包含多种子类型来考察不同能力。
6. **核心idea一句话**：通过多层级多类型的定位基准，系统暴露当前 3D 视觉定位模型在空间推理和细粒度感知上的根本不足。
