...
**本文要解决什么？** 论文将问题分解为三个层次：（1）如何刻画和理解稀疏视角 3DGS 中高斯体之间的协同适应现象？（2）如何定量度量这种协同适应的严重程度？（3）如何设计轻量级策略来有效缓解这种协同适应，从而消除外观伪影、提升新视角渲染质量？
...
**核心 idea 一句话**：将神经网络中关于协同适应的理论洞察迁移到 3DGS 场景表征中，通过随机丢弃高斯体（Dropout）或注入不透明度噪声来强制每个高斯体独立编码正确的场景外观，从而消除稀疏视角下的外观伪影。
...
1. **Co-Adaptation Score（CA）度量指标**:
    ...
    - 做什么：定量衡量一组优化好的高斯体在某个视角下的协同适应严重程度，输出一个标量值，CA 值越高表示高斯体之间的纠缠越严重。
...
2. **Random Gaussian Dropout 正则化**:
    ...
    - 做什么：在 3DGS 训练的每一轮迭代中，以概率 $p$ 随机丢弃一部分高斯体，仅用剩余的子集进行渲染和梯度回传，测试时使用所有高斯体但对不透明度进行缩放补偿。
...
3. **Multiplicative Opacity Noise 注入**:
    ...
    - 做什么：在训练过程中对每个高斯体的不透明度参数注入乘性高斯噪声，轻微扰动每个高斯体对最终像素颜色的贡献权重。
