...
**本文目标**：如何在不使用任何标注的前提下，实现快速（秒级）、可扩展且可泛化的 4D 场景重建？

**切入角度**：跨大量场景训练让网络自动学会静/动分解（数据驱动先验）；结合 LiDAR 处理高分辨率（≥1080p）密集多视角（≥60 帧）输入。

**核心 idea**：极简设计（仅光度损失 + 静态偏好正则），通过跨场景学习让前馈网络自动实现静动态分解和 4D 重建。
...
1. **LiDAR 引导的场景初始化**:
    ...
    - 功能：从 LiDAR 点云初始化 3D 高斯的位置、尺度和颜色
...
2. **前馈流预测网络**:
    ...
    - 功能：用 3D 稀疏卷积 U-Net 从初始高斯预测精细化属性和运动速度
...
4. **迭代精细化与运动增强（Flux4D-full）**:
    ...
    - 功能：通过 3D 梯度反馈迭代精细化外观，用多项式运动模型替代线性模型
