...
1. **领域现状**：点云补全从残缺点云恢复完整形状，对机器人、自动驾驶至关重要。现有方法（PCN、SeedFormer、PointAttn、ProxyFormer）在训练分布上表现好，但对新的缺失模式和传感器噪声泛化差
2. **现有痛点**：
...
3. **核心矛盾**：补全需要同时利用先验（知道"通常长什么样"）和输入特异性（当前观测到了什么）
4. **切入角度**：测试时适应（TTA）——将每个点云视为独立"域"，用自监督信号在线更新编码器
5. **核心 idea**：用 MAML 元训练确保辅助任务的梯度方向与主任务对齐（解决传统 TTA 中辅助-主任务不对齐的问题），测试时仅优化辅助目标但保证改善补全质量
