...
1. **空间不规则性**：全球气象站分布极不均匀，大多数集中在北美和西欧，不同经纬度密度差异巨大。现有 GNN 方法在同时学习高密度和低密度区域时面临困难。

2. **动态分布**：气象站的数量和位置随时间变化（新建/废弃），使用固定站点集训练的模型容易过拟合，无法对未见位置进行预测。
