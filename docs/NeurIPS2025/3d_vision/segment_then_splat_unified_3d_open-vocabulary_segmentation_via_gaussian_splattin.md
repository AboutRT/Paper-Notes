---
description: "【论文笔记】Segment then Splat: Unified 3D Open-Vocabulary Segmentation via Gaussian Splatting 论文解读 | NeurIPS 2025 | arXiv 2503.22204 | 3D高斯溅射 | 提出\"先分割再重建\"的新范式，在3D高斯溅射重建之前就将高斯分配到不同目标集合，从而消除几何和语义歧义，实现静态和动态场景的统一3D开放词汇分割。"
tags:
  - NeurIPS 2025
  - 3D高斯溅射
  - 开放词汇分割
...
1. **2D像素级分割的局限性**：大多数方法（如LangSplat、LEGaussians）在3DGS旁学习一个语言场，将语言嵌入渲染成2D特征图再做查询。这本质上是2D分割，不同视角下分割结果不一致，无法获取真正的3D目标信息。

2. **几何-语义歧义**：在"先重建再分割"范式下，每个高斯可能编码来自多个目标的几何和语义信息，导致目标边界模糊。

3. **动态场景不可用**：在动态场景中，同一个高斯在不同时间步可能代表不同目标，导致高斯-目标错位（misalignment），现有方法无法直接处理。
