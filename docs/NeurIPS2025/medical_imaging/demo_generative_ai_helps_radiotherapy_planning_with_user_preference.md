...
description: >-
  [NeurIPS 2025 (GenAI for Health Workshop)][医学图像][放疗规划] 提出 Flexible Dose Proposer (FDP)，通过两阶段训练框架（VQ-VAE 预训练 + 多条件编码）实现基于滑块的用户偏好交互式 3D 剂量分布预测…
tags:
...
**核心 idea**：用 VQ-VAE 预训练稳定剂量解码器 + 用户偏好编码作为条件输入，实现可交互的个性化剂量预测
