...
description: >-
  [NeurIPS 2025][医学图像][肽段测序] CrossNovo 融合自回归（AR）和非自回归（NAR）解码器，通过共享谱编码器 + 重要性退火 + 梯度阻断知识蒸馏，让 NAR 的双向全局理解增强 AR 的序列生成能力…
tags:
  - "NeurIPS 2025"
  - "医学图像"
  - "肽段测序"
...
**本文目标** 在保持 AR 生成质量的前提下，引入 NAR 的双向表征来增强 AR 的全局理解能力。

**切入角度**：共享编码器让两个解码器学到互补的表征→NAR 表征作为额外上下文蒸馏给 AR→梯度阻断防止 AR 的损失破坏 NAR 特征。

**核心 idea**：NAR 解码器提供双向全局表征，通过梯度阻断的跨解码器注意力蒸馏给 AR 解码器，结合共享质谱编码器实现 hybrid 序列生成。
...
2. **重要性退火多任务训练（Stage 1）**:
    ...
    - 功能：联合训练 AR（CE 损失）和 NAR（CTC 损失），动态调整权重
...
- [\[ICML 2025\] Improved Off-policy Reinforcement Learning in Biological Sequence Design](../../ICML2025/medical_imaging/improved_off-policy_reinforcement_learning_in_biological_sequence_design.md)
