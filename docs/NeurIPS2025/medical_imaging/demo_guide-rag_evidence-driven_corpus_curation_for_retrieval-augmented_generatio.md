...
1. **领域现状**：AI 聊天机器人在临床医学中快速普及，但对于复杂、异质、证据尚不充分的新兴疾病（如长新冠），设计有效的临床辅助 AI 面临重大挑战
2. **现有痛点**：
...
3. **核心矛盾**：在证据快速演变、缺乏共识的新兴疾病场景下，检索范围过窄（miss evolving evidence）和过宽（引入噪声）之间存在根本张力
4. **切入角度**：以长新冠为案例，系统比较不同语料库策展策略对 RAG 性能的影响
5. **核心idea一句话**：将临床指南与少量高质量系统综述结合（GS-4），可以在窄共识文档和无过滤初级文献之间取得最佳平衡
