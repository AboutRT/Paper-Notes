...
1. **分子对接的局限性**：传统方法依赖分子对接（docking）评估结合亲和力，但对接计算开销大，且评分函数基于线性能量组合，与实验结合亲和力的相关性差
2. **现有生成方法的不足**：已有框架如DrugMetric、NP-VAE等虽能生成高新颖性分子，但常牺牲对接保真度或药效团一致性。大多数方法仅优化对接分数或依赖特定结合位点
3. **药效团方法的独特优势**：药效团关注关键相互作用特征（如氢键供体/受体、芳香基团、疏水基团）的空间排列，提供了更可解释、更鲁棒的生物活性代理指标，可跨越不同化学骨架
