...
**代码**: https://github.com/pth1993/FairTTE (有)  
**领域**: 医学影像 / AI公平性  
...
1. **数据稀缺**：缺乏同时包含医学图像、时间-事件结局和敏感属性的公开数据集
2. **偏差机制不明**：不了解医学影像中的偏差如何具体影响TTE预测的公平性
3. **缺乏统一指标**：TTE预测没有被广泛接受的公平性度量标准
4. **删失问题**：TTE数据中广泛存在的右删失(right-censoring)使标准评估指标不适用
