...
3. **核心矛盾**：现有数据集的地域/肤色/病种偏倚导致 AI 模型对深肤色人群准确率下降 30-40%（DDI 数据集实验），无法公平服务全球人口
4. **切入角度**：从印度门诊前瞻性采集，构建兼具疾病覆盖广度、肤色代表性和多维标注的数据集
5. **核心idea一句话**：提供首个 South Asian 肤色为主、同时包含诊断层次 + 解剖位置 + 病灶描述符三重标注的皮肤病数据集
