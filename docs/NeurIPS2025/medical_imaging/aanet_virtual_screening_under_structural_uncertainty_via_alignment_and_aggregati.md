...
description: >-
  [NeurIPS 2025][医学图像][虚拟筛选] 针对现实药物发现中蛋白质 holo 结构不可用的问题，提出 AANet——通过三模态对比学习（配体-holo pocket-检测cavity）对齐表征并用交叉注意力聚合多个候选结合位点…
tags:
  - "NeurIPS 2025"
  - "医学图像"
  - "虚拟筛选"
...
**本文目标** 如何在不知道配体结合位置的情况下进行准确的虚拟筛选？

**切入角度**：几何检测的 cavity 是 holo pocket 的"噪声代理"——学习对齐两者的表征。

**核心 idea**：三模态对齐（配体-holo pocket-cavity）+ 交叉注意力聚合候选位点，使模型在结合位点未知时仍能准确筛选。
...
1. **三模态对比对齐 (Tri-modal Contrastive Alignment)**:
    ...
    - 功能：学习配体、holo pocket、检测 cavity 三者的对齐表征
