...
1. **近似遗忘的脆弱性**：梯度上升、NPO 等近似方法虽计算高效，但已被证明可被攻击恢复遗忘数据（Lucki et al., Hu et al.），且缺乏形式化保证
2. **精确遗忘被视为"金标准"**：通过在去除目标数据后从头重训练，精确遗忘被普遍认为能完全消除数据影响，对提取/反演攻击免疫
3. **被忽视的威胁模型**：在实际部署中，遗忘前的模型检查点/logits API 通常是可获取的——开源模型的快照可被保存，API 用户可能已保存之前的查询结果。这意味着攻击者可同时访问遗忘前后两个版本
