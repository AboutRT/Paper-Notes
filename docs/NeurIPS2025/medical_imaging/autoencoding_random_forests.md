...
description: >-
  [NeurIPS 2025][医学图像][随机森林] RFAE 首次为随机森林构建了原则性的编码-解码框架——利用 RF 核的正定性和普适性进行扩散映射谱分解得到低维编码，通过 k-NN 回归在叶节点空间中解码回原始特征…
tags:
  - "NeurIPS 2025"
  - "医学图像"
  - "随机森林"
...
**本文目标** 为随机森林构建理论上有保证的编码器（低维嵌入）和解码器（从嵌入恢复原始特征）。

**切入角度**：RF 核 $k_n^{RF}(\mathbf{x}, \mathbf{x}')$（两个样本落入同一叶节点的频率）被证明是正定、渐近普适和特征性的（Theorem 3.4），可以直接用扩散映射做谱分解。

**核心 idea**：RF 核的正定性 → 扩散映射谱分解 = 编码器；k-NN 回归在叶节点空间 = 解码器；理论保证普适一致性。
