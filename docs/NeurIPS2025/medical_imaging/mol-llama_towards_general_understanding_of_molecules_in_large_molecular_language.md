...
1. **知识和推理能力不足**：现有模型依赖特定任务的公开数据库（如 PubChem 的简短描述），知识范围窄、缺乏对分子特征因果关系的深入解释，常错误预测分子分类和性质
2. **分子结构理解有限**：仅使用单一类型的分子编码器（2D 或 3D 图），无法同时捕获键信息/连接性（2D 优势）和空间排列/表面积/体积（3D 优势）
