...
description: >-
  [NeurIPS 2025][医学图像][鉴别诊断] H-DDx 提出基于 ICD-10 分类层级的鉴别诊断评估框架——将预测和真实诊断扩展到祖先节点后计算层级 F1（HDF1），奖励"临床相关的近似正确"而非仅精确匹配…
tags:
  - "NeurIPS 2025"
  - "医学图像"
  - "鉴别诊断"
...
**本文目标** 设计尊重 ICD-10 层级结构的评估指标，奖励临床相关的近似预测。

**切入角度**：将预测和真实诊断集合沿 ICD-10 树向上扩展到所有祖先节点，在扩展后的集合上计算精度/召回/F1。

**核心 idea**：将诊断集合沿 ICD-10 树扩展到章/节/类祖先节点 → 在扩展集合上计算层级 F1 → 奖励同类/同节的近似正确预测。
...
1. **诊断到 ICD-10 映射流水线**:
    ...
    - 功能：将 LLM 输出的自然语言诊断映射到标准 ICD-10 编码
...
3. **层级级联分析**:
    ...
    - 功能：在不同 ICD-10 层级（章/节/类/亚类）分别计算 HDF1
