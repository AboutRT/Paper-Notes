---
description: "【论文笔记】Efficient Adaptive Experimentation with Noncompliance 论文解读 | NeurIPS 2025 | arXiv 2505.17468 | 自适应实验 | 提出 AMRIV——首个面向带非依从性（noncompliance）的自适应实验的半参数高效、多重鲁棒的ATE估计器，结合方差最优的工具变量分配策略和序贯推断保证。"
tags:
  - NeurIPS 2025
  - 自适应实验
  - 工具变量
...
1. **领域现状**：自适应实验通过根据累积数据调整分配策略来高效估计处理效应，已被 FDA 正式认可。直接分配处理的自适应 ATE 估计已有成熟工具包（A2IPW、Neyman 分配等）。

2. **现有痛点**：在很多现实场景中，**处理不能直接分配**，只能通过**工具变量（IV）鼓励**。例如：
...
4. **本文要解决什么**：将完整的现代半参数工具——高效影响函数、自适应策略学习、鲁棒插补估计、随时有效推断——引入带非依从性的自适应 IV 设置。

5. **切入角度**：基于 Wang & Tchetgen Tchetgen 的非混杂依从性假设和多重鲁棒影响函数，推广到自适应设置。

6. **核心idea一句话**：推导带 IV 的自适应实验的半参数效率界和最优分配策略，并构建序贯估计器实现该界。
