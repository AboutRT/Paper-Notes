...
1. **极端维度**：fMRI数据为 $x \in \mathbb{R}^{H \times W \times D \times T}$ 的四维张量，空间和时间维度同时极高
2. **变异性主导**：PCA分析显示个体差异解释最大方差，相位编码方向次之，任务诱发信号仅为微弱的第三成分——任务信号被"淹没"
3. **已有方法的简化**：前人工作回避了体素级动态，转而使用ROI时间序列、功能连接矩阵或静态3D激活图等简化表示，丢失了关键的体素级时空信息
4. **缺乏神经科学导向的评估**：FID等标准图像指标无法评估生成的fMRI是否保留了任务特异性时空动态
