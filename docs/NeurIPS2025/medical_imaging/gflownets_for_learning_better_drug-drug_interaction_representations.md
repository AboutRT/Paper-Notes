...
1. **领域现状**: DDI 预测是药物安全的关键任务，现有方法利用化学结构和生物网络等多种特征构建预测模型。
2. **现有痛点**: DDI 数据集中存在严重的类别不平衡——常见交互类型（如协同效应）主导数据集，而罕见但临床上重要的交互类型严重不足，导致模型在低频类别上表现差。
3. **核心矛盾**: 现有SOTA方法多将 DDI 预测视为二分类问题（有/无交互），忽略了不同交互类型的语义异质性，加剧了对频繁类型的偏向。
4. **本文要解决什么**: 如何在不丢失频繁类别性能的前提下，提升模型对稀有交互类型的覆盖率和预测精度。
5. **切入角度**: 利用 GFlowNet 的奖励比例采样特性，有选择性地为低频类别生成合成 DDI 样本。
6. **核心idea一句话**: 用 GFlowNet 按"稀有度×可信度"的奖励函数生成合成 DDI 样本来增强训练数据的类别平衡性。
...
2. **GFlowNet 合成 DDI 生成**:
   - **做什么**: 按奖励比例采样生成合成 DDI 三元组 $(d_i, d_j, t)$
