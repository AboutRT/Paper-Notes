...
1. **领域现状**：表型药物发现通过 Cell Painting 高通量显微镜成像观察细胞形态变化来筛选药物候选分子。生成模型（尤其是潜在扩散模型）被用来模拟 Cell Painting 图像以降低实验成本。SD-VAE 作为潜在扩散模型的核心压缩组件被广泛采用。
2. **现有痛点**：SD-VAE 是在自然图像（LAION-2B）上预训练的，直接应用于分布外的多通道荧光显微镜图像时，是否会丢失关键生物信息尚未被定量验证。之前的工作（MorphoDiff 等）使用了 SD-VAE 但没有隔离评估其重建质量。
3. **核心矛盾**：生成模型的下游生物学解释依赖于 VAE 的重建保真度——如果编码-解码过程丢失了细微的表型差异，那么整个生成管线的生物学价值就存疑。
4. **本文要解决什么**：系统量化 SD-VAE 对 Cell Painting 图像的重建质量，特别关注生物信号是否被保留。
5. **切入角度**：不仅使用像素级指标（MAE、SSIM、EMD），还引入特征空间指标（FID）、潜在空间指标（KLD）和基于信息检索的生物学指标（FR - Fraction Retrieved）进行多层次评估。
6. **核心 idea**：SD-VAE 重建保留了足够的表型信号 + InceptionV3 在检索任务上足以替代领域专用模型 + 为显微镜数据的生成模型评估提供通用框架。
...
1. **SD-VAE 编码-解码评估**:
   - 做什么：用冻结的 SD v1-4 VAE 对 Cell Painting 图像进行编码和解码，评估重建保真度
