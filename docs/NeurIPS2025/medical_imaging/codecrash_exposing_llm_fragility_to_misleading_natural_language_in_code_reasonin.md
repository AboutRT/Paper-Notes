...
1. **领域现状**：LLM 在代码生成、补全、修复等任务上表现卓越，已被集成到 GitHub Copilot 等 IDE 工具中。然而真实代码库中充满歧义标识符、无用代码和不一致注释等噪声。
2. **现有痛点**：传统鲁棒性研究主要关注代码结构变换（重命名变量、修改控制流、插入不可达代码），这些仅测试模式匹配能力；NL 方向的扰动研究集中在 NL-to-Code 任务（评估提示敏感性），无人系统评估 LLM 是否能在代码推理中优先考虑可执行语义而非自然语言线索。
3. **核心矛盾**：LLM 在代码理解中将注释和自然语言线索视为高优先级证据，但这些在程序执行中语义无关——模型无法区分功能性代码和非功能性上下文。
4. **本文要解决什么**：设计一个覆盖结构层和 NL 层的代码推理鲁棒性 benchmark，剖析 LLM 在面对误导信息时的失败模式。
5. **切入角度**：将扰动分为上下文级（明显错误的 NL 线索）和推理级（貌似合理但错误的暗示），创造性地利用 AST 解析保证扰动后代码功能等价。
6. **核心 idea**：LLM 过度依赖 NL 线索走捷径推理 → CoT 能缓解但不能根除 → LRM 的内部推理虽更鲁棒但在合理暗示下触发病态自省和推理崩溃。
...
3. **推理级误导扰动（MHC）**:
   - 做什么：注入关于程序输出的高层级错误暗示（Misleading Hint Comments）
