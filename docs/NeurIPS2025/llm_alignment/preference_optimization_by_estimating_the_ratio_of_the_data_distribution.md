...
description: >-
  [NeurIPS 2025][LLM对齐][DPO] 将 DPO 重新解释为似然比估计（ratio matching）问题，基于 Bregman 散度框架提出 BPO（Bregman Preference Optimization），包含 DPO 为特例的广义损失函数族…
tags:
  - "NeurIPS 2025"
  - "LLM对齐"
  - "DPO"
...
**本文目标** 找到一种既保持目标最优性、又不需要额外计算开销、同时支持多种损失函数实例的通用偏好优化框架。

**切入角度**：从似然比估计的视角重新理解 DPO——最优策略可以通过其**似然比**来唯一确定（无需奖励模型或配分函数），因此问题转化为用 Bregman 散度做比率匹配。

**核心 idea**：DPO 本质上在匹配模型比率 $R_\theta$ 到数据比率 $R_{\text{data}}$，选择不同的 Bregman 散度 $h$ 就得到不同的损失函数，所有实例都保最优性且无额外开销。
