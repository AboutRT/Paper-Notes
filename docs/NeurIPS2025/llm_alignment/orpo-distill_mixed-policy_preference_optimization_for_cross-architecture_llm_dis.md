...
1. **白盒 KD**：依赖教师 logits，要求师生共享词表/架构，限制灵活性
2. **黑盒 KD**：仅需教师生成序列，允许跨架构，但通常只用 CoT 蒸馏

**现有黑盒 KD 的三个问题**：

1. **单一 CoT 蒸馏**监督信号有限：只用一条教师推理链
2. **缺少对比学习**：未区分"好的推理路径"和"差的推理路径"
3. **分布不匹配**：训练时用教师序列，推理时学生自回归生成，分布存在 gap
