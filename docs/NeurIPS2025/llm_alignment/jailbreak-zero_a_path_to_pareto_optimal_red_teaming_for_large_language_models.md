...
1. **可扩展性差**：固定的示例列表无法覆盖所有真实安全风险，尤其当策略频繁变化时
2. **评估单一**：仅依赖攻击成功率（ASR），忽略了安全评估的多维性——覆盖率、多样性、对真实用户输入的保真度
3. **有效性存疑**：如果目标 LLM 针对预定义行为进行微调，拒绝率的提升可能反映的是记忆而非真正的安全性
4. **人工依赖重**：现有方法通常需要复杂的迭代算法、人工设计的越狱策略或大量 prompt engineering
