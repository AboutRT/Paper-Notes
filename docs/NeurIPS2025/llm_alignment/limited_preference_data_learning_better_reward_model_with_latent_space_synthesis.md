...
1. **人工标注成本高**：偏好数据需要人类进行成对比较标注，耗时且昂贵，难以大规模收集
2. **现有文本合成方法计算开销大**：传统方法需要两阶段流程——先用 LLM 生成多个响应，再用辅助 LLM 标注偏好对，计算复杂度高且随响应数量二次增长
3. **资源受限场景需求**：小型实验室和创业公司难以承担数十亿参数模型的推理成本
