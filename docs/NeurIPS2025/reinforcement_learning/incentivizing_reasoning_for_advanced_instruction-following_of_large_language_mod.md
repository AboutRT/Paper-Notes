---
title: >-
  [论文解读] Incentivizing Reasoning for Advanced Instruction-Following of Large Language Models
description: >-
  [NeurIPS 2025][强化学习][指令跟随] 提出 RAIF，通过 RL+规则中心奖励培养 LLM 在复杂指令（含 And/Chain/Selection/Nested 组合约束）下的深度推理能力：发现 vanilla CoT 对指令跟随有负面影响（因 LLM 只会浅层复述指令），设计 superior CoT enforcement（样本级对比过滤无效推理）+ 行为克隆控制分布偏移，1.5B 模型匹配 8B 性能，7 个 benchmark 平均提升 11.74%。
tags:
  - NeurIPS 2025
  - 强化学习
  - 指令跟随
...
**代码**: [https://github.com/yuleiqin/RAIF](https://github.com/yuleiqin/RAIF)  
**领域**: 强化学习  
...
**核心矛盾**：CoT 在数学问题上有效是因为分步推导是获得答案的前提。但指令跟随中，LLM 的"推理"往往是**浅层复述指令**（paraphrasing），不去分析约束间的层级关系和依赖——这种假推理反而引入噪声。

**本文目标** 解决两个问题：（a）如何为复杂指令合成多样化的训练数据（含可验证奖励源）；（b）如何用 RL 培养对指令跟随**真正有益**的深度推理（而非浅层复述）。

**切入角度**：从 R1/o1 的成功出发——它们用 RL 激励数学推理。但指令跟随不同于数学：（a）没有唯一正确答案；（b）推理不是获得答案的前提（可以不推理直接回答）；（c）语义质量也重要。因此需要专门设计。

**核心 idea**：用 GRPO + 规则中心奖励（code 验证 + LLM 判别）+ 样本级 CoT 优劣对比过滤（只保留推理确实优于不推理的样本）+ 行为克隆防分布偏移，培养对指令跟随真正有效的深度推理。
...
1. **规则中心奖励（Rule-Centric Reward）**：
    ...
    - 功能：将复杂指令分解为 $C$ 个原子约束 $\{c_j\}$，逐条验证是否满足
...
2. **Superior CoT Enforcement（样本级对比过滤）**：
    ...
    - 功能：过滤掉推理反而变差的样本，只保留推理确实有益的训练信号
...
3. **行为克隆分布偏移控制**：
    ...
    - 功能：在 RL 目标中加入 SFT Loss $\mathcal{J}_{SFT} = -\log \pi_\theta(\tilde{y}|x)$，用专家回复约束策略不偏移太远
    - 设计动机：RL 只优化约束满足度，可能导致回复虽然满足约束但语义退化（不流畅、不连贯）。SFT 正则确保语义质量，比单靠 KL 惩罚更显式

4. **LLM-based 指令演化**：

    - 功能：从 WildChat/Alpaca 种子指令出发，按 CFBench 约束分类+ComplexBench 组合结构进行演化扩展
