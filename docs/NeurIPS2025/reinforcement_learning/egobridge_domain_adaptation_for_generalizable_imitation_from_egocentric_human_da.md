...
**关键词**: cross-embodiment, domain adaptation, optimal transport, egocentric, imitation learning
...
1. **领域现状**：行为克隆(BC)结合大规模遥操作数据在机器人操作中取得显著进展。但遥操作数据收集成本高、难以覆盖多样场景。穿戴设备（如 AR 眼镜）可低成本采集大量第一人称人类操作数据，包含观测和动作信息。
2. **现有痛点**：人类数据和机器人数据之间存在多重域差距——(a) 视觉外观差异（人手 vs 机械臂）；(b) 运动学差异（同一动作空间下行为分布不同）；(c) 传感模态差异（机器人有腕部相机，人类没有）。直接混合训练(co-training)不能自动产生有效迁移。
3. **核心矛盾**：简单 co-training 假设共享潜在空间自然涌现，但实际上人类和机器人的潜在特征形成分离的聚类（latent covariate shift），$\mu_H \neq \mu_R$，导致从人类数据学到的行为无法迁移到机器人。
4. **本文要解决什么**：(a) 显式对齐人类和机器人域的潜在表示；(b) 在对齐过程中保留动作相关信息；(c) 使机器人能执行仅在人类数据中出现的新行为。
5. **切入角度**：将跨具身学习形式化为域适应问题。不同于全局分布对齐（如对抗训练、MMD），利用 OT 的几何结构保留局部动作对应关系。
6. **核心idea一句话**：用 DTW 引导 OT 的代价函数，在对齐潜在特征时自动发现行为相似的人类-机器人伪配对，实现动作感知的联合分布对齐。
...
1. **联合分布最优传输 (Joint OT)**:
   - 做什么：对齐人类和机器人数据在潜在空间中的联合分布 $P(f_\phi(O), A)$
