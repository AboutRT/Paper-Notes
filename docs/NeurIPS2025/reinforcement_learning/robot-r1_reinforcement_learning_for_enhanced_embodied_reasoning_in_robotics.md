...
1. **SFT 数据集的局限性**：现有的具身推理训练数据集多为启发式构建，语言描述无法捕捉低级机器人控制所需的精确数量信息，且数据集并未针对实际机器人动作预测进行优化。
2. **灾难性遗忘问题**：SFT 方法在面对训练分布外的输入输出格式时表现急剧下降，会遗忘已学到的通用对话能力。
3. **泛化性不足**：SFT 训练的模型在空间推理和运动推理等对机器人控制至关重要的能力上提升有限，难以泛化到新任务。
