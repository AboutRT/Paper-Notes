...
1. **环境因素过于简化**：未能真实地捕捉时变的电网碳强度、电力价格、天气变化等因素的交互影响
2. **数据中心物理模型缺失**：忽视了 CPU、GPU、内存、HVAC（暖通空调）能耗等详细的数据中心物理特性
3. **地理分布网络动态缺失**：未建模延迟、传输成本等跨数据中心的网络动态
4. **缺乏可复现性**：部分工作使用私有数据或不可复现的实验设置
