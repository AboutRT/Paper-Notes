...
1. **领域现状**：DRL 中快速高效的动作选择仍是核心挑战，特别是在具有空间结构的环境（机器人操作中关节运动耦合、游戏中方向动作邻接）。现有方法将动作视为正交独立的 one-hot 向量，完全忽略动作间的拓扑关系
2. **现有痛点**：
...
3. **核心矛盾**：动作空间有内在拓扑结构，但这种结构信息在标准 DRL 中完全被丢弃
4. **切入角度**：在果蝇的中央复合体中发现的环形吸引子是一种经实验验证的神经回路，能稳定地编码方向和空间信息
5. **核心 idea**：将环形吸引子作为动作选择的"大脑"——将 Q 值转化为环上的高斯输入信号（幅度=Q值、角度=动作方向、宽度=不确定性），通过兴奋-抑制动力学选出最优动作
...
1. **环形吸引子架构（Touretzky 模型）**：
   - 做什么：N 个兴奋性神经元环形排列 + 1 个中心抑制性神经元，通过距离衰减的突触连接实现局部兴奋和全局抑制
...
2. **Q 值到环输入的映射**：
   - 做什么：将 DQN 输出的 Q 值转化为环形吸引子的高斯输入信号
...
3. **贝叶斯不确定性注入**：
   - 做什么：用贝叶斯线性回归（BLR）作为 DQN 输出层，自然提供每个动作的 Q 值方差
...
4. **集成 DL 模块（可复用 RNN）**：
   - 做什么：将环形吸引子动力学用 GRU/LSTM 实现，作为 DRL 框架中的即插即用组件
