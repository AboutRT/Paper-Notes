...
- 现实机器人任务需要达成**序列子目标**同时遵守**动态安全约束**（如仓库机器人需按顺序补货并避障、保持电池可达范围）
- 目标条件 RL（GCRL）能达到单个目标但无法推理目标序列或确保安全；安全 RL 只能处理**静态**安全约束
- LTL 可表达复杂时序任务，但现有 LTL+RL 方法依赖稀疏二值奖励，在复杂连续环境中表现差
- **核心差距**：没有可扩展的方法能同时支持时序目标和安全约束
