...
description: >-
  [NeurIPS 2025][强化学习][GFlowNet] 提出多智能体生成流网络（MA-GFlowNets）的理论框架，证明了"局部-全局原理"——联合流函数可分解为各智能体独立流的乘积形式，设计了四种算法（CFN/IFN/JFN/CJFN）…
tags:
  - "NeurIPS 2025"
  - "强化学习"
  - "GFlowNet"
...
**领域现状**：GFlowNet 是一种通过流匹配损失学习随机策略的生成模型，能以正比于奖励的概率采样对象——相比 RL 只追求最高奖励的策略，GFlowNet 能保持多样性。然而，GFlowNet 的理论和算法都仅限于单智能体场景。

**现有痛点**：(a) 现有 GFlowNet 无法支持多智能体系统——多智能体需要联合动作空间，复杂度随智能体数指数增长；(b) 多智能体 RL（MARL）可以做协同决策，但倾向于模式坍塌到单一最优策略，不支持多样性采样；(c) 已有的分布式 GFlowNet（如 Meta-GFlowNet）要求所有智能体观测和目标相同，不适用于部分可观测的多智能体问题。

**核心矛盾**：中心化训练（CFN）准确但联合动作空间 $O(|A|^N)$ 复杂度爆炸；独立训练（IFN）高效但面临非平稳奖励（每个智能体看到的局部奖励受其他智能体影响而变化），导致模式坍塌。

**本文目标**
   - 如何建立多智能体 GFlowNet 的理论框架？
   - 如何实现 CTDE（中心化训练+去中心化执行）的 GFlowNet 算法？

**切入角度**：类比 MARL 中的值分解方法（VDN、QMIX），将全局流函数分解为局部流的乘积，通过"局部-全局原理"在全局流匹配约束和局部流匹配约束之间建立理论联系。

**核心 idea**：全局 GFlowNet 的流可以分解为各智能体局部 GFlowNet 流的乘积，使得在全局流匹配约束下训练局部模型既保证了理论正确性又实现了 CTDE。
...
4. **CJFN（Conditioned Joint Flow Network）**
    ...
    - 功能：解决 JFN 的限制——JFN 只能精确采样乘积形式的奖励。
