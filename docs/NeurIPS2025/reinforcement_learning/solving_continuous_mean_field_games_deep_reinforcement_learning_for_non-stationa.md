...
现有MFG求解方法存在三大局限：
1. **离散限制**：大多数RL方法（Guo等2019; Elie等2020; Laurière等2022a）限于有限状态/动作空间
2. **平稳假设**：大部分方法（Perrin等2021; Angiuli等2023）假设种群分布不随时间变化
3. **密度不可得**：现有方法无法直接获取种群分布的概率密度值 $\mu(x)$，只能采样——这使得涉及局部密度依赖的MFG（如拥堵模型）无法精确求解
