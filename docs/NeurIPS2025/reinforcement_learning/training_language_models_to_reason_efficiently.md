...
description: >-
  [NeurIPS 2025][强化学习][高效推理] 通过在 RL 奖励中加入长度惩罚项——正确回答的奖励乘以 $(1 - \alpha \cdot \sigma(\text{norm\_len}))$，用单一超参数 $\alpha$ 控制 token-准确率权衡曲线…
tags:
  - "NeurIPS 2025"
  - "强化学习"
  - "高效推理"
...
**本文目标** 如何系统地训练推理模型使用更少 token 达到正确答案，同时保留对难题使用更多 token 的能力？

**切入角度**：在 RL 奖励函数中引入长度正则项，只对**正确回答**施加长度惩罚（错误回答奖励始终为 0），并通过 per-prompt 归一化确保难题不被过度惩罚。

**核心 idea**：在正确性奖励上乘以长度折扣 $(1 - \alpha \cdot f(\text{len}))$，用 $\alpha$ 平滑控制效率-准确率权衡，让模型自适应地对简单题更简洁、对难题仍深入思考。
