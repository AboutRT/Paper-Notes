...
1. **领域现状**：长期时间序列预测是经济、交通、能源等领域的关键任务。主流深度模型（DLinear、TimesNet、iTransformer 等）普遍在前向传播中使用季节-趋势分解模块来提取有效表征。
2. **现有痛点**：标准 MSE 损失直接计算预测值与真实值的逐点差异，但无法显式约束预测结果在季节性和趋势两个维度上的准确性。作者观察到三类失败模式：(a) 季节性预测差但趋势可接受；(b) 趋势预测差但季节性可接受；(c) 二者都差。即使模型前向传播中做了分解，损失函数端仍然"一视同仁"。
3. **核心矛盾**：前向传播中的分解提取了归纳偏置，但损失函数端未利用这个先验——预测窗口内的季节性和趋势没有被独立监督。
4. **切入角度**：既然分解在前向传播端有用，何不在损失计算端也做分解？在预测窗口内对预测和 GT 分别做 EMA 分解，对趋势和季节分量分别计算损失再加权融合。
5. **核心idea**：DBLoss = EMA 分解 + 分量级独立损失 + scale alignment 加权机制，零额外参数、可组合任意 backbone。
...
1. **EMA 分解模块**：
   - 做什么：将预测和 GT 分解为趋势 $Y_T$ 和季节 $Y_S = Y - Y_T$。
