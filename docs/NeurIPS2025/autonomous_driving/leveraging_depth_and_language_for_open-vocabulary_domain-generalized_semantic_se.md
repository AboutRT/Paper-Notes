---
description: "【论文笔记】Leveraging Depth and Language for Open-Vocabulary Domain-Generalized Semantic Segmentation 论文解读 | NeurIPS 2025 | arXiv 2506.09881 | 开放词汇分割 | 提出Vireo框架，首次将开放词汇语义分割（OVSS）和域泛化语义分割（DGSS）统一到单阶段框架中，通过GeoText Query融合深度几何特征与语言线索，在极端环境和未见类别上均实现SOTA表现。"
tags:
  - NeurIPS 2025
  - 开放词汇分割
  - 域泛化
...
1. **领域现状**：开放词汇语义分割（OVSS）能识别任意文本描述的类别，域泛化语义分割（DGSS）能在未见域上保持鲁棒性，两者各有所长但互补。
2. **现有痛点**：OVSS的文本-视觉对齐模块在域外（如夜晚、雨天）性能大幅下降；DGSS的域不变策略可能抑制细粒度语义线索，影响对文本查询的精确响应。
3. **核心矛盾**：如何同时实现跨域鲁棒性和开放词汇识别能力？DGSS侧重编码器端特征泛化，OVSS侧重解码器端开放识别——两者天然互补。
4. **本文要解决什么**：构建统一的OV-DGSS（开放词汇域泛化语义分割）框架，在域偏移下对未见类别也能鲁棒分割。
5. **切入角度**：利用深度图的域不变性（深度和几何线索对光照、纹理变化不敏感），结合冻结VFM的泛化能力。
6. **核心idea一句话**：用GeoText Query在冻结VFM层间注入深度几何和文本语义先验，辅以CMPE增强梯度流和DOV-VEH融合多模态特征。
