---
description: "【论文笔记】V2X-Radar: A Multi-Modal Dataset with 4D Radar for Cooperative Perception 论文解读 | NeurIPS 2025 | arXiv 2411.10962 | cooperative perception | 提出 V2X-Radar，首个大规模真实世界多模态车路协同感知数据集，包含 4D 雷达、LiDAR 和多视角相机数据，覆盖多种天气和光照条件，提供 20K LiDAR 帧、40K 相机图像、20K 4D 雷达数据和 350K 标注框，并建立三个子数据集的全面基准。"
tags:
  - NeurIPS 2025
...
1. **现有 V2X 数据集缺少 4D 雷达**：OPV2V、DAIR-V2X、V2V4Real 等数据集仅包含相机和 LiDAR，忽视了 4D 雷达在恶劣天气下的鲁棒性优势
2. **单车数据集已有 4D 雷达**：K-Radar、Dual-Radar 等证明了 4D 雷达对雨雪雾等极端天气的强适应性，但协同感知领域尚无此类数据集
3. **场景多样性不足**：现有真实 V2X 数据集多为特定时段/天气采集，缺乏昼夜、晴雨雪雾的全面覆盖
