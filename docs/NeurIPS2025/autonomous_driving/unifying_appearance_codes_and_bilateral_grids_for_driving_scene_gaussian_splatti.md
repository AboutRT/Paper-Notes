...
**领域现状**：驾驶场景的 3D Gaussian Splatting 重建受跨图像光度不一致（自动曝光、白平衡变化）困扰。现有方法用全局外观编码（每图一个向量）或像素级双边网格来补偿。

**现有痛点**：全局外观编码表达力有限（只能建模整体色调变化）；像素级双边网格参数量大（27.8M）且难以优化——在小场景数据下容易过拟合。两者各有优劣但不兼容。

**核心矛盾**：需要同时建模全局色调偏移（外观编码擅长）和局部光影变化（双边网格擅长），但二者的参数化方式不同。

**本文目标** 设计统一框架同时获得全局和局部光度补偿能力，且参数量可控。

**切入角度**：多尺度网格金字塔——粗网格 = 全局外观编码，中网格 = 区域补偿，细网格 = 像素级调整。三级逐步叠加，序列组合变换。

**核心 idea**：3 级双边网格金字塔（2×2×1→4×4×2→8×8×4）统一全局/区域/像素级光度建模 + 亮度引导切片 + 自适应正则化。
