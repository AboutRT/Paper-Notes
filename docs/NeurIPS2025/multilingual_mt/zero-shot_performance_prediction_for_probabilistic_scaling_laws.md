...
1. **参数化拟合方法**（power law、指数函数等）：需要对每条曲线独立拟合，无法在任务间迁移
2. **贝叶斯神经网络 (BNN)**：需要较多配置（Klein et al. 最小用了 256 个配置），在小数据集（30 条曲线以下）效果差
3. **传统 GP 方法**：假设数据内部结构扁平，未利用层次结构
4. **已有 Scaling Law 研究**：计算密集型的经验研究，缺乏不确定性量化
