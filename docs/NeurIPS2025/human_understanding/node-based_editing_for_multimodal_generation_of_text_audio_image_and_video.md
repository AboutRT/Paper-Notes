---
description: "【论文笔记】Node-Based Editing for Multimodal Generation of Text, Audio, Image, and Video 论文解读 | NeurIPS 2025 | arXiv 2511.03227 | 节点图接口 | 提出一个节点图式故事编辑系统，允许创作者通过自然语言和节点级操作迭代地生成、编辑和比较多模态内容（文本、音频、图像、视频），支持线性和分支叙事结构。"
tags:
  - NeurIPS 2025
  - 节点图接口
  - 叙事生成
...
1. **领域现状**：Sora、DALL-E 等生成模型使内容创作变得容易，但当前主要采用单轮提示范式。
2. **现有痛点**：(1) 单一提示难以充分表达复杂叙事意图；(2) 全部重新生成低效；(3) 线性编辑接口无法表示分支故事；(4) 缺乏机制探索多种叙事方向。
3. **核心矛盾**：强大的生成能力 vs 贫乏的交互控制——模型能生成好内容，但创作者无法精细控制叙事结构。
4. **切入角度**：采用图式节点表示使叙事结构显式化，集成对话界面与节点编辑兼顾控制粒度。
