...
1. **数据异质性**：地球物理观测数据跨越不同模态（应力角、应变角、板块类型、断层类型、盆地类型/年龄、沉积厚度、地幔温度），各模态在空间分辨率、稀疏程度和数据类型上差异巨大
2. **学科专业化**：现有基础模型局限于单一子领域（地震学、气候预测、海洋学），无法跨学科利用信息
3. **数据稀疏性**：许多地区的直接观测数据稀缺且昂贵，需要模型从有限观测中泛化
