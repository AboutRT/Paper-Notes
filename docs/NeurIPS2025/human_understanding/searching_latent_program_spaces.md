...
1. **归纳式方法（Program Synthesis）**: 在领域专用语言（DSL）中搜索程序，泛化能力强但搜索空间爆炸，且需要人工设计 DSL，不可扩展。
2. **转导式方法（In-Context Learning）**: 神经网络直接从示例预测输出，可扩展但缺乏结构化测试时适应能力，且无法保证与给定示例的**一致性**（consistency）。
