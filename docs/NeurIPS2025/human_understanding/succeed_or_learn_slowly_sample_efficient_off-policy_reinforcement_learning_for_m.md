...
1. **GPT-4o提示工程方法**：虽然能利用强大的先验知识，但每步需要多次API调用，推理延迟高达40-60秒，且运营成本昂贵
2. **SFT微调方法**：在人类演示数据上训练的小模型泛化能力有限，尤其是当目标任务分布与训练数据存在显著偏移时
3. **标准RL方法**：在基础模型微调场景下面临两个根本问题——负优势样本的策略梯度更新会扰乱模型已学习的表征，导致性能退化
