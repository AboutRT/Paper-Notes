...
**领域现状**：LLM agent 的在线学习面临**稀疏奖励**挑战——多轮任务中需要连续正确操作才能获得奖励

### 现有痛点

**现有痛点**：时序信用分配**（temporal credit assignment）旨在将稀疏反馈分解为每步的密集训练信号

### 核心矛盾

**核心矛盾**：传统 RL 需要学习任务特定的值函数来做信用分配，样本效率低、泛化差

### 解决思路

**解决思路**：已有 LLM 自我纠正方法（Reflexion 等）使用轨迹级反馈，粒度粗且假设反馈可跨轨迹迁移

### 补充说明

**补充说明**：核心洞察**：LLM 的预训练知识 + 回顾式上下文更新 → 可精确估计优势函数
