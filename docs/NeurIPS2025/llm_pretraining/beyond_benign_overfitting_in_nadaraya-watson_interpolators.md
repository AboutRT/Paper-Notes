...
description: >-
  [NeurIPS 2025][预训练][良性过拟合] 通过调节 Nadaraya-Watson 插值器中的单一带宽参数 $\beta$，精确刻画了从灾难性过拟合（$\beta < d$）→ 良性过拟合（$\beta = d$）→ 温和过拟合（$\beta > d$）的完整相变谱…
tags:
  - "NeurIPS 2025"
  - "预训练"
  - "良性过拟合"
...
**本文目标**：完整刻画 NW 插值器在所有 $\beta$ 取值下的过拟合类型，给出精确的相变条件。

**切入角度**：NW 插值器只有一个超参数 $\beta$（控制核的局部/全局程度），是研究过拟合相变的理想对象——结构足够简单以获得精确理论，又足够丰富以展示三种过拟合行为。

**核心 idea**：通过分析 NW 插值器中距离权重 $\|\mathbf{x} - \mathbf{x}_i\|^{-\beta}$ 的局部性/全局性，证明 $\beta$ 与数据维度 $d$ 的关系完整决定了过拟合类型。
...
1. **温和过拟合证明 ($\beta > d$)**:
    ...
    - 功能：证明当 $\beta > d$ 时，$C_1 \cdot p^{c(\beta/d)} \leq \mathcal{L}(\hat{h}_\beta) \leq \tilde{O}(p)$
...
2. **灾难性过拟合证明 ($\beta < d$)**:
    ...
    - 功能：构造显式分布使得 $\mathcal{L}(\hat{h}_\beta) \geq C_2 \cdot c(\beta, d) > 0$，与噪声 $p$ 无关
...
3. **内禀维度推广**:
    ...
    - 功能：猜想对于支撑在低维流形上的分布，$d$ 可以替换为内禀维度 $d_\text{int}$
