...
1. **模型指纹的需求**：模型指纹允许所有者通过 API 访问识别被非法使用的模型
2. **可扩展性为何关键**：降低误检率、抵御指纹泄露（每次验证暴露一个指纹）、防御共谋攻击（多用户联合规避）
3. **现有方法瓶颈**：RANDOM（随机 token key）可扩展但不安全；ENGLISH-RANDOM（自然语言 key + 随机 response）超过 256 指纹后性能急剧下降
