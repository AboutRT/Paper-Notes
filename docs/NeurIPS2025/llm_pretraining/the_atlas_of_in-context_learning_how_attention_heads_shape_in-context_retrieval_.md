...
description: >-
  [NeurIPS 2025][上下文学习] 通过 AttnLRP 归因方法系统解剖 LLM 在 in-context retrieval augmented QA 中的内部机制，发现三类功能特化的注意力头——Task heads（中间层，解析指令/问题）、Retrieval heads（后层，逐字复制上下文答案）、Parametric heads（编码参数化知识），并通过 Function Vector 注入和来源追踪探针验证其功能，在 Llama-3.1/Mistral/Gemma 上 ROC AUC ≥94%。
tags:
  - NeurIPS 2025
  - 上下文学习
  - 注意力头分析
...
**代码**: [https://github.com/pkhdipraja/in-context-atlas](https://github.com/pkhdipraja/in-context-atlas)  
**领域**: LLM预训练  
**关键词**: 上下文学习, 注意力头分析, Retrieval Augmentation, AttnLRP, 知识归因
...
1. **AttnLRP 归因方法**:
    ...
    - 功能：为每个注意力头计算其对最终输出 token 的因果贡献
...
2. **注意力头识别与分类**:
    ...
    - 功能：将头分为 in-context heads 和 parametric heads
...
3. **Function Vector (FV) 验证**:
    ...
    - 功能：提取特定类型头的输出向量，注入到没有该功能的场景中，验证是否诱导出对应行为
...
4. **来源追踪探针**:
    ...
    - 功能：在 Retrieval heads 的激活上训练线性探针，判断答案来源是上下文还是参数记忆
