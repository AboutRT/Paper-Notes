...
1. **认知科学的质疑**：研究者认为 ViT 缺乏动态灵活分组特征的机制、缺乏迭代精炼的循环连接、作为纯连接主义模型无法进行真正的符号处理
2. **目标中心学习的局限**：Slot Attention 等方法通过外部模块强制绑定，但引入了额外的扩展和训练挑战
3. **关键问题**：ViT 是否能在没有显式架构归纳偏置的情况下，仅通过大规模预训练就自然习得目标绑定能力？
