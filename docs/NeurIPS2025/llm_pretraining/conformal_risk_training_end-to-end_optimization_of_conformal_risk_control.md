...
1. **领域现状**：深度学习模型虽然在预测精度上表现出色，但其预测通常不附带任何关于风险或可靠性的可证明保证。Conformal Risk Control (CRC) 提供了一种分布无关的有限样本方法，可以控制任何有界单调损失函数的期望值，并且可以方便地事后应用于任何预训练模型。
...
3. **核心矛盾**：安全保证与性能之间的 trade-off——事后 CRC 提供保证但牺牲性能；端到端优化可能改善性能但需要保持保证
...
5. **切入角度**：认识到 CRC 的阈值选择过程是可微的——可以将整个 CRC 流程（包括校准集上的阈值搜索）嵌入到训练循环中，通过梯度反传优化模型参数。

6. **核心idea**：将共形 OCE 风险控制过程嵌入模型训练的前向传播中，使端到端梯度能流过 CRC 的阈值选择步骤，从而让模型学会在保持风险保证的同时最大化平均性能。
