...
**代码**: 未公开  
**领域**: ai_safety  
**关键词**: model protection, edge deployment, TEE, permutation, LLM security, model stealing
...
1. **领域现状**：专有 LLM（如 ChatGPT、Claude）正越来越多地部署到边缘设备上以实现低延迟和隐私保护（如 Apple Intelligence 在 iOS 上部署 3B 参数 LLM）。
2. **现有痛点**：
...
3. **核心矛盾**：现有方案在安全性和效率之间存在根本矛盾——足够安全的保护带来不可接受的计算/通信开销，而高效方案的安全性不充分。尤其是"基础能力窃取"威胁：攻击者可微调锁定模型以利用其泛化能力执行新任务。
4. **本文要解决什么？** 设计一个即插即用的保护方案，使锁定模型仅在 TEE 授权下正常工作，且计算和通信开销可忽略不计。
5. **切入角度**：利用置换矩阵的数学性质（$\pi\pi^T = I$）实现"锁+钥匙"机制，并通过列置换传播使授权信号自动流过所有后续层，避免重复 TEE 调用。
6. **核心idea一句话**：行置换锁定权重 + 列置换传播授权，将 LLM 保护的 TEE 交互从每层 2 次压缩到全模型仅 5 轮。
...
1. **保护协议（Protection Protocol）**:
   - 做什么：对输入处理层（QKV 投影、FFN 输入层）进行行置换使其"锁定"
...
2. **传播协议（Propagation Protocol）**:
   - 做什么：使授权信号自动从一层传播到下一层，避免每层都需 TEE 授权
...
3. **推理授权与 OTP 加密**:
   - 做什么：安全地生成首次授权的列置换特征，防止攻击者通过输入-输出对比推导 $\pi$
