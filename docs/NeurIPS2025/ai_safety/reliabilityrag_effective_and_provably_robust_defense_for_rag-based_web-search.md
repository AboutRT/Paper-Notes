...
**现有防御的局限**：
1. **RobustRAG**：基于关键词或 next-token 概率的多数投票，信息损失大，在良性场景下性能差，长文本生成任务上严重退化
2. **AstuteRAG / InstructRAG**：设计目标是良性性能而非对抗鲁棒性，遇到攻击时防御力不足
3. **共同盲点**：所有现有防御将检索到的文档视为无序集合，忽略了检索系统提供的排名/可靠性信号
