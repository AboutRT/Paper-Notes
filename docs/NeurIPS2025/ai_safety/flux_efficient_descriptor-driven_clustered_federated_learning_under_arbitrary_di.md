...
**本文要解决什么**：设计一个在训练和测试阶段均不需要任何先验知识的CFL框架，统一应对四种分布偏移（P(X)特征偏移、P(Y)标签偏移、P(Y|X)概念偏移、P(X|Y)概念偏移），同时保持与FedAvg相当的计算和通信效率。

**切入角度**：不在模型参数或损失值上做聚类（这些指标对分布偏移的区分力不足），而是直接从数据分布统计特征出发——提取紧凑的分布描述符作为聚类依据。

**核心idea一句话**：用客户端数据的矩统计量（均值+协方差）近似2-Wasserstein距离构建描述符，再用自适应DBSCAN自动聚类，将CFL问题分解为描述符提取、无监督聚类、局部分类器三个可独立优化的子问题。
