...
1. **领域现状**：LLM 越来越多地被用于社会和经济决策场景，其作为社会规划者的潜力备受关注。分配公平性——如何在多个个体间公平分配资源——是社会科学和算法决策的核心问题。
2. **现有痛点**：已有研究主要关注 LLM 在博弈论场景（囚徒困境、最后通牒等）中的行为，但对非策略性资源分配（社会规划者角色）的公平性研究几乎空白。
3. **核心矛盾**：公平性本身没有统一定义——公平性 (EQ)、无嫉妒性 (EF)、Rawls 最大化最小值 (RMM) 可能相互冲突，LLM 的偏好层级是否与人类一致？
4. **本文要解决什么？** LLM 在资源分配中是否与人类价值观对齐？行为受哪些公平公理支配？错位根源是什么？
5. **切入角度**：采用 Herreiner & Puppe (2010) 的经典人类实验数据集，设计不可分物品（有/无金钱）的分配实例，构造公平-效率之间的 tradeoff 场景。
6. **核心idea一句话**：人类优先追求平等性 (EQ)，而 LLM 优先追求经济效率 (PO/USW) 和无嫉妒性 (EF)——但当 LLM 从预设选项中选择（而非自行生成）时，GPT-4o 和 Claude 能正确识别最公平方案。
...
1. **实例设计与数据集**:
   - 做什么：采用 10 个精心设计的实例 I_1 - I_10，每个构造特定公平概念之间的 tradeoff
...
3. **选择题模式实验 (Section 4.1)**:
   - 做什么：不要求 LLM 生成方案，而是从 5 个预设选项中选择最公平的
...
4. **Persona / CoT / 意图实验 (Section 5)**:
   - 做什么：给 LLM 赋予特定公平概念的 persona，或使用 Chain-of-Thought 提示
