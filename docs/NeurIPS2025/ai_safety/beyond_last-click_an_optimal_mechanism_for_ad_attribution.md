---
title: >-
  [论文解读] Beyond Last-Click: An Optimal Mechanism for Ad Attribution
description: >-
  [NeurIPS 2025][AI安全][Last-Click] 从博弈论角度分析广告归因中 Last-Click 机制的策略操纵漏洞——平台可以通过篡改时间戳获取不公正的归因信用，提出 Peer-Validated Mechanism（PVM）——每个平台的信用仅取决于其他平台的报告（类比同行评审），理论证明 PVM 是占优策略激励兼容（DSIC）且在同质设置下最优，准确率从 34% 提升到 75%（2 平台）。
tags:
  - NeurIPS 2025
  - AI安全
  - Last-Click
...
**领域现状**：广告归因确定"哪个平台的广告导致了用户转化"。行业标准是 Last-Click（最后点击归因）——将转化信用给最后一个被用户点击的广告。

**现有痛点**：去重定向追踪（redirect-less tracking）中，平台可以策略性地篡改交互时间戳——将自己的点击时间报告得更晚以获取 Last-Click 信用。现有系统缺乏对这种操纵的防御。

**核心矛盾**：Last-Click 机制激励平台报告虚假（更晚的）时间戳——越晚报告越可能获得归因信用，导致所有平台都倾向于虚报。

**本文目标** 设计一种归因机制使平台没有动机篡改时间戳——即策略安全（DSIC）。

**切入角度**：同行验证——每个平台的归因信用不依赖于自身报告，只依赖于其他平台的报告。类似同行评审中论文分数不由作者决定。

**核心 idea**：每个平台的信用 = 其他平台的报告是否验证了该平台的交互 → 自己虚报毫无收益 = DSIC。
...
2. **DSIC 最优性证明（Theorem 5）**:
    ...
    - 功能：证明 PVM 在同质设置下是最优 DSIC 机制
    - 核心思路：在所有满足 DSIC 的归因机制中，PVM 达到最高归因准确率
    - 设计动机：不仅策略安全还尽可能准确

3. **准确率分析**:

    - 功能：量化 PVM vs LCM 的准确率
