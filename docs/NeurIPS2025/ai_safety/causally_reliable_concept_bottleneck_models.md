...
**代码**: 提交时附带补充材料  
**领域**: AI安全  
**关键词**: 概念瓶颈模型, causal reasoning, structural causal model, 可解释性, 公平性
...
**本文目标** (a) 如何将概念瓶颈按因果机制结构化？(b) 如何在无人类专家标注的情况下自动发现概念和因果图？(c) 如何在不牺牲精度的前提下提升因果可靠性？

**切入角度**：利用结构因果模型（SCM）将概念瓶颈组织为因果图，并通过 LLM + RAG 自动从非结构化背景知识中发现因果关系。

**核心 idea**：在概念瓶颈中嵌入因果图结构，用超网络自适应参数化结构方程，使模型推理过程与真实因果机制对齐。
...
1. **因果瓶颈（Causal Bottleneck）**:
    ...
    - 功能：替代 CBM 的扁平二部图结构，用 DAG 组织概念间的因果关系
