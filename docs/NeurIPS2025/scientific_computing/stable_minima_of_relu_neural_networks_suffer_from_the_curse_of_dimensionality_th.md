...
## 研究背景与动机
1. **领域现状**: 深度学习中过参数化模型存在无穷多全局极小值，梯度下降(GD)似乎能找到泛化良好的解。大量工作从动态稳定性(dynamical stability)角度研究 GD 的隐式偏置——稳定极小值是 GD 能"稳定收敛"到的那些极小值，其特征是 loss 曲率有界（即 Hessian 最大特征值 $\leq 2/\eta$）。
2. **现有痛点**: 已有工作（Mulayoff et al., 2021; Qiao et al., 2024）要么假设插值(interpolation)，要么局限于单变量输入。Qiao et al. (2024) 在单变量情形给出了良好的风险界，但仅限于数据支撑严格内部的区间，且未涉及多变量/高维情形。
3. **核心矛盾**: 平坦极小值在低维（d=1）下表现良好，但在高维情形下的行为完全未知。直觉上，稳定性/平坦性应该蕴含某种正则性从而帮助泛化——但这个正则性在高维下是否足够？
4. **本文要解决什么？**: 精确回答"两层过参数化 ReLU 网络的稳定极小值在高维非插值情形下的泛化表现如何"这一根本问题。
5. **切入角度**: 建立稳定极小值的函数空间刻画（加权变分范数），然后分别推导泛化间隙和 MSE 的上下界，并通过基于球面覆盖的新型极小极大下界构造揭示几何机制。
