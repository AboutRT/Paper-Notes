...
3. **核心矛盾**：谱初始化方法效果好但代价高，且成功机理不清。能否在不做 SVD 的情况下获得同等效果？

4. **本文要解决什么？** (a) 揭示谱初始化真正起作用的机制；(b) 设计一种无需 SVD 的高效替代方案。

5. **切入角度**：从**权重更新幅度**（weight update magnitude）的视角出发，$\nu[W_{\text{LoRA}}] = \frac{1}{mn}\|W_{\text{LoRA}}\|_F^2$，分析 LoRA 训练动态中各超参数如何通过幅度影响性能。

6. **核心 idea 一句话**：谱初始化的本质不是"知识保留"而是"幅度放大"，用确定性正交基 + 从预训练权重统计量推导的缩放因子即可复现其效果。
