---
description: "【论文笔记】F-Adapter: Frequency-Adaptive Parameter-Efficient Fine-Tuning in Scientific Machine Learning 论文解读 | NeurIPS 2025 | arXiv 2509.23173 | 参数高效微调 | 本文首次系统研究了科学机器学习中预训练大型算子模型(LOM)的参数高效微调(PEFT)，发现 LoRA 在傅里叶层中存在深度放大的近似误差下界，而 Adapter 保留了通用逼近能力；据此提出频率自适应 Adapter（F-Adapter），按频谱能量分配 Adapter 容量，在 3D Navier-Stokes 预测任务上仅调参不到 2% 即达到 SOTA。"
tags:
  - NeurIPS 2025
  - 参数高效微调
  - 傅里叶神经算子
...
## 研究背景与动机
1. **领域现状**: 参数高效微调(PEFT)在 NLP 和 CV 大模型适配中已被广泛验证（LoRA、Adapter、Prompt Tuning 等），但在科学机器学习(SciML)中尚未被系统探索。近年来大型算子模型(LOM)如 DPOT（10亿参数）通过多 PDE 预训练获得了强大的泛化能力。
2. **现有痛点**: LOM 参数量庞大（DPOT-H 达 1B），全量微调代价高昂（训练显存约 25-37 GB，参数量 100%）；而直接将 NLP/CV 中常用的 LoRA 迁移到 LOM 上效果很差，不同 rank 设置下 L2RE 均约为 0.63-0.64，远不如全量微调的 0.54。
3. **核心矛盾**: PDE 解的解流形具有宽频带、级联耦合的频谱特征，不同于自然语言/图像的低秩结构。LoRA 的线性低秩约束在傅里叶层中会产生深度放大的逼近误差下界，无法有效匹配 PDE 解的频谱特性。
4. **本文要解决什么？**: 能否设计出一种尊重 PDE 解频率自适应结构和物理先验的 PEFT 方法，使其既高效又保持频谱保真度？
5. **切入角度**: 从理论分析入手——证明 LoRA 的固有缺陷（Proposition 3.2 的深度放大下界）和 Adapter 的优势（Proposition 3.2 的指数衰减误差），再结合 PDE 解的能量频谱集中特性（低频主导能量），设计频率感知的 Adapter。
6. **核心idea一句话**: 低频频段包含 PDE 解的大部分能量，应分配更大的 Adapter 瓶颈维度；高频频段稀疏且噪声敏感，用轻量级 Adapter 即可。
