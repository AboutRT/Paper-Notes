...
**本文目标** 定义新的风险泛函 $\mathcal{R} = \text{性能损失} + c \cdot E[\tau]$，推导最优下界，设计匹配算法。

**切入角度**：两种风险泛函——$\mathcal{R}_{MI}$（误识别概率+成本）和 $\mathcal{R}_{SR}$（简单遗憾+成本），各自有不同的最优策略和相变行为。

**核心 idea**：将 BAI 统一为风险泛函最小化 → 推导含相变的下界 → DBCARE 动态预算消除算法匹配下界到对数因子。
...
1. **相变现象的发现**:
    ...
    - 功能：推导风险下界，发现当臂间差距小于临界值时最优策略是直接猜
