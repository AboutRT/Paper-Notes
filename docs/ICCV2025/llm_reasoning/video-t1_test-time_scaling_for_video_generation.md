---
description: "【论文笔记】Video-T1: Test-Time Scaling for Video Generation 论文解读 | ICCV 2025 | arXiv 2503.18942 | 测试时缩放 | 将LLM中的测试时缩放(TTS)思想迁移到视频生成领域，将TTS重新定义为从高斯噪声空间到目标视频分布的搜索问题，提出Tree-of-Frames (ToF)搜索算法实现高效的推理时计算扩展，在VBench上持续稳定提升各类视频生成模型的质量。"
tags:
  - ICCV 2025
  - 测试时缩放
  - 视频生成
...
1. **训练扩展成本极高**：视频生成模型的训练需要海量数据和计算资源，扩展代价远超LLM
2. **推理时可能性未被探索**：视频生成领域几乎没有系统性研究如何通过增加推理计算来提升质量
3. **视频生成的特殊挑战**：不同于LLM的token序列，视频需要同时保证空间质量和时间一致性；扩散模型的多步去噪过程使计算扩展更复杂
...
- **做什么**：随机采样N个高斯噪声，分别执行完整去噪生成N个视频，选择验证器评分最高的一个（Best-of-N策略）
...
- **做什么**：利用自回归模型的逐帧生成特性，在时间维度上引入推理时推理，通过树结构自适应扩展和剪枝视频分支
- **核心思路**：分三阶段生成——
