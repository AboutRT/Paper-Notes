...
1. **大域差距任务**：当下游任务与预训练域差异大时（如ImageNet预训练→医学影像），低秩不足以捕捉域特异性复杂性。实验表明，在VTAB-Specialized数据集上随着秩增加LoRA性能持续提升，而在Natural数据集上低秩即足够
2. **少样本学习**：数据极其有限时，固定低秩既可能不够（表达力不足）也可能过大（过拟合）
