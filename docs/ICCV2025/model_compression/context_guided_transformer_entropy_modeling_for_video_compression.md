---
description: "【论文笔记】Context Guided Transformer Entropy Modeling for Video Compression 论文解读 | ICCV 2025 | arXiv 2508.01852 | 视频压缩 | 提出Context Guided Transformer (CGT) 条件熵模型，通过时间上下文重采样器降低计算开销、依赖加权空间上下文分配器显式建模空间依赖关系，在视频压缩中将熵建模时间减少约65%，同时实现11% BD-Rate改进。"
tags:
  - ICCV 2025
  - 视频压缩
  - 熵模型
...
1. **Temporal Context Resampler (TCR)**:
   - 做什么：从多种类型和尺度的时间上下文中提取有效特征，生成固定长度的紧凑token序列
...
2. **Dependency-Weighted Spatial Context Assigner (DWSCA)**:
   - 做什么：显式建模空间上下文的位置依赖关系，确定最具信息量的上下文用于未解码token
...
3. **随机掩码代理任务**:
   - 做什么：解决教师网络中"当前帧已解码内容"在训练时无法预定义的问题
