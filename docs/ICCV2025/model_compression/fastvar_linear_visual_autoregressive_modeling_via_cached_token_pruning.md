...
1. **计算复杂度随分辨率剧增**：不同于 next-token 每步只处理一个 token，VAR 每步需处理整个 token map。Token 数量以 O(n²) 增长（n 为图像分辨率），注意力层达到 O(n⁴) 复杂度。
2. **大尺度步骤是延迟瓶颈**：即使启用 FlashAttention，推理延迟仍呈超线性增长。最后两个尺度步骤占总运行时间的 60%。
3. **无法扩展到高分辨率**：现有 VAR 模型在生成 2K 分辨率图像时因显存不足而 OOM，阻碍了实际应用。
4. **现有加速方法不适用**：扩散模型的加速方法（DeepCache、ToMeSD）无法直接用于 VAR；AR 的并行解码策略（speculative decoding）也不适用于 next-scale 范式。
