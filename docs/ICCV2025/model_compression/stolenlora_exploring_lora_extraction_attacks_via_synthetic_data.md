...
1. **LoRA 参数的脆弱性**：LoRA 仅微调少量低秩矩阵，模型的核心知识依赖公开的预训练基础模型。这意味着攻击者只需复制 LoRA 适配部分即可获得完整模型功能，门槛远低于传统全模型提取。
2. **现有提取方法的局限**：
...
3. **预训练模型公开可得**：大量预训练 ViT 模型公开发布在 Hugging Face 等平台，攻击者可轻松获取与受害者相同或相似的 base model，进一步降低了攻击门槛。
4. **研究空白**：传统模型提取研究关注整个模型的功能复制，但 LoRA 场景下只需提取紧凑的适配参数，这一新方向尚未被充分探索。
