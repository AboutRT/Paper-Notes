...
1. **特征依赖性高**：现有CV-based精炼器需要高维度、高区分度的特征来锐化相关体积（Correlation Volume），迫使使用重量级特征提取器
2. **误差校正范围有限**：CV-based精炼器只能校正局部窗口内的误差，处理更大误差需要更复杂的全局初始化
3. **联合优化不充分**：当误差超出局部窗口时，现有优化策略强迫精炼器修复自身无法处理的错误，而非将梯度传播到前级模块进行合理优化
