...
1. **One-hot 编码方法**（FaceFormer、CodeTalker 等）：用训练集中说话人的 ID 向量表示风格，推理时无法处理未见过的说话人，且同一音频使用不同 ID 会产生不同结果，不具泛化性。
2. **参考 3D 网格方法**（Imitator、Mimic、Yang et al.）：推理时需要额外提供一段目标说话人的 3D 面部运动序列来编码风格，但在实际应用中获取这些数据极不实用。
