...
1. **数据增强困难**：受弱监督限制，缺少单模态标签信息，模型难以学习大量段级事件组合
2. **跨模态噪声**：两个模态中的事件可能完全无关，简单的模态交互会引入噪声干扰
3. **长序列建模**：视觉输入序列token长度接近ViT-S的阈值（约2000），Transformer处理长序列效率低下，而单纯的Mamba在单帧图像识别上存在缺陷
