...
1. **频谱混叠问题（Spectrum Aliasing）**: 将现有联合扩散方法（如MultiDiffusion的逐步均值化）应用于频谱类音频生成时，重叠区域出现严重的时频分辨率下降和失真——表现为白色条带、视觉模糊和音频失真。这在频谱细节丰富的音频（如音景和协奏曲）中尤为明显。

2. **跨视图不一致性**: 在远距离子视图之间缺乏全局一致性指导，导致颜色/风格/音色不连贯。
