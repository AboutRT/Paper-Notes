...
1. **评估维度单一**：VLRewardBench 仅包含 Pairwise Comparison（两两比较），无法全面评估 VLRM 能力
2. **缺乏步骤级标注**：多数基准不包含推理步骤级别的标签
3. **聚焦语言领域**：现有 RM 基准（如 PRMBench、ProcessBench）主要针对纯文本，不适用于视觉语言场景
4. **挑战性不足**：简单的基准无法暴露 VLRM 的潜在缺陷
