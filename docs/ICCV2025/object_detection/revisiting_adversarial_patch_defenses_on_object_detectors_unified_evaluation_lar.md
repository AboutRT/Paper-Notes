...
1. **缺乏统一框架**：不同论文使用不一致的参数、攻击方法和补丁放置策略，无法公平比较
2. **度量不合适**：部分工作仅用补丁检测精度评估防御效果，但高检测精度不等于好的防御性能
3. **分析不全面**：忽视实时性、不同补丁尺寸/类型的影响、物理世界适用性等关键因素
4. **攻击不够强**：现有补丁数据集规模小、缺乏按检测器分类的覆盖
