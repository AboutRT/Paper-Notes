---
description: "【论文笔记】Visual Modality Prompt for Adapting Vision-Language Object Detectors 论文解读 | ICCV 2025 | arXiv 2412.00622 | 视觉提示 | 提出 ModPrompt，一种基于编码器-解码器的视觉提示策略，将视觉-语言目标检测器（如 YOLO-World、Grounding DINO）适应到红外和深度等新模态，同时保留零样本检测能力。"
tags:
  - ICCV 2025
  - 视觉提示
  - 模态适应
...
1. **全量微调（Full Fine-tuning）**：虽能提升目标模态的检测精度，但会导致灾难性遗忘，丧失零样本检测能力
2. **传统视觉提示（Visual Prompt）**：对每张图像施加相同的线性提示变换（如固定 patch、随机 padding），不依赖输入图像内容，在大模态偏移场景下效果有限
3. **图像翻译方法（如 HalluciDet、ModTr）**：仅适用于传统检测器，未探索视觉-语言检测器的跨模态适应；且一些方法会丢失预训练知识
...
2. **MPDR（模态提示解耦残差）**:
   - 做什么：在文本嵌入空间进行高效的模态适应，同时保留原始零样本知识
...
3. **检测器无关设计**:
   - 做什么：使 ModPrompt 可灵活集成到不同架构的视觉-语言检测器中
