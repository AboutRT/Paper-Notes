...
1. **封闭集检测的局限**：大多数研究聚焦于固定类别集的闭集检测，模型对新类别和新场景泛化性差
2. **现有开放词汇方法不足**：已有遥感OVD方法（DescReg、CastDet、OVA-DETR）受限于小规模数据集，无法应对遥感特有的旋转目标检测、小目标检测等挑战
3. **速度与精度矛盾**：深度跨模态融合方法（如Grounding DINO）精度高但推理慢，对大规模遥感图像分析不实用
