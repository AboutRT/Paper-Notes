...
1. **伪造攻击（Forgery Attack）**：生成视觉相似的副本同时移除水印，使版权验证失败$\mathcal{V}(x^*) \neq w$
2. **模糊攻击（Ambiguity Attack）**：替换为新水印制造所有权争议$\mathcal{V}(x^*) = w^*$
...
1. **显存爆炸**：每个时间步需计算梯度$\frac{\partial x_{t-1}^*}{\partial x_t^*}$，10步需>100GB显存
2. **过度平滑**：跳步估计梯度导致图像模糊
3. **噪声输出**：高扰动产生噪声而非有意义的语义变化
