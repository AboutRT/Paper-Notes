...
1. **重复模式(Repetitive Patterns)**: 激活最大化倾向于在图像中重复出现相同模式以反复刺激目标神经元，导致"像万花筒一样"的视觉效果
2. **伪影(Artifacts)**: 生成的图像包含不自然的颜色、纹理等伪影
3. **无关特征(Irrelevant Features)**: 可视化中混入了与目标神经元无关的背景特征（如鸟喙检测器的可视化中出现草地）
