...
1. **单神经元分析**（NetDissect、CLIP-Dissect等）：将概念关联到单个神经元，忽视了表征的分布式本质——概念通过多个神经元跨层协作编码
2. **VCC**（Visual Concept Connectome）：用概念激活向量（CAV）分析层间连接，但与网络结构不对齐，无法精确定位概念在哪里出现
3. **ADVC**（Rajaram等人的方法）：使用梯度×激活值的跨层归因迭代发现电路，但仅构建一个与类别标签相关的统一电路，缺乏概念级细粒度分析
