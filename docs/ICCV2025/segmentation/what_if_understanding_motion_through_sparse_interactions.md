...
现有运动预测方法的问题：
1. **密集视频预测**（如 diffusion video generation）必须选择*一种*轨迹输出，忽略了运动的多模态性——即使产出逼真帧，也不代表理解了物理过程
2. **确定性光流预测**（如 RAFT）仅估计给定两帧间的运动，需要未来帧，无法预测未来
3. **现有条件运动生成**（如 DragAPart, PuppetMaster）在 RGB 空间直接合成结果，底层运动表征难以访问，且不提供不确定性估计
