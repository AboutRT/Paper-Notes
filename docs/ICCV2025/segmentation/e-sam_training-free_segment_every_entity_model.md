...
**领域现状**：实体分割（Entity Segmentation, ES）旨在分割图像中所有视觉可辨别的实体而不依赖预定义类别标签。与传统语义/实例/全景分割不同，ES 是类别无关的，更贴近人类视觉感知方式。代表方法有 EntitySeg、CropFormer 等，但它们依赖大量标注数据、高训练成本，且泛化能力有限。
