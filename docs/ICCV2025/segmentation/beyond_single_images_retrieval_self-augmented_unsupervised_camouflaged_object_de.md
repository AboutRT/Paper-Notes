...
1. **领域现状**：伪装目标检测（COD）旨在从高度相似的背景中分割出目标物体。主流全监督方法依赖密集的像素级标注，标注一张图像可能耗时一小时。弱监督和半监督方法减轻了标注负担但仍需部分标注。
2. **现有痛点**：(a) 无监督方法（TokenCut, MaskCut, ProMerge 等）主要基于单图像内特征相似度来区分前背景，但伪装目标和背景的特征高度相似，导致单图像方法效果差；(b) 基于提示的方法用 SAM + 任务提示词，仍需某种形式的监督且对 COD 的特定上下文理解有限；(c) 基于扩散模型或多模态 LLM 生成伪标签的方法（GenSAM, ProMac）需要数天时间且 GPU 开销大。
3. **核心矛盾**：在单张图像中，伪装目标和背景的 DINOv2 特征非常接近（t-SNE 上几乎重叠）——仅基于图像内部相似度无法有效区分。但如果看整个数据集，前景目标与前景原型库的相似度是高于与背景原型库的。
4. **本文要解决什么？** 如何在完全无标注的情况下利用数据集级别的上下文信息来区分伪装前景和背景。
5. **切入角度**：从数据集本身挖掘原型——通过先粗后精的策略（先聚类得粗 mask，再检索提纯原型），构建高质量的前景/背景原型库，然后对每张图像用 KNN 检索分类每个特征为前景或背景。
6. **核心idea一句话**：不靠单图像内的相似度，而是靠整个训练集的原型库+KNN检索来区分伪装目标与背景，实现无监督 COD。
...
1. **Clustering-then-Retrieval (CR) — 原型库构建**:
   - 做什么：从无标注的 COD 数据集中构建高质量的前景/背景原型库
   - 核心思路：
