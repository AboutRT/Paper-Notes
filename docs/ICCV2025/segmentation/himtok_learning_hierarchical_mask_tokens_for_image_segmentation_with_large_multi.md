...
1. **边界点序列**（如PolyFormer、VistaLLM）：将掩码表示为多边形顶点序列，但有限的顶点数难以表示复杂形状和多区域
2. **隐藏状态+掩码解码器**（如LISA、PixelLM、PSALM）：LMM输出特殊token的隐藏状态，再由额外的SAM/Mask2Former解码。存在三个局限：
   - LLM对精确空间定位学习不充分
   - 掩码在输入输出间表示不一致（特殊token仅作标识，丢失了对应的hidden state信息）
   - 架构复杂，掩码解码器需再次使用原图
3. **图像生成式**（VQ-GAN量化为2D token）：过于冗余、性能不够竞争力
