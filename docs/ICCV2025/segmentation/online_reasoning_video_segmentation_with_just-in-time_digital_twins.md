...
现有 RS 方法的三大局限：
1. **推理能力受限**：依赖多模态 LLM 同时处理感知和推理，在需要多步推理或复杂空间/时间关系的查询上表现差。LLM 必须将丰富视觉信息压缩为有限 token，丢失细粒度空间和时间细节
2. **维护成本高**：需要 LLM 微调，随着 LLM 快速迭代，需反复重新调参以避免灾难性遗忘
3. **不支持在线处理**：主要为静态图像或离线视频设计，无法处理实时视频流
