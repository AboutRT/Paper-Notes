...
1. **Dense-token 方法**（如 InterFormer）：将点击编码为密集提示 token，空间感知能力强，分割精度高，但计算量大，在 CPU 上推理极慢（每步 >1 秒）
2. **Sparse-token 方法**（如 SAM）：使用稀疏提示 token 进行高效交叉注意力，推理快，但牺牲了空间感知和边界精度
