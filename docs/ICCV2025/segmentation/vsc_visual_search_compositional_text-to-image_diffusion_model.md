...
1. **语言结构理解不足**：CLIP 倾向于将文本表示为"概念袋"（bag-of-concepts），忽略层次化关系和修饰词
2. **交叉注意力图错误**：扩散模型 UNet 的交叉注意力图本身就是不准确的，属性 token 的注意力分布与对象不对齐
3. **复杂度扩展性差**：现有推理时注意力控制方法（如 Attend-and-Excite）随绑定对数量增加性能急剧下降
