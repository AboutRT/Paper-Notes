...
1. **隐式共现依赖脆弱**：现有方法（如SSLBKD、CorruptEncoder）依赖后门触发器与目标对象在随机增强视图中的共现来建立关联，但这种隐式关联不够可靠
2. **缺乏噪声压缩**：投毒图像中原始语义特征（如"熊猫""树木"等非触发器特征）会主导表征空间，干扰后门触发器的有效性
...
1. **噪声对齐目标的理论分解**:
   - 做什么：将Oracle攻击的目标 $\mathcal{L}_{\text{align}} = \mathbb{E}[1 - \cos(f(\mathbf{x}_s \oplus \mathbf{p}), f(\mathbf{x}_r))]$ 分解为两个正交成分
...
3. **离线布局优化（核心创新）**:
   - 做什么：不需要控制训练过程，通过预先优化投毒样本的空间布局来模拟噪声对齐
