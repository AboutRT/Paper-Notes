...
**代码**: 无  
**领域**: AI安全  
**关键词**: 联邦学习, 后门攻击, Scaffold, 控制变量, Non-IID
...
1. **Scaffold 的安全性未被研究**：虽然 FL 后门攻击已被广泛研究，但几乎所有现有工作针对 FedAvg 等标准聚合算法，而 Scaffold 引入的控制变量机制创造了全新的攻击面——这一安全隐患被完全忽视
2. **non-IID 场景对攻击者不利**：攻击者不了解全局数据分布，直接注入后门会导致模型在良性任务上性能严重下降（如 PTA 从 85% 降至 25%），容易被发现
3. **后门持久性差**：攻击者停止参与后，良性更新会逐渐冲刷掉后门功能——现有基线在停止攻击 20 轮后后门精度即降至 50% 以下
...
- **做什么**：在 non-IID 场景下，攻击者仅持有部分类别的数据。使用 GAN 生成攻击者缺失的其他类别样本
...
#### 3. **控制变量约束的后门优化**
- **做什么**：利用全局控制变量 $c$ 预测全局模型未来的收敛方向，优化后门使其在未来轮次中仍然有效
- **核心公式**：
