...
**代码**: https://github.com/wangruoyu02/NoiseQuery  
**领域**: 扩散模型 / 图像生成  
...
1. **领域现状**：扩散模型在 T2I 生成中取得了显著成功，但仍面临生成内容与文本 prompt 不一致的问题。现有增强方法包括：微调 UNet（DPO）、增强文本编码器（LaVi-Bridge）、改进推理策略（CFG++）、以及优化初始噪声（ReNO）。噪声优化方向近年受到关注，因为同一初始噪声生成的图像在单个模型内常表现出高相似性。
...
3. **核心矛盾**：初始噪声对生成结果有重大影响（不仅在单模型内，跨模型也是如此），但现有方法要么忽视这一影响，要么以高成本的方式利用它。如何高效、通用地利用初始噪声中蕴含的生成倾向？

4. **本文要解决什么？** (a) 揭示初始噪声的跨模型生成一致性现象及其理论原因；(b) 构建可复用的噪声库，实现高效的噪声检索；(c) 利用噪声控制文本难以描述的低级视觉属性。

5. **切入角度**：从扩散模型的前向加噪过程分析，有限步数的噪声调度器无法完全消除原始图像信息（如 Stable Diffusion 的最终步仍保留 $\sqrt{\bar\alpha_T} = 0.068265$ 的原始信号），这导致模型在训练时"学会了"利用噪声中的残留信号作为快捷方式。推理时，模型继续依赖这些隐式知识来"解读"纯高斯噪声，使得初始噪声成为一个"沉默的助手"。

6. **核心 idea 一句话**：利用无条件生成（空 prompt）来揭示初始噪声的隐式生成倾向（generative posterior），预建大规模噪声库并提取多粒度特征索引，推理时通过特征匹配快速检索最优噪声，实现语义和低级属性的双重控制。
...
1. **隐式生成倾向的理论分析**:
   - 做什么：从理论上解释为什么初始噪声会影响生成结果，且这种影响是跨模型一致的
...
3. **多粒度特征索引与匹配**:
   - 做什么：为每个噪声样本的 generative posterior 提取多种特征，支持多目标检索
