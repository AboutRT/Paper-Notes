...
1. **稀疏/稠密控制的两难**：部分模型擅长稀疏路径点导航（GMD、Trace and Pace），部分擅长稠密轨迹跟随（TLControl），但难以同时支持两者
2. **精度与质量的矛盾**：OmniControl 支持任意关节控制但精度不高（平均误差 3.38cm）；TLControl 通过测试时优化达到高精度（1.08cm）但牺牲了生成质量（FID 0.271）
3. **方法局限于扩散模型**：现有可控运动生成方法几乎全部基于扩散模型，存在运动空间冗余、计算开销大、生成速度慢等问题
4. **缺乏零样本泛化**：现有方法无法在推理时适应任意目标函数
