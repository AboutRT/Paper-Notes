...
1. **聚合偏好数据集**（如 Pick-a-Pic）：收集用户对图像对的评分，但反映的是**群体平均偏好**
2. **身份定制数据集**（如 DreamBooth）：支持特定人/物的概念注入，但不处理**偏好风格**
3. **缺少个体级偏好**：没有大规模记录单个用户与其定制模型交互的数据
4. **现有个性化方法**（如 ViPer）限于零样本，无法利用用户间相似性
