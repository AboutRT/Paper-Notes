...
1. **OOD 检测的挑战**：部署在开放世界中的深度网络不可避免地遇到 OOD 样本，准确识别至关重要。CLIP-based 方法（如 NegLabel）通过引入负标签显著提升了 OOD 检测，但仍难以处理靠近 InD/OOD 边界的困难样本。

2. **CLIP 的局限**：图像在特征空间中通常比标签更密集，导致边界附近的 OOD 样本更倾向于与 InD 标签对齐，CLIP 无法建立清晰的语义分界。
...
4. **核心思路**：用基础模型（MLLM + 扩散模型）生成高质量、靠近边界的 OOD 样本来微调 CLIP。
