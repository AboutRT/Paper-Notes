---
description: "【论文笔记】PersonalVideo: High ID-Fidelity Video Customization without Dynamic and Semantic Degradation 论文解读 | ICCV 2025 | arXiv 2411.17048 | 视频定制化 | 本文提出 PersonalVideo 框架，通过混合奖励监督（身份一致性奖励+语义一致性奖励）直接对生成视频施加反馈，消除了传统方法中 T2I 调优与 T2V 推理之间的分布差距，在保持高身份保真度的同时避免了运动动态和语义跟随的退化。"
tags:
  - ICCV 2025
  - 视频定制化
  - 身份保持
...
1. **分布不一致**：T2I 模型与 T2V 模型的先验分布存在偏差。在 T2I 上用静态图像重建来学习身份，会显著偏移 T2V 模型的视频先验，导致生成的视频趋于静态（动态退化）且无法跟随提示词（语义退化）。

2. **身份保真度不足**：由于调优阶段是在图像上做重建而推理阶段是在视频上生成，这种差距也让身份注入的效果打折扣。人眼对面部特征高度敏感，要求更高的一致性。

3. **数据需求过多**：为了在保持动态的同时注入身份，传统方法通常需要更多参考图片甚至额外的视频输入，给用户带来极大不便。
...
2. **语义一致性奖励（SCR）**:
   - 做什么：维持原始 T2V 模型的语义分布，防止动态和语义退化
...
3. **模拟提示增强（Simulated Prompt Augmentation）**:
   - 做什么：用 LLM 生成 50 个与参考图无关的多样化提示词，在训练中随机采样
