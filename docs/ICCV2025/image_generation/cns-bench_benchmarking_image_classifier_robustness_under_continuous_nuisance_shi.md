...
1. **人工采集数据**（如 OOD-CV）：费时费力，难以控制干扰因素，且不同干扰之间耦合严重
2. **合成腐蚀**（如 ImageNet-C）：支持连续严重度，但仅限于简单的像素级扰动，无法反映真实世界分布偏移
3. **渲染管线**（如 3D 资产渲染）：需要大量 3D 模型，难以扩展到大规模类别
4. **基于扩散模型的生成**（如 Dataset Interfaces）：能生成逼真图像，但仅支持**二值**偏移（有/无），无法捕捉连续变化
