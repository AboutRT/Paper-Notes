...
1. **任务特定方法缺乏泛化**：传统方法（如 ZDCE++、AOD-Net）针对特定退化设计网络，无法推广到其他退化类型
2. **监督统一方法受限于闭集**：AirNet、PromptIR、DiffUIR 等在特定数据集上训练，遇到训练中未见的退化类型时性能显著下降
3. **现有后验采样方法不稳定**：GDP 等方法依赖像素级扩散和显式退化建模（y = Ax + B），对复杂真实退化不适用
