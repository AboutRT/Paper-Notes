...
1. **真实多图数据集稀缺**：训练编码器方法需要同一对象在不同姿态、背景、光照下的多张图像，但从互联网收集这样的大规模数据集几乎不可能——真实图像通常没有对象身份标注。

2. **单图训练的局限**：现有编码器方法（如 IP-Adapter、BLIP-Diffusion）大多在单图或多视角（固定背景）数据集上训练。为防止过拟合到参考图的姿态或背景，它们被迫使用紧凑的特征空间，这牺牲了身份保持能力。

3. **优化方法的高成本**：DreamBooth、Textual Inversion 等优化方法虽然效果好，但每个新对象都需要数分钟的微调，计算成本高且速度慢。

4. **现有合成数据质量不足**：JeDi 等工作也尝试创建合成数据集，但仅依赖文本提示来生成一致对象，缺乏显式的一致性约束，导致数据质量有限。
