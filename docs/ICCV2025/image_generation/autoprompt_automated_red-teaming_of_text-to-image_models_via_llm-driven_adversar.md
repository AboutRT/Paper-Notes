---
description: "【论文笔记】AutoPrompt: Automated Red-Teaming of Text-to-Image Models via LLM-Driven Adversarial Prompts 论文解读 | ICCV 2025 | arXiv 2510.24034 | Red-Teaming | 本文提出APT（AutoPrompT），一种基于LLM的黑盒红队测试框架，通过\"优化-微调\"交替训练管线和双规避策略，自动生成可被人类阅读且不被内容过滤器拦截的对抗性后缀，有效突破T2I模型的安全机制，并具有零样本跨提示迁移能力。"
tags:
  - ICCV 2025
  - Red-Teaming
  - Text-to-Image
...
1. **白盒依赖**：大多数方法（Ring-A-Bell、P4D、UnlearnDiffAtk）需要目标模型的梯度信息，在实际场景中不可行
2. **语义不可读**：基于离散优化的方法生成的对抗提示是无意义的字符拼接（"gibberish"），容易被困惑度过滤器检测和拦截
3. **包含违禁词**：生成的对抗提示经常显式包含黑名单中的敏感词汇，被词过滤器直接拦截
...
1. **对抗后缀优化**:
   - 做什么：为给定的良性提示 $x$ 生成对抗后缀 $S_T = [s_1, \ldots, s_T]$，使拼接后的提示 $[x, S_T]$ 诱导T2I模型生成不安全内容
   - 核心思路：
...
2. **双规避策略**:
   - 做什么：使生成的对抗提示同时绕过困惑度过滤器和黑名单词过滤器
   - 核心思路：
...
3. **后缀生成器微调**:
   - 做什么：用优化得到的高质量后缀微调LLM，使其逐步学会直接生成有效后缀
