...
1. **忽略标注多样性**：大多数方法将多位标注者的帧级重要性评分取平均作为训练目标，这种回归式方法会丢失不同标注者的独立视角。例如，若一半标注者选择视频前1/4的片段，另一半选择后1/4的片段，简单平均会让两部分获得相似的中间分数，无法区分两种有效的摘要方式。
2. **确定性输出**：给定视频只能生成一个摘要，无法反映多种合理的摘要可能。
3. **评估指标不完善**：对背包（knapsack）步骤的分析不够深入，F1 分数对视频分割过度敏感。
