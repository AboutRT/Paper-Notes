...
description: >-
  [ICCV 2025][图像生成][模型压缩] 首次将预训练的dense DiT（FLUX.1 12B）转换为Mixture-of-Experts稀疏结构，通过Taylor度量专家初始化+MoE层替换FFN+Mixture-of-Blocks动态选择block+多步蒸馏pipeline，激活参数减少60%的同时保持原始生成质量，全面超越剪枝方法。
tags:
  - ICCV 2025
  - 图像生成
  - Transformer
...
1. **Taylor度量专家初始化 + MoE层设计**：
    ...
    - 做什么：将DiT每个block的FFN替换为包含1个shared expert和12个normal experts的MoE层（配置1S12E2A），每个token只激活shared expert + top-2 normal experts，FFN激活参数压缩62.5%。
...
2. **Mixture of Blocks (MoB)**：
    ...
    - 做什么：将连续的transformer block分组，每组通过router动态选择激活其中的一部分block，直接减少推理时的模型深度。
...
3. **多步蒸馏与归一化特征权重**：
    ...
    - 做什么：设计归一化的block feature loss权重，平衡不同层特征的尺度差异，确保各层参与有效学习。
