...
1. **数据集缺失**: 现有舞蹈数据集要么只有身体动作（AIST++），要么缺少面部表情（FineDance），且大多来自视频姿态估计，精度不足
2. **建模困难**: 身体、手部、面部之间存在复杂的层次化依赖关系，现有方法无法有效捕捉跨部位的协调运动
3. **跨模态对齐差**: 现有方法直接将低级音频特征（如 Librosa、Jukebox 特征）输入运动生成器，缺乏显式的音乐-舞蹈对齐机制
