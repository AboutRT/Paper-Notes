...
1. **生成模型与分类器的解耦问题**：标准流程中生成模型和分类器是独立的。分类器高度依赖生成样本的质量，两者之间存在知识传递的瓶颈。
2. **生成样本质量退化**：即使 SOTA 扩散模型也无法精确建模数据分布，反复用生成数据训练分类器会导致分类器性能持续下降。
3. **可塑性-稳定性权衡**：现有方法要么过于稳定（无法学新任务），要么过于可塑（快速遗忘旧任务）。
