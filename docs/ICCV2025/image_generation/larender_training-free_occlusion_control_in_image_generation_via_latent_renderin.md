...
1. **文本到图像方法**（SDXL、FLUX）：只能通过 prompt 描述遮挡关系（如 "a dog behind a cat"），但即使最先进的 FLUX 也无法在复杂场景中准确控制遮挡
2. **布局到图像方法**（MIGC、3DIS）：可以控制物体位置，但无法显式处理遮挡关系。同一个 bounding box 布局在不同遮挡关系下是相同的，因此布局信息不足以确定遮挡
3. **训练条件模型**：需要图像-遮挡标注配对数据，采集成本高昂
