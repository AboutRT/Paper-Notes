...
**领域**: 图像/视频生成安全  
**关键词**: 文本到视频生成, 后门攻击, 冗余信息利用, 时空组合, 内容安全
...
这些冗余信息分为两类：
1. **静态冗余**：单帧内多余的空间元素（如背景物体、过度渲染的细节）
2. **动态冗余**：时间维度的多余过渡（如未指定的运动序列、场景演变）
...
1. **策略一：时空组合（Spatio-Temporal Composition, STC）**:
   - 做什么：将恶意内容沿时间维度分解，分散嵌入不同帧的冗余元素中
...
2. **策略二：语义概念转换（Semantic Concept Transition, SCT）**:
   - 做什么：在视频冗余元素（如背景广告牌）上引入语义概念的时序过渡来传达恶意信息
