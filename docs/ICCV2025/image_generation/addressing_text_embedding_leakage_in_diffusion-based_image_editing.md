...
### 属性泄露的两种类型
1. **目标外泄露（TEL, Target-External Leakage）**：编辑目标对象时意外影响非目标区域。例如将"红色甜椒"编辑为"金色苹果"时，旁边的"绿色甜椒"也变成金色
2. **目标内泄露（TIL, Target-Internal Leakage）**：多对象编辑时，一个目标对象的属性意外影响另一个目标对象。例如编辑"黄色甜椒→红色南瓜"时，另一个应为"金色苹果"的区域呈现出红色南瓜的混合属性
