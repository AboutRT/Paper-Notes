...
1. **评估维度单一**：大多数工作仅关注目标概念是否被移除 + 图像质量是否保持，忽略了副作用
2. **prompt 数量不足**：之前最多仅使用数十到数百条 prompt，无法充分测试遗忘效果
3. **概念覆盖有限**：不同方法使用不同概念，缺乏统一比较基础
4. **缺少重要维度**：如精确性（是否过度遗忘相关概念）、多语言鲁棒性（非英文 prompt 是否仍能触发）、实际效率等
...
2. **VLM-based 概念检测框架**:
   - 做什么：对无专用分类器的概念（IP、Style），使用 VLM 进行通用概念检测
