...
1. **ERM 基线过强**：Gulrajani & Lopez-Paz 已证明大多数复杂域泛化方法在严格评估下甚至不如简单的 ERM（经验风险最小化）
2. **需要域标签的方法（CORAL, SagNet, DANN 等）**：依赖训练时的显式域标签，在域标签不可用或有噪声时不适用
3. **数据增强方法**：用文生图模型做数据增强需要微调扩散模型，成本高且需要测试数据
4. **现有伪域方法**：DA-ERM 需要训练专门的域原型网络，AdaClust 使用的特征空间域分离能力有限
...
- **做什么**：从冻结的预训练扩散模型中提取样本特征，通过无监督聚类发现数据中隐含的域结构
...
- **做什么**：将伪域表征从 $\Psi$ 空间变换到分类器特征空间 $\Phi$，然后拼接用于训练
...
- **做什么**：系统分析 6 种预训练模型在 7 个数据集上的域分离能力（域 NMI vs 类别 NMI）
