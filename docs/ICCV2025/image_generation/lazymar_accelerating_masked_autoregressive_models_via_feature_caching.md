...
1. **领域现状**：自回归（AR）图像生成模型（如VQGAN+Transformer）通过逐token生成图像，但序列依赖导致效率低。Masked Autoregressive（MAR）模型（如MaskGIT、MAR）通过并行预测被掩码的token来加速，同时保持甚至超越AR模型的生成质量。

2. **现有痛点**：MAR模型使用双向注意力（bidirectional attention），虽然可以并行解码多个token，但双向注意力要求同时访问所有token，**无法使用传统的KV Cache**。AR模型的KV Cache之所以有效，是因为causal attention下之前token的KV不变。MAR的双向注意力下每个token的表示都可能因新token的解码而改变，传统KV Cache完全不适用。这导致MAR的推理效率出人意料地不如预期。

3. **核心矛盾**：MAR的并行解码优势被双向注意力的缓存失效所抵消，急需为MAR设计新的缓存机制。

4. **本文要解决什么？** 为MAR模型设计适配的特征缓存机制，在保持生成质量的前提下大幅加速推理。

5. **切入角度**：通过分析MAR解码过程中的计算冗余，作者发现两种关键冗余：(a) 大部分token在相邻解码步中的特征几乎不变（token冗余）；(b) classifier-free guidance中条件输出与无条件输出的差值在相邻步间变化极小（条件冗余）。

6. **核心idea一句话**：利用MAR解码过程中的token冗余和条件冗余，设计token cache和condition cache实现免训练的即插即用加速。
...
2. **条件冗余分析与Condition Cache**:
   - 做什么：跳过classifier-free guidance中无条件分支的完整计算
   - 核心思路：
