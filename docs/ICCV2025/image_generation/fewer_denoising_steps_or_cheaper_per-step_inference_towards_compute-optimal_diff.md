...
1. **领域现状**：扩散模型在图像/视频生成任务上取得巨大成功，但其迭代去噪的本质和复杂的模型架构导致计算密集，限制了资源受限平台上的部署。
2. **现有痛点**：
...
3. **核心矛盾**：在不微调的 post-training 部署场景下，应该选择更少的去噪步数还是更便宜的每步推理？这个问题对研究者和工程师都至关重要，但缺乏明确答案。
4. **本文要解决什么？**
   - 提出统一框架 PostDiff 同时从输入和模块两个层面减少冗余
   - 通过受控实验系统比较两种策略
   - 发现并解释混合分辨率去噪的"低分辨率增强低频→提升最终质量"现象
5. **切入角度**：早期去噪步主要生成低频语义布局，不需要高分辨率；后期步骤添加高频细节才需要高分辨率。这种阶段性特征可以被利用。
6. **核心idea一句话**：早期步用低分辨率去噪（增强低频+省算力），后期切换到高分辨率精炼细节，配合模块缓存，实现"降低每步成本比减少步数更有效"。
...
1. **混合分辨率去噪（Mixed-Resolution Denoising）**
   - 做什么：在去噪过程中动态切换输入分辨率——早期步用低分辨率，后期步用高分辨率。
...
2. **"低分辨率增强"现象的可视化验证**
   - 做什么：逐步可视化去噪过程中每一步的 CLIP Score 变化。
