...
1. **领域现状**：图像特征匹配是 3D 重建、视觉定位等任务的基础。近年来，利用预训练视觉基础模型（DINOv2、CLIP）提升匹配泛化能力成为主流范式，RoMa、OmniGlue、CasMTR 等方法取得显著进展
2. **现有痛点**：
...
3. **核心矛盾**：基础模型的强大泛化能力 vs 其训练目标（单图理解/全局语义）与匹配任务需求（跨图理解/实例级细节）的根本不匹配
4. **本文要解决什么？**：(1) 什么样的基础模型更适合特征匹配？(2) 如何设计跨图像交互机制使基础模型的单图理解能力转化为跨图理解？
5. **切入角度**：生成式扩散模型的内部表征天然包含每个对象和实例的独特外观和结构信息（DIFT 研究已证实），且扩散模型的条件机制（prompt）可作为跨图交互的天然通道
6. **核心idea一句话**：用扩散模型替代对比学习模型作为特征提取器以保留实例细节，并利用扩散模型的 prompt 机制设计跨图交互模块（CIPM）生成个性化提示引导特征提取
...
1. **扩散模型作为特征提取器**:
   - 做什么：使用冻结的 SD 2-1 UNet 直接处理干净图像（无需加噪去噪），从上采样块提取特征
...
2. **跨图像交互提示模块（CIPM）**:
   - 做什么：为每张图像生成包含对方图像信息的个性化 prompt，引导扩散模型的特征提取
