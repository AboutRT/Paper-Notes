...
### 核心挑战
1. **全局信息缺失**：Patch-level token 只携带局部信息，AR 模型难以在逐步生成时维持全局一致性
2. **因果注意力的局限**：AR 模型使用因果注意力逐 token 预测，无法看到后续 token，导致长程依赖困难
3. **零样本补全质量差**：当 LlamaGen 等模型被要求补全图像下半部分时，生成内容与上半部分语义不一致（如鱼变成"鱼-鸟"混合体）
