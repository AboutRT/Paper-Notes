---
description: "【论文笔记】Multi-turn Consistent Image Editing 论文解读 | ICCV2025 | arXiv 2505.04320 | multi-turn editing | 提出基于 flow matching 的多轮图像编辑框架，通过双目标 LQR 引导和自适应注意力机制，有效抑制多轮编辑中的误差累积，在保持内容一致性的同时实现灵活可控的迭代编辑。"
tags:
  - ICCV2025
  - multi-turn editing
  - flow matching
...
1. **Diffusion-based inversion 方法**（如 DDIM Inversion、Null-text Inversion）：反演精度不够高，多轮编辑下误差累积尤为严重
2. **Flow matching 单步编辑**（如 RF-Solver、FireFlow）：虽然二阶 ODE 求解器能降低单步截断误差，但多轮累积误差仍然显著
3. **RF-Inversion**：采用单目标 LQR 以源图像为参考，单步编辑效果好，但多轮编辑时仅参考上一轮结果、逐渐偏离原始图像
4. **注意力控制方法**（如 Prompt2Prompt、MasaCtrl）：在 FLUX 等 DiT 架构中的适用性尚未充分探索
