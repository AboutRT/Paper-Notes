...
1. **扩散模型 SR 的效率问题**：扩散模型大幅提升了超分的感知质量，但大量迭代采样导致推理效率低下，尤其处理高分辨率图像时计算开销巨大。

2. **统一采样的次优性**：现有加速方法（条件蒸馏、重定义扩散过程）统一减少所有区域的采样步数，忽略了不同区域重建难度的差异——结构简单的区域几步即可重建，而纹理丰富的区域需要更多步骤。

3. **文本提示的局限**：在 SR 任务中，文本提示与图像内容的对齐度远低于 T2I 任务，局部纹理恢复更需要视觉级条件信息而非文本描述。

4. **核心观察**：如图 1(a) 所示，简单 patch 仅需 2 步即可高质量重建，而复杂 patch 需要 15 步。
