---
description: "【论文笔记】Learning Pixel-adaptive Multi-layer Perceptrons for Real-time Image Enhancement 论文解读 | ICCV 2025 | arXiv 2507.12135 | 图像增强 | 提出 BPAM 框架，将双边网格的空间建模能力与 MLP 的非线性映射能力相结合，通过为每个像素动态生成独特的微型 MLP 参数实现高质量、实时的图像增强。"
tags:
  - ICCV 2025
  - 图像增强
  - 双边网格
...
1. **端到端深度学习方法**：以 Restormer、NAFNet 为代表，增强质量高但计算开销大，难以实时处理高分辨率图像

2. **混合方法**：将深度学习与高效物理模型结合
...
3. **全局 MLP 方法**：如 CSRNet 用 MLP 替代传统色彩变换，但参数在整個图像上共享，面对局部变化的色彩/光照时力不从心，需要很深的隐层才能获得跨区域的泛化能力
...
1. **像素自适应 MLP 学习（Pixel-Adaptive MLP）**:
   - 做什么：为图像中每个像素生成一个独特的三层 MLP（3-8-3），将颜色变换从全局共享参数变为像素级自适应
...
2. **网格分解策略（Grid Decomposition）**:
   - 做什么：将双边网格拆分为多个子网格，每个子网格搭配独立的引导通道
