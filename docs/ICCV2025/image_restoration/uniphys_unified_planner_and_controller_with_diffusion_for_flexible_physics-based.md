...
1. **分层框架的域间隙**：主流方法将控制分为两级——高层扩散模型规划器生成运动学目标，低层 RL 控制器跟踪执行。但规划器的运动学输出与物理约束存在不一致，导致抖动和滑步等伪影，且需要为每个任务微调控制器。

2. **多任务策略的局限**：现有文本驱动策略（如 SuperPADL、PDP）运动多样性和表达力不足，且缺乏对新引导信号的泛化能力。MaskedMimic 等方法无法超越预定义的控制信号集合。

3. **行为克隆的累积误差**：将控制问题转化为模仿学习时，自回归预测中的小误差会逐步累积，导致长程生成失控。
