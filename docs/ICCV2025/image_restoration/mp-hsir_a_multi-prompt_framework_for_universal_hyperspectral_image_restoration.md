...
1. **任务专用方法缺乏通用性**：针对特定退化设计的方法（如去噪、超分辨率、去雾）无法泛化到其他退化场景，实际应用中需要维护多个专用模型。

2. **现有 all-in-one 方法不适用于 HSI**：
...
3. **HSI 专用 all-in-one 方法不足**：DDS2M 和 HIR-Diff 等扩散模型方法推理慢、需复杂超参调优；PromptHSI 仅使用文本提示，缺乏光谱引导。
