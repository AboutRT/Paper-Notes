...
1. **领域现状**：SVG 是现代数字设计的核心格式。分层 SVG 允许设计师独立操控各层的笔画属性、空间布局和合成效果。当前 SVG 生成方法分为三类：
...
3. **核心矛盾**：专业设计中 SVG 的分层结构是按"从底到顶、从背景到前景"的认知逻辑构建的，但现有方法无法建模这种创作过程，导致输出虽然视觉上可接受但不具备设计规范的可编辑性。

4. **本文切入角度**：与其直接生成最终 SVG，不如学习设计师的创作过程——按时序生成每一层的光栅化图像，然后将这些"施工蓝图"矢量化为分层 SVG。
...
1. **Serpentine Dataset 构建**：
   - 做什么：将 2 万+ 设计师创建的分层 SVG 自动拆解为时序创建序列
   - 核心思路：
...
2. **Layer-wise Image Generation**（文本→分层像素）：
   - 做什么：使用 LoRA 微调 DiT，从文本 prompt 生成包含分层创作过程的网格图像
