...
1. **时间生成方法**（MagicDrive, Vista等）：能预测未来视频但与预定义轨迹绑定，无法合成新视角
2. **隐式重建方法**（3DGS, NeRF, PVG等）：擅长NVS但需要逐场景优化，计算代价高
3. **前馈NVS方法**（STORM, FreeVS等）：能快速NVS但缺乏时间外推能力
4. **混合方法**（DreamDrive, MagicDrive3D）：尝试桥接两者但仍继承逐场景优化的限制
...
3. **DiST-S（空间NVS）+ 自监督循环一致性**:
   - 做什么：从已有viewpoint的RGB-D投影到任意新视角
   - 核心思路：
