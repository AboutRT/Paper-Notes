...
1. **位姿噪声**：SLAM输出的设备位姿因传感器噪声和Lidar里程计漂移而不准确
2. **时间异步**：RGB图像与设备位姿采集存在时间偏移（可达50ms），导致估计位姿与真实位姿偏差
3. **标定误差**：相机内参和Lidar-相机外参标定不完美引入额外误差
4. **COLMAP耗时**：COLMAP处理需4-12小时，且在重复纹理场景可能失败
