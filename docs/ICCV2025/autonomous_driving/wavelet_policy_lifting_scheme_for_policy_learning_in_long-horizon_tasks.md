...
1. **领域现状**：策略学习的目标是让智能体基于观测生成最优动作。近年来从行为克隆、强化学习发展到更复杂的方法，如 Diffusion Policy（利用条件去噪扩散过程建模多模态动作分布）和 Behavior Transformer（用 Transformer 处理离散化动作）。
2. **现有痛点**：复杂长horizon任务面临三大挑战：(1) 必须在多步之间保持一致行为、管理长时间依赖，否则误差会累积；(2) 多模态动作模式——达到相同目标常存在多种有效动作序列；(3) 精确操作要求——微小的控制误差可能导致严重问题。
3. **核心矛盾**：现有策略学习方法直接在原始时域学习动作序列，难以同时捕捉全局趋势（长horizon一致性）和局部细节（精确操作）。长序列包含的高频噪声也干扰了多模态模式的识别。
4. **本文要解决什么？** 如何从信号处理视角构建策略学习框架，同时捕获动作序列的全局趋势和精细变化？
5. **切入角度**：观察到对机器人关节动作序列做小波分解后，粗尺度（低频）分量清晰展现了几个不同的动作"模式"且无噪声波动，而细尺度（高频）捕获快速变化。这启发了从粗到细逐步生成动作的思路，类似残差连接。
6. **核心idea一句话**：基于可学习提升方案的小波策略网络，在分析阶段将观测序列多尺度分解为低频近似和高频细节，通过转换器映射到动作空间，在合成阶段从粗到细逐层重建动作序列。
...
1. **可学习提升方案 (Learnable Lifting Scheme)**:
   - 做什么：替代传统固定小波（Haar、Daubechies），实现端到端可学习的多尺度信号分解与重建
...
2. **因果膨胀卷积实例化**:
   - 做什么：将 $\mathcal{P}$、$\mathcal{U}$、$\hat{\mathcal{P}}$、$\hat{\mathcal{U}}$ 实例化为保持时间因果性的膨胀卷积
