---
description: "【论文笔记】TrafficLoc: Localizing Traffic Surveillance Cameras in 3D Scenes 论文解读 | ICCV 2025 | arXiv 2412.10308 | 图像-点云配准 | 提出 TrafficLoc，一种粗到细的图像-点云配准方法，通过几何引导注意力损失(GAL)、模态间-模态内对比学习(ICL)和稠密训练对齐(DTA)，实现交通监控相机在3D参考地图中的高精度定位，在自建 Carla Intersection 数据集上较 SOTA 提升达 86%。"
tags:
  - ICCV 2025
  - 图像-点云配准
  - 交通相机定位
...
1. **大视角差异**：图像和3D参考点云在不同时间、不同视角下采集，传统配准方法所需的精确初始猜测难以获取
2. **跨模态匹配困难**：直接将点云投影到图像上会产生"bleeding problem"，模态间特征差异大
3. **内参变化**：交通相机通常使用变焦镜头，内参频繁变化
...
2. **几何引导特征融合（GFF）+ Geometry-guided Attention Loss（GAL）**:
   - 做什么：增强 Transformer 跨模态融合时的几何感知能力
