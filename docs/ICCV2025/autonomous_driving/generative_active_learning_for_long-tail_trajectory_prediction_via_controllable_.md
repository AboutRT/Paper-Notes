...
1. **尾部样本的致命短板**：罕见驾驶行为（U 型转弯、突然超车、紧急变道）在数据中极度不足，模型的表示偏向频繁出现的头部样本，导致在安全关键的尾部场景中预测失败。现有基准主要评估整体（头部为主）性能，掩盖了这一问题。
...
3. **数据增强的特殊挑战**：轨迹预测是多 agent 回归任务，不同于图像分类的类条件生成。交通场景中多个 agent 相互作用，简单生成随机场景无法解决长尾不平衡——需要保留尾部行为特征的同时引入场景多样性。
