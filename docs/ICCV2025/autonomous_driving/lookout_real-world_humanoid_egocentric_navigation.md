...
description: >-
  [ICCV 2025][自动驾驶][自我中心导航] 提出LookOut——从第一人称视频预测未来6D头部姿态的人形导航框架，通过DINO特征反投影+3D时序聚合+BEV推理，学习避障和主动信息采集行为；同时贡献Aria Navigation Dataset (4小时真实动态场景)，在动静态障碍避碰和轨迹预测上全面超越基线。
tags:
  - ICCV 2025
  - 自动驾驶
  - 自我中心导航
...
**本文要解决什么？** 三个子问题：(1) 在动态环境中预测无碰撞的未来轨迹；(2) 建模头部旋转以学习主动信息采集行为；(3) 设计低成本数据采集管线支持规模化训练数据收集。

**切入角度**：用 Project Aria 眼镜（轻量、低调、易部署）采集真实导航数据，将问题形式化为"从带位姿的自我中心视频预测未来 6D 头部姿态序列"。

**核心idea一句话**：将 DINO 语义特征反投影到 3D 体素空间并时序聚合为 BEV 表示，统一解决动态避障和主动信息采集行为的预测。
...
1. **DINO 特征编码 + 无参数 3D 反投影**:
    ...
    - 做什么：从每帧 RGB 提取语义特征，并利用相机位姿将 2D 特征"搬"到 3D 体素空间
...
2. **BEV 投影与 BEV Net**:
    ...
    - 做什么：将 3D 特征体压缩为 2D 鸟瞰图特征，高效地进行空间推理
...
3. **头部中心标准坐标系（Head-Centered Canonical Frame）**:
    ...
    - 做什么：定义以当前头部为原点、朝向正前方且平行于地面的坐标系
