...
1. **现有基准聚焦感知和基础规划**：nuScenes-QA、DriveLM等关注空间理解和碰撞避免，很少评估对交通规则的理解——如限速标志、右转优先权、罕见标识
2. **长尾规则覆盖不足**：真实驾考覆盖大量边缘案例（如特殊施工标志、复杂路口避让规则），这些在实际驾驶数据中极少出现
3. **MLLM的交通知识有限**：虽然MLLM从预训练数据中可能继承部分交通知识，但实验表明这种知识和推理能力仍然有限
4. **商业系统的实际问题**：Tesla FSD等系统在交通规则解读上常出现错误的实例证据
