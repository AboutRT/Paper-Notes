...
1. **手工设计场景不可扩展**：传统结构化测试依赖人工设计失效场景，无法覆盖长尾分布
2. **扩散模型可控性受限**：现有扩散轨迹生成方法（CTG、SAFE-SIM 等）依赖后训练的领域特定引导函数（guidance function），需要专家知识设计且推理变慢
3. **语言条件的缺失**：语言是最直觉的控制接口，但尚无方法在训练阶段直接学习语言-轨迹的映射关系
4. **交互行为数据稀缺**：现有数据集（如 ProSim-Instruct）以单智能体行为为主，缺乏多智能体交互的精细标注
