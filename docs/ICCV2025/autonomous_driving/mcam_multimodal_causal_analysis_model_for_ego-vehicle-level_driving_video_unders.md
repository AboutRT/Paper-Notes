...
1. **浅层因果关系挖掘**：现有视觉到语言方法（如 ADAPT、DriveGPT4）停留在概率相关性层面，缺乏对驾驶行为的深层因果推理。例如，看到前车刹车灯亮就判断"因为前车减速而停车"，但真正的原因可能是远处的红灯。

2. **跨模态虚假相关**：视觉特征和语言描述之间存在虚假相关。比如在某些场景中，路边的停车标志和车辆无关的停靠场景在特征空间中会产生错误关联，模型可能据此生成不准确的推理。

3. **忽略自车级别因果建模**：现有工作主要关注事件级别的因果关系，而非以自车状态转换为核心的因果理解。自车从安全状态出发，因环境变化面临潜在危险，采取驾驶动作回归安全状态——这一完整闭环缺乏形式化建模。
