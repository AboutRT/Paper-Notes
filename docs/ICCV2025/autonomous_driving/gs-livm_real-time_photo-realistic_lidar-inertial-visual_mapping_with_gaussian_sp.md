...
1. **点云稀疏不均匀**：多线旋转 LiDAR 和非重复扫描 LiDAR 产生的点云分布不均，直接用于 3DGS 会导致内存低效和优化困难
2. **方向偏置**：室外 SLAM 主要是单向运动，导致 3D 高斯优化偏向相机方向，新视角渲染质量显著下降
3. **实时性困境**：现有方法（如 Gaussian-LIC）每帧迭代约 1 秒，远达不到实时要求（100ms），且无法处理完整室外序列（GPU 内存溢出）
4. **离线方法过拟合**：离线优化方法过度拟合监督视角，新视角合成质量差
...
1. **体素级高斯过程回归（Voxel-GPR）**:
   - 做什么：将稀疏不均匀的 LiDAR 点云转换为均匀分布的网格点云
...
2. **协方差驱动的 3D 高斯快速初始化**:
   - 做什么：利用 GPR 输出的协方差快速估计每个 3D 高斯的尺度和旋转参数
