...
1. **SDS 的局限**：Score Distillation Sampling 在编辑任务中会引入全局优化干扰，导致非编辑区域模糊和伪影，因为它是面向整个 prompt 全局优化的。
2. **DDS 的不足**：Delta Denoising Score 通过引入源分支消除模型偏差，但缺乏对源内容结构的显式保护，导致非编辑区域也被修改（如编辑人物面部时衣服也被改变）。
3. **CSD 的问题**：Classifier Score Distillation 使用双分类器获取跨提示词编辑方向，但同样缺少源结构保持机制，导致结构变形和伪影。
4. **编辑强度不足**：DDS 类方法在风格编辑等场景中容易出现编辑力度不够，最终几乎未产生变化。
