...
1. **计算成本高**：对预训练T2I模型进行侵入式修改并全量微调，训练时需同时处理n个视角图像，无法扩展到更大的基座模型和更高分辨率
2. **图像质量下降**：高质量3D数据稀缺，全模型微调容易过拟合，导致生成质量退化
3. **缺乏灵活性**：修改原始模型结构后，无法兼容个性化模型、LoRA、ControlNet等T2I衍生工具
