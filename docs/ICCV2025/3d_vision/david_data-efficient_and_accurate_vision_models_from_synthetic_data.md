...
1. **人工标注不可行**：逐像素的深度/法线标注对人类来说几乎不可能
2. **实验室采集受限**：复杂相机阵列或专用传感器获取的标注基于光度测量或噪声传感器，保真度有限
3. **实验室多样性不足**：受限于拍摄环境，难以覆盖真正的 in-the-wild 场景
4. **现有方法计算成本高**：Sapiens 预训练最大变体需要 1024 块 A100 训练 18 天，DepthPro 使用多阶段混合训练
