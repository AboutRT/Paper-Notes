...
**现有痛点**：
1. **远视图一致性差**：扩散方法使用单一参考视图（如正面图）来生成所有视图。当生成背面图时，参考视图的重叠内容几乎为零，引导信息失效，导致纹理不一致
2. **条件切换不灵活**：扩散方法在改变条件类型（文本→图像→形状）时通常需要大幅修改架构
3. **本质矛盾**：扩散方法建模联合概率分布 $p(v_1, v_2, ..., v_n)$，这种同时生成所有视图的方式不符合人类观察3D物体的渐进过程
