...
1. **领域现状**：动态3D场景生成是AR/VR和具身AI的核心需求。目前主流方法分为两类：纯物理仿真方法和条件视频生成方法。
2. **现有痛点**：
...
3. **核心矛盾**：物理仿真有精确的动作响应但视觉质量差且材质受限；视频生成有高质量视觉但缺乏动作可控性。
4. **本文要解决什么？** 如何从单张图像出发，在接受3D物理动作（重力、风力场、点力）输入的同时，生成涵盖多种材质的逼真动态3D场景？
5. **切入角度**：将物理仿真器和视频生成器的角色重新定义——物理仿真器提供粗糙但可控的运动引导，视频生成器负责精细化运动和视觉质量。
6. **核心idea一句话**：用物理仿真器产生粗糙3D动态作为条件信号，驱动视频扩散模型生成逼真视频，再用生成的视频反向更新3D场景，形成闭环。
