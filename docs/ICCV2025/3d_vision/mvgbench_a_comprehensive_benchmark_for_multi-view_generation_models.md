...
1. **与 GT 比较不合理**: 生成模型采样自解空间分布，可能与 GT 不同但仍然正确；现有 PSNR/SSIM 逐视图独立评估忽略了 3D 一致性
2. **方法不可比**: 不同 MVG 使用不同的相机设置（焦距、距离、仰角），在各自的 GT 下评估产生不可比较的数值
3. **评估维度不足**: 未覆盖泛化到真实图像的能力、对输入扰动的鲁棒性等重要方面
