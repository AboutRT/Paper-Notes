...
1. **两视图学习更优**：在2D自监督学习中，两视图范式（如SimCLR、MoCo）已被证明比单视图更有效，因为两视图引入更多方差和信息
2. **自重建太简单**：单视图的mask-and-reconstruct任务难度有限，模型可能走捷径而非学到深层语义
3. **跨重建更有挑战**：从一个视图重建另一个视图（Cross Reconstruction）比自重建困难得多，因为需要同时学习两视图间的相对位置关系和视图内的空间信息
