...
1. **可扩展性不足**：现有方法（LangSplat、LERF等）主要面向室内场景，直接应用于超过 $1 \text{km}^2$ 的大规模城市数据时面临严重的内存和计算瓶颈。例如 LERF 和 LangSplat 在 UrbanScene3D（5×10⁶ m²）上直接OOM。

2. **任务多样性有限**：当前3D语言场主要局限于单词级的物体定位（grounding），无法满足城市应用中的多种需求——空间关系解释、物体计数与尺寸量化、地标识别等。城市场景中的查询天然具有组合性（如"距Chase银行100米内的红色店铺"），需要分解为多步推理。
