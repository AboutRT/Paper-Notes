...
1. **仅支持形状morphing**：以往方法主要依赖建立点对点对应关系和确定平滑变形轨迹，局限于无纹理的拓扑对齐数据集（如FAUST人体形状、Shrec'20四足动物），无法处理纹理
2. **劳动密集的预处理**：对新数据进行morphing需要繁琐的配准和匹配步骤
3. **有限的morphing能力**：受限于物体多样性不足和小规模数据集，导致模糊和不合理的插值
