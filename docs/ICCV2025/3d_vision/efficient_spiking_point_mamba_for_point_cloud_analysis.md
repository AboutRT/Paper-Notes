...
1. **长距离依赖建模困难**：现有 MLP 和 Transformer 基础的 SNN 架构难以捕获不规则点序列中的远程依赖
2. **静态时间编码**：传统 direct encoding 沿时间维度简单重复输入，无法充分利用 SNN 时间特征提取能力
3. **脉冲信息退化**：脉冲驱动计算过程中信息密度显著低于 ANN（每个时间步激活减少约 30%），整合 Mamba 等序列模型时问题加剧
