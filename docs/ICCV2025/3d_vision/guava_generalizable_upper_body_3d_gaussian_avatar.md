...
创建逼真的、可表达表情的上半身虚拟人对影视、游戏、虚拟会议等应用至关重要。现有方法存在多重局限：

**3D方法的局限**：
- 需要 per-ID 训练（GART、GaussianAvatar、ExAvatar），每个人需要数分钟到数小时的优化
- 需要多视图或单目视频输入，采集成本高
- 头部重建方法（GAGAvatar）缺乏身体运动表征，全身方法忽略面部细微表情

**2D扩散方法的局限**：
- 生成质量高但 ID 一致性差（尤其大姿态变化时）
- 推理速度慢（每帧需多步去噪）
- 无法灵活控制视角
