---
description: "【论文笔记】PolarAnything: Diffusion-based Polarimetric Image Synthesis 论文解读 | ICCV 2025 | arXiv 2507.17268 | 偏振图像合成 | 提出 PolarAnything，首个基于单张 RGB 图像生成偏振图像的扩散模型框架，通过对编码后的 AoLP 和 DoLP 进行去噪扩散，实现了物理准确且逼真的偏振属性合成，无需 3D 资产或偏振相机。"
tags:
  - ICCV 2025
  - 偏振图像合成
  - 扩散模型
...
1. **数据集稀缺**：现有真实偏振数据集要么量极小（Morimatsu 仅 40 组，Qiu 仅 38 组），要么场景单一（HAMMER/HouseCat6D 专注于室内 6D 位姿）
2. **模拟器局限**：Mitsuba 等基于物理的渲染器需要完整的 3D 资产（mesh + PBR 材质 + 环境光照），且参数化 pBRDF 模型与真实偏振属性存在 gap，无法大规模生成逼真偏振图像
3. **学习方法受限**：DeepSfP 等学习方法训练数据仅 236 张，严重制约泛化性
