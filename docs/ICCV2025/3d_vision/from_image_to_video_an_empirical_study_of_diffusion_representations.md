...
1. **视频扩散表示几乎未探索**：尽管视频扩散模型已能生成高质量视频内容，但其表示的理解能力完全未知。相比之下，图像扩散表示已有大量研究。

2. **缺乏公平对比**：要比较图像vs视频扩散的表示质量，需要相同架构在不同训练目标下的对比。但现有模型（如Stable Diffusion 2.1 vs SVD）参数量差异太大（865M vs 1.5B），无法得出有意义的结论。

3. **时间信息的作用未知**：视频训练引入的时序理解如何影响表示质量？运动信息是否增强了语义理解、3D感知或物体追踪能力？
