...
1. **真实世界数据采集困难**：由人类专家执行的大规模真实演示既昂贵又低效，难以大规模扩展。

2. **仿真与真实的鸿沟 (sim-to-real gap)**：虽然物理仿真平台（Isaac Sim、PyBullet等）提供了可控环境，但sim-to-real gap仍是重大障碍。传统仿真中修改场景（如更换杯子颜色）需要重新编程仿真环境，效率极低。
