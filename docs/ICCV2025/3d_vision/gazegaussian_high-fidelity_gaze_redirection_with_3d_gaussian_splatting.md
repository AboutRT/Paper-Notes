...
1. **2D方法**（STED等）：将视线重定向表述为2D图像操作，忽略了头部和视线操控的固有3D特性，导致空间一致性差、合成保真度有限
2. **NeRF-based方法**（GazeNeRF等）：计算开销大，渲染效率低，且通过隐式地旋转特征图来改变视线方向，控制不够精确
3. **3DGS-based头部方法**（Gaussian Head Avatar等）：忽略了精确的视线方向控制，无法在不同被试之间泛化
