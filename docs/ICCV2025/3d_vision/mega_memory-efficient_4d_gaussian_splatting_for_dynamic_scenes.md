...
1. **球谐系数的巨大冗余**：每个4D Gaussian的161个参数中，144个是4D球谐（SH）系数，占比89%。这些SH系数用于编码视点和时间相关的颜色变化，但存在大量冗余。

2. **Gaussian数量爆炸**：渲染Birthday场景需要高达1300万个Gaussian，存储开销约7.79GB。这主要源于两个原因：
...
3. **现有3DGS压缩方法不适用**：3DGS的剪枝、SH蒸馏、矢量量化等技术面向静态场景设计，未考虑4DGS的时间和多视点因素。
