...
1. **合成数据集过于"干净"**：主流数据集（PCN、ShapeNet55、MVP等）均从CAD模型均匀采样，缺乏噪声、非均匀稀疏和复杂拓扑结构。现有方法在这些数据集上性能已接近饱和。

2. **真实世界点云本质不同**：实际传感器获取的点云具有三个显著特点：
   - **噪声**：测量误差导致的点偏移
   - **非均匀稀疏**：远处稀疏、近处密集，遮挡区域缺失
   - **丰富的拓扑特征**：复杂工业结构包含连通分量（0维同调）和环/洞（1维同调）

3. **缺乏真实世界配对数据集**：OmniObject3D、ScanNet等真实数据集要么在受控环境中采集，要么不提供ground truth完整形状，无法用于监督训练。
