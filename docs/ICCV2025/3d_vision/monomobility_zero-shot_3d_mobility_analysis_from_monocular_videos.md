...
description: >-
  [ICCV 2025][3D视觉][关节物体分析] MonoMobility 首次实现从单目视频零样本分析关节物体的运动部件及运动属性（轴向、类型），通过深度估计+光流分析初始化，结合2D高斯泼溅表示和专为关节物体设计的端到端动态场景优化算法，自监督精细化运动参数。
tags:
  - ICCV 2025
  - 3D视觉
  - 关节物体分析
...
1. **初始分析与场景初始化**：
    ...
    - 做什么：利用一系列现成方法从视频中提取几何和运动信息，构建初始的运动部件分割和运动轴估计，初始化2D高斯泼溅场景表示。
...
2. **端到端动态场景优化**：
    ...
    - 做什么：核心创新——专门为关节物体设计的自监督动态场景优化算法，通过迭代采样帧对、变换高斯、多目标联合优化来精细化运动参数。
...
3. **运动类型判定与部件裁剪**：
    ...
    - 做什么：优化收敛后根据最终的运动量自动判定每个部件的运动类型（纯旋转/纯平移/复合），并裁剪掉误识别的运动部件。
