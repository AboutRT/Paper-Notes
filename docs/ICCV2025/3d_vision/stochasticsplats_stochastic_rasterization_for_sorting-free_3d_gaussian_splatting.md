...
1. **排序计算昂贵**：全局排序操作是渲染流程中的显著开销
2. **Popping伪影**：排序基于高斯均值深度，微小相机运动可导致排序顺序突变，产生时间不连续性
3. **无法灵活权衡**：对于固定表示，降低渲染分辨率反而不一定更快（每个像素覆盖更多高斯）
4. **Billboard 近似不准确**：将 3D 高斯投影为相机平行的 billboard，无法正确处理体积混合
5. **可移植性差**：多数实现使用 CUDA 定制光栅器，难以移植到非 NVIDIA 平台和标准图形管线
