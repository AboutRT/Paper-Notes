...
1. **知识不足**：现有 OCD 方法仅从标注数据中挖掘可迁移知识，当标注数据/类别数量稀少时（特别是细粒度场景），已知类别包含的知识往往不足以支撑未知类别的发现
2. **哈希推理局限**：现有方法（如 SMILE）使用哈希码作为类别描述子，二值化处理不可避免地降低了高维特征的表示能力
3. **数据增强无效**：传统的类别保持型数据增强（如 T2I 模型生成已知类别图像）对未知类别发现帮助有限——实验表明仅增强已知类别数据难以提升未知类别发现
