...
1. **领域现状**：协同感知(Collaborative Perception)让车辆通过V2X共享信息来扩展感知范围、穿透遮挡。中间特征层融合是当前主流方案，在准确率和带宽之间取得平衡。

2. **异构挑战**：现实中不同厂商的自动驾驶车辆使用不同传感器配置和感知模型，导致编码后的中间特征处于不同语义空间(域差距)。直接融合异构特征会严重降低性能——实验显示直接融合baseline的AP仅约53%。
...
4. **核心问题**：能否在推理阶段动态调整模型参数来适应不同协作者，完全跳过联合训练？
