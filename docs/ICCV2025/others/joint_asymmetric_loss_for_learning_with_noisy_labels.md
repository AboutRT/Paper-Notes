...
三种主要噪声类型：
1. **对称噪声**：真实标签以均匀概率翻转到其他类别
2. **非对称噪声**：噪声率依赖于类别
3. **实例相关噪声**：噪声率依赖于具体样本

### 已有方法的不足

1. **对称损失的欠拟合问题**：MAE 等对称损失虽然理论上噪声容忍，但过于严格的对称条件限制了拟合能力，优化困难
2. **APL 框架的局限**：Ma et al. 提出的 Active Passive Loss (APL) 通过组合主动损失（如 NCE）和被动损失（如 MAE）来改善拟合，但目前 APL 框架中的被动损失全部基于对称条件
3. **非对称损失的不兼容**：Zhou et al. 提出的非对称损失函数（ALFs）理论上优于对称损失（仅需更宽松的"干净标签占主导"条件），但现有非对称损失（如 AGCE、AUL）全部是**主动损失**——在被动损失场景下实现非对称条件一直是未解决的难题
4. **AGCE 不能替代 NCE**：实验表明用非对称的 AGCE 替换 APL 中的主动 NCE 并不有效（AGCE+MAE 在 0.8 对称噪声下仅 44.61%），NCE/NFL 在 APL 中不可替代
...
- **做什么**：设计一个同时满足非对称条件和被动损失定义的新损失函数
