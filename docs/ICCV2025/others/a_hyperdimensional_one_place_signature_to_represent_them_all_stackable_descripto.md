...
核心 idea 一句话：用超维计算的 bundling 操作将同一地点的多个参考描述子逐元素求和，生成不增加维度的融合表示。
...
### 关键设计
1. **Bundling 融合（核心模块）**:
   - 做什么：将 $K$ 条参考集中同一地点 $i$ 的描述子 $\mathbf{r}_i^k \in \mathbb{R}^n$ 进行逐元素求和，得到融合描述子 $\mathbf{r}_{\text{fused},i} = \sum_{k=1}^{K} \mathbf{r}_i^k$
...
2. **高斯随机投影降维**:
   - 做什么：利用 Johnson-Lindenstrauss 引理，将融合后的高维描述子通过高斯随机投影矩阵 $\mathbf{G} \in \mathbb{R}^{o \times n}$ 投影到低维空间 $\hat{\mathbf{r}}_{\text{fused},i} = \mathbf{G} \mathbf{r}_{\text{fused},i}$
...
3. **合成图像叠加**:
   - 做什么：当多条真实参考遍历不可用时，使用图像增强（颜色抖动、高斯模糊、灰度化等）生成合成的多条件参考集，再进行 HOPS 融合
