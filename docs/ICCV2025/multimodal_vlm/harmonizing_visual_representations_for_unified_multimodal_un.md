---
description: >-
  [ICCV2025][多模态] 提出Harmon统一框架，以共享MAR编码器同时服务图像生成与理解，通过三阶段训练在GenEval上超越所有统一方法，图像理解匹配使用独立语义编码器的方法。
tags: [ICCV2025, 多模态, 统一模型, 图像生成, 图像理解, MAR, 自回归生成]
---
...
**本文要解决什么**: 找到一种天然兼顾生成与理解的视觉表示，构建真正共享编码器的统一框架。

**切入角度**: 掩码图像建模（MIM）通过"遮挡-重建"预训练学到丰富语义；MAR将MIM扩展到自回归生成——其编码器可能天然具备双重能力。

**核心idea一句话**: MAR编码器的"在生成中学理解"特性使其成为统一编码器的理想选择——生成能力是理解能力的副产品。
...
1. **共享MAR编码器用于双任务**:
    ...
    - 做什么：用同一个MIM预训练的MAR编码器同时服务图像生成和理解
