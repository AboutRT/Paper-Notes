...
description: >-
  [ICCV 2025][多模态VLM][多模态] 提出首个百万规模的多模态agent调优数据集MMAT-1M，通过四阶段数据引擎（基础数据→推理轨迹生成→反思纠错→格式整合）为MLLM注入CoT推理、工具调用和反思能力，在InternVL2.5-8B上平均提升2.7%，RAG任务上提升8.8%。
tags:
  - "ICCV 2025"
  - "多模态VLM"
  - "多模态"
...
**本文目标**：构建一个百万级别的多模态agent调优数据集，同时支持CoT推理、动态API调用、反思纠错，并提供one-turn和multi-turn两种格式以平衡效率和精度。

**切入角度**：从已有的公开多模态QA数据集出发，用GPT-4o逐步生成推理轨迹、动态整合工具调用结果，并通过反思步骤修复推理中的跳步和作弊行为。

**核心 idea**：用四阶段数据引擎（Foundation→Rationale→Reflection→Integration）从公开QA数据合成百万级agent调优数据，同时支持CoT、工具调用和自我反思。
...
2. **五类外部工具的动态调用**:
    ...
    - 功能：在推理过程中自适应地调用Image Caption、OCR、OVD、Face Detection、RAG五类算子。
...
3. **反思与纠错机制**:
    ...
    - 功能：检测并修复推理轨迹中的逻辑缺陷，包括通用反思和数学反思两种prompt。
