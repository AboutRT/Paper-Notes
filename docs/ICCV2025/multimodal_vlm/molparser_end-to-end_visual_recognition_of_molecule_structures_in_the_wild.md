...
1. **表示局限**：标准 SMILES 无法表示专利文献中常见的 Markush 结构（包含 R-group 变量的分子家族）、连接点、抽象环、聚合物等
2. **数据稀缺**：最大公开数据集仅 30 万合成样本（MolGrapher-300k），且全为合成数据，与真实文献的风格差异显著
3. **In-the-wild 鲁棒性差**：真实专利/论文中的分子图像存在缩写、噪声、模糊、多样绘制风格等问题，现有方法表现不佳
