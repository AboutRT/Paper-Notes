...
description: >-
  [ICCV 2025][多模态VLM][hallucination mitigation] 提出ONLY，一种training-free的单层干预解码方法——通过Text-to-Visual Entropy Ratio（TVER）选择偏向文本的attention head生成textually-enhance…
tags:
  - "ICCV 2025"
  - "多模态VLM"
...
1. **Text-to-Visual Entropy Ratio (TVER)**:
    ...
    - 功能：衡量每个attention head对文本vs视觉token的信息分散程度。
...
2. **Textual-Enhanced MHA (TE-MHA)**:
    ...
    - 功能：在选定层$\tilde{\ell}$中，保留TVER高于该层平均值的head，屏蔽其余head，生成偏向文本的attention输出。
...
3. **自适应解码策略**:
    ...
    - 功能：根据原始logits和增强logits之间的曼哈顿距离，动态选择协作或对比解码。
