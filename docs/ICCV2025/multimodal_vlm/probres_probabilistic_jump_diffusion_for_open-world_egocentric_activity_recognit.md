...
1. **搜索空间爆炸**：活动由动作（action）和物体（object）组合而成，组合数随开放程度成指数增长，穷举枚举在计算上不可行
2. **VLM 的局限性**：虽然 CLIP、LAVILA 等 VLM 具有强大的零样本泛化能力，但它们依赖穷举枚举进行推理，对大规模开放世界推理效率低下
3. **开放程度定义模糊**：现有研究对"开放世界"缺乏统一定义，不同因素（物体、动作、领域）独立贡献不确定性，比较方法和评估泛化能力困难
