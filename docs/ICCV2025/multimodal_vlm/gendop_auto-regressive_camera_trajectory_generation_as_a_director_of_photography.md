...
1. **传统方法**：依赖几何优化或手工设计的程序化系统，每种运动都需要单独的几何建模或代价函数工程，限制了创意合成能力
2. **基于扩散的方法**：CCD 和 E.T. 使用人体追踪数据集训练，继承了以人物为中心的运动偏差；Director3D 虽然引入了物体/场景中心轨迹，但缺少轨迹级别的描述文本，导致生成的路径由几何合理性驱动而非导演意图
3. **数据集缺失**：现有数据集（RealEstate10K、MVImgNet 等）要么聚焦于简单的物体/场景中心路径，要么局限于追踪运动，缺乏自由运动（free-moving）的艺术化轨迹和对应的导演意图描述
