---
description: "【论文笔记】Sparsity Outperforms Low-Rank Projections in Few-Shot Adaptation 论文解读 | ICCV 2025 | arXiv 2504.12436 | 稀疏优化 | 提出稀疏优化（SO）框架，通过动态稀疏梯度选择和基于重要性的动量剪枝来替代低秩适配方法（如LoRA），在11个数据集上的少样本VLM适配任务中实现了SOTA，同时降低了内存开销。"
tags:
  - ICCV 2025
  - 稀疏优化
  - 少样本适配
...
1. **适配器/提示学习方法**：通过引入额外可训练参数来适配模型，但容易过拟合且依赖大量超参搜索。
2. **低秩重参数化方法**（LoRA、DoRA等）：将权重更新分解为低秩矩阵来减少参数量，但存在根本性矛盾——**低秩约束限制了模型表达能力**，而**固定秩值需要在欠拟合和过拟合之间权衡**。
...
1. **局部稀疏与全局稠密（Local Sparsity & Global Density）**:
   - 做什么：每次迭代仅更新参数的一个极小子集（由密度比 $\kappa$ 控制），且该子集动态变化
...
2. **局部随机与全局重要性（Local Randomness & Global Importance）**:
   - 做什么：梯度通过**随机选择**稀疏化，一阶动量通过**重要性排序**稀疏化
   - 核心思路：
