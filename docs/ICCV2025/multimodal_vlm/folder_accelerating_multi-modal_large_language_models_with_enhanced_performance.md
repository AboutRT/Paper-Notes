...
1. **训练时方案**（Q-Former、Resampler、Pooling）：性能下降明显，可扩展性差，与特定架构绑定
2. **推理时方案**（ToMe、Turbo）：采用各层均匀压缩的策略，未考虑信息损失的差异性，效果欠优
3. **FastV**：在 LLM 内部基于注意力剪枝视觉 token，但需要全程保留 KV-cache 中的视觉 token，不适合长对话场景
