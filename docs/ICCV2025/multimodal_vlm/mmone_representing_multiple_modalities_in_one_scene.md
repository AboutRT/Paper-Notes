...
1. **属性差异（Property Disparity）**：不同模态的数据特征本质不同。例如 RGB 是 3 维颜色向量，语言需要高维特征空间；热成像中纸张不会遮挡热源，但在 RGB/语言中会遮挡
2. **粒度差异（Granularity Disparity）**：不同模态的信息粒度不同。热成像相对粗糙，RGB 更精细，语言在物体内部保持一致。因此对物体边界，热模态偏好少量大高斯，RGB 需要多个小高斯
