...
1. **人工标注成本高**：姿态转换涉及多个关节部位的精细运动描述，标注者可能遗漏细微变化、使用不一致的措辞或引入主观语言
2. **基于规则的生成受限**：如PoseFix的方法依赖预定义的聚合规则和模板化句子，受限于固定的"paircode"描述符（基于绝对3D关键点位置），表达能力和泛化性不足
3. **数据稀缺**：与FashionIQ、CIRR等CIR数据集不同，CPR缺乏大规模标注数据，且姿态变化是连续的、缺少纹理线索
...
1. **阶段一：身体部位级描述生成**:
   - 做什么：利用MLLM逐个分析和比较两个姿态之间各身体部位的变化
...
4. **循环一致性约束**:
   - 做什么：训练时通过正向和反向转换描述构建循环约束，缓解MLLM生成错误
