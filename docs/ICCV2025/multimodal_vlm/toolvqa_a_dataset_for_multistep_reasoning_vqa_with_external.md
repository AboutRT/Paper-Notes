...
description: >-
  [ICCV 2025][多模态VLM][tool-augmented VQA] 提出ToolVQA，一个23K样本的多模态工具使用VQA数据集，通过ToolEngine数据生成pipeline（图像引导DFS + LCS示例匹配）从真实图像中构造隐式多步推理问题（平均2.78步）…
tags:
  - "ICCV 2025"
  - "多模态VLM"
  - "tool-augmented VQA"
...
**本文目标**：构建一个大规模、真实场景、隐式多步推理的工具使用VQA数据集，并验证在其上微调小模型的泛化能力。

**切入角度**：用DFS在工具图上搜索生成推理轨迹（真实调用工具获取结果），再用LCS动态匹配不同示例指导每步决策，最后基于轨迹反向构造问题。

**核心 idea**：用图像引导DFS生成工具使用轨迹+LCS动态示例匹配+反向问题生成，构建首个大规模真实场景多步推理工具使用VQA数据集。
