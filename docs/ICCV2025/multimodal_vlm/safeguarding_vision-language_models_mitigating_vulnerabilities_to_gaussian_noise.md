...
1. **视觉模态引入新攻击面**：传统基于优化的对抗攻击（PGD等）可以注入不可感知的扰动来越狱VLM
2. **关键盲点——高斯噪声脆弱性**：作者发现一个被忽视的根本问题——**主流VLM（InternVL2、LLaVA、MiniGPT-4）在训练中缺少噪声增强**，导致即使简单的高斯噪声就能同时降低模型的有用性和安全性
...
2. **噪声增强安全微调**:
   - 做什么：在Robust-VLGuard上用LoRA微调视觉编码器，训练时以70%概率对图像添加随机标准差（σ ∈ [0.01, 0.15]）的高斯噪声
