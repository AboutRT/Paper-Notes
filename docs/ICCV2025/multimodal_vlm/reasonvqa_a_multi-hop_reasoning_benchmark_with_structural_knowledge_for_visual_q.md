...
1. **缺乏外部知识整合**：标准 VQA 数据集（VQAv2、GQA）主要关注图像中显式可见的内容，例如物体识别和属性判断，而真实世界的问题往往需要超越图像本身的知识
2. **多跳推理缺失**：像"这座教堂所在国家的首都是什么？"这类问题需要多步推理——先识别教堂，再查询其所在国家，最后查询该国首都
3. **构建成本高昂**：现有需要外部知识的数据集（如 OK-VQA、KVQA）依赖大量人工标注，扩展性差
4. **规模受限**：Encyclopedic-VQA 约 1M 问题，INFOSEEK 约 1.35M，仍然有限
