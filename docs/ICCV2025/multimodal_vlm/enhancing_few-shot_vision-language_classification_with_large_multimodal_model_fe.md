...
1. **生成式 LMM 在分类任务上表现差**：数十亿参数的 LMM 在图像分类上甚至不如 CLIP 和 SigLIP 等小模型，原因是其生成输出不适合离散标签任务
2. **现有特征提取方法有局限**：
...
3. **CLIP 特征是单模态的**：CLIP 提取的是视觉特征或文本特征，无法处理交错的图文输入（如 VQA 中的图像+问题）
...
- **做什么**：从 $L \times H$ 个头中选出与目标分类任务最相关的 $k$ 个头
- **核心思路**：
