...
1. **缺乏基准**: 主流 VLM 报告中评估的基准（MMMU、VQAv2、AI2D 等）平均分辨率低于 1K，不适合 HRI 评估
2. **场景单一**: 现有 HRI 数据集仅关注特定场景（如远距离图像）或特定分辨率（如 8K）
3. **诊断不足**: 现有 multi-modal NIAH 测试主要关注长文本或低分辨率多图像，缺乏对 HRI 区域利用能力的诊断
