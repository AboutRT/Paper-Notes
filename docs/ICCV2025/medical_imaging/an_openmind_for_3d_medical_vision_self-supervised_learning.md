...
**领域**: 医学图像 / 自监督学习  
**关键词**: 3D 医学图像, 自监督预训练, 基准测试, 脑部 MRI, 基础模型
...
1. **缺乏大规模开放数据集**：大型数据集（如 UK-BioBank >10 万、ABCD >4 万）需审批才能使用，且数据使用协议（DUA）限制苛刻（如需在论文标题中包含数据集名称、需内部审稿等）。已有的公开 SSL 方法要么基于受限的大数据集开发，要么仅在小规模公开数据上验证。
2. **缺乏可比性**：不同方法使用不同的预训练数据、不同的网络架构（CNN/ViT/Swin/混合）、且在不同的下游任务上评估，导致公平比较几乎不可能。
3. **微调策略的重要性被忽视**：不同的微调策略对预训练模型的最终表现影响巨大，但现有工作缺少系统性的微调策略对比。
