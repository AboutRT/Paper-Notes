...
1. **多模态RAG的知识冗余与低相关性**：启发式和基于Agent的mRAG方法检索到的知识往往不精确，存在信息冗余
2. **现有MMKG基准不够挑战**：多数MMKG使用VQA数据集或网络资源构建，这些知识很可能已被现有MLLM学习，无法真正评估MMKG的增强效果
3. **现有MMKG模态单一**：主要整合文本和图像，缺少视频等更丰富的模态
4. **训练检索器成本高**：在低资源领域训练有效的图谱检索器不现实，且无法将新图谱整合到闭源模型中
...
- **做什么**：由3位资深玩家手工构建22个子图（每个怪物一个），合并为完整图谱 $\mathcal{G} = (\mathcal{E}, \mathcal{V}, \mathcal{R})$
...
- **做什么**：将检索到的子图 $\hat{\mathcal{I}}$ 上的路径转为文本，输入MLLM生成答案
