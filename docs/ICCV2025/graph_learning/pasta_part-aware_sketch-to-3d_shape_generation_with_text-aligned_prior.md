...
1. **领域现状**：条件3D生成主要基于草图或文本，但两者各有明显局限——文本缺乏精确几何控制，草图缺乏语义信息且存在歧义。
2. **现有痛点**：纯草图方法（LAS-D、SENS）难以从简化草图恢复完整结构（如缺少扶手、腿数不对）；纯文本方法无法精确控制几何。
3. **核心矛盾**：如何同时利用草图的几何控制力和文本的语义表达力来生成准确的3D形状。
4. **本文切入角度**：利用VLM从草图中自动提取部件描述（如"椅背形状、座椅、4条腿、无扶手"），作为文本先验补充草图缺失的语义线索。
5. **核心idea**：文本先验 + 视觉-文本Transformer解码器融合两种条件 + ISG-Net双GCN建模部件间结构关系。
