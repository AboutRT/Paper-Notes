...
1. **分阶段处理导致误差累积**：典型 pipeline 为"光源标定 → 逐视图 PS 线索估计 → 多视图融合"，每个阶段的误差向下传播
2. **逐视图 PS 忽略跨视图信息**：独立估计每个视角的法线图，无法利用跨视图的互补观测，且容易产生**跨视图不一致**
3. **依赖光源标定**：需要铬球或漫反射白板等校准设备
4. **要求视图对齐的 OLAT**：传统方法要求每个视角都有完整的 OLAT 图像栈，限制了采集灵活性
