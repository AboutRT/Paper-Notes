...
1. **模板匹配式方法的局限**：传统方法先检索最匹配模板再对齐，但检索失败直接导致位姿不准。这种分步流程将问题分解为模板检索 → 对应点估计 → 位姿预测 → 可选精化，每步错误会累积。

2. **单视图约束不足**：多视图几何为3D感知提供核心约束，但现有单目方法无法利用多视图约束。虽然模板图像本身就是带位姿的多视图，但现有方法仅用于分类匹配而非几何推理。

3. **扩散模型的适用性差距**：RayDiffusion 等方法在相机位姿估计中展示了强大的泛化能力，但相机位姿定义在大尺度世界坐标系中，而物体位姿在紧凑的物体中心空间，直接迁移效果不佳。
