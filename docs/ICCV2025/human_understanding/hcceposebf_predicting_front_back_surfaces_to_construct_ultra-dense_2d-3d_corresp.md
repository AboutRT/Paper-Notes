...
1. **只利用前表面**：现有方法仅预测物体正面可见表面的3D坐标，忽略了背面和内部区域的潜在信息。更密集的2D-3D对应关系能帮助RANSAC-PnP求解器得到更精确的位姿
2. **编码精度不足**：现有的层级二值编码（如ZebraPose）中，神经网络在明暗条纹边界附近难以准确学习二值码，导致坐标预测精度受限
