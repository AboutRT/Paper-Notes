---
description: "【论文笔记】Signs as Tokens: A Retrieval-Enhanced Multilingual Sign Language Generator 论文解读 | ICCV 2025 | arXiv 2411.17799 | 手语生成 | 提出 SOKE，一种基于预训练语言模型的多语言手语生成框架，通过解耦式 tokenizer 将连续手语动作离散化为 token 序列，结合多头解码和检索增强策略，实现从文本到多语种 3D 手语 avatar 的高质量生成。"
tags:
  - ICCV 2025
  - 手语生成
  - 多语言手语
...
1. **忽略手语的语言学本质**：多数方法将 SLG 视为视觉内容生成任务（视频/关键点/运动），使用 GAN 或扩散模型，未能利用预训练 LM 的泛化性和可扩展性
2. **gloss 依赖**：传统方法依赖 gloss（手语的书写形式）作为中间表示，但 gloss 需要大量标注且造成信息瓶颈
3. **单语言局限**：现有方法通常只处理一种手语（如 ASL），缺乏统一的多语言模型
4. **解码效率低**：使用解耦 tokenizer 时，将多个身体部位的 token 展平为单一序列，导致解码步数成倍增加
5. **词级精度不足**：句子级生成难以保证每个手语词的精确性
