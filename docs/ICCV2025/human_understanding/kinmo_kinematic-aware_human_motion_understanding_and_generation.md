...
1. **多对多映射歧义**：同一运动可用多种文本描述（"pick up an object" vs "bend down to reach something"），同一文本也可对应多种运动变体（"running"可以是快跑/慢跑/抬臂跑等）
2. **局部运动不可控**：现有模型擅长从全局描述生成连贯全身运动，但无法独立控制特定身体部位。例如无法指定"走路时右手向上举"
3. **空间细节缺失**：全局描述无法捕获速度、肢体定位、运动学动力学等细节
