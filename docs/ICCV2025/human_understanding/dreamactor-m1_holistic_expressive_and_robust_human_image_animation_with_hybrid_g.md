...
1. **精细整体控制不足**：现有方法要么擅长面部（GAN/NeRF方法仅限肖像区域），要么擅长身体（扩散方法忽略精细面部表情），无法同时精确控制面部和身体
2. **多尺度适应能力差**：不同输入（肖像/上半身/全身）的信息密度和关注重点不同，现有方法难以在单一框架下处理
3. **长时一致性问题**：由于无法一次性生成长视频，多段拼接时参考图像中未见区域（如背面衣服纹理）的信息逐渐丢失，导致不一致
4. **单一控制信号困境**：用骨架控制面部不够精细，用3DMM参数又无法控制身体
