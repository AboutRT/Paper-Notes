...
1. **高质量 3D 训练数据匮乏**：Objaverse 等数据集中高质量、富含复杂几何细节的 3D 资产严重不足，导致网络生成的模型过于简化
2. **域差距**：训练数据通常来自合成渲染，与真实世界图像存在显著风格差异，推理时性能明显下降
3. **RGB 图像的固有歧义**：光照、着色和复杂纹理使得从 RGB 图像中提取精确几何信息非常困难
