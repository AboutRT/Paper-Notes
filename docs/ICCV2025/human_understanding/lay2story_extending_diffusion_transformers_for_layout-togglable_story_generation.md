...
1. **无训练方法**（如 ConsiStory、StoryDiffusion）：通过修改跨帧自注意力来保持一致性，但缺乏细粒度的引导和帧间交互，在复杂场景下主体外观容易发生漂移。
2. **基于训练的方法**（如 FLUX.1-dev IP-Adapter）：通过学习连续帧中的视觉概念来保持一致性，但缺乏大规模高质量的布局标注数据集。
