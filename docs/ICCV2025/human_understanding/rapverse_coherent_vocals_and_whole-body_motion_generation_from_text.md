...
1. **声音和动作内在耦合**：心理学证据表明人类的声音和动作生成高度相关耦合。统一生成系统允许更丰富的情感表达，一种模态的生成可指导和辅助另一种。

2. **级联系统的累积误差**：文本→语音 + 语音→动作的级联方案在每个阶段累积错误。例如语音合成的误差会导致不准确的面部表情和唇同步。同时需要多个模型的独立训练和推理，计算开销大。

3. **数据集缺失**：不存在同时包含歌词、歌声和3D全身运动标注的数据集。现有歌声数据集多为中文或规模小；动作数据集仅含文本描述无音频；语音-动作数据集关注语音而非唱歌。
