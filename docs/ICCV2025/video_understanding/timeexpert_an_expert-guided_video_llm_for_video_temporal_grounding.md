...
1. **通用Video-LLM的时间粗粒度**：VideoQA等粗粒度任务的成功难以迁移到需要精确时间定位的VTG任务——缺乏显式时间建模机制
2. **VTG专用方法的共享参数瓶颈**：TimeChat、TRACE等方法虽引入时间token，但将时间戳、分数和文本token**不加区分地**通过同一LLM处理，共享参数导致**任务干扰**：
   - 时间戳预测需要精确的数值回归能力
   - 显著性评分需要全局重要性判断
   - 文本生成需要语义理解和语言组织
   这三种能力要求截然不同的特征表示
3. **静态计算分配**：所有token获得相同的计算资源，忽略了不同任务token重要性的差异
