...
1. **去噪与运动估计割裂**：现有去噪方法通常独立设计，将运动估计作为单独的后续任务。然而，**运动是事件数据的本质属性**——没有运动就无法感知场景边缘。二者应该是协同的。

2. **Ground Truth 获取困难**：学习方法需要 GT 噪声标签，但在真实数据中无法定义。现有方案要么依赖仿真，要么通过激进的预过滤来获取"纯信号"数据，但这种处理可能改变事件的信号/噪声特性。

3. **循环依赖问题**：去噪需要知道真实运动（将信号事件与噪声分开），而准确的运动估计又需要信号事件（因为噪声不携带运动信息）。
