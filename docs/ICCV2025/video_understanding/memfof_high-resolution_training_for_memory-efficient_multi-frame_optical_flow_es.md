...
1. **相关体积分辨率降低（1/8 → 1/16）**：
   - 做什么：将工作分辨率从标准的 1/8 降至 1/16，使相关体积大小缩减为原来的 1/16
...
2. **高分辨率训练策略（FullHD-centric）**：
   - 做什么：在训练时将标准数据集（如 Things、Sintel、KITTI）2x 上采样，使用接近 FullHD 的 crop size（如 864×1920）训练
...
3. **GMA 全局运动注意力与自适应缩放**：
   - 做什么：重新引入 GMA 模块以增强运动一致性，并修改注意力缩放因子以适应不同分辨率
