...
1. **GCN 方法**（ST-GCN, CTR-GCN 等）：用邻接矩阵表示二元关节连接，但**只能建模两个关节之间的关系**
2. **Transformer 方法**（SkateFormer 等）：用注意力图建模拓扑，效果好但参数量和 GFLOPs 显著更高
3. **已有超图方法**（Hyper-GNN, Selective-HCN）：尝试用超图建模多关节关系，但**依赖固定的手工设计**
