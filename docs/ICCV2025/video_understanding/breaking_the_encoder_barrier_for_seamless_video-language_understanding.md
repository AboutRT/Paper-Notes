...
1. **计算开销累积**：视频需要逐帧通过视觉编码器提取特征，帧数越多开销越大；大型编码器（如 InternViT-6B）进一步加剧了这一问题
2. **时空分辨率约束**：编码器对固定尺寸的视觉表示施加分辨率偏差，无法根据内容动态调整分辨率
3. **多模态交互瓶颈**：依赖预提取特征，限制了视频像素与文本 token 之间的底层交互，以及帧间依赖的建模
