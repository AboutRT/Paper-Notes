...
1. **仅评估单场景质量**：VBench、EvalCrafter 等指标只评价单个视频片段的画质，不涉及场景间的过渡和布局控制。
2. **不兼容 3D/4D 方法**：现有 benchmark 缺少相机轨迹和参考图像输入，无法对需要相机位姿或种子图像的 3D/4D 方法进行评估。
3. **缺乏多场景、长序列评测**：没有 benchmark 要求模型进行多步场景生成和长序列世界生成任务。
