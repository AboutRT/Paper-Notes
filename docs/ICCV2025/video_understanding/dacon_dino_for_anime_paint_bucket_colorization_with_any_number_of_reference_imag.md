...
1. **参考图像数量受限**：之前的方法（如AnT）依赖Multiplex Transformer，固定了输入结构，最多支持1-2张参考图像，无法利用多角度色彩设计图的完整信息
2. **关键帧上色精度低**：关键帧之间构图差异大（姿态、视角、遮挡变化），线稿仅包含轮廓信息，难以建立语义对应，导致精度远低于连续帧上色
3. **生成式方法的局限**：基于扩散模型的方法可能生成参考图中不存在的颜色，且输出为合成图像而非分段层，难以后期修改
