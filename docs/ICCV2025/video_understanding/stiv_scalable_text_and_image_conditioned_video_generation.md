...
**代码**: 无  
**领域**: 视频理解  
**关键词**: 视频生成, Diffusion Transformer, 图像条件, 文本到视频, 可扩展训练
...
1. **帧替换策略（Frame Replacement）**:
   - 做什么：在训练时将噪声视频 latent 的第一帧替换为未加噪的图像条件 latent，并 mask 掉该帧的损失
...
2. **联合图像-文本 Classifier-Free Guidance (JIT-CFG)**:
   - 做什么：在训练时随机 dropout 图像条件和文本条件，推理时使用联合 CFG
...
4. **渐进式训练（Progressive Training）**:
   - 做什么：从 T2I → T2V → STIV，从低分辨率 → 高分辨率，从短时长 → 长时长逐步训练
