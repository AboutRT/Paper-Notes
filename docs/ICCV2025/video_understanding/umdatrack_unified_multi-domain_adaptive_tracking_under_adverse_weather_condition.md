...
1. **巨大的域偏移**：现有主流跟踪器（OSTrack、ARTrackV2 等）在 LaSOT/TrackingNet 等日间数据集上训练，迁移到恶劣天气时因外观分布差异显著下降。
2. **单域方法的局限**：现有跨域跟踪器（如 UDAT）仅针对单一天气条件设计。例如 UDAT 专为夜间优化，进入雾天时性能反而急剧下降，缺乏多条件泛化能力。
3. **数据合成成本高**：现有方法需要生成大量目标域样本进行知识迁移，过程耗时且各域独立处理，忽略了目标对象在不同域间的内在关联。
4. **冗余参数问题**：对不同天气条件分别引入独立的特征对齐参数，无法高效进行跨域交互。
