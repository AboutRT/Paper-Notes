...
### 现有方法的问题
1. **安全隐患**: 现有 LLM in-context learning 方法不显式确保优化后的提示安全，可能生成有害内容
2. **意图扭曲**: 提示重写可能无意中改变用户本意或引入偏差
3. **忽视视频质量**: 现有方法优化提示的语义丰富度，但不考虑对最终视频质量的影响
4. **LLM拒绝问题**: LLM 可能拒绝处理含敏感关键词的查询（如抽象表述"20 - 11 coins = 9 coins"）
