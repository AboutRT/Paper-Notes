...
1. **回放方法**（如Replay、SEEKR）：需要存储和回放历史数据，计算成本高且存在隐私风险，对于LLM海量预训练数据不切实际
2. **参数高效微调方法**（如LoRAMoE、O-LoRA）：使用固定数量的共享参数吸收所有任务知识，不同任务在共享模块上的顺序微调导致灾难性遗忘
3. **正则化方法**（如EWC）：在参数空间施加约束，但在LLM的巨大参数空间中确定参数重要性仍然困难
