...
1. **区域描述模糊**：模型生成的 grounded 区域描述经常不够详细，对象定位不准确
2. **注意力偏移**：模型可能被查询区域外的对象"分心"，未能聚焦于用户指定的区域
3. **缺乏正负样本反馈**：现有指令微调（SFT）仅优化模型模仿正样本（ground truth），无法惩罚推理时产生的错误定位。这与传统目标检测中正负样本训练机制的缺失形成对比
