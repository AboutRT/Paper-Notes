...
1. **现有方法假设完美对齐**：大多数深度学习方法使用逐像素损失（$\ell_1$/$\ell_2$），在错位存在时导致光谱失真、双边缘和模糊
2. **自适应修复不够灵活**：SIPSA 使用固定尺度对齐、LAGConv/CANConv 使用自相似性聚合而非显式几何对齐
3. **扩散模型虽质量好但过慢**：PanDiff 和 TMDiff 推理时间分别为 2.955s 和 9.997s，难以部署
