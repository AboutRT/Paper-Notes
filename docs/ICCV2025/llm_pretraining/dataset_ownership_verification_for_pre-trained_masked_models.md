...
1. **针对监督模型**：大多数 DOV 方法依赖数据点与决策边界的关系，无法适用于自监督模型；
2. **依赖后门水印**：在数据中注入水印会降低模型性能，且容易被水印移除技术对抗；
3. **不适用于掩码模型**：近期针对对比学习模型的 DOV 方法利用嵌入空间中的对比关系差距，但掩码建模（MAE、BEiT 等）与对比学习的代理任务差异显著，表征更难区分。
