...
1. **门基方案的局限**：数字量子电路是离散操作序列，难以表达 ResNet 和 Neural ODE 等具有连续动力学的模型
2. **混合方案不完全量子化**：Liang 等人的混合量子-经典模型仍依赖经典计算完成残差连接
3. **连续变量 QNN**：Killoran 等人利用连续变量基实现 ResNet，但未利用 Rydberg 原子系统的独特能力
4. **现有 Rydberg 原子工作**：Lu 等人的二值 MNIST 分类使用数字-模拟混合方案，不适用于现有仅支持模拟功能的商用设备
...
#### 1. **Rydberg 原子哈密顿量参数化**
- **做什么**：将 ResNet 的参数和数据编码到量子系统的物理控制量中
