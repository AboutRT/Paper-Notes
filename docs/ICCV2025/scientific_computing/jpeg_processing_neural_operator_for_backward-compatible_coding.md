---
description: "【论文笔记】JPEG Processing Neural Operator for Backward-Compatible Coding 论文解读 | ICCV 2025 | arXiv 2507.23521 | JPEG | 提出JPNeO，一个完全后向兼容JPEG格式的下一代编解码器，通过在编码和解码阶段分别引入神经算子(JENO和JDNO)以及可训练量化矩阵，显著提升JPEG重建质量（尤其是色度分量），同时保持低内存和少参数量的优势。"
tags:
  - ICCV 2025
  - JPEG
  - 神经算子
...
1. **编码端的信息损失不可逆**：JPEG的两大损失来源——量化和色度下采样——在编码端就丢失了信息，传统解码器受限于编码端约束的互信息
2. **色度分量恢复差**：现有JPEG伪影去除方法主要关注亮度分量，色度分量（CbCr）因下采样而严重退化
3. **编码和解码独立优化**：之前的工作要么优化编码要么优化解码，没有共享信息实现协同增效
4. **兼容性问题**：DNN编解码器无法与现有JPEG基础设施互操作
...
- **做什么**：用一个线性层学习优化的量化矩阵，替代JPEG标准的默认矩阵
