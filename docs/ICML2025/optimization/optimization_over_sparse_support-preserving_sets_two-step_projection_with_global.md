---
description: "【论文笔记】Optimization over Sparse Support-Preserving Sets: Two-Step Projection with Global Optimality Guarantees 论文解读 | ICML 2025 | arXiv 2506.08558 | sparse optimization | 针对带有额外支撑保持约束的稀疏优化问题，提出两步投影IHT算法（先硬阈值再投影凸集），在RSC/RSS条件下给出全局目标值保证（无系统误差），揭示稀疏度松弛与次优性间的新trade-off。"
tags:
  - ICML 2025
  - sparse optimization
  - IHT
...
## 研究背景与动机
1. **领域现状**: 稀疏优化中用l0约束相比凸松弛可精确控制稀疏度。IHT算法是标准方法，已有全局收敛保证。
2. **现有痛点**: 实际中常需额外约束（预算约束、范数约束等），现有算法要么需混合约束闭式投影（常不存在），要么仅提供局部保证。
3. **核心矛盾**: 全局保证的IHT不适用于混合约束；适用混合约束的方法仅有局部保证。两者之间的gap亟需填补。
4. **本文要解决什么**: 在支撑保持约束（lp球等）下获得全局目标值保证的IHT变种。
5. **切入角度**: 定义"支撑保持集"——k-稀疏向量投影后支撑不变（如lp球p>=1）。
6. **核心idea**: 用两步投影替代混合约束欧几里得投影：(1) 硬阈值选top-k坐标；(2) 投影到凸集。简单且适用面广。
