...
1. **领域现状**: 大语言模型（LLM）的预训练和微调需要巨大的内存。子空间优化算法如 GaLore（Zhao et al., 2024）通过将梯度投影到低秩子空间来减少优化器状态的内存占用，取得了良好的实践效果。

2. **现有痛点**: 虽然 GaLore 在实践中效果不错，但其收敛性缺乏理论保证。特别是在随机梯度设定下（标准 mini-batch SGD），GaLore 是否总能收敛到最优解是一个未解的基本问题。

3. **核心矛盾**: GaLore 通过 SVD 分解梯度来选择投影子空间。但在随机设定下，svd 分解的是随机梯度而非真实梯度，这引入了系统性偏差——投影方向与真实梯度方向不一致，这种偏差能否被消除？

4. **本文要解决什么**: (1) 明确 GaLore 的收敛极限，(2) 给出保证收敛的改进方案。

5. **切入角度**: 构造反例证明 GaLore 不收敛，然后分析收敛的充分条件，最后设计一种规避投影偏差的新算法。

6. **核心 idea**: 用随机投影（而非基于 SVD 的数据依赖投影）来避免系统性偏差，从而保证收敛。
