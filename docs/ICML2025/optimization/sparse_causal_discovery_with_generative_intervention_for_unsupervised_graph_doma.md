...
1. **因果-虚假特征纠缠**：图数据天然包含因果关系和统计相关性。传统方法仅依赖语义标签训练模型，无法区分二者。例如在 PTC 毒性预测数据集中，分子骨架结构是致癌性的因果因素，但性别、物种等实验变量仅是统计相关。未显式解耦时，虚假因素通过残余相关干扰跨域泛化。

2. **全局对齐策略失效**：现有对抗学习方法采用全局域分布对齐策略，容易导致信息坍塌——丢弃关键的稀有子结构，同时无法有效抑制虚假因素。图数据因结构复杂和高维稀疏性使这些问题更加严重。
