...
1. **外部验证器方法**（如 Luo et al., 2024）：依赖大规模外部模型做验证并触发重新生成，计算开销大
2. **RL 方法**（如 SCoRe, Kumar et al., 2024）：训练不稳定、计算量巨大（约 150 万次生成/3000步），且不显式建模中间推理步骤的验证
3. **Self-Refine 类方法**（Madaan et al., 2023）：在复杂任务上实际性能下降，且 LLM 本身缺乏自纠错能力（Huang et al., 2024 已论证）
