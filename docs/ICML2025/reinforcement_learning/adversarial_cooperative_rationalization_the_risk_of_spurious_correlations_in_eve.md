...
1. **领域现状**：协作理据化（RNP）是主流的模型无关自解释框架——生成器选择输入中最信息的子集作为理据，预测器基于理据进行预测，两者协作训练最大化准确率。
2. **现有痛点**：作者发现一个反直觉的现象——即使去掉"最大化准确率"目标让生成器随机选择，预测器仍能达到很高准确率。说明预测器可能利用了理据选择过程引入的虚假相关。
3. **核心矛盾**：生成器的采样过程改变了数据分布——$P(Y|Z)$ 在采样后可能不等于 $P(Y|Z|g)$，即使原始数据中 $Y \perp Z$。
4. **本文要解决什么**：检测和消除理据化过程引入的虚假相关。
5. **切入角度**：通过对抗攻击暴露虚假相关→用指令机制阻止预测器学习这些虚假模式。
6. **核心 idea**：对抗性检查+指令干预的两阶段方法。
