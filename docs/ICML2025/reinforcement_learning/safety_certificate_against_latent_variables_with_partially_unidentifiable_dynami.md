...
1. **动力学部分不可辨识**：完整状态 $(X_t, W_t)$ 无法获取，底层转移核 $\mathcal{P}(X_{t+1}, W_{t+1} | X_t, W_t, U_t)$ 无法完全辨识
2. **离线-在线分布偏移**：离线数据中行为策略 $\pi^b$ 依赖潜变量（如人类司机看到路滑会更用力刹车），导致 $P_{\text{offline}}(X_{t+1}|X_t,U_t) \neq P_{\text{online}}(X_{t+1}|X_t,U_t)$
