...
1. **模仿学习需要独立奖励模型**：IL 场景下目标分布密度未知，现有方法需先训练独立奖励模型再用 RL 技术求解，增加训练和计算成本
2. **FM 损失在连续设置中不可解**：朴素前向策略（如扩散模型中的加噪）导致 star inflow $f^*_\leftarrow$ 需要计算高维积分，计算上不可行
3. **无环性约束**：需要手工构造额外结构，而环路在朴素实现和 RL 环境中自然出现
4. **0-flow 不稳定性**：基于散度的损失函数在存在遍历测度时可能不稳定，这一理论预测尚未被实验验证
