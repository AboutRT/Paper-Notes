...
1. **现有 TTA 方法的脆弱性**: 在严重域偏移下，基于熵/置信度的自监督指标（如 TENT）不可靠，导致适应失败
2. **主动 TTA 的高成本**: 近期的 active TTA 方法需要完整类别标签，标注代价过高（50类分类任务中全标注平均 11.7 秒/样本，错误率 12.7%）
3. **二元反馈的效率**: 相比之下，二元比较只需 1.6 秒/样本，错误率仅 0.8%。从信息论角度，全标注需要 $\log(\text{num\_class})$ 倍于二元反馈的比特数
