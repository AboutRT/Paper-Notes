...
1. **拉格朗日方法**（PPO-Lag、TRPO-Lag）：将约束转化为惩罚项，但对偶变量的更新不稳定，容易在约束边界振荡，导致训练过程中频繁违反约束。
2. **惩罚/障碍方法**（IPO、P3O）：引入固定权重的惩罚项简化优化，但会改变原始目标函数，产生**优化偏差**——最优解不再是原始 CMDP 的最优解。
3. **CPO 类信赖域方法**：在信赖域和安全策略集的交集中优化，理论上安全但实际依赖嘈杂的代价优势估计，导致在约束边界附近高频振荡和超调。
