...
1. **领域现状**：SOTA RL 算法（MuZero、Agent57）需要分布式计算集群和数周训练时间，远超小型实验室和爱好者的能力。
2. **现有痛点**：Rainbow DQN 需要 34,200 GPU 小时（1435 天），新算法的计算需求更高——RL 不像 NLP/CV 有基础模型可微调，每个环境都需从头训练。
3. **核心矛盾**：高性能与计算可及性的矛盾。
4. **本文要解决什么**：设计一个在桌面 PC 上就能快速训练的高性能 RL 算法。
5. **切入角度**：延续 Rainbow DQN 的方法论——精选独立改进组件并组合。
6. **核心 idea**：选择 6 个兼顾性能和计算效率的改进，组合成 BTR。
