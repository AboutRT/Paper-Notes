...
**现有方法的不足**：
1. **智能体建模（AM）的表示与策略脱节**：标准 AM 学到的信念表示并非针对最大化价值函数优化，导致次优（如 LIAM、SoMM 等）
2. **未过滤冗余状态信息**：直接使用全局状态的压缩嵌入反而会损害性能，因为联合状态中存在对单个智能体不具信息量的冗余特征
3. **AM 未用于改善探索**：现有 AM 方法无法提升初始随机探索策略，在稀疏奖励环境中尤为低效
4. **实用性限制**：许多方法假设单一控制器、需要先验知识或执行时访问其他智能体的动作等
