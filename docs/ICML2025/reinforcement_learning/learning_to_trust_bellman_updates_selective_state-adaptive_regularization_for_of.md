...
1. **全局固定系数的局限**：现有方法（CQL、TD3+BC等）使用固定的全局正则化系数 $\beta$，但最优系数因任务、训练阶段、数据密度而异——弱正则化导致值过估计，强正则化退化为行为克隆
2. **训练动态变化**：训练初期策略不可靠需强约束，后期策略接近数据集后应放松约束以释放Bellman更新的潜力
3. **数据质量差异**：高数据密度的状态可以信任Bellman更新结果，低密度状态需收紧约束
4. **Offline-to-Online效率**：固定系数导致离线与在线Q值差距大，影响微调效率
