...
1. **基于期望的约束方法**（CPO、PID Lagrangian 等）：以累积代价的期望值作为约束，仅保证"平均"满足约束，忽略了代价分布的变异性，尤其是尾部的极端事件（黑天鹅事件），导致频繁的约束违反。
2. **基于概率的约束方法**（WCSAC、QCPO 等）：使用 CVaR 或分位数约束，但 WCSAC 用高斯近似代价分布，无法准确捕捉尾部衰减行为；QCPO 忽略了极端样本在训练中的关键影响。
