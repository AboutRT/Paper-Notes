...
1. **领域现状**：增强 LLM 推理能力的主要方法包括 chain-of-thought 提示、蒸馏强模型的推理轨迹、以及用 RL 优化最终答案正确性。
2. **现有痛点**：(a) 蒸馏依赖强模型（如 GPT-4），获取成本高; (b) 仅用最终答案奖励的 RL 忽略了中间推理步骤的质量; (c) 固定的训练数据限制了持续改进。
3. **核心矛盾**：高质量推理轨迹是稀缺资源，但 RL 训练需要大量多样化的推理轨迹。
4. **本文要解决什么**：如何在没有外部强模型的情况下，让 LLM 自我改进推理过程？
5. **切入角度**：自举——用当前模型生成推理轨迹，筛选高质量轨迹作为下一轮训练数据。
6. **核心 idea**：过程级奖励模型评估每个推理步骤的质量，RL 优化步骤级策略，迭代自举不断提升。
