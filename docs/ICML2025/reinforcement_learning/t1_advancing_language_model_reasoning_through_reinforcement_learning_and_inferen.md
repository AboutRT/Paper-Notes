---
description: "【论文笔记】T1: Advancing Language Model Reasoning through Reinforcement Learning and Inference Scaling 论文解读 | ICML 2025 | arXiv 2501.11651 | 强化学习 | T1 通过合成 CoT 数据初始化 + RL 训练中的过采样和熵奖励策略来鼓励探索，使开源 LLM 展现出推理时缩放（inference scaling）行为，在 MATH500、AIME2024 等挑战性数学推理基准上超越了 QwQ-32B-Preview。"
tags:
  - ICML 2025
  - 强化学习
  - 推理能力
...
1. **领域现状**：大语言模型（LLM）在复杂推理任务上展现了显著能力，但主流方法依赖模仿学习（imitation learning），即从专家解题路径中进行监督学习。这类方法的核心缺陷是无法通过自主探索发现新的推理策略。

2. **现有痛点**：虽然强化学习（RL）理论上可以让模型通过 self-exploration 和从反馈中学习来获得更强的推理能力，但已有的 RL 尝试在复杂推理任务上仅取得了modest的改进。主要原因有两个：（1）RL 训练中的采样多样性不足，模型容易陷入局部最优；（2）test-time scaling（推理时计算量增大能否换来更好结果）难以实现。

3. **核心矛盾**：如何让 RL 训练真正 scale up？关键在于——RL 需要充分的探索才能学到好策略，但大模型的 RL 训练中采样多样性极低，策略容易过早收敛。同时，现有方法在推理时即使给更多计算量也很难获得更好结果，因为它们缺乏有效的 inference scaling 机制。

4. **本文要解决什么**：如何在 RL 训练中鼓励探索以获得真正的推理能力提升，以及如何实现 inference scaling —— 即增加推理预算能直接带来性能改善。

5. **切入角度**：从 SFT 初始化和 RL 训练两个阶段切入——先用高质量合成 CoT 给模型一个好的起点，再通过精心设计的 RL 策略（过采样 + 熵奖励 + 动态锚点）扩大探索空间。

6. **核心 idea**：用融合 trial-and-error 和 self-verification 的 CoT 数据做 SFT 暖身，再用过采样增加 RL 训练中的采样多样性、用熵奖励防止策略坍缩、用动态锚点稳定奖励优化，最终实现推理能力和 inference scaling 的双重突破。
...
1. **合成 CoT 数据（SFT 初始化）**:
   - **做什么**：构造包含 trial-and-error 和 self-verification 的长 CoT 推理轨迹
...
4. **动态锚点正则化（Dynamic Anchor）**:
   - **做什么**：设置一个动态更新的参考策略（anchor policy），对当前策略进行正则化约束
...
4. **本文要解决什么**：如何通过 RL 有效提升 LLM 的推理能力，同时实现推理时缩放——即增加推理计算预算就能获得更好的性能。
...
1. **合成 CoT 数据（SFT 初始化）**:
   - **做什么**：构造融合了 trial-and-error（试错）和 self-verification（自我验证）的 CoT 训练数据
