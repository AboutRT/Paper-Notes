...
description: >-
  [ICML2025][多任务强化学习] 提出 M3DT 框架，将 MoE 引入 Decision Transformer 实现参数分离，通过任务分组将大量任务拆分为小子集分配给各专家，并设计三阶段训练机制依次优化骨干、专家、路由器，成功将离线多任务 RL 扩展到 160 个任务且性能持续随专家数增加而提升。
tags:
  - ICML2025
  - 多任务强化学习
  - Transformer
...
**本文要解决什么**：如何在任务数量极大（160 个）时既保持每个任务的学习质量，又实现高效的参数扩展？
...
**核心 idea 一句话**：用 MoE 实现"参数分离 + 任务分组"，增加专家数量同时扩展参数和降低任务负载，再配合三阶段训练避免梯度干扰。
...
1. **MoE 增强的 DT 架构**
    ...
    - 做什么：在 DT 每个 Transformer block 的 FFN 旁并行添加多个专家 FFN，保留原始 FFN 以维护骨干知识
...
2. **任务分组策略**
    ...
    - 做什么：将所有任务显式分为若干组，每组分配给一个专家独立训练
