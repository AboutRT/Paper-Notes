...
**[Constrained Hamiltonian Systems On Observation-Induced Fiber Bundles Theory Of S](constrained_hamiltonian_systems_on_observation-induced_fiber_bundles_theory_of_s.md)**
...
**[Diversity By Design Leveraging Distribution Matching For Offline Model-Based Opt](diversity_by_design_leveraging_distribution_matching_for_offline_model-based_opt.md)**
...
**[Efficient Optimization With Orthogonality Constraint A Randomized Riemannian Sub](efficient_optimization_with_orthogonality_constraint_a_randomized_riemannian_sub.md)**

:   提出随机黎曼子流形下降方法 (RSDM)，通过将每步更新限制在随机低维子流形上，将正交约束优化中 retraction 操作的复杂度从 $O(np^2)$ 降至 $O(r^3)$，同时保持与全空间黎曼梯度下降相匹配的总计算复杂度。

**[Enhancing Certified Robustness Via Block Reflector Orthogonal Layers And Logit A](enhancing_certified_robustness_via_block_reflector_orthogonal_layers_and_logit_a.md)**
...
**[Fishers For Free Approximating The Fisher Information Matrix By Recycling The Sq](fishers_for_free_approximating_the_fisher_information_matrix_by_recycling_the_sq.md)**
...
**[Glgenn A Novel Parameter-Light Equivariant Neural Networks Architecture Based On](glgenn_a_novel_parameter-light_equivariant_neural_networks_architecture_based_on.md)**
...
**[Gpu-Friendly And Linearly Convergent First-Order Methods For Certifying Optimal ](gpu-friendly_and_linearly_convergent_first-order_methods_for_certifying_optimal_.md)**
...
**[Improved Generalization Bounds For Transductive Learning By Transductive Local C](improved_generalization_bounds_for_transductive_learning_by_transductive_local_c.md)**
...
**[K2Ie Kernel Method-Based Kernel Intensity Estimators For Inhomogeneous Poisson P](k2ie_kernel_method-based_kernel_intensity_estimators_for_inhomogeneous_poisson_p.md)**

:   提出 K²IE——基于 RKHS 最小二乘正则化的核强度估计器，证明其 representer theorem 的对偶系数恒为 1，从而将经典核强度估计 (KIE) 与现代核方法在理论上统一，同时兼顾 KIE 的高效性与核方法的边缘校正优势。

**[Lapsum -- One Method To Differentiate Them All Ranking Sorting And Top-K Selecti](lapsum_--_one_method_to_differentiate_them_all_ranking_sorting_and_top-k_selecti.md)**
...
**[Maximum Coverage In Turnstile Streams With Applications To Fingerprinting Measur](maximum_coverage_in_turnstile_streams_with_applications_to_fingerprinting_measur.md)**
...
**[Optimal Sensor Scheduling And Selection For Continuous-Discrete Kalman Filtering](optimal_sensor_scheduling_and_selection_for_continuous-discrete_kalman_filtering.md)**
...
**[Position Solve Layerwise Linear Models First To Understand Neural Dynamical Phen](position_solve_layerwise_linear_models_first_to_understand_neural_dynamical_phen.md)**
...
**[Principled Algorithms For Optimizing Generalized Metrics In Binary Classificatio](principled_algorithms_for_optimizing_generalized_metrics_in_binary_classificatio.md)**
...
**[Provably Efficient Algorithm For Best Scoring Rule Identification In Online Prin](provably_efficient_algorithm_for_best_scoring_rule_identification_in_online_prin.md)**
...
**[Randomized Dimensionality Reduction For Euclidean Maximization And Diversity Mea](randomized_dimensionality_reduction_for_euclidean_maximization_and_diversity_mea.md)**

:   证明了对一大类欧氏最大化问题（最大匹配、最大TSP、最大生成树、子图多样性等），使用数据无关的高斯 JL 变换将维度降至 $O(\lambda)$（$\lambda$ 为数据集倍增维度）即可近似保持所有候选解的值，并证明该依赖是紧的。

**[Regression For The Mean Auto-Evaluation And Inference With Few Labels Through Po](regression_for_the_mean_auto-evaluation_and_inference_with_few_labels_through_po.md)**
...
**[Sassha Sharpness-Aware Adaptive Second-Order Optimization With Stable Hessian Ap](sassha_sharpness-aware_adaptive_second-order_optimization_with_stable_hessian_ap.md)**
...
**[Sparse Training From Random Initialization Aligning Lottery Ticket Masks Using W](sparse_training_from_random_initialization_aligning_lottery_ticket_masks_using_w.md)**
...
**[The Price Of Freedom Exploring Expressivity And Runtime Tradeoffs In Equivariant](the_price_of_freedom_exploring_expressivity_and_runtime_tradeoffs_in_equivariant.md)**

:   本文系统分析了 $E(3)$-等变神经网络中多种张量积操作的表达力与运行时间权衡，发现理论复杂度与实际性能差距悬殊，并提出基于球面网格的简化 Gaunt 张量积实现，在 MACE 原子间势能训练中加速 30%。

**[Theoretical Performance Guarantees For Partial Domain Adaptation Via Partial Opt](theoretical_performance_guarantees_for_partial_domain_adaptation_via_partial_opt.md)**
...
**[Wgformer An Se3-Transformer Driven By Wasserstein Gradient Flows For Molecular G](wgformer_an_se3-transformer_driven_by_wasserstein_gradient_flows_for_molecular_g.md)**
