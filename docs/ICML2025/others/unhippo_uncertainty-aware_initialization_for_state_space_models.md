...
## 研究背景与动机
1. **领域现状**：状态空间模型（SSM）如 S4、Mamba 等正成为序列建模的主导架构。HiPPO（High-order Polynomial Projection Operators）框架为 SSM 提供了优良的初始化方案，是 S4 系列成功的关键。HiPPO 通过在线逼近输入信号的多项式投影来初始化 $A, B$ 矩阵。
2. **现有痛点**：HiPPO 的核心假设是数据无噪声——输入信号被视为确定性控制信号。但实际数据通常包含观测噪声（传感器噪声、量化误差等），噪声会显著影响 SSM 的训练和推理质量。
3. **核心矛盾**：HiPPO 产生的初始化在无噪声下最优，但在有噪声场景中，这种初始化会将噪声无差别地传递到状态表示中，降低信号质量。
4. **本文要解决什么**：设计一种考虑噪声的 SSM 初始化方案。
5. **切入角度**：将 HiPPO 重新解读为线性随机控制/估计问题。
6. **核心idea**：数据不是确定性控制信号而是潜在系统的噪声观测，用 Kalman 滤波的思想推导出自动降噪的初始化。
