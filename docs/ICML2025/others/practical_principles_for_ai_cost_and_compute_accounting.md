---
description: >-
  提出AI成本与计算量核算七项原则，系统性封堵蒸馏漏洞等监管规避手段，兼顾防博弈性、风险缓解激励与跨企业一致性。
tags:
  - ICML2025
  - AI治理
  - 计算审计
...
1. **原则1: 计入项目全部上游成本和计算量**:
    - 做什么：要求开发者报告最终 AI 系统上游的所有技术成本和计算量
...
2. **原则3+6: 排除纯安全活动 + 使用独立双阈值**:
    - 做什么：允许开发者豁免仅用于降低社会风险的活动（CSAM 过滤、安全测试等）；同时要求成本和计算量各有独立阈值
...
3. **原则5+7: 分项核算报告 + 定期更新标准**:
    - 做什么：要求开发者提交可审计的分项报告（每项活动的描述、目的、估算方法、最终计算量）；标准需季度或半年度更新
