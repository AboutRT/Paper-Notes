...
## 研究背景与动机
1. **领域现状**: DRO通过优化在最坏情况分布下的期望来做鲁棒决策。常用KL散度或Wasserstein距离定义不确定集。
2. **现有痛点**: KL散度不确定集可能包含不合理的分布；Wasserstein不确定集在高维时保守。需要利用先验知识（如分布族信息）的更精细不确定集。
3. **核心矛盾**: 不确定集太大→过于保守；太小→缺乏鲁棒性。需在两者间取平衡。
4. **本文要解决什么**: 在指数族框架下精确构造不确定集并高效求解DRO。
5. **切入角度**: 利用指数族在自然参数空间的结构——不确定集定义在自然参数上。
6. **核心idea**: 指数族DRO将不确定集限制为同族分布的邻域，比一般KL-DRO更精细，且有高效的对偶求解方法。
