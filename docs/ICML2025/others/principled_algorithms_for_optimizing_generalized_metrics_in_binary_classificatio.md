---
description: "【论文笔记】Principled Algorithms for Optimizing Generalized Metrics in Binary Classification 论文解读 | ICML 2025 | arXiv 2512.23133 | generalized metrics | 本文提出了优化广义分类指标（如 $F_\beta$、Jaccard、加权准确率等）的有原则算法 METRO，基于 $H$-一致性界和代理损失理论，将指标优化重新表述为广义代价敏感学习问题，具有有限样本泛化保证。"
tags:
  - ICML 2025
  - generalized metrics
  - F-measure
...
## 研究背景与动机
1. **领域现状**：在类别不平衡或非对称代价场景中，$F_\beta$-score、AM measure、Jaccard 系数等广义指标比标准 0-1 损失更合适。然而，这些指标通常是不可分解的（non-decomposable），无法写成单样本损失之和。
2. **现有痛点**：现有方法通常依赖 Bayes 最优分类器的刻画，先估计类概率再寻找最优阈值。这导致：(1) 算法未针对受限假设集裁剪；(2) 缺乏有限样本性能保证。
3. **核心矛盾**：理论上需要直接优化目标指标，但这些指标非凸非可分解，难以直接优化。
4. **本文要解决什么**：为广义指标的优化提供有原则的算法，同时具有 $H$-一致性和有限样本泛化界。
5. **切入角度**：将指标优化重新表述为广义代价敏感学习，设计具有可证明 $H$-一致性的代理损失。
6. **核心idea**：对每种广义指标，存在一个等价的代价敏感学习问题，可以用设计好的代理损失来高效优化。
...
1. **广义指标到代价敏感学习的归约**:
   - 做什么：证明优化 $\Psi$ 等价于求解一个特定的代价敏感分类问题
...
2. **$H$-一致性代理损失**:
   - 做什么：设计新的代理损失函数，满足对受限假设集 $H$ 的一致性
