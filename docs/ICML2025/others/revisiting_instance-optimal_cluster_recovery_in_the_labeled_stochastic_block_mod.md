---
description: >-
  提出IAC算法，在标签随机块模型(LSBM)中首次实现匹配实例最优下界的社区恢复，兼具期望和高概率保证，计算复杂度仅O(n polylog(n))。
tags:
  - ICML2025
  - 社区检测
  - 随机块模型
...
1. **改进的谱聚类初始化**:
    - 做什么：从观测的标签邻接矩阵中提取粗糙的簇估计，同时自动确定簇数 $K$
    - 核心思路：
...
3. **"良行"节点集 $H$ 的分析框架**:
    - 做什么：将所有节点分为可被正确分类的"良行"集 $H$ 和不可避免会被误分的 $\mathcal{I} \setminus H$
