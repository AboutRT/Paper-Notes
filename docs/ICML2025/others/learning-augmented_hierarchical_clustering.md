---
description: "【论文笔记】Learning-Augmented Hierarchical Clustering 论文解读 | ICML 2025 | arXiv 2506.05495 | hierarchical clustering | 本文研究借助分裂预言机（splitting oracle）的辅助信息来突破层次聚类的近似硬度障碍，获得 Dasgupta 目标的 $O(1)$ 常数近似和 Moseley-Wang 目标的 $(1-o(1))$ 近似，并推广到流式和并行计算场景。"
tags:
  - ICML 2025
...
## 研究背景与动机
1. **领域现状**：层次聚类（HC）是数据分析的基础技术，将数据集递归划分为树状结构。Dasgupta (STOC'16) 提出了经典的层次聚类目标函数，Moseley-Wang (NeurIPS'17) 提出了对偶目标。
2. **现有痛点**：对于这两个目标函数，存在很强的近似硬度障碍。在 Small Set Expansion Hypothesis (SSEH) 下，Dasgupta 目标不存在多项式时间的常数近似算法，Moseley-Wang 目标不存在 $(1-C)$ 近似。
3. **核心矛盾**：理论上不可能的高质量近似 vs 实际应用对高质量聚类的迫切需求。
4. **本文要解决什么**：利用自然的辅助信息（oracle）来突破近似硬度。
5. **切入角度**：Learning-Augmented Algorithms 范式——允许算法访问（可能有噪声的）预言机。
6. **核心idea**：分裂预言机（splitting oracle）提供三元组关系信息，可以指导层次聚类突破传统硬度限制。
