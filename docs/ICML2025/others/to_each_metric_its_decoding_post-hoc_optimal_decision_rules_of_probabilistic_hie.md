---
description: "【论文笔记】To Each Metric Its Decoding: Post-Hoc Optimal Decision Rules of Probabilistic Hierarchical Classifiers 论文解读 | ICML 2025 | arXiv 2506.01552 | hierarchical classification | 本文提出了针对概率层次分类器的后处理最优解码框架，为不同评价指标（如层次 F_β）推导了最优决策规则，在候选集限于节点集时给出通用算法，对子集预测推导了专门的层次 hF_β 最优策略。"
tags:
  - ICML 2025
...
## 研究背景与动机
1. **领域现状**：层次分类利用标签的树状结构来区分错误严重程度（如将"狗"错分为"猫"比错分为"汽车"更可接受）。现有方法通常训练概率模型后用启发式规则做预测解码。
2. **现有痛点**：启发式解码（如选择最大概率的叶节点，或沿树贪心搜索）未必与评价指标对齐。不同指标需要不同的最优策略，但缺乏系统的最优解码理论。
3. **核心矛盾**：概率模型提供了丰富的不确定性信息，但解码时未充分利用这些信息来优化目标指标。
4. **本文要解决什么**：给定一个已训练的概率层次分类器和目标评价指标，如何后处理（解码）以最优化该指标。
5. **切入角度**：Bayes 最优决策理论——条件期望损失最小化。
6. **核心idea**：不同指标的最优解码规则不同，作者为一系列指标推导了解析或算法化的最优规则。
...
2. **子集预测的最优规则（$hF_\beta$ 专用）**:
   - 做什么：当预测可以是一组节点时（如预测一条从根到叶的路径），推导层次 $F_\beta$ 的最优策略
