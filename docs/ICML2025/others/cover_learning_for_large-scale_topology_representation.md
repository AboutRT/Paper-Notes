...
description: >-
  [ICML2025][拓扑数据分析] 提出 Cover Learning 作为统一的无监督学习问题，通过优化学习数据集的拓扑忠实覆盖，结合测度、几何、拓扑三项损失函数和模糊覆盖参数化，实现比几何复形更紧凑、比 Mapper 图更高维的大规模拓扑表示。
tags:
  - ICML2025
  - 拓扑数据分析
  - 无监督学习
...
**本文要解决什么**：如何直接学习数据集的覆盖（而非依赖预设的滤波函数或全局距离尺度），使得通过 Nerve 构造所得的单纯复形既紧凑又拓扑忠实？
...
**核心 idea 一句话**：将覆盖学习视为优化问题，通过模糊覆盖的连续参数化和三项损失（测度+几何+拓扑）的梯度下降，直接学习拓扑忠实的覆盖。
...
1. **模糊覆盖的连续参数化**
    ...
    - 做什么：将离散的覆盖空间嵌入到连续的函数空间中，使优化成为可能
...
2. **三项可计算损失函数（Theorem 3.2）**
    ...
    - 做什么：将 Cover Learning 的三个形式化目标（测度小、边界规则、拓扑忠实）转化为可计算并可微的损失
