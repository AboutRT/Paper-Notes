...
## 研究背景与动机
1. **领域现状**：机器学习模型部署到安全关键领域（医疗、金融、自动驾驶）后，需要持续监控其性能。然而部署环境中通常没有真实标签可用于直接评估。
2. **现有痛点**：现有方法要么需要标签（不现实），要么依赖分布漂移检测（只能检测输入分布变化，不能直接关联到性能下降），要么依赖校准（但校准在漂移下也会失效）。
3. **核心矛盾**：部署后需要可靠评估 but 没有标签；检测分布漂移 ≠ 检测性能退化。
4. **本文要解决什么**：在无标签场景下，直接检测分类器准确率是否显著低于在标签测试集上的表现。
5. **切入角度**：利用模型输出中对协变量偏移敏感、与预测错误相关的特征作为代理信号。
6. **核心idea**：比较模型在测试集和用户数据上的"适用性信号"分布差异，通过假设检验做出性能是否退化的决策。
...
1. **适用性信号（Suitability Signals）**:
   - 做什么：从模型输出中提取对协变量偏移敏感且与错误率相关的特征
...
3. **模块化设计**:
   - 做什么：使信号选择、检验方法、阈值设定相互独立，便于适配不同场景
