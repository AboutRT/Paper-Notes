---
description: "【论文笔记】Latent Variable Estimation in Bayesian Black-Litterman Models 论文解读 | ICML2025 | arXiv 2505.02185 | Black-Litterman 模型 | 将经典 Black-Litterman 组合优化模型中的主观投资者观点 $(q, \Omega)$ 视为隐变量，通过贝叶斯网络从市场特征数据中自动推断，消除对人工主观输入的依赖，在 30 年道琼斯和 20 年 ETF 数据上 Sharpe 比率提升约 50%、换手率降低约 55%。"
tags:
  - ICML2025
  - Black-Litterman 模型
  - 组合优化
...
1. **主观偏差**：不同投资者给出的观点差异大，结果不可复现
2. **外部估计器的误差传播**：已有工作用 GARCH、LSTM、SVM 等外部模型生成 $(q, \Omega)$，但将独立估计器嵌入 BL 框架会引发多阶段误差累积
3. **不一致的置信度校准**：独立估计 $q$ 和 $\Omega$ 可能导致过度自信（低 $\Omega$）放大 $q$ 中的误差
