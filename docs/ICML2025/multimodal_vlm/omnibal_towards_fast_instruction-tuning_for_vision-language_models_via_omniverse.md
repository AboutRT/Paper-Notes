...
1. **数据不平衡**：VLM 输入包含变长文本和变数量图像，导致 mini-batch 大小剧烈波动（输入大小标准差高达 $1.4K \pm 0.9K$ tokens）
2. **模型不平衡**：ViT 和 LLM 的 transformer block 计算量差异大（前向时间标准差达 $85 \pm 93$ ms）
3. **内存不平衡**：动态输入让 GPU 内存需求波动（$39 \pm 23$ G），迫使使用最激进的重计算策略
