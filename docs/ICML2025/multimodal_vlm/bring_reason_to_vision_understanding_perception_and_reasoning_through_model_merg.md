...
1. **领域现状**：VLM 在视觉感知+语言任务上表现优秀，但在复杂多模态推理（如数学图表解读）上远落后于纯文本 LLM，部分原因是多模态推理数据匮乏。
2. **现有痛点**：提升 VLM 推理能力通常需要收集大量多模态推理数据并微调，成本高昂。
3. **核心矛盾**：感知和推理能力能否解耦？推理能力能否从 LLM 直接迁移到 VLM？
4. **本文要解决什么**：探索模型融合作为跨模态能力迁移的途径。
5. **切入角度**：VLM 的文本部分与 LLM 共享相同架构和初始化，满足模型融合的连通子空间假设。
6. **核心 idea**：对 VLM 和数学 LLM 的文本参数做加权平均，实现零训练的推理能力迁移。
