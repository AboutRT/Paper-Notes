...
## 研究背景与动机
- 多模态信息瓶颈（MIB）方法通过最大化表示与标签的互信息、最小化与输入的互信息来平衡充分性与简洁性
- **现有问题一**：正则化参数 β 通常靠手工调参，取值不当会导致要么保留冗余信息、要么丢失任务相关信息，无法保证最优 MIB
- **现有问题二**：各模态的正则化权重固定，忽略了模态间任务相关信息不平衡问题（弱模态可能携带少量但关键信息）
- **现有问题三**：已有理论未同时覆盖一致性、互补性、特异性、充分性、简洁性五个信息论因素
- 作者首次给出了 β 的理论上界 $M_u$，在此范围内优化可保证达到最优 MIB
