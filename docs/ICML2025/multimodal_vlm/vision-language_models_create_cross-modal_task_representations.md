...
1. **领域现状**: 自回归 VLMs（如 Idefics2、LLaVA 等）可以在单个模型里处理多种任务——给不同的 in-context examples 或指令就能切换任务。这种灵活性的内部表征机制尚不清楚。
...
3. **核心矛盾**: VLMs 明明可以分别处理文本和图像任务，但跨模态 few-shot prompting 却大幅失败。这说明**full prompt 的跨模态传递**有问题，但如果存在一种**压缩的任务表征**，它有可能跨越模态鸿沟吗？

4. **本文要解决什么**: 探究 VLMs 内部是否形成了**模态无关**的共享任务表征，以及这种表征如何被利用来修复跨模态失败。

5. **切入角度**: 借鉴 LLM 中 task vector 的研究范式，设计**跨模态 patching 实验**——从一个模态提取任务向量，注入到另一个模态的推理过程中。

6. **核心idea一句话**: VLM 中存在模态无关的任务向量，一个模态的任务向量可直接用于驱动另一个模态的正确生成。
