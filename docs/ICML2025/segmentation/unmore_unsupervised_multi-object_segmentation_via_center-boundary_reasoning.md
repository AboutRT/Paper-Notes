...
1. **Slot-based 方法**（如 SlotAttention）：依赖图像重建目标学习物体表征，在合成数据上有效但难以扩展到复杂真实场景
2. **自监督特征蒸馏方法**（如 TokenCut、CutLER、CuVLER）：利用 DINO/v2 预训练特征的物体定位线索发现多目标，但仍存在 **欠分割** 问题——倾向于将相邻物体聚合为一个
