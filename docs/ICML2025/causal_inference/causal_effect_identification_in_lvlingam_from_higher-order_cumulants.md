...
1. **领域现状**：因果效应识别旨在从观测数据预测未见干预的影响，在医学、政策评估、公平决策等领域至关重要。结构因果模型（SCM）是主流框架，但潜在混淆通常使因果效应不可识别。
2. **现有痛点**：
...
3. **核心矛盾**：保守假设（多代理/充足工具变量）在实际中难以满足，但放松这些假设后可识别性是否仍然成立？
4. **本文要解决什么**：在更弱假设下利用高阶累积量识别因果效应。
5. **切入角度**：非高斯分布的高阶累积量（三阶及以上）包含超越协方差的额外信息——这些信息可用于在单代理/欠定 IV 设置下识别因果效应。
6. **核心 idea**：高阶累积量 = 非高斯性的额外"信号"→在协方差矩阵信息不足时弥补缺失的方程→实现此前不可能的识别。
