...
1. **领域现状**：Diffusion Transformer（DiT）在图像生成任务上表现卓越，但推理成本极高——每次生成需要多步去噪，每步都要完整前向传播整个 Transformer，导致实际部署面临巨大挑战。
2. **现有方法**：特征缓存（Feature Caching）是一种有效的加速策略，核心思想是存储并复用相邻去噪步骤间的冗余计算特征，从而跳过部分层的计算。现有学习型缓存方法（如 Learning-to-Cache）通过训练一个轻量路由器来自适应决定哪些层可以缓存。
3. **现有痛点**：现有学习型缓存存在两个关键不对齐问题：
...
4. **本文要解决什么**：如何让缓存路由器的训练过程与实际推理过程一致？如何让优化目标直接关联最终图像质量？
5. **切入角度**：核心观察——训练时应模拟推理的逐步去噪过程（而非独立训练每步），且应用图像域的误差信号指导缓存决策。
6. **核心 idea**：通过"协调"（Harmonize）训练和推理来构建更好的特征缓存——SDT 让训练看到推理时的真实中间状态，IEPO 让优化目标从 noise 空间转向 image 空间。
...
1. **Step-Wise Denoising Training (SDT)**:
   - 做什么：在训练缓存路由器时模拟完整的逐步去噪过程，而非独立训练每个时间步
...
2. **Image Error Proxy-Guided Objective (IEPO)**:
   - 做什么：用一个高效的代理函数近似最终图像误差，替代传统的 noise prediction loss 来指导缓存决策
...
3. **缓存路由器设计**:
   - 做什么：为 DiT 每一层在每个去噪步决定是"计算"还是"从缓存读取"
