...
1. **缺乏面向单步研究任务的系统基准**：已有基准多关注代码实现和模型训练（MLAgentBench 等），忽略了 idea 生成、实验规划、论文审稿等认知密集型环节。
2. **需要可自动化的评估指标**：生成 idea 等任务需要大量人工评估（Si et al., 2024），阻碍了大规模对比。
3. **单步任务透明度更高**：相比复杂任务链，单步任务有明确的输入输出预期，能更精确地定位模型能力边界。
