...
1. **基于演示的 grounding**：直接从人类演示中学习，反映人类意图，而非仅依赖先验世界知识
2. **归纳泛化**：从几个特定高度的塔推断出构建任意高度塔的通用程序
3. **层次化组合**：复杂概念可以表示为简单概念的组合（如楼梯 = 递增高度的塔序列）
4. **约束可修改性**：学到的概念表征能灵活适应新指令中的附加约束（如颜色限制）
