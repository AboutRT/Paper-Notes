...
1. **设计空间巨大**：以 $12 \times 24$ 的网格为例，每个单元有金属/空两种状态，总搜索空间为 $2^{288} \approx 10^{86}$，远超 NAS ($10^4 \sim 10^{18}$) 和分子设计 ($10^6$)。
2. **评估代价极高**：每次评估需要求解 Maxwell 方程的全波电磁仿真，单次仿真耗时 660 到 42,780 秒不等，无法用简单解析近似替代。
