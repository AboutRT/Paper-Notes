...
1. **二次复杂度瓶颈**: 标准 self-attention 计算所有成对交互，复杂度为 O(N²)，当节点数达到数万级别时计算代价不可接受
2. **尺度鸿沟**: 计算化学中模型通常在几十个原子的分子上训练和验证，但实际的分子动力学模拟往往涉及数千个原子，小尺度验证的模型在大尺度上缺乏必要的架构组件
3. **长程交互与多尺度耦合**: 物理系统中存在慢衰减势函数导致的长程效应，以及不同尺度间的耦合，需要同时捕获局部精细特征和全局特征
4. **不规则几何**: 点云和非均匀网格无法直接套用图像领域的 patch 策略
