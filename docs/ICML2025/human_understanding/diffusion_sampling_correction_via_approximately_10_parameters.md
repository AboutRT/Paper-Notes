---
title: >-
  [论文解读] PAS: Diffusion Sampling Correction via Approximately 10 Parameters
description: >-
  [ICML2025][扩散模型加速] 提出PCA-based Adaptive Search (PAS)——利用PCA在低维子空间中获取采样轨迹基向量，仅学习约10个坐标参数修正快速求解器的截断误差，以亚分钟训练和即插即用方式大幅提升扩散模型的少步采样质量。
tags:
  - ICML2025
  - 扩散模型加速
  - PCA采样修正
...
**代码**: https://github.com/onefly123/PAS (有)  
**领域**: 扩散模型 / 采样加速  
...
**本文要解决什么**：设计一种即插即用的极低参数方法，让现有快速求解器（DDIM、iPNDM等）在NFE<10时也能获得高质量采样，同时保留原始扩散路径的插值能力。

**切入角度**：作者观察到两个关键几何特性——(1) 单个样本的采样轨迹处于高维空间的约3维子空间中；(2) 不同样本的累积截断误差呈统一的"S"形分布。前者意味着可以用PCA提取极少基向量来表达采样方向，后者意味着只在高曲率区域修正即可跳过大部分步骤。

**核心idea一句话**：用PCA将高维采样方向修正问题降维到低维坐标搜索问题，只需约10个参数和亚分钟训练即可大幅改善采样质量。
...
1. **PCA基向量采样修正**：
    ...
    - 做什么：在每个采样步 $t_i \to t_{i-1}$ 中，利用PCA从已有采样轨迹 $\{x_{t_N}, d_{t_N}, ..., d_{t_{i+1}}\}$ 中提取少量正交基向量，然后学习对应坐标 $\mathbf{C}=[c_1, c_2, c_3, c_4]$ 来纠正采样方向 $d_{t_i}$。
...
2. **自适应搜索策略**：
    ...
    - 做什么：根据"S"形截断误差分布，自动判断哪些采样步需要修正、哪些可以跳过。
...
3. **SGD坐标训练**：
    ...
    - 做什么：用SGD优化坐标参数，使修正后的采样点逼近ground truth轨迹。
