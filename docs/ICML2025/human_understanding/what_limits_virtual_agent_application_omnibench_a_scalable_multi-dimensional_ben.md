...
1. **领域现状**: 基于多模态大语言模型（MLLM）的虚拟 Agent 近年来取得了显著进展，能够在手机、电脑等平台上自主完成界面操作任务。评估这些 Agent 的 benchmark（如 AITW、OSWorld）越来越多。
...
3. **核心矛盾**: 如何在保证任务质量的同时实现大规模、多场景、多维度的 Agent 评估？人工标注质量高但不可扩展，自动生成可扩展但质量难控。

4. **本文要解决什么**: 构建一个既可扩展又能多维评估虚拟 Agent 能力的基准。

5. **切入角度**: 利用图（graph）结构表示任务——每个节点为原子操作（subtask），边为依赖关系。通过子任务组合自动合成可控复杂度的复合任务。

6. **核心 idea**: 将 Agent 任务建模为有向无环图（DAG），通过图的拓扑结构自动控制任务复杂度，通过图的节点类型自然引入多种能力维度的评估。
...
2. **跨平台自动化流水线（Cross-Platform Automated Pipeline）**:
   - 做什么：将抽象图任务自动转化为不同平台（Android、Web、Desktop）上的具体任务
