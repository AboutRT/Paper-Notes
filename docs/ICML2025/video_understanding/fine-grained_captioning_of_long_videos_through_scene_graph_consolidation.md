...
对此，现有解决方案分为三类，但各有缺陷：
1. **Memory-based / 递归框架**：需要在目标数据集上进行监督微调，泛化性差
2. **LLM-based 汇总方法**（如 VidIL、Video ChatCaptioner）：利用 LLM 总结多片段信息，无需微调但**推理开销巨大**（需调用商用 GPT API、参数量 7B+），且 LLM 有时会忽略场景细节或产生幻觉
3. **零样本方法**（如 ZeroCap、MAGIC）：通过 CLIP 引导语言模型，但在复杂事件视频上效果不佳
