...
1. **领域现状**：大模型的自回归解码是逐 token 串行生成的，每个 token 必须等前一个生成完毕才能开始，这严重限制了现代 GPU 并行计算能力的发挥。随着模型规模膨胀（数十亿到万亿参数）和长链式推理（long CoT）等应用的兴起，解码延迟成为关键瓶颈。
...
3. **核心矛盾**：加速解码的需求与输出一致性保证之间的矛盾——现有方法要么需要额外模型资源，要么牺牲输出质量。

4. **本文要解决什么？**
   - 如何在不引入辅助模型、不修改原模型参数的前提下加速解码？
   - 如何保证加速后的输出与标准自回归解码完全一致？

5. **切入角度**：作者观察到许多 token（特别是简单、高可预测性 token）在中间层就能被准确预测，后续层对预测结果几乎不产生改变。问题在于原始 LM Head 无法直接利用中间层表示做预测（因为它只在最后一层训练过）。

6. **核心 idea 一句话**：在中间层引入轻量级 LM Head 实现高置信度早期预测 token，将剩余层的计算延迟到后续 token 处理时并行完成，并通过验证步骤确保输出一致性。
...
1. **轻量级中间层 LM Head**
   - 做什么：在选定的中间层（如第 8、16、24 层）引入可训练的 LM Head，用于预测下一个 token
...
2. **自适应层并行（Adaptive Layer Parallelism）**
   - 做什么：根据中间层预测置信度动态决定在哪一层提前预测 token，并将未完成的计算延迟并行化
...
3. **验证与回退机制**
   - 做什么：在所有延迟计算完成后，验证早期预测的 token 是否与标准自回归解码结果一致
