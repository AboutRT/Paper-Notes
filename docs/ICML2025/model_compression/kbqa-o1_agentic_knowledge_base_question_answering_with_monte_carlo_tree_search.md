...
1. **KB 环境感知不足**：端到端方法直接生成逻辑形式，难以感知 KB 中的实体和关系，导致生成不合法的查询
2. **局部最优 vs 搜索空间爆炸**：CoT 方法易陷入局部最优，ToT 方法虽扩展搜索空间但面临指数级增长
3. **高度依赖标注数据**：训练开源 LLM 需要大量高质量人工标注，但对大规模 KB 标注成本极高
