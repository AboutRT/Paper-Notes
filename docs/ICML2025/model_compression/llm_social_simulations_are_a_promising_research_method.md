...
## 研究背景与动机
1. **领域现状**: LLM 模拟人类研究对象（如调查回答、实验参与者行为）的准确性和可验证性有望提供理解人类行为的可及数据源。
2. **现有痛点**: 结果参差不齐——有些研究显示 LLM 能模拟人类反应，有些显示存在系统偏差。少有社会科学家采用这一方法。
3. **核心矛盾**: LLM 能力快速提升，但缺乏系统的评估框架来判断何时/如何可靠地使用 LLM 社会模拟。
4. **本文解决什么**: 系统论证 LLM 社会模拟的可行性，识别关键挑战并提出解决方向。
5. **切入角度**: 综合综述实证结果、评论文章和相关工作。
6. **核心 idea**: 五个可解决的挑战——(1) 代表性偏差; (2) 响应格式偏差; (3) 评估方法论; (4) 提示设计; (5) 概念框架——逐一解决后，LLM 社会模拟将成为强大的研究工具。
...
4. **本文要解决什么**：系统梳理挑战，论证它们是可以解决的，并为未来研究提供路线图。
