...
1. **Prompt/Prefix Tuning**: 在输入前附加可训练向量，用梯度下降优化。但这些向量通常用随机 token 初始化，完全没有利用 few-shot 示例中的任务相关信息。
2. **Test-Time Training (TTT)**: 在推理时用 LoRA 微调模型权重，效果好但计算代价是示例数的**二次方**（需要对示例做排列组合）。
