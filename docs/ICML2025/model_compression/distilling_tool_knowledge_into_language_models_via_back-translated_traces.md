---
description: "【论文笔记】Distilling Tool Knowledge into Language Models via Back-Translated Traces 论文解读 | ICML 2025 (Workshop: Multi-Agent Systems in the Era of Foundation Models) | arXiv 2506.19171 | 工具集成推理 | 本文提出一个多智能体回译流水线，先用 Solver Agent 调用工具（代码解释器）解数学题生成 TIR trace，再用 Translator Agent + Rephrase Agent 将工具调用轨迹转化为纯自然语言推理链，最终用这些合成数据微调小模型，使其在无需工具访问的情况下内化工具知识和结构化推理能力。"
tags:
...
1. **领域现状**：大语言模型（LLM）在需要精确计算或多步代数推理的数学问题上经常出错。工具集成推理（Tool-Integrated Reasoning, TIR）通过调用外部工具（如 Python 代码解释器）确保计算正确性，是当前提升 LLM 数学能力的主流方案。
...
3. **核心矛盾**：TIR 提供了准确性但牺牲了部署灵活性；纯自然语言推理灵活但不准确。能否将工具带来的准确性和结构化推理"蒸馏"到纯自然语言推理中？

4. **本文要解决什么**：设计一种方法将工具知识纯粹通过自然语言蒸馏到 LLM 中——训练时利用工具生成高质量推理过程，推理时完全不依赖工具。

5. **切入角度**：不是直接让小模型模仿工具调用，而是用回译（back-translation）将工具调用链转化为等价的自然语言解释。这样小模型学到的是"工具会做什么"的知识，而非"如何调用工具"。

6. **核心 idea**：用多智能体协作的回译流水线（Solver → Translator → Rephrase），将交织了规划、工具调用、反思的 TIR trace 转化为流畅连贯的纯自然语言推理链。
...
1. **Solver Agent（求解智能体）**:
   - **做什么**：给定数学问题，交替进行规划（planning）、工具调用（symbolic tool calls）和反思推理（reflective reasoning）来求解
...
2. **Translator Agent（翻译智能体）**:
   - **做什么**：对 TIR trace 中每个工具调用，生成对应的自然语言解释
...
3. **Rephrase Agent（润色智能体）**:
   - **做什么**：将 Translator 输出的逐步解释合并为一篇流畅、全局连贯的推理叙述
