...
## 研究背景与动机
1. **领域现状**: LoRA 是 LLM 微调标准范式，但训练仍需 GPU。
2. **现有痛点**: 即使 LoRA 参数少，计算梯度仍需 GPU 前向/反向传播，只有 CPU 的用户无法使用。
3. **核心矛盾**: LoRA 的参数高效性未转化为计算可及性。
4. **本文解决什么**: 纯 CPU 上生成 LoRA adapter。
5. **切入角度**: 将"数据集→LoRA"映射抽象为元学习问题。
6. **核心 idea**: 不训练 LoRA，而是从预训练 adapter 库中组合最适合目标数据集的 adapter。
