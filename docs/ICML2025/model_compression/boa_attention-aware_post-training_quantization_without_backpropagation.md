...
1. **领域现状**：PTQ 是 LLM 部署的关键技术。GPTQ 等方法使用 Hessian 信息逐层优化量化权重，但假设层间独立。
2. **现有痛点**：逐层独立优化忽略了注意力模块中 Q/K/V/O 投影层之间的交互——一层的量化误差会通过注意力传播并影响其他层的最优量化。
3. **核心矛盾**：考虑跨层依赖需要更大的 Hessian 矩阵，计算和内存开销巨大。
4. **本文要解决什么**：如何在无反向传播框架下高效考虑注意力层间的依赖性？
5. **切入角度**：将 Hessian 从逐层重建误差扩展到注意力模块级重建误差。
6. **核心 idea**：注意力感知 Hessian + Hessian 松弛 + 逐头同时量化，在精度和效率间取得平衡。
