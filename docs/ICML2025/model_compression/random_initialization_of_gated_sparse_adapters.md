...
- 微调 LLM 学习新任务时面临**灾难性遗忘**问题
- LoRA 通过低秩约束实现参数高效微调，但低秩限制在复杂任务上表现欠佳
- 稀疏微调提供了一种替代方案，不施加秩约束，可能允许更具表达力的适应
- 彩票假说 (LTH) 表明密集网络中存在稀疏子网络能匹配全网性能
- 现有稀疏微调方法（如 LT-SFT）将差分矩阵 $\Delta W$ 初始化为零，无法包含"幸运"初始化
