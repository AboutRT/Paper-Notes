---
description: "【论文笔记】Core Context Aware Transformers for Long Context Language Modeling 论文解读 | ICML 2025 | arXiv 2412.12465 | 长上下文建模 | 提出 Core Context Aware (CCA) Attention，通过全局感知池化将输入 token 动态压缩为少量核心 token，结合局部保持模块捕获邻近细粒度信息，实现即插即用地替换标准自注意力，在 128K 上下文下获得 7.9× 加速和 46% 显存节省，同时保持建模性能。"
tags:
  - ICML 2025
  - 长上下文建模
  - 高效注意力
...
1. **冗余信息激增**：并非所有上下文对目标 token 都同等重要。注意力得分在大多数层和头中呈现高度稀疏分布，大量计算被浪费在冗余上下文上。
2. **计算与存储开销**：标准自注意力的计算复杂度为 $O(L^2)$，KV 缓存存储为 $O(L)$，在超长上下文下成为严重瓶颈。
