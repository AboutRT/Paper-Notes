---
description: >-
  DriveGPT提出1.4B参数自回归驾驶行为模型，在1.2亿真实驾驶片段上训练，首次系统探索驾驶行为建模的数据/模型/计算缩放定律。
tags:
  - ICML2025
  - 自动驾驶
  - 行为建模
...
2. **多模态场景编码器**:
    - 做什么：将异构输入（智能体轨迹 + 地图多段线）统一编码为场景嵌入
...
3. **大规模数据集构建与缩放实验设计**:
    - 做什么：从百万英里真实驾驶数据中筛选 1.2 亿高质量片段，覆盖美国/日本/阿联酋多城市
