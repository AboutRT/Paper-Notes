...
1. **任务类型单一**：大多聚焦于简单的位置查询（如 POI 检索），缺乏对复杂空间关系、导航规划等任务的覆盖
2. **缺少多模态评估**：未同时考察文本、视觉和 API 交互三种模态下的推理能力
3. **地理覆盖不足**：现有数据集往往集中于少数城市/国家，缺乏全球多样性
4. **缺少工具交互评估**：真实的地图使用场景涉及 API 调用（如 Google Maps API），但现有基准未涉及此类评估
