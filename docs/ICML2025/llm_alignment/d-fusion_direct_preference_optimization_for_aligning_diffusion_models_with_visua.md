...
**代码**: https://github.com/hu-zijing/D-Fusion (有)  
**领域**: LLM对齐/RLHF  
...
## 研究背景与动机
1. **领域现状**：扩散模型在文本到图像生成中取得了显著成功，但生成图像与文本 prompt 之间的对齐问题（misalignment）仍然严重，限制了实际应用。
2. **现有痛点**：近期研究将 DPO 引入扩散模型来增强对齐，但效果有限。核心原因在于 DPO 训练数据中存在**视觉不一致**问题：从不同噪声去噪得到的 high-preference 和 low-preference 图像在结构、风格、外观上差异巨大，模型难以分辨哪些因素与对齐正相关。
3. **核心矛盾**：在语言模型的 RLHF 中，可以对 token 级别做精细编辑得到一致的训练对；但在扩散模型中，手动编辑操作在像素级别进行，会**丢失逐时间步的去噪轨迹**，导致编辑后的图像无法用于 RL 训练。
4. **本文要解决的问题**：如何生成既**视觉一致**又**保留去噪轨迹**的 RL 可训练图像对？
5. **切入角度**：利用扩散模型 U-Net 中的自注意力机制，在去噪过程中逐步进行注意力融合，既保证生成图像与原始低偏好图的视觉一致性，又自然地保留了完整的去噪轨迹。
6. **核心 idea**：通过 cross-attention mask 定位对齐相关区域，在自注意力层将高偏好样本的对齐信息融合到低偏好样本中，生成可直接用于 DPO 训练的视觉一致样本。
...
1. **Cross-Attention Mask 提取**:
   - 做什么：从 reference image（高偏好图像）的去噪过程中自动提取与对齐相关的区域 mask
...
2. **Self-Attention Fusion（自注意力融合）**:
   - 做什么：在去噪过程的每个时间步，将 reference image 的自注意力特征在 mask 区域内融合到 base image 中
...
3. **去噪轨迹保留与 RL 训练**:
   - 做什么：收集融合过程中每个时间步的中间噪声状态，组合形成 target image 的完整去噪轨迹
