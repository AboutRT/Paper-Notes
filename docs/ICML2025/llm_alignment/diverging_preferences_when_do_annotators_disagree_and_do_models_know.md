...
## 研究背景与动机
1. **领域现状**：RLHF 已成为对齐 LLM 的标准方法，但在偏好数据收集中，标注者之间的分歧是普遍现象——MultiPref 中 39%、HelpSteer2 中 24% 的样本存在标注者分歧。
2. **现有痛点**：当前的奖励建模流程（如 Bradley-Terry）将标注者分歧视为简单噪声，通过多数投票聚合标签。这种做法忽视了分歧往往反映了不同用户视角的合理差异。
3. **核心矛盾**：标准奖励模型对分歧偏好和高一致偏好预测出相似的奖励差距，无法区分两者，导致 RLHF 训练的 LLM 只学会迎合一种用户视角，违背多元化对齐的目标。
4. **本文要解决什么**：（1）理解标注者分歧的根本原因；（2）设计能识别分歧偏好的奖励模型；（3）发现并缓解 LLM-as-Judge 评估中的偏见。
5. **切入角度**：从数据分析入手，释放 MultiPref 和 HelpSteer2 的个体标注信息，建立分歧原因的分类法，然后基于此分析推动奖励建模和评估方法的改进。
6. **核心 idea**：将奖励建模为分布（而非单一标量），既能预测偏好方向，也能捕捉标注者之间的分歧程度。
...
2. **Mean-Var 分布式奖励模型 (KL)**:
   - 做什么：将每个响应的奖励建模为正态分布 $r_A \sim \mathcal{N}(\mu_A, \sigma_A^2)$，同时预测均值和方差
...
3. **LLM-as-Judge 偏见分析与缓解**:
   - 做什么：分析 LLM-as-Judge 在分歧偏好上的行为，提出过滤方法
