...
1. **MoE 路由**：将查询分发给不同专家，但不产生新模型，面对超出现有专家能力的任务束手无策
2. **Learn-to-Fuse**：设计可训练组件将专家"粘合"，但需要大量监督数据训练，且模块化程度低，难以灵活增删专家
3. **模型算术**：通过权重或概率的算术运算组合专家（如 Task Arithmetic、TIES-Merging），但依赖对专家和组合方式的强假设（如"室内狮子 = 室外狮子 + (室内狗 - 室外狗)"）
