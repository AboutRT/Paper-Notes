...
## 研究背景与动机
1. **领域现状**：医学大视觉语言模型（Med-LVLM）在疾病诊断和治疗规划中应用广泛，但仍面临严重的事实性问题——模型倾向于优先使用训练中学到的文本知识而非实际视觉输入，导致幻觉。
2. **现有痛点**：已有偏好优化工作直接套用通用 LVLM 的偏好数据构建流程，忽略了医学领域的**临床相关性**。结果是偏好数据中的 dispreferred 响应缺乏临床意义，模型很容易区分，训练效果打折扣。例如 "右肺叶的胆结石" 这种明显的事实错误，不需要对齐训练模型也能识别。
3. **核心矛盾**：偏好优化的效果取决于偏好数据的质量——dispreferred 响应需要足够"可信"才能提供有效的学习信号；同时，视觉模态的病灶区域理解对医学准确性至关重要，但现有方法很少引导模型关注局部病灶。
4. **本文要解决什么**：设计一种考虑临床相关性的多模态偏好优化方法，使 Med-LVLM 的对齐训练更加有效。
5. **切入角度**：从偏好数据构建（两种 dispreference 策略）和偏好样本重要性量化（临床相关性评分）两个维度同时改进。
6. **核心 idea**：不是所有偏好样本都同等重要——临床相关性高的样本（dispreferred 响应更"可信"、病灶检测更准确的样本）应获得更大的训练权重。
...
1. **幻觉注入生成 Dispreferred 响应（策略一，$\mathcal{D}_t$）**:
   - 做什么：利用目标 Med-LVLM 和 GPT-4o 生成包含**可信医学错误**的幻觉响应
   - 核心思路：
...
2. **局部病灶加噪构建 Dispreferred（策略二，$\mathcal{D}_v$）**:
   - 做什么：用医学视觉工具（如 MedKLIP）定位病灶区域，仅在病灶区加噪生成 dispreferred 样本
...
3. **多 Med-LLM 协作临床相关性评分**:
   - 做什么：通过多个医学 LLM（如 Med42-7B、Med42-70B、BioMistral-7B）的辩论共识评估 dispreferred 响应的临床相关性
   - 流程：
