...
**代码**: 无  
**领域**: 强化学习 / 组合优化  
**关键词**: Preference Optimization, 组合优化, REINFORCE, 熵正则化, 局部搜索
...
1. **领域现状**: 强化学习（RL）已成为神经组合优化（RL4CO）的主流范式，REINFORCE及其变体被广泛用于训练端到端神经求解器（AM、POMO、Sym-NCO等），无需专家标注即可从环境交互中学习启发式策略。
2. **现有痛点**: 
...
3. **核心矛盾**: REINFORCE依赖数值奖励差驱动学习，但COP中后期该差值极小；同时轨迹空间太大，无法有效正则化探索
4. **本文要解决什么**: 设计一种不依赖奖励绝对数值、自带探索能力、能无缝集成局部搜索的通用RL优化框架
5. **切入角度**: 借鉴RLHF中DPO的思想——将定量奖励转化为定性偏好比较，通过重参数化避免不可行计算
6. **核心idea一句话**: 用偏好比较代替绝对奖励值，通过熵正则化目标的解析解重参数化奖励函数，使策略优化转化为分类问题
...
1. **偏好信号生成（Preference Comparison）**: 
   - **做什么**: 把定量奖励r(x,τ)转化为定性偏好标签y=𝟙(r(x,τ₁)>r(x,τ₂))
...
2. **重参数化熵正则化目标（Reparameterized Entropy-Regularized Objective）**:
   - **做什么**: 将最大熵RL目标的解析最优策略代入偏好模型，消除不可行的分区函数Z(x)
   - **核心思路**: 
...
3. **偏好模型选择与梯度分析**:
   - **做什么**: 支持多种偏好模型（Bradley-Terry、Thurstone、Plackett-Luce），分析其隐式优势信号特性
...
4. **局部搜索微调（Fine-Tuning with Local Search）**:
   - **做什么**: 在策略收敛后，将局部搜索（如2-Opt、swap*）集成到训练中而非推理时
