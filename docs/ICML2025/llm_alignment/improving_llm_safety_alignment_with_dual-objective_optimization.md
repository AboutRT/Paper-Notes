...
1. **学习率不平衡**：DPO的有效学习率 $r_\theta^\beta(y^h|x)/[r_\theta^\beta(y^s|x)+r_\theta^\beta(y^h|x)]$ 会随着安全与有害响应的差距增大而指数级衰减（$\lesssim e^{-\beta C}$），导致安全token的logit增长过早饱和。模型"够好就停"，无法把拒绝概率推到足够高。

2. **OOD泛化缺陷**：DPO没有显式惩罚OOD响应，梯度项可能与OOD数据正相关，无意中增加OOD响应的logit，反而降低安全响应的概率。这对于文本接口带来的几乎无限攻击面是致命的。
