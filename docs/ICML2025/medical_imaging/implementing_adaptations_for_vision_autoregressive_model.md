...
1. **适配方法缺失**：扩散模型已有丰富的微调技术（DiffFit、DreamBooth、Textual Inversion），但VAR的适配方法几乎未被探索。
2. **差分隐私适配空白**：当微调数据为敏感数据（如医学影像）时，需要隐私保护。DP适配已在DM上广泛研究，VAR尚无此类方案。
3. **实现层面的挑战**：VAR原始代码库的注意力算子和前向函数需要打补丁才能引入LoRA和DP-SGD。
