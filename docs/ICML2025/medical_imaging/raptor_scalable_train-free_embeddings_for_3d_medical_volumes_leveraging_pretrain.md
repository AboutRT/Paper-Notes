...
description: >-
  [ICML2025][医学图像][3D医学体积] 提出 Raptor（Random Planar Tensor Reduction），一种完全免训练的方法，利用冻结的 2D 基础模型（DINOv2-L）对 3D 医学体积沿三轴提取视觉 token，再通过随机投影大幅压缩维度…
tags:
  - "ICML2025"
  - "医学图像"
  - "3D医学体积"
...
**计算复杂度**：将卷积或 Transformer 架构从 2D 扩展到 3D 时，计算开销呈立方或更高阶增长，训练成本极高（如 VoCo 使用 8×H100，SuPreM 使用 8×A100 训练 7 天以上）

**数据稀缺**：最大的 3D 医学数据集仅约 160K 体积，比 2D 图像数据集（1.2B 图像）小几个数量级
