...
1. **检测类方法**（Mask R-CNN, DoNet）：依赖目标检测框架，对细长细胞和重叠区域容易漏检，且计算复杂度高
2. **轮廓预测方法**（UNet, DCAN）：引入边界类别实现实例区分，但性能高度依赖轮廓阈值设置
3. **距离映射方法**（StarDist, HoverNet, CellViT）：使用多解码分支 + 复杂后处理，显著增加计算开销
