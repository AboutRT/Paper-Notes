...
1. **领域现状**：计算病理学中的全切片图像（WSI）包含 $10000^2 \sim 100000^2$ 像素，需要从数千到数万个 patch 中识别分散的诊断特征——"大海捞针"。多实例学习（MIL）成为主流弱监督框架。
2. **现有痛点**：
...
3. **核心矛盾**：一个关键观察——WSI 中 patch 间的相关性高度依赖上下文。肿瘤边界区域与远处的类似浸润模式高度相关，但与相邻的正常组织不相关。固定的注意力模式无法捕捉这种上下文依赖的异质性关系。
4. **本文要解决什么**：在保持完整注意力建模能力的同时实现计算效率。
5. **切入角度**：每个 query patch 动态决定"哪些远处区域与我相关"——通过高效的区域级元数据估计重要性，只对高重要性区域做完整注意力。
6. **核心 idea**：区域级元数据（min/max 特征压缩）→ 重要性评分 → 选择 top-K 区域 → 稀疏但精准的注意力。
...
1. **区域级 Min-Max 元数据**:
   - 做什么：将每个区域的 K 个 patch 特征压缩为两个向量（min 和 max）
   - 核心思路：
...
2. **Query-Aware 重要性评分**:
   - 做什么：为每个 query patch 动态评估所有区域的相关性
