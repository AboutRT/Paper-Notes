...
1. **隐私训练** (如 DP-SGD)：在训练过程中加噪，使最终模型参数满足 DP。缺点是隐私参数须预先固定，训练代价高，且可能引入对特定群体的偏差。
2. **隐私预测** (Private Prediction)：对非隐私模型的输出加噪。优势在于可动态调整隐私预算、易于与联邦学习等复杂训练范式结合。但现有方法依赖全局敏感度 $\text{GS}(f)$，导致隐私-效用权衡远逊于隐私训练。
