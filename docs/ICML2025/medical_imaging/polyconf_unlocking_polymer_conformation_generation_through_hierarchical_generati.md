...
1. **小分子方法难以扩展**：GeoDiff、TorsionalDiff 等方法设计用于小分子（原子数较少），直接应用于含数千原子的聚合物时性能显著下降
2. **蛋白质方法不适用**：蛋白质有统一的骨架框架（N-Cα-C-O 结构）和强方向性分子内相互作用，聚合物缺乏这些先验约束，具有更高的柔性和更少的有序性
3. **数据稀缺**：传统分子动力学（MD）模拟计算成本极高，导致聚合物构象数据集严重匮乏
4. **重复单元构象多样**：虽然同一聚合物的重复单元共享相同的 SMILES 字符串，但各单元在 3D 空间中的排列差异显著，不能简单地将聚合物建模为单一预定义重复单元构象的刚性组装
