...
1. **局部反事实解释** (如 CF-GNNExplainer、CFF、RCExplainer)：为每个输入图生成一个独立的反事实，解释数量多且难以在模型层面归纳理解。
2. **全局反事实解释** (如 GCFExplainer)：生成一小组反事实图覆盖所有输入图，但同一个反事实对不同输入图意味着截然不同的变换 (recourse)，不适用于提供可操作的指导。
3. **GLOBE-CE**：通过学习翻译方向生成全局解释，但尚未应用于图数据。
