...
## 研究背景与动机
1. **领域现状**：主流范式是"扩大数据和模型规模即可提升性能"。但组合理解——通过组合已知简单概念理解新场景——是人类智能基石，现有视觉模型表现不佳。  
2. **现有痛点**：即使 LAION-400M 这样的大规模数据集，概念组合覆盖上也存在严重稀疏性。组合爆炸导致大多数可能组合在训练集中缺失。  
3. **核心矛盾**：Scaling laws 预测扩大数据能持续提升性能，但概念组合空间指数增长，堆量可能无法解决组合稀疏。  
4. **本文要解决什么**：精确回答"视觉模型能否组合泛化？在什么条件下？"  
5. **切入角度**：设计 (n,k) 框架参数化控制概念空间复杂性和训练数据多样性。  
6. **核心idea一句话**：组合泛化不是靠"堆量"解决的，而是靠增加概念组合多样性迫使模型发现线性分解表示结构。
