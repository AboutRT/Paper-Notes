...
1. **压缩度（Compression）**：如 fertility（每词 token 数）和 CTC（语料 token 总数），但已有研究表明压缩度与性能之间无稳健关联。
2. **Rényi 效率**：考虑了频率分布，但后续工作认为其不能全面衡量分词质量。
3. **形态对齐（Morphological Alignment）**：衡量 token 边界是否与语素边界一致。例如，英文单词 "books" 的理想分切为 [book + s]，而非 [boo + ks]。
