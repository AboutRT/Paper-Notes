---
description: >-
  提出Whitened CLIP (W-CLIP)，通过可逆线性白化变换将CLIP嵌入空间转化为各向同性标准正态分布，首次实现图像和文本的直接似然函数计算，应用于伪影检测、分布偏移分析和全圆SLERP插值。
tags:
  - ICML2025
  - CLIP
  - 白化变换
...
1. **PCA 白化变换**:
    - 做什么：将 CLIP 嵌入从各向异性椭球分布转化为各向同性超球分布
...
2. **范数-似然映射与正态性验证**:
    - 做什么：统计验证白化嵌入近似标准正态分布，建立范数与似然的精确对应
...
3. **全圆球面线性插值 (Full-Circle SLERP)**:
    - 做什么：将标准 SLERP 从 $t \in [0,1]$ 扩展到全 $360°$，实现图像间的插值与外推
