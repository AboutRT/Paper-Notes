...
**本文要解决什么？** (1) LoRA 能否有效适配光谱基础模型到全新巡天？(2) 微调模型的不同模块（基础模型、投影头、下游 MLP）对性能的影响如何？(3) 预训练中嵌入的 Gaia XP 跨模态知识能否帮助 DESI 适配？

**切入角度**：光谱数据具有类似语言的结构化特性（局部特征对应物理信息），LoRA 在 NLP/CV 中已证明有效，作者首次将其引入天文光谱领域。SpecCLIP 的多模态对比预训练提供了丰富的跨巡天知识基准。

**核心idea一句话**：用 LoRA 以极少参数（不到模型总参数的 3%）和极少样本（约 100 个标注）将光谱基础模型高效迁移到新巡天。
...
1. **SpecCLIP 预训练基础（冻结骨干）**:
    ...
    - 做什么：为 LAMOST LRS 和 Gaia XP 两种模态各建立基础模型，并通过对比学习对齐
...
2. **四模块 LoRA 微调策略**:
    ...
    - 做什么：对模型的四个不同模块分别或联合插入 LoRA 进行微调
...
3. **跨巡天数据适配流程**:
    ...
    - 做什么：将 DESI 光谱标准化后接入 SpecCLIP 流程
