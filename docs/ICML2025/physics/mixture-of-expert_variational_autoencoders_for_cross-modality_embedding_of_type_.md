...
### 核心挑战
1. **模态异质性**：光变曲线（测光数据）和光谱是高度异质的两种数据模态，序列长度不固定、采样不规则
2. **多对多映射**：一条光变曲线可对应多个不同时间点的光谱，同一光谱也可能与多条光变曲线兼容
3. **推理时缺失模态**：绝大多数超新星仅有测光数据，模型需在仅有单一模态时进行推理
4. **现有方法局限**：对比学习（Maven）仅学习对齐表征但不支持生成；扩散模型（Shen & Gagliano 2025）仅做条件生成但不提供通用表征
