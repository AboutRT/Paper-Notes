---
description: "【论文笔记】Cascade: Token-Sharded Private LLM Inference 论文解读 | ICML 2025 | arXiv 2507.05228 | 隐私推理 | 提出 Cascade——一种基于 token 维度分片的多方推理协议，通过将隐藏状态按 token 维度分发给不同计算节点，避免密码学原语的高昂开销，在保持抵抗 vocab-matching 攻击能力的同时实现比 SMPC 方案快 100× 的推理速度。"
tags:
  - ICML 2025
  - 隐私推理
  - Token分片
...
1. **领域现状**：LLM 参数量增长使得第三方推理服务日益流行，但用户数据隐私面临风险。安全多方计算（SMPC）提供可证安全性，但在非线性操作上有极大的计算和通信开销。
2. **现有痛点**：
...
3. **核心矛盾**：密码学方案安全但不可扩展到大型 LLM；置换方案快但不安全。
4. **本文要解决什么**：如何在安全性和效率之间找到新的平衡点？
5. **切入角度**：不在 token 置换层面做文章，而是在 token 维度做分片——每个计算节点只看到部分 token 的隐藏状态，且相邻 token 之间保持足够大的间隔。
6. **核心 idea**：vocab-matching 攻击的计算复杂度随隐藏状态之间的 token 间隔呈指数增长（$V^g$，其中 $g$ 是间隔），只要间隔足够大（≥3），攻击就变得不可行。
