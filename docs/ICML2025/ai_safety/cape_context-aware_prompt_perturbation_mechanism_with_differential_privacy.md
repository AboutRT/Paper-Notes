...
1. **领域现状**：LLM 推理服务（如 ChatGPT）要求用户将明文 prompt 发送到服务器，prompt 中可能包含敏感信息（邮件、商业机密等），存在隐私泄露风险。
2. **现有痛点**：
...
3. **核心矛盾**：NLP 词表极大（30K+），标准指数机制在如此大的采样空间中会出现长尾现象——大量低效用 token 的累积概率很大，导致频繁采样到无关 token。
4. **本文要解决什么**：如何在黑盒推理场景下，用高效的 DP 机制扰动 prompt，同时保持语义一致性和隐私？
5. **切入角度**：(a) 引入上下文 logit 信息增强效用函数；(b) 设计分桶采样抑制长尾效应
6. **核心 idea**：将上下文相关性（由客户端小模型提供的 logit）和 token 嵌入距离融合为混合效用函数，配合等宽分桶指数机制，实现上下文感知的 prompt 扰动。
