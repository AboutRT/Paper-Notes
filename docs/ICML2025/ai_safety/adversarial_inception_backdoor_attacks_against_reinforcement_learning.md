...
1. **领域现状**：DRL 在安全关键领域（自动驾驶、网络防御、机器人）的广泛应用使其成为对抗攻击的目标。后门攻击在训练阶段操纵智能体，使其在部署时遇到特定触发器时执行预定义的对抗行为。
2. **现有痛点**：现有后门攻击（TrojDRL、SleeperNets）假设攻击者可以任意控制奖励信号——注入远超环境自然范围的极端奖励值。这导致：(a) 奖励裁剪/归一化即可使攻击失效；(b) 异常大的奖励值易被简单的规则检测器发现。
3. **核心矛盾**：当奖励被约束在环境自然范围 $[\inf[R], \sup[R]]$ 内时，攻击者无法仅通过奖励操纵使对抗动作的期望回报超过最优动作——特别是当最优动作的累积回报很高时（如接近 $\gamma/(1-\gamma)$），需要无界奖励才能抵消。
4. **本文要解决什么**：如何在严格奖励约束下实现有效的 RL 后门攻击？
5. **切入角度**：不操纵奖励值本身，而是操纵训练数据中的动作——将高回报时刻的最优动作替换为目标对抗动作，让智能体"曾经以为"对抗动作带来了高回报。
6. **核心 idea**：在训练轨迹中"植入记忆"（inception）——让智能体相信目标动作是高回报的，因为它在历史数据中看到了目标动作与高回报的关联。
