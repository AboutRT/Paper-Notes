...
1. **领域现状**：多模态大语言模型（MLLM）如 LLaVA、InternVL 等在 VQA 等视觉语言任务中表现出色，但其训练数据不可避免地包含隐私敏感信息。现有隐私泄露研究（模型提取攻击、成员推理攻击 MIA）主要关注与训练目标自然对齐的敏感内容。
2. **现有痛点**：先前工作聚焦于"任务相关"的隐私内容——文本模态中的个人信息通过 next-token prediction 自然被记忆，视觉模态中的私人图像特征也与主任务紧密关联。但一个被忽视的问题是：与训练目标完全无关的隐私内容是否也会被模型记忆？
3. **核心矛盾**：从全局训练目标来看，任务无关内容不应影响模型学习；但在 partial mini-batch 训练动态中，这些内容可能与 VQA 的输出产生虚假相关性，导致模型通过正常的梯度更新将其编码到参数中——这构成了一种全新的、难以预见的隐私风险。
4. **本文要解决什么**：回答三个核心问题——(RQ1) 随机任务无关内容是否影响微调动态？(RQ2) 模型是否在内部表示中编码了这些无关内容？(RQ3) 这种编码在不同层中如何分布？
5. **切入角度**：以可控的方式注入完全随机的水印（task-irrelevant watermark）作为模拟的"私密内容"，系统性地研究其被记忆的机制和可检测性。
6. **核心 idea**：设计水印注入实验 + 层级线性探测框架，证明即使内容与任务完全无关，MLLM 也会在其隐状态中编码这些信息。
...
1. **水印注入策略 (Watermark Injection)**:
   - 做什么：在 VQA 图像上叠加随机文本水印，模拟任务无关的私密内容
...
2. **Mini-batch 虚假相关性分析**:
   - 做什么：分析为什么全局无关的内容在 mini-batch 尺度上变得"相关"
...
3. **层级线性探针 (Layer-wise Linear Probing)**:
   - 做什么：对训练完成的 MLLM 的每一层隐状态训练线性分类器，探测水印是否可从内部表示中恢复
