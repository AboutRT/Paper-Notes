...
1. **表示崩塌 (Representation Collapse)**: 所有输入的表示坍缩到同一常向量
2. **维度崩塌 (Dimensional Collapse)**: 嵌入仅占据向量空间的低维子空间
3. **聚类崩塌 (Cluster Collapse)**: 数据点仅被分配到可用原型的子集
4. **簇内崩塌 (Intracluster Collapse)**: 同一聚类内样本的表示差异趋近于零
