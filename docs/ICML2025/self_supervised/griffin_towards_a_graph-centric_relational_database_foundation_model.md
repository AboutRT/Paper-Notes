...
1. **结构复杂性**：多表间关系交错，简单的单表展平（flatten）会丢失大量关系信息
2. **特征空间异构**：不同 RDB 的分类特征、数值特征、文本特征语义各不相同，难以统一
3. **任务多样性**：分类和回归任务共存，不同任务的标签空间和数值范围差异巨大
4. **大规模时序约束**：实际 RDB 常含时间戳，预测须满足因果约束（仅用历史数据）
