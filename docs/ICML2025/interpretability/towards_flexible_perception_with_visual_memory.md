...
1. **数据更新困难**：新数据可用或旧数据需撤回（隐私、公平性）时，必须重新训练或微调
2. **机器遗忘（unlearning）几乎不可能**：要从模型中移除某些训练样本的影响极其困难
3. **决策不可解释**：无法追溯一个预测是由哪些训练数据驱动的
4. **概念漂移（concept drift）**：现实世界不断变化，静态模型迅速过时
