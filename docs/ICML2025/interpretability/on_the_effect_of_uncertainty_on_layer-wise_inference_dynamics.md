...
## 研究背景与动机
1. **领域现状**: 理解 LLM 内部表示和处理预测是检测不确定性和防止幻觉的核心问题。
2. **现有痛点**: 虽知模型在隐藏状态中编码不确定性，但不确定性如何影响处理过程未被探索。
3. **核心矛盾**: 如果确定和不确定预测的层间动态相似，那么简单的中间层检测方法可能无效。
4. **本文解决什么**: 系统研究不确定性对层间推理动态的影响。
5. **切入角度**: 使用 Tuned Lens 跟踪各层概率轨迹。
6. **核心 idea**: 确定和不确定预测经历类似的"信心突变"模式，不确定性不影响推理动态结构。
