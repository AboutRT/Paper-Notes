...
## 研究背景与动机
1. **领域现状**: 集成方法（boosting, bagging, random forest等）是最成功的ML方法之一，但通常被视为"黑盒"。
2. **现有痛点**: 缺乏关于集成可解释性的系统理论。什么因素使集成不可解释？基模型可解释是否意味着集成也可解释？
3. **核心矛盾**: 集成通过组合简单模型提高性能，但组合过程本身引入了复杂性。
4. **本文要解决什么**: (1) 量化集成可解释性；(2) 识别影响因素；(3) 提出可解释集成方法。
5. **切入角度**: 定义集成可解释性度量，基于基模型的多样性、一致性和冗余性。
6. **核心idea**: 集成的不可解释性主要来自基模型间的不一致特征使用——当不同基模型依赖不同特征且缺乏结构化模式时，集成变得不可解释。
