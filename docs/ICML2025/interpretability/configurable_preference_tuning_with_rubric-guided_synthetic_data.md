...
### 关键设计
1. **Rubric定义与分数条件生成**:
   - 做什么：定义一组rubric $\{\mathcal{R}_i\}$，每个rubric详细描述响应的某个属性维度（如"非传统性"、"华丽巴洛克风格"、"神秘象征主义"等），并按5个等级给出详细评分标准
...
2. **System Prompt合成**:
   - 做什么：将每个rubric+分数组合总结为2-3句话的简洁system prompt
...
3. **对称偏好对构造**:
   - 做什么：从同一对响应 $(y_1, y_2)$ 构造两个互补的DPO训练样本
