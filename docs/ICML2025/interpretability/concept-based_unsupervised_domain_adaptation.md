...
1. **领域现状**：概念瓶颈模型（CBM）通过人类可理解的概念作为中间表示来增强可解释性（如先预测"黑眼睛"+"实心肚子"再分类为"暗色信天翁"）。但 CBM 假设训练和测试数据同分布。
2. **现有痛点**：
...
3. **核心矛盾**：CBM 需要概念一致性来保证可解释性，但域偏移下概念分布本身就不同——过度对齐会损害概念的准确性。
4. **本文要解决什么**：在域偏移下实现可解释的分类。
5. **切入角度**：松弛的概念对齐——允许概念分布在域间有小差异（如"棕色为主色"在源域占 19%、目标域占 17% 是合理的），而非强制完全一致。
6. **核心 idea**：松弛一致性对齐 + 目标域概念推断（利用对齐后的嵌入空间推断目标域概念） + CBM×DA 的泛化误差理论界。
