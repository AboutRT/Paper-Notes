...
1. **离散标签不可用**：从有限支持到无限支持的转变使得图无法按类别分组，CAL 的后门调整和 DisC 的反事实推理都依赖离散标签信息。
2. **混淆子图假设过强**：现有方法（如 CAL、DisC）假设混淆子图 $S$ **完全不含预测信息**，但在实际场景中（如分子属性预测中，分子量与毒性虽非因果关系但有强相关），这一假设明显不合理。
