...
1. **非欧嵌入的兴起**：大量研究表明，树状层次数据适合嵌入到双曲空间（负曲率），环状/周期数据适合嵌入到球面空间（正曲率）。现实数据往往包含多种拓扑结构，因此乘积流形 $\mathcal{P} = \mathcal{M}_1 \times \mathcal{M}_2 \times \cdots \times \mathcal{M}_k$（各分量曲率不同）成为更合适的嵌入空间。

2. **传统方法的局限**：经典 DT/RF 仅在欧几里得空间工作，直接应用到非欧空间会破坏几何结构。先前工作（HyperDT）仅处理纯双曲空间，无法应对混合曲率。

3. **乘积流形上 ML 工具匮乏**：虽然已有在乘积流形上的感知机、SVM、GCN 等，但缺少简单、可解释、无需梯度优化的非参数方法。DT/RF 因其可解释性和鲁棒性，是理想的候选工具。

4. **核心挑战**：如何在每个分量流形上定义"分裂超平面"，使其既尊重流形几何，又保留 DT 的递归贪心分裂框架？
