...
1. **领域现状**：LLM 在机器人任务规划中展现潜力，但受限于单向依赖（无法回顾已生成的 token）、缺乏错误纠正、固定计算量无法动态分配。
2. **现有痛点**：自精炼（self-refinement）策略可解决上述问题（引入双向依赖+动态纠错），但训练困难——需要通过无限自精炼步骤做反向传播，或构建复杂的 RL/验证器管道。
3. **核心矛盾**：自精炼的训练如何简单高效地实现？
4. **本文要解决什么**：用简单的监督学习训练自精炼的 LLM 规划器。
5. **切入角度**：将自精炼视为不动点迭代 $x_{t+1} = f_\theta(x_t, c)$，理想计划是均衡点 $x^* = f_\theta(x^*, c)$。
6. **核心 idea**：用深度均衡模型的隐式微分绕过无限步反向传播，实现端到端监督训练。
