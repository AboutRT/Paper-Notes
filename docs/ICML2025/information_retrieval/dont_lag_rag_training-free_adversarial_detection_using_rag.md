...
1. **领域现状**: 深度学习模型（CNN、ViT）在计算机视觉任务中表现卓越，但对**对抗补丁攻击**（adversarial patch attacks）高度脆弱。对抗补丁是一种局部化的高幅度扰动，可以被打印并放置在真实场景中，在不同光照和视角下仍可导致模型误分类。
...
3. **核心矛盾**: 传统防御要么需要训练（不够灵活），要么是启发式的（不够准确）。如何在**完全不训练**的情况下检测多种类型的对抗补丁？

4. **本文要解决什么**: 构建一个**免训练、可扩展、基于检索增强的对抗补丁检测框架**，能动态适配不断演化的攻击。

5. **切入角度**: 将对抗补丁检测看作**视觉检索 + VLM 推理**问题——用数据库存储已知攻击模式，用检索找到最相似的攻击，再用 VLM 进行分类判断。

6. **核心idea一句话**: 用 RAG 范式连接对抗补丁数据库和 VLM，实现免训练的上下文感知对抗检测。
