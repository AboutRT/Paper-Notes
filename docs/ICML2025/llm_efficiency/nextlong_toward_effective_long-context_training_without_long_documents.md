---
description: "【论文笔记】NExtLong: Toward Effective Long-Context Training without Long Documents 论文解读 | ICML 2025 | arXiv 2501.12766 | 长上下文训练 | 本文提出 NExtLong 框架，通过将文档分割为 meta-chunk 并在 chunk 之间插入从预训练语料检索的硬负例干扰文本来合成长上下文训练数据，迫使模型区分长距离依赖信息和干扰内容，在 HELMET 和 RULER 基准上比此前最佳的长上下文合成方法 Quest 平均提升 7.33%。"
tags:
  - ICML 2025
  - 长上下文训练
  - 数据合成
...
## 研究背景与动机
1. **领域现状**：LLM 的上下文长度快速增长（从 Llama2 的 4K 到 Llama3.1 的 128K），长上下文能力是解锁文档摘要、长篇QA、代码规划等任务的关键。
2. **现有痛点**：主流方法需要大量高质量长文档进行继续训练，但高质量长文档在大多数领域极为稀缺，随着目标上下文长度增加问题更加严重。
3. **核心矛盾**：现有合成方法（KNN 相似文档拼接、随机拼接、Quest 关键词检索拼接）缺乏显式的长距离依赖建模机制——拼接得到的文档虽然长，但各部分之间的依赖关系是弱的或随机的。
4. **本文要解决什么**：如何在没有天然长文档的情况下，合成能有效训练长距离依赖建模能力的长上下文数据。
5. **切入角度**：受对比学习中硬负例技术的启发——在依赖片段之间插入语义相似但不相关的干扰文本，既延长了依赖距离，又迫使模型学会在噪声中识别真正相关的上下文。
6. **核心 idea**：将短文档拆分为 meta-chunk，在相邻 meta-chunk 之间插入检索得到的硬负例，创建 "Negative Extended" 长文档，强制模型在大量干扰中捕捉跨 chunk 的长距离依赖。
...
1. **文档分块 (Document Chunking)**:
   - 做什么：将 meta-document 按最大长度 s 分割为多个 meta-chunk
...
2. **硬负例挖掘 (Hard Negative Mining)**:
   - 做什么：从预训练语料中为每个 meta-chunk 检索语义相似但内容不同的 chunk 作为干扰
   - 核心思路：
...
3. **长文档合成与依赖建模**:
   - 做什么：拼接所有 extended chunk 形成长文档 t = [l1, l2, ..., lp] 用于训练
