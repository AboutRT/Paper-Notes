...
description: >-
  [ICML 2025][LLM效率][Mixture-of-Experts] 本文将多头注意力（MHA）重新表述为求和形式，借鉴 MoE 思想提出 Mixture-of-Head Attention（MoH），通过路由器为每个 token 动态选择最相关的注意力头子集…
tags:
  - "ICML 2025"
  - "LLM效率"
  - "Mixture-of-Experts"
...
**代码**: [https://github.com/SkyworkAI/MoH](https://github.com/SkyworkAI/MoH)  
**领域**: LLM效率 / 注意力机制  
...
**本文目标** (a) 如何让每个 token 动态选择需要的注意力头？(b) 如何在减少激活头数的同时保持甚至提升性能？(c) 如何将已有预训练模型高效转换为稀疏头模型？

**切入角度**：作者注意到 MHA 可以从标准的拼接形式改写为**求和形式**，即输出等于各头输出的简单相加。既然是相加，就可以像 MoE 一样为各项加权、稀疏激活，自然引出"头即专家"的类比。

**核心 idea**：把注意力头视为 MoE 中的专家，用 router 为每个 token 选择 Top-K 头，并通过 shared head + 两阶段路由增强路由稳定性。
