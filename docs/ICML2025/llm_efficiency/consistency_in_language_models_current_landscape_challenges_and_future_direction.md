---
description: >-
  系统综述LLM一致性研究现状，提出行为一致性的分类体系（逻辑/语义/事实/道德），分析现有评测方法不足，呼吁建立标准化基准和跨学科方法以提升一致性同时保留实用性。
tags:
  - ICML2025
  - LLM一致性
  - 综述
...
1. **行为一致性的二级分类体系**:
    - 做什么：将散乱的一致性概念组织为结构化分类，区分逻辑/形式一致性与非逻辑/非形式一致性
    - 核心分类：
...
2. **评测方法的系统分析与批判**:
    - 做什么：从输入采样、输出采样、基础指标、聚合方式四个层面分析现有评测框架的优劣
    - 核心发现：
