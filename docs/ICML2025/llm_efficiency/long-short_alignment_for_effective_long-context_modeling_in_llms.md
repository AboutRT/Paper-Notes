...
## 研究背景与动机
1. **领域现状**：LLM 受 Transformer 固定上下文窗口限制，长上下文建模是核心挑战。扩大上下文窗口可以带来更多 in-context learning 样本和更长的推理链。
2. **现有痛点**：长上下文训练极其耗时耗内存，因此理解和提升长度泛化（从短序列训练泛化到长序列测试）至关重要。
3. **核心矛盾**：现有工作主要从输入端（位置编码设计）或模型内部机制（RASP 分析等）理解长度泛化，忽略了一个关键维度——模型的输出行为。
4. **本文要解决什么**：揭示模型输出分布在不同长度上的一致性（长短对齐）如何影响长度泛化，并提出改善方法。
5. **切入角度**：通过合成任务对比实验揭示现象——均值预测任务泛化好（输出空间固定在 [0,1]），长度预测任务泛化差（输出空间随长度变化），然后将洞察推广到自然语言任务。
6. **核心 idea**：模型在不同长度输入上产生一致的输出分布是长度泛化的关键，可以通过正则化项显式促进这种一致性。
...
1. **合成任务分析与 Output Reparameterization (OutRep)**:
   - 做什么：通过均值预测 vs 长度预测的对比，揭示输出空间的稳定性决定长度泛化能力
