...
1. **领域现状**：建模人类偏好是基础模型对齐的核心。Bradley-Terry（BT）模型用标量奖励表示偏好，是 RLHF 的标准选择。监督配对偏好模型（PairRM/PairPM）直接拼接两个响应预测偏好。
2. **现有痛点**：
...
3. **核心矛盾**：表达力（BT:低, PairPM:高）与效率（BT: $O(K)$, PairPM: $O(K^2)$）的权衡。
4. **本文要解决什么**：设计兼具高表达力和低查询复杂度的偏好模型。
5. **切入角度**：将每个响应嵌入到 $d$ 维潜空间（而非 1 维标量奖励），用嵌入向量间的关系表示偏好——$d$ 维嵌入比标量丰富得多，但查询仍为 $O(K)$（每个响应独立嵌入一次）。
6. **核心 idea**：偏好嵌入 = 高维奖励表示，偏好关系由嵌入间的非对称函数决定（如 $P(A>B) = \sigma(\phi(e_A, e_B))$），保持 $O(K)$ 效率但突破标量奖励的表达力瓶颈。
...
1. **偏好嵌入 (Preference Embedding)**:
   - 做什么：将每个 (prompt, response) 对映射到 $d$ 维向量
