...
## 研究背景与动机
1. **领域现状**：将预训练模型适配到新任务通常需要梯度下降微调，耗时且依赖标注数据。参数生成方法（HyperNetworks、G.pt）尝试直接生成权重但性能有限。
2. **现有痛点**：(a) 每个新任务都需要独立微调；(b) 低资源/隐私敏感场景无法获取充足标注数据；(c) 现有参数生成方法未充分探索面对未知任务的泛化能力。
3. **核心矛盾**：能否跳过梯度优化，直接用生成模型按需"采样"好的任务参数？
4. **本文要解决什么？** (RQ1) 能否为已知任务生成好参数？(RQ2) 能否在多任务间插值？(RQ3) 能否泛化到未知任务？
5. **切入角度**：将 LoRA adapter 参数视为高维分布，用隐扩散模型学习并采样。
6. **核心idea一句话**：用参数 VAE 编码 LoRA 权重到隐空间，再用条件扩散模型在隐空间中生成新参数。
...
1. **参数 VAE**:
   - 做什么：将多层 LoRA 参数展平拼接为 1D 向量，学习紧凑隐表示
