...
1. **LID（Local Intrinsic Dimensionality）方法**（Ross et al., 2024）：仅能在生成最终步（$t \approx 0$）检测，无法提前预警
2. **基于训练数据比对的方法**（Carlini et al., 2023）：需要访问训练集，计算开销大，不适合实时检测
3. **Wen et al. (2024) 的 score 差异指标**：虽然有效但缺乏理论解释
4. **Prompt 修改策略**：缓解记忆化的同时会损害生成质量
