...
1. **收敛慢**：标准 DDPM 采用标准高斯 $\mathcal{N}(\mathbf{0}, \mathbf{I})$ 作为先验分布，与真实数据分布差距大，导致训练需要大量迭代才能收敛
2. **推理步数多**：先验与数据分布的差距也意味着反向过程需要更多步骤才能还原信号
3. **现有改进有限**：PriorGrad 提出用基于规则的方法从条件输入中提取先验信息，但这种手工设计的先验需要领域知识，且并非所有任务都能设计出合适的规则
