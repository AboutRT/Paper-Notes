...
1. **高质量渲染难题**：生成与真实录音无法区分的双耳音频需要精确建模双耳线索（ILD/ITD）、房间混响和环境噪声。传统 DSP 方法（如 SoundSpaces）依赖简化的几何模拟和非个性化 HRTF，质量有限；回归式神经网络方法（如 WarpNet）无法生成输入信号中不存在的混响和噪声。
2. **流式推理难题**：实时应用要求连续、低延迟的流式生成，但现有生成模型（扩散模型等）使用非因果架构（非因果卷积+全局注意力）和多步迭代推理，无法支持流式处理。
