...
1. **领域现状**：3D 分子生成模型（扩散/流匹配）在药物设计中表现出色，但一次性生成整个分子，无法保证可合成性。GFlowNet 可以按合成步骤构建分子但限于 2D。
2. **现有痛点**：
   - 扩散/流模型一次生成所有原子——无法 mask 无效生成动作，无法保证合成可行性
   - GFlowNet 按步构建但仅处理离散 2D 图——不建模 3D 构象（蛋白质-配体相互作用依赖 3D）
   - 自回归模型按步生成 3D 但早期步骤的微小误差会级联放大
3. **核心矛盾**：需要同时建模组合结构（合成路径的离散序列）和连续状态（3D 原子坐标），但现有方法只能处理其中一个。
4. **本文要解决什么**：统一框架下联合生成合成路径和 3D 构象。
5. **切入角度**：将流匹配的插值过程扩展到组合状态转换——在组合步骤中逐步构建结构的同时，用流匹配传输对应的连续状态。
6. **核心 idea**：两个交织的流过程——(1) Compositional Flow 逐步拆解/构建组合结构; (2) State Flow 传输与各组件关联的连续状态。两者通过共享输入实现相互依赖。
...
1. **组合流匹配插值的扩展**:
   - 做什么：将标准流匹配的线性插值从"全量噪声→全量数据"扩展为"空结构→逐步构建完整结构"
