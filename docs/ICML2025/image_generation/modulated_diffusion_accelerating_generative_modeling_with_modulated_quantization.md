---
description: "【论文笔记】Modulated Diffusion: Accelerating Generative Modeling with Modulated Quantization 论文解读 | ICML 2025 | arXiv 2506.22463 | 扩散模型加速 | MoDiff 提出了调制量化(Modulated Quantization)与误差补偿相结合的框架来加速扩散模型，将激活量化从 8-bit 降至 3-bit 且无性能损失，同时继承缓存和量化方法的双重优势。"
tags:
  - ICML 2025
  - 扩散模型加速
  - 量化
...
1. **领域现状**：扩散模型在生成任务中表现卓越，但迭代采样带来巨大计算开销。加速技术主要包括：步数减少（蒸馏）、缓存（复用中间计算）和量化（低精度计算）。

2. **现有痛点**：缓存方法虽然避免冗余计算，但引入的近似误差会随时间步累积；量化方法在低比特（<8 bit）下生成质量显著下降；两者的组合更容易产生叠加误差。

3. **核心矛盾**：激进的加速（更低比特/更多缓存）与生成质量之间的根本矛盾。量化误差和缓存误差如何在数学上理解和控制？

4. **本文要解决什么**：提供一个统一框架，深入分析缓存和量化的误差来源，并设计有效的误差补偿机制。

5. **切入角度**：从理论分析出发，揭示量化和缓存的内在联系，设计"调制量化"来动态调整量化参数以补偿误差。

6. **核心 idea 一句话**：通过调制量化参数来主动补偿扩散过程中的量化误差，在极低比特下保持生成质量。
