...
1. **改进采样器**：DDIM、SDE/ODE solver（DPM-Solver）等，通过更好的步长调度减少采样步数，但步数过少时质量下降明显；
2. **知识蒸馏**：Progressive Distillation、Consistency Distillation 等，训练学生模型用更少步数匹配教师输出，但通常伴随性能损失。
