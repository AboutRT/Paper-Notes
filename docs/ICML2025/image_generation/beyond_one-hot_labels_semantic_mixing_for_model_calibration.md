...
1. **领域现状**：深度神经网络通常过度自信——预测置信度不能准确反映实际正确概率。模型校准旨在让置信度与准确率一致。现有方法分为后处理校准（Temperature Scaling）、训练时正则化（Label Smoothing、Focal Loss）和数据增强（Mixup）。
2. **现有痛点**：
...
3. **核心矛盾**：模型需要学习"什么时候该不确定"，但训练数据中没有不确定的样本和标注。
4. **本文要解决什么**：生成带有真实不确定性标注的高保真混合样本来训练校准。
5. **切入角度**：利用扩散模型的条件生成能力——用同一噪声但不同类别混合比例的条件生成一系列语义连续变化的样本。
6. **核心 idea**：扩散模型生成语义混合样本 + CLIP 重标注精确置信度 + $L_2$ 损失实现平衡学习。
