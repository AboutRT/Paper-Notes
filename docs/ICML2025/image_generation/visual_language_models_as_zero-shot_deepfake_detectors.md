---
description: "【论文笔记】Visual Language Models as Zero-Shot Deepfake Detectors 论文解读 | ICML 2025 | arXiv 2507.22469 | Deepfake检测 | 提出基于 VLM token 概率归一化的图像分类框架，将 deepfake 检测从二元判断升级为概率估计，在零样本设置下用 InstructBLIP 超越多数专用 deepfake 检测器，微调后在 DFDC-P 上接近完美。"
tags:
  - ICML 2025
  - Deepfake检测
  - 视觉语言模型
...
## 研究背景与动机
1. **领域现状**：Deepfake 检测方法大多训练专用分类器（FaceForensics++、SBI、MAT 等），依赖标注数据且对新型 deepfake 泛化差。
2. **现有痛点**：(a) 现有检测器在分布外数据上性能急剧下降；(b) 已有 VLM deepfake 研究只做 yes/no 二元判断，无法输出置信度概率；(c) 缺乏 FAR/FRR 等实际部署指标的支持。
3. **核心矛盾**：真实部署需要概率输出来调节阈值（平衡误报率和漏报率），但 VLM 的 argmax 输出只能给 0/1。
4. **本文要解决什么？** 如何从 VLM 的 token 分布中提取有意义的分类置信度？
5. **切入角度**：利用 VLM 在"Is this photo real?"回答时 "yes"/"no" token 的概率比作为置信度。
6. **核心idea一句话**：将 yes/no token 概率归一化为 $\tilde{P}_{\text{fake}} = P_{\text{no}} / (P_{\text{no}} + P_{\text{yes}})$，得到可用于 ROC 分析的连续置信度。
