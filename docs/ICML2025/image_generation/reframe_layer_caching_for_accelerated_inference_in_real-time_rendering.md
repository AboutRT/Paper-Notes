...
## 研究背景与动机
1. **领域现状**：实时渲染（如 DLSS 4.0）大量依赖 U-Net 风格神经网络进行去噪、超分、帧外推等任务，网络推理已占渲染 pipeline 的显著比例。
2. **现有痛点**：(a) 渲染帧之间有极高的时间相关性，但每帧都做完整推理；(b) DeltaCNN 等利用帧间差异的方法因稀疏计算在现有 GPU 上难以加速；(c) DeepCache 仅针对扩散模型的多步推理设计。
3. **核心矛盾**：渲染中每一帧都必须单独产出高质量输出（不像扩散模型可以容忍中间步的近似），但帧间特征变化缓慢。
4. **本文要解决什么？** 如何在实时渲染的严格质量要求下利用帧间冗余？
5. **切入角度**：缓存 U-Net 深层的中间特征，跳过编码器-解码器的大部分计算，仅重新计算浅层（对新输入变化敏感的部分）。
6. **核心idea一句话**：实时渲染中帧间特征缓慢变化 → 缓存深层特征 → 自适应刷新策略 → 免训练加速。
