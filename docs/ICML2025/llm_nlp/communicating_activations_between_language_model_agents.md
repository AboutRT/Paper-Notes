...
1. **领域现状**：多 LLM 智能体通过自然语言对话（如 debate）可以提升推理能力，但计算成本随智能体数量和消息长度快速增长。
2. **现有痛点**：(a) 自然语言通信需要完整的生成+解析循环，计算开销大; (b) 解码过程将丰富的内部表示压缩为单一 token，丢失大量信息; (c) 研究表明模型中间层包含比输出层更丰富的实体表示。
3. **核心矛盾**：自然语言是为人类设计的通信媒介，未必是 LLM 间最优的通信方式。
4. **本文要解决什么**：能否让 LLM 用更高效、更高信息密度的方式——直接传递激活向量——来通信？
5. **切入角度**：Hernandez et al. 发现模型在约一半层处已构建了丰富的实体表示，但到后面层会压缩为下一个 token 的表示——这意味着中间层激活比最终输出携带更多信息。
6. **核心 idea**：暂停模型 B 在第 j 层的计算，用函数 $f$ 融合模型 A 第 k 层的激活，然后继续 B 的前向传播。
