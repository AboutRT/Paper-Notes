...
1. **单一冲突类型**：之前的方法（如 PH3）主要针对句子级替换冲突（substitution conflict），在更难的段落级连贯冲突（coherent conflict）下效果大幅下降
2. **排他性假设有误**：先前研究假设存在排他的"记忆头"（memory head）和"上下文头"（context head），但本文发现同一注意力头可以**同时**贡献参数知识和上下文知识
3. **单向视角**：多数工作只关注增强上下文依赖（解决 RAG 幻觉），缺少统一的双向调控方法
