...
description: >-
  [ICML2025][LLM/NLP][黑盒优化] 提出UniSO框架，将异构搜索空间的设计变量统一编码为字符串，通过语言模型嵌入实现跨域跨维度的通用离线黑盒优化，结合元数据引导的嵌入分布对齐和局部Lipschitz平滑正则化，在Design-Bench和SOO-Bench上验证了跨任务泛化能力。
tags:
  - ICML2025
  - LLM/NLP
  - 黑盒优化
...
**本文要解决什么**：建立一个能同时处理多种类型、多种维度离线BBO任务的通用框架，实现跨任务知识迁移，解决数据稀缺问题。

**切入角度**：将所有设计变量序列化为JSON字符串（如 {"x0":0.5, "x1":1.2}），利用语言模型天然的序列处理能力统一表示异构空间，再通过元数据引导和嵌入正则化来保证多任务区分性和优化友好性。

**核心idea一句话**：字符串统一表示+语言模型嵌入+元数据引导的嵌入空间正则化 = 通用离线黑盒优化。
...
1. **两种建模范式（UniSO-T与UniSO-N）**：
    ...
    - 做什么：UniSO-T将目标值也编码为token序列（P10编码），用next-token prediction训练端到端序列模型；UniSO-N用预训练T5编码器将字符串嵌入统一潜空间，再训练MLP回归器映射到数值分数。
...
2. **元数据引导的嵌入分布对齐**：
    ...
    - 做什么：通过对比学习将输入嵌入的相似度分布与元数据嵌入的相似度分布对齐——元数据相似的任务嵌入应彼此靠近，不相似的应分离。
...
3. **局部Lipschitz平滑正则化**：
    ...
    - 做什么：约束同一任务内嵌入空间的局部平滑性，使嵌入距离与目标值差异的比值不超过某个Lipschitz常数。
