...
description: >-
  [CVPR 2025][LLM对齐][RLHF] 通过梯度分析发现多模态 RLHF 相比持续 SFT 的核心优势在于 rejected response 中的负监督信号，据此提出 nSFT 方法，用 LLM 从拒绝回复中提取错误信息并构造纠正性对话数据…
tags:
  - "CVPR 2025"
  - "LLM对齐"
  - "RLHF"
...
## 研究背景与动机
**领域现状**：VLM 通常经过预训练 → SFT 后，再通过 RLHF（如 DPO、PPO）进行偏好对齐来减少幻觉、提升多模态理解能力。业界普遍认为 RLHF 在偏好对齐阶段严格优于持续 SFT。

**现有痛点**：多模态 RLHF 虽然有效，但存在明显的工程瓶颈——DPO 需要同时加载 2 个大模型（policy + reference），PPO 则需要 4 个（policy + reference + reward + critic），显存开销巨大，训练也不稳定。而直接做持续 SFT（用偏好数据的 chosen response 继续训练）效果又明显不如 RLHF。

**核心矛盾**：为什么同样的数据，SFT 就不行而 RLHF 就行？到底差在哪里？

**本文要解决的问题**：找到 RLHF 优于 SFT 的根本原因，然后把这个优势以 SFT 的方式实现，兼顾效果和效率。

**切入角度**：作者从梯度角度分析 DPO loss，发现其梯度本质上就是 chosen SFT 梯度和 rejected SFT 梯度的线性组合。持续 SFT 缺失的恰好就是 rejected response 上的 SFT loss（负监督信号）。

**核心idea**：用 LLM 从 rejected response 中提取错误信息并构造纠正性训练数据，使普通 SFT 也能获得 RLHF 的负监督信号优势。
...
2. **负监督信号解耦与数据构造（$G(\cdot)$ 函数）**：
    ...
    - 功能：将 RLHF 中隐式的负监督信号显式地转化为 SFT 可用的训练数据
...
3. **视觉错误码表（Vision Error Codebook, VEC）**：
    ...
    - 功能：为 LLM 提供全面的图像相关错误类型，指导其精确识别 rejected response 中的幻觉
