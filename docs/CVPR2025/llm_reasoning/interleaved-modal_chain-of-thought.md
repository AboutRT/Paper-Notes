---
title: >-
  [论文解读] Interleaved-Modal Chain-of-Thought
description: >-
  [CVPR 2025][LLM推理][Chain-of-Thought] 提出交错模态思维链（ICoT），在推理步骤中穿插图像区域 crop 作为视觉 rationale，通过无参数的 Attention-driven Selection（ADS）从输入图像中智能选取关键区域插入生成序列，在 Chameleon 和 Qwen2-VL 上相比现有多模态 CoT 提升高达 14%。
tags:
  - CVPR 2025
  - LLM推理
  - Chain-of-Thought
...
1. **Attention-driven Selection (ADS)**：
    ...
    - 功能：在每个推理步骤开始时，从输入图像中选取最相关的 patch 插入生成序列
...
2. **ICoT 提示设计**：
    ...
    - 功能：设计包含交错视觉-文本 rationale 的 few-shot 示例来引导 VLM
