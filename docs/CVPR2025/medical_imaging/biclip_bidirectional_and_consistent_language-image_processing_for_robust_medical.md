...
description: >-
  [CVPR 2025][医学图像][医学图像分割] BiCLIP 提出了一种双向一致性视觉-语言分割框架，通过双向多模态融合（BMF，让视觉特征反向精炼文本嵌入）和图像增强一致性（IAC，跨弱/强扰动正则化），在 COVID-19 CT 分割上以仅 1% 标注数据即可保持鲁棒性能…
tags:
  - "CVPR 2025"
  - "医学图像"
  - "医学图像分割"
...
**本文目标** 如何在标注极度稀缺和图像退化条件下实现鲁棒的医学图像分割？

**切入角度**：双向融合让视觉反哺文本 + 一致性正则化确保扰动不变性。

**核心 idea**：双向视觉-文本融合（视觉→文本的伪图像生成器）+ 弱/强增强一致性正则化。
