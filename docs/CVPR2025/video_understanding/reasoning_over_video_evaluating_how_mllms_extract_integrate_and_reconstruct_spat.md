...
1. **领域现状**：现有视频时空基准（VSI-Bench、VSTI-Bench）主要测试"提取式推理"——答案可从单帧或局部时空事件中直接提取（如物体出现顺序、相对方向）。
2. **现有痛点**：提取式推理不能评估模型是否能形成全局一致的空间表示——是否能从碎片化的第一人称观察中重建房屋平面图？能否跨房间计数？能否理解房间之间的全局方位关系？
3. **核心矛盾**：具身智能需要的"抽象时空推理"能力（整合分散观察、推断隐含空间结构）几乎未被评估。
4. **本文要解决什么？** 构建可控基准来系统评估 MLLM 的抽象 vs 提取式时空推理能力。
5. **切入角度**：不是在已有视频上标问题（证据固定、难以设计抽象推理问题），而是"先设计问题、再生成环境和视频"——query-conditioned video construction。
6. **核心 idea 一句话**：从提取式推理扩展到抽象式推理的一对一任务对比，用可控合成环境暴露 MLLM 的时空推理瓶颈。
