...
1. **领域现状**：可控视频生成是视频生成的关键方向，包括相机轨迹控制（CameraCtrl、MotionCtrl）和物体运动控制（Motion-I2V、MOFA-Video）。
...
3. **核心矛盾**：不同控制信号（相机参数、拖拽箭头、参考视频）本质上都在描述像素运动，但表示方式完全不同，难以统一融合。

4. **核心idea**：如果把所有控制信号都统一转化为逐帧光流表示，就能用统一的方式引导视频生成，自然解决信号冲突。
