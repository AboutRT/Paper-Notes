...
**领域**: PEFT / 多任务学习  
**关键词**: 参数高效微调, Mixture-of-Experts, LoRA, 多尺度特征金字塔, 任务嵌入
...
**现有痛点**: 现有 MoE-LoRA 方法普遍使用**统一架构的专家**（相同 rank 和容量），忽略了任务复杂度的层次性差异——简单任务只需高层语义抽象，复杂推理则需要细粒度句法操作。

**核心矛盾**: "One-size-fits-all" 的专家设计限制了模型表达能力和参数效率；独立学习每个专家的参数又导致冗余。

**本文目标** 如何在保持参数高效的前提下，让不同专家具备不同粒度的特征捕获能力，同时共享通用语言知识。

**切入角度**: 从 CV 领域的 Feature Pyramid Network (FPN) 获得灵感——识别不同尺度的目标需要不同分辨率的特征，类比到 NLP 中，多任务适配也需要"参数金字塔"。

**核心 idea**: 学习一个低维共享元知识种子，通过不同尺寸的反卷积核投影到不同尺度的参数矩阵，形成专家金字塔。
...
1. **共享元知识子空间**
    ...
    - 功能：构建所有任务和专家共享的低维潜在表示 Z_meta ∈ R^{h×w}，其中 h, w ≪ d_model
...
2. **金字塔投影机制 (Pyramid Projection)**
    ...
    - 功能：利用 N 个反卷积专家，每个专家具有不同的核尺寸 s_i，将元知识投影到不同尺度
...
3. **自适应 LoRA 剪枝器 (Adaptive LoRA Pruner)**
    ...
    - 功能：动态裁剪元知识基的活跃参数以匹配当前任务尺度所需的粒度
