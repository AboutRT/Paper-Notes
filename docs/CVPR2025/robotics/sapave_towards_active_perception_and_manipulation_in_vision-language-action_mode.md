...
description: >-
  [CVPR 2025][机器人][VLA] SaPaVe 提出了一种端到端的主动操作框架，通过解耦相机运动和操作动作的 action space，采用自底向上的两阶段训练策略（先学语义相机控制，再联合优化），在 200K 语义相机运动数据集上训练主动感知先验，配合 3D 几何感知模块增强视角变化下的执行鲁棒性，在真实世界任务中比 GR00T N1 和 $\pi_0$ 分别高 31.25% 和 40% 成功率。
tags:
  - CVPR 2025
  - 机器人
  - VLA
...
## 研究背景与动机
**领域现状**：VLA 模型（$\pi_0$, GR00T-N1）在固定近最优视角下训练和部署，取得了良好的操作能力。

**现有痛点**：（1）现实世界中存在遮挡和视野外物体，固定视角无法覆盖所有情况；（2）将相机运动直接加入 VLA 的 action space 会破坏已有的固定视角操作先验，且需要大量昂贵的主动操作数据；（3）VLA 模型缺乏 3D 几何理解，在视角变化时执行不稳定。

**核心矛盾**：主动操作需要两种互补能力——语义主动感知（选择合适视角）和主动视角执行（在非最优视角下仍能操作）——但现有方法要么不支持语义输入的主动感知，要么无法应对视角变化下的操作。

**本文目标** 如何以数据高效的方式在 VLA 中同时实现语义驱动的主动感知和视角鲁棒的执行？

**切入角度**：相机运动是 embodiment-agnostic 的（与机器人本体无关），可以用大规模图像-语言-相机运动数据单独学习；操作动作是 embodiment-specific 的，需要联合优化。

**核心 idea**：解耦 action space（相机 vs 操作）+ 自底向上训练（先学主动感知，再学主动操作）+ 3D 几何注入。
...
1. **解耦动作头 + Camera Adapter**:
    ...
    - 功能：将相机控制和操作动作分开为两个解码器，用 LoRA adapter 学习相机运动
...
2. **Universal Spatial Knowledge Injection**:
    ...
    - 功能：将 3D 几何信息（深度图、相机内外参等）注入动作生成过程
