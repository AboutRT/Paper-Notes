...
### 主实验
- 最少8个事件即可恢复完整6-DoF运动（已知旋转时只需5个）
- 在合成和真实事件相机数据上均成功估计角速度和线速度
- 成功解耦旋转和平移分量，无需IMU辅助
...
| 配置 | 说明 |
|------|------|
| 单线段 vs 批量线段 | 批量线段（IncBat）更稳定但更慢 |
| 入射关系 vs 共面关系 | 共面关系在线段不相交时更灵活 |
| 有/无纯旋转处理 | 纯旋转是退化情况，需要专门检测和处理 |
| 一阶旋转近似 | 在小运动假设下高效且精确 |

### 关键发现
- 共面关系是本文的理论创新——不需要线段相交或共享点即可建立约束
- 纯旋转情况下平移不可估计（理论退化），需要运动分类检测
- Adam优化器+一阶近似的组合在效率和精度间取得良好平衡
