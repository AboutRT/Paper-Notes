...
description: >-
  [CVPR 2025][动作生成][缩放律] 首次在动作生成领域验证缩放律，提出Motion FSQ-VAE解决codebook collapse(利用率23%→96%)、构建260小时MotionUnion数据集、文本前缀自回归Transformer，发现归一化损失与FLOPs的对数律及模型/词汇表/数据的幂律关系
tags:
  - CVPR 2025
  - 动作生成
  - 缩放律
...
1. **Motion FSQ-VAE（有限标量量化）**:
    - 做什么：用简单的round操作替代VQ的argmin匹配，从根本上解决codebook collapse问题
...
2. **文本前缀自回归Transformer（Text-Prefix AR）**:
    - 做什么：用冻结的T5-XL编码器生成词级embeddings作为prefix，motion token部分用因果注意力自回归生成
...
3. **MotionUnion数据集**:
    - 做什么：构建260小时、150k序列的大规模文本-动作数据集
