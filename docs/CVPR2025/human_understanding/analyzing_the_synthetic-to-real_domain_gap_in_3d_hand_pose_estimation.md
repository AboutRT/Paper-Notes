...
1. **领域现状**：3D手势估计依赖大量3D标注数据进行训练，但真实数据标注昂贵耗时。在人脸识别和人体姿态估计领域，合成数据已能达到SOTA水平，但手势估计仍存在显著的合成到真实域差距。

2. **现有痛点**：现有合成手势数据集（RHD、ObMan、DARTset、RenderIH等）各有不同的局限——背景简单、纹理有限、缺少手臂、无物体交互。由于这些数据集在多个维度上同时与真实数据不同，无法分离各因素的独立贡献。

3. **核心矛盾**：手势相比人脸和人体有更严重的自遮挡和物体遮挡，加之不同数据集使用不同的骨架拓扑结构（MANO 21关节 vs NIMBLE 25关节），使得域差距来源复杂交织。

4. **核心idea**：设计一个可控的数据合成管线，能独立调节各图像组件（手纹理、背景、手臂、物体、姿态分布），从而分解分析每个组件对域差距的贡献。
...
1. **高质量手模型渲染**：
   - 做什么：基于NIMBLE模型渲染具有逼真骨骼/肌肉/皮肤/纹理的3D手
