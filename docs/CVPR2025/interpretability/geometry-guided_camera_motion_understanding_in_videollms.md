---
description: "【论文笔记】Geometry-Guided Camera Motion Understanding in VideoLLMs 论文解读 | CVPR 2025 | arXiv 2603.13119 | camera motion | 提出一个从基准构建、诊断到注入的完整框架，通过 3D 基础模型（VGGT）提取相机运动线索并以结构化提示注入 VideoLLM，实现无需训练的相机运动感知增强。"
tags:
  - CVPR 2025
  - camera motion
  - VideoLLM
...
1. **领域现状**: VideoLLM 在高层视频语义（物体、动作、叙事）上表现良好，但对相机运动（pan、tilt、dolly 等）的细粒度识别严重不足。
2. **现有痛点**: 相机运动是时空几何信号，无法定位到单帧，容易被物体运动、剪切、运动模糊干扰；大规模视频数据集缺乏相机运动的显式监督。
3. **核心矛盾**: VideoLLM 的 vision encoder 在深层进行 token 压缩以优化语义对齐，但这会衰减运动敏感线索；而相机运动理解需要精确的几何信息。
4. **本文要解决什么**: 让 VideoLLM 具备可靠的细粒度相机运动识别能力，并能生成相机感知的视频描述。
5. **切入角度**: 不修改 VideoLLM 权重，而是利用冻结的 3D 基础模型提取几何相机线索，通过轻量分类器预测运动原语，以结构化 prompt 注入。
6. **核心 idea 一句话**: 用 3DFM 的几何先验补偿 VideoLLM 缺失的相机运动表示，通过 plug-and-play 的结构化提示实现零训练的相机运动增强。
