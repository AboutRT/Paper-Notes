---
description: "【论文笔记】Multimodal OCR: Parse Anything from Documents 论文解读 | CVPR 2025 | arXiv 2603.13032 | 文档解析 | 提出 Multimodal OCR (MOCR) 范式，将文档中的文本和图形（图表、图标、UI 等）统一解析为结构化文本表示（包括 SVG 代码），3B 模型在 olmOCR-Bench 上达到 83.9 SOTA，图形解析超越 Gemini 3 Pro。"
tags:
  - CVPR 2025
  - 文档解析
  - OCR
...
1. **领域现状**：现有文档解析主要是 text-centric——识别和组织文本内容，遇到图表、图标等图形区域就裁剪为像素图，丢弃其中的结构和语义信息。
2. **现有痛点**：文档中大量信息通过图形传达（图表、流程图、化学结构式等），但传统 OCR 管道将这些视为"黑箱"。这使得文档解析本质上是"有损"的，限制了从文档中提取的结构化监督信号量。
3. **核心矛盾**：图形不像文本有标准的文字表示，需要找到一种统一的可执行、可编辑、可验证的表示形式来捕获图形的结构信息。
4. **本文要解决什么？** 设计一个统一架构同时解析文档中的文本和图形，把图形也变成可复用的结构化输出。
5. **切入角度**：现代 VLM 已具备从图像生成可执行表示的能力（如 SVG），可以将文档图形解析为可渲染代码而非像素裁剪。
6. **核心 idea 一句话**：文档解析不应止步于文字识别，图形也是一等公民——用 SVG 代码统一表示，实现"parse anything"。
...
4. **OCR Arena 自动评估**：
   - 做什么：用 LLM-as-Judge（Gemini 3 Flash）做成对比较，Elo 评分
