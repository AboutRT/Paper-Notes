...
1. **CLIP无法捕获时间动态**：CLIP设计用于静态图像-文本对齐，无法理解状态变化、动作和时间上下文。例如"正在跑的狗"vs"坐着的狗"在CLIP特征中难以区分。
2. **缺乏像素对齐的物体级视频特征**：现有视觉模型（如VideoCLIP）主要提取全局视频级特征，裁剪物体则混入背景信息或无法区分物体/相机运动。
...
1. **多模态逐物体视频提示（Multimodal Object-Wise Video Prompting）**:
   - 做什么：引导MLLM为视频中每个物体生成时间一致、高质量的逐帧caption
...
2. **状态可变形网络（Status Deformable Network）**:
   - 做什么：建模每个高斯点语义特征的时间演变，约束其在有限状态原型间平滑过渡
