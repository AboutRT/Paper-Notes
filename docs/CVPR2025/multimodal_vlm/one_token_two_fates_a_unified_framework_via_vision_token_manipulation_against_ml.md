...
**MLLM幻觉的核心机制**: 多模态大语言模型(MLLM)生成文本时，视觉注意力随生成步骤逐渐衰减，而语言先验（text inertia）逐渐主导，导致生成内容与视觉证据矛盾——即"幻觉"

**现有两类训练无关方法的局限**:
   - 视觉增强类(如PAI)：通过放大注意力权重强化视觉信号，但面对强语言先验仍力不从心
   - 文本抑制类(如VCD)：通过对比解码在logit层抑制语言惯性，但依赖图像级扭曲(modality-gap)产生的负样本噪声大且不稳定

**朴素组合失败**: 直接组合PAI+VCD实验表明性能无提升——两种方法设计理念不同、干预层级不同（一个在attention层、一个在output logit），产生信号冲突

**核心洞察**: Vision token是视觉-语言交互的桥梁，可以同时承担两种角色：(1)通过增强图片提供互补视觉语义(F2)；(2)通过裁剪token(information-gap)精确隔离幻觉倾向(F3)

**核心idea**: 统一框架在隐表示层操作，从vision token派生所有校准信号——增强和抑制协同协作
