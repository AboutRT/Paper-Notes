---
description: "【论文笔记】Active Data Curation Effectively Distills Large-Scale Multimodal Models 论文解读 | CVPR 2025 | arXiv 2411.18674 | 知识蒸馏 | 提出 ACID（主动数据筛选即隐式蒸馏）和 ACED（结合显式蒸馏），证明用大模型作为参考来主动筛选训练数据是一种比传统知识蒸馏更有效的多模态模型压缩方式，两者互补结合后在 27 个零样本任务上以更少推理 FLOPs 达到 SOTA。"
tags:
  - CVPR 2025
  - 知识蒸馏
  - 主动数据筛选
...
然而存在两个关键gap：
1. **KD 方法越来越复杂**：多种损失函数组合、bespoke 架构、复杂训练流程，可复现性差
2. **数据筛选领域的"共识"限制了想象力**：现有主动数据筛选方法假设参考模型应小于/等于学习模型（为效率考虑），且认为用大模型筛选的数据对小模型不适用（capacity gap）
...
1. **ACID：主动筛选即隐式蒸馏**:
   - 做什么：用预训练的大参考模型 $\theta_{ref}$ 从超级批次 $\mathcal{S}$（大小 $B$）中筛选出高信息量的小批次 $\mathcal{B}$（大小 $b$）用于训练
   - 核心思路：两种筛选评分策略——
...
2. **联合批次采样（Joint Batch Sampling）**:
   - 做什么：批次内样本选择相互依赖（对比学习中每个样本的损失取决于批次内其他样本），需联合采样
...
3. **ACED：结合隐式与显式蒸馏**:
   - 做什么：将 ACID 与 Softmax-KD 结合，同时利用两种互补的知识转移途径
