...
description: >-
  [CVPR 2025][时间序列][CPC预测] 将付费搜索CPC预测重构为"部分可观测竞争下的预测"问题，通过语义邻域（Transformer嵌入）、行为邻域（DTW对齐）和地理意图三类竞争代理逼近不可观测的竞争状态，在1811个关键词×127周的Google Ads数据上显示竞争感知增强在中长期预测（6/12周）上显著优于单变量和弱上下文baseline。
tags:
  - CVPR 2025
  - 时间序列
  - CPC预测
...
**核心 idea**：用语义/行为/地理三类代理逼近潜在竞争，通过协变量+图两条路线注入预测模型，在中长期预测上获得稳定提升。
...
1. **语义邻域与语义图**
    ...
    - 功能：用Transformer嵌入捕捉意图级别的关键词相似性
...
- [\[CVPR 2025\] Learning Extremely High Density Crowds as Active Matters](learning_extremely_high_density_crowds_as_active_matters.md)
