...
description: >-
  [CVPR 2025][图像恢复][稀疏正则化] 系统比较 $\ell_1$ 正则化 (LASSO) 与 Variational Garrote (VG, 概率 $\ell_0$ 近似) 在信号重采样、去噪和稀疏视角 CT 重建三种逆问题上的表现…
tags:
  - "CVPR 2025"
  - "图像恢复"
  - "稀疏正则化"
...
**本文目标**：统一框架下比较 $\ell_1$ (LASSO) 和概率 $\ell_0$ (VG) 在不同信息瓶颈下的重建行为。

**切入角度**：通过 train-generalization error curve（扫描正则化强度，比较各方法在最优正则化强度下的最小泛化误差）实现模型无关的公平比较。

**核心 idea**：当底层系数分布是强稀疏时，spike-and-slab 型先验 (VG) 比 Laplace 型先验 (LASSO) 在 support recovery 上更准确，从而在强欠定域获得更低重建误差。
...
1. **Variational Garrote (VG)**：
    ...
    - 功能：通过隐二值门控变量 $s_i \in \{0,1\}$ 近似 $\ell_0$ 稀疏
...
2. **公平比较协议**：
    ...
    - 功能：通过 train-generalization error curve 进行模型无关比较
...
- [\[CVPR 2025\] Complexity Experts are Task-Discriminative Learners for Any Image Restoration](complexity_experts_are_task-discriminative_learners_for_any_image_restoration.md)
