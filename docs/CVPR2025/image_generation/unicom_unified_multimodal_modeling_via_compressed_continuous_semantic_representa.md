...
## 研究背景与动机
**统一模型的核心挑战**：当前统一多模态模型需要找到一个"统一 token"表示，同时支持理解和生成，但视觉表示设计一直是核心瓶颈

**离散化的信息损失**：基于向量量化的方法（如 VQ-VAE）不可避免地丢弃细粒度语义信息，导致理解任务性能次优

**混合编码器的表示分裂**：VAE latent + ViT 特征的混合方案导致理解和生成基于不同特征空间，限制了深度统一

**连续特征建模困难**：直接对高维连续 ViT 表示进行生成建模面临分布复杂、收敛慢、训练不稳定等问题

**流形假设**：高维 VLM 嵌入空间的有效信息实际位于低维子流形上，提示可通过学习压缩来发现该子流形

**实际需求**：需要在保留语义精度的同时简化数据分布，使生成建模变得可行
