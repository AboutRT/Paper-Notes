...
**领域现状**：卷积算子是 CNN 的基石，但其本质是固定的、线性的、局部平均操作。

**现有痛点**：卷积的四个结构性限制：线性（不建模非线性交互）、平移等变（对位置依赖统计不敏感）、固定局部性（受限感受野）、均匀加权（不区分边缘与噪声）。

**核心矛盾**：大量替代卷积的工作分散在信号处理、数值线性代数、模糊数学和深度学习多个社区，缺乏统一视角。

**本文目标**：提供系统化分类学框架，让从业者理解何时、为何用什么算子替代卷积。

**切入角度**：以标准卷积的四个结构性质为参考点。

**核心 idea**：算子选择不仅是实现细节，而是基本建模决策，编码信号和任务的先验知识。
...
1. **分解型算子（Decomposition-Based）**
    ...
    - 功能：用 SVD/HOSVD 对局部 patch 做矩阵/张量分解，阈值化分离结构和噪声
