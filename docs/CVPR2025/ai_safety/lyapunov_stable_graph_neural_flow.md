---
title: >-
  [论文解读] Lyapunov Stable Graph Neural Flow
description: >-
  [CVPR 2025][AI安全][图神经网络] 将 Lyapunov 稳定性理论（整数阶和分数阶）与图神经流集成，通过可学习 Lyapunov 函数和投影机制将 GNN 特征动态约束在稳定空间中，首次为图神经流提供可证明的对抗鲁棒性保证，且与对抗训练正交可叠加。
tags:
  - "CVPR 2025"
  - "AI安全"
  - "图神经网络"
...
**领域现状**：领域现状**：基于动力系统的 GNN（如 GRAND、GREAD、FROND）将节点特征更新建模为微分方程演化，但仍然脆弱于对抗扰动
...
**本文目标**：能否用 Lyapunov 稳定性理论为图神经流提供严格的理论稳定性保证？

**切入角度**：Lyapunov 直接法——构造正定且递减的 Lyapunov 函数即可判定系统稳定，无需显式求解微分方程

**核心 idea**：用可学习的 Lyapunov 函数 + 投影机制将图神经流的动态约束到稳定流形上
...
2. **稳定性投影机制**:
    ...
    - 功能：将基础图神经流的动态投影到满足 Lyapunov 条件的稳定流形
