...
**代码**: [https://anhthuan1999.github.io/varsplat/](https://anhthuan1999.github.io/varsplat/)  
**领域**: 3D视觉  
**关键词**: 3D Gaussian Splatting, SLAM, uncertainty modeling, RGB-D, 位姿估计
...
1. **Per-splat 外观方差 $\sigma_i^2$**:
    ...
    - 功能：为每个 Gaussian splat 新增一个 3 维参数 $\sigma_i^2 \in \mathbb{R}^3$，表示该 splat 颜色均值周围的方差
...
2. **Per-pixel 不确定性渲染（全方差定律）**:
    ...
    - 功能：将 per-splat 方差传播为 per-pixel 方差 $V$
...
3. **端到端方差学习（Gaussian NLL）**:
    ...
    - 功能：通过高斯负对数似然损失 $\mathcal{L}_{\text{var}}$ 训练方差
