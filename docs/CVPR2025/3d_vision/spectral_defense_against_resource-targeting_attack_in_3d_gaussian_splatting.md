...
**本文目标**：如何在训练监督被投毒的情况下，从频域角度同时抑制高斯过度增长和渲染图像中的攻击伪影？

**切入角度**：发现投毒图像在频谱上表现为异常高频放大和角向能量各向异性（正常图像的高频能量近似各向同性），从而在 3D 参数空间和 2D 渲染空间分别设计频域防御。

**核心 idea**：投毒攻击的根因是频谱行为异常而非空间结构异常——频域防御同时在 3D 高斯剪枝和 2D 渲染正则化上起效。
...
1. **3D 频率滤波器（高斯频率表征 → 频率感知剪枝）**：
    ...
    - 功能：将高斯协方差与其高频响应关联，选择性移除异常高频 splat
...
4. **本文目标** 如何抵御利用 3DGS 生长机制的资源耗尽攻击？
5. **切入角度**：攻击的本质是注入异常高频分量——用频谱分析可以检测和过滤。
6. **核心 idea**：3D 频域剪枝异常 splat + 2D 角度熔正则化保护渲染质量。
