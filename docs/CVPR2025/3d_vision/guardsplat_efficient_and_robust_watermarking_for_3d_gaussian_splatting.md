...
- **功能**: 仅训练消息解码器 $\mathcal{D}_M$（3 层 MLP，512→L），不需要训练编码器或任何图像
- **核心思路**:
...
- **功能**: 为每个 3D Gaussian 创建可学习 SH 偏移量 $\mathbf{h}_i^o \in \mathbb{R}^{48}$，冻结其余所有属性
- **核心思路**: 
...
- **功能**: 在优化过程中引入可微失真层，随机模拟裁剪、缩放、旋转、JPEG 压缩、亮度抖动
