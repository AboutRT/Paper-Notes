...
**本文目标**：(1) 消除VLM的运动学幻觉实现准确部件分割；(2) 保证关节参数在物理仿真中无碰撞可执行

**切入角度**：将边界提取与语义推理解耦——先在3D原生空间提取纯几何基元，再用运动学先验锚定VLM推理；用SDF约束轨迹优化确保无穿模

**核心 idea**：SP4D 运动学先验消除 VLM 幻觉 + 物理约束轨迹优化消除穿模
