...
**代码**: [https://virtualhumans.mpi-inf.mpg.de/hoi3dgen/](https://virtualhumans.mpi-inf.mpg.de/hoi3dgen/)  
**领域**: 3D视觉  
**关键词**: 人物交互生成, 文生3D, 接触语义, 数据标注, SMPL
...
**现有痛点**：SDS 方法（如 InterFusion）存在 Janus 问题、纹理噪声、穿模严重，且无法精确控制接触点。直接3D生成方法（如 TRELLIS）不具备交互感知能力。根本原因是缺乏高质量的**文本-3D交互配对数据**

**核心矛盾**：现有3D交互数据集（BEHAVE等）交互丰富但缺少精细文本描述；手动标注不可扩展；GPT-4V标注成本高且描述不自然

**本文目标**：(1) 如何自动生成高质量交互文本标注？(2) 如何用极少数据让模型学会交互生成？(3) 如何确保3D结果有正确的接触语义？

**切入角度**：将复杂标注任务分解为外观/动作/接触子任务让开源MLLM分别解决；只需400个高质量样本即可微调出交互能力

**核心 idea**：用分解式MLLM标注管线 + 极少量精筛数据微调扩散模型，实现文本驱动的精确接触控制3D HOI生成
...
1. **分解式自动标注管线（Decomposed Annotation Pipeline）**:
    ...
    - 功能：从 ProciGen 3D交互数据集自动生成高质量文本描述
...
3. **视角条件化2D生成（View-Conditioned Generation）**:
    ...
    - 功能：微调 SANA 扩散模型，使其能生成指定视角的交互图像
