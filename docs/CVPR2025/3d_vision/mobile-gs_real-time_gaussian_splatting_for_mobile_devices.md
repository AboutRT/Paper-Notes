...
**本文目标**：(1) 消除排序瓶颈实现无序渲染，(2) 补偿无序渲染的质量损失，(3) 压缩存储以适配移动端

**切入角度**：深度感知的权重函数可以隐式模拟近-远排序效果；视角依赖 MLP 可以补偿透明度伪影

**核心 idea**：用深度感知权重替代排序 + 神经网络补偿 + 多维压缩实现移动端实时3DGS
...
1. **深度感知无序渲染（Depth-aware Order-independent Rendering）**:
    ...
    - 功能：消除 alpha blending 的排序依赖，用加权求和替代
...
2. **神经视角依赖增强（Neural View-dependent Enhancement）**:
    ...
    - 功能：用 MLP 预测视角依赖的不透明度和权重，补偿无序渲染的透明度伪影
