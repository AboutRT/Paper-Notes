...
description: >-
  [CVPR 2025][3D视觉][4D重建] 将单目视频的4D马匹重建解耦为运动估计（AniMoFormer时空Transformer）和外观重建（EquineGS单图前馈3DGS），依托VAREN参数化模型和两个大规模合成数据集，在真实数据上达到SOTA几何+外观重建效果，且能零样本泛化到驴和斑马。
tags:
  - "CVPR 2025"
  - "3D视觉"
  - "4D重建"
...
**核心 idea**：运动和外观解耦后分别用专门的网络+合成数据解决，再通过VAREN模型的skinning将外观驱动到每帧姿态，实现高效高质量4D马匹重建。
