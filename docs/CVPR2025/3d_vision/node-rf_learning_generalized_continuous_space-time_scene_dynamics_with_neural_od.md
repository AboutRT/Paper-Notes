...
**本文目标**：(1) 连续时间场景外推——预测训练序列之外的任意时间点；(2) 轨迹泛化——从多个共享动力学的序列学习后泛化到未见初始条件

**切入角度**：Neural ODE 天然建模连续时间演化，其微分方程形式允许平滑、一致的长期预测

**核心 idea**：用 Neural ODE 驱动 NeRF 的潜在状态随时间连续演化，实现外推和泛化
...
1. **ODE 驱动的时序演化**:
    ...
    - 功能：用 Neural ODE 建模潜在场景状态的连续时间演化
