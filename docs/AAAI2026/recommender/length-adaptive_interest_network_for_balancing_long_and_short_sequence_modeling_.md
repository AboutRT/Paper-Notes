---
description: "【论文笔记】Length-Adaptive Interest Network for Balancing Long and Short Sequence Modeling in CTR Prediction 论文解读 | AAAI 2026 | arXiv 2601.19142 | CTR预测 | 提出LAIN框架，通过将序列长度作为显式条件信号注入CTR模型，缓解长序列用户与短序列用户之间的性能不均衡问题，包含谱长度编码器、长度条件提示和长度调制注意力三个轻量级即插即用模块。"
tags:
  - AAAI 2026
  - CTR预测
  - 序列长度自适应
...
1. **性能失衡**：增加最大输入序列长度时，长序列用户（>200）性能提升，但短序列用户（<100）的AUC反而显著下降
2. **数据失衡**：短序列用户占57%的用户群体，但仅贡献14.3%的训练样本；而25%的长序列用户却主导了68%的训练数据
...
**做什么**：从长度嵌入生成$k$个可学习的提示token，注入行为序列中。
...
**做什么**：根据序列长度自适应地调整注意力机制的行为。包含两个子模块。
