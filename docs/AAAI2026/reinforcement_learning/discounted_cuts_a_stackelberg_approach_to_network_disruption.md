...
1. **非可加代价函数**：标准最小切割假设代价函数可加，但对抗场景中攻击者折扣部分边代价使之变为非可加
2. **理论空白**：尽管 Most Vital Links 问题历史悠久（Wollmer 1980s），文献中缺乏其在一般图上的 NP-hardness 证明
3. **跨领域统一**：连接 AI（多智能体）、算法博弈论（Stackelberg）和运筹学（网络优化）
