...
1. **传统 MARL 的局限**：标准多智能体强化学习假定干净、结构化的通信协议，无法处理现实世界中嘈杂、语义丰富、甚至具有欺骗性的自然语言信息。
2. **LLM 的局限**：虽然 LLM 擅长处理语言信号，但现有工作大多用 LLM 直接生成动作或模拟行为，缺乏系统性的策略优化，不足以解决复杂经济问题或产生鲁棒的可操作策略。
3. **核心问题**：在复杂的多智能体经济环境中，智能体如何解释和利用自然语言信息来支持最优决策？
