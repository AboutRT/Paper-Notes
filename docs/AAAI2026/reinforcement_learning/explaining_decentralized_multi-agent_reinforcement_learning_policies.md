...
1. **不确定性与非确定性**：去中心化执行导致任务完成顺序存在本质的不确定性，不同智能体可能异步完成任务
2. **智能体间协作难以观测**：每个智能体只观测自身局部状态，跨智能体的协调行为无法从单一轨迹直接推断
3. **现有方法的不足**：单智能体抽象策略图（Topin 2019）、集中式 MARL 宏动作抽象（Boggess 2022）都假定单一输入策略，无法处理多个独立策略的交互
