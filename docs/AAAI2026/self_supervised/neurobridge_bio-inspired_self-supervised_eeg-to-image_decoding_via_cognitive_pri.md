---
description: "【论文笔记】NeuroBridge: Bio-Inspired Self-Supervised EEG-to-Image Decoding via Cognitive Priors and Bidirectional Semantic Alignment 论文解读 | AAAI 2026 | arXiv 2511.06836 | EEG解码 | 提出NeuroBridge框架，通过认知先验增强（CPA，非对称增广模拟感知变异性）和共享语义投影器（SSP，双向对齐到统一语义空间），在THINGS-EEG数据集200类零样本EEG-图像检索任务上达到63.2% Top-1（+12.3%）和89.9% Top-5（+10.2%），大幅超越现有SOTA。"
tags:
  - AAAI 2026
  - EEG解码
  - 跨模态对比学习
...
### 已有工作的不足
1. **动态变异性鸿沟**：同一被试观看同一图像的EEG响应因注意力波动、心理状态、生理噪声等因素而存在显著变异——不同人关注同一张猫图片的不同语义区域，产生差异化的EEG响应
2. **静态固有鸿沟**：EEG是时序、低维、含噪信号，图像是空间结构化的高维语义密集数据，模态差异根本性存在
3. **单向对齐局限**：已有方法（如NICE、ATM）多采用单向对齐策略，将EEG对齐到冻结的CLIP空间，但CLIP的语义空间由视觉-语言数据塑造，与EEG反映的感知/认知过程存在语义不匹配
4. **增广策略不足**：UBP仅在视觉侧引入模糊先验，Neural-MCRL仅在EEG侧做语义补全，缺乏综合性双模态增强框架
5. **数据稀缺**：EEG配对数据规模远小于视觉-语言领域的大规模数据集
