...
1. **天真融合策略**：MTCNN 等方法直接拼接不同模态特征，给所有模态分配相同权重。但识别不同材质时，关键模态是变化的——有些材质靠声音更好识别，有些靠摩擦力。等权重融合会削弱关键模态的优势。

2. **对噪声和缺失不鲁棒**：真实世界中传感器采样率不同导致时序不对齐，传感器可能故障或数据被噪声污染。现有模型在噪声环境下性能急剧下降。

3. **忽略细粒度子类别区分**：如区分软木和硬木——现有方法只做粗粒度分类（8 大类），未探索 193 种细粒度子类别。
