---
description: "【论文笔记】SpatialActor: Exploring Disentangled Spatial Representations for Robust Robotic Manipulation 论文解读 | AAAI 2026 | arXiv 2511.09555 | 语义-几何解耦 | 提出 SpatialActor 框架，通过将语义与几何表征显式解耦，并设计语义引导几何模块（SGM）自适应融合深度噪声特征与预训练深度估计专家先验、以及空间 Transformer（SPT）编码低级空间位置线索，在 RLBench 50+ 任务上达到 87.4% 成功率（SOTA +6.0%），且在重噪声条件下比 RVT-2 高出 19.4%。"
tags:
  - AAAI 2026
  - 语义-几何解耦
  - 深度估计先验
...
1. **3D 空间理解对机器人操控至关重要**：现实操控任务发生在三维空间，需要精确的空间推理、遮挡处理和细粒度物体交互，纯 2D 视觉方法在这些场景下能力不足。
2. **点云方法的稀疏性问题**：基于点云的方法（PolarNet、PerAct 等）虽然能显式表示 3D 几何，但稀疏采样导致细粒度语义信息丢失，且 3D 标注成本高昂限制了预训练规模。
3. **图像方法的语义-几何纠缠问题**：基于 RGB-D 的图像方法（RVT、RVT-2 等）将 RGB 和深度送入共享特征空间，语义与几何的纠缠使模型对深度噪声极为敏感——RVT-2 在轻微噪声下成功率即下降 8.9%。
4. **真实深度噪声不可避免**：传感器噪声、光照变化、表面反射等因素使真实世界中的深度测量始终存在噪声，严重限制了现有方法的实际部署。
5. **缺乏低级空间线索建模**：现有联合建模方法主要保留高层几何信息，忽略了对精确交互至关重要的低级空间线索（如精确的 2D-3D 对应关系）。
6. **核心问题**：如何构建同时具备细粒度空间理解、传感器噪声鲁棒性和低级空间线索支持的稳健空间表征？
