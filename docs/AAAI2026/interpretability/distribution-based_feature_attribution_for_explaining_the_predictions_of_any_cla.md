...
1. **局部近似方法**（如LIME、DLIME、MAPLE）：在目标实例周围拟合简单的代理模型（如线性回归），从中提取特征重要性。但这类方法只利用了数据集的一个局部子集，无法充分利用全局信息。
2. **扰动方法**（如SHAP、PFI）：通过扰动/遮挡特征并观察模型输出变化来衡量特征重要性。但很多实现（如SHAP的Shapley采样值）会创建合成实例，混合来自目标实例和背景数据集的特征值，产生分布外（OOD）数据。
