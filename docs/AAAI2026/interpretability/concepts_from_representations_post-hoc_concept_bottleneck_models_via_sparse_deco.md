---
description: "【论文笔记】Concepts from Representations: Post-hoc Concept Bottleneck Models via Sparse Decomposition of Visual Representations 论文解读 | AAAI 2026 | arXiv 2601.12303 | 概念瓶颈模型 | 提出 PCBM-ReD，通过从预训练视觉编码器中自动提取概念、MLLM 标注/过滤、重建引导选择，再利用 CLIP 视觉-文本对齐将图像表示稀疏分解为概念嵌入的线性组合，构建事后概念瓶颈模型，在 11 个分类任务上达到 SOTA 精度且保持可解释性。"
tags:
  - AAAI 2026
  - 概念瓶颈模型
  - 可解释性
...
1. **事后方法**：提取的概念不保证反映网络真实推理过程，概念与目标之间缺乏因果关系
2. **手工概念**（原始 CBM）：耗时且覆盖不全
3. **LLM 生成概念**（LaBo、Label-free CBM）：包含非视觉特征（如食物味道、鸟类行为），且与数据分布和编码器能力无关
4. **概念独立性**：现有方法不保证概念间的线性无关，影响干预效果
5. **精度-可解释性权衡**：现有 CBM 与端到端模型相比精度损失明显
