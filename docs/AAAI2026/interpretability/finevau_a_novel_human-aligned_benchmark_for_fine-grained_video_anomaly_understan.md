...
1. **N-gram类指标**（如BLEU、ROUGE-L）仅测量词汇重叠，无法捕捉自由形式回答的语义等价性。一个事实正确但用词不同的描述会被错误惩罚。
2. **LLM-based指标**（如AnomEVAL、VAU-EVAL）侧重评估语言流畅性和推理一致性，缺乏对异常视频特定元素的细粒度检测。这些指标给出模糊、主观的分数，与人类对异常的感知严重不对齐。
