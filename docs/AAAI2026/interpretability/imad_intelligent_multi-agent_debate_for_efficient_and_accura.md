...
**本文目标** 何时应该触发多 Agent 辩论？——在零样本设置下做出高效的辩论触发决策。

**切入角度**：单纯的置信度分数不可靠（LLM 常过度自信，错误答案也给高分），需要从 LLM 响应中提取更丰富的犹豫信号（hedging、矛盾、浅层推理），并通过校准损失函数学习泛化的模型行为模式。

**核心 idea**：用自我批判提示引出犹豫信号 + 41 个特征 + FocusCal 损失训练的轻量分类器，选择性触发 MAD。
...
1. **结构化自我批判提示（Self-Critique Prompt）**:
    ...
    - 功能：引导单 Agent 产出三部分内容——初始 CoT 推理、强制的反驳论点、双方置信度分数
...
3. **FocusCal 损失函数**:
    ...
    - 功能：训练辩论决策分类器，使其能在零样本设置下做出准确的触发决策
