...
description: >-
  [AAAI 2026][可解释性][参数高效微调] 提出 GateRA，在 PEFT 方法（LoRA/DoRA/HiRA）中引入轻量级 token 感知门控模块，通过 sigmoid 门控动态调整每个 token 的适配强度——对分布内/简单 token 抑制更新以保留预训练知识…
tags:
  - "AAAI 2026"
  - "可解释性"
  - "参数高效微调"
...
**本文目标** 在几乎不增加参数的前提下，实现 token 级别的自适应调制。

**切入角度**：在 HiRA 的乘法低秩更新中插入 sigmoid 门控函数 $g(x)$，使得 $W' = (g(x) \cdot AB + 1) \cdot W_0$。

**核心 idea**：用轻量门控实现"需要适配的 token 多适配，不需要的保持预训练权重"。
