...
description: >-
  [AAAI 2026][LLM/NLP][Transformer] 提出 TransMamba，一种序列级别的 Transformer-Mamba 混合架构，通过共享 QKV/CBx 参数和 Memory Converter 在不同 token 长度时动态切换 Attention 和 SSM…
tags:
  - "AAAI 2026"
  - "LLM/NLP"
  - "Transformer"
...
**领域现状**：Transformer ($O(T^2)$复杂度)是LLM的主流架构。Mamba（SSM, $O(T)$线性复杂度）在长序列上效率更高但上下文学习和多任务泛化不稳定。现有混合方法（Jamba、Zamba等）采用层级交替（固定比例的Transformer层+Mamba层），但存在结构僵化问题——必须遵循特定的层顺序和比例规则。

**现有痛点**：(a) Transformer在短上下文训练更快，Mamba在长上下文更高效——但层级混合无法在同一序列内利用两者各自的效率优势；(b) 层级混合比例固定（如4:1），不满足特定规则性能就会退化；(c) Mamba2揭示了Attention和SSM在数学形式上的一致性（对偶形式），Wang等人通过蒸馏验证了QKV和CBx参数可互相转移——这暗示可以更大胆地统一两种机制。

**核心矛盾**：需要一种灵活的框架，在同一序列的不同位置自适应使用Attention或SSM，且切换时信息不丢失。

**切入角度**：利用Attention和SSM的参数一致性（Q↔C, K↔B, V↔x），让一套参数同时支持两种计算模式。

**核心 idea**：共享QKV/CBx参数+Memory Converter无损转换+TransPoint调度 = 序列级别灵活切换Attention和SSM。
...
1. **共享参数映射（QKV↔CBx）**：
    ...
    - 功能：一套参数同时支持Attention和SSM两种计算模式
...
2. **Memory Converter（无损信息转换）**：
    ...
    - 功能：在TransPoint处将Attention的K/V无损转换为SSM的初始隐状态 $h_0$
...
3. **TransPoint调度策略**：
    ...
    - 功能：决定每层在哪个token位置从Attention切换到SSM
