...
1. **不受控的树增长**：VFDT 对所有叶子节点一视同仁，导致低活跃/低增益节点也会分裂，浪费内存
2. **集成场景中的冗余**：SOTA 集成方法（如 Adaptive Random Forest）通常以 VFDT 为基学习器，但不对单棵树做后剪枝，导致内存效率低下
3. **固定超参数**：grace period $n_{min}$ 和 tie threshold $\tau$ 通常需要手动调优
