...
3. **序贯敏感度分析（核心模块）**:
   - 做什么：逐一遮蔽 top-K 重要词，测量每次遮蔽造成的嵌入变化
...
4. **GPS 特征张量与 BiLSTM 检测器**:
   - 做什么：将敏感度序列与重要性序列堆叠成 $N \times 2$ 的特征矩阵 $\mathbf{Z} = [\mathbf{s} \| \boldsymbol{\alpha}]$，保留原始词序位置信息
