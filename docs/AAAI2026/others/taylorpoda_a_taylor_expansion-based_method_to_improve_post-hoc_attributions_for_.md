...
description: >-
  [AAAI 2026][local attribution] 在Taylor展开框架下提出精确性(precision)、联合性(federation)、零偏差(zero-discrepancy)三个公设规范特征归因，并引入自适应属性(adaptation)通过AUP目标优化交互效应的分配权重…
tags:
  - "AAAI 2026"
  - "local attribution"
  - "Taylor expansion"
...
## 研究背景与动机
**领域现状**: 事后可解释AI（XAI）中，局部归因（Local Attribution）是主流策略，LIME和SHAP被广泛应用。Deng et al. (2024) 提出用Taylor展开统一各种归因方法的分析框架。

**现有痛点**: 现有方法存在两个核心问题——(F1) 将不相关的Taylor项错误归属给目标特征；(F2) Taylor项的不完整分配或重叠分配。此外，交互效应的分配通常是固定的预定义方案（如SHAP假设涉及特征等分），缺乏任务适应性。
...
2. **自适应属性 (Adaptation Property)**
    ...
    - 功能：允许交互效应在涉及特征之间的分配权重 $\xi_{i,S}$ 是可调的，$\xi_{i,S} \in (0,1)$ 且 $\sum_{i \in S} \xi_{i,S} = 1$
    - 核心思路：区别于SHAP的固定等分（$1/|S|$），TaylorPODA让分配比例可基于下游目标优化
    - 设计动机：在缺乏ground truth解释的事后设置下，固定分配可能偏离真实特征重要性

3. **AUP优化策略**

    - 功能：以Area Under Prediction recovery curve（AUP）作为优化目标，通过Dirichlet分布随机搜索 $\xi_{i,S}$ 的最优组合
