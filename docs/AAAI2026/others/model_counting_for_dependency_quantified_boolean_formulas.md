...
1. **Poly-monious 规约**:
   - 做什么：定义一种新的多项式时间规约，用于建立 #EXP-困难性。
...
2. **#2-DQBF 的 #EXP-完全性证明**:
   - 做什么：证明核心定理——仅含两个存在量词变量的 DQBF 模型计数就已 #EXP-完全。
...
3. **一阶模型计数的应用**:
   - 做什么：证明一阶模型计数（FOMC）即使限制到域大小为 2 且基础逻辑是 PSPACE-可判定片段，仍然是 #EXP-完全的。
...
4. **2-DQBF 模型计数算法**:
   - 做什么：设计一个实用的 #2-DQBF 计数器 sharp2DQR。
