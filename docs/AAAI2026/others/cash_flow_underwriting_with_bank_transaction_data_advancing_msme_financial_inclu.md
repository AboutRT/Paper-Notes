...
1. **后向性（backward-looking）**：只关注过去还款行为，不反映当前或未来还款能力
2. **忽视实时信号**：无法捕捉运营动态和当前财务健康状态
3. **遗漏替代指标**：忽略现金流一致性、应收/应付模式、数字交易行为等可衡量信用的指标
...
**做什么**：将非结构化的银行账单 PDF 转化为结构化数据并提取信用相关特征。
...
**做什么**：用证据权重（WOE）和信息值（IV）量化每个特征的预测能力并进行特征选择。
