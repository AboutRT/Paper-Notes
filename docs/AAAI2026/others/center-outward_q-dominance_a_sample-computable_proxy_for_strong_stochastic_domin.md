...
1. **标量化**：将多目标转为单目标，丢失分布结构信息，依赖预定义的标量化函数
2. **矩代理**（均值/方差/CVaR）：用统计摘要替代随机目标，忽略依赖结构
3. **弱随机支配（Weak FSD）**：通过 CDF 比较实现，但在多维（$d>1$）下过于宽松——可能声明 $\mathbf{X} \succeq_w \mathbf{Y}$，但存在某些单调效用函数下 $\mathbf{X}$ 实际不如 $\mathbf{Y}$
4. **Rioux et al. (2025)**的 OT 方法：为每对分布计算一个最优耦合，多分布比较时缺乏全局一致的参考框架，且计算量随分布对数平方增长
