---
description: "【论文笔记】LeanRAG: Knowledge-Graph-Based Generation with Semantic Aggregation and Hierarchical Retrieval 论文解读 | AAAI 2026 | arXiv 2508.10391 | 检索增强生成 | 提出 LeanRAG 框架，通过语义聚合算法在层次化知识图谱的摘要节点间自动构建显式关系打破\"语义孤岛\"，并基于最近公共祖先（LCA）的自底向上检索策略高效导航层次结构，在四个 QA 基准上取得 SOTA 同时减少 46% 的检索冗余。"
tags:
  - AAAI 2026
  - 检索增强生成
  - 知识图谱
...
1. **语义孤岛问题**：高层摘要节点之间缺乏显式关系连接，彼此孤立。不同概念社区之间无法进行跨社区推理，高层知识变成了一堆互不相通的"岛屿"
2. **结构-检索脱节问题**：检索过程不感知图的拓扑结构，往往退化为对所有节点的平坦语义搜索。精心构建的层次索引在检索时被"展平"使用，丰富的结构信息仅用于检索后的上下文扩展，而非指导检索本身
