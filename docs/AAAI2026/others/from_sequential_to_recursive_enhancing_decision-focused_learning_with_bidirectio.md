...
1. **显式展开方法（Explicit Unrolling）**:
   - 做什么：将预测-优化的循环交互展开为 $K$ 层序列，每层执行一次 $\hat{\boldsymbol{c}}_i = \mathcal{F}_\theta(\boldsymbol{x}_{i-1}, \boldsymbol{v})$ 和 $\boldsymbol{x}_i = \mathcal{G}(\hat{\boldsymbol{c}}_i)$
...
2. **隐式微分方法（Implicit Differentiation）**:
   - 做什么：将递归系统建模为不动点问题 $\boldsymbol{x}^* = \Phi_\theta(\boldsymbol{x}^*)$，在均衡点处直接计算梯度
...
3. **梯度等价性证明**:
   - 做什么：证明当展开步数 $K \to \infty$ 时，显式展开法和隐式微分法给出的梯度完全一致
