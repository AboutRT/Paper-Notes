...
1. **WDNF上的线性时间Shapley值公式**:
   - 做什么：为伪布尔函数的加权析取范式提供封闭形式的Shapley值计算
...
2. **决策模式（Decision Pattern）**:
   - 做什么：将消费者/基线的特征值在决策树上的行为编码为二进制序列
...
3. **从决策模式到WDNF的转换规则**:
   - 做什么：将消费者和基线的决策模式对映射到WDNF中的加权立方体
...
4. **$O(n+m)$的Background SHAP推导**:
   - 做什么：将原始$O(nm)$的Background SHAP计算降为$O(n+m)$
