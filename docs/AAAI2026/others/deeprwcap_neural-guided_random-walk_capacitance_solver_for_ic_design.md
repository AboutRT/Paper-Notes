...
1. **有限差分法（FDM）**：精度高但可扩展性差
2. **模式匹配方法**：计算高效但无精度保证，依赖专家知识
3. **浮动随机游走（FRW）**：可扩展的随机框架，但在多介质域中计算转移核（transition kernel）的代价极高
4. **纯学习方法（CNN-Cap, GNN-Cap）**：需要针对每个工艺节点重训练
5. **混合方法（GE-CNN）**：存在信息瓶颈，CPU-only 实现比原始 FRW 慢12倍
