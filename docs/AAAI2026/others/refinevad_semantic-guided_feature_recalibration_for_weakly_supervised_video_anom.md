...
1. **时序建模浅层且僵化**：大多依赖固定池化或简单聚合方案，无法适应真实异常中多样的运动特征。许多异常由动态、非均匀或上下文相关的运动模式定义，时序刚性严重限制了定位精度。
2. **语义多样性被忽视**：大多数框架将所有异常事件视为单一通用类别，忽略了不同异常类型之间的语义差异。例如打架涉及突然的双向运动，而爆炸表现为突然闪光和空间爆裂——忽略这些差异限制了模型学习判别性特征的能力。
