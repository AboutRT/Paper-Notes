...
1. **领域现状**：多模态情感识别（MER）利用音频、文本、视频等多源信息，但实际场景中常出现模态缺失（背景噪声导致音频不可用、语音识别失败等）。
...
3. **核心矛盾**：如何在模态缺失的情况下，既处理不完整性又解决模态不平衡？

4. **本文要解决什么？** 通过跨模态提示学习同时解决缺失处理和模态平衡。

5. **切入角度**：不恢复缺失数据，而是将每个模态的信息压缩为提示（prompt），传播到其他模态来增强任务相关的一致性信息，同时通过动态调度达到平衡。

6. **核心 idea 一句话**：用渐进式提示生成压缩跨模态一致性信息，通过知识传播模块增强各模态的任务相关特征，增强而非恢复。
