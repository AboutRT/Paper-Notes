---
description: "【论文笔记】SpeakerLM: End-to-End Versatile Speaker Diarization and Recognition with Multimodal Large Language Models 论文解读 | AAAI 2026 | arXiv 2508.06372 | 说话人分离与识别 | SpeakerLM 是首个专为端到端说话人分离与识别（SDR）设计的多模态大语言模型，通过音频编码器-投影器-LLM 架构和灵活的说话人注册机制，在多个公开基准上大幅超越级联基线系统（cpCER 绝对降低最高达 13.82%），并在域外测试集上展现强鲁棒性。"
tags:
  - AAAI 2026
  - 说话人分离与识别
  - 多模态大语言模型
...
1. **级联系统的错误传播**: 传统 SDR 系统采用 SD + ASR 级联框架，SD 模块的错误（如说话人边界不准、标签分配错误）会直接传递到 ASR 模块，导致转录质量下降
2. **重叠语音处理困难**: 传统 SD 系统基于 VAD（语音活动检测），假设每个时间段只有单个说话人，无法有效处理现实中常见的多人同时说话场景
3. **缺乏联合优化**: SD 和 ASR 模块通常使用不同数据集和框架独立训练，无法充分利用两个任务之间的协同效应
4. **LLM 后处理的局限性**: 使用 LLM 对级联系统输出进行修正虽有帮助，但受限于前端系统输出质量，且 LLM 的幻觉问题会导致修改原始文本内容
