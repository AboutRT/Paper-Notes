...
### 现有痛点
1. **现有数据集格式单一**：大多数声明验证数据集的证据只有一种格式——要么全是文本表格（如SciTab），要么全是图表（如MuSciClaims）。即使SciVer包含多种模态，其中表格也以图像而非结构化文本呈现
2. **缺乏对齐评估**：没有数据集能提供表达**相同信息**的表格和图表配对，因此无法公平评估模型在不同格式上的表现
3. **模型偏见未知**：多模态LLM是否在处理语义等价但格式不同的证据时表现一致？这个关键问题尚未被系统研究
