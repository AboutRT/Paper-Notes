...
1. **独立压缩丢失关联**：传统方法对每个模态（如视觉、音频）独立压缩，虽然保留了单模态统计特性，但**破坏了跨模态的语义对应关系**。例如，独立压缩后的合成图像和合成音频可能不再语义匹配。
...
3. **分布匹配度量局限**：广泛使用的MMD（最大均值差异）依赖核函数选择（通常是启发式的高斯核），可能无法捕获分布间的所有统计差异。
