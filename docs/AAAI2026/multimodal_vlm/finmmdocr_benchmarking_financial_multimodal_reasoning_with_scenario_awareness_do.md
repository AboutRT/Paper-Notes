...
1. **缺乏真实金融情景**：传统基准（如FinQA、TAT-QA）仅提取明确陈述的信息，忽略了金融分析师需要根据市场环境做出假设和判断的能力
2. **多模态文档理解不足**：部分基准仅使用纯文本输入，多模态基准中的图表和表格过于稀疏孤立，长文档基准又缺乏领域特定数值推理题
3. **忽视精确多步计算**：金融决策需要精确的数值计算，但现有基准忽略单位、百分比和小数问题，或允许过大的误差容限（如1%）
