...
1. **正则化方法**（EWC, MAS）：通过保护关键参数来保留知识，但在多模态架构中难以处理模态冲突，且对细粒度交互建模不足

2. **回放方法**（ER, DER）：通过存储样本的回放来缓解遗忘，但受限于存储开销和隐私约束
...
4. **共性不足**：缺乏对多模态特征质量的关注（噪声视觉区域干扰推理）；记忆管理效率低，未能平衡保留、适应和泛化。
