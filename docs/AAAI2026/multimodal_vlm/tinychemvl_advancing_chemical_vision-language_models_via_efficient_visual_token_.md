...
### 核心痛点
1. **视觉token冗余严重**：分子图像中，所有结构信息集中在分子图形区域，而大量背景区域是无意义的。例如ChemVLM中，一张800×800的图像需要1280个视觉token，约为文本问题token数的100倍，极大增加了计算开销。此外，将分子图像切割为patch的过程还会破坏重要的分子结构信息。
2. **任务范围狭窄**：现有化学VLM主要关注分子级别任务（如SMILES OCR、性质预测），忽视了反应级别任务。反应级别任务（如反应预测）需要模型同时具备识别和推理能力，是更具挑战性的方向。
3. **分子图像生成瓶颈**：ChemMLLM提出的VQ-GAN方案生成分子图像后，需要外部工具将图像解析回SMILES，流程复杂且架构不匹配。
