...
1. **感知冗余**：现有 VLA 框架采用通用的**指令无关视觉编码器**（ViT, CLIP, SigLIP, DINOv2），均匀处理所有像素，背景杂乱、任务无关的干扰和环境噪声被无差别编码，导致计算成本过高且对任务关键线索的注意力被稀释。

2. **指令-视觉语义对齐的表面化**：现有模型主要依靠 LLM 进行通用的跨模态对齐，这种浅层对齐无法捕捉机器人操控中的复杂语义关系，限制了识别全局动作线索、局部语义锚点以及结构化指令-空间依赖的能力。
