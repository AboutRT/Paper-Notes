...
1. **假设漂移间隔固定且已知**：实际中漂移是不可预测的
2. **假设每个环境的完整数据立即可用**：实际中数据是逐步到达的
3. **缺乏可靠的漂移检测机制**：可能导致过拟合过时分布或忽略循环模式

### 现有漂移检测方法的局限

流数据挖掘领域的漂移检测方法（DDM、ADWIN等）主要面向分类任务：

1. **假设离散标签或有界输出**：不适用于SDDO中无界实值域
2. **关注预测错误率的突变**：忽略了优化景观中的渐进性能退化
3. **基于阈值的方法缺乏泛化性**：手工规则难以适应不同场景
