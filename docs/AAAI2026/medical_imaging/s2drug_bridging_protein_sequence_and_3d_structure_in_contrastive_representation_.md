...
1. **领域现状**：虚拟筛选（VS）是药物发现的核心步骤，目标是从海量化合物库中找到与靶蛋白口袋结合的小分子。现有方法分为分子对接（如 AutoDock Vina，精确但慢）和深度学习方法（如 DrugCLIP/DrugHash，用对比学习对齐蛋白-配体表示）。

2. **现有方法的关键盲区 — 蛋白质序列被忽视**：
...
4. **切入角度**：两阶段学习——序列预训练解决数据规模与泛化问题，结构融合微调解决空间精度问题。
