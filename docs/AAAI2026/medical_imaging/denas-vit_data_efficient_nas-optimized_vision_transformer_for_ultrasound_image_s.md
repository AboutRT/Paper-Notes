...
1. **领域现状**：超声图像分割对心脏疾病诊断至关重要，深度学习方法（UNet++、TransUNet、EfficientViT）取得进展但依赖手动设计架构。NAS 可自动优化架构，但现有 NAS 方法仅在模块级搜索（选择卷积或Transformer块），忽略了模块内部更细粒度的操作。
...
3. **核心矛盾**：NAS 需要大量数据来搜索最优架构，但超声分割恰好缺少标注数据——两者对数据需求的冲突如何解决？

4. **本文要解决什么？** 设计高效的token级NAS + 数据高效的半监督学习框架。

5. **切入角度**：在 ViT 的注意力计算之前做 NAS——搜索最优的多尺度 token 表示（而非搜索整个模块），同时用 NAS 引导的半监督学习减少对标注数据的依赖。

6. **核心 idea 一句话**：ViT 内部的 Token 级 NAS 搜索 + NAS 约束驱动的半监督协同训练。
