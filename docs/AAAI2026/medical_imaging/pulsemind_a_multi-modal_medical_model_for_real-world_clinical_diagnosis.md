...
1. **训练数据的局限性**：大多数医学数据集要么只做 VQA（单轮问答），要么只包含单一影像模态，缺乏真实临床场景中的多来源异构输入和多轮医患对话。

2. **评估基准的不足**：现有医学多模态基准无法反映真实临床的复杂性——实际诊断过程中，医生需要主动追问缺失信息、整合化验报告/影像/病历等多种数据源，并在多轮交互中保持上下文一致性。

3. **优化方法的局限**：常见的强化学习方法（如 GRPO）使用绝对分数作为奖励信号，但在临床对话场景中：(a) 模型评分不稳定且主观性强；(b) 绝对分数往往难以区分顶级模型间的微小差异。
