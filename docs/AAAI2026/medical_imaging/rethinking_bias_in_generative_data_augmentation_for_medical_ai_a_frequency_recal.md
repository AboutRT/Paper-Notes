...
1. **通用 AI 领域已发现偏差**：语言模型在反复训练生成内容后性能崩溃；视觉模型使用 GDA 后无法持续获得一致提升。
2. **医学领域尚未重视**：虽然许多研究报告了 GDA 的正面效果，但 AI 合成样本是否一致有益或可能引入有害特征，仍不清楚。
3. **频率差异是关键线索**：近期研究发现 AI 生成图像与真实图像在高频成分上存在显著差异。医学图像（MRI、X 光等）因成像特性和对细微病理细节的依赖，对高频变化特别敏感。
