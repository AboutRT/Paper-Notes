...
**[A Principle-Driven Adaptive Policy For Group Cognitive Stimu](a_principle-driven_adaptive_policy_for_group_cognitive_stimu.md)**
...
**[Advancing Safe Mechanical Ventilation Using Offline Rl With ](advancing_safe_mechanical_ventilation_using_offline_rl_with_.md)**
...
**[Ambiguity-Aware Truncated Flow Matching For Ambiguous Medica](ambiguity-aware_truncated_flow_matching_for_ambiguous_medica.md)**

:   提出 ATFM 框架，通过数据层级推理范式将预测精度和多样性解耦到分布级和样本级分别优化，结合高斯截断表示（GTR）和分割流匹配（SFM）两个模块，在模糊医学图像分割任务中同时提升预测的精度、保真度和多样性。

**[Apo2Mol 3D Molecule Generation Via Dynamic Pocket-Aware Diff](apo2mol_3d_molecule_generation_via_dynamic_pocket-aware_diff.md)**
...
**[Bayesian Meta-Analyses Could Be More A Case Study In Trial Of Labor After A Cesa](bayesian_meta-analyses_could_be_more_a_case_study_in_trial_of_labor_after_a_cesa.md)**
...
**[Bidirectional Channel-Selective Semantic Interaction For Semi-Supervised Medical](bidirectional_channel-selective_semantic_interaction_for_semi-supervised_medical.md)**

:   提出 BCSI 框架，通过通道选择路由器动态筛选关键特征通道，在标注和未标注数据流之间进行双向通道级交互，结合语义-空间扰动的弱到强一致性学习，显著提升半监督医学图像分割性能。

**[Bridging Vision And Language For Robust Context-Aware Surgical Point Tracking Th](bridging_vision_and_language_for_robust_context-aware_surgical_point_tracking_th.md)**

:   提出首个大规模多模态手术点追踪数据集 VL-SurgPT，结合视觉坐标与文本状态描述，并设计文本引导追踪方法 TG-SurgPT，通过语义信息显著提升复杂手术场景下的追踪精度和鲁棒性。

**[Cd-Dpe Dual-Prompt Expert Network Based On Convolutional Dictionary Feature Deco](cd-dpe_dual-prompt_expert_network_based_on_convolutional_dictionary_feature_deco.md)**

:   提出 CD-DPE 网络，通过迭代卷积字典特征解耦模块（CD-FDM）将多对比度 MRI 特征分离为跨对比度共有和模态特有成分，再利用双提示特征融合专家模块（DP-FFEM）进行自适应融合重建，在多个公开数据集上超越现有 SOTA 方法。

**[Clicare Grounding Large Language Models In Clinical Guidelines For Decision Supp](clicare_grounding_large_language_models_in_clinical_guidelines_for_decision_supp.md)**
...
**[Cocolit Controlnet-Conditioned Latent Image Translation For Mri To Amyloid Pet S](cocolit_controlnet-conditioned_latent_image_translation_for_mri_to_amyloid_pet_s.md)**
...
**[Cross-Sample Augmented Test-Time Adaptation For Personalized Intraoperative Hypo](cross-sample_augmented_test-time_adaptation_for_personalized_intraoperative_hypo.md)**

:   提出 CSA-TTA 框架，通过跨样本库构建、粗到细检索和多任务优化，在测试时从其他患者数据中检索低血压事件信号来增强个性化术中低血压预测。

**[Decoding With Structured Awareness Integrating Directional Frequency-Spatial And](decoding_with_structured_awareness_integrating_directional_frequency-spatial_and.md)**

:   提出面向医学图像分割的新型解码器框架，包含三个模块：方向感知的自适应交叉融合注意力（ACFA）、空间-频率-小波三分支融合注意力（TFFA）和结构感知多尺度掩码模块（SMMM），在多个基准数据集上超越现有方法。

**[Deepgb-Tb A Risk-Balanced Cross-Attention Gradient-Boosted Convolutional Network](deepgb-tb_a_risk-balanced_cross-attention_gradient-boosted_convolutional_network.md)**
...
**[Denas-Vit Data Efficient Nas-Optimized Vision Transformer For Ultrasound Image S](denas-vit_data_efficient_nas-optimized_vision_transformer_for_ultrasound_image_s.md)**

:   提出 DeNAS-ViT，首次将 NAS 应用于 ViT 的 Token 级搜索实现超声图像分割的多尺度特征提取优化，并设计基于 NAS 约束的半监督学习框架（网络独立性损失+层次对比损失+阶段式优化），在有限标注数据下达到 SOTA。

**[Dia-Gnostic Vlvae Disentangled Alignment-Constrained Vision Language Variational](dia-gnostic_vlvae_disentangled_alignment-constrained_vision_language_variational.md)**
...
**[Divide Conquer And Unite Hierarchical Style-Recalibrated Prototype Alignment For](divide_conquer_and_unite_hierarchical_style-recalibrated_prototype_alignment_for.md)**
...
**[Do Large Language Models Think Like The Brain Sentence-Level Evidences From Laye](do_large_language_models_think_like_the_brain_sentence-level_evidences_from_laye.md)**

:   本文通过对比14个公开LLM的逐层表示与人类被试听自然叙事时的fMRI数据，在句子级别系统地研究了LLM与人脑语言处理的对齐程度，发现中间层最对齐、指令微调显著增强对齐、且存在与经典神经语言学理论一致的半球偏侧化模式。

**[Dual-Path Knowledge-Augmented Contrastive Alignment Network For Spatially Resolv](dual-path_knowledge-augmented_contrastive_alignment_network_for_spatially_resolv.md)**
...
**[Dualfete Revisiting Teacher-Student Interactions From A Feedback Perspective For](dualfete_revisiting_teacher-student_interactions_from_a_feedback_perspective_for.md)**
...
**[Error Correction In Radiology Reports A Knowledge Distillation-Based Multi-Stage](error_correction_in_radiology_reports_a_knowledge_distillation-based_multi-stage.md)**
...
**[Experience With Single Domain Generalization In Real World Medical Imaging Deplo](experience_with_single_domain_generalization_in_real_world_medical_imaging_deplo.md)**
...
**[Fdp A Frequency-Decomposition Preprocessing Pipeline For Unsupervised Anomaly De](fdp_a_frequency-decomposition_preprocessing_pipeline_for_unsupervised_anomaly_de.md)**
...
**[Fia-Edit Frequency-Interactive Attention For Efficient And High-Fidelity Inversi](fia-edit_frequency-interactive_attention_for_efficient_and_high-fidelity_inversi.md)**
...
**[Fine-Tuned Llms Know They Dont Know A Parameter-Efficient Approach To Recovering](fine-tuned_llms_know_they_dont_know_a_parameter-efficient_approach_to_recovering.md)**
...
**[Funkan Functional Kolmogorov-Arnold Network For Medical Image Enhancement And Se](funkan_functional_kolmogorov-arnold_network_for_medical_image_enhancement_and_se.md)**
...
**[G2Lfrom Giga-Scale To Cancer-Specific Large-Scale Pathology Foundation Models Vi](g2lfrom_giga-scale_to_cancer-specific_large-scale_pathology_foundation_models_vi.md)**
...
**[Gp-Molformer-Sim Test Time Molecular Optimization Through Contextual Similarity ](gp-molformer-sim_test_time_molecular_optimization_through_contextual_similarity_.md)**
...
**[Grover Graph-Guided Representation Of Omics And Vision With Expert Regulation Fo](grover_graph-guided_representation_of_omics_and_vision_with_expert_regulation_fo.md)**
...
**[Guidegen A Text-Guided Framework For Paired Full-Torso Anatomy And Ct Volume Gen](guidegen_a_text-guided_framework_for_paired_full-torso_anatomy_and_ct_volume_gen.md)**
...
**[Hifusion Hierarchical Intra-Spot Alignment And Regional Context Fusion For Spati](hifusion_hierarchical_intra-spot_alignment_and_regional_context_fusion_for_spati.md)**
...
**[Intervention Efficiency And Perturbation Validation Framework Capacity-Aware And](intervention_efficiency_and_perturbation_validation_framework_capacity-aware_and.md)**
...
**[Maisi-V2 Accelerated 3D High-Resolution Medical Image Synthesis With Rectified F](maisi-v2_accelerated_3d_high-resolution_medical_image_synthesis_with_rectified_f.md)**

:   提出 MAISI-v2，首个将 Rectified Flow 引入 3D 医学图像合成的框架，通过替换 DDPM 实现 33 倍加速，并设计区域特异性对比损失增强对肿瘤等小区域条件的忠实度，在下游肿瘤分割任务中验证了合成数据的增强价值。

**[Mapi-Gnn Multi-Activation Plane Interaction Graph Neural Network For Multimodal ](mapi-gnn_multi-activation_plane_interaction_graph_neural_network_for_multimodal_.md)**

:   提出 MAPI-GNN，通过多维特征判别器在语义子空间中动态构建多个激活图，再经层次化融合网络聚合样本内和样本间关系，在前列腺癌和冠心病两个多模态诊断任务上显著超越现有 SOTA（PI-CAI 上 ACC 0.9432，AUC 0.9838）。

**[Measuring Stability Beyond Accuracy In Small Open-Source Medical Large Language ](measuring_stability_beyond_accuracy_in_small_open-source_medical_large_language_.md)**
...
**[Mindcross Fast New Subject Adaptation With Limited Data For Cross-Subject Video ](mindcross_fast_new_subject_adaptation_with_limited_data_for_cross-subject_video_.md)**
...
**[Mirage Scaling Test-Time Inference With Parallel Graph-Retrieval-Augmented Reaso](mirage_scaling_test-time_inference_with_parallel_graph-retrieval-augmented_reaso.md)**

:   提出MIRAGE框架，将传统的线性推理链扩展为并行多链推理范式，结合结构化医学知识图谱的自适应检索（邻域扩展和多跳遍历），通过跨链验证解决矛盾，在三个医学QA基准上持续优于GPT-4o、ToT和Search-o1等方法。

**[Mirnet Integrating Constrained Graph-Based Reasoning With Pre-Training For Diagn](mirnet_integrating_constrained_graph-based_reasoning_with_pre-training_for_diagn.md)**
...
**[Note2Chat Improving Llms For Multi-Turn Clinical History Taking Using Medical No](note2chat_improving_llms_for_multi-turn_clinical_history_taking_using_medical_no.md)**
...
**[Nutriscreener Retrieval-Augmented Multi-Pose Graph Attention Network For Malnour](nutriscreener_retrieval-augmented_multi-pose_graph_attention_network_for_malnour.md)**
...
**[Pairing-Free Group-Level Knowledge Distillation For Robust Gastrointestinal Lesi](pairing-free_group-level_knowledge_distillation_for_robust_gastrointestinal_lesi.md)**
...
**[Panfoma A Lightweight Foundation Model And Benchmark For Pan-Cancer](panfoma_a_lightweight_foundation_model_and_benchmark_for_pan-cancer.md)**
...
**[Pings-X Physics-Informed Normalized Gaussian Splatting With Axes Alignment For E](pings-x_physics-informed_normalized_gaussian_splatting_with_axes_alignment_for_e.md)**
...
**[Priorrg Prior-Guided Contrastive Pre-Training And Coarse-To-Fine Decoding For Ch](priorrg_prior-guided_contrastive_pre-training_and_coarse-to-fine_decoding_for_ch.md)**
...
**[Propl Universal Semi-Supervised Ultrasound Image Segmentation Via Prompt-Guided ](propl_universal_semi-supervised_ultrasound_image_segmentation_via_prompt-guided_.md)**

:   提出 ProPL 框架，通过共享视觉编码器 + 提示引导双解码器 + 不确定性驱动伪标签校准，首次实现通用半监督超声图像分割，在 5 个器官 8 个任务上以极少标注数据（1/16）超越全监督方法 5.18% mDice。

**[Protsae Disentangling And Interpreting Protein Language Models Via Semantically-](protsae_disentangling_and_interpreting_protein_language_models_via_semantically-.md)**
...
**[Q-Fsru Quantum-Augmented Frequency-Spectral Fusion For Medical Visual Question A](q-fsru_quantum-augmented_frequency-spectral_fusion_for_medical_visual_question_a.md)**
...
**[Radiation-Preserving Selective Imaging For Pediatric Hip Dysplasia A Cross-Modal](radiation-preserving_selective_imaging_for_pediatric_hip_dysplasia_a_cross-modal.md)**

:   提出一种"超声优先、保辐射"的跨模态选择性成像策略，通过自监督预训练的冻结编码器、测量忠实的轻量头网络和共形预测校准的单侧下界，实现了在发育性髋关节发育不良（DDH）诊断中有据可依地决定何时仅用超声即可、何时需要额外的 X 光检查。

**[Recon-Ipsundrum An Inspectable Recurrent Persistence Loop Agent With Affect-Coup](recon-ipsundrum_an_inspectable_recurrent_persistence_loop_agent_with_affect-coup.md)**
...
**[Rethinking Bias In Generative Data Augmentation For Medical Ai A Frequency Recal](rethinking_bias_in_generative_data_augmentation_for_medical_ai_a_frequency_recal.md)**
...
**[Rethinking Surgical Smoke A Smoke-Type-Aware Laparoscopic Video Desmoking Method](rethinking_surgical_smoke_a_smoke-type-aware_laparoscopic_video_desmoking_method.md)**
...
**[S2Drug Bridging Protein Sequence And 3D Structure In Contrastive Representation ](s2drug_bridging_protein_sequence_and_3d_structure_in_contrastive_representation_.md)**
...
**[Spacrd Multimodal Deep Fusion Of Histology And Spatial Transcriptomics For Cance](spacrd_multimodal_deep_fusion_of_histology_and_spatial_transcriptomics_for_cance.md)**
...
**[Trinitydna A Bio-Inspired Foundational Model For Efficient Long-Sequence Dna Mod](trinitydna_a_bio-inspired_foundational_model_for_efficient_long-sequence_dna_mod.md)**
