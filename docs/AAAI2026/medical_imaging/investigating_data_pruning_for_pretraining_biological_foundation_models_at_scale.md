...
1. **领域现状**：生物基础模型（BioFMs）如 RNA-FM（23M RNA序列）和 ESM（28亿蛋白质序列）在结构预测、功能注释等任务上表现出色，但训练成本极高（RNA-FM：8×A100 训练 30天），严重限制了学术实验室的可复现性和可及性。
...
3. **核心问题**：能否在不访问完整训练过程的前提下，通过后验（post-hoc）方法找到信息量最大的训练子集？

4. **切入角度**：利用影响函数理论，在小子集上近似全训练集的曲率信息，高效估计每个样本的重要性。
