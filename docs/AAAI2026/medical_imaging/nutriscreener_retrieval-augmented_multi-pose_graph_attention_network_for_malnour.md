...
1. **领域现状**：截至2024年，全球约1.5亿5岁以下儿童发育迟缓（stunting），超过4200万患有消瘦（wasting），营养不良是儿童不可逆发育损害和死亡的主要原因。低资源地区尤其缺乏及时的筛查能力。
...
3. **核心矛盾**：低资源场景需要低成本、可扩展的筛查方案，但已有AI方法要么依赖专用硬件，要么在少数类检测上表现差，无法满足实际部署需求。

4. **本文要解决什么？** 从普通手机拍摄的多姿态2D图像中，同时实现：(1) 二分类营养状态判断；(2) 身高、体重、MUAC、头围四项人体测量回归预测。

5. **切入角度**：将每个受试者建模为图（节点=各姿态CLIP嵌入），用GAT建模姿态间关系，再用检索增强模块从知识库中找相似样本来补偿少数类偏差。

6. **核心 idea 一句话**：多姿态CLIP嵌入 + GAT跨姿态推理 + 类别增强FAISS检索 + 上下文感知自适应融合。
