...
**领域**: 小样本学习 / 多模态学习  
**关键词**: Few-Shot Learning, 多模态原型, LLM语义增强, 数据增强, CLIP
...
1. **LLM-based Multi-Variant Semantic Enhancement (LMSE)**:
   - 做什么：利用大语言模型为每个类别生成多变体语义描述，丰富支持集的语义信息。
...
2. **Hierarchical Multi-View Augmentation (HMA)**:
   - 做什么：通过层次化的数据增强策略生成多视角特征，增强支持集的视觉多样性。
...
3. **Adaptive Uncertain Class Absorber (AUCA)**:
   - 做什么：动态构造不确定类，吸收位于类别决策边界附近的不确定样本，减轻类间干扰。
