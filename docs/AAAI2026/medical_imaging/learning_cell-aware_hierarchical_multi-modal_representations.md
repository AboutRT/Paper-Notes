...
**本文要解决什么？** (1) 如何在模态高度缺失的情况下鲁棒地学习分子表征？(2) 如何显式建模分子、细胞、基因三个层级之间的层次依赖？

**切入角度**：作者观察到分子之间存在结构相似性，相似分子的生物响应也相似，可以通过图传播来增强缺失模态；同时生物响应天然具有从分子→细胞→基因的层次结构，可以用树状结构显式编码。

**核心idea一句话**：用结构感知图传播填补缺失生物模态，用树状向量量化编码跨尺度层次语义，实现面向缺失模态的鲁棒分子表征学习。
...
1. **模态增强与语义一致性对齐(MA + SCA)**:
    ...
    - 做什么：为缺失的外部生物模态生成合理的伪特征，并确保增强后的特征与原始特征在语义上一致
...
2. **树状向量量化(Tree-VQ)**:
    ...
    - 做什么：将多模态特征映射到一棵共享的二叉树上，用树的层级编码生物层次（浅层对应分子指纹，深层对应细胞表型和基因表达）
...
3. **上下文传播重建(CPR)**:
    ...
    - 做什么：利用生物先验知识图上的随机游走提供跨模态上下文监督信号
