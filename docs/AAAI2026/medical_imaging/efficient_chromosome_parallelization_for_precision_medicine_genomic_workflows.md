...
1. **染色体大小差异巨大**：人类1号染色体比22号染色体大约4倍（Fig. 1），导致简单的一任务一核心分配会产生严重的负载不均衡
2. **内存消耗不可预测**：每个染色体处理任务的RAM需求难以精确预估，静态资源分配容易导致内存溢出（OOM）错误
3. **资源利用率低**：保守估计会导致过度分配，浪费计算资源；激进估计则导致任务失败和重跑
