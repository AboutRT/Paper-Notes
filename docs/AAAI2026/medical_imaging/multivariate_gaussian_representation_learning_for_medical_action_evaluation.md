---
description: "【论文笔记】Multivariate Gaussian Representation Learning for Medical Action Evaluation 论文解读 | AAAI 2026 | arXiv 2511.10060 | CPR评估 | 提出 GaussMedAct 框架，将关节运动轨迹建模为多元高斯混合分布并结合笛卡尔-向量双流编码，在自建的 CPREval-6k 数据集上实现 92.1% Top-1 准确率，仅需 ST-GCN 10% 的计算量。"
tags:
  - AAAI 2026
  - CPR评估
  - 高斯混合模型
...
1. **领域现状**：心肺复苏（CPR）质量直接影响心脏骤停存活率。人工评估仅 74.8% 准确率，现有视觉系统难以捕捉厘米级运动偏差和毫秒级频率变化。
2. **现有痛点**：
   - RGB 方法（如 TimeSformer）计算量大、缺乏解剖学建模
   - 骨架方法（如 ST-GCN）通过刚性时序池化丢弃运动语义，对噪声敏感
   - 缺少合适的 CPR 评估数据集——现有数据集规模小、标注粗粒度
3. **切入角度**：受 3D Gaussian Splatting 用少量高斯分布高效表示稠密点云的启发，将关节运动轨迹视为时空点集，用高斯混合模型进行紧凑且抗噪的表示
...
1. **多元高斯表示 (MGR)**:
   - 做什么：将每个关节的时空轨迹 $\mathcal{X}_i = \{(x, y, \alpha \cdot t)\}$ 建模为 $K$ 个高斯分布的混合，用 EM 算法估计参数
...
2. **混合空间编码 (HSE)**:
   - 做什么：笛卡尔坐标流捕捉全局轨迹一致性，骨骼向量流编码运动转变
