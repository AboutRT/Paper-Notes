---
description: "【论文笔记】Steering One-Step Diffusion Model with Fidelity-Rich Decoder for Fast Image Compression 论文解读 | AAAI 2026 | arXiv 2508.04979 | 图像压缩 | 提出 SODEC，一种基于单步扩散的图像压缩模型，通过保真度引导模块(FGM)将高保真VAE解码器的先验注入扩散生成过程，结合速率退火训练策略实现极低码率下的高质量压缩，解码速度比多步扩散方法快20×以上，同时在率-失真-感知权衡上达到SOTA。"
tags:
  - AAAI 2026
  - 图像压缩
  - 扩散模型
...
1. **高延迟**：多步去噪过程导致巨大的解码延时和计算成本（PerCo耗时6.2秒，DiffEIC耗时7.8秒），不适合实时或资源受限场景
2. **低保真度**：扩散模型严重依赖预训练先验而非输入本身，重建偏离原始内容
...
三个核心设计：
1. **单步解码**：信息丰富的latent + 单步扩散 = 高质量重建（无需多步）
2. **保真度引导模块(FGM)**：用VAE解码器生成高保真初步重建，作为视觉引导条件
3. **速率退火训练策略**：先高码率预训练学习丰富表征，再逐渐退火到目标低码率
