---
description: "【论文笔记】Continuous Degradation Modeling via Latent Flow Matching for Real-World Super-Resolution 论文解读 | AAAI 2026 | arXiv 2602.04193 | 真实世界超分辨率 | 提出 DegFlow，通过残差自编码器 + 潜空间 Flow Matching 从离散尺度的真实 HR-LR 对学习连续退化轨迹，仅需单张 HR 图像即可合成任意连续尺度的逼真 LR 图像，用于训练超分模型达到 SOTA。"
tags:
  - AAAI 2026
  - 真实世界超分辨率
  - 退化建模
...
1. **手工退化管线**（Real-ESRGAN、BSRGAN）：用模糊核+噪声+下采样+压缩伪影组合增强训练数据，提升了鲁棒性但仍无法表示真实退化的复杂特性。
2. **物理设备采集**（RealSR、DRealSR、RealArbiSR）：用 DSLR 相机变焦镜头采集配对数据，退化真实但采集成本高、场景多样性有限。
3. **学习退化模型**（DeFlow、RealDGen）：从少量真实对中学习退化过程并合成 LR 图像，但缺乏**显式尺度控制**，无法生成任意连续尺度的退化。
4. **InterFlow**：通过在 LR 潜空间插值实现连续尺度合成，但推理时需要两个不同尺度的 LR 图像作为输入，实用性受限。
