...
1. **自回归模型 (ARM)**：序列建模能力强、准确率高，但逐字符解码效率低
2. **并行解码模型 (PDM)**：速度快但缺乏上下文建模，准确率低于 ARM
3. **BERT-like 精炼模型 (ReM)**：先并行预测再双向精炼，但对初始预测错误敏感
4. **掩码扩散模型 (MDM)**：新兴范式，通过从部分遮掩输入恢复原始序列来学习全向依赖

**MDM 的潜力**：与 ARM 的单向建模不同，MDM 可以捕获更灵活、更全面的全向上下文依赖，这对需要语言理解的 STR 任务至关重要。同时其去噪过程高效可控，仅需几步即可得到准确预测。

**MDM 直接应用于 STR 的两个关键问题**：

1. **训练-推理噪声差距（Noising Gap）**：训练时使用随机掩码，但推理时从全掩码开始，后续的 remask 模式也与训练不匹配，导致泛化性差
2. **推理时过度自信（Overconfident Predictions）**：MDM 倾向于给错误预测分配过高的置信分数（如将错误字符"F"的置信度报为 0.95），导致基于置信度的 remask 机制失效，错误无法在后续步骤中被纠正
