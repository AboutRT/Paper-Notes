---
description: "【论文笔记】RetrySQL: Text-to-SQL Training with Retry Data for Self-Correcting Query Generation 论文解读 | AAAI 2026 | arXiv 2507.02529 | Text-to-SQL | 提出 RetrySQL 训练范式，通过在推理步骤中注入 retry data（错误步骤 + [BACK] 标记 + 正确步骤）来持续预训练小型编码模型，使 1.5B 参数的开源模型学会自纠正能力，在 BIRD 和 SPIDER 基准上分别提升整体执行准确率最高 4 和 3.93 个百分点，挑战性样例提升高达 9 个百分点。"
tags:
  - AAAI 2026
  - Text-to-SQL
  - 自纠正
...
1. **小模型潜力未被挖掘**：1.5B 参数的编码模型（如 OpenCoder、Qwen2.5-Coder）在零样本下表现很差（EX_overall ~30-37%），但训练效率高、推理成本低，适合实际部署
2. **自纠正能力未被应用于 Text-to-SQL**：虽然 DeepSeek-R1 等已证明 RL 可以学习自纠正，更近期研究表明标准自回归训练配合 retry data 就能学到自纠正（数学推理任务），但这一方法**尚未被应用于 Text-to-SQL 领域**
3. **现有纠正都是后处理**：如 DIN-SQL 的纠正步骤在生成之后进行，而非在生成过程中自纠正
