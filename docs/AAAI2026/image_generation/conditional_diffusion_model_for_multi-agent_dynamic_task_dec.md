...
description: >-
  [AAAI 2026][图像生成][任务分解] 提出 CD3T，一个两层层次化 MARL 框架：用条件扩散模型学习动作语义表示（以观测和他人动作为条件，预测下一观测和奖励），通过 k-means 聚类得到子任务划分，高层选择子任务、低层在受限动作空间执行策略…
tags:
  - "AAAI 2026"
  - "图像生成"
  - "任务分解"
...
**核心 idea**：用条件扩散模型作为灵活的特征提取器，学习捕捉动作对环境影响的语义表示，聚类得到子任务，再用子任务表示增强值分解中的信用分配。
...
1. **条件扩散模型学动作表示**:
    ...
    - 功能：将 Agent $i$ 的 one-hot 动作 $a_i$ 编码为 $d$ 维表示 $z_a^i$，条件为局部观测 $o_i$ 和其他 Agent 的动作 $a_{-i}$
...
2. **子任务动态分解**:
    ...
    - 功能：将学到的动作表示聚类为 $g$ 个子任务，每个子任务对应一个受限的动作空间
...
3. **子任务感知的值分解（Subtask-based Credit Assignment）**:
    ...
    - 功能：在 mixing network 中引入子任务/动作表示，增强信用分配
