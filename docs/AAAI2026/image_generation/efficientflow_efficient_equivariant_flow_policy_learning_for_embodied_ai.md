---
description: "【论文笔记】EfficientFlow: Efficient Equivariant Flow Policy Learning for Embodied AI 论文解读 | AAAI 2026 | arXiv 2512.02020 | Flow Matching | 提出 EfficientFlow，将等变性引入 Flow Matching 策略学习框架，理论证明各向同性先验+等变速度网络保证动作分布等变，并提出 Flow Acceleration Upper Bound (FABO) 正则化加速采样，在 MimicGen 12 个任务上实现比 EquiDiff 快 20-56 倍的推理速度且性能更优。"
tags:
  - AAAI 2026
  - Flow Matching
  - 等变性
...
1. **领域现状**：扩散模型策略（Diffusion Policy）在机器人操作中表现出色但有两大瓶颈：数据效率低（需大量演示）和采样效率低（需数百步去噪）。EquiDiff 通过等变性提升数据效率，但仍基于 DDPM，推理很慢。

2. **现有痛点**：
   - 扩散策略需要 100+ 步去噪才能生成一个动作序列
   - Flow Policy 虽然更快但现有版本未考虑等变性
   - 等变性+Flow Matching 的理论关系未被建立

3. **核心矛盾**：如何同时实现数据高效（等变性）和采样高效（Flow Matching + 加速）？

4. **切入角度**：(1) 理论证明等变性在 Flow Matching 中可以自然保持 (2) 提出 FABO 正则化使流轨迹更直，减少所需积分步数。

5. **核心 idea 一句话**：等变 Flow Matching + FABO 加速正则化，统一数据效率和采样效率。
