...
1. **上下文偏差不足**：时间序列的过去与未来常存在分布偏移（distribution drift），纯似然优化的方法（如TimeGrad、CSDI）无法追踪这种漂移，导致训练不稳定、采样方差爆炸——"多样性过高以至于无用"
2. **多样性-精度权衡**：TimeDiff和mr-Diff通过预训练回归器提供初始轨迹来稳定训练，但这固化了扩散过程、限制了生成灵活性，本质上退化为回归模型
