...
1. **扩散模型推理慢**：多步去噪过程导致推理延迟高达 ~130ms，难以满足实时控制需求（如力控操作、高频反馈回路）。
2. **流匹配方法受限于一致性约束**：FlowPolicy 等方法通过强制一致性损失实现 1-NFE 推理，但这引入了额外的结构假设，可能限制策略的表达能力。
3. **ODE 求解器误差**：传统 Flow Matching 在推理时需要积分瞬时速度场，数值 ODE 求解器引入的误差会降低轨迹精度。
...
**做什么**：学习区间平均速度场替代瞬时速度场，实现无 ODE 求解器的单步动作生成。
...
**做什么**：在 MeanFlow 框架中引入 CFG 增强条件控制能力。
...
**做什么**：训练时正则化策略网络的中间特征空间，防止不同状态的特征坍缩，提升泛化能力。
