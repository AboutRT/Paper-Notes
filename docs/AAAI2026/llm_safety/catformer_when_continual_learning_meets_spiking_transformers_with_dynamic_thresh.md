...
1. **正则化方法**（EWC、SI）：通过约束重要参数的更新来防遗忘，但在 SNN 上效果很差（CIFAR-100 50 tasks 下 SI 仅 1.84%）
2. **重放方法**（iCaRL、DER++）：需要存储过去数据，违反隐私约束，且在硬件受限平台上不可行
3. **架构方法**（DSD-SNN）：随任务增多性能持续下降，从 10 tasks 60.47% 降至 50 tasks 50.55%
4. **SNN 持续学习**的研究主要基于 CNN 架构，尚未有人探索脉冲 Vision Transformer 的持续学习
