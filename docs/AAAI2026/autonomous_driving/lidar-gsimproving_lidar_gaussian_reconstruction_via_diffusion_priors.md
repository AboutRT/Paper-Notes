...
### 核心挑战
1. **摄像头vs LiDAR发展不均衡**：摄像头仿真器已有FreeSim等工作解决外推问题，但LiDAR再仿真领域尚未明确处理外推质量问题，这阻碍了多模态传感器L4级驾驶代理的发展。
2. **生成数据的真实性**：利用跨模态提示（文本、地图、bbox）生成的LiDAR数据存在明显域差距，不适合新视角合成。
3. **生成数据的一致性**：直接将生成扫描与真实扫描混合会导致已收敛区域出现幻觉和矛盾。
