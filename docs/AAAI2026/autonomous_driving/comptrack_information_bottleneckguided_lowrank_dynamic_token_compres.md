...
description: >-
  [AAAI 2026][自动驾驶][点云跟踪] 提出 CompTrack 框架首次同时解决 LiDAR 点云中的双重冗余问题：SFP 通过信息熵分析滤除背景噪声解决空间冗余；IB-DTC 通过在线 SVD 估计有效秩、自适应确定压缩率将前景压缩为低秩代理 token 解决信息冗余…
tags:
  - "AAAI 2026"
  - "自动驾驶"
  - "点云跟踪"
...
**核心 idea**：空间冗余用信息熵导向的前景预测器滤除 + 信息冗余用在线 SVD 估计有效秩后通过学习型查询的交叉注意力压缩。
...
2. **IB-DTC 模块**
    - **功能**：将冗余前景 $\mathbf{X}_{fg} \in \mathbb{R}^{N \times C}$ 压缩为 $\mathbf{X}_{proxy} \in \mathbb{R}^{K \times C}$（$K \ll N$）
