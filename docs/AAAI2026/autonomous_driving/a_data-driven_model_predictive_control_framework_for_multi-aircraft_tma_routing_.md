...
description: >-
  [AAAI 2026][自动驾驶][空中交通管理] 提出基于数据驱动MPC的终端区多飞机无冲突路径规划框架，结合ADS-B数据预测、MILP优化和滚动时域仿真，实现峰值拥堵下7倍计算加速和鲁棒运行。
tags:
  - AAAI 2026
  - 自动驾驶
  - 空中交通管理
...
1. **数据驱动 TMA 边界到达时间预测**
    ...
    - **做什么**：用 XGBoost 预测每架飞机到达 50NM TMA 边界的时间 $T_f^t = \mathcal{F}(X_f^t)$
...
2. **完整 STAR 网络 MILP 优化模型**
    ...
    - **做什么**：在 50NM 半径的真实 STAR 网络上求解多飞机无冲突最优路径
    - **核心约束**：
        - 路径守恒律（类似车辆路径问题的流守恒）
        - 到达/出发时间约束（含离散速度等级选择）
...
3. **滚动时域闭环控制**
    ...
    - **做什么**：将全规划域分割为若干子问题，通过 MPC 与仿真器双向交互实现实时控制
    - **核心思路**：
