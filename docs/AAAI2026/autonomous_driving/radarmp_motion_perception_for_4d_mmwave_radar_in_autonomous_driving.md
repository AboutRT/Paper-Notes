...
1. **检测与运动估计解耦**：现有方法将雷达目标检测和运动估计分为两个独立阶段，检测阶段产生的稀疏噪声点云直接影响后续场景流估计的精度
2. **光学传感器监督的矛盾**：使用 LiDAR/相机来监督雷达检测模型（如 RPDNet），迫使雷达关注一些低反射特征，反而削弱了多模态感知的互补性
3. **雷达场景流研究稀缺**：目前仅有 RaFlow 和 CMFlow 两个工作研究雷达点云场景流，且性能远不如 LiDAR 方法
...
**做什么**：将 tesseract 中的多普勒维度从冗余表示转化为紧凑的运动感知特征。
...
**做什么**：捕捉全局运动上下文，区分噪声的无序运动、静态目标的全局关联运动和动态目标的局部关联运动。
