...
1. **维度间隙 (Dimension Gap)**：输入图像是 2D 平面视角，像素特征因遮挡混淆了多个物体的语义（粗糙像素语义）。SSC 需要 3D 立体视角下细粒度的体素语义，需要展开和解耦粗糙像素特征
2. **密度间隙 (Density Gap)**：LiDAR 传感器的手动标注本质上是稀疏的（有间距），但真实世界场景具有密集的占用和丰富的上下文细节，需要检测缺失体素并修正错误体素
