...
1. **缺乏人在环的感知**：仅强调实例级别的识别，在事件级或整体场景级理解方面不足
2. **可扩展性和可解释性有限**：在复杂环境中灵活识别未预见的物体和事件的能力不足
3. **缺乏语义推理能力**：现有基准仅测量感知准确性，无法评估模型是否理解隐含的交通规则
...
**做什么**：解决 MLLM 中简单 token 拼接导致的视觉-语言交互问题。
...
**做什么**：通过知识蒸馏与认知迁移增强轻量级路侧 MLLM 的推理能力。
