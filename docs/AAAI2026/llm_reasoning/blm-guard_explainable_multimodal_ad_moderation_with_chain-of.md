---
title: >-
  [论文解读] BLM-Guard: Explainable Multimodal Ad Moderation with Chain-of-Thought and Policy-Aligned Rewards
description: >-
  [AAAI 2026][LLM推理][广告审核] 提出 BLM-Guard，一个面向短视频商业广告的可解释多模态审核框架：先通过 Rule-driven ICoT 数据合成 + SFT 冷启动建立结构化推理能力，再用 Self-Adaptive GRPO 强化学习（结合规则正确性奖励 + 自适应一致性奖励 SCA-R）优化策略对齐，在真实广告 benchmark 上达到 91.4% 严格准确率和 0.845 推理一致性分数。
tags:
  - AAAI 2026
  - LLM推理
  - 广告审核
...
**本文目标**：如何构建一个既准确又可解释的多模态广告审核系统？需要：结构化推理 + 策略对齐 + 跨模态一致性检测。

**切入角度**：将广告审核建模为政策对齐的多步推理任务——先观察（模态内容描述）→ 风险筛查 → 因果分析 → 最终判决，用 CoT 结构化这个过程，用 RL 确保与政策的动态对齐。

**核心 idea**：用 Interleaved CoT 做结构化推理冷启动 + 自适应 GRPO 做策略对齐强化学习，同时处理模态内和模态间违规。
...
2. **Interleaved Multi-stage CoT Generation（交替多阶段 CoT）**:
    ...
    - 功能：用冻结的大模型（InternVL-3-78B）合成结构化推理数据
