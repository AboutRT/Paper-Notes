...
description: >-
  [AAAI 2026][LLM推理][数据分析] 构建NbQA数据集（从真实Jupyter Notebook提取3.8万task-solution对）+ 提出Jupiter框架（将数据分析建模为状态级搜索问题，用值模型引导PUCT搜索）…
tags:
  - "AAAI 2026"
  - "LLM推理"
  - "数据分析"
...
**领域现状**：LLM在数据分析任务上的应用日益广泛，但现有方法要么关注孤立的分析阶段，要么依赖专有模型（如GPT-4o）和复杂agent架构（AutoGen、TaskWeaver），开源模型在多步数据分析上表现不佳。
...
**核心矛盾**：开源小模型（7B-14B）在数据分析上远逊于闭源大模型，缺乏高质量训练数据和有效的推理时搜索策略。

**本文目标** (1)从真实Notebook构建高质量训练数据 (2)设计推理时值引导搜索提升多步数据分析准确率。

**切入角度**：观察到Jupyter Notebook天然包含问题-代码-执行结果的多步结构，是理想的数据分析训练来源；同时MCTS+值模型在推理任务上的成功可以迁移到数据分析场景。

**核心 idea**：真实Notebook数据 + 值模型引导的推理时搜索 = 开源小模型匹敌GPT-4o的数据分析能力。
...
1. **NbQA数据集构建Pipeline**:
    ...
    - 功能：从真实Jupyter Notebook中提取标准化的数据分析task-solution对
