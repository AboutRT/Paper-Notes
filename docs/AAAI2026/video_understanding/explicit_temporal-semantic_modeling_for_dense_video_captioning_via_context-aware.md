...
**关键词**: Dense Video Captioning, 跨模态检索, 时序聚类, 特征增强, Retrieval-Augmented Generation
...
1. **时序建模不足**：固定窗口的视觉特征只关注局部片段，导致不连续的语义检索，无法捕获事件序列间的时序连贯性
2. **模态鸿沟**：检索到的语义特征与视觉表示通过简单操作（拼接或基本注意力）融合，不足以桥接视觉和文本模态的内在差异
