...
1. **交互式视频被忽视**：直播是在线视频消费中增长最快的形态（Instagram Live、TikTok Live 等），但没有任何基准专门评估模型对直播视频的理解能力
2. **直播的独特特征未被覆盖**：直播涉及送礼、实时对话、弹幕评论、多人连麦等交互行为，这些与普通视频有本质区别
3. **标注流程不透明**：现有基准要么完全依赖人工标注（高成本），要么完全依赖单一模型自动标注（引入偏差），缺乏设计良好的半自动流程
4. **实时评论处理困难**：直播往往伴随海量弹幕（数据集包含约 145 万条评论），对模型的上下文长度和信息抽取能力构成巨大挑战
