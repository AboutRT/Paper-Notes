...
1. **卡尔曼滤波器的线性假设**：作为标准运动预测器，卡尔曼滤波计算高效但假设线性运动，在 DanceTrack 等非线性运动数据集上表现差。
2. **数据驱动预测器的局限**：DiffMOT（基于扩散模型）、TrackSSM（基于状态空间模型）等方法能捕捉非线性动态，但存在领域过拟合和计算开销问题。
3. **伪二元对立**：学界将卡尔曼滤波和数据驱动方法视为互斥选择，忽视了它们的互补性。
