...
1. **静态单子空间表示**：GrNet、GDLNet 等方法对输入数据仅使用一个固定的正交子空间建模，难以捕捉数据中的局部几何变化和多模态分布，导致表达能力受限
2. **缺乏子空间交互**：欧氏空间中深度学习的成功很大程度归功于多通道交互与非线性激活（如 LeNet-5 的多通道卷积），但 Grassmann 流形上的多子空间交互一直被忽视
