...
现有视频情绪数据集存在以下问题：
1. **规模有限**：如 CAER（12h）、MELD（1.4h）、DEAP（2h）等数据集规模较小
2. **内容单一**：几乎所有数据集都聚焦于真实场景中的人脸表情，缺乏风格化和非写实内容
3. **模态不全**：很多数据集缺少音频或文本描述
4. **不适用于生成任务**：现有数据集主要面向情绪识别，缺乏丰富的视觉多样性
...
1. **多源数据收集**:
   - 做什么：从三种来源收集视频——MagicAnime 数据集（2,807 个动画面部片段）、Condensed Movies（13,255 个电影片段，经 PySceneDetect 切分并保留 4-30 秒片段）、Tenor API（6,696 个 GIF 表情贴纸）
   - 核心思路：覆盖多种艺术风格和内容类型，涵盖美式/中式/日式动画、电影场景、网络表情包
   - 设计动机：确保情绪表达的多样性和跨域通用性

2. **人机协作标注方案**:
   - 做什么：采用 Mikels 八情绪模型（amusement, awe, contentment, excitement, anger, disgust, fear, sadness），结合人工标注和 VLM 自动标注
...
3. **多维度属性标注与分析**:
   - 做什么：为每个片段提取颜色属性（colorfulness, brightness, hue）、用 NVILA-8B 生成文本描述
   - 核心发现：
