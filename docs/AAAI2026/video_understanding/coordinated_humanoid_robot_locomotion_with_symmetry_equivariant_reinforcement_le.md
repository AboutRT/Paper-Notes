...
**代码**: 无  
**领域**: 强化学习 / 机器人控制  
**关键词**: 人形机器人, 对称等变, 深度强化学习, 运动协调, PPO, Sim-to-Real  
...
1. **对称观测产生不对称动作**：对于镜像对称的状态输入（如"左脚悬空"与"右脚悬空"），策略网络可能输出完全不同的动作响应，导致左右关节运动风格不一致。
2. **运动不协调**：例如 DreamWaQ 策略下左右脚的步幅分别为 0.13m 和 0.25m，步态严重不对称，既不自然也降低了跟踪精度。
3. **用户体验差**：不协调的运动模式看起来不自然，且降低了任务性能。
