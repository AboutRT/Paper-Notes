...
现有蒸馏方案的局限：
1. **集成蒸馏（ED）**：将DE蒸馏为输出单一softmax的SNN，但丢失了EU信息（单一分布无法表达关于不确定性的不确定性）
2. **集成分布蒸馏（EDD）**：蒸馏为输出Dirichlet分布的模型，但缺乏ground-truth Dirichlet标签，且Dirichlet模型在实践中准确率严重下降（VGG16上仅74.56% vs SNN的91.79%），近期理论批评其EU解释不合理
