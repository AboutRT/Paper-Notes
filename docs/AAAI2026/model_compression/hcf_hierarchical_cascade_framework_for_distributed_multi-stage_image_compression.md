...
1. **渐进压缩框架（PCF）**：通过比特流截断实现质量适配（"编码一次，解码多次"），但中间节点只能被动截断，**无法利用节点的计算资源**进行主动优化
2. **分布式重压缩框架（DRF）**：在每个节点执行完整的解压-重压缩循环，虽然利用了计算资源，但**在像素域反复编解码导致累积质量退化和计算冗余**
3. **固定参数模型（SSF）**：在集中式设置下性能最优，但只提供单一操作点，**缺乏编码后的灵活性**
