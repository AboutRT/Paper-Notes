...
现有非对称方法的问题：
1. **k-NN 依赖**：CSD、D3still 等方法依赖 k 近邻信息来传递上下文知识，但预计算和存储大规模图库嵌入的 k-NN 信息成本极高——在 VPR 中涉及数百万张图片，k-NN 计算尤为昂贵
2. **容量差距**：轻量查询模型难以完全捕获高容量图库模型能编码的复杂特征分布和变异性
3. **在其他检索领域探索过，但在 VPR 中几乎未被研究**，而 VPR 恰恰有一个独特优势——天然的地理位置元数据
