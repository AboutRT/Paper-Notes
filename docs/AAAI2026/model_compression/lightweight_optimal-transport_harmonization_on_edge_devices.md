...
### 现有痛点
1. **计算资源要求高**：密集预测模型（encoder-decoder 架构）无法在移动 GPU、XR 头显等边缘设备上实时运行
2. **AR 场景缺乏支持**：ARKit、ARCore 等主流 AR 平台仅依赖光照估计（方向光、环境图、球谐光照），缺少先进的颜色协调
3. **曝光偏差问题**：标准训练数据集（如 iHarmony4）中的 mask 包含边界处的背景像素泄漏，模型会过度依赖这些信息。而 AR 场景中渲染引擎提供像素精确的 mask，导致训练-推理不匹配
4. **AR 评测数据稀缺**：不存在带像素精确 mask 的真实 AR 合成图像数据集
