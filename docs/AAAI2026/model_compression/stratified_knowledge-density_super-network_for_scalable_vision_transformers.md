...
1. **传统剪枝方法**：对每个目标尺寸都需要单独执行剪枝+微调，无法一次性获得多尺度模型
2. **Learngene 范式**（如 TLEG、WAVE）：从预训练模型中提取核心权重并扩展为不同大小的后代模型，但依赖手工设计的扩展规则，且需要额外的知识蒸馏和长时间微调
3. **低秩压缩**：将权重矩阵分解为两个低秩矩阵，但分解后的模型信息保留能力有限
