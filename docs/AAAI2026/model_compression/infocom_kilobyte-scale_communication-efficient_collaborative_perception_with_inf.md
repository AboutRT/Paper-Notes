---
description: "【论文笔记】InfoCom: Kilobyte-Scale Communication-Efficient Collaborative Perception with Information-Aware Feature Compression 论文解读 | AAAI 2026 | arXiv 2512.10305 | 协同感知 | 提出InfoCom框架，基于扩展的信息瓶颈原理将协同感知的通信量从MB级压缩至KB级（相比Where2comm降低440倍），同时保持近无损的感知性能，核心包含信息感知编码、稀疏掩码生成和多尺度解码三个模块。"
tags:
  - AAAI 2026
  - 协同感知
  - 通信效率
...
1. **信息感知编码（Information-Aware Encoding, IAE）**:
   - 做什么：将高维中间特征 $Z \in \mathbb{R}^{N \times C \times H \times W}$ 压缩为极低维的信息感知特征 $E \in \mathbb{R}^{N \times D}$，其中 $D \ll C \times H \times W$
...
2. **稀疏掩码生成（Sparse Mask Generation, SMG）**:
   - 做什么：以极小的通信开销提供关键的空间先验信息，弥补极端压缩下的信息损失
...
3. **多尺度解码（Multi-Scale Decoding, MSD）**:
   - 做什么：在接收端从极度压缩的消息 $\{E, M^q\}$ 渐进式重建可操作的BEV特征
