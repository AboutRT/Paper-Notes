...
### 现有痛点
1. **激活离群值困境**：LLM的激活中存在极端值（outliers），扩大了量化动态范围，导致严重精度下降
2. **平滑方法的本质限制**：SmoothQuant等方法将量化难度从激活转移到权重（通过缩放因子），但这只是"拆东墙补西墙"——权重出现新的离群值和更大的动态范围
3. **旋转方法的开销**：SpinQuant、QuaRot引入旋转层来对齐分布，但带来不可忽略的运行时开销
4. **SVD方法的局限**：SVDQuant用全局低秩近似吸收离群值，但无法捕捉通道级的离群值结构
