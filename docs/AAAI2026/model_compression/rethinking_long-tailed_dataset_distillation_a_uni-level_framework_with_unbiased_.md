...
1. **合成集被头类主导**：尾类表示不足
2. **BN统计量偏倚**：不平衡的类别频率破坏了BN的均值和方差估计
3. **训练不稳定**：中频类接收到不稳定或不充分的梯度反馈

**DAMED的局限（唯一先前工作）**：DAMED是唯一明确针对长尾DD的工作，但存在三个根本问题：
1. **尾类表示不足**：依赖在长尾数据上训练但未去偏的特征提取专家，尾类表示质量差
2. **轨迹匹配的无意权衡**：双层优化中，中频类受到不稳定梯度影响
3. **计算开销大**：双层轨迹优化需要大量GPU内存和时间
