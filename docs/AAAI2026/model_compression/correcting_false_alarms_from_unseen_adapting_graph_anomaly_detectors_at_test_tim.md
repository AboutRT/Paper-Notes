...
1. **领域现状**：图异常检测（GAD）旨在识别图结构数据中的异常节点，GNN-based 方法在监督设定下效果好。但现有方法假设训练和测试分布一致。
...
3. **核心矛盾**：GAD 模型过拟合于训练时见过的正常模式，无法泛化到新的正常类别——但新正常类别的出现在现实中极其普遍。

4. **切入角度**：不修改预训练 GAD 模型，而是在测试时通过特征变换将新正常数据"对齐"回已知分布。关键创新——利用聚合污染本身作为无监督信号来优化对齐器。

5. **核心 idea 一句话**：图对齐器做特征残差修正 + 无聚合的双分支架构估计聚合污染 + 最小化两分支差异作为无监督 TTA 目标。
