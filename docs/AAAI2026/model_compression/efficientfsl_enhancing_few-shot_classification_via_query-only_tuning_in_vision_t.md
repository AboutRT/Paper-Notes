...
### 现有痛点
1. **全参数微调代价高**：ViT-B 有85.8M参数，全微调需要为每个任务存储完整参数副本，GPU 显存和训练时间成本高
2. **PETL方法的耦合问题**：现有参数高效方法（Adapter, LoRA, Prompt Tuning 等）通过在冻结骨干中插入小模块来适配——但这些模块**修改了特征流**并与骨干权重**耦合**，在FSL数据极少的情况下容易过拟合
3. **支持集-查询集分布偏移**：少样本中支持集和查询集的图像在背景、光照、拍摄角度等方面存在差异，导致从支持集计算的原型偏离查询集的分布中心
