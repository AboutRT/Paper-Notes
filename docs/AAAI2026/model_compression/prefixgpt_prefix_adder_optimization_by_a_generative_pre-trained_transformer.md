...
1. **频繁违反设计规则**：迭代修改经常破坏前缀加法器的三条严格设计规则（输入规则、输出规则、合并规则），必须额外引入修复步骤，修复本身又会引入额外硬件开销
2. **强烈的初始化偏差**：方法性能高度依赖初始设计选择（如Sklansky、Kogge-Stone、Brent-Kung），不同初始化下性能差异巨大甚至完全失败
