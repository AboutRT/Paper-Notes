...
**三大结构性缺陷**：
1. **时间稀疏采样**：大多数 VideoLLM 仅均匀采样 8/16/32 帧，大量视频片段未被检查
2. **空间 token 下采样**：激进的 token 压缩（如 14×14 → 7×7）丢失细粒度空间信息
3. **模态融合不平衡**：语言先验在注意力预算中占主导，视觉线索即使被编码器捕获也可能在生成时被忽略
...
1. **帧替换攻击 (Frame-Replacement Attack, FRA)**:
   - 做什么：在视频的随机位置用有害视频片段替换 t_r 秒的原始内容
