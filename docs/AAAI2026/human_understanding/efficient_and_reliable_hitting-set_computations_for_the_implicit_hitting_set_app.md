---
description: "探索隐式击中集(IHS)框架中击中集计算的替代方法，包括伪布尔推理、随机局部搜索及其组合，实现可认证IHS计算"
tags: [combinatorial-optimization, pseudo-boolean, hitting-set, IHS, certifiable-computing]
---
...
**本文要解决什么？** 如何在保持合理性能的同时，为IHS计算提供可认证的正确性保证。

**切入角度** 利用冲突驱动的伪布尔求解器的近期发展，探索基于PB推理的击中集计算替代方案，并与IP求解和随机局部搜索进行混合组合。

**核心idea一句话** 通过PB推理、SLS和IP求解的巧妙组合，实现首个可认证IHS计算框架，在效率损失可控的前提下获得可验证的正确性保证。
...
1. **多种PB优化算法实例化Solve-HS**:
    ...
    - 做什么：用冲突驱动的伪布尔优化替代IP求解器来计算核集合上的最优解
...
2. **混合策略与SLS集成**:
    ...
    - 做什么：将随机局部搜索（SLS）和证明生成PB求解器与IP求解器组合，最小化对证明生成器的调用次数
