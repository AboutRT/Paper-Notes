...
然而，现有方法存在两个核心痛点：
1. **推理幻觉**：VLM 缺乏情绪领域先验知识，容易生成看似合理但实际错误的情绪解释，比如遗漏关键面部特征或对 AU 的误判。
2. **推理与识别错位**：即使模型在推理过程中识别了正确的情绪线索，最终的情绪标签仍可能与推理结论矛盾，因为推理路径与标签之间缺乏内在的因果关联。
...
3. **迭代数据合成**:
   - 做什么：利用前两个阶段训练好的模型自动生成大规模情绪推理数据
