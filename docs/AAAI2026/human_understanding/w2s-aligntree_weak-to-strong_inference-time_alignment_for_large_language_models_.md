...
1. **领域现状**：LLM 对齐主流方法是 RLHF（训练奖励模型 + PPO）和 DPO（直接偏好优化），均在训练阶段通过序列级反馈调整模型参数。
...
3. **核心矛盾**：训练时对齐方法推理时"固化"，无法动态调整。现有推理时方法如 CBS 搜索能力有限。

4. **切入角度**：MCTS 已在 AlphaGo 中证明了大搜索空间中平衡探索-利用的能力；W2SG 表明弱模型可提供有效对齐信号。两者结合可实现无需修改参数的动态推理时对齐。

5. **核心 idea**：构建生成搜索树，弱模型提供步级代理值函数 $V_{\text{proxy}} = \log(\pi_{\text{weak}}^*/\pi_{\text{weak}}^{\text{ref}})$，配合 MCTS 选择-扩展-评估-回传循环引导强模型生成，再通过全局重排选出最优响应。
...
1. **弱到强代理值函数（W2S Proxy Mapping）**:
   - 做什么：将弱模型的对齐信号转化为步级密集反馈，实时评估每步对齐质量
