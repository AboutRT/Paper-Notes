...
1. **标签噪声（Label Noise）**：可用标签通常由启发式规则生成，不可避免地包含噪声，在训练中引入歧义，损害泛化能力
2. **标签稀缺（Label Scarcity）**：地面真值关联极其有限，模型无法建立全面的决策边界
3. **图结构稀疏**：地址关联稀少导致图结构高度稀疏，许多节点孤立，限制了消息传递和特征聚合的效率
