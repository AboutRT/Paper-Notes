...
**本文目标** 回答两个关键问题：(1) 能否为连续推理生成多样化的推理路径？(2) 能否训练有效的PRM/ORM来对这些路径重排？

**切入角度**: 通过dropout注入随机性解决第一个问题，然后适配MATH-Shepherd数据标注框架训练PRM/ORM，最后通过几何分析、轨迹动态和扰动实验深入分析失败原因。

**核心 idea**: 连续推理的inference-time scaling潜力巨大，但连续思维的几何同质性是阻碍奖励模型有效区分的根本瓶颈。
...
1. **Dropout采样生成多样轨迹**
    ...
    - 功能：在连续推理阶段选择性启用dropout，文本生成阶段关闭，为确定性COCONUT注入可控随机性
...
2. **连续空间的MC标注与奖励模型训练**
    ...
    - 功能：适配MATH-Shepherd框架，对每个连续思维向量进行MC标注
