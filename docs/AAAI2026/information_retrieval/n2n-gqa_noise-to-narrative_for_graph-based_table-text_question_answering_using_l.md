...
**本文目标** 在不做任何任务特定训练的前提下，通过图结构组织检索结果来恢复文档间的关系，实现零样本开放域多跳 QA。

**切入角度**：将检索文档建模为图的节点，用 TF-IDF 共享词权重作为边，利用图中心性识别结构上重要的桥接文档。

**核心 idea**：把 RAG 的扁平检索列表转化为结构化证据图，用图中心性剪枝过滤噪声、保留推理链。
...
3. **桥接感知混合选择器**:
    ...
    - 功能：在最终证据聚合阶段识别和优先处理连接表格和文本的桥接文档
