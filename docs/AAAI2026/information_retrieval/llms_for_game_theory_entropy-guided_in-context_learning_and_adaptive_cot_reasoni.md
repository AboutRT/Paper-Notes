---
description: "【论文笔记】LLMs for Game Theory: Entropy-Guided In-Context Learning and Adaptive CoT Reasoning 论文解读 | AAAI 2026 | arXiv 2601.10775 | 大语言模型 | 提出一种基于熵引导的自适应 LLM 推理框架，结合动态上下文检索和自适应链式思维（CoT）推理，在井字棋博弈任务中将 LLM 的平均对局结果从 -11.6% 提升至 +9.5%，同时保持较低的 LLM 查询次数。"
tags:
  - AAAI 2026
  - 大语言模型
  - 博弈论
...
### 现有痛点
1. **CoT 推理效果不稳定**：链式思维提示虽能改善复杂推理，但性能在不同任务和领域间差异很大，且评估指标往往不够精细
2. **计算资源浪费**：Tree-of-Thoughts 等多路径推理方法虽提升性能，但在模型已经自信的情况下仍进行大量计算
3. **缺少理论最优参照**：大多数推理任务缺乏客观最优解，难以量化评估 CoT 推理质量
4. **上下文检索静态**：传统 RAG 固定检索数量，未根据模型不确定性动态调整
