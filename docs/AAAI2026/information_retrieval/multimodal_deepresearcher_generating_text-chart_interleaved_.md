...
**核心 idea**：用 FDV 文本化图表+四阶段 Agent pipeline（搜索→示例文本化→规划→图文生成）+Actor-Critic 渲染精炼，从零生成高质量图文交替研究报告。
