...
**现有解决方案的局限**：
1. **启发式过滤**（SimANS、ADORE）：基于全局统计量（均值、方差）对高相似度负样本降权。但使用**全局阈值**忽略了查询特异性变化——不同查询的负样本分布差异很大，统一过滤可能丢弃有用样本或保留有害样本
2. **交叉编码器过滤**（RocketQA）：用交叉编码器判断假阴性，但计算开销大且仅限于训练阶段
3. **几何约束**（TriSampler）：在查询-正样本-负样本之间加几何约束，改善信息量但仍缺乏假阴性估计
