...
**核心矛盾**：IR 社区缺乏一个能**系统性、细粒度地评估检索器逻辑推理能力**的基准，尤其无法量化否定处理能力。

**本文目标** 创建一个涵盖 14 种一阶逻辑查询类型的 IR 基准，并提出合适的评估指标。

**切入角度**：借助 LLM 生成+子图引导提示设计+专家验证，确保每条查询的逻辑结构精准且有据可查。

**核心 idea**：用子图指示器引导 LLM 从段落中生成特定逻辑结构的查询，构建首个全面的复杂逻辑查询 IR 基准，揭示检索器的逻辑推理短板。
