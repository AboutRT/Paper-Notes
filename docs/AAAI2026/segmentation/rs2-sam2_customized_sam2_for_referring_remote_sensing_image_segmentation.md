...
1. **视觉-语言对齐不足**：SAM2 在遥感场景中效果下降，因为遥感图像的目标可区分性低。现有适配方法如 SAM2-Adapter 仅做单模态调整，缺乏层次化的视觉-语言信息交互，无法细粒度理解文本信息。
2. **缺乏文本引导的提示生成**：SAM2 不具备文本提示集成能力，现有的 EVF-SAM 通过联合编码和 MLP 生成稀疏提示，但稀疏提示在遥感场景中不足以处理细微或不明显的目标，无法提供像素级的精确引导。
