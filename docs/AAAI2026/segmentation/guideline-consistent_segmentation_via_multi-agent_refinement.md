...
1. **标注指南复杂冗长**：如 Waymo 的行人指南包含十多条规则——"骑滑板车的人算行人"、"排除人体模型/雕像/反射"、"持有小于2m物品的行人算单一标签"等
2. **现有方法失败于长文本**：LISA、GroundedSAM 等开放词汇分割方法在短语提示下表现良好，但面对段落级指南时性能急剧下降（LISA-7B 从43.42 gIoU降至23.41）
3. **人工标注也不一致**：Waymo官方GT本身存在违反自身指南的情况（同一场景不同时间戳标注不一致）
4. **重训练不可行**：标注规则可能更新，依赖特定任务重训练不实际
