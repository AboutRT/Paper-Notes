...
### 现有痛点
1. **开放式对话评估困难**：与有客观正确答案的数学任务不同，心理咨询的成功取决于共情参与、伦理遵循、人类偏好等主观因素，缺乏严格的"正确性"标准
2. **MCTS 方法不适配**：现有面向结果的 MCTS 方法以预定义终态为搜索目标，应用于开放式对话时可能产生与人类期望不一致的回复
3. **LLM 原则遵循差**：LLM 往往难以深入理解并持续遵循复杂、抽象的心理咨询标准
4. **真实数据稀缺**：心理咨询的真实对话数据极其稀缺，合成数据质量是关键
5. **缺乏标准化评估**：缺少专门针对多轮心理咨询对话的评估基准
