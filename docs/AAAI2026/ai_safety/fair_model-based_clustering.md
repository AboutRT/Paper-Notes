...
1. **领域现状**：公平聚类要求每个簇中敏感属性（如性别、种族）的比例与整体数据集相似。现有方法大多基于 K-means 聚类，在公平约束下同时优化簇中心和赋值映射。
...
3. **核心矛盾**：如何使公平聚类算法的计算复杂度与样本量解耦，同时保持公平性保证？

4. **本文要解决什么？** 开发参数量独立于样本量的可扩展公平聚类算法。

5. **切入角度**：用概率混合模型替代几何距离，将公平约束从赋值映射转移到模型参数上——参数化的赋值映射天然支持小批量和新数据分配。

6. **核心 idea 一句话**：基于有限混合模型的公平聚类，通过在模型参数上施加 Gap 约束实现参数量与 $N$ 无关的可扩展性。
