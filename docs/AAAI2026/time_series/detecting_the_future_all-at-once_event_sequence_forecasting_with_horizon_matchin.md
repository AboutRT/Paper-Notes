...
1. **自回归方法的退化问题**：传统自回归模型逐步预测下一事件，但随着预测步数增加，输出迅速收敛为常数或重复模式。这是因为模型将自己的预测作为后续输入，误差不断累积放大
2. **逐位置配对损失的缺陷**：现有horizon预测方法（包括GAN和扩散模型）使用逐位置配对（pairwise）损失，将预测事件和真实事件按位置一一对应计算损失。但预测事件的数量和位置可能与真实事件不匹配，导致错误对齐（如图1a所示）
3. **预测多样性不足**：自回归和扩散方法在长程预测中倾向于产生重复的事件类型，无法反映真实的事件分布多样性
