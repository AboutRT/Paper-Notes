...
1. **无法处理时序数据**：传统表格 GAM 假设输入是静态特征向量，不能处理时序序列
2. **无法捕捉时序模式**：NATM 简单地在每个时间步使用独立函数 $f_{i,j}(x_{i,j})$，但各时间步之间没有交互，无法学习跨时间步的依赖关系
3. **固定长度限制**：NATM 需要固定长度输入，无法处理变长时间序列（如医院中患者住院时长不可预知）
