...
description: >-
  [AAAI 2026][LLM Agent][设备-云端协作] 提出 EcoAgent，一个闭环设备-云端协作的多 Agent 移动自动化框架，通过 Dual-ReACT 双层推理规划 + 设备端轻量验证反馈 + Pre-Understanding 文本压缩模块，在 AndroidWorld 上达到与全云端 Agent 相当的成功率，同时大幅降低延迟（3.9s vs 15.3s）、云端调用（降89%）和上行数据量（降48.6倍）。
tags:
  - AAAI 2026
  - LLM Agent
  - 设备-云端协作
...
**代码**: [https://github.com/Yi-Biao/EcoAgent](https://github.com/Yi-Biao/EcoAgent)  
**领域**: Agent / 移动端自动化  
...
## 研究背景与动机
**领域现状**：基于 MLLM 的移动 Agent 主要有三种架构——纯云端（如 M3A，双 GPT-4o）推理能力强但延迟高、成本大；纯设备端（如 ShowUI 2B）延迟低但复杂任务规划能力不足；开环设备-云端协作（如 UGround）云端规划+设备端执行但缺少反馈闭环。

**现有痛点**：(1) 开环协作需要频繁上传手机截图到云端做验证，暴露用户隐私且增加延迟；(2) 一旦执行出错，设备端无法反馈给云端重新规划，只能继续错误执行，无法从失败中恢复。

**核心矛盾**：云端 MLLM 推理能力强但通信成本高+隐私风险大；设备端 MSLM 延迟低+保护隐私但推理能力弱。需要在两者之间找到最佳的任务分配方式。

**本文目标** 如何实现闭环的设备-云端协作——让设备端能自主验证执行结果并在需要时向云端提供轻量反馈，使云端能据此做反思和重规划。

**切入角度**：观察到设备端通用 MSLM（未在大量 GUI 数据上微调的）虽然不擅长精确定位 UI 元素，但擅长理解屏幕语义和判断描述是否匹配——可以用来做轻量级的执行验证。

**核心 idea**：用 Dual-ReACT 让云端一次生成带期望的执行计划，设备端用轻量模型逐步验证并在失败时提供文本化反馈，实现隐私保护的闭环协作。
...
1. **Dual-ReACT 双层推理规划**:
    ...
    - 功能：将传统 ReACT 扩展为全局+局部两层推理，一次性生成完整的可执行计划。
...
2. **Pre-Understanding Module（预理解模块）**:
    ...
    - 功能：将屏幕截图压缩为 50-150 token 的文本描述，替代原始图像传输到云端。
...
3. **Memory + Reflection 反思重规划**:
    ...
    - 功能：执行失败时，Planning Agent 利用存储的屏幕描述轨迹和失败原因进行反思，生成新计划。
