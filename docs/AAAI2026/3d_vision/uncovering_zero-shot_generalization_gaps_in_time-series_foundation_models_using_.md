...
1. **训练数据过度依赖合成增强**：例如 Chronos 使用 KernelSynth 和 TSMixup 生成合成训练数据，这些合成数据能否覆盖真实世界的时序动态存疑
2. **评估数据集多样性不足**：现有 benchmark 多来自金融、能源、交通等传统领域，分布单一。M4 数据集仅 5% 的序列是平稳的，多样性有限
3. **传感器数据和股价数据已被广泛研究**：缺乏来自全新来源的时间序列数据来测试模型的真正泛化能力
...
**做什么**：对每个跟踪点，先前向跟踪再后向跟踪，检查是否回到原始位置。
...
**做什么**：计算同一视频中所有轨迹的互相关，保留 5 条相关性最低的。
