...
1. **PointGoal 导航**：给出目标相对方向和距离，缺乏中间引导
2. **ImageGoal 导航**：提供目标位置图像，但缺少中间导航线索
3. **视觉语言导航（VLN）**：用自然语言描述导航路径，目前最活跃
...
1. **高用户可达性**：非专业用户可通过点击或绘制轨迹自然地指定导航目标
2. **丰富的空间信息**：俯视图天然保留完整空间布局
3. **高复用性**：俯视图通过无人机航拍或 3D 重建获取，构建一次即可重复使用
