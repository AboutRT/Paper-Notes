...
1. **固定消息传播深度问题**：现有 GNN 采用固定的消息传播层数，每个顶点只能感知预设跳数范围内的邻居。在细密网格上，固定深度导致感受野覆盖不足；在粗糙网格上则会产生过度平滑。
2. **位移幅度分辨率相关性**：在更高分辨率的网格中，相同的全局运动被分配到更多的顶点上，导致每个顶点的位移幅度变小。这种内在的分辨率依赖性使得在低分辨率上训练的模型在高分辨率上会过估位移。
