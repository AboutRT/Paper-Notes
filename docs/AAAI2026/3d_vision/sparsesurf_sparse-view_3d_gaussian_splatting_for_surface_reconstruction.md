...
**扁平化高斯与过拟合的矛盾**：为了更好贴合表面几何，FatesGS等方法将高斯压扁为2D平面。但扁平化带来的**高各向异性**在稀疏视角下会**加剧过拟合**风险——虽然在训练视角上看起来正常，但在新视角上会出现严重退化

**单目深度先验的尺度模糊问题**：现有方法常用单目深度估计器提供几何约束，但单目深度存在**尺度模糊**且缺乏置信度估计，在稀疏视角下容易引入噪声，导致多视角不一致
