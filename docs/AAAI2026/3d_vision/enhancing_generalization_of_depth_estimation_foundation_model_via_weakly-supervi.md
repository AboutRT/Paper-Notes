...
1. **领域现状**：Depth Anything 系列等基础模型通过大规模数据训练实现了优秀的零样本单目深度估计泛化能力。但在面对下游任务的分布偏移时（如恶劣天气、传感器噪声、域外场景），性能仍有提升空间。
...
3. **核心矛盾**：如何在利用下游数据提升性能的同时，保持模型的泛化能力不被破坏？

4. **本文要解决什么？** 设计一个参数高效且鲁棒的自适应框架，在少量目标域数据（无标注或弱标注）下增强深度基础模型的泛化性。

5. **切入角度**：三管齐下——自训练提供密集结构监督 + 弱监督提供稀疏但独立的序数约束打破确认偏差 + 权重正则化锚定预训练知识防遗忘。

6. **核心 idea 一句话**：用语义感知的分层归一化自训练 + 低成本成对序数弱监督 + LoRA 正则化三者协同，安全地适应深度基础模型到新域。
...
1. **语义感知分层深度归一化（SA-HDN）**:
   - 做什么：解决自训练中 teacher 伪标签和 student 预测之间的尺度/偏移歧义
