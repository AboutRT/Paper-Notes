...
1. **粗到细范式**（PCN、FoldingNet）：全局形状先验 → 细化
2. **Transformer 方法**（PoinTr、SeedFormer、CRA-PCN）：捕获长程依赖，当前主流
3. **对称先验方法**（SymmCompletion）：利用对称性学习逐点局部仿射变换
4. **扩散方法**（PDR、PCDreamer）：在点坐标空间做扩散

### 现有痛点

作者聚焦于**利用对称先验的方法**（如 SymmCompletion），指出其两个关键缺陷：

1. **过拟合**：回归方式倾向于记忆训练集中实例特定的变换模式，而非学到可泛化的几何对齐规则。在跨域场景（如 KITTI 真实数据）上泛化能力差。
2. **噪声敏感**：逐点独立回归变换，对遮挡和噪声高度敏感，导致全局结构碎片化或扭曲。
