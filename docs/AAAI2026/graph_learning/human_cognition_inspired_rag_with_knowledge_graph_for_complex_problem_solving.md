...
1. **先规划再执行**：在检索前先构建完整的推理计划（思维导图），而非边检索边推理
2. **一次性检索而非迭代检索**：基于全局思维导图一次性检索所有需要的知识
3. **双 LLM 验证**：推理 LLM 和验证 LLM 分工合作，模拟人类的自我反思
...
**做什么**：将原始复杂问题递归分解为层次化的语义连贯子问题，形成思维导图 $\mathcal{M}$。
...
**做什么**：基于思维导图从知识图谱 $\mathcal{G}$ 中检索支持推理的外部知识。
...
**做什么**：从叶节点开始自底向上回答子问题，每个答案由验证 LLM 审核，错误答案重新生成。
