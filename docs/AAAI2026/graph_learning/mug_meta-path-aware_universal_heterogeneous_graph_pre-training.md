...
1. **输入统一困难**：不同异质图的节点类型、关系类型、属性维度各不相同，无法直接构建统一的表示空间。现有 UGP 方法假设固定的实体和关系类型集合，在异质图上直接失效。
2. **学习信息难以迁移**：传统异质图方法（如 HAN、HeCo、HGMAE）设计了类型特定的编码器和元路径特定的聚合权重，这些参数与特定数据集紧密耦合，无法泛化到新的异质图。
