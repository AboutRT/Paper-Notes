...
### 现有痛点
1. **传统蒸馏成本高**：需要分别训练教师和学生模型，成本随模型数量线性增加
2. **固定推理成本**：标准模型必须经过所有层才能输出，无法根据任务难度调整计算量
3. **中间层价值未被利用**：SimCLR 和 Valeriani 等研究表明中间层而非最终层往往持有最语义丰富的表示
4. **NSP 损失的局限**：传统 Next Sentence Prediction 损失微调后几乎无益，且对代码输入的长上下文利用效率低
