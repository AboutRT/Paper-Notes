...
1. **k-means 系列**：需要预指定聚类数 k，对初始化敏感，在高维和含噪数据上退化
2. **凸聚类（Convex Clustering）**：通过凸优化保证全局最优解，但在高维含噪数据上面临困难——强融合正则化（大的 γ 值）会导致异常值与真实簇不当合并
3. **鲁棒性缺口**：现有凸聚类方法（如 Robust Convex Clustering）在权重分配上依赖 k-NN + 高斯核，任意选择带宽 ϕ 可能导致簇坍塌或任意簇形成
