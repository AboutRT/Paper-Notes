...
1. **设计偏差**：这些函数本质上是为区分传统优化算法的性能而手工设计的，而非为训练 MetaBBO 的泛化策略量身定制
2. **多样性不足**：24 个基础函数的覆盖范围有限，容易导致 MetaBBO 过拟合，在未见问题上泛化性差
3. **现有替代方案不理想**：
...
**做什么**：将 BBO 问题实例的高维 ELA (Exploratory Landscape Analysis) 特征映射到二维潜在空间，便于度量多样性和均匀采样。
...
**做什么**：设计表达力强的符号集，利用遗传编程 (GP) 搜索与目标潜在点最匹配的函数公式。
...
**做什么**：对每个后代符号树，在 2D、5D、10D 三个维度上实例化，选择目标值最优的维度。
